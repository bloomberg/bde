...
//            `blp_common64.h` header, as well as renamed them to
//            `is_integer64`, and `mantissa_counter64` respectively.  Also
//            move the `DOUBLE_MANTISSA_BITS`, `DOUBLE_EXPONENT_BITS`, and
...
int blp_d2m_buffered_n(double f, char* result) {
  ...
  if (needsDecimalNotaton && is_integer64(ieeeMantissa, ieeeExponent)) {
...
int blp_d2d_buffered_n(double f, char* result) {
  ...
  // Integers are printed precisely in decimal notation
  if (is_integer64(ieeeMantissa, ieeeExponent)) {
    // The following call re-does some of the work we did here, but not enough
    // of it to worry.
    return ryu_d2fixed_buffered_n(f, 0, result);
  }
