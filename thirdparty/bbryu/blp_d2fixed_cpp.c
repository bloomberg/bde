...
static inline int copy_special_str_printf_safe(char* const result, char * const end, const bool sign, const uint64_t mantissa) {
  if (end - result < 3 + sign) {
    return 0;
  }
  ...
  memcpy(result + sign, mantissa?"nan":"inf", 3);
  return sign + 3;
}



#define BLP_SAFE_APPEND(count, ...)             \
    do {                                         \
        if (end - result < (count)) { return 0; } \
        do { __VA_ARGS__; } while (false);         \
        result += (count);                          \
    } while(false)


int blp_d2fixed_buffered_n_safe(double d, uint32_t precision, char* result, char *end) {
  char * const origStart = result;
  ...
  if (ieeeExponent == ((1u << DOUBLE_EXPONENT_BITS) - 1u)) {
    return copy_special_str_printf_safe(result, end, ieeeSign, ieeeMantissa);
  }
  if (ieeeExponent == 0 && ieeeMantissa == 0) {
    const int bytesRequired = ieeeSign + 1 + (precision > 0) * (precision + 1);
    if (end - result < bytesRequired) {
      return 0;
    }
    if (ieeeSign) {
      *result++ = '-';
  ...
  if (e2 >= -(DOUBLE_MANTISSA_BITS)) {
    ...
    for (int32_t i = len - 1; i >= 0; --i) {
      ...
      if (nonzero) {
        BLP_SAFE_APPEND(9, append_nine_digits(digits, result));
      } else if (digits != 0) {
        const uint32_t olength = decimalLength9(digits);
        BLP_SAFE_APPEND(olength, append_n_digits(olength, digits, result));
        nonzero = true;
      }
    }
  }
  if (!nonzero) {
    BLP_SAFE_APPEND(1, *result = '0');
  }
  if (precision > 0) {
    BLP_SAFE_APPEND(1, *result = '.');
  }
  ...
  if (e2 < 0) {
    ...
    if (blocks <= MIN_BLOCK_2[idx]) {
      i = blocks;
      BLP_SAFE_APPEND(precision, memset(result, '0', precision));
    } else if (i < MIN_BLOCK_2[idx]) {
      i = MIN_BLOCK_2[idx];
      BLP_SAFE_APPEND(9 * i, memset(result, '0', 9 * i));
    }
    for (; i < blocks; ++i) {
      ...
      if (p >= POW10_OFFSET_2[idx + 1]) {
        ...
        BLP_SAFE_APPEND(fill, memset(result, '0', fill));
        break;
      }
      // Temporary: j is usually around 128, and by shifting a bit, we push it to 128 or above, which is
      // a slightly faster code path in mulShift_mod1e9. Instead, we can just increase the multipliers.
      ...
      if (i < blocks - 1) {
        ...
        if (maximum > 0) {
          BLP_SAFE_APPEND(maximum, append_c_digits(maximum, digits, result));
        }
        break;
      }
    }
...
#endif
    if (roundUp != 0) {
      int roundIndex = result - origStart;
      int dotIndex = 0; // '.' can't be located at index 0
      while (true) {
        --roundIndex;
        char c;
        if (roundIndex == -1 || (c = origStart[roundIndex], c == '-')) {
          origStart[roundIndex + 1] = '1';
          if (dotIndex > 0) {
            origStart[dotIndex] = '0';
            origStart[dotIndex + 1] = '.';
          }
          BLP_SAFE_APPEND(1, *result = '0');
          break;
        }
  ...
  return result - origStart;
}

int blp_d2exp_buffered_n_safe(double d, uint32_t precision, char* result, char* end)
{
  char* const origStart = result;
  ...
  if (ieeeExponent == 0 && ieeeMantissa == 0) {
    ...
    if (precision > 0) {
      BLP_SAFE_APPEND(precision + 1,
                      *result = '.'; memset(result + 1, '0', precision));
    }
    BLP_SAFE_APPEND(4, memcpy(result, "e+00", 4));
    return result - origStart;
  }
    ...
    for (int32_t i = len - 1; i >= 0; --i) {
      ...
      if (printedDigits != 0) {
        ...
        if (printDecimalPoint) {
          BLP_SAFE_APPEND(availableDigits + 1, // +1 for decimal point
                          append_d_digits(availableDigits, digits, result));
        } else {
          BLP_SAFE_APPEND(1, *result = (char)('0' + digits));
        }
  ...
  if (printedDigits != 0) {
    BLP_SAFE_APPEND(maximum,
                    if (digits == 0) {
                      memset(result, '0', maximum);
                    } else {
                      append_c_digits(maximum, digits, result);
                    });
  } else {
    if (printDecimalPoint) {
      BLP_SAFE_APPEND(maximum + 1, // +1 for decimal point
                      append_d_digits(maximum, digits, result));
    } else {
      BLP_SAFE_APPEND(1, *result = (char)('0' + digits));
    }
  }
  ...
  if (exp >= 100) {
    ...
    BLP_SAFE_APPEND(3,
                    memcpy(result, DIGIT_TABLE + 2 * (exp / 10), 2);
                    result[2] = (char) ('0' + c));
  } else {
    BLP_SAFE_APPEND(2, memcpy(result, DIGIT_TABLE + 2 * exp, 2));
  }

  return result - origStart;
}

#undef BLP_SAFE_APPEND
