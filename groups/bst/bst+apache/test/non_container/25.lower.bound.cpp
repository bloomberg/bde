...
#if DRQS // stlport's lower_bound assumes that ForwardIterator::value_type is the same as T.  Also, __Less_2 takes a by-value second arg (should be const ref).
        : std::lower_bound (first, last, value);
#else
        : std::lower_bound (first, last, value.xval_);
#endif
