...
class BerEncoderOptions {
    ...
    bool  d_encodeArrayLengthHints;
        // This *backward*-*compatibile* option controls whether or not array
        // length hints are encoded.  For backward-compatibility purposes, the
        // default value of this option is 'false'.  Setting this option to
        // 'true' requires the receiving decoder to come from BDE release
        // '4.27.x' or later, or have the 'SkipUnknownElements' decoder option
        // set to 'true'.

    // PRIVATE ACCESSORS
    bool isEqualTo(const BerEncoderOptions& rhs) const;
  ...
  public:
    ...
    enum {
      ...
      , ATTRIBUTE_ID_ENCODE_ARRAY_LENGTH_HINTS            = 7
    };
    ...
    enum {
      ...
      , ATTRIBUTE_INDEX_ENCODE_ARRAY_LENGTH_HINTS            = 7
    };
  ...
  public:
    ...
    static int maxSupportedBdexVersion(int versionSelector = 0);
        ...
        // this class and the specified 'versionSelector'.
        // See the 'bslx' package-level documentation for more
    ...
    void setEncodeArrayLengthHints(bool value);
        // Set the "EncodeArrayLengthHints" attribute of this object to the
        // specified 'value'.
    ...
    bool encodeArrayLengthHints() const;
        // Return the value of the "EncodeArrayLengthHints" attribute of this
        // object.

    // HIDDEN FRIENDS
    friend bool operator==(const BerEncoderOptions& lhs,
...
// PRIVATE ACCESSORS
inline
bool BerEncoderOptions::isEqualTo(const BerEncoderOptions& rhs) const
{
    return this->traceLevel() == rhs.traceLevel() &&
           this->bdeVersionConformance() == rhs.bdeVersionConformance() &&
           this->encodeEmptyArrays() == rhs.encodeEmptyArrays() &&
           this->encodeDateAndTimeTypesAsBinary() == rhs.encodeDateAndTimeTypesAsBinary() &&
           this->datetimeFractionalSecondPrecision() == rhs.datetimeFractionalSecondPrecision() &&
           this->disableUnselectedChoiceEncoding() == rhs.disableUnselectedChoiceEncoding() &&
           this->preserveSignOfNegativeZero() == rhs.preserveSignOfNegativeZero() &&
           this->encodeArrayLengthHints() == rhs.encodeArrayLengthHints();
}
...
// MANIPULATORS
template <typename t_STREAM>
t_STREAM& BerEncoderOptions::bdexStreamIn(t_STREAM& stream, int version)
{
    if (stream) {
...
#pragma GCC diagnostic ignored "-Wmaybe-uninitialized"
        ...
        switch (version) {
          case 3: {
            ...
            bslx::InStreamFunctions::bdexStreamIn(stream, d_encodeArrayLengthHints, 1);
          } break;
          case 2: {
            bslx::InStreamFunctions::bdexStreamIn(stream, d_traceLevel, 1);
            bslx::InStreamFunctions::bdexStreamIn(stream, d_bdeVersionConformance, 1);
            ...
            bslx::InStreamFunctions::bdexStreamIn(stream, d_disableUnselectedChoiceEncoding, 1);
            bslx::InStreamFunctions::bdexStreamIn(stream, d_preserveSignOfNegativeZero, 1);
            d_encodeArrayLengthHints = false;
          } break;
          case 1: {
            reset();
...
template <typename t_MANIPULATOR>
int BerEncoderOptions::manipulateAttributes(t_MANIPULATOR& manipulator)
{
    int ret;
    ...
    ret = manipulator(&d_encodeArrayLengthHints, ATTRIBUTE_INFO_ARRAY[ATTRIBUTE_INDEX_ENCODE_ARRAY_LENGTH_HINTS]);
    if (ret) {
        return ret;
    }
...
template <typename t_MANIPULATOR>
int BerEncoderOptions::manipulateAttribute(t_MANIPULATOR& manipulator, int id)
{
    enum { NOT_FOUND = -1 };
    ...
    switch (id) {
      ...
      case ATTRIBUTE_ID_ENCODE_ARRAY_LENGTH_HINTS: {
        return manipulator(&d_encodeArrayLengthHints, ATTRIBUTE_INFO_ARRAY[ATTRIBUTE_INDEX_ENCODE_ARRAY_LENGTH_HINTS]);
      }
      default:
        return NOT_FOUND;
    }
...
inline
void BerEncoderOptions::setEncodeArrayLengthHints(bool value)
{
    d_encodeArrayLengthHints = value;
}

// ACCESSORS
template <typename t_STREAM>
t_STREAM& BerEncoderOptions::bdexStreamOut(t_STREAM& stream, int version) const
{
    switch (version) {
      case 3: {
        ...
        bslx::OutStreamFunctions::bdexStreamOut(stream, this->encodeArrayLengthHints(), 1);
      } break;
      case 2: {
        bslx::OutStreamFunctions::bdexStreamOut(stream, this->traceLevel(), 1);
        bslx::OutStreamFunctions::bdexStreamOut(stream, this->bdeVersionConformance(), 1);
...
template <typename t_ACCESSOR>
int BerEncoderOptions::accessAttributes(t_ACCESSOR& accessor) const
{
    int ret;
    ...
    ret = accessor(d_encodeArrayLengthHints, ATTRIBUTE_INFO_ARRAY[ATTRIBUTE_INDEX_ENCODE_ARRAY_LENGTH_HINTS]);
    if (ret) {
        return ret;
    }
...
template <typename t_ACCESSOR>
int BerEncoderOptions::accessAttribute(t_ACCESSOR& accessor, int id) const
{
    enum { NOT_FOUND = -1 };
    ...
    switch (id) {
      ...
      case ATTRIBUTE_ID_ENCODE_ARRAY_LENGTH_HINTS: {
        return accessor(d_encodeArrayLengthHints, ATTRIBUTE_INFO_ARRAY[ATTRIBUTE_INDEX_ENCODE_ARRAY_LENGTH_HINTS]);
      }
      default:
        return NOT_FOUND;
    }
...
inline
bool BerEncoderOptions::encodeArrayLengthHints() const
{
    return d_encodeArrayLengthHints;
}
