...
// +--------------------------------+
// ```
// * HEADER -- The header is 44 bytes that describe the file.
// * TRANSITION TIMES -- `numTransitions` number of 4-byte big-endian signed
...
// * TRANSITION-TIME INDEXES -- `numTransitions` number of unsigned bytes.
...
//   transitions.
// * LOCAL-TIME TYPES -- An array of `numLocalTimeTypes` local-time types,
//   where each entry consist of:
//   - gmtOffset -- 4-bytes big-endian signed integer indicating the number of
//     seconds added to UTC to compute the local time in the time zone
//   - isDst -- 1-byte boolean indicating whether the local-time type is
//     considered daylight saving time
//   - abbreviationIndex -- 1 unsigned byte indicating the index of the first
...
// * TIME-ZONE ABBREVIATION STRINGS -- `numAbbrevChar` number of bytes
...
// * LEAP SECONDS -- `numLeaps` number of *pairs* of 4-byte signed big-endian
...
//   information.)
// * IS_GMT(s) -- `numIsGmt` number of 1-byte booleans indicating whether the
...
// * IS_STD(s) -- `numIsStd` number of 1-byte booleans indicating whether the
//   transition time for the local-time type with the corresponding index in
...
//
// * http://www.iana.org/time-zones -- Central links for Zoneinfo time-zone DB
...
// * https://github.com/eggert/tz -- Unofficial GitHub repository for unstable
