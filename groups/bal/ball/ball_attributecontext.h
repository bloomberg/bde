...
// category using those stored attributes.  The 'ball::AttributeContext' class
// is statically initialized (using the 'initialize()' method) with a
// 'ball::CategoryManager' object containing a 'ball::RuleSet' the represents
...
// methods.  Finally the 'ball::AttributeContext' provides methods (used
...
// current thread's attributes (i.e., 'ball::Rule::evaluate()' returns 'true'
// for the collection of attributes maintained for the current thread by the
// thread's 'ball::AttributeContext' object).  The 'determineThresholdLevels()'
...
// the singleton 'ball::LoggerManager' object when the logger manager is
// initialized.
//
///Usage
///-----
...
class AttributeContext_RuleEvaluationCache {
    // This is an implementation type of 'AttributeContext' and should
    ...
    // 'AttributeContainerList' object).  The rules this cache evaluates
    // are contained in a 'RuleSet' object.  The 'RuleSet::MaskType'
    ...
    RuleSet::MaskType d_resultMask;      // set of bits, each of which
  ...
  public:
    ...
    RuleSet::MaskType update(
        ...
        // are satisfied by 'attributes' (i.e., if 'Rule::evaluate()'
...
class AttributeContext {
    ...
    // category using those stored attributes.  The 'AttributeContext'
    // class is statically initialized (using the 'initialize()' method) with
    // a 'CategoryManager' object containing a 'RuleSet' the
    ...
    // 'AttributeContext' provides methods (used primarily by other
    ...
    // attributes (i.e., 'Rule::evaluate()' returns 'true' for the
    // collection of attributes maintained for the current thread by the
    // thread's 'AttributeContext' object).  The
    ...
    // 'LoggerManager' object when the logger manager is initialized.

    // PRIVATE TYPES
    typedef AttributeContext_RuleEvaluationCache RuleEvaluationCache;

    // CLASS DATA
    static CategoryManager *s_categoryManager_p;  // holds the rule set,
    ...
    static void removeContext(void *arg);
        // Destroy the 'AttributeContext' object pointed to by the
    ...
    AttributeContext(bslma::Allocator *globalAllocator = 0);
        // Create a 'AttributeContext' object initially having no
  ...
  public:
    ...
    static void initialize(CategoryManager *categoryManager,
        ...
        // Initialize the static data members of 'AttributeContext' using
        ...
        // be called *automatically* when the 'LoggerManager' is
    ...
    void removeAttributes(iterator element);
        ...
        // invoked safely, even if 'AttributeContext::initialize()' has
        // not been called.
    ...
    bool hasRelevantActiveRules(const Category *category) const;
        ...
        // current thread's attributes (i.e., 'Rule::evaluate()' returns
        ...
        // unless 'AttributeContext::initialize()' has previously been
        // invoked.
...
class AttributeContextProctor {
   ...
   // has been created), and set the thread local storage pointer to 0.
    ...
    AttributeContextProctor& operator=(
                                          const AttributeContextProctor&);
  public:
    // CREATORS
    explicit AttributeContextProctor();
        // Create a 'AttributeContextProctor' object, which will destroy
...
AttributeContext::addAttributes(
