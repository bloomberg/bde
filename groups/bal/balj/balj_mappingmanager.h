// balj_mappingmanager.h                                             -*-C++-*-
#ifndef INCLUDED_BALJ_MAPPINGMANAGER
#define INCLUDED_BALJ_MAPPINGMANAGER
...
//
//@CLASSES:
//      balj::MappingManager: manager of mmap pages.
//
...
///THREAD SAFETY
///-------------
// 'balj::MappingManager' is thread-safe, meaning that multiple threads may
...
//      NUM_PRIORITIES = 1
//  };
//  balj::MappingManager mappingManager(MAPPING_LIMIT, NUM_PRIORITIES);
//..
...
//..
//  balj::MappingManager::PageListHandle pageList
//                                           = mappingManager.createPageList();
//  assert(pageList);
//..
// Next, assuming that the pages are stored in a file with descriptor 'fd',
// add a few pages to this manager and associate each page with 'pageList'
// (we can use bdlsu::MemoryUtil to figure out platform-dependent page size):
//..
//  enum { NUM_PAGES = 100 };
//  const int PAGE_SIZE = bdlsu::MemoryUtil::pageSize();
//  balj::MappingManager::Handle pageHandles[NUM_PAGES];
//  for (int i = 0; i < NUM_PAGES; ++i)
//  {
//     pageHandles[i] = mappingManager.addPage(fd,
...
namespace balj {struct MappingManager_Page;
class MappingManager;

class MappingManager_PageList {
  ...
  private:
    // CREATORS
    explicit MappingManager_PageList(PageListIndex index);

  private:
    // NOT DEFINED for the sake of bsl::vector (and other evangelists)
    // MappingManager_PageList(const MappingManager_PageList&);

    // MappingManager_PageList&
    //     operator=(const MappingManager_PageList&);
...
class MappingManager {
  public:
    typedef MappingManager_Page          *Handle;
    typedef MappingManager_PageList      *PageListHandle;

    static const Handle INVALID_HANDLE;

    BSLALG_DECLARE_NESTED_TRAITS(MappingManager,
                                 bslalg::TypeTraitUsesBslmaAllocator);
  private:
    // PRIVATE TYPES
    typedef MappingManager_PageList PageList;
    typedef MappingManager_Page     Page;
    ...
    MappingManager& operator=(const MappingManager&);
  public:
    // CREATORS
    explicit MappingManager(bsl::size_t       mappingLimit,
                                  int               numPriorityLevels,
                                  bslma::Allocator *basicAllocator = 0);
        // Create an empty 'MappingManager' with the specified
    ...
    ~MappingManager();
        // Destroy this 'MappingManager'.  Note that all outstanding
    ...
    Handle addPage(bdlsu::FilesystemUtil::FileDescriptor   fileDescriptor,
        ...
        // this 'MappingManager'.  The page will be mapped read/write if
    ...
    void removePage(Handle pageHandle);
        ...
        // 'MappingManager'.  The behavior is undefined unless
    ...
    char *usePage(Handle pageHandle, bool isDirty = false);
        ...
        // The 'MappingManager' will try to prevent the total size of
...
inline
void MappingManager::setMappingLimit(size_t numBytes)
{
    d_mappingLimit = numBytes;
}

inline
bsl::size_t MappingManager::mappingLimit() const
{
    return d_mappingLimit;
}

inline
int MappingManager::mapCount() const
{
    return d_mapCount;
}

inline
int MappingManager::numPriorityLevels() const
{
