...
void getTmpFileName(char* filename, int testCase) {
   ...
   if (envPrefix) {
       snprintf(filename, MAX_TMPFILENAME,
                "%s%s%d",
                envPrefix, randLetters, bdlsu::ProcessUtil::getProcessId());
   } else {
#ifdef BSLS_PLATFORM_OS_WINDOWS
...
{
    bcecs::PersistentJournal     d_journal;
...
void OldJournalWrapper::reopen()
{
    ...
    int rc = d_journal.openJournal(d_filename.c_str(),
            bcecs::PersistentJournal::READ_WRITE);
    ASSERT(rc == 0);
}
...
{
    bcecs::PersistentJournalIterator it(&d_journal, record);
...
class NewJournalWrapper : public BaseJournal {
    balj::MappingManager                         d_mappingManager;
    balj_Journal                                d_journal;
    bsl::string                                  d_filename;
    int                                          d_mode;
    enum {
...
void caseNeg10Test(balj_Journal *mX, int numRecords, int recordSize)
{
    char *buffer =
       (char*)bslma::Default::allocator()->allocate(
                                                 recordSize * mX->blockSize());
    bsl::memset(buffer, 0xAB, recordSize * mX->blockSize());
    bsl::vector<balj_Journal::RecordHandle> handles;
...
int Case16::run(double commitInterval)
{
    ...
    d_queue_p = new bdlcc::FixedQueue<Case16_Record>(QUEUE_SIZE);

    if (0 < commitInterval) {
        d_commitHandle =
            d_eventScheduler.startClock(bsls::TimeInterval(commitInterval),
                                    bdlf::BindUtil::bind(
    ...
    for(int i=0; i<NUM_CONSUMERS; ++i) {
        consumerThreads.addThread(bdlf::BindUtil::bind(
...
int main(int argc, char *argv[]) {
    ...
    ball::DefaultObserver observer(bsl::cerr);
...
//             for(int i=0; i<NUM_ITERATIONS; i++)
      ...
      case 15: {
          ...
          static const int DEF_ByPB =
             balj::JournalParameters::DEFAULT_BLOCK_SIZE;
          static const int DEF_BlPB =
             balj::JournalParameters::DEFAULT_BLOCKS_PER_PAGE;
          balj::MappingManager mappingManager(MAPPING_LIMIT, 2);
      ...
      case 11: {
          ...
          param.blockSize() = balj::JournalParameters::DEFAULT_BLOCK_SIZE;
      ...
      case 8: {
          ...
          const int USER_DATA_SIZE =
                     (balj::JournalParameters::DEFAULT_BLOCK_SIZE *
                      balj::JournalParameters::DEFAULT_BLOCKS_PER_PAGE * 5)/2;
      ...
      case -7: {
          ...
          int loopStart =
                  balj::JournalParameters::DEFAULT_BLOCK_SIZE *
                  balj::JournalParameters::DEFAULT_BLOCKS_PER_PAGE - 4;
...
//             ASSERT(0 != mX.removeRecord(handle));
      ...
      case 1: {
        ...
        balj::MappingManager mappingMgr(MAPPING_LIMIT, NUM_PRIO);
