// balj_journal.h                                                    -*-C++-*-
#ifndef INCLUDED_BALJ_JOURNAL
#define INCLUDED_BALJ_JOURNAL
...
//
//@CLASSES:
//  balj_Journal: persistent journal backed by a file
//
...
// 32-bit processes.  The 'balj_Journal' class maps pages automatically as
// needed using an externally-pluggable mapping manager (see the
// 'balj_mappingmanager' component).  The mapping manager can be shared
...
///Thread-Safety
///-------------
// 'balj_Journal' is thread-safe and thread-aware; that is, multiple threads
...
// 'balj_Journal' object concurrently from multiple threads only after the
...
// the 'balj_Journal' object should be shared between the threads instead.
//
///Usage
///-----
...
// by 'balj_Journal' as the second argument to the constructor of the mapping
// manager:
//..
...
namespace balj {class JournalLockGuard;
class JournalReadLockGuard;
class JournalPageHeader;
}  // close package namespace
class balj_Journal_BlockRef;


namespace balj {struct Journal_SegmentHeader;
}  // close package namespace
...
class balj_Journal {
    ...
    // FRIENDS
    friend class balj_Journal_PageHeaderReadGuard;
    friend class balj_Journal_PageHeaderWriteGuard;
    friend class balj_Journal_LockGuard;
    friend class balj_Journal_ReadLockGuard;
    friend class balj_Journal_BlockRef;

    typedef balj::MappingManager::Handle PageHandle;
  ...
  private:
    ...
    typedef bdlsu::FileUtil::FileDescriptor FileDescriptor;
    typedef bdlsu::FileUtil::Offset         Offset;
    ...
    bdlsu::FileUtil::Offset    d_fileSize;

    FileDescriptor            d_fd;

    bsl::string               d_filename;

    balj::JournalHeader       d_header;
    void                     *d_headerPage;
    ...
    bdlsu::FileUtil::Offset    d_diskSpaceWarningThreshold;
    ...
    balj::Journal_SegmentHeader* segmentHeaderForBlock(unsigned page,
                                                       unsigned block,
                                                       bool     isDirty) const;
    ...
    bdlsu::FileUtil::Offset getFileSize(FileDescriptor fd);

    balj::JournalPageHeader* getAvailablePageHeader(
                                      unsigned *page, unsigned numPages);

    void addRecordToList(balj::JournalHeaderRecordList* list, unsigned handle);
    void removeRecordFromList(balj::JournalHeaderRecordList* list,
                              unsigned handle);

    void addPageToList(balj::JournalHeaderPageList* list, unsigned page);
    ...
    // not implemented
    balj_Journal(const balj_Journal&);
    balj_Journal& operator=(const balj_Journal&);

  public:
    // CREATORS
    balj_Journal(balj::MappingManager *mappingManager,
    ...
    void setDiskSpaceLoggingThresholds(bdlsu::FileUtil::Offset warningThreshold,
    ...
    bdlt::Datetime journalCreationDatetime() const;
        // Return the journal creation datetime.

    bdlt::Datetime journalModificationDatetime() const;
    ...
    bdlt::Datetime getRecordCreationDatetime(RecordHandle handle) const;
    ...
    static const char* errorMessage(int errorCode);
        ...
        // 'balj_Journal', or a string "unknown error" if the 'errorCode'
        // is not recognized.
};
...
inline
int balj_Journal::copyRecordData(char               *buffer,
                                  unsigned            size,
                                  RecordHandle        handle) const
{
...
inline
int balj_Journal::setRecordAttributes(RecordHandle handle,
                                       unsigned attributes)
{
...
// ACCESSORS
inline
const char *balj_Journal::filename() const {
    return d_filename.c_str();
}

inline
unsigned balj_Journal::mode() const {
    return d_mode;
}
