...
BSLMF_ASSERT(bslma::UsesBslmaAllocator<Option>::value);
BSLMF_ASSERT(bdlb::HasPrintMethod<Option>::value);
    ...
    // first unsuccessful parsing position.
{
    ...
    if (Parser::skipRequiredToken(&input, input, "NON_OPTION")) {
        if (Parser::skipRequiredToken(&input, input, "TAG")
         || Parser::skipWhiteSpace   (&input, input)
         || local_TypesParser::parseQuotedString(&tagString, &input, input)
    ...
    if (Parser::skipWhiteSpace   (&input, input)
     ...
     || Parser::skipWhiteSpace   (&input, input)
     || local_TypesParser::parseQuotedString(&nameString, &input, input)
    ...
    if (Parser::skipWhiteSpace   (&input, input)
     ...
     || Parser::skipWhiteSpace   (&input, input)
     || local_TypesParser::parseQuotedString(&descriptionString, &input, input)
...
// BDE_VERIFY pragma: -IND01   // Possibly mis-indented line
        ...
        for (int m = 0; m < NUM_OPTION_OCCURRENCES;  ++m) {
            ...
            const TypeInfo       EXP_TYPEINFO        = createTypeInfo(
                                                                   TYPE,
                                                                   VARIABLE,
                                                                   CONSTRAINT);
...
// BDE_VERIFY pragma: -IND01   // Possibly mis-indented line
        ...
        for (int m = 0; m < NUM_OPTION_OCCURRENCES;  ++m) {
            ...
            int parseRet = parseOption(&output, X, output);
            LOOP4_ASSERT(X, ss1, output, parseRet, 0 == parseRet);

            output = ss3.c_str();

            parseRet = parseOption(&output, X, output);
