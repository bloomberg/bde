      ...
      case 22: {
        ...
        // Second test with smart pointer releasing at the end of the scope
        {
            ...
            BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(obj) {
                ...
                bslma::ManagedPtr<void> p1;         // Might throw
                p1.load(obj.allocate(13), &obj);
                ASSERT(p1);

                bslma::ManagedPtr<void> p2;         // Might throw
                p2.load(obj.allocate(15), &obj);
                ASSERT(p2);

                bslma::ManagedPtr<void> p3;         // Might throw
                p3.load(obj.allocate(0), &obj);
                ASSERT(0 == p3.get());

                bslma::ManagedPtr<void> p4;         // Might throw
                p4.load(obj.allocate(146), &obj);
                ASSERT(p4.get());
