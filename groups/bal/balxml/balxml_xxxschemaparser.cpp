// balxml_xxxschemaparser.cpp                                            -*-C++-*-
#include <balxml_xxxschemaparser.h>

#include <bsls_ident.h>
BSLS_IDENT_RCSID(balxml_xxxschemaparser_cpp,"$Id$ $CSID$")
...
const int ANON_NAMESPACE = balxml::NamespaceRegistry::BAEXML_PREDEF_MIN - 1000;
...
static inline
bdlmxxx::ElemType::Type bdemArrayType(bdlmxxx::ElemType::Type orig)
...
{
    bdlmxxx::ElemType::Type result = bdlmxxx::ElemType::toArrayType(orig);
    return bdlmxxx::ElemType::BDEM_VOID == result ? orig : result;
}

static
int parseDefaultValue(bdlmxxx::ElemRef elemRef, const bsl::string& defaultValue)
    ...
    // otherwise.
{
    ...
    switch (elemRef.type()) {
      case bdlmxxx::ElemType::BDEM_CHAR: {
        char value;
        if (! balxml::TypesParserUtil::parseDefault(&value, cStrValue, len)) {
...
struct XMLTypeDescriptor {
    ...
    bdlmxxx::ElemType::Type d_bdemType;        // 'bdem' equivalent type
    ...
    // ACCESSORS
    bdlmxxx::ElemType::Type bdemType() const;
...
const XMLTypeDescriptor COMPLEX_TYPE_DESCRIPTOR =
    ...
    // complex types.  Note that the type of 'bdlmxxx::RecordDef' created for an
...
inline
bdlmxxx::ElemType::Type XMLTypeDescriptor::bdemType() const
{
    return d_bdemType;
}
...
, d_id(bdlmxxx::RecordDef::BDEM_NULL_FIELD_ID)
, d_minOccurs(1)
, d_maxOccurs(1)
{
}
...
// bdlmxxx::ElemType::Type SchemaElement::bdemType() const
...
class SchemaType {
  ...
  private:
    ...
    bdlmxxx::RecordDef          *d_bdemRecord;      // generated bdem record
                                                // (held, not owned).
    EnumerationVector        d_enumerations;    // list of enumerators
    bdlmxxx::EnumerationDef     *d_bdemEnumeration; // generated bdem enumeration
                                                // (held, not owned)
  ...
  public:
    ...
    void setBdemRecord(bdlmxxx::RecordDef *record);
    void addEnumeration(const bsl::string& value, int id);
    void setPreserveEnumIds(bool val);
    void setBdemEnumeration(bdlmxxx::EnumerationDef *enumeration);
    ...
    bdlmxxx::RecordDef *bdemRecord() const;
    bool isDefined() const;
    bdlmxxx::EnumerationDef *bdemEnumeration() const;
...
inline
void SchemaType::setBdemRecord(bdlmxxx::RecordDef *record)
{
    d_bdemRecord = record;
}
...
{
   if (bdltuxxx::Unset<int>::unsetValue() == id) {
...
inline
void SchemaType::setBdemEnumeration(bdlmxxx::EnumerationDef *enumeration)
{
...
bdlmxxx::ElemType::Type SchemaType::bdemType() const
{
    const XMLTypeDescriptor* xmlTypeDesc = xmlTypeDescriptor();
    if (0 == xmlTypeDesc) {
        return bdlmxxx::ElemType::BDEM_VOID;
    }
    else {
        bdlmxxx::ElemType::Type result = xmlTypeDesc->bdemType();
        if (this->isList()) {
...
inline
bdlmxxx::RecordDef *SchemaType::bdemRecord() const
{
    return d_bdemRecord;
}
...
inline
bdlmxxx::EnumerationDef *SchemaType::bdemEnumeration() const
{
...
{
    bdlb::Print::indent(os, indentLevel, spacesPerLevel);
    os << '{';
    if (0 > indentLevel) {
    ...
    bdlb::Print::newlineAndIndent(os, indentLevel, spacesPerLevel);
    ...
    if (! d_fields.empty()) {
        ...
        for (FieldIterator i = beginFields(); i != endFields(); ++i) {
            bdlb::Print::newlineAndIndent(os, indentLevel + 1, spacesPerLevel);
            os << "Name = " << i->first;
            if (0 == i->second) {
                os << ", (null element)";
                continue;
...
bdlmxxx::ElemType::Type SchemaElement::bdemType() const
{
    bdlmxxx::ElemType::Type result = type()->bdemType();
    if (d_maxOccurs > 1) {
        result = bdemArrayType(result);
    }
...
inline
balxml::PrefixStack *ExternalSchemaInfo::prefixStack()
{
    return &d_prefixes;
}
inline
...
class SchemaContentHandler
{
  ...
  public:
    // PUBLIC TYPES
    typedef  balxml::ErrorInfo::Severity Severity;
  ...
  private:
    ...
    typedef balxml::SchemaParser::SchemaElementAttributes
    ...
    // PRIVATE CONSTANTS
    enum {
        NSID_XMLSCHEMA = balxml::NamespaceRegistry::BAEXML_XMLSCHEMA,
        NSID_BDEM      = balxml::NamespaceRegistry::BAEXML_BDEM
    };
    ...
    void generateTypeTree(bdlmxxx::Schema *outputSchema, SchemaType *type);
        // Generate the 'bdlmxxx::RecordDef' (recursively) and 'bdlmxxx::RecordDef'
        // objects described by 'type'.  This function inspects 'type' and
        // delegates to 'generateRecord' and 'generateEnumeration' functions
        // as appropriate.

    void generateRecord(bdlmxxx::Schema *outputSchema, SchemaType *type);
        // Recursively generate the 'bdlmxxx::RecordDef' object described by
        // 'type'.

    void generateEnumeration(bdlmxxx::Schema *outputSchema, SchemaType *type);
        // Generate the 'bdlmxxx::EnumerationDef' object described by
        // 'type'.
  ...
  public:
    ...
    // MANIPULATORS
    int  generateBdemSchema(bdlmxxx::Schema *outputSchema);
    ...
    setError(const bsl::string& msg,
             Severity           severity = balxml::ErrorInfo::BAEXML_ERROR);
    ...
    const balxml::ErrorInfo&  errorInfo() const;
...
SchemaType& SchemaContentHandler::getType(const QualifiedSymbol& typeSymbol)
{
    ...
    if (insertRet.second &&
        ...
        if (descriptor && bdlmxxx::ElemType::BDEM_VOID == descriptor->bdemType()) {
                                   ...
                                   SchemaElement      *element)
{
    ...
    if (element->maxOccurs() > 1) {
        ...
        bdlmxxx::ElemType::Type bType = xmlTypeDesc->bdemType();
        if (bdlmxxx::ElemType::isArrayType(bType)) {
...
int
SchemaContentHandler::generateBdemSchema(bdlmxxx::Schema *outputSchema)
{
    ...
    // that root element names appear in the constructed 'bdlmxxx::Schema', which
    ...
    // Generate 'bdlmxxx::Schema' records for each complex top-level element.
    for (elementsIter = d_topLevelElements.begin();
         elementsIter != d_topLevelElements.end(); ++elementsIter)
    {
    ...
    return d_errorInfo.severity() >= balxml::ErrorInfo::BAEXML_ERROR ?
        -1 : 0;
}

void
SchemaContentHandler::generateTypeTree(bdlmxxx::Schema *outputSchema,
                                       SchemaType  *type)
{
...
void
SchemaContentHandler::generateEnumeration(bdlmxxx::Schema *outputSchema,
                                          SchemaType  *type)
{
    // Create a 'bdlmxxx::EnumerationDef' for the specified XML simple 'type'.

    if (0 != type->bdemEnumeration()) {
        // A bdlmxxx::EnumerationDef has already been created for this simpleType.
        return;
    }
    ...
    bdlmxxx::EnumerationDef *enumeration =
...
void
SchemaContentHandler::generateRecord(bdlmxxx::Schema *outputSchema,
                                     SchemaType  *type)
{
    // Create a 'bdlmxxx::RecordDef' for the specified XML complex 'type'.

    if (0 != type->bdemRecord()) {
        // A bdlmxxx::RecordDef has already been created for this complexType.
        // This check has the effect of stopping recursion, but is necessary
        // even if the schema is not recursive.
        return;
    }

    // Create 'bdlmxxx::RecordDef' for this XML complex 'type'.
    ...
    const bdlmxxx::RecordDef::RecordType recordType = type->isChoice()
                                        ? bdlmxxx::RecordDef::BDEM_CHOICE_RECORD
                                        : bdlmxxx::RecordDef::BDEM_SEQUENCE_RECORD;
    bdlmxxx::RecordDef *thisRecord = outputSchema->createRecord(recordNameCstr,
                                                            recordType);
    if (0 == thisRecord) {
    ...
    for (SchemaType::FieldIterator fieldIter = type->beginFields();
        ...
        bdlmxxx::ElemType::Type bdemType = fieldElement->bdemType();
        if (bdlmxxx::ElemType::BDEM_VOID == bdemType) {
            setError(recordName + "." + fieldName + " has no type.");
            continue;
        }

        const bdlmxxx::RecordDef *constraint = fieldType->bdemRecord();
        if (constraint) {
            BSLS_ASSERT(bdlmxxx::ElemType::isAggregateType(bdemType));

            // Fix up 'bdemType' to correspond to its constraining record and
            // the 'maxOccurs' attribute.

            const bdlmxxx::RecordDef::RecordType constraintRecordType =
                                                      constraint->recordType();
            const int maxOccurs = fieldElement->maxOccurs();

            if (bdlmxxx::RecordDef::BDEM_CHOICE_RECORD == constraintRecordType) {
                bdemType = 1 == maxOccurs ? bdlmxxx::ElemType::BDEM_CHOICE
                                          : bdlmxxx::ElemType::BDEM_CHOICE_ARRAY;
            }
            else {
                BSLS_ASSERT(
                 bdlmxxx::RecordDef::BDEM_SEQUENCE_RECORD == constraintRecordType);

                bdemType = 1 == maxOccurs ? bdlmxxx::ElemType::BDEM_LIST
        ...
        bdlmxxx::FieldDefAttributes fieldAttr(bdemType);
        const bsl::string& defaultValue = fieldElement->defaultValue();
        if (! defaultValue.empty() &&
            ! bdlmxxx::ElemType::isAggregateType(bdemType)) {
...
//          && !bdlmxxx::ElemType::isArrayType(bdemType)) {
...
//             }
//         }
        bdlmxxx::EnumerationDef *enumerationConstraint = 0;
        if (formattingMode & bdeat_FormattingMode::BDEAT_NILLABLE
         && bdlmxxx::ElemType::isArrayType(bdemType)
         && !bdlmxxx::ElemType::isAggregateType(bdemType)
         && defaultValue.empty()) {
            bdlmxxx::RecordDef *tmpRecord = outputSchema->createRecord(
                                         0,
                                         bdlmxxx::RecordDef::BDEM_SEQUENCE_RECORD);
            bdlmxxx::ElemType::Type baseType =
                                        bdlmxxx::ElemType::fromArrayType(bdemType);
            bdlmxxx::FieldDefAttributes
                            baseAttr(baseType,
                                     false,
                                     bdeat_FormattingMode::BDEAT_NILLABLE);

            const bdlmxxx::EnumerationDef *enumeration =
                                                 fieldType->bdemEnumeration();
            enumeration ? tmpRecord->appendField(baseAttr, enumeration)
                        : tmpRecord->appendField(baseAttr);
            fieldAttr.reset(bdlmxxx::ElemType::BDEM_TABLE);
        ...
        const bdlmxxx::FieldDef *field = enumerationConstraint
    ...
    switch (parentXsTag()) {
      case XSTAG_complexType: {
        ...
        if ((0 == parentElement && minOccurs < 1) || maxOccurs > 1) {
            // bdlmxxx::Schema cannot represent a top-level aggregate that is
...
int SchemaContentHandler::startIncludeDef(int currXsTag)
{
    ...
    ///-------------------------------------------------
    // The 'balxml::SchemaParser' starts by parsing initial (top-level) schema.
    ...
    // generates the 'bdlmxxx::Schema' and returns.  Otherwise the parser picks up
...
{
    ExternalSchemaInfo  *schemaInfo  = currentSchemaInfo();
    balxml::PrefixStack  *oldPrefixes = d_reader->prefixStack();
    ...
    while (d_errorInfo.severity() < balxml::ErrorInfo::BAEXML_ERROR)
    {
        ...
        balxml::Reader::NodeType nodeType = d_reader->nodeType();

        switch(nodeType)
        {
          case balxml::Reader::BAEXML_NODE_TYPE_ELEMENT:
            dispatchStartElement();
            if (d_reader->isEmptyElement() &&
                d_errorInfo.severity() < balxml::ErrorInfo::BAEXML_ERROR) {
                dispatchEndElement();
            }
            break;
          case balxml::Reader::BAEXML_NODE_TYPE_END_ELEMENT:
            dispatchEndElement();
            break;
          default:
            break;
...
inline
const balxml::SchemaParser::SchemaElementAttributes&
...
inline
const balxml::ErrorInfo& SchemaContentHandler::errorInfo() const
{
    return d_errorInfo;
}
                               ...
                               const bdlb::StringRef&   inputId)
{
    ...
    Reader::XmlResolverFunctor resolver = d_reader->resolver();
    ...
    if (parseErrInfo.severity() <= ErrorInfo::BAEXML_WARNING) {
    ...
    if (d_errorInfo != 0 &&
        parseErrInfo.severity() != ErrorInfo::BAEXML_NO_ERROR) {
