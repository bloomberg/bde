// balxml_xxxschemaparser.t.cpp                  -*-C++-*-
...
typedef BloombergLP::balxml::MiniReader   Reader;
#endif
...
//                             TEST PLAN
//-----------------------------------------------------------------------------
// Although 'balxml::SchemaParser' has very few public functions, there is a
...
// 'parse' function, and comparing the resulting 'bdlmxxx::Schema' object
...
// 'gBdemSchema', to construct a 'bdlmxxx::Schema' object from a different
// compact script language.
//
// Test cases were selected to exercise branches and loops in the code, as
// determined by a white-box review of 'balxml_xxxschemaparser.cpp'.  Each XSD
...
typedef balxml::SchemaParser     Obj;
typedef balxml::ErrorInfo        ErrInfo;
    ...
    // function!
{
    ...
    *fieldId        = bdlmxxx::RecordDef::BDEM_NULL_FIELD_ID;
...
void runTest(int         line,
    ...
    // 'gBdemSchema' to generate a 'bdlmxxx::Schema' from 'SchemaScript'.  Create
    // a 'balxml::SchemaParser' object and parse the XSD document.  Assert that
    // the parse succeeds and that the 'bdlmxxx::Schema' constructed from the
    // parse matches the 'bdlmxxx::Schema' specified in the 'TestData' row.  Also
...
class Resolver
{
  ...
  public:
    ...
    typedef bslma::ManagedPtr< bsl::istringstream >  InpStrStreamPtr;
                             ...
                             const char *targetNs)
    {
        ...
        InpStrStreamPtr issPtr (
            new (*allocator) bsl::istringstream(buffer), allocator);
...
bool compareSchema(const bdlmxxx::Schema& lhs, const bdlmxxx::Schema& rhs)
{
    ...
    for (int i = 0; i < numRecords; ++i) {
        const bdlmxxx::RecordDef& lhsRecord = lhs.record(i);
        const bdlmxxx::RecordDef& rhsRecord = rhs.record(i);
        ...
        for (int j = 0; j < numFields; ++j) {
            const bdlmxxx::FieldDef& lhsField   = lhsRecord.field(j);
            const bdlmxxx::FieldDef& rhsField   = rhsRecord.field(j);

            const int            lhsFieldId = lhsRecord.fieldId(j);
            const int            rhsFieldId = rhsRecord.fieldId(j);

            const bdlmxxx::ElemType::Type type = lhsField.elemType();
    ...
    for (int i = 0; i < numEnums; ++i) {
        const bdlmxxx::EnumerationDef& lhsEnum = lhs.enumeration(i);
        const bdlmxxx::EnumerationDef& rhsEnum = rhs.enumeration(i);
        ...
        if (! bdlmxxx::EnumerationDef::areEquivalent(lhsEnum, rhsEnum)) {
            return false;                                             // RETURN
        }
    }
...
// registry, prefix stack, XML reader, error info, 'balxml::SchemaParser' and a
// 'bdlmxxx::Schema' objects:
//..
...
//..
    ...
    if (0 != dateField) ASSERT(bdlmxxx::ElemType::BDEM_DATETZ
                               == dateField->elemType());
    ...
    if (0 != nameField) ASSERT(bdlmxxx::ElemType::BDEM_STRING
                               == nameField->elemType());
    ...
    if (0 != sizeField) ASSERT(bdlmxxx::ElemType::BDEM_DOUBLE
                               == sizeField->elemType());
//..
      ...
      case 18: {
        ...
        balxml::NamespaceRegistry namespaces1, namespaces2;
        balxml::PrefixStack       prefixes1(&namespaces1),
                                 prefixes2(&namespaces2);
        balxml::MiniReader        reader1, reader2;
        reader1.setPrefixStack(&prefixes1);
        reader2.setPrefixStack(&prefixes2);
        balxml::ErrorInfo         errInfo1, errInfo2;
        balxml::SchemaParser      parser1(&reader1, &errInfo1);
        balxml::SchemaParser      parser2(&reader2, &errInfo2);
        bdlmxxx::Schema              schema1, schema2;
...
#define BODY                                                                  \
    ...
    "\n"
      ...
      case 16: {
        ...
        //    generated, yielding a 'bdlmxxx::RecordDef' with a single, untagged
        ...
        //  - Generate the 'bdlmxxx::Schema' for each XML schema and compare with
        //    the expected results.
        //  - White-box note: The order of generated 'bdlmxxx::RecordDef's in the
        //    generated 'bdlmxxx::Schema' is not guaranteed by this component.
        ...
        for (int i = 0; i < DATA_SIZE; ++i) {
            ...
            bdlmxxx::Schema expectedBdemSchema;
      ...
      case 15: {
        ...
        //   - If the 'bdlmxxx::Schema' object is non-empty when 'parse' is called,
      ...
      case 13: {
        ...
        //   - For a variety of errors detectable by the 'balxml::SchemaParser'
...
{ L_, "[0][1]{[4A]{[12]{[9]^{[6Bh]}}}}"       , "within <simpleType>."  }, // 1
      ...
      case 12: {
        ...
        //   - Verify that they all produce the same, expected 'bdlmxxx::Schema'.
        //
        // --------------------------------------------------------------------
...
{ L_, "<xs:whiteSpace>"
      ...
      case 8: {
        ...
        //      mapped to unnamed fields in the resulting 'bdlmxxx::RecordDef'.
        ...
        //    the XSD and verify that the resulting 'bdlmxxx::Schema' matches the
        //    expected value.
        //
...
//{ L_, "[1]{[4A]{[12]{[22Ah][15g]{}}}}",        "only on <sequence>"  },
      ...
      case 7: {
        ...
        //      field of array type.
        //    6 Duplicate 'bdlmxxx::RecordDef's are created for all root elements
        //      sharing the same <complexType>.
        //    7 An <element> of <complexType> with maxOccurs > 1 constructs a
        //      constrained TABLE field in the 'bdlmxxx::Schema'.
...
{ L_, "[1]{[4A]{[2]{[9]{[6Bh@1@5@6]}}}}",                 "A{Bm@D1@NF}" }, // 9
      ...
      case 6: {
        ...
        //      be mapped to an unnamed record in the resulting 'bdlmxxx::Schema'.
        ...
        //     'bdlmxxx::Schema' with the expected result.
...
{ L_, "[1]{[4A]{[2]{[9]{[6Bb@1][6Cc@1@5][4D]{[12]{[16d]}}}}}}",
        ...
        // schema must be built using 'bdlmxxx::Schema' calls directly.
...
{ L_,  "[1]{[13E@22]{[15q]{[24B][24C][24A]}}[4D]{[2]{[9]{[6FE]}}}}",
      ...
      case 4: {
        ...
        //     within the constructed 'bdlmxxx::Schema'.
        ...
        //     string.
        //   - Parse each XSD string into a 'bdlmxxx::Schema' and compare the
        ...
        if (verbose) bsl::cout << "Testing <sequence>." << bsl::endl;
        {
            ...
            // 'bdlmxxx::Schema' output.  They differ in the way the array-ness of
            ...
            for (int j = 0; j < NUM_XSD_SCRIPTS; ++j) {
                ...
                for (int i = 0; i < sizeof XMLTYPECHARS - 1; ++i) {
                    *xsdTypeCharPtr = XMLTYPECHARS[i];
                    ...
                    bdlmxxx::Schema bdemSchema;
                    gBdemSchema(&bdemSchema, bdemScript);

                    if (veryVerbose) {
                        bsl::cout << "    XML Schema = \n" << xmlSchema
                                  << "    Expected bdlmxxx::Schema = \n"
                                  << bdemSchema;
                    }
                    ...
                    bdlmxxx::Schema outputSchema;
                    bsl::string targetNamespace;

                    int result = parser.parse(input, &outputSchema,
                                              &targetNamespace);

                    if (veryVerbose) bsl::cout << "    Actual bdlmxxx::Schema = \n"
                                               << outputSchema;
...
//==== ==========                                          ===========
 ...
 { L_, "[0][1]{[3A]{[9]{[6Bh]}}[3D]{[9]{[6BA@23@5]}}[6CD]}",
      ...
      case 3: {
        ...
        //   - Parse each XML schema string into a 'bdlmxxx::Schema' and compare
...
//                     bdemScript += "Bw@FAD";
      ...
      case 1: {
        ...
        bdlsb::FixedMemInStreamBuf input(SCHEMA1, SCHEMA1_LEN);
        bdlmxxx::Schema schema;
        bsl::string targetNamespace;
      ...
      case -1: {
        // --------------------------------------------------------------------
        // balxml::SchemaParser INTERACTIVE TEST
        //
        // Invoke this test program with argument -1 and redirect input to an
        // xml schema file.  The program will construct the bdlmxxx::Schema and
        // print it out.
        //
        // --------------------------------------------------------------------
