...
//                                   Overview
//                                   --------
// Testing 'balxml::DataUtil' is divided into 2 parts (apart from
...
typedef balxml::MiniReader Reader;
typedef balxml::ErrorInfo  ErrorInfo;

typedef bdlmxxx::ElemRef             ERef;
typedef bdlmxxx::ElemType            EType;
typedef bdlmxxx::FieldSpec           FieldSpec;
typedef bdlmxxx::FieldDefAttributes  FieldAttr;
typedef bdlmxxx::RecordDef           Record;
typedef bsls::Types::Int64       Int64;
typedef bdlmxxx::SchemaAggregateUtil SAU;
...
class TestValidatingReader : public balxml::ValidatingReader
{
private:
    ...
    balxml::Reader      *d_reader;         // Actual reader
...
public:
    ...
    // PUBLIC CREATORS
    TestValidatingReader(balxml::Reader *reader);
    virtual ~TestValidatingReader(void);

    balxml::Reader *implementationReader()
    {
        return d_reader;
    }
...
TestValidatingReader::TestValidatingReader(balxml::Reader *reader)
: d_reader(reader)
...
class ScalarData {
  private:
    ...
    bdlt::DateTz     d_dateTz;
    bdlt::TimeTz     d_timeTz;
  ...
  public:
    ...
    ScalarData(const bdlt::Date& d) : d_type(EType::BDEM_DATE), d_date(d) {}
    ScalarData(const bdlt::Time& t) : d_type(EType::BDEM_TIME), d_time(t) {}
    ...
    ScalarData(const bdlt::DateTz& d) : d_type(EType::BDEM_DATETZ), d_dateTz(d)
    {}
    ScalarData(const bdlt::TimeTz& t) : d_type(EType::BDEM_TIMETZ), d_timeTz(t)
    {}

    // ACCESSORS
    EType::Type type() const { return d_type; }
    EType::Type arrayType() const;
    void appendToList(bdlmxxx::List *list) const;
    void appendArrayToList(bdlmxxx::List *list, int arrSize = REPEAT) const;
    void addToRow(bdlmxxx::Row *row, int index) const;
    void addArrayToRow(bdlmxxx::Row *row, int index, int arrSize = REPEAT) const;
};
...
void ScalarData::appendToList(bdlmxxx::List *list) const
{
    switch (d_type) {
...
void ScalarData::appendArrayToList(bdlmxxx::List *list, int arrSize) const
{
    switch(d_type) {
      case EType::BDEM_CHAR: {
      ...
      case EType::BDEM_DATE: {
        bsl::vector<bdlt::Date> dateArray;
        for (int i = 0; i < arrSize; ++i) {
      ...
      case EType::BDEM_DATETIMETZ: {
          bsl::vector<bdlt::DatetimeTz> datetimeTzArray;
          for (int i = 0; i < arrSize; ++i) {
      ...
      case EType::BDEM_DATETZ: {
          bsl::vector<bdlt::DateTz> dateTzArray;
          for (int i = 0; i < arrSize; ++i) {
      ...
      case EType::BDEM_TIMETZ: {
          bsl::vector<bdlt::TimeTz> timeTzArray;
          for (int i = 0; i < arrSize; ++i) {
...
void ScalarData::addToRow(bdlmxxx::Row *row, int index) const
{
...
void ScalarData::addArrayToRow(bdlmxxx::Row *row, int index, int arrSize) const
{
...
const ScalarData VALUES[] = {
    ...
    (double)10.5,                                // f
    bsl::string("one"),                          // g
    bdlt::Datetime(2000, 1, 1, 0, 1, 2, 3),       // h
    bdlt::Date(2000, 1, 1),                       // i
    bdlt::Time(0, 1, 2, 3),                       // j
    ...
    bdlt::Datetime(2000, 1, 1, 0, 1, 2, 3),       // r array repeat REPEAT times
    bdlt::Date(2000, 1, 1),                       // s array repeat REPEAT times
    bdlt::Time(0, 1, 2, 3),                       // t array repeat REPEAT times
};
    ...
    // function!
{
    ...
    *isNullable     = false;
    ...
    if (*formattingMode & bdeat_FormattingMode::BDEAT_NILLABLE
     && !bdlmxxx::ElemType::isArrayType(fieldType)) {
        *isNullable = true;
    }
}
...
// Generate a 'bdlmxxx::Schema' from a compact specification string.
//
...
//
// Input spec           Output bdlmxxx::Schema
// ----------           ------------------
// A{BaDl}S{WvAKb}      {
//                          RECORD "Apple" {
//                              CHAR "Banana";
...
//                          }
//                      }
void gBdemSchema(bdlmxxx::Schema *schema, const bsl::string& spec,
...
class Container {
    ...
    // stored in a stack during the construction of bdlmxxx::List from scratch.
    // This is used in generator gBdemList.
  public:
    enum Type { IS_LIST, IS_TABLE, IS_ROW } d_type;
  private:
    union {
        bdlmxxx::List  *d_list_p;
        bdlmxxx::Table *d_table_p;
        bdlmxxx::Row   *d_row_p;
    }                     d_container;
    int                   d_currentIndex;
  ...
  public:
    // CREATORS
    Container(bdlmxxx::List *bdemList)
    : d_type(IS_LIST)
    , d_currentIndex(-1)
    {
    ...
    Container(bdlmxxx::Table *bdemTable)
    : d_type(IS_TABLE)
    , d_currentIndex(-1)
    {
...
// * lowercase letters [a-t]: bdlmxxx::ElemType scalar or array type
//   a0 - t0: scalar or array field is null
// * bdlmxxx::ElemType LIST and TABLE
...
void gBdemList(bdlmxxx::List *bdemList, const bsl::string& spec)
{
    ASSERT('(' == spec[0]);
          ...
          case OPEN_PAREN: {
            ...
            switch (currentContainer.type()) {
              case Container::IS_TABLE: {
                ...
                bdlmxxx::Table *currentTable = currentContainer.table();
                ASSERT(currentTable);
              ...
              case Container::IS_ROW: {
                ...
                bdlmxxx::Row *currentRow = currentContainer.row();
                ASSERT(currentRow);
              ...
              case Container::IS_LIST: {
                // this '(' means a new list
                bdlmxxx::List *currentList = currentContainer.list();
                ASSERT(currentList);
                ...
                if (isInColumnDef) {
                    ...
                    if (EType::isScalarType(currentBdemType)) {
                        if (currentContainer.type() == Container::IS_LIST) {
                            bdlmxxx::List& theList = *(currentContainer.list());
                            ...
                            bdlmxxx::Row& theRow = *(currentContainer.row());
                            currentContainer.incrementCurrentIndex();
...
//         be because the bdlmxxx::Schema structure maps record name to the root
          ...
          bool useAnonymousNames = false)
{
    ...
    balxml::Formatter formatter(oss, 0, 4, INT_MAX);
    formatter.addHeader();
...
void testData(const Data DATA[], int NUM_DATA,
    ...
    // Driver for test Data array.  Generate bdlmxxx::Schema and expected
    // bdlmxxx::List, and xml document.  Parse the document and get a bdlmxxx::List
    ...
    // same as the expected bdem_List.
{
    for (int i = 0; i < NUM_DATA; ++i) {
        ...
        bsl::shared_ptr<bdlmxxx::Schema>  schemaPtr(
                                         &schema,
        ...
        if ("" != LISTSPEC) {
            ...
            int status = balxml::DataUtil::createAggregateFromDocument(
                        &aggregate,
                        xmlStream.rdbuf(),
                        schemaPtr,
                        &reader,
            ...
            const bdlmxxx::List& LIST = aggregate.asElemRef().theList();
...
// for the 'balxml::DataUtil::createAggregateFromDocument'
//---------------------------------------------------------------------------

int doTest(bdlaggxxx::Aggregate   *aggregate,
           const char       *xmlData,
...

{
    ...
    bdlsb::FixedMemInStreamBuf xmlDataStream(xmlData,
                                            bsl::strlen(xmlData));
    bdlsb::FixedMemInStreamBuf xmlSchemaStream(xmlSchema,
    ...
    balxml::ErrorInfo *parmErrInfo = (flags & TF_ERR_INFO) ? &errorInfo : 0;
    ...
    if (flags & TF_VALIDATION) {
        ...
        if (flags & TF_OPTIONS) {
            ...
            rc = balxml::DataUtil::createAggregateFromDocument(
                        aggregate,
                        &xmlDataStream,
                        &xmlSchemaStream,
                        &vreader,
...
// for the 'balxml::DataUtil::createAggregateFromDocument'
//---------------------------------------------------------------------------
int doTest2(bdlaggxxx::Aggregate                      *aggregate,
            const char                          *xmlData,
            const bsl::shared_ptr<bdlmxxx::Schema>&  bdemSchema,
            bslma::Allocator                    *allocator,
            int                                  flags)

{
    ...
    bdlsb::FixedMemInStreamBuf xmlDataStream(xmlData, bsl::strlen(xmlData));
...
bsl::shared_ptr<bdlmxxx::Schema> buildBdemSchema(
                const char         *xmlSchema,
                bslma::Allocator   *allocator,
                int                 flags)
{
               ...
               bslma::Allocator *allocator)
{
    ...
    for (int i=1; i < maxFlags; ++i) {
        ...
        bsl::shared_ptr<bdlmxxx::Schema> bdemSchema =
...
{
  public:
    typedef balxml::Reader::StreamBufPtr    StreamBufPtr;
                             ...
                             const char *targetNamespace)
    {
        ...
        if (bsl::strcmp(schemaLocation,  "location1") == 0) {
            ret.load(new bdlsb::FixedMemInStreamBuf(SCHEMA_INC,
                                                   sizeof(SCHEMA_INC) - 1));
        }
...
//..
    ...
    if (veryVerbose) {
      ...
      case 11: {
        ...
        //
        // Plan:
        //   Create bdlmxxx::Schema's of records that contain recursive list
        //   fields, which self-reference the enclosing record, as well as
        //   cross-reference other records.  Create a baexml::DataParser for
        //   this
...
{ L_, "A{Bc}D{EvA}F{GvF}", "F{G{G}}",           "([v([v([v])])])"            },
      ...
      case 10: {
        ...
        //
        // Plan:
        //   Create bdlmxxx::Schema's of records that contain non-recursive table
        //   fields that cross reference records.  Create a baexml::DataParser
        //   for this schema and parse XML documents containing no such
        //   elements.  Notice the absence of such elements really indicates
        //   unset table fields in the bdlmxxx::List that's bound to that schema.
        ...
        if (verbose) {
            bsl::cout << "TEST parsing of XML documents without certain "
                      ...
                      << "This produces a bdlmxxx::List with unset "
...
{ L_, "A{BbCc}D{EvAFdGvAHeIf}G{HvAIgKvD}L{MvGNvDOhPiQj}",
      ...
      case 9: {
        ...
        //   'baexml::DataParser' for this schema and parse XML documents
...
{ L_, "A{Bo@FL}D{Ep@FLFvA}G{HvDIq}J{KvG}",
      ...
      case 8: {
        ...
        //
        // Concerns:
        //   The innermost recursive field in the bdlmxxx::List is itself unset,
...
{ L_, "D{Ed@NTFuD}G{HuD@NTIe@NTJuG}",
      ...
      case 7: {
        ...
        //
        // Plan:
        //   Create bdlmxxx::Schema's of records that contain non-recursive list
        //   fields that cross reference records.  Create a baexml::DataParser
...
{ L_, "A{Bc}D{EdFuA@NT}G{HuD@NTIe}J{KfLuG@NT}M{NuJ@NTOg}P{QhRuM@NT}S{TuP@NTUi}"
      ...
      case 6: {
        ...
        //   Create bdlmxxx::Schema's of records that contain non-recursive list
        //   fields that cross reference one another.  Create a
        //   baexml::DataParser for the schema.  Parse an XML document
...
{ L_, "A{BbCc}D{EuAFdGuAHeIf}G{HuAIgKuD}L{MuGNuDOhPiQj}",
      ...
      case 5: {
        ...
        //
        // Plan:
        //   Create a bdlmxxx::Schema with a record that contains fields of all
        //   scalar/array types, and create a baexml::DataParser on this
        //   bdem_Schema.
        ...
        //   and verify the resulting bdlmxxx::List is as expected.
        // --------------------------------------------------------------------
        ...
        const bdlmxxx::List& list1 = aggregate.asElemRef().theList();
        int index = aggregate.recordDef().recordIndex();
        ...
        status = balxml::DataUtil::createAggregateFromDocument(
                        &aggregate,
                        xmlStream2.rdbuf(),
                        schemaPtr,
                        &reader,
        ...
        const bdlmxxx::List& list2 = aggregate.asElemRef().theList();
      ...
      case 4: {
        ...
        // Plan:
        //   Create a bdlmxxx::Schema with a record that contains fields of all
        //   scalar/array types, and a create a baexml::DataParser on this
        ...
        //   the resulting bdlmxxx::List are the same as the expected bdem_List.
        // --------------------------------------------------------------------
        ...
        bdlmxxx::List& list1 = aggregate.asElemRef().theModifiableList();
        int index = aggregate.recordDef().recordIndex();
        ...
        bdlmxxx::List& list2 = aggregate.asElemRef().theModifiableList();
        index = aggregate.recordDef().recordIndex();
        ...
        bdlmxxx::List& list3 = aggregate.asElemRef().theModifiableList();
        index = aggregate.recordDef().recordIndex();
      ...
      case 3: {
        ...
        bdlmxxx::List& list = aggregate.asElemRef().theModifiableList();
        int index = aggregate.recordDef().recordIndex();
      ...
      case 2: {
        ...
        bdlsb::FixedMemInStreamBuf xmlSchemaStream(SCHEMA, sizeof(SCHEMA)-1);
        bdlsb::FixedMemInStreamBuf xmlDataStream(DATA, sizeof(DATA)-1);
        ...
        int rc = balxml::DataUtil::createAggregateFromDocument(
                        &result,
                        &xmlDataStream,
                        &xmlSchemaStream,
                        &reader,
      ...
      case 1: {
        ...
        //   build the 'bdlaggxxx::Aggregate' object.  Verify that aggregate
