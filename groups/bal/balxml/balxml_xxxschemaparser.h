// balxml_xxxschemaparser.h                  -*-C++-*-
#ifndef INCLUDED_BALXML_XXXSCHEMAPARSER
#define INCLUDED_BALXML_XXXSCHEMAPARSER
...
//@PURPOSE: Provide an XML schema parser that produces a 'bdlmxxx::Schema' object.
//
//@CLASSES:
//  balxml::SchemaParser: XML schema parser
//
//@SEE_ALSO: bdlmxxx_schema, baexml_schemagenerator, balxml_decoder
//
...
// 'bdlmxxx::Choice', 'bdlmxxx::ChoiceArray', 'bdlmxxx::List', 'bdlmxxx::Table', and
// 'bdlmxxx::Schema'.  An XML schema (XSD) document describes and constrains XML
// instance documents in much the same way that a 'bdlmxxx::Schema' object
// describes and constrains 'bdem' aggregates ('bdlmxxx::Choice',
// 'bdlmxxx::ChoiceArray', 'bdlmxxx::List', and 'bdlmxxx::Table').
//
// This component translates an XSD document (conforming to a subset of the XML
// Schema specification) into an isomorphic 'bdlmxxx::Schema' object.  A companion
// component, 'DataParser', translates an XML data document into a 'bdlmxxx::List'
...
// be used to construct a ('bdlmxxx::Schema', 'bdlmxxx::List') pair such that the
// 'bdlmxxx::Schema' describes and validates the 'bdlmxxx::List'.
//
///XML Schema Format
///-----------------
...
// same (or different) element with the type, "date".  A 'balxml::SchemaParser'
// object would construct the following 'bdlmxxx::Schema' object from the above
// XSD document:
//..
//  {
//      RECORD {
...
//  </schema>
//..
// When the above XML schema is parsed, the resulting 'bdlmxxx::Schema' object is
...
//   TABLE<"paragraph">
//..
// Here is the complete 'bdlmxxx::Schema' generated for the above XSD file:
//..
//  {
//      RECORD "paragraph" {
//          INT "indent";
...
///------------------------------------
// The 'balxml::SchemaParser' class recognizes and translates a specific subset
...
// 'bdlmxxx::Schema' is ignored.  Any *schema* *construct* that is not recognized
// or cannot be translated into 'bdlmxxx::Schema' is either ignored or results in
// an error, as listed below.
//
// The parser translates a '<schema>' construct into a 'bdlmxxx::Schema' object, a
// '<complexType>' construct within a '<schema>' into a 'bdlmxxx::RecordDef' object
// within the 'bdlmxxx::Schema', and an '<element>' construct within a
// '<complexType>' into a 'bdlmxxx::FieldDef' object within the 'bdlmxxx::RecordDef'.
...
//    entire 'bdlmxxx::Schema' object.  It may contain attributes that define
//    namespaces, including the target namespace for XML elements that are
//    being defined in this schema.
//..
// <complexType [name="name"]>
//..
//    Describes a 'bdlmxxx::RecordDef' object.  A schema-level '<complexType>' must
//    have a name, which is assigned to its 'bdlmxxx::RecordDef'.  Only
...
//    in the creation of 'bdlmxxx::RecordDef' objects.
//..
...
//    describes a 'bdlmxxx::FieldDef'.  An '<element>' construct appearing at the
//    schema-level identifies a "root" element in conforming XML data
//    documents.  In the above example, "memo" is the root element.  The
//    'bdlmxxx::RecordDef' corresponding to the '<complexType>' directly
...
//    that the latter does not show up in the populated 'bdlmxxx::Schema' object
//    for the corresponding 'bdlmxxx::RecordDef'.
//
...
//    a numeric value greater than "1", then the corresponding 'bdlmxxx::FieldDef'
...
//    corresponding 'bdlmxxx::FieldDef' generated from the '<element>' construct
...
//    fields of the constructed 'bdlmxxx::RecordDef' object.  The type of the
//    'bdlmxxx::RecordDef' created for a 'complexType' containing a '<choice>'
//    child element will be 'bdlmxxx::RecordDef::BDEM_CHOICE_RECORD'.  In the case
//    of '<all>' and '<sequence>', the type of the corresponding
//    'bdlmxxx::RecordDef' will be bdlmxxx::RecordDef::BDEM_SEQUENCE_RECORD.  Although
//    the '<all>' and '<sequence>' constructs generate the same
//    'bdlmxxx::RecordDef' output, they have different semantics when used by the
...
//    construct will describe a 'bdlmxxx::EnumerationDef' object within the
//    'bdlmxxx::Schema'.  Each '<enumeration>' (which must be nested within a
...
//    effect on the constructed 'bdlmxxx::Schema'.
//..
// <annotation>
// <appinfo>
// <attribute>
...
// <whiteSpace>
//..
//    The above constructs are skipped by 'balxml::SchemaParser' and have no
//    effect on the constructed 'bdlmxxx::Schema'.  Any nested constructs are also
...
// <union>
//..
//    'balxml::SchemaParser' will produce an error on any of the above XML
//    schema elements.
//
...
//..
//  XML Data Type         bdem::ElementType  bdeat_FormattingMode
//  -------------         ----------------  --------------------
...
// anonymous record and the constructed 'bdlmxxx::RecordDef' is not given a name.
// No attempt is made to position the 'bdlmxxx::RecordDef' such that its index
// within the 'bdlmxxx::Schema' corresponds to the value of '##' (see
// "Limitations" below).
//
...
// constructed 'bdlmxxx::EnumerationDef' is not given a name.  No attempt is made
// to position the 'bdlmxxx::EnumerationDef' such that its index within the
// 'bdlmxxx::Schema' corresponds to the value of '##'.
//
...
// the constructed 'bdlmxxx::FieldDef' is not given a name.  No attempt is made to
// position the 'bdlmxxx::FieldDef' such that its index within its parent
// 'bdlmxxx::RecordDef' corresponds to the value of '##' (see "Limitations" below).
...
// (counting from zero), then its index within the 'bdlmxxx::RecordDef' object
// *will* correspond to 'n'.  For example, if 'FIELD_2' is the third element in
// a 'complexType', then a 'bdlmxxx::FieldDef' will be constructed at position 2
// within its parent 'bdlmxxx::RecordDef'.
//
///Limitations
///-----------
// XSD is an expressive format for defining and constraining the contents of a
// wide variety of XML documents.  'balxml::SchemaParser' translates only a
...
// 'bdlmxxx::Schema'; others are simply ignored, as listed above.  Below are some
// specific limitations in the handling of XML schema (XSD) documents.
//
///Does not Validate Fully
///- - - - - - - - - - - -
// 'balxml::SchemaParser' does not fully check its input stream to ensure that
...
// error when read by 'balxml::SchemaParser':
//..
//  <complexType>
//      <sequence>
...
/// - - - - - - - - -- - - - - -
// The 'bdlmxxx::FieldDef' class does not have provisions for constraints on
// values (except for structural constraints on choice, choice array, list,
// table values, and enumeration constraints on strings).
// 'balxml::SchemaParser' must therefore ignore useful information in the XML
// schema when constructing the 'bdlmxxx::Schema' object.  However, if the XML
...
// 'bdlmxxx::Schema' does not.
//
///No Support for Groups
///- - - - - - - - - - -
// The '<group>' construct is not supported at this time, although there is
// an obvious mapping from the XML Schema concept to 'bdlmxxx::Schema' concepts.
// This construct may be supported in a future release.
//
///No Support for Unconstrained Choices, Choice Arrays, Lists, and Tables
/// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// A 'bdlmxxx::FieldDef' with type 'CHOICE', 'CHOICE_ARRAY', 'LIST', or 'TABLE' is
// not required to be constrained by a 'bdlmxxx::RecordDef' describing the
...
// deliberate; the use of unconstrained aggregates in 'bdlmxxx::Schema' is
// discouraged.
//
...
// 'bdlmxxx::Schema' back to XSD format.  This works well for '<complexType>'
// constructs containing a '<sequence>' or '<all>', but does not work for
// '<simpleType>' or '<choice>', as there is no place in the 'bdlmxxx::Schema'
...
/// - - - - - - - - - - - - - - - - - - - - -
// If the 'bdlmxxx::Schema' object constructed by 'balxml::SchemaParser' is fed to
...
//
// First, since XML schema is much richer than 'bdlmxxx::Schema' (e.g., in its
...
// 'bdlmxxx::Schema'.  All annotations, comments, documentation and whitespace are
// similarly discarded.
//
...
// translated into 'INT_ARRAY' types in 'bdlmxxx::FieldDef':
//..
...
//
// Third, 'bdlmxxx::Schema' supports unnamed records and unnamed fields which can
...
// 'balxml::SchemaParser' does not attempt to preserve the indexes of these
// unnamed entities, although it does correctly discard the name when
// constructing the 'bdlmxxx::Schema' object.
//
// Finally, the ordering of '<complexType>' constructs in the XML schema is not
// preserved when constructing the 'bdlmxxx::RecordDef' objects in the
// 'bdlmxxx::Schema'.
//
// All this having been said, translating back-and-forth between XML schema
// and 'bdlmxxx::Schema' will produce byte-for-byte matches after four
// translations.  I.e, starting with a file, 'xml1.xsd' and successively
// translating it back and forth to 'bdlmxxx::Schema':
//..
//  xml1.xsd -> bdemSchema1 -> xml2.xsd -> bdemSchema2 -> xml3.xsd
//..
// 'bdem::Schema1' will compare equal to 'bdemSchema2' (by 'operator==') and
// 'xml2.xsd' will have the same contents as 'xml3.xsd'.
//
///Usage
///-----
// Frequently, the schema for a 'bdlmxxx::List' can be hard-wired into the
...
// 'balxml::SchemaParser' object and a 'bdlmxxx::Schema' object:
//..
//  balxml::MiniReader reader;
//  balxml::ErrorInfo  errorInfo;
//..
// Given 'reader', we can now create the following 'parser'; we also supply a
// 'balxml::ErrorInfo' and diagnostic stream, both of which are optional.  In
// our case, we use 'bsl::cerr' for the diagnostic stream:
//..
//  balxml::SchemaParser parser(&reader, &errorInfo, &bsl::cerr);
...
//..
// Now we call the 'parse' method to construct the 'bdlmxxx::Schema' object from
...
//  }
//..
// Confirm that the 'bdlmxxx::Schema' object has the desired structure:
//..
//  const bdlmxxx::RecordDef *mailOrderRec = schema.lookupRecord("mailOrder");
//  assert(0 != mailOrderRec);
//  const int mailOrderRecLen = (mailOrderRec ? mailOrderRec->numFields() : 0);
//  assert(3 == mailOrderRecLen);
//
//  const bdlmxxx::RecordDef *itemRec = schema.lookupRecord("item");
//  assert(0 != itemRec);
//  const int itemRecLen = (itemRec ? itemRec->numFields() : 0);
//  assert(3 == itemRecLen);
//
//  const bdlmxxx::EnumerationDef *idsEnum = schema.lookupEnumeration("ids");
//  assert(0 != idsEnum);
//  const int idsEnumLen = (idsEnum ? idsEnum->numEnumerators() : 0);
//  assert(3 == idsEnumLen);
//
//  const bdlmxxx::FieldDef *dateField = mailOrderRec->lookupField("orderDate");
//  assert(0 != dateField);
//  if (0 < mailOrderRecLen) assert(&mailOrderRec->field(0) == dateField);
//  if (0 != dateField)
//      assert(bdlmxxx::ElemType::BDEM_DATETZ == dateField->elemType());
//
//  const bdlmxxx::FieldDef *nameField = mailOrderRec->lookupField("customerName");
//  assert(0 != nameField);
//  if (1 < mailOrderRecLen) assert(&mailOrderRec->field(1) == nameField);
//  if (0 != nameField)
//      assert(bdlmxxx::ElemType::BDEM_STRING == nameField->elemType());
//
//  const bdlmxxx::FieldDef *itemsField = mailOrderRec->lookupField("items");
//  assert(0 != itemsField);
//  if (2 < mailOrderRecLen) assert(&mailOrderRec->field(2) == itemsField);
//  if (0 != itemsField) {
//      assert(bdlmxxx::ElemType::BDEM_TABLE == itemsField->elemType());
//      assert(itemRec == itemsField->constraint());
//  }
//
//  const bdlmxxx::FieldDef *itemNameField = itemRec->lookupField("itemName");
//  assert(0 != itemNameField);
//  if (0 < itemRecLen) assert(&itemRec->field(0) == itemNameField);
//  if (0 != itemNameField) {
//      assert(bdlmxxx::ElemType::BDEM_STRING == itemNameField->elemType());
//  }
//
//  const bdlmxxx::FieldDef *idField = itemRec->lookupField("id");
//  assert(0 != idField);
...
//  }
//
//  const bdlmxxx::FieldDef *sizeField = itemRec->lookupField("size");
//  assert(0 != sizeField);
//  if (2 < itemRecLen) assert(&itemRec->field(2) == sizeField);
//  if (0 != sizeField)
//      assert(bdlmxxx::ElemType::BDEM_DOUBLE == sizeField->elemType());
//..
...
class SchemaParser {
    // Translate an XML schema document into an isomorphic 'bdlmxxx::Schema'
    // object.
  ...
  private:
    // DATA
    Reader    *d_reader;        // for parsing XML (held, not owned)
    ErrorInfo *d_errorInfo;     // for reporting error (held, not owned)
  ...
  public:
    ...
    // ACCESSORS
    Reader *reader() const;
        // Return the 'Reader' used by this schema parser.
...
// CREATORS
inline
SchemaParser::SchemaParser(Reader    *reader,
                                         ErrorInfo *errorInfo,
...
inline
void SchemaParser::setVerboseStream(bsl::ostream *verboseStream)
{
    d_verboseStream = verboseStream;
}

inline
ErrorInfo* SchemaParser::errorInfo() const
{
    return d_errorInfo;
}

inline
Reader* SchemaParser::reader() const
{
    return d_reader;
}
