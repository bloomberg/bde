...
// 'bcec_AtomicRingBufferIndexManager' using a supplied 'pushCombinedIndex'
...
struct AtomicRingBufferIndexManagerDataMembers {
...
// CREATORS
AtomicRingBufferState::AtomicRingBufferState(
                         const bcec_AtomicRingBufferIndexManager *indexManager)
: d_data(reinterpret_cast<const AtomicRingBufferIndexManagerDataMembers *>(
                                                                 indexManager))
{
}

// ACCESSORS
unsigned int AtomicRingBufferState::pushIndex() const
{
    return d_data->d_pushIndex % capacity();
}

unsigned int AtomicRingBufferState::pushGeneration() const
{
    return d_data->d_pushIndex / capacity();
}

unsigned int AtomicRingBufferState::popIndex() const
{
    return d_data->d_popIndex % capacity();
}

unsigned int AtomicRingBufferState::popGeneration() const
{
    return d_data->d_popIndex / capacity();
}

unsigned int AtomicRingBufferState::elementGeneration(unsigned int index) const
{
    return d_data->d_states[index] >> e_GENERATION_COUNT_SHIFT;
}

ElementState AtomicRingBufferState::elementState(unsigned int index) const
{
    return static_cast<ElementState>(
                              d_data->d_states[index] & e_ELEMENT_STATE_MASK);
}

unsigned int AtomicRingBufferState::maxGeneration() const
{
    return d_data->d_maxGeneration;
}

unsigned int AtomicRingBufferState::maxCombinedIndex() const
{
    return d_data->d_maxCombinedIndex;
}

unsigned int AtomicRingBufferState::capacity() const
{
            ...
            for (int i = 0; i < NUM_DATA; ++i) {
                ...
                for (int i = 0; i < NUM_WRITERS; ++i) {
                    int rc = bcemt_ThreadUtil::create(
                        &handles[thread],
                        bdef_BindUtil::bind(&writerThread, &x, &state, DELAY));
                ...
                for (int i = 0; i < NUM_READERS; ++i) {
                    int rc = bcemt_ThreadUtil::create(
                     &handles[thread],
                     bdef_BindUtil::bind(&readerThread, &x, &state, DELAY));
                    BSLS_ASSERT_OPT(0 == rc); // test invariant
                    ++thread;
                }
                for (int i = 0; i < NUM_EXCEPTIONS; ++i) {
                    int rc = bcemt_ThreadUtil::create(
                        &handles[thread],
                        bdef_BindUtil::bind(&exceptionThread,&x,&state,DELAY));
                    BSLS_ASSERT_OPT(0 == rc); // test invariant
                    ++thread;
                }
...
//           ======================================
        ...
        for (int i = 0; i < NUM_DATA; ++i) {
            ...
            for (int i = 0; i < NUM_PROBES; ++i) {
                bcemt_ThreadUtil::sleep(bdet_TimeInterval(delayPerIter));
                state.suspendTest();
                assertValidState(&x);
                if (veryVeryVerbose) {
                    P(X);
...
//           =========================
        ...
        for (int i = 0; i < NUM_DATA; ++i) {
            ...
            for (int i = 0; i < NUM_WRITERS; ++i) {
                int rc = bcemt_ThreadUtil::create(
                  &handles[thread],
                  bdef_BindUtil::bind(&writerThread, &x, &writeCount, &state));
                BSLS_ASSERT_OPT(0 == rc); // test invariant
                ++thread;
            }
            for (int i = 0; i < NUM_READERS; ++i) {
                int rc = bcemt_ThreadUtil::create(
                  &handles[thread],
                  bdef_BindUtil::bind(&readerThread, &x, &readCount, &state));
                BSLS_ASSERT_OPT(0 == rc); // test invariant
                ++thread;
            }
