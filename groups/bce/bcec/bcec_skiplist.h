...
//
//@CLASSES:
//  bcec_SkipList:           generic thread-aware ordered map
//  bcec_SkipListPair:       type for opaque pointers
//  bcec_SkipListPairHandle: scope mechanism for safe item references
//
//@SEE_ALSO:
//
...
// basis of their 'KEY' value.  In addition, 'bcec_SkipList' provides methods
...
//
//   * PairHandle: An object (of type 'bcec_SkipListPairHandle') with scope
...
///------------------------------------------------------
// All methods of 'bcec_SkipList' that result in a search through the list
...
///Thread Safety
///-------------
// 'bcec_SkipList' is thread-safe and thread-aware; that is, multiple
...
//
// 'bcec_SkipListPairHandle' is only *const* *thread-safe*.  It is not safe
// for multiple threads to invoke non-const methods on the same PairHandle
// object concurrently.
//
// 'bcec_SkipListPair' is a name used for opaque pointers; the concept of
// thread safety does not apply to it.
//
///Exception Safety
///----------------
// 'bcec_SkipList' is exception-neutral: no method invokes 'throw' or 'catch'.
...
// list.
//
// No method of 'bcec_SkipListPairHandle' can throw.
//
// 'bcec_SkipListPair' is only a name used for opaque pointers; the concept of
// exception safety does not apply to it.
//
///Usage Example
///-------------
// The "R" methods of 'bcec_SkipList' make it ideal for use in a scheduler,
...
// enabled also makes 'bcec_SkipList' well-suited to use in a scheduler - a
...
// of a simple scheduler class using 'bcec_SkipList'.  Note that the mutex in
// the scheduler is used only in connection with the scheduler's condition
// variable - thread-safe access to the 'bcec_SkipList' object does *not*
...
//            {
//                bcemt_UnLockGuard<bcemt_Mutex> g(&d_condMutex);
...
struct bcec_SkipList_Control {
    ...
    int level() const;
       // Return the level stored in this control word.
                       ...
                       // ==============================
                       // local class bcec_SkipList_Node
                       // ==============================

template<class KEY, class DATA>
struct bcec_SkipList_Node {
    ...
    int level() const;
                ...
                // ==============================================
                // local class bcec_SkipList_RandomLevelGenerator
                // ==============================================
...
class bcec_SkipList_RandomLevelGenerator {
    ...
    // DATA
    bces_AtomicInt d_seed;        // current random seed

    bces_AtomicInt d_randomBits;  // 14 random bits and a sentinel bit at
                                  // the 15th position

  public:
    // CREATORS
    bcec_SkipList_RandomLevelGenerator();
    ...
    int randomLevel();
       // Return a random integer between 0 and BCEC_MAX_LEVEL.
                        ...
                        // ==================================
                        // local class bcec_SkipList_PoolUtil
                        // ==================================

class bcec_SkipList_PoolManager;

struct bcec_SkipList_PoolUtil {
    // This component-private utility handles the lock-free pool of list
    // nodes.

    // TYPES
    typedef bcec_SkipList_PoolManager PoolManager;
    ...
    static void deletePoolManager(bslma_Allocator *basicAllocator,
       ...
       // 'poolManager' was not allocated from 'basicAllocator'.
                 ...
                 // ============================================
                 // local class bcec_SkipList_NodeCreationHelper
                 // ============================================

template<class KEY, class DATA>
class bcec_SkipList_NodeCreationHelper {
    // This component-private structure is a scoped guard that initializes
    // new nodes and releases them in case of exception.

    // PRIVATE TYPES
    typedef bcec_SkipList_PoolManager     PoolManager;
    typedef bcec_SkipList_PoolUtil        PoolUtil;
  ...
  public:
    ...
    ~bcec_SkipList_NodeCreationHelper();
    ...
    void construct(const KEY& key, const DATA& data);
      ...
      // scoped guard object.
                          ...
                          // =======================
                          // class bcec_SkipListPair
                          // =======================

template <class KEY, class DATA>
class bcec_SkipListPair {
    // Pointers to objects of this class are used in the "raw" API of
    // 'bcec_SkipList'; however, objects of the class are never constructed as
  ...
  private:
    // NOT IMPLEMENTED
    bcec_SkipListPair();
    bcec_SkipListPair(const bcec_SkipListPair&);
    bcec_SkipListPair& operator=(const bcec_SkipListPair&);
  ...
  public:
    ...
    DATA& data() const;
        // Return a reference to the modifiable "data" of this pair.
                      ...
                      // =============================
                      // class bcec_SkipListPairHandle
                      // =============================

template <class KEY, class DATA>
class bcec_SkipListPairHandle {
    // Objects of this class refer to an association (pair) in a
    // 'bcec_SkipList'.  A 'PairHandle' is implicitly convertible to a
    // 'const Pair*' and thus may be used anywhere in the 'bcec_SkipList' API
    // that a 'const Pair*' is expected.

    // PRIVATE TYPES
    typedef bcec_SkipListPair<KEY, DATA> Pair;

    // DATA
    bcec_SkipList<KEY, DATA> *d_list_p;
    Pair                     *d_node_p;

    // FRIENDS
    friend class bcec_SkipList<KEY, DATA>;

  private:
    // PRIVATE MANIPULATORS
    bcec_SkipListPairHandle(bcec_SkipList<KEY, DATA> *list,
                            Pair                     *reference);
    ...
    void reset(const bcec_SkipList<KEY, DATA> *list,
               Pair                           *reference);
  ...
  public:
    // CREATORS
    bcec_SkipListPairHandle();
          // Construct a new PairHandle that does not refer to a pair.

    bcec_SkipListPairHandle(const bcec_SkipListPairHandle& original);
    ...
    // MANIPULATORS
    bcec_SkipListPairHandle& operator=(const bcec_SkipListPairHandle& rhs);
    ...
    void releaseReferenceRaw(bcec_SkipList<KEY, DATA> **list,
                             Pair                     **reference);
...
class bcec_SkipList {
  ...
  public:
    ...
    // TYPES
    typedef bcec_SkipListPair<KEY, DATA>       Pair;
    typedef bcec_SkipListPairHandle<KEY, DATA> PairHandle;
  ...
  private:
    ...
    typedef bcec_SkipList_Node<KEY, DATA>      Node;
    typedef bcec_SkipList_NodeCreationHelper<KEY, DATA>
                                               NodeGuard;

    typedef bcemt_Mutex                        Lock;
    typedef bcemt_LockGuard<bcemt_Mutex>       LockGuard;

    // DATA
    bcec_SkipList_RandomLevelGenerator         d_rand;

    bces_AtomicInt                             d_listLevel;
    Node                                      *d_head_p;
    Node                                      *d_tail_p;
    ...
    // FRIENDS
    friend class bcec_SkipListPair<KEY, DATA>;
    friend class bcec_SkipListPairHandle<KEY, DATA>;
    friend bool operator==<> (const bcec_SkipList<KEY, DATA>& lhs,
                              const bcec_SkipList<KEY, DATA>& rhs);
    friend bool operator!=<> (const bcec_SkipList<KEY, DATA>& lhs,
                              const bcec_SkipList<KEY, DATA>& rhs);
  ...
  public:
    ...
    // MANIPULATORS
    bcec_SkipList& operator=(const bcec_SkipList& rhs);
...
bsl::ostream& operator<<(bsl::ostream&                   stream,
                         const bcec_SkipList<KEY, DATA>& list);
...
template <class KEY, class DATA>
inline
const KEY& bcec_SkipListPair<KEY, DATA>::key() const
{
    return bcec_SkipList<KEY, DATA>::key(this);
}

template <class KEY, class DATA>
inline
DATA& bcec_SkipListPair<KEY, DATA>::data() const
{
    return bcec_SkipList<KEY, DATA>::data(this);
}
...
template <class KEY, class DATA>
inline
bcec_SkipListPairHandle<KEY, DATA>::bcec_SkipListPairHandle()
: d_list_p(0)
, d_node_p(0)
{
}
...
inline
bcec_SkipListPairHandle<KEY, DATA>::bcec_SkipListPairHandle(
                                       const bcec_SkipListPairHandle& original)
...
template <class KEY, class DATA>
inline
void bcec_SkipListPairHandle<KEY, DATA>::release()
{
    if (d_node_p) {
...
template <class KEY, class DATA>
inline
bcec_SkipListPairHandle<KEY, DATA>&
bcec_SkipListPairHandle<KEY, DATA>::operator=(
                                            const bcec_SkipListPairHandle& rhs)
{
...
template <class KEY, class DATA>
inline
void bcec_SkipListPairHandle<KEY, DATA>::releaseReferenceRaw(
                                          bcec_SkipList<KEY, DATA> **list,
                                          Pair                     **reference)
{
    *list      = d_list_p;
...
template <class KEY, class DATA>
inline
void bcec_SkipListPairHandle<KEY, DATA>::reset(
                                    const bcec_SkipList<KEY, DATA> *list,
                                    Pair                           *reference)
{
    release();
    d_list_p = const_cast<bcec_SkipList<KEY, DATA> *>(list);
    d_node_p = reference;
}
...
template <class KEY, class DATA>
inline
bool bcec_SkipListPairHandle<KEY, DATA>::isValid() const
{
...
template <class KEY, class DATA>
inline
const KEY& bcec_SkipListPairHandle<KEY, DATA>::key() const
{
    BSLS_ASSERT_SAFE(isValid());

    return bcec_SkipList<KEY, DATA>::key(d_node_p);
}

template <class KEY, class DATA>
inline
DATA& bcec_SkipListPairHandle<KEY, DATA>::data() const
{
    BSLS_ASSERT_SAFE(isValid());

    return bcec_SkipList<KEY, DATA>::data(d_node_p);
}
...
inline
bcec_SkipListPairHandle<KEY, DATA>::
                           operator const bcec_SkipListPair<KEY, DATA>*() const
{
    return d_node_p;
}

                             // ------------------------
                             // class bcec_SkipList_Node
                             // ------------------------

template<class KEY, class DATA>
inline
void bcec_SkipList_Node<KEY, DATA>::initControlWord(int level)
{
...
template<class KEY, class DATA>
inline
int bcec_SkipList_Node<KEY, DATA>::level() const
{
...
template<class KEY, class DATA>
inline
int bcec_SkipList_Node<KEY, DATA>::incrementRefCount()
{
...
template<class KEY, class DATA>
inline
int bcec_SkipList_Node<KEY, DATA>::decrementRefCount()
{
...
template<class KEY, class DATA>
inline
bcec_SkipList_NodeCreationHelper<KEY, DATA>::
                                            ~bcec_SkipList_NodeCreationHelper()
{
    if (d_node_p) {
        if (d_keyFlag) {
...
template<class KEY, class DATA>
inline
void bcec_SkipList_NodeCreationHelper<KEY, DATA>::construct(const KEY&  key,
                                                            const DATA& data)
{
...
// PRIVATE MANIPULATORS
template<class KEY, class DATA>
void bcec_SkipList<KEY, DATA>::addNode(bool *newFrontFlag, Node *node)
{
...
template<class KEY, class DATA>
void bcec_SkipList<KEY, DATA>::addNodeImpR(bool *newFrontFlag,
                                           Node *node,
                                           bool  lock)
{
...
template<class KEY, class DATA>
inline
void bcec_SkipList<KEY, DATA>::addNodeR(bool *newFrontFlag, Node *node)
{
    addNodeImpR(newFrontFlag, node, true);  // true -> lock
}

template<class KEY, class DATA>
int bcec_SkipList<KEY, DATA>::addNodeUnique(bool *newFrontFlag, Node *node)
{
    LockGuard guard(&d_lock);
...
template<class KEY, class DATA>
int bcec_SkipList<KEY, DATA>::addNodeUniqueR(bool *newFrontFlag, Node *node)
{
    LockGuard guard(&d_lock);
...
template<class KEY, class DATA>
bcec_SkipList_Node<KEY, DATA> *
bcec_SkipList<KEY, DATA>::allocateNode(int         level,
                                       const KEY&  key,
                                       const DATA& data)
{
...
template<class KEY, class DATA>
void bcec_SkipList<KEY, DATA>::initialize()
{
...
template<class KEY, class DATA>
void bcec_SkipList<KEY, DATA>::insertImp(bool *isNewFront,
                                         Node *location[],
                                         Node *node)
{
...
template<class KEY, class DATA>
void bcec_SkipList<KEY, DATA>::moveImp(bool *isNewFront,
                                       Node *location[],
                                       Node *node)
{
    int level = node->level();
...
template<class KEY, class DATA>
bcec_SkipList_Node<KEY, DATA> *bcec_SkipList<KEY, DATA>::popFrontImp()
{
    LockGuard guard(&d_lock);
...
template<class KEY, class DATA>
inline
void bcec_SkipList<KEY, DATA>::releaseNode(Node *node)
{
...
template<class KEY, class DATA>
int bcec_SkipList<KEY, DATA>::removeAllImp(bsl::vector<Pair *> *removed,
                                           bool                 unlock)
{
    Node *p = d_head_p;
...
template<class KEY, class DATA>
int bcec_SkipList<KEY, DATA>::removeNode(Node *node)
{
    LockGuard guard(&d_lock);
...
template<class KEY, class DATA>
int bcec_SkipList<KEY, DATA>::updateNode(bool       *isNewFront,
                                         Node       *node,
                                         const KEY&  key,
                                         bool        allowDuplicates)
{
...
template<class KEY, class DATA>
int bcec_SkipList<KEY, DATA>::updateNodeR(bool       *isNewFront,
                                          Node       *node,
                                          const KEY&  key,
                                          bool        allowDuplicates)
{
...
template<class KEY, class DATA>
bcec_SkipList_Node<KEY, DATA> *
bcec_SkipList<KEY, DATA>::backNode() const
{
    LockGuard guard(&d_lock);
...
template<class KEY, class DATA>
bcec_SkipList_Node<KEY, DATA> *bcec_SkipList<KEY, DATA>::findNode(
                                                          const KEY& key) const
{
...
template<class KEY, class DATA>
bcec_SkipList_Node<KEY, DATA> *bcec_SkipList<KEY, DATA>::findNodeR(
                                                          const KEY& key) const
{
    Node *locator[BCEC_MAX_NUM_LEVELS];
...
template<class KEY, class DATA>
bcec_SkipList_Node<KEY, DATA> *bcec_SkipList<KEY, DATA>::frontNode() const
{
    LockGuard guard(&d_lock);
...
template<class KEY, class DATA>
void bcec_SkipList<KEY, DATA>::lookupImp(Node *update[], const KEY& key) const
{
    Node *p = d_head_p;
...
template<class KEY, class DATA>
void bcec_SkipList<KEY, DATA>::lookupImpR(Node *update[], const KEY& key) const
{
    Node *q = d_tail_p;
    for (int k = d_listLevel; k >= 0; --k) {
...
template<class KEY, class DATA>
bcec_SkipList_Node<KEY, DATA> *
bcec_SkipList<KEY, DATA>::nextNode(Node *node) const
{
...
template<class KEY, class DATA>
bcec_SkipList_Node<KEY, DATA> *
bcec_SkipList<KEY, DATA>::prevNode(Node *node) const
{
    BSLS_ASSERT(node != d_head_p && node != d_tail_p);
...
template<class KEY, class DATA>
int bcec_SkipList<KEY, DATA>::skipBackward(Node **node_p) const
{
    Node *node = *node_p;
    BSLS_ASSERT(node);
    BSLS_ASSERT(node != d_head_p && node != d_tail_p);
...
template<class KEY, class DATA>
int bcec_SkipList<KEY, DATA>::skipForward(Node **node_p) const
{
    Node *node = *node_p;
    BSLS_ASSERT(node);
    BSLS_ASSERT(node != d_head_p && node != d_tail_p);
...
template<class KEY, class DATA>
inline
const KEY& bcec_SkipList<KEY, DATA>::key(const Pair *reference)
{
...
template<class KEY, class DATA>
inline
DATA& bcec_SkipList<KEY, DATA>::data(const Pair *reference)
{
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::level(const Pair *reference)
{
    Node *node = (Node *)(void *)const_cast<Pair *>(reference);
    return node->level();
}
...
template<class KEY, class DATA>
bcec_SkipList<KEY, DATA>::~bcec_SkipList()
{
...
// MANIPULATORS
template<class KEY, class DATA>
bcec_SkipList<KEY, DATA>&
bcec_SkipList<KEY, DATA>::operator=(const bcec_SkipList& rhs)
{
    if (&rhs == this) {
        return *this;
    }
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::popFront(PairHandle *item)
{
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::popFrontRaw(Pair **handle)
{
    Node *node = popFrontImp();
    if (!node) {
        return BCEC_NOT_FOUND;
...
template<class KEY, class DATA>
inline
void bcec_SkipList<KEY, DATA>::releaseReferenceRaw(const Pair *reference)
{
    Node *node = (Node *)(void *)const_cast<Pair *>(reference);
    releaseNode(node);
}
...
template<class KEY, class DATA>
inline
void bcec_SkipList<KEY, DATA>::add(PairHandle  *result,
                                   const KEY&   key,
                                   const DATA&  data,
                                   bool        *newFrontFlag)
{
...
template<class KEY, class DATA>
inline
void bcec_SkipList<KEY, DATA>::add(const KEY&   key,
                                   const DATA&  data,
                                   bool        *newFrontFlag)
{
...
template<class KEY, class DATA>
inline
void bcec_SkipList<KEY, DATA>::addAtLevelRaw(Pair        **result,
                                             int           level,
                                             const KEY&    key,
                                             const DATA&   data,
                                             bool         *newFrontFlag)
...
template<class KEY, class DATA>
int bcec_SkipList<KEY, DATA>::addAtLevelUniqueRaw(Pair        **result,
                                                  int           level,
                                                  const KEY&    key,
                                                  const DATA&   data,
                                                  bool         *newFrontFlag)
...
template<class KEY, class DATA>
inline
void bcec_SkipList<KEY, DATA>::addRaw(Pair        **result,
                                      const KEY&    key,
                                      const DATA&   data,
                                      bool         *newFrontFlag)
{
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::addUnique(PairHandle  *result,
                                        const KEY&   key,
                                        const DATA&  data,
                                        bool        *newFrontFlag)
{
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::addUnique(const KEY&   key,
                                        const DATA&  data,
                                        bool        *newFrontFlag)
{
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::addUniqueRaw(Pair        **result,
                                           const KEY&    key,
                                           const DATA&   data,
                                           bool         *newFrontFlag)
{
...
template<class KEY, class DATA>
inline
void bcec_SkipList<KEY, DATA>::addR(PairHandle  *result,
                                    const KEY&   key,
                                    const DATA&  data,
                                    bool        *newFrontFlag)
{
...
template<class KEY, class DATA>
inline
void bcec_SkipList<KEY, DATA>::addR(const KEY&   key,
                                    const DATA&  data,
                                    bool        *newFrontFlag)
{
...
template<class KEY, class DATA>
int bcec_SkipList<KEY, DATA>::addUniqueR(PairHandle  *result,
                                         const KEY&   key,
                                         const DATA&  data,
                                         bool        *newFrontFlag)
{
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::addUniqueR(const KEY&   key,
                                         const DATA&  data,
                                         bool        *newFrontFlag)
{
...
template<class KEY, class DATA>
inline
void bcec_SkipList<KEY, DATA>::addRawR(Pair        **result,
                                       const KEY&    key,
                                       const DATA&   data,
                                       bool         *newFrontFlag)
{
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::addUniqueRawR(Pair        **result,
                                            const KEY&    key,
                                            const DATA&   data,
                                            bool         *newFrontFlag)
{
...
template<class KEY, class DATA>
int bcec_SkipList<KEY, DATA>::addAtLevelUniqueRawR(Pair        **result,
                                                   int           level,
                                                   const KEY&    key,
                                                   const DATA&   data,
                                                   bool         *newFrontFlag)
...
template<class KEY, class DATA>
inline
void bcec_SkipList<KEY, DATA>::addAtLevelRawR(Pair        **result,
                                              int           level,
                                              const KEY&    key,
                                              const DATA&   data,
                                              bool         *newFrontFlag)
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::remove(const Pair *reference)
{
    if (0 == reference) {
        return BCEC_INVALID;
    }
...
template<class KEY, class DATA>
int bcec_SkipList<KEY, DATA>::removeAll(bsl::vector<PairHandle> *removed)
{
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::removeAllRaw(bsl::vector<Pair *> *removed)
{
    d_lock.lock();
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::update(const Pair  *reference,
                                     const KEY&   newKey,
                                     bool        *newFrontFlag,
                                     bool         allowDuplicates)
{
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::updateR(const Pair  *reference,
                                      const KEY&   newKey,
                                      bool        *newFrontFlag,
                                      bool         allowDuplicates)
{
...
template<class KEY, class DATA>
inline
bcec_SkipListPair<KEY, DATA> *
bcec_SkipList<KEY, DATA>::addPairReferenceRaw(const Pair *reference) const
{
    Node *node = (Node *)(void *)const_cast<Pair *>(reference);
    node->incrementRefCount();
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::back(PairHandle *back) const
{
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::backRaw(Pair **back) const
{
...
template<class KEY, class DATA>
bool bcec_SkipList<KEY, DATA>::exists(const KEY& key) const
{
    Node *locator[BCEC_MAX_NUM_LEVELS];
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::findRaw(Pair **item, const KEY& key) const
{
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::findRRaw(Pair **item, const KEY& key) const
{
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::find(PairHandle *item, const KEY& key) const
{
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::findR(PairHandle *item, const KEY& key) const
{
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::frontRaw(Pair **front) const
{
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::front(PairHandle *front) const
{
...
template<class KEY, class DATA>
inline
bool bcec_SkipList<KEY, DATA>::isEmpty() const
{
    LockGuard guard(&d_lock);
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::length() const
{
    LockGuard guard(&d_lock);
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::nextRaw(Pair **next, const Pair *reference) const
{
    Node *node = (Node *)(void *)const_cast<Pair *>(reference);
...
inline
int
bcec_SkipList<KEY, DATA>::next(PairHandle *next, const Pair *reference) const
{
    if (0 == reference) {
        return BCEC_INVALID;
    }
...
inline
int
bcec_SkipList<KEY, DATA>::previousRaw(Pair       **prevPair,
...
inline
int
bcec_SkipList<KEY, DATA>::previous(PairHandle *prevPair,
                                   const Pair *reference) const
{
    if (0 == reference) {
        return BCEC_INVALID;
...
template<class KEY, class DATA>
bsl::ostream&
bcec_SkipList<KEY, DATA>::print(bsl::ostream& stream,
                                int           level,
                                int           spacesPerLevel) const
{
    ...
    if (0 <= spacesPerLevel) {
        ...
        for (Node *node = d_head_p->d_ptrs[0].d_next_p;
            ...
            bdeu_Print::indent(stream, levelPlus2, spacesPerLevel);
        ...
        for (Node *node = d_head_p->d_ptrs[0].d_next_p;
            ...
            bdeu_PrintMethods::print(stream, node->d_key, 0, -1);
            stream << " => ";
            bdeu_PrintMethods::print(stream, node->d_data, 0, -1);
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::skipBackward(PairHandle *handle) const
{
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::skipForward(PairHandle *handle) const
{
    BSLS_ASSERT_SAFE(handle->isValid());
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::skipForwardRaw(Pair **handle) const
{
...
template<class KEY, class DATA>
inline
int bcec_SkipList<KEY, DATA>::skipBackwardRaw(Pair **handle) const
{
    BSLS_ASSERT_SAFE(handle);
                ...
                const bcec_SkipList<KEY, DATA>& rhs)
{
    ...
    bcemt_LockGuard<bcemt_Mutex> lhsGuard(&lhs.d_lock);
    bcemt_LockGuard<bcemt_Mutex> rhsGuard(&rhs.d_lock);
    ...
    for (bcec_SkipList_Node<KEY, DATA>
