// bcec_multipriorityqueue.h              -*-C++-*-
#ifndef INCLUDED_BCEC_MULTIPRIORITYQUEUE
#define INCLUDED_BCEC_MULTIPRIORITYQUEUE
...
//
//@CLASSES:
//  bcec_MultipriorityQueue: thread-enabled, parameterized multi-priority queue
//
...
// 'bcec_MultipriorityQueue', implementing a special-purpose priority
...
//
///Thread-Enabled Idioms in the 'bcec_MultipriorityQueue' Interface
///----------------------------------------------------------------
// The thread-enabled 'bcec_MultipriorityQueue' is, in many regards, similar
...
// 'bdec_MultipriorityQueue<TYPE>' type currently does not support any
...
///----------------------------
// In addition to 'popFront' and 'tryPopFront', a 'bcec_MultipriorityQueue'
...
///- - - - - - - - - - - - - - -
// This example demonstrates how we might use a 'bcec_MultipriorityQueue' to
...
// 'bcec_MultipriorityQueue' guarantees that each thread gets a unique
...
//  }
//..
// The 'myConsumerThread' function below is a callback for 'bcemt_ThreadUtil',
// which requires a "C" signature.  'bcemt_ThreadUtil::create()' expects
...
// required type ('bcec_MultipriorityQueue<MyWorkRequest> *'), and then
...
// roles: it creates the 'bcec_MultipriorityQueue', starts the consumer
...
// 'bcemt_thread' component-level documentation for details):
//..
...
//
//      bcec_MultipriorityQueue<MyWorkRequest> queue(NUM_PRIORITIES);
//
...
// pointer to 'bcec_MultipriorityQueue<MyWorkRequest>'.
//
...
// 'bcemt_ThreadUtil::create' takes the traditional 'void' pointer.  The
...
//
//      bcec_MultipriorityQueue<MyEvent> queue(NUM_PRIORITIES);
//
//      assert(0 < NUM_THREADS && NUM_THREADS <= MAX_CONSUMER_THREADS);
...
class bslma_Allocator;
              ...
              // ==============================================
              // local class bcec_MultipriorityQueue_Node<TYPE>
              // ==============================================

template <typename TYPE>
class bcec_MultipriorityQueue_Node {
    ...
    // NOT IMPLEMENTED
    bcec_MultipriorityQueue_Node(const bcec_MultipriorityQueue_Node&);
    bcec_MultipriorityQueue_Node& operator=(
                                 const bcec_MultipriorityQueue_Node&);
  ...
  public:
    ...
    const TYPE& item() const;
        // Return a reference to the non-modifiable item stored in this node.
                     ...
                     // ===================================
                     // class bcec_MultipriorityQueue<TYPE>
                     // ===================================

template <typename TYPE>
class bcec_MultipriorityQueue {
    ...
    // PRIVATE TYPES
    typedef bcec_MultipriorityQueue_Node<TYPE> Node;
    ...
    // DATA
    mutable bcemt_Mutex d_mutex;          // used to synchronize access
                                          // (including 'const' access)

    bcemt_Condition     d_notEmptyCondition;
    ...
    bcema_Pool          d_pool;           // memory pool used for node storage
    ...
    bcec_MultipriorityQueue& operator=(const bcec_MultipriorityQueue&);
  ...
  public:
    ...
    void pushBackMultipleRaw(const TYPE& item,
        ...
        // 'bcep_MultipriorityThreadPool'.  The behavior is undefined unless
        // '0 <= itemPriority < numPriorities()'.
...
template <typename TYPE>
inline
bcec_MultipriorityQueue_Node<TYPE>*&
                                bcec_MultipriorityQueue_Node<TYPE>::nextPtr()
{
    return d_next_p;
}
...
template <typename TYPE>
inline
const bcec_MultipriorityQueue_Node<TYPE> *
                            bcec_MultipriorityQueue_Node<TYPE>::nextPtr() const
{
    return d_next_p;
}
...
template <typename TYPE>
inline
const TYPE& bcec_MultipriorityQueue_Node<TYPE>::item() const
{
...
// PRIVATE MANIPULATORS
template <typename TYPE>
int bcec_MultipriorityQueue<TYPE>::tryPopFrontImpl(TYPE *item,
                                                   int  *itemPriority,
                                                   bool  blockFlag)
{
...

    {
        ...
        priority = bdes_BitUtil::find1AtSmallestIndex(d_notEmptyFlags);
...
// MANIPULATORS
template <typename TYPE>
int bcec_MultipriorityQueue<TYPE>::pushBack(const TYPE& item,
                                            int         itemPriority)
{
    enum { BCEC_SUCCESS = 0, BCEC_FAILURE = -1 };
...
template <typename TYPE>
void bcec_MultipriorityQueue<TYPE>::pushFrontMultipleRaw(
                                                    const TYPE& item,
                                                    int         itemPriority,
                                                    int         numItems)
{
...
template <typename TYPE>
void bcec_MultipriorityQueue<TYPE>::pushBackMultipleRaw(
                                                    const TYPE& item,
                                                    int         itemPriority,
                                                    int         numItems)
{
...
template <typename TYPE>
inline
int bcec_MultipriorityQueue<TYPE>::tryPopFront(TYPE *item, int *itemPriority)
{
...
template <typename TYPE>
inline
void bcec_MultipriorityQueue<TYPE>::popFront(TYPE *item, int *itemPriority)
{
    tryPopFrontImpl(item, itemPriority, true);
}

template <typename TYPE>
void bcec_MultipriorityQueue<TYPE>::removeAll()
{
...

    {
        ...
        while (d_notEmptyFlags) {
            const int priority =
                           bdes_BitUtil::find1AtSmallestIndex(d_notEmptyFlags);
...
template <typename TYPE>
inline
void bcec_MultipriorityQueue<TYPE>::enable()
{
    bcemt_LockGuard<bcemt_Mutex> lock(&d_mutex);
...
template <typename TYPE>
inline
void bcec_MultipriorityQueue<TYPE>::disable()
{
    bcemt_LockGuard<bcemt_Mutex> lock(&d_mutex);
...
template <typename TYPE>
inline
int bcec_MultipriorityQueue<TYPE>::length() const
{
    return d_length;
}
...
template <typename TYPE>
inline
bool bcec_MultipriorityQueue<TYPE>::isEmpty() const
{
    return 0 == d_length;
}
...
template <typename TYPE>
inline
bool bcec_MultipriorityQueue<TYPE>::isEnabled() const
{
