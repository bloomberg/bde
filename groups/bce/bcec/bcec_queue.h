...
class bcec_Queue {
    ...
    // PRIVATE TYPES
    typedef typename bdec_Queue<TYPE>::InitialCapacity QueueCapacity;
    ...
    bdec_Queue<TYPE> d_queue;             // the queue, with allocator as last
                                          // data member
  ...
  public:
    ...
    bcemt_Condition& insertCondition();
        // Return a reference to the modifiable condition variable used by this
    ...
    bcemt_Mutex& mutex();
        ...
        // synchronize access to its underlying 'bdec_Queue' object.
    ...
    bcemt_Condition& notFullCondition();
        // Return the condition variable used by this queue to signal that the
        // queue is not full (i.e., has fewer items than its high-water
        // mark).

    bdec_Queue<TYPE>& queue();
        // Return a reference to the modifiable underlying 'bdec_Queue' object
...
template <class TYPE>
inline
bcec_Queue<TYPE>::~bcec_Queue()
{
}

// MANIPULATORS
template <class TYPE>
void bcec_Queue<TYPE>::popBack(TYPE *buffer)
{
    unsigned int length;
...
template <class TYPE>
int bcec_Queue<TYPE>::timedPopBack(TYPE                     *buffer,
...
template <class TYPE>
void bcec_Queue<TYPE>::popFront(TYPE *buffer)
{
    unsigned int length;
...
template <class TYPE>
int bcec_Queue<TYPE>::timedPopFront(TYPE                     *buffer,
                                    const bdet_TimeInterval&  timeout)
{
    unsigned int length;
...
template <class TYPE>
int bcec_Queue<TYPE>::tryPopFront(TYPE *buffer)
{
    unsigned int length;
...
template <class TYPE>
void bcec_Queue<TYPE>::tryPopFront(int maxNumItems, bsl::vector<TYPE> *buffer)
{
    bcemt_LockGuard<bcemt_Mutex> lock(&d_mutex);
...
template <class TYPE>
int bcec_Queue<TYPE>::tryPopBack(TYPE *buffer)
{
    unsigned int length;
...
template <class TYPE>
void bcec_Queue<TYPE>::tryPopBack(int maxNumItems, bsl::vector<TYPE> *buffer)
{
    bcemt_LockGuard<bcemt_Mutex> lock(&d_mutex);
...
template <class TYPE>
void bcec_Queue<TYPE>::removeAll(bsl::vector<TYPE> *buffer)
{
    bcemt_LockGuard<bcemt_Mutex> lock(&d_mutex);
...
template <class TYPE>
void bcec_Queue<TYPE>::pushBack(const TYPE& item)
{
    bcemt_LockGuard<bcemt_Mutex> lock(&d_mutex);
...
template <class TYPE>
void bcec_Queue<TYPE>::pushFront(const TYPE& item)
{
    bcemt_LockGuard<bcemt_Mutex> lock(&d_mutex);
    if (d_highWaterMark >= 0) {
        while (d_queue.length() >= d_highWaterMark) {
...
template <class TYPE>
int bcec_Queue<TYPE>::timedPushBack(const TYPE&              item,
                                    const bdet_TimeInterval& timeout)
{
    bcemt_LockGuard<bcemt_Mutex> lock(&d_mutex);
    if (d_highWaterMark >= 0) {
...
template <class TYPE>
int bcec_Queue<TYPE>::timedPushFront(const TYPE&              item,
                                     const bdet_TimeInterval &timeout)
{
    bcemt_LockGuard<bcemt_Mutex> lock(&d_mutex);
    if (d_highWaterMark >= 0) {
...
template <class TYPE>
inline
void bcec_Queue<TYPE>::forcePushFront(const TYPE& item)
{
    bcemt_LockGuard<bcemt_Mutex> lock(&d_mutex);
    d_queue.pushFront(item);
    d_notEmptyCondition.signal();
...
template <class TYPE>
inline
bcemt_Condition& bcec_Queue<TYPE>::condition()
{
...
template <class TYPE>
inline
bcemt_Condition& bcec_Queue<TYPE>::insertCondition()
{
...
template <class TYPE>
inline
bcemt_Mutex& bcec_Queue<TYPE>::mutex()
{
    return d_mutex;
}
...
template <class TYPE>
inline
bcemt_Condition& bcec_Queue<TYPE>::notEmptyCondition()
{
    return d_notEmptyCondition;
}
...
template <class TYPE>
inline
bcemt_Condition& bcec_Queue<TYPE>::notFullCondition()
{
    return d_notFullCondition;
}
...
template <class TYPE>
inline
bdec_Queue<TYPE>& bcec_Queue<TYPE>::queue()
{
    return d_queue;
}
