...
// non-'const' member function for every 'bcefi_Vfunc9MN' class.  Also, verify
...
//  'c_O'     for 'const OBJ'
//-----------------------------------------------------------------------------
// 'bcefi_Vfunc9Null' methods:
// [ 9] bcefi_Vfunc9Null<A1-9>(ba *);
// [ 9] void execute<A1-9>(c_A1-9&) const;
// [ 9] ~bcefi_Vfunc9Null<A1-9>();
//-----------------------------------------------------------------------------
// 'bcefi_Vfunc9F9' methods:
// [ 9] bcefi_Vfunc9F9<F, A1-9>(c_F&, ba *);
// [ 9] void execute<F, A1-9>(c_A1-9&) const;
// [ 9] ~bcefi_Vfunc9F9<F, A1-9>();
// ----------------------------------------------------------------------------
// 'bcefi_Vfunc9C9' methods:
// [ 9] bcefi_Vfunc9C9<OBJ, C, A1-9>( OBJ *, c_C&, ba *);
// [ 9] void execute<OBJ, C, A1-9>(c_A1-9&) const;
// [ 9] ~bcefi_Vfunc9C9<OBJ, C, A1-9>();
// ----------------------------------------------------------------------------
// 'bcefi_Vfunc9M9' methods:
// [ 9] bcefi_Vfunc9M9<OBJ, M, A1-9>( OBJ *, c_M&, ba *);
// [ 9] void execute<OBJ, M, A1-9>(c_A1-9&) const;
// [ 9] ~bcefi_Vfunc9M9<OBJ, M, A1-9>();
//-----------------------------------------------------------------------------
// [11] USAGE EXAMPLE
...
//       // that 'bcefi_vfunc9X4' classes pass when calling the callback
//       // function.
//       //
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      ...
      case 9: {
        ...
        //       functions by 'bcefi_Vfunc9M9' instances.
        //    5. Destructor of each class under test is properly executed.
        //
        // Plan:
        //   Create objects of every 'bcefi_Vfunc9X9' class.  Initialize
        //   'bcefi_Vfunc9F9' to have 'checkArgumentValues9' as the callback
        //   function.  Initialize 'bcefi_Vfunc9C9' and 'bcefi_Vfunc9M9' to
        //   have 'checkArgumentValues9' method of the 'CbClass' class as the
        //   callback function.  Call the 'execute' method for each
        //   'bcefi_Vfunc9X9' class.  The arguments in this call are
        ...
        //
        //   Create objects of every 'bcefi_Vfunc9X9' class.  Initialize
        //   'bcefi_Vfunc9F9' to have 'checkParameterTypes9' as the callback
        //   function.  Initialize 'bcefi_Vfunc9C9' to have
        //   'checkParameterTypesC9' method of the 'CbClass' class as the
        //   callback function.  Initialize 'bcefi_Vfunc9M9' to have
        //   'checkParameterTypesM9' method of the 'CbClass' class as the
        //   callback function.  The latter will test wether the
        //   'bcefi_Vfunc9M9' can have non-'const' functions as the callback
        //   function.
        //
        //   Having a function of 'checkParameterTypes' family as a callback
        //   function forces 'bcefi_Vfunc9X9' to be templatized so that each
        ...
        // Testing:
        //   'bcefi_Vfunc9F9' methods:
        //   bcefi_Vfunc9F9<F, A1-9>(c_F&, ba *);
        //   void execute<F, A1-9>(c_A1-9&) const;
        //   ~bcefi_Vfunc9F9<F, A1-9>();
        // --------------------------------------------------------------------
        //   'bcefi_Vfunc9C9' methods:
        //   bcefi_Vfunc9C9<OBJ, C, A1-9>( OBJ *, c_C&, ba *);
        //   void execute<OBJ, C, A1-9>(c_A1-9&) const;
        //   ~bcefi_Vfunc9C9<OBJ, C, A1-9>();
        // --------------------------------------------------------------------
        //   'bcefi_Vfunc9M9' methods:
        //   bcefi_Vfunc9M9<OBJ, M, A1-9>( OBJ *, c_M&, ba *);
        //    void execute<OBJ, M, A1-9>(c_A1-9&) const;
        //   ~bcefi_Vfunc9M9<OBJ, M, A1-9>();
        // --------------------------------------------------------------------
        //   'bcefi_Vfunc9Null' methods:
        //   bcefi_Vfunc9Null<A1-9>(ba *);
        //   void execute<A1-7>(c_A1-9&) const;
        //   ~bcefi_Vfunc9Null<A1-9>();
        // --------------------------------------------------------------------
            ...
            cout << "\nTesting constructors and argument values." << endl;
        {
            ...
            typedef void (CbClass::*CbFuncC9)(int *, int *, int *, int *,
                                              int *, int *, int *, int *,
                                              int *) const;
            typedef bcefr_Vfunc9<int *, int *, int *, int *, int *,
            ...
            if (verbose) cout << "\t'bcefi_Vfunc9F9' object." << endl;
            {
                typedef bcefi_Vfunc9F9<CbFuncF9,
                                       int *, int *, int *, int *,
            ...
            if (verbose) cout << "\t'bcefi_Vfunc9C9' object." << endl;
            {
                typedef bcefi_Vfunc9C9<CbClass, CbFuncC9, int *, int *,
                                       int *, int *, int *, int *, int *,
                                       int *, int *> Obj;
                CbClass cbObject;
                if (verbose) cout << "\t\tTesting constructor." << endl;
            ...
            if (verbose) cout << "\t'bcefi_Vfunc9M9' object." << endl;
            {
                typedef bcefi_Vfunc9M9<CbClass, CbFuncC9,
                                       int *, int *, int *, int *,
                                       int *, int *, int *, int *, int *> Obj;
                CbClass cbObject;
                if (verbose) cout << "\t\tTesting constructor." << endl;
            ...
            cout << "\nTesting parameter types and destructors." << endl;
        {
            ...
            typedef void (CbClass::*CbFuncM9)(T1, T2, T3, T4, T5, T6, T7, T8,
                                              T9);
            typedef bcefr_Vfunc9<T1, T2, T3, T4, T5, T6, T7, T8, T9> ObjBase;
            ...
            if (verbose)
                cout << "\tWhen calling 'bcefi_Vfunc9F9' 'execute' method."
                     << endl;
            {
                typedef bcefi_Vfunc9F9<CbFuncF9, T1, T2, T3, T4, T5, T6, T7,
                                       T8, T9> Obj;
                dcEnabled = 0;
            ...
            if (verbose)
                cout << "\tWhen calling 'bcefi_Vfunc9C9' 'execute' method."
                     << endl;
            {
                typedef bcefi_Vfunc9C9<CbClass, CbFuncC9, T1, T2,
            ...
            if (verbose)
                cout << "\tWhen calling 'bcefi_Vfunc9M9' 'execute' method."
                     << endl;
            {
                typedef bcefi_Vfunc9M9<CbClass, CbFuncM9, T1, T2, T3, T4,
                                       T5, T6, T7, T8, T9> Obj;
                dcEnabled = 0;
                destructorCheck = DESTRUCTOR_FLAG9;
                numDeallocations = ta.numDeallocation();
            ...
            if (verbose)
                cout << "\nTesting bcefi_Vfunc9Null object." << endl;
            {
                destructorCheck = DESTRUCTOR_FLAG9;
                numDeallocations = ta.numDeallocation();

                typedef
                    bcefi_Vfunc9Null<T1, T2, T3, T4, T5, T6, T7, T8, T9> Obj;
