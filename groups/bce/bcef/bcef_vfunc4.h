...
//
//@CLASSES:
//   bcef_Vfunc4: signature-specific function object (functor)
//
...
// returns void and takes four arguments of arbitrary type; it is invoked in a
// manner similar to that of following free function:
//..
//   void functionObject(arg1, arg2, arg3, arg4);
//..
...
// construction.
//
// The 'bcef_Vfunc4' functor (envelope) holds a polymorphic, reference-counted,
...
// signature and (2) the supplied user data.  See the 'bcefu_vfunc4' component
// for more information on populating 'bcef_Vfunc4' function objects with
...
class bcef_Vfunc4 {
    ...
    //..
    // which takes four arguments and returns void.  Note that pointer (but not
    ...
    // NOT IMPLEMENTED
    bool operator==(const bcef_Vfunc4&) const;
    bool operator!=(const bcef_Vfunc4&) const;

    template<class B1, class B2, class B3, class B4>
    bool operator==(const bcef_Vfunc4<B1, B2, B3, B4>&) const;
    template<class B1, class B2, class B3, class B4>
    bool operator!=(const bcef_Vfunc4<B1, B2, B3, B4>&) const;

  public:
    // TRAITS
    BSLALG_DECLARE_NESTED_TRAITS(bcef_Vfunc4,
    ...
    bcef_Vfunc4(bcefr_Vfunc4<A1, A2, A3, A4> *representation);
        ...
        // components (see 'bcefu_vfunc4') to create an appropriate
        // representation and load it into an existing functor.

    bcef_Vfunc4(const bcef_Vfunc4<A1, A2, A3, A4>& functionObject);
    ...
    ~bcef_Vfunc4();
        ...
        // the static 'deleteObject' method of 'bcefr_Vfunc4' base class).

    // MANIPULATORS
    bcef_Vfunc4<A1, A2, A3, A4>& operator=(
        const bcef_Vfunc4<A1, A2, A3, A4>& functionObject);
        ...
        // 'deleteObject' method of 'bcefr_Vfunc4' base class.
...
template <class A1, class A2, class A3, class A4>
inline bcef_Vfunc4<A1, A2, A3, A4>::bcef_Vfunc4()
: d_rep_p(0)
{
}
...
template <class A1, class A2, class A3, class A4>
inline
bcef_Vfunc4<A1, A2, A3, A4>::bcef_Vfunc4(
    bcefr_Vfunc4<A1, A2, A3, A4> *rep)
: d_rep_p(rep)
{
    if (d_rep_p) {
        d_rep_p->increment();
...
template <class A1, class A2, class A3, class A4>
inline bcef_Vfunc4<A1, A2, A3, A4>::bcef_Vfunc4(const bcef_Vfunc4& functor)
...
inline bcef_Vfunc4<A1, A2, A3, A4>::~bcef_Vfunc4()
{
    if (d_rep_p && 0 == d_rep_p->decrement()) {
        bcefr_Vfunc4<A1, A2, A3, A4>::deleteObject(d_rep_p);
    }
}

template <class A1, class A2, class A3, class A4> inline
bcef_Vfunc4<A1, A2, A3, A4>&
bcef_Vfunc4<A1, A2, A3, A4>::operator=(const bcef_Vfunc4<A1, A2, A3, A4>& rhs)
{
...
template <class A1, class A2, class A3, class A4>
inline void bcef_Vfunc4<A1, A2, A3, A4>::operator()(
                                                const A1& argument1,
                                                const A2& argument2,
                                                const A3& argument3,
...
template <class A1, class A2, class A3, class A4>
inline bcef_Vfunc4<A1, A2, A3, A4>::operator const void *() const
{
    return d_rep_p;
}
