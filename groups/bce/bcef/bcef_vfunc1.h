...
//
//@CLASSES:
//   bcef_Vfunc1: signature-specific function object (functor)
//
...
// construction.
//
// The 'bcef_Vfunc1' functor (envelope) holds a polymorphic, reference-counted,
...
// signature and (2) the supplied user data.  See the 'bcefu_vfunc1' component
// for more information on populating 'bcef_Vfunc1' function objects with
...
// as we use references (and occationally pointers) to pass constant
...
// callback function one argument at its invokation: a modifiable object of
// type 'MyGuiInformation'.
//..
//   class MyGuiContext {
//       int d_changedFlag;
//
...
//
//     public:
//       MyGuiButton(const bcef_Vfunc1<MyGuiInformation *>&
//                                              buttonPressCallback);
...
//   }
//..
// The 'guiInfo' argument is mandatory for every function
// called at the press of the button (the number '1' in the name of
// 'bcef_Vfunc1' class specifies the number of required arguments).  However,
...
// presented below.
//..
//   static void buttonpressFunction(MyGuiInformation *a,
...
//   }
//..
// The 'bcef_Vfunc1<MyGuiInformation *>' class used in 'MyGuiButton'
// class has a private member of an abstract class 'bcefr_vfunc1', whose
// 'execute' method is called when the functor is being called.  The 'execute'
// method of the 'bcefr_vfunc1' class, the functor invocation operator of the
// 'bcef_Vfunc1' class, and the 'pressButton' method of 'MyGuiButton' class
// have one parameter.  The 'buttonpressFunction' callback function, however,
// has to be called with two arguments.  To achieve the proper calling
// signature the user implements a concrete class derived from 'bcefr_vfunc1'
// as follows:
//..
...
//           // followed by the argument value specified at
//           // construction.
//       {
//           d_f(argument1, d_d1);
//       };
...
//
//   typedef void (*BpFun)(const MyGuiInformation*, int *);
...
//   // (4) Use the object.
//
//   MyGuiInformation guiInfo;           assert(0 == globalCounter);
//                                       assert(0 == gc.isChanged());
//   button.pressButton(&guiInfo);       assert(1 == globalCounter);
//                                       assert(1 == gc.isChanged());
//   button.pressButton(&guiInfo);       assert(2 == globalCounter);
...
class bcef_Vfunc1 {
    ...
    bcefr_Vfunc1<A1> *d_rep_p;  // polymorphic functor representation

    // NOT IMPLEMENTED
    bool operator==(const bcef_Vfunc1&) const;
    bool operator!=(const bcef_Vfunc1&) const;

    template<class B1>
    bool operator==(const bcef_Vfunc1<B1>&) const;
    template<class B1>
    bool operator!=(const bcef_Vfunc1<B1>&) const;

  public:
    // TRAITS
    BSLALG_DECLARE_NESTED_TRAITS(bcef_Vfunc1,
    ...
    bcef_Vfunc1(bcefr_Vfunc1<A1> *representation);
        ...
        // components (see 'bcefu_vfunc1') to create an appropriate
        // representation and load it into an existing functor.

    bcef_Vfunc1(const bcef_Vfunc1<A1>& functionObject);
    ...
    ~bcef_Vfunc1();
        ...
        // the static 'deleteObject' method of 'bcefr_Vfunc1' base class).

    // MANIPULATORS
    bcef_Vfunc1<A1>& operator=(const bcef_Vfunc1<A1>& functionObject);
        ...
        // 'deleteObject' method of 'bcefr_Vfunc1' base class.
...
template <class A1>
inline bcef_Vfunc1<A1>::bcef_Vfunc1()
: d_rep_p(0)
{
}

template <class A1>
inline bcef_Vfunc1<A1>::bcef_Vfunc1(bcefr_Vfunc1<A1> *rep)
: d_rep_p(rep)
{
    if (d_rep_p) {
        d_rep_p->increment();
...
template <class A1>
inline bcef_Vfunc1<A1>::bcef_Vfunc1(const bcef_Vfunc1& functor)
...
bcef_Vfunc1<A1>&
bcef_Vfunc1<A1>::operator=(const bcef_Vfunc1<A1>& rhs)
{
...
template <class A1>
inline void bcef_Vfunc1<A1>::operator()(const A1& argument1) const
{
    d_rep_p->execute(argument1);
}

template <class A1>
inline bcef_Vfunc1<A1>::operator const void *() const
{
    return d_rep_p;
}
