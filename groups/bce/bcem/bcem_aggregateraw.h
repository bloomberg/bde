...
//   bcem_AggregateRaw_NameOrIndex: simple discriminated union of string, index
//
...
struct bcem_AggregateRaw_BdeatUtil;
                        ...
                        // ===================================
                        // class bcem_AggregateRaw_NameOrIndex
                        // ===================================
...
class bcem_AggregateRaw_NameOrIndex {
  ...
  public:
    // CREATORS
    bcem_AggregateRaw_NameOrIndex();
        // Create an empty name/index having no name or index.

    bcem_AggregateRaw_NameOrIndex(const char *name);
    ...
    bcem_AggregateRaw_NameOrIndex(int index);
...
class bcem_AggregateRaw {
  ...
  private:
    // PRIVATE TYPES
    typedef bcem_AggregateRaw_NameOrIndex NameOrIndex;  // short hand
    ...
    int assignToNillableScalarArrayImp(const TYPE& value) const;
        ...
        // is a scalar array, or is convertible to one.  If value is null, then
        // make this aggregate null.  Leave this aggregate unchanged if 'value'
        // is not convertible to the type stored in this aggregate.  The
        // parameterized 'TYPE' shall be either 'bdem_ElemRef' or
        // 'bdem_ConstElemRef'.
  ...
  public:
    ...
    void clearParent();
        // Make this aggregate a top-level aggregate by resetting the parent
        // information.
    ...
    int getField(bcem_AggregateRaw   *resultField,
        ...
        // unused argument results in the construction of a null 'NameOrIndex',
        // which is treated as the end of the argument list.
    ...
    fieldType(NameOrIndex fieldOrIdx1,
        ...
        // specifies a field name or array index.  Return the type of the field
        // on success, or 'bdem_ElemType::BDEM_VOID' on a navigation error.  An
        // empty string can be used for any of the 'fieldOrIdx' arguments to
        // specify the current selection within a choice object.  An unused
        // argument results in the construction of a null 'NameOrIndex', which
        // is treated as the end of the argument list.
    ...
    bdem_ElemRef fieldRef(NameOrIndex fieldOrIdx1,
        ...
        // providing modifiable access to the specified field on success or a
        // 'VOID' element reference on error.  An empty string can be used for
        // any of the arguments to specify the current selection within a
        // choice object.  An unused argument results in the construction of a
        // null 'NameOrIndex', which is treated as the end of the argument
        // list.  Note that if 'true == isNull()' for any field in the chain of
    ...
    bdem_ElemRef fieldRefById(int fieldId) const;
        // Return an element reference providing modifiable access to the field
        // specified by 'fieldId' within this aggregate or a 'VOID' element
        // reference on error.  If this aggregate refers to a choice value,
        // then a 'fieldId' of 'bdem_RecordDef::BDEM_NULL_FIELD_ID' can be used
        // to specify the current selection.  Modifying the element through the
    ...
    bdem_ElemRef fieldRefByIndex(int index) const;
        // Return an element reference providing modifiable access to the field
        // within this aggregate specified by the (zero-based) 'fieldIndex' or
        // a 'VOID' element reference on error.  If this aggregate refers to a
        // choice value, then a 'fieldIndex' of -1 can be used to specify the
        // current selection.  Modifying the element through the returned
        // reference will modify the original aggregate.  Note that if
...
// CREATORS
inline
bcem_AggregateRaw_NameOrIndex::bcem_AggregateRaw_NameOrIndex()
...
inline
bcem_AggregateRaw_NameOrIndex::bcem_AggregateRaw_NameOrIndex(const char *name)
...
inline
bcem_AggregateRaw_NameOrIndex::bcem_AggregateRaw_NameOrIndex(int index)
...
// ACCESSORS
inline
bool bcem_AggregateRaw_NameOrIndex::isEmpty() const
{
    return BCEM_NOI_EMPTY == d_state;
}

inline
bool bcem_AggregateRaw_NameOrIndex::isName() const
{
    return BCEM_NOI_NAME == d_state;
}

inline
bool bcem_AggregateRaw_NameOrIndex::isIndex() const
{
    return BCEM_NOI_INDEX == d_state;
}

inline
const char *bcem_AggregateRaw_NameOrIndex::name() const
{
    return BCEM_NOI_NAME == d_state ? d_name : 0;
}

inline
int bcem_AggregateRaw_NameOrIndex::index() const
{
    return BCEM_NOI_INDEX == d_state
                           ? d_index
...
inline
void bcem_AggregateRaw::setSchemaPointer(const bdem_Schema *schema)
{
    BSLS_ASSERT(schema);
