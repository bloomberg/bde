// bces_atomicutilimpl_amd64.t.cpp           -*-C++-*-
...
// [2 ] setInt64(bces_AtomicUtilImpl...::Int64 *, bsls_PlatformUtil::Int64);
// [2 ] getInt64(const bces_AtomicUtilImpl...::Int64 &aInt);
// [2 ] getPtr(bces_AtomicUtilImpl...::Pointer &aPointer);
// [2 ] setPtr(bces_AtomicUtilImpl...::Pointer *aPointer, void *value);
// [4 ] swapPtr(bces_AtomicUtilImpl...::Pointer *aPointer, void *value);
// [4 ] testAndSwapPtr(bces_AtomicUtilImpl...::Pointer *, void *, void *);
// [6 ] spinLock(bces_AtomicUtilImpl...::SpinLock *aSpin);
...
typedef bces_AtomicUtilImpl<bsls_Platform::CpuX86_64> Obj;
...
static Obj::Int64 transactionCount;
static Obj::Int64 successCount;
static Obj::Int64 failureCount;
...
class my_CountedHandleRep {
    Obj::Int d_count;       // number of active references
...
{
    return Obj::decrementIntNv(&d_count);
}
                        // -------------------------
                        // class my_CountedHandle
                        // -------------------------
...
{
    return d_rep_p ? Obj::getInt(d_rep_p->d_count) : 0;
}
...
class my_PtrStack {
    ...
    Obj::Pointer  d_list_p;
    Obj::Pointer  d_freeList_p;
...
{
    Obj::initPointer(&d_freeList_p,0);
    Obj::initPointer(&d_list_p,0);
}
...
typename my_PtrStack<TYPE>::Node *my_PtrStack<TYPE>::allocateNode()
{
    Node *node;
    do {
        node = (Node*)Obj::getPtr(d_freeList_p);
        if (!node) break;
    }while (Obj::testAndSwapPtr(&d_freeList_p,
                                            node,
                                            node->d_next
                                           ) != node);
    if (!node) {
...
{
    do {
        node->d_next = (Node*)Obj::getPtr(d_freeList_p);
    }while (Obj::testAndSwapPtr(&d_freeList_p,
                                            node->d_next,
                                            node
                                           ) != node->d_next);
...
inline void my_PtrStack<TYPE>::push(TYPE *item)
{
    ...
    node->d_item = item;
    do {
        node->d_next = (Node*)Obj::getPtr(d_list_p);
    }while (Obj::testAndSwapPtr(&d_list_p,
                                            node->d_next,
                                            node
                                           ) != node->d_next);
}
...
int main(int argc, char *argv[]) {
#ifndef BSLS_PLATFORM__CPU_X86_64
    ...
    switch (test) { case 0:
      ...
      case 6: {
        ...
        //   iterations.
        // Testing:
        //   initSpinLock(Obj::SpinLock *aSpin);
        //   spinLock(Obj::SpinLock *aSpin);
        //   spinTryLock(Obj::SpinLock *aSpin, int retries);
        //   spinUnlock(Obj::SpinLock *aSpin);
        // --------------------------------------------------------------------
        if (verbose) cout << "\nTesting Spinlocks"
                          << "\n================="
                          << endl;
      ...
      case 4: {
        ...
        //   swapInt64(Obj::Int64 *, bsls_PlatformUtil::Int64);
        //   testAndSwapInt64(Obj::Int64 *,
      ...
      case 3: {
        ...
        //   addInt64(Obj::Int64 *, bsls_PlatformUtil::Int64);
        //   addIntNv(Obj::Int *aInt, int value);
        //   addInt64Nv(Obj::Int64 *, bsls_PlatformUtil::Int64);
        // --------------------------------------------------------------------
      ...
      case 2: {
        ...
        //   setInt64(Obj::Int64 *, bsls_PlatformUtil::Int64);
