// bces_atomicutilimpl_intel_pentium.t.cpp  -*-C++-*-
...
typedef bces_AtomicUtilImpl<bsls_Platform::CpuX86> Obj;

const int INT_SWAPTEST_VALUE1 = 0x33ff33ff;
const int INT_SWAPTEST_VALUE2 = 0xff33ff33;

const bsls_PlatformUtil::Int64 INT64_SWAPTEST_VALUE1 = 0x33ff33ff33ff53ffLL;
const bsls_PlatformUtil::Int64 INT64_SWAPTEST_VALUE2 = 0xff33ff53ff33ff33LL;
    ...
    // have been incremented by exactly NTHREADS * NITERATIONS.
{
    ...
    for(int i=0; i < args->d_iterations; ++i) {
        ...
        int count = args->d_count;
        for (int j = 0 ; j < args->d_numIncrements; ++j) {
            args->d_count++;
        }
        ASSERT(args->d_count == (count + args->d_numIncrements));
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:
      ...
      case 6: {
        ...
        // Concerns:
        //   In addition to proper locking/unlocking, we should test that
        //   writing to spinlock (a char) does not buffer overflow.
        // Plan:
                          ...
                          << endl;
        {
            int result;
            struct {
                Obj::SpinLock d_spin;        // a char
                unsigned char d_padding[3];  // unsigned, just like 0xDE, etc.
            } mSP;
            mSP.d_padding[0] = 0xDE;
            mSP.d_padding[1] = 0xAD;
            mSP.d_padding[2] = 0xBF;

            Obj::SpinLock& mS1 = mSP.d_spin;
            ...
            ASSERT(mSP.d_padding[0] == 0xDE);
            ASSERT(mSP.d_padding[1] == 0xAD);
            ASSERT(mSP.d_padding[2] == 0xBF);
        }
        if (verbose) cout << "\nTesting Spinlocks Thread Safety"
                          << endl;
        {
            enum {
                NTHREADS    = 4,
                NITERATIONS = 1000,
                NUMINCREMENTS = 1000,
                EXPTOTAL    = NTHREADS * NITERATIONS * NUMINCREMENTS
            };
            ...
            args.d_numIncrements = NUMINCREMENTS;
      ...
      case 5: {
                          ...
                          << endl;
        {
            ...
            for (int i = 0; i < NUM_VALUES; ++i) {
                ...
                Obj::initInt64(&x,0);
                volatile double doubleValue2 = argc;
                ASSERT(0 == Obj::getInt64(X));
                volatile double doubleValue = 1;
                if (!(doubleValue >= 0)) {
                    ASSERT(0 && "FPU state incorrect");
                }
                if (doubleValue2 <= 0.9 || doubleValue2 >= 9.9) {
                    ASSERT(0 && "FPU state incorrect");
                }
