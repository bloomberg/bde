// bces_atomicutilimpl_sparc32.t.cpp  -*-C++-*-
...
typedef bces_AtomicUtilImpl<bsls_Platform::CpuSparc_32> Obj;
...
static void *setInt64TestThread(void *ptr)
    // This function is used to test the 'testAndSwapInt64' function.
{
    Int64SetTestThreadArgs *args=(Int64SetTestThreadArgs*)ptr;
    args->d_mutex.lock();
    int mynum = ++args->d_countStarted;
    args->d_startSig.signal();
    args->d_mutex.unlock();

    ASSERT((mynum & 0xff) == mynum);
    bsls_PlatformUtil::Int64 pattern;
    bsl::memset(&pattern, mynum, sizeof(pattern));

    args->d_barrier.wait();

    for(int i=0; i < args->d_iterations; ++i) {
        if (mynum % 2) {
            Obj::setInt64(args->d_int_p, pattern);
            Obj::setInt64(args->d_int_p, pattern);
            continue;
        }
        bsls_PlatformUtil::Int64 ret;
        char *ptr = (char *) &ret;
        ret = Obj::getInt64(*args->d_int_p);
        for (int j = 1; j < sizeof(ret); ++j) {
            ASSERT(ptr[j] == ptr[0]);
        }
...
int main(int argc, char *argv[]) {
#ifndef BSLS_PLATFORM__CPU_SPARC_32
    ...
    switch (test) { case 0:
      ...
      case 2: {
        ...
        if (verbose) cout << "\nTesting 'setInt64' Thread Safeness"
                          << endl;
        {
            const int NTHREADS=4;
            ...
            for (int i=0; i < NTHREADS; ++i) {
                args.d_startSig.reset();
                myCreateThread(&threadHandles[i], setInt64TestThread,
                               &args);
                args.d_startSig.wait();
            }
            ASSERT(NTHREADS == args.d_countStarted);
