...
//                                  Overview
//                                  --------
// The 'bcema_Multipool' class consists of one constructor, a destructor,
...
//-----------------------------------------------------------------------------
// [ 2] bcema_Multipool(int numPools, bslma_Allocator *ba = 0);
// [ 8] bcema_MultipoolAllocator(numPools, minSize);
// [ 8] bcema_MultipoolAllocator(numPools, poolNumObjects);
// [ 8] bcema_MultipoolAllocator(numPools, minSize, poolNumObjects);
// [ 8] bcema_MultipoolAllocator(numPools, minSize, Z);
// [ 8] bcema_MultipoolAllocator(numPools, poolNumObjects, Z);
// [ 8] bcema_MultipoolAllocator(numPools, minSize, poolNumObjects, Z);
// [ 2] ~bcema_Multipool();
...
///Usage
///-----
///Example 1: Using a 'bcema_Multipool' Directly
///- - - - - - - - - - - - - - - - - - - - - - -
// A 'bcema_Multipool' can be used by containers that hold different types of
...
// messages, we use a 'bcema_Multipool'.
//
...
//
        // DATA
        bcema_Multipool d_multipool;  // multipool used to supply memory
//
      public:
        // CREATORS
...
//..
// A 'bcema_Multipool' is ideal for allocating the different sized messages
// since repeated deallocations might be necessary (which renders a
// 'bcema_SequentialPool' unsuitable) and the sizes of these types are all
// different:
//..
    // MANIPULATORS
...
//..
//
///Example 2: Implementing an Allocator Using 'bcema_Multipool'
/// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// 'bslma_Allocator' is used throughout the interfaces of BDE components.
// Suppose we would like to create a multipool allocator,
// 'my_MultipoolAllocator', that allocates memory from multiple 'bcema_Pool'
// objects in a similar fashion to 'bcema_Multipool'.  This class can be used
// directly to implement such an allocator.
//
// Note that the documentation for this class is simplified for this usage
// example.  Please see 'bcema_multipoolallocator' for full documentation of a
// similar class.
//..
...
//
        // DATA
        bcema_Multipool d_multiPool;  // memory manager for allocated memory
                                      // blocks
//
      public:
        // CREATORS
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:
      ...
      case 8: {
        ...
        if (verbose) {
            cout << "bcema_Multipool(int n, const S *gsa, "
        ...
        for (int si = 0; si < NUM_SDATA; ++si) {
            for (int mi = 0; mi < NUM_MDATA; ++mi) {
                ...
                bcema_Pool *pool[NUM_POOLS];
                int INIT = 8;
                for (int j = 0; j < NUM_POOLS; ++j) {
                    pool[j] = new(bsa)bcema_Pool(INIT,
                                                 SDATA[si][j],
                                                 MDATA[mi][j],
                                                 &pta);
                    INIT <<= 1;
...

        {
            ...
            // Initialize the pools
            bcema_Pool *pool[DEFAULT_NUM_POOLS];
            int INIT = 8;
        ...
        if (verbose) {
            cout << "bcema_Multipool(int n, const Strategy *gsa, Allocator "
                    "*ba = 0)" << endl;
        }
        for (int si = 0; si < NUM_SDATA; ++si) {
            bslma_TestAllocator pta("pool test allocator",
        ...
        if (verbose) {
            cout << "bcema_Multipool(int n, Strategy gs, int mbpc, Allocator "
                    "*ba = 0)" << endl;
        }
        {
            bslma_TestAllocator pta("pool test allocator",
        ...
        if (verbose) {
            cout << "bcema_Multipool(int n, const S *gsa, int mbpc, Alloc "
                    "*ba = 0)"
                 << endl;
        }
        for (int si = 0; si < NUM_SDATA; ++si) {
        ...
        if (verbose) {
            cout << "bcema_Multipool(int n, Strat gs, const int *ma, Alloc "
                    " *ba = 0)" << endl;
        }
        for (int mi = 0; mi < NUM_MDATA; ++mi) {
            ...
            for (int i = 0; i < NUM_PDATA; ++i) {
                ...
                for (int j = 0; j < NUM_ODATA; ++j) {
                     ...
                     for (int k = -1; k <= 1; ++k) {
                        ...
                        LOOP3_ASSERT(i, j, k,
                               0 == bsls_Types::UintPtr(p) % MAX_ALIGN);
                        ...
                        LOOP3_ASSERT(i, j, k,
                               0 == bsls_Types::UintPtr(q) % MAX_ALIGN);
      ...
      case 2: {
        ...
        // Concerns:
        //   We have the following concerns:
        //    1) The 'bcema_Multipool' constructor works properly:
        ...
        //
        // Testing:
        //   bcema_Multipool(int numPools, bslma_Allocator *ba = 0);
        //   ~bcema_Multipool();
        // --------------------------------------------------------------------
      ...
      case 1: {
        ...
        //
        // Concerns:
        //   We are concerned that the basic functionality of 'bcema_Multipool'
        //   works properly.
        //
        // Plan:
