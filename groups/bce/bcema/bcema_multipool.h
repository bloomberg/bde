...
#define INCLUDED_BCEMA_MULTIPOOL
...
//
//@CLASSES:
//   bcema_Multipool: memory manager that manages pools of varying block sizes
//
//@SEE_ALSO: bcema_pool, bcema_multipoolallocator
//
...
// dispensing memory blocks of a unique size.  The 'bcema_Pool' objects are
...
// the 'release' method and the destructor of a 'bcema_Multipool' release all
// memory currently allocated via the object.
//
// A 'bcema_Multipool' can be depicted visually:
//..
...
// 'bcema_Pool' maintained by the multipool, from which memory blocks of
...
///-----------------------------
// When creating a 'bcema_Multipool', clients can optionally configure:
//
...
// currently installed default allocator at the time the 'bcema_Multipool' was
// created.
//
...
class bcema_Multipool {
    ...
    // number of 'bdema_Pool' objects, each dispensing memory blocks of a
    // unique size.  The 'bcema_Pool' objects are placed in an array, with each
    ...
    // a 'bdema_Multipool' release all memory currently allocated via the
    // object.
    ...
    // DATA
    bcema_Pool      *d_pools_p;       // array of memory pools, each dispensing
    ...
    bdema_BlockList  d_blockList;     // memory manager for "large" memory
                                      // blocks.
  ...
  private:
    // NOT IMPLEMENTED
    bcema_Multipool(const bcema_Multipool&);
    bcema_Multipool& operator=(const bcema_Multipool&);

   private:
    // PRIVATE MANIPULATORS
    void initialize(bsls_BlockGrowth::Strategy        growthStrategy,
                    int                               maxBlocksPerChunk);
    void initialize(const bsls_BlockGrowth::Strategy *growthStrategyArray,
                    int                               maxBlocksPerChunk);
    void initialize(bsls_BlockGrowth::Strategy        growthStrategy,
    ...
    void initialize(const bsls_BlockGrowth::Strategy *growthStrategyArray,
        ...
        // individual 'bdema_Pool' maintained by this multipool is initialized
  ...
  public:
    // CREATORS
    bcema_Multipool(bslma_Allocator                  *basicAllocator = 0);
...
template <class TYPE>
inline
void bcema_Multipool::deleteObject(const TYPE *object)
{
...
template <class TYPE>
inline
void bcema_Multipool::deleteObjectRaw(const TYPE *object)
{
...
// ACCESSORS
inline
int bcema_Multipool::numPools() const
{
    return d_numPools;
}

inline
int bcema_Multipool::maxPooledBlockSize() const
{
