...
//
//@CLASSES:
//   bcema_Pool: thread-safe memory manager that allocates uniform sized blocks
//
//@AUTHOR: Ilougino Rocha (irocha)
//
//@SEE ALSO: bdema_pool
//
//@DESCRIPTION: This component implements a memory pool, 'bcema_Pool', that
// allocates and manages memory blocks of some uniform size specified at
// construction.  A 'bcema_Pool' object maintains an internal linked list of
...
//
// Whenever the linked list of free memory blocks is depleted, the 'bcema_Pool'
...
///-----------------------------
// When creating a 'bcema_Pool', clients must specify the specific block size
...
// currently installed default allocator at the time the 'bcema_Pool' was
// created.
//
///Overloaded Global Operator 'new'
///--------------------------------
// This component overloads the global 'operator new' to allow convenient
// syntax for the construction of objects using a 'bcema_Pool'.  The 'new'
// operator supplied in this component takes a 'bcema_Pool' argument indicating
// the source of the memory.  Consider the following use of standard placement
// 'new' syntax (supplied by 'bsl_new.h') along with a 'bcema_Pool' to allocate
...
//..
//  ::operator new(bsl::size_t size, bcema_Pool& pool);
//..
...
class bcema_Pool {
    ...
    struct Link {
        ...
        // replicated in 'bcema_pool.cpp' to provide access to a compatible
    ...
    bcemt_Mutex      d_mutex;      // protects access to the block list
  ...
  private:
    // NOT IMPLEMENTED
    bcema_Pool(const bcema_Pool&);
    bcema_Pool& operator=(const bcema_Pool&);
  ...
  public:
    ...
    bcema_Pool(int                         blockSize,
        ...
        // positive, grow the pool with internal memory chunks of constant
        // 'blockSize * maxBlocksPerChunk' bytes, alternatively, if
        // 'maxBlocksPerChunk' is negative, grow the internal chunk size
...
void *operator new(bsl::size_t size, BloombergLP::bcema_Pool& pool);
    ...
    //..
    //  void deleteMyType(my_Type *t, bcema_Pool *pool)
    //  {
    //      pool->deleteObject(t);
    //  }
    //..
...
inline
void operator delete(void *address, BloombergLP::bcema_Pool& pool);
    ...
    // 'bcema_Pool::deleteObject()' instead.
...
// ACCESSORS
inline
int bcema_Pool::blockSize() const
{
    return d_blockSize;
}
...
#if !defined(BSL_LEGACY) || 1 == BSL_LEGACY
inline
int bcema_Pool::objectSize() const
{
    return d_blockSize;
}
#endif
