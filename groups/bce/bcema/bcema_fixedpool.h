...
//
//@CLASSES:
//  bcema_FixedPool: thread-safe pool of limited # of blocks of uniform size
//
//@SEE_ALSO: bcema_pool
//
...
// component's semantics are identical to 'bcema_Pool'.  In particular, this
...
// corresponding methods in 'bcema_Pool'.
//
// Like 'bcema_Pool', this component is intended to be used to implement
// *out-of-place* container classes that hold elements of uniform size.
//
///Usage
///-----
// 'bcema_FixedPool' is intended to implement *out-of-place* container classes
...
// job objects - the destructor of 'bcema_FixedPool' will release any remaining
// allocated memory.  However, it *is* necessary to invoke the destructors of
// all these objects, as the destructor of 'bcema_FixedPool' will not do so.
...
struct bcema_FixedPool_Node {
    ...
    // allocated from 'bcema_FixedPool' objects.
...
class bcema_FixedPool {
    ...
    // PRIVATE TYPES
    typedef bcema_FixedPool_Node Node;     // type of memory block "header"

    // DATA
    bces_AtomicInt       d_freeList;       // head of free list
    ...
    bcemt_Mutex          d_nodePoolMutex;  // mutex for access to 'd_nodePool'

    bdema_Pool           d_nodePool;       // underlying memory pool
    ...
    // NOT IMPLEMENTED
    bcema_FixedPool(const bcema_FixedPool&);
    bcema_FixedPool& operator=(const bcema_FixedPool&);
...
// FREE OPERATORS
inline
void *operator new(bsl::size_t size, BloombergLP::bcema_FixedPool& pool);
    ...
    // member function 'bcema_FixedPool::deleteObject' parameterized by 'TYPE'
    // that performs the equivalent of the following:
    //..
    //  void deleteMyType(bcema_FixedPool *pool, my_Type *t) {
    //      t->~my_Type();
    //      pool->deallocate(t);
    //  }
    //..
...
inline
void operator delete(void *address, BloombergLP::bcema_FixedPool& pool);
    ...
    // 'bcema_FixedPool::deleteObject()' instead.
...
inline
void bcema_FixedPool::setBackoffLevel(int backoffLevel)
{
...
inline
int bcema_FixedPool::backoffLevel() const
{
    return d_backoffLevel;
}

inline
int bcema_FixedPool::indexFromAddress(void *address) const
{
...
inline
int bcema_FixedPool::objectSize() const
{
    return d_objectSize;
}
