...
BDES_IDENT_RCSID(bcema_pool_cpp,"$Id$ $CSID$")
...
struct LLink {
    ...
    // memory blocks.  Note that this type is copied from 'bcema_pool.h' to
...
static
void replenishImp(bces_AtomicPointer<LLink>       *nextList,
                  bdema_InfrequentDeleteBlockList *blockList,
                  int                              blockSize,
                  int                              numBlocks)
    ...
    // blockSize' and '1 <= numBlocks'.
{
    ...
    for (char *p = start; p < end; p += blockSize) {
        ...
        bces_AtomicUtil::initInt(&nextLink->d_refCount, 0 );
        nextLink->d_next_p = toLink(p + blockSize);
    }
    bces_AtomicUtil::initInt(&toLink(end)->d_refCount, 0 );
...
int computeInternalBlockSize(int blockSize)
    ...
    // represent a 'object' LLink (on the 'bcema_Pool' objects free list).
...
void bcema_Pool::replenish()
{
    replenishImp(reinterpret_cast<bces_AtomicPointer<LLink> *>(&d_freeList),
                 &d_blockList,
                 d_internalBlockSize,
                 d_chunkSize);
...
// MANIPULATORS
void bcema_Pool::reserveCapacity(int numBlocks)
{
    ...
    if (numBlocks > 0) {
        replenishImp(
                   reinterpret_cast<bces_AtomicPointer<LLink> *>(&d_freeList),
                   &d_blockList,
                   d_internalBlockSize,
                   numBlocks);
    }
...
void *bcema_Pool::allocate()
{
    ...
    for (;;) {
        ...
        if (BSLS_PERFORMANCEHINT_PREDICT_UNLIKELY
                         (2 != bces_AtomicUtil::addIntNv(&p->d_refCount, 2))) {
            BSLS_PERFORMANCEHINT_UNLIKELY_HINT;
            for (int i = 0; i < 3; ++i) {
        ...
        BSLS_PERFORMANCEHINT_UNLIKELY_HINT;
        for (;;) {
            int refCount = bces_AtomicUtil::getInt(p->d_refCount);

            if (refCount & 1) {
                if (refCount ==
                        bces_AtomicUtil::testAndSwapInt(
                            &p->d_refCount,
                            refCount,
                            refCount^1)) {
...
{
    Link *p = (Link *)(void *)((char *)address - offsetof(Link, d_next_p));
    int refCount = bces_AtomicUtil::getIntRelaxed(p->d_refCount);
    for (;;) {
        if (BSLS_PERFORMANCEHINT_PREDICT_LIKELY(2 == refCount)) {
            refCount = bces_AtomicUtil::testAndSwapInt(&p->d_refCount, 2, 0);
            if (BSLS_PERFORMANCEHINT_PREDICT_LIKELY(2 == refCount)) {
                break;
            }
        }
        ...
        refCount = bces_AtomicUtil::testAndSwapInt(&p->d_refCount,
                                                   refCount,
                                                   refCount - 1);
