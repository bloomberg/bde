// bcep_timereventscheduler.t.cpp      -*-C++-*-
...
//                                   Overview
//                                   --------
// Testing 'bcep_TimerEventScheduler' is divided into 2 parts (apart from
...
//-----------------------------------------------------------------------------
// CREATORS
// [01] bcep_TimerEventScheduler(allocator = 0);
//
// [08] bcep_TimerEventScheduler(dispatcher, allocator = 0);
//
// [01] ~bcep_TimerEventScheduler();
//
// MANIPULATORS
// [09] int start();
//
...
//
// [02] Handle startClock(interval, callback, startTime=bdet_TimeInterval(0));
//
...
typedef bcep_TimerEventScheduler          Obj;
...
class my_Server {
    ...
    // TYPES
    struct Connection {
        bcep_TimerEventScheduler::Handle d_timerId;
        my_Session *d_session_p;
    };

    // DATA
    bsl::vector<Connection*>     d_connections;
    bcep_TimerEventScheduler     d_scheduler;
    bdet_TimeInterval            d_ioTimeout;
    ...
    // it will get executed before the event does.
{
    ...
    scheduler->startClock(T2, bdef_MemFnUtil::memFn(&TestClass1::callback,
                                                    clock));

    scheduler->scheduleEvent(bdetu_SystemTime::now() + T4,
                             bdef_MemFnUtil::memFn(&TestClass1::callback,
                                                   event));
}
...
void test7_d()
{
    ...
    Handle handleToBeCancelled =
      x.startClock(T, bdef_BindUtil::bind(&cancelClockCallback,
                                          &x,
                                          &handleToBeCancelled,
                                          1,
                                          0));
...
void test7_e()
{
    ...
    x.startClock(T,
                 bdef_BindUtil::bind(&cancelAllClocksCallback,
                                     &x, 1));
...
void test6_a()
{
    ...
    bdet_TimeInterval now = bdetu_SystemTime::now();
    x.startClock(T3, bdef_MemFnUtil::memFn(&TestClass1::callback,
                                           &testObj1));

    x.startClock(T5, bdef_MemFnUtil::memFn(&TestClass1::callback,
                                           &testObj2));
...
void test6_b()
{
    ...
    bdet_TimeInterval now = bdetu_SystemTime::now();
    x.startClock( T, bdef_MemFnUtil::memFn(&TestClass1::callback,
                                           &testObj1),
                 now - T3);

    x.startClock(T2, bdef_MemFnUtil::memFn(&TestClass1::callback,
                                           &testObj2),
                 now - T2);
...
void test3_b()
{
    ...
    bdet_TimeInterval now = bdetu_SystemTime::now();
    (void)      x.scheduleEvent(now + T, bdef_MemFnUtil::memFn(
                                             &TestClass1::callback,
                                             &testObj1));

    Handle h2 = x.scheduleEvent(now + T2, bdef_MemFnUtil::memFn(
                                             &TestClass1::callback,
                                             &testObj2));
    myMicroSleep(T3, 0);
    x.start();
    ...
    if ((bdetu_SystemTime::now() - now).totalSecondsAsDouble() < 1.29) {
        ASSERT( 0 == testObj2.numExecuted() );
    }
    x.stop();
...
//      const bool DELAYED          = DATA[i].d_delayed;
        ...
        if (ISCLOCK) {
            ...
            x.startClock(bdet_TimeInterval(PERIODICINTERVAL * DECI_SEC),
...
void test1_a()
{
    ...
    x.scheduleEvent(now + T6, bdef_MemFnUtil::memFn(&TestClass1::callback,
                                                    &testObj2));
...
void test1_b()
{
    ...
    bdet_TimeInterval now = bdetu_SystemTime::now();
    Handle h1 = x.startClock(T3, bdef_MemFnUtil::memFn(
                                     &TestClass1::callback,
                                     &testObj1));

    Handle h2 = x.startClock(T3, bdef_MemFnUtil::memFn(
                                     &TestClass1::callback,
                                     &testObj2));
...
void test1_c()
{
    ...
    bdet_TimeInterval now = bdetu_SystemTime::now();
    Handle h = x.startClock(T3, bdef_MemFnUtil::memFn(
                                  &TestClass1::callback,
                                  &testObj));
...
void test1_d()
{
    ...
    bdet_TimeInterval now = bdetu_SystemTime::now();
    Handle h1 = x.scheduleEvent(now + T, bdef_MemFnUtil::memFn(
                                             &TestClass1::callback,
                                             &testObj1));
    Handle h2 = x.scheduleEvent(now + T2, bdef_MemFnUtil::memFn(
                                             &TestClass1::callback,
                                             &testObj2));
    Handle h3 = x.scheduleEvent(now + T3, bdef_MemFnUtil::memFn(
                                             &TestClass1::callback,
                                             &testObj3));
    x.cancelAllEvents();
...
void test1_e()
{
    ...
    bdet_TimeInterval now = bdetu_SystemTime::now();
    Handle h = x.scheduleEvent(now + T2, bdef_MemFnUtil::memFn(
                                             &TestClass1::callback,
                                             &testObj));
...
void test1_g()
{
    ...
    x.startClock(T3, bdef_MemFnUtil::memFn(&TestClass1::callback,
                                           &testObj1));
    x.scheduleEvent(now + T3, bdef_MemFnUtil::memFn(
                                             &TestClass1::callback,
                                             &testObj2));
    bdet_TimeInterval elapsed = bdetu_SystemTime::now() - now;
    if (elapsed < T3) {
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      ...
      case 18: {
        ...
        for (int i = 0; i < 10; ++i) {
            Obj::Handle handle =
                      mX.scheduleEvent(now - bdet_TimeInterval((10 - i) * 0.1),
                                                                      Func(i));
      ...
      case 15: {
        ...
        for (ii = 0; ii <= MAX_LOOP; ++ii) {
            bcep_TimerEventScheduler scheduler(&ta);
            scheduler.start();
            Slowfunctor sf;
            Fastfunctor ff;
            ...
            scheduler.startClock(bdet_TimeInterval(CLOCKTIME1),
                           bdef_MemFnUtil::memFn(&Slowfunctor::callback, &sf));
            bdet_TimeInterval afterStarted = bdetu_SystemTime::now();

            scheduler.scheduleEvent(
                           afterStarted + bdet_TimeInterval(2.0),
                           bdef_MemFnUtil::memFn(&Fastfunctor::callback, &ff));

            scheduler.scheduleEvent(
                           afterStarted + bdet_TimeInterval(3.0),
                           bdef_MemFnUtil::memFn(&Fastfunctor::callback, &ff));
            ...
            // Testing of that routine has been moved to bcemt_thread.t.cpp.
      ...
      case 14: {
        ...
        //   'bcemt_Condition::timedWait', this test is going to fail a
        ...
        for (ii = 0; ii <= MAX_LOOP; ++ii) {
            ...
            scheduler.startClock(bdet_TimeInterval(CLOCKTIME1),
                                 bdef_MemFnUtil::memFn(&Slowfunctor::callback,
                                                       &sf));
...
// TBD
        ...
        // routine has been moved to bcemt_thread.t.cpp.
      ...
      case 13: {
        ...
        for (int testIter = 0; testIter < NUM_TESTS; ++testIter) {
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                threadGroup.addThreads(bdef_BindUtil::bind(&startClock,
                                                           &mX,
                                                           &numAdded,
                                                           (bces_AtomicInt*)0,
                                                           &barrier),
      ...
      case 8: {
        // -----------------------------------------------------------------
        // TESTING 'bcep_TimerEventScheduler(dispatcher, allocator = 0)':
        //   Verifying 'bcep_TimerEventScheduler(dispatcher, allocator = 0)'.
        //
        // Concerns:
        ...
        //
        // Testing:
        //   bcep_TimerEventScheduler(dispatcher, allocator = 0);
        // -----------------------------------------------------------------

        if (verbose) cout << endl
   << "TESTING 'bcep_TimerEventScheduler(dispatcher, allocator = 0)'" << endl
   << "=============================================================" << endl;
        ...
        bcep_TimerEventScheduler::Dispatcher dispatcher =
      ...
      case 7: {
        ...
        for (int i = 0; i < NUM_THREADS; ++i) {
            ASSERT(0 == bcemt_ThreadUtil::create(&handles[i], funcPtrs[i]));
        }
      ...
      case 5: {
        ...
        using namespace BCEP_TIMER_EVENT_SCHEDULER_TEST_CASE_5;
        {
          ...
          bdet_TimeInterval now = bdetu_SystemTime::now();
          Handle h = x.startClock(T2, bdef_MemFnUtil::memFn(
                                                 &TestClass1::callback,
                                                 &testObj));
...

        {
          ...
          bdet_TimeInterval now = bdetu_SystemTime::now();
          (void)      x.startClock(T,  bdef_MemFnUtil::memFn(
                                                 &TestClass1::callback,
                                                 &testObj1));

          Handle h2 = x.startClock(T2, bdef_MemFnUtil::memFn(
                                                 &TestClass1::callback,
                                                 &testObj2));
          myMicroSleep(T3, 0);
          x.start();
          ...
          bool cancelledInTime =
                 (bdetu_SystemTime::now() - now).totalSecondsAsDouble() < 0.99;
          myMicroSleep(T20, 0);
          if (cancelledInTime) {
...

        {
          ...
          bdet_TimeInterval now = bdetu_SystemTime::now();
          Handle h = x.startClock(T, bdef_MemFnUtil::memFn(
                                                 &TestClass1::callback,
                                                 &testObj));

          myMicroSleep(T2, 0);
          ASSERT( 0 == x.cancelClock(h) );
          if ((bdetu_SystemTime::now() - now).totalSecondsAsDouble() < 0.59) {
              ASSERT( 0 == testObj.numExecuted() );
              myMicroSleep(T5, 0);
              makeSureTestObjectIsExecuted(testObj, mT, 100);
              ASSERT( 1 == testObj.numExecuted() );
      ...
      case 4: {
...

        {
          ...
          const bdet_TimeInterval  T(1 * DECI_SEC);
          const bdet_TimeInterval T2(2 * DECI_SEC);
          const bdet_TimeInterval T7(7 * DECI_SEC);
...

        {
          ...
          for (ii = 0; ii <= MAX_LOOP; ++ii) {
              ...
              if (bdetu_SystemTime::now() - now >= T6) {
                  if (verbose || MAX_LOOP == ii) {
                      P_(L_) Q(Overslept);
                  }
                  continue;
