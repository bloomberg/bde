// bcep_multiprioritythreadpool.h              -*-C++-*-
#ifndef INCLUDED_BCEP_MULTIPRIORITYTHREADPOOL
#define INCLUDED_BCEP_MULTIPRIORITYTHREADPOOL
...
//
//@CLASSES:
//    bcep_MultipriorityThreadPool: mechanism to parallelize prioritized jobs
//
...
// appropriately configured 'bcemt_Attribute' object.  (See the 'bcemt_thread'
...
//..
//       bcep_MultipriorityThreadPool pool(20,  // # of threads
//                                         2);  // # of priorities
//
...
//   bces_AtomicInt scannedTo[TOP_NUMBER];   // if 'P' is a prime, what is the
//                                           // highest multiple of 'P' that
//                                           // we have marked
//                                           // 'isStillPrime[P] = false'
//
//   bces_AtomicInt maxPrimeFound;           // maximum prime identified so far
...
//         doneFlag = false;
//
//         bcep_MultipriorityThreadPool threadPool(20, NUM_PRIORITIES);
...
class bcep_MultipriorityThreadPool {
  ...
  public:
    // TYPES
    typedef bdef_Function<void(*)()> ThreadFunctor;
  ...
  private:
    // DATA
    bcemt_Mutex      d_mutex;             // mutex for worker threads as they
                                          // analyze state, and for methods
                                          // that manipulate that state

    bcemt_Mutex      d_metaMutex;         // mutex for '[start|stop]Threads',
    ...
    bcec_MultipriorityQueue<ThreadFunctor>
    ...
    bces_AtomicInt   d_threadSuspendState;// enum of type 'ResumeState' (local
                                          // to .cpp) regarding whether threads
                                          // are suspended

    bces_AtomicInt   d_numStartedThreads; // number of threads currently
                                          // started

    bces_AtomicInt   d_numSuspendedThreads;
                                          // number of threads currently
                                          // suspended

    bces_AtomicInt   d_numActiveThreads;  // number of threads currently
                                          // processing jobs

    bcemt_Condition  d_allThreadsStartedCondition;
                                          // broadcast when number of started
                                          // threads reaches 'd_numThreads',
                                          // watched during starting

    bcemt_Condition  d_allThreadsSuspendedCondition;
                                          // broadcast when number of started
                                          // threads reaches 'd_numThreads',
                                          // watched during starting

    bcemt_Condition  d_resumeCondition;   // broadcast when suspended threads
                                          // are to resume

    // NOT IMPLEMENTED
    bcep_MultipriorityThreadPool(const bcep_MultipriorityThreadPool&);
    bcep_MultipriorityThreadPool& operator=(
                                 const bcep_MultipriorityThreadPool&);
