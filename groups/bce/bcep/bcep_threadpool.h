...
// 'bcemt_thread' package documentation for a description of 'bcemt_Attribute'.
//
...
// 'bcep_ThreadPool' will handle the queue management, thread management, and
...
///---------------------------
// A thread pool ensures that, on unix platforms, all the threads in the
// pool block all asynchronous signals.  Specifically all the signals,
// except the following synchronous signals are blocked.
//
// SIGBUS
// SIGFPE
// SIGILL
...
class bcep_ThreadPool {
  ...
  private:
    ...
    mutable bcemt_Mutex d_mutex;           // mutex used to control access
                                           // to this thread pool

    bcemt_Condition    d_drainCond;        // condition variable used to
                                           // signal that the queue is fully
                                           // drained and that all active jobs
                                           // have completed
    ...
    volatile int       d_createFailures;   // number of thread create
                                           // failures
    ...
    bces_AtomicInt64   d_lastResetTime;    // last reset time of percent-busy
                                           // metric in nanoseconds from some
                                           // arbitrary but fixed point in
                                           // time

    bces_AtomicInt64   d_callbackTime;     // the total time spent running jobs
