...
void TestJobFunction3(void *ptr)
    ...
    // the 'wait' on a barrier signifying the end of the function
{
...
// search.  The program creates a 'bcep_ThreadPool', and then enqueues a single
...
// threads, access to the array is controlled via a 'bcemt_Mutex'.
//
...
// uses the 'bdef_Function' component to provide type-safe wrappers that
...
//..
        for (int i = 0; i < count; ++i) {
            ...
            bdef_Function<void (*)()> jobHandle
                = bdef_BindUtil::bind(&my_FastFunctorSearchJob, &job);
...
void testJobFunction13_2(bcemt_Barrier *barrier)
{
    barrier->wait();
}
...
class Test13Object {
  ...
  public:
    Test13Object(bcemt_Barrier *barrier_p, Obj *threadPool_p);
    ~Test13Object();
};

Test13Object::Test13Object(bcemt_Barrier *barrier_p, Obj *threadPool_p)
...
int main(int argc, char *argv[])
{
    ...
    bcemt_Default::setDefaultThreadStackSize(
                           bcemt_Default::recommendedDefaultThreadStackSize());
    ...
    switch (test) { case 0: // 0 is always the first test case
      ...
      case 8: {
            ...
            cout << "\tWhen thread pool is running.  " << endl;
        {
            ...
            bdef_Function<void (*)()> noop(&noopFunc);
            ...
            bdef_Function<void (*)()> count =
                bdef_BindUtil::bind(&counter, &result, 2000000);
            ...
            // Xlc (at least 8.0) seems to be so aggressive in its optimization
            // that the percentBusy is only at about 35% on all builds,
            // and this happens also in optimized builds on other platforms.
            // MS VC 2005 : percentBusy can be 9% (on 4 CPU box)
#endif
        }
      ...
      case 6: {
                          ...
                          << "====================================" << endl ;
        {
            ...
            bcemt_Barrier startBarrier(MAX + 1);
            bcemt_Barrier stopBarrier(MAX + 1);
            ...
            // The other MAX-BURST threads have been idle since t1,
            ...
            // This suspends for AT LEAST duration...
            // one millisecond has 1000 microseconds.

            bcemt_ThreadUtil::microSleep(duration * 1000);
            bcemt_ThreadUtil::yield();
            ASSERT(BURST == x.numActiveThreads());
            ...
            bcemt_ThreadUtil::microSleep(IDLE * 300);
            bcemt_ThreadUtil::yield();
            ASSERT(0 <= x.numWaitingThreads());
            ...
            bcemt_ThreadUtil::microSleep(IDLE * 1300);
            bcemt_ThreadUtil::yield();
            if (0 < x.numActiveThreads()) {
                cout << "WARNING: Failure in case 6 (still "
                     ...
                     << endl;
                while(0 < x.numActiveThreads()) {
                    bcemt_ThreadUtil::microSleep(IDLE * 1000);
                    bcemt_ThreadUtil::yield();
                }
            }
      ...
      case 1: {
        ...
        //   Verify that the support functions used to test 'bcep_ThreadPool'
