...
struct TestJobFunctionArgs1 {
    bcemt_Barrier   *d_startBarrier_p;
    bcemt_Barrier   *d_stopBarrier_p;
...
struct myFastSearchJobInfo {
    ...
    bcemt_Mutex         *d_mutex;   // mutex to control access the result
                                    // file list.
...
void testJobFunction9_2(bcemt_Barrier *barrier)
{
    barrier->wait();
}
...
class Test9Object {
  ...
  public:
    Test9Object(bcemt_Barrier *barrier_p, Obj *threadPool_p);
    ~Test9Object();
};

Test9Object::Test9Object(bcemt_Barrier *barrier_p, Obj *threadPool_p)
...
{
    bdef_Function<void(*)(void)> job = bdef_BindUtil::bind(
                                                          &testJobFunction9_2,
                                                          d_barrier_p);
...
Obj *xP;
bcemt_Barrier barrier(DEPTH_LIMIT + 1); // to ensure that all the threads
                                        // have started

bces_AtomicInt depthCounter;
    ...
    */
    ...
    switch (test) { case 0:  // case 0 is always the first case
      case 14: {
        ...
        //   'bcep_fixedthreadpool' in the 'baem' test drivers.
        //
        // Plan:
...
#       endif
      ...
      case 7: {
        ...
        // --------------------------------------------------------------------
          ...
          for (int i = 0; i < NUM_THREADS; ++i) {
              tp.enqueueJob(bdef_BindUtil::bind(&bcemt_Barrier::wait,
                                                &barrier));
          }
      ...
      case 5: {
        ...
        for (int i = 0; i < NUM_VALUES; ++i) {
            ...
            bcemt_Barrier startBarrier(THREADS + 1);
            bcemt_Barrier stopBarrier(THREADS + 1);
      ...
      case 4: {
        ...
        for (int i = 0; i < NUM_VALUES; ++i) {
            ...
            bcemt_Mutex mutex;
            bcemt_Condition startCond;
            bcemt_Condition stopCond;
      ...
      case 1: {
        ...
        //   'bcep_FixedThreadPool' behave as expected.  The 'testJobFunction1'
...

        {
            ...
            bcemt_ThreadUtil::Handle threadHandles[NITERATIONS];
            ...
            for (int i=0; i<NITERATIONS; ++i) {
                ...
                bcemt_ThreadUtil::create(&threadHandles[i], attributes,
                                         (bcemt_ThreadUtil::ThreadFunction)
                                         &testJobFunction1,
                                         &args );
                while( !args.d_startSig ) {
                    startCond.wait(&mutex);
                              ...
                              << "\t===========================" << endl;
        {
            ...
            bcemt_Barrier startBarrier(NITERATIONS + 1);
            bcemt_Barrier stopBarrier(NITERATIONS + 1);
            TestJobFunctionArgs1 args;
