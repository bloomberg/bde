                          ...
                          int              size)
{
    ...
    for (int i = 0; i != size; ++i) {
        ...
        ASSERT(x.end() !=  it);
        ASSERT(data[i] == *it);
        const int countValues = x.count(keyForValue<CONTAINER>(testValue));
...
if (veryVerbose) cout << "<(H2)>" << endl;
        testBuckets(mX);
        validateIteration(mX);
if (veryVeryVerbose) cout << "<<I>>" << endl;
