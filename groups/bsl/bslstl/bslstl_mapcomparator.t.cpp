...
// CREATORS
// [ 3] MapComparator();
// [ 3] explicit MapComparator(const COMPARATOR& keyComparator);
// [  ] MapComparator(const MapComparator&);
// [  ] ~MapComparator();

// inheriting traits: bitwise
//
// ACCESSORS
...
struct TestDriver {
    ...
    typedef TYPE                                                   Key;
    typedef int                                                    Value;
    typedef TreeNode<bsl::pair<const Key, Value> > Node;
...
void TestDriver<TYPE>::test4()
{
    ...
    {
        FunctorType fA(1);
        MapComparator<Key, Value, FunctorType> mA(fA);
        const MapComparator<Key, Value, FunctorType>& A = mA;

        FunctorType fB(2);
        MapComparator<Key, Value, FunctorType> mB(fB);
        const MapComparator<Key, Value, FunctorType>& B = mB;
               ...
               "for objects constructed with a function pointer\n");
    {
        ...
        MapComparator<Key, Value, FunctionType> mA(fA);
        const MapComparator<Key, Value, FunctionType>& A = mA;
...
void TestDriver<TYPE>::test3()
{
               ...
               "for default constructed objects\n");
    {
        ...
        const MapComparator<Key, Value, FunctorType>& comp = mComp;

        MapComparator<Key, Value, NonConstFunctorType> ncComp;
        ...
        AllocTraits::construct(allocator, &mN1.value(),
                               bsltf::TemplateTestFacility::create<Key>(1), 0);
               ...
               "for objects constructed with a functor\n");
    {
        ...
        MapComparator<Key, Value, FunctorType> mComp(functor);
        const MapComparator<Key, Value, FunctorType>& comp = mComp;

        NonConstFunctorType ncFunctor(1);
        MapComparator<Key, Value, NonConstFunctorType> ncComp(ncFunctor);
               ...
               "for objects constructed with a function pointer\n");
    {
        ...
        const MapComparator<Key, Value, FunctionType>& comp = mComp;
...
void TestDriver<TYPE>::test2()
{
    ...
    {
        // MapComparator<Key, Value, FunctionType> mComp;  // This will not
        //                                                 // compile
    }
}
...
void TestDriver<TYPE>::test1()
{
    ...
    if (verbose)
        printf("Verify Using Functor Comparator in MapComparator.\n");
    {
        Alloc allocator(&oa);
        ...
        AllocTraits::construct(allocator, &mN.value());

        mN.value() = K0;
        ASSERTV(N.value(), K0, comp(N, K1));

        AllocTraits::destroy(allocator, &mN.value());

        AllocTraits::deallocate(allocator, nPtr, 1);
    }

    if (verbose)
        printf("Verify Using Function Pointer Comparator in MapComparator.\n");
    {
        Alloc allocator(&oa);
