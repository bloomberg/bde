...
//  'il'            - an object of type 'initializer_list<value_type>'
...
//  |                                                    | is distance(i1,i2) |
//  +----------------------------------------------------+--------------------+
//  | set<K> a(il, c);                                   | O[N] if il         |
//  | set<K> a(il, c, al);                               | is sorted with     |
//  | set<K> a(il, al);                                  | 'a.value_comp()',  |
//  |                                                    | O[N * log(N)]      |
//  |                                                    | otherwise, where N |
//  |                                                    | is il.size()       |
//  +----------------------------------------------------+--------------------+
...
#if defined(BSLS_COMPILERFEATURES_SUPPORT_GENERALIZED_INITIALIZERS)
    set(native_std::initializer_list<value_type> init,
        const COMPARATOR& comparator = COMPARATOR(),
        const ALLOCATOR&  basicAllocator = ALLOCATOR());
        // Construct a set, and insert each object in 'init', ignoring those keys
        // that appears earlier in the sequence.
        ...
        // parameter 'ALLOCATOR' argument is of type 'bsl::allocator' (the default),
        // then 'basicAllocator', if supplied, shall be convertible to
        // 'bslma::Allocator *'.  If the template parameter 'ALLOCATOR' argument is
        // of type 'bsl::allocator' and 'basicAllocator' is not supplied, the
        // currently installed default allocator is used to supply memory.  If the
        // sequence in 'init' is ordered according to the identified 'comparator',
        // then this operation has 'O[N]' complexity, where 'N' is 'init.size()',
        // otherwise this operation has 'O[N * log(N)]' complexity.
    
    set(native_std::initializer_list<value_type> init,
        const ALLOCATOR&  basicAllocator);
        // Construct a set that will use the specified 'basicAllocator'
        // to supply memory and insert each object in 'init', ignoring those keys that
        // appears earlier in the sequence.
        // Use a default-constructed object of the (template parameter) type
        // 'COMPARATOR' to order the keys contained in this set.  If the
        // template parameter 'ALLOCATOR' argument is of type 'bsl::allocator'
        // (the default), then 'basicAllocator' shall be convertible to
        // 'bslma::Allocator *'. If the sequence in 'init' is ordered according to
        // the comparator, then this operation has 'O[N]' complexity, where 'N' is
        // 'init.size()', otherwise this operation has 'O[N * log(N)]' complexity.
#endif
...
#if defined(BSLS_COMPILERFEATURES_SUPPORT_GENERALIZED_INITIALIZERS)
    set& operator=(native_std::initializer_list<value_type> il);
        // Assign to this object the values in 'il', ignoring the keys that appear
        // earlier in the sequence and return a reference providing modifiable
        // access to this object.
        // All existing objects are either assigned to or destroyed.
        // If the sequence in 'il' is ordered according to the comparator then
        // this operation has 'O[N]' complexity, where 'N' is 'il.size()',
        // otherwise this operation has 'O[N * log(N)]' complexity.
        // Note that this method requires that the (template parameter) type
        // 'KEY' type be "copy-constructible" (see {Requirements on 'KEY'}).
#endif
...
#if defined(BSLS_COMPILERFEATURES_SUPPORT_GENERALIZED_INITIALIZERS)
    void insert(native_std::initializer_list<value_type> il);
        // Insert into this set the values in 'il' which are not already in this
        // set.
        // If the sequence in 'il' is ordered according to the comparator then
        // this operation has 'O[N]' complexity, where 'N' is 'il.size()',
        // otherwise this operation has 'O[N * log(N)]' complexity.
...
template <class KEY, class COMPARATOR, class ALLOCATOR>
inline
set<KEY, COMPARATOR, ALLOCATOR>::set(native_std::initializer_list<value_type> init,
    const COMPARATOR& comparator,
    const ALLOCATOR&  basicAllocator)
: set(init.begin(), init.end(), comparator, basicAllocator)
{
}
...
inline
set<KEY, COMPARATOR, ALLOCATOR>&
set<KEY, COMPARATOR, ALLOCATOR>::operator=(native_std::initializer_list<value_type> il)
{
    clear();
    insert(il);
    return *this;
...
template <class KEY, class COMPARATOR, class ALLOCATOR>
inline
void set<KEY, COMPARATOR, ALLOCATOR>::insert(native_std::initializer_list<value_type> il)
{
