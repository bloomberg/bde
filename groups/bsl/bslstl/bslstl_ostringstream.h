...
class basic_ostringstream
  ...
  public:
    ...
    /// that have been written to this stream.  `rdbuf` is created using
    /// `ios_base::out`.  Optionally specify the `allocator` used to supply
    ...
    /// `basic_string_view<CHAR_TYPE, CHAR_TRAITS>` but not to
    /// `const CHAR_TYPE *`) indicating the initial sequence of characters
    /// that have been written to this stream.  Use the specified
    /// `modeBitMask` to indicate whether this buffer may be read from,
    /// written to, or both.  `rdbuf` is created using
    /// `modeBitMask | ios_base::out`.  Optionally specify the `allocator`
    /// used to supply memory.  If `allocator` is not supplied, a
    /// default-constructed object of the (template parameter) `ALLOCATOR`
    /// type is used.  If the `ALLOCATOR` argument is of type
...
// NOLINTNEXTLINE(performance-noexcept-move-constructor)
    ...
    /// Reset the internally buffered sequence of characters maintained by
    /// this stream object to the specified `value` (of a type convertible
    /// to `basic_string_view<CHAR_TYPE, CHAR_TRAITS>` but not to
    /// `const CHAR_TYPE *`).
    template <class STRING_VIEW_LIKE_TYPE>
...
#endif  // INCLUDED_BSLSTL_OSTRINGSTREAM
