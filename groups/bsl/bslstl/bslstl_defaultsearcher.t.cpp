// bslstl_defaultsearcher.t.cpp                                       -*-C++-*-
...
// [ 2] FIN needleFirst() const;
// [ 2] FIN needleLast() const;
// [ 2] EQUAL equal() const;
// ----------------------------------------------------------------------------
// [ 1] BREATHING TEST
// [ 4] USAGE EXAMPLE
...
void doTestRun(bsl::list<float> *data)
{
    ASSERT(data);
...
void processTestRun(bsl::list<float>::const_iterator first,
                    bsl::list<float>::const_iterator last)
{
    // Stub.
    (void)first;
    (void)last;
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:
      ...
      case 3: {
        ...
        for (BSL::size_t ti = 0; ti < numDATA; ++ti) {
            ...
            // Searcher type-space:  Fwd and Rnd Iterators
            //                       CaseSensitive and Case Insensitive

            typedef CharList< char>::const_iterator       FwdConstItr;
            typedef CharArray<char>::const_iterator       RndConstItr;
            typedef bsl::pair<FwdConstItr, FwdConstItr>   FwdResult;
            ...
            const CharList< char> needleFwd(needleChars);
            const CharArray<char> needleRnd(needleChars);

            ASSERT(static_cast<FwdDiff>(BSL::strlen(NEEDLE))
                == bsl::distance(needleFwd.begin(), needleFwd.end()));

            ASSERT(static_cast<RndDiff>(BSL::strlen(NEEDLE))
                == bsl::distance(needleRnd.begin(), needleRnd.end()));

            // Construct haystacks having Fwd and Rnd Iterators.

            bsl::vector<char> haystackChars;
            for (const char *itr = HAYSTACK; 0 != *itr; ++itr) {
                haystackChars.push_back(*itr);
            }
            const CharList< char> haystackFwd(haystackChars);
            const CharArray<char> haystackRnd(haystackChars);

            ASSERT(static_cast<FwdDiff>(BSL::strlen(HAYSTACK))
                == bsl::distance(haystackFwd.begin(), haystackFwd.end()));

            ASSERT(static_cast<RndDiff>(BSL::strlen(HAYSTACK))
                == bsl::distance(haystackRnd.begin(), haystackRnd.end()));

            // Create the matrix of objects to be tested.
            const FwdMechCs fwdMechCs(needleFwd.begin(), needleFwd.end());
            const RndMechCs rndMechCs(needleRnd.begin(), needleRnd.end());
            const FwdMechCi fwdMechCi(needleFwd.begin(), needleFwd.end());
            const RndMechCi rndMechCi(needleRnd.begin(), needleRnd.end());


            // Invoke each using both kinds of haystack: Fwd and Rnd
  //--------^
  FwdResult fwdResultFwdCs = fwdMechCs(haystackFwd.begin(), haystackFwd.end());
  RndResult rndResultFwdCs = fwdMechCs(haystackRnd.begin(), haystackRnd.end());

  FwdResult fwdResultRndCs = rndMechCs(haystackFwd.begin(), haystackFwd.end());
  RndResult rndResultRndCs = rndMechCs(haystackRnd.begin(), haystackRnd.end());

  FwdResult fwdResultFwdCi = fwdMechCi(haystackFwd.begin(), haystackFwd.end());
  RndResult rndResultFwdCi = fwdMechCi(haystackRnd.begin(), haystackRnd.end());

  FwdResult fwdResultRndCi = rndMechCi(haystackFwd.begin(), haystackFwd.end());
  RndResult rndResultRndCi = rndMechCi(haystackRnd.begin(), haystackRnd.end());
  //--------v

            // Check each result against expected offset value.
 //---------^
 ASSERT(OFFSET_CS == bsl::distance(haystackFwd.begin(), fwdResultFwdCs.first));
 ...
 ASSERTV(LINE, OFFSET_CS == bsl::distance(haystackRnd.begin(), rndResultRndCs.first));
 ...
 ASSERT(OFFSET_CI == bsl::distance(haystackFwd.begin(), fwdResultRndCi.first));
 ASSERT(OFFSET_CI == bsl::distance(haystackRnd.begin(), rndResultRndCi.first));
 //---------v

            // Check each result against expected length value.
            ASSERT(LENGTH_CS == bsl::distance(fwdResultFwdCs.first,
                                              fwdResultFwdCs.second));
            ASSERT(LENGTH_CS == bsl::distance(rndResultFwdCs.first,
                                              rndResultFwdCs.second));

            ASSERT(LENGTH_CS == bsl::distance(fwdResultRndCs.first,
                                              fwdResultRndCs.second));
            ASSERTV(LINE, LENGTH_CS == bsl::distance(rndResultRndCs.first,
                                              rndResultRndCs.second));

            ASSERT(LENGTH_CI == bsl::distance(fwdResultFwdCi.first,
                                              fwdResultFwdCi.second));
            ASSERT(LENGTH_CI == bsl::distance(rndResultFwdCi.first,
                                              rndResultFwdCi.second));

            ASSERT(LENGTH_CI == bsl::distance(fwdResultRndCi.first,
                                              fwdResultRndCi.second));
      ...
      case 2: {
        ...
        if (verbose) printf("\n" "Default Equality Functor" "\n");
        {
            ...
            // Create a object without providing equality comparison functor.
        ...
        if (verbose) printf("\n" "Non-Default Equality Functor" "\n");
        {
            ...
            ASSERT(needleFirst        == X.needleFirst());
            ASSERT(needleLast         == X.needleLast());
            ASSERT(functor.id()       == X.equal().id());
            ASSERT(functor.useCount() == X.equal().useCount());
            ...
            ASSERT(bsl::distance(needleFirst, needleLast) > 0);
            --needleLast;  // Fortunately, we are using a random iterator.
            ...
            // Show that a copy is returned.

            const CharEqualCaseInsensitive& equal = X.equal();
            ASSERT(equal.useCount() == X.equal().useCount());
      ...
      case -1: {
        ...
        getDataForHaystack(&DATA, &NUM_DATA, haystackOption);
        if (0 != rc) {
            P(haystackOption)
            fprintf(stderr, "3 %s\n", performanceSyntaxMessage(test));
            return -1;                                                // RETURN
        ...
        for (BSL::size_t ti = 0; ti < NUM_DATA; ++ti) {
            ...
            if (0 < numRepetitions) {
                ...
                ASSERTV(wasFound, wasFound == EXPECTED);
