...
void *bsl::Function_Rep::initRep(std::size_t       sooFuncSize,
                                 ...
                                 integral_constant<AllocCategory,
                                                   e_BSLMA_ALLOC_PTR>)
{
    // TBD: the 'alloc' passed in occasionally '0'.  Should this be supported
    // behavior?  If so, update the documented contract accordingly.  Otherwise
    // we should assume the BDE default ruless that pointers may not be null
    // unless explicitly documented, and so 'BSLS_ASSERT(basicAllocator);', in
    // addition to tracking down which 'function' calls are forwarding a null
    // pointer to this 'imp' method.
