...
// reference type, function type, or already 'const'-qualified at the
// top-level.
//
...
struct AddConst_Imp {
    // This 'struct' template provides an alias 'Type' that add the 'const'
    // qualifier to the (template parameter) 'TYPE' if the (template parameter)
    // 'ADD_CONST_FLAG' is 'true'.  This generic default template adds the
    // 'const' qualifier to 'TYPE' in the 'Type' alias.  A template
    ...
    typedef TYPE const Type;
        ...
        // (template parameter) 'TYPE' except that a top-level 'const'
...
struct add_const {
    ...
    // reference, nor a function, nor already 'const'-qualified, then 'type' is
    // an alias to 'TYPE' with a top-level 'const' qualifier added; otherwise,
