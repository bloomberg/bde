...
//
//@CLASSES:
//  bslmf::IsBitwiseMoveable<TYPE>: bit-wise moveable trait metafunction
//
//@SEE_ALSO:
//
...
//  p1->~TYPE();
//..
// An object of a 'TYPE' is *bit-wise* *moveable*, if the above operation
...
// classes are are bit-wise moveable, but generic code must assume that an
// arbrary 'TYPE' is not bitwise-moveable, as bit-wise moving a type that is
// not bit-wise moveable is likely to result in a dangling pointer.  Thus, it
// is necessary to explicitly associate the bit-wise moveable trait with a
// class (via template specialization or by use of the
// 'BSLMF_DECLARE_NESTED_TRAIT' macro) in order to generic algorithms to
// recognize that class as bit-wise moveable.
//
///What classes are not bit-wise moveable?
///---------------------------------------
...
//:   not bit-wise moveable.
//: o Its constructor registers a pointer to itself in some static registry.
//: o It contains a data member that is not bit-wise moveable.
//
...
// bit-wise moveable but not also bit-wise copiable.  For example, a class
