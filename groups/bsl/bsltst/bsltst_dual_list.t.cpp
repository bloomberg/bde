...
//                             TEST PLAN
//
// All we test here is that std::list and bsl::list are distinct types and
// whether they use the bslma_Allocator.  Testing of std::list is not our
// responsibility, and detailed testing of bsl::list is in
// bsltst_bsl_list.t.cpp.
//-----------------------------------------------------------------------------
// [ 1] Breathing Test
//-----------------------------------------------------------------------------
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      case 1: {
        ...
        ASSERT(false == sameType(bsl::list<int>(), std::list<int>()));
        ASSERT(false == sameType(bsl::list<int>(), native_std::list<int>()));
        ASSERT(true  == sameType(std::list<int>(), native_std::list<int>()));
        ASSERT(true  == usesBslmaAllocator(bsl::list<int>()));
        ASSERT(false == usesBslmaAllocator(std::list<int>()));
