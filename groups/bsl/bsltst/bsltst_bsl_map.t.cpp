...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      case 6: {
        ...
        bsl::multimap<int, int> ma(&ta), mb(&ta);
        int j;
        ...
        {
            Comparep comparator(Comparep::LESS);
            bsl::multimap<int, int, Comparep> mTmp(comparator);
            ASSERT(mTmp.empty());
            mTmp.insert(ma.begin(), ma.end());
            ASSERT(!mTmp.empty());
            j = 0;
            for (bsl::multimap<int, int, Comparep>::iterator it = mTmp.begin();
                 mTmp.end() != it; ++it, ++j) {
                LOOP2_ASSERT(it->first,  j, it->first  == j);
                LOOP2_ASSERT(it->second, j, it->second == j);
            }
        }

        {
            bsl::multimap<int, int> mTmp(ma);
            ASSERT(!mTmp.empty());
            ASSERT(mTmp == ma);
        }

        {
            bsl::multimap<int, int> mTmp(ma, &ta);
            ASSERT(!mTmp.empty());
            ASSERT(mTmp == ma);
        }

        {
            bsl::multimap<int, int> mTmp(ma.begin(), ma.end());
            ASSERT(!mTmp.empty());
            ASSERT(mTmp == ma);
        }

        {
            bsl::multimap<int, int, Comparep> mTmp(ma.begin(), ma.end(),
                                              Comparep(Comparep::LESS));
            ASSERT(!mTmp.empty());
            j = 0;
            for (bsl::multimap<int, int, Comparep>::iterator it = mTmp.begin();
                 mTmp.end() != it; ++it, ++j) {
        ...
        {
            bsl::multimap<int, int> mTmp(ma.begin(), ma.end(), &ta);
            ASSERT(!mTmp.empty());
            ASSERT(mTmp == ma);
        }

        {
            typedef bsl::multimap<int, int, Greaterp> BackMap;
            ...
            for (j = 0; j < 4; ++j) {
                LOOP2_ASSERT(mmGet(&mBackwards, j), j,
                                                   mmGet(&mBackwards, j) == j);
            }
            ...
            for (it = mBackwards.begin(), j = 3;
                ...
                LOOP2_ASSERT(j, it->second, j == it->second);
            }
            ASSERT(-1 == j);
        ...
        {
            bsl::multimap<int, int, Comparep> mBackwards(
                       ma.begin(), ma.end(), Comparep(Comparep::GREATER), &ta);
            ...
            bsl::multimap<int, int>::iterator it;
            for (it = mBackwards.begin(), j = 3;
                 mBackwards.end() != it; ++it, --j) {
                LOOP2_ASSERT(j, it->second, j == it->second);
            }
        ...
        bsl::pair<const int, int> prA(4,4);

        bsl::multimap<int, int>::iterator pResA = ma.insert(prA);
        ASSERT(ma.end() != pResA);
...
//      bsl::pair<bsl::map<int, int>::iterator, bool> pResB = ma.insert(prA);
//      pResB = ma.insert(prA);
//      ASSERT(!pResB.second);
...
//      ASSERT(4 == ma[4]);
        ...
        const bsl::multimap<int, int> mc(ma, &ta);
        ASSERT(mc == ma);
        ...
        bsl::multimap<int, int>::iterator it;
        bsl::multimap<int, int>::const_iterator itc;
        ...
        bsl::pair<bsl::multimap<int, int>::iterator,
                  bsl::multimap<int, int>::iterator> prIt;
        bsl::pair<bsl::multimap<int, int>::const_iterator,
                  bsl::multimap<int, int>::const_iterator> prCIt;
        ...
        for (j = 0, it = ma.begin(); j < 10; ++j, ++it) {
            ...
            for (int m = 0; m < k; ++m) {
                ...
                ASSERT(ma.end() != it);
                LOOP3_ASSERT(j, m, it->first, it->first == j);
                LOOP2_ASSERT(j, it->second, it->second == 2 * j);
            }
        }
        ASSERT(ma.end() == it);
        ...
        const bsl::multimap<int, int> mc2 = ma;
      ...
      case 5: {
...

        {
            ...
            ASSERT(!mTmp.empty());
            j = 0;
            for (bsl::map<int, int, Comparep>::iterator it = mTmp.begin();
                 mTmp.end() != it; ++it, ++j) {
                LOOP2_ASSERT(it->first,  j, it->first  == j);
                LOOP2_ASSERT(it->second, j, it->second == j);
            }
        ...
        {
            bsl::map<int, int> mTmp(ma);
            ASSERT(!mTmp.empty());
            ASSERT(mTmp == ma);
        }

        {
            bsl::map<int, int> mTmp(ma, &ta);
            ASSERT(!mTmp.empty());
            ASSERT(mTmp == ma);
        }

        {
            bsl::map<int, int> mTmp(ma.begin(), ma.end());
            ASSERT(!mTmp.empty());
            ASSERT(mTmp == ma);
        }

        {
            bsl::map<int, int, Comparep> mTmp(ma.begin(), ma.end(),
                                              Comparep(Comparep::LESS));
            ASSERT(!mTmp.empty());
            j = 0;
            for (bsl::map<int, int, Comparep>::iterator it = mTmp.begin();
        ...
        {
            bsl::map<int, int> mTmp(ma.begin(), ma.end(), &ta);
            ASSERT(!mTmp.empty());
            ASSERT(mTmp == ma);
        }

        {
            typedef bsl::map<int, int, Greaterp> BackMap;
            BackMap mBackwards(&ta);
            ...
            for (j = 0; j < 4; ++j) {
                LOOP2_ASSERT(mBackwards[j], j, mBackwards[j] == j);
            }
        ...
        {
            bsl::map<int, int, Comparep> mBackwards(
                       ma.begin(), ma.end(), Comparep(Comparep::GREATER), &ta);
        ...
        bsl::pair<bsl::map<int, int>::iterator, bool> pResA = ma.insert(prA);
        ASSERT(pResA.second);
        ...
        const bsl::map<int, int> mc(ma, &ta);
        ASSERT(mc == ma);
        ...
        bsl::map<int, int>::iterator it;
        bsl::map<int, int>::const_iterator itc;
        ...
        bsl::pair<bsl::map<int, int>::iterator,
                  bsl::map<int, int>::iterator> prIt;
        bsl::pair<bsl::map<int, int>::const_iterator,
                  bsl::map<int, int>::const_iterator> prCIt;
      ...
      case 4: {
        ...
        bsl::less<int> lessInt;
        Greaterp greaterInt;

        {
            bsl::multimap<int, Cargo> m;
        ...
        {
            bsl::multimap<int, Cargo> m(lessInt);
        ...
        {
            bsl::multimap<int, Cargo> m(lessInt, &ta);
...

        {
            ...
            bsl::multimap<int, Cargo> mm(m.begin(), m.end());
            for (int i = 0; i < 10; ++i) {
...

        {
            ...
            bsl::multimap<int, Cargo, Greaterp>
                                            mm(m.begin(), m.end(), greaterInt);
            for (int i = 0; i < 10; ++i) {
                ASSERT(mm.count(i) == (i < 4 ? 1 : 0));
            }
...

        {
            ...
            bsl::multimap<int, Cargo, Greaterp>
                                       mm(m.begin(), m.end(), greaterInt, &ta);
            for (int i = 0; i < 10; ++i) {
                ASSERT(mm.count(i) == (i < 4 ? 1 : 0));
            }
        ...
        {
            bsl::multimap<int, Cargo> mmA;
            const bsl::multimap<int, Cargo>& MMA = mmA;
            ...
            bsl::multimap<int, Cargo> mmB(MMA);
            for (int i = 0; i < 10; ++i) {
...

        {
            ...
            bsl::multimap<int, Cargo> mmB(MMA, &ta);
            for (int i = 0; i < 10; ++i) {
                ASSERT(mmB.count(i) == (i < 2 ? 2 : 0));
            }
      ...
      case 3: {
        ...
        std::multimap<int, int> s(&ta);
        native_std::multimap<int, int> n;

        bsl::multimap<int, int>::iterator bit;
        std::multimap<int, int>::iterator sit;
        native_std::multimap<int, int>::iterator nit;
      ...
      case 2: {
        ...
        //   amounts of memory, and verify that large amounts of memory are
        //   consumed in the allocator that was passed to the container.
        //   Create maps with all c'tors in the standard.
        // --------------------------------------------------------------------
        ...
        {
            bsl::map<int, Cargo> m(lessInt, &ta);
            ...
            LOOP_ASSERT(ta.numBytesInUse(),
                                    ta.numBytesInUse() >= memUsed + 12 * 1000);
            LOOP_ASSERT(tda.numBytesInUse(),
                                        defaultMemUsed == tda.numBytesInUse());
            PV(ta.numBytesInUse());
        }

        {
            ...
            int defaultMemUsed = tda.numBytesInUse();
            bsl::map<int, Cargo> mB(mA.begin(), mA.end());
            for (int i = 0; i < 10; ++i) {
...

        {
            ...
            int defaultMemUsed = tda.numBytesInUse();
            bsl::map<int, Cargo, Greaterp>
                                          mB(mA.begin(), mA.end(), greaterInt);
            for (int i = 0; i < 10; ++i) {
                ASSERT((i < 3) == mB.count(i));
            }
...

        {
            ...
            bsl::map<int, Cargo, Greaterp>
                                     mB(mA.begin(), mA.end(), greaterInt, &ta);
            for (int i = 0; i < 10; ++i) {
                ASSERT((i < 3) == mB.count(i));
            }
        ...
        {
            bsl::map<int, Cargo> mA;    const bsl::map<int, Cargo>& MA = mA;
      ...
      case 1: {
        ...
        std::map<int, int> s(&ta);
        native_std::map<int, int> n;

        bsl::map<int, int>::iterator bit;
        std::map<int, int>::iterator sit;
        native_std::map<int, int>::iterator nit;
