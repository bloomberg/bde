...
// only with compilers for which 'BSLS_PLATFORM__CMP_GNU' is defined.
//
...
//    This annotation is used to inform the compiler that the return value of
//    the so-annotated function is the address of an allocated block of memory
//    whose size (in bytes) is given by one or two of the function parameters.
//    Certain compilers use this information to improve the correctness of
//    built-in object-size functions (e.g., '__builtin_object_size' with
//    'BSLS_PLATFORM__CMP_GNU').
//
//    The function parameter(s) denoting the size of the block are specified by
//    one or two integer arguments supplied to the macro.  The allocated size
//    (in bytes) is either the value of the single function argument or the
//    product of the two arguments.  Argument numbering starts at one.  For
//    example:
//
...
//    time), and the diagnostic will include the exact location of the function
//    call.
//
...
//    functions that take 'printf'-style arguments, which should be
//    type-checked against a format string.
//
//    The 'stringIndex' parameter is the one-based index to the 'const' format
//    string.  The 'firstToCheck' parameter is the one-based index to the
...
//    argument parameter that is a valid format string for a 'printf'-style
//    function and returns a format string that is consistent with that format.
//    This allows format strings manipulated by translation functions to be
...
//    warnings about non-literal formats.  For example:
//
//    const char *translateFormat(const char *locale, const char *format)
//                                                   BSLS_ANNOTATION_FORMAT(2);
//
//    On a conforming compiler, this will validate the "Mike" argument against
//    the 'format' specification passed to 'translateFormat'.
//
...
//    must not be null.  If the compiler determines that a null pointer is
//    passed to an argument slot marked by this annotation, a warning is
//    issued.  If the 'BSLS_ANNOTATION_ARG_NON_NULL' annotation is used, it
//    expects a variable list of argument slots to be specified.  An argument
//    slot is a one-based index of the argument in the function parameters.
//    The 'BSLS_ANNOTATION_ARGS_NON_NULL' annotation specifies that all pointer
//    arguments must not be null.
//
//  BSLS_ANNOTATION_NULL_TERMINATED
//  BSLS_ANNOTATION_NULL_TERMINATED_AT(x)
//    This annotation ensures that a parameter in a function call is an
//    explicit 'NULL'.  The annotation is valid only on variadic functions.  By
//    default, the sentinel is located at position 0, the last parameter of the
//    function call.  If an optional position is specified, the sentinel must
//    be located at that index, counting backwards from the end of the argument
//    list.
//
...
//    what they should use instead.  For example:
//
...
#if defined(BSLS_PLATFORM__CMP_GNU) && BSLS_PLATFORM__CMP_VER_MAJOR >= 30300
...
#if defined(BSLS_PLATFORM__CMP_GNU) && BSLS_PLATFORM__CMP_VER_MAJOR >= 30100
    #define BSLS_ANNOTATION_DEPRECATED __attribute__((__deprecated__))
#else
    #define BSLS_ANNOTATION_DEPRECATED
#endif

#if defined(BSLS_PLATFORM__CMP_GNU) || defined(BSLS_PLATFORM_CMP_IBM)
...
#if defined(BSLS_PLATFORM__CMP_GNU) && BSLS_PLATFORM__CMP_VER_MAJOR >= 30400
