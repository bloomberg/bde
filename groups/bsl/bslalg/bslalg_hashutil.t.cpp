...
//-----------------------------------------------------------------------------
#if 0
namespace HashUtil_USAGE_EXAMPLE {
...
//..
        ...
        // is required that 'input' never be default nor repeat itself.
    {
        ...
        for (int i = 0; i < numElements; ++i, input.next()) {
            unsigned int hash = HashUtil::hash1(input.data(),
                                                     input.length());
            ++table[hash % size];
        }
...
//..
        ...
        // is required that 'input' never be default nor repeat itself.
    {
        ...
        for (int i = 0; i < numElements; ++i, input.next()) {
            unsigned int hash1 = HashUtil::hash1(input.data(),
                                                      input.length());
            ...
            if (ResultType() == table[bucket]) {
                ...
                unsigned int computeHash = HashUtil::computeHash(input.data(),
                                                          input.length());
...
// identical for 'hash1' (used in the code above) or 'computeHash' (code identical
// except 'computeHash' is used in place of 'hash1' in 'computeChainingCollisions').
//..
// Displaying average(max) for chaining:
// --------------------------------------
...
} // close namespace HashUtil_USAGE_EXAMPLE
#endif
//=============================================================================
//                              MAIN PROGRAM
//-----------------------------------------------------------------------------
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
        ...
        //using namespace HashUtil_USAGE_EXAMPLE;
      ...
      case 2: {
        ...
        ASSERT(2509914878U == HashUtil::computeHash((unsigned long)0x12345678));
        ASSERT(2509914878U ==
