...
//
// * `bslma::UsesBslmaAllocator<TYPE>` is explicitly specialized to derive
//   from `bsl::true_type`.
// * `TYPE` has a 'BSLMF_NESTED_TRAIT_DECLARATION(TYPE,
//   bslma::UsesBslmaAllocator);' declaration.
// * There exists a nested `TYPE::allocator_type` that is convertible from
...
// * `bslma::Allocator *` is implicitly convertible to `TYPE` but other,
...
//
// * The allocator supplied at construction of an object will be used for
...
// * The allocator used by an object is not changed after construction (e.g.,
//   the assignment operator does not change the allocator used by an object).
// * Transient memory allocations -- i.e., allocations performed within the
...
// * The allocator used by an object is not part of an object's value (e.g.,
//   it is not tested by the equality-comparison operator `operator==`).
// * If an allocator is not supplied at construction, then the currently
//   installed default allocator will typically be used (see `bslma_default`).
// * If a move constructor is called with no allocator argument, the allocator
//   used by the new object will be the same as the allocator used by the
//   source object.
// * Singleton objects, when necessary, allocate memory from the global
...
#define BSLMA_USESBSLMAALLOCATOR_DEPRECATE_IF(COND, MSG) enum {           \
    ...
    = sizeof(UsesBslmaAllocator_DeprecatedIf<!!(COND)>::check(MSG)) }
              ...
              // ==============================================
              // class SuppressesImplicitConvertibilityWarnings
              // ==============================================

/// This `struct` template implements a meta-function to determine whether a
/// deprecation warning emitted for allocator-aware types that have an
/// implicit conversion constructor from `Allocator*`, should be suppressed
/// for (template parameter) `t_TYPE`.  This `struct` derives from
/// `bsl::true_type` if the warning should be suppressed and from
/// `bsl::false_type` otherwise.  Note that this meta-function is used for
/// testing to disable the warnings generated by this component when
/// deliberately testing the deprecated feature.
template <class t_TYPE>
struct SuppressesImplicitConvertibilityWarnings : bsl::false_type
{
};
...
struct UsesBslmaAllocator_Sniff {
  ...
  private:
    ...
    enum {
        ...
        // User can explicitly suppress deprecation warnings.
        k_SUPPRESS_DEPRECATION_WARNINGS =
                         SuppressesImplicitConvertibilityWarnings<TYPE>::value,
    ...
    BSLMA_USESBSLMAALLOCATOR_DEPRECATE_IF(
        k_VALUE && !k_SUPPRESS_DEPRECATION_WARNINGS,
...
/// * `TYPE` has a 'BSLMF_NESTED_TRAIT_DECLARATION(TYPE,
///   bslma::UsesBslmaAllocator);' declaration.
/// * `TYPE` has a nested `allocator_type` and `bslma::Allocator *` is
