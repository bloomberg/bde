...
// unique counter, using a 'bslma_TestAllocator'.  As each object is destroyed,
// its destructor increments the counter held by the object, indicating the
// number of times the object's destructor is called.  After the proctor is
// destroyed, we verify that the corresponding counters of the object managed
// by the proctor are modified, and all allocated memory are deallocated.
//-----------------------------------------------------------------------------
// [3] bslma_RawDeleterProctor<TYPE, ALLOCATOR>(obj, allocator);
// [3] ~bslma_RawDeleterProctor<TYPE, ALLOCATOR>();
// [4] void release();
// [5] void reset(obj);
//-----------------------------------------------------------------------------
// [1] Breathing Test
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      case 6: {
        ...
        //   of 'my_List' using a 'bslma_TestAllocator' to verify that memory
        ...
        bslma_TestAllocator z(veryVeryVerbose);
        const bslma_TestAllocator &Z = z;
        int counter = 0;
      ...
      case 5: {
        ...
        //
        // Plan:
        //   Create a 'my_Class' object using a 'bslma_TestAllocator' and
        //   initialize it with a counter.  Next create another 'my_Class'
        //   object and initialize it with a different counter.  Finally
        //   initialize a 'bslma_RawDeleterProctor' object with the first
        //   object and 'bslma_TestAllocator'.  Call 'reset' on the proctor
        ...
        my_Class *pC1;
        {
            ...
            bslma_RawDeleterProctor<my_Class, bslma_Allocator> proctor(pC1,
                                                                       &z);
            ASSERT(0                    == counter1);
            ASSERT(0                    == counter2);
            ASSERT(2 * sizeof(my_Class) == Z.numBytesInUse());
      ...
      case 4: {
        ...
        //
        // Plan:
        //   Create 'my_Class' objects using 'bslma_TestAllocator' and
        //   initialize it with a counter.  Next initialize a
        //   'bslma_RawDeleterProctor' object with the corresponding 'my_Class'
        //   object and 'bslma_TestAllocator'.  Call 'release' on the proctor
        //   before it goes out of scope.  Verify that the counter is not
        //   incremented, and the memory allocated by the test allocator is not
        //   deallocated.
        //
        // Testing:
        //   void release();
        ...
        my_Class *pC;
        {
            ...
            bslma_RawDeleterProctor<my_Class, bslma_Allocator> proctor(pC, &z);
            ASSERT(0                == counter);
            ASSERT(sizeof(my_Class) == Z.numBytesInUse());
      ...
      case 3: {
        ...
        //   'bslma_Allocator' is supplied to the 'bslma_RawDeleterProctor',
        ...
        //   'bslma_RawDeleterGuard' object to guard the created 'myChild'
        ...
        //
        // Testing:
        //   bslma_RawDeleterProctor<TYPE, ALLOCATOR>(obj, allocator);
        ...
        if (verbose) cout << "\nTesting with bslma_TestAllocator" << endl;
        {
                                 ...
                                 "class pointer" << endl;
            {
                ...
                bslma_RawDeleterProctor<myChild, bslma_Allocator>
                                                               proctor(pC, &z);
                ASSERT(0               == counter);
                ASSERT(sizeof(myChild) == Z.numBytesInUse());
            }
                                 ...
                                 "class pointer" << endl;
            {
                ...
                bslma_RawDeleterProctor<myParent, bslma_Allocator>
                                                               proctor(pP, &z);
                ASSERT(0               == counter);
                ASSERT(sizeof(myChild) == Z.numBytesInUse());
            }
                                 ...
                                 "class pointer" << endl;
            {
                ...
                bslma_RawDeleterProctor<myMostDerived, bslma_Allocator>
                                                              proctor(pMD, &z);
                ASSERT(0                     == counter);
                ASSERT(sizeof(myMostDerived) == Z.numBytesInUse());
            }
        ...
        if (verbose) cout << "\nTesting with my_Pool" << endl;
        {
            ...
            myChild *pC = 0;
            {
                ...
                bslma_RawDeleterProctor<myChild, my_Pool> proctor(pC,
                                                                  &myAlloc);
                ASSERT(0               == counter);
                ASSERT(0               == allocCounter);
                ASSERT(sizeof(myChild) == Z.numBytesInUse());
            ...
            myParent *pP = 0;
            {
                ...
                bslma_RawDeleterProctor<myParent, my_Pool> proctor(pP,
                                                                  &myAlloc);
                ASSERT(0               == counter);
                ASSERT(0               == allocCounter);
                ASSERT(sizeof(myChild) == Z.numBytesInUse());
            ...
            myMostDerived *pMD;
            {
                ...
                bslma_RawDeleterProctor<myMostDerived, my_Pool>
                                                        proctor(pMD, &myAlloc);
                ASSERT(0                     == counter);
                ASSERT(0                     == allocCounter);
                ASSERT(sizeof(myMostDerived) == Z.numBytesInUse());
      ...
      case 1: {
        ...
        //
        // Concerns:
        //   1) The 'bslma_RawDeleterProctor' can be constructed and destructed
        //      gracefully.
        //   1) The allocator's 'deallocate' method is invoked.
        //   2) The (managed) object's destructor is invoked.
        //
        // Plan:
        //   Allocate a 'std::string' with a 'bslma_TestAllocator' and guard it
        //   with 'bslma_RawDeleterProctor' to show that both the destructor
        ...
        if (verbose) cout << "\tTesting with 'string' object" << endl;
        {
            ...
            ASSERT(0 < Z.numBytesInUse());
            *s = "Hello World!";
            bslma_RawDeleterProctor<string, bslma_Allocator> guard(s,
                                                                   &allocator);
        }
        ASSERT(0 == Z.numBytesInUse())
