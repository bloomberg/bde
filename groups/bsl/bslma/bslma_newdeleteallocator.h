...
//@PURPOSE: Provide singleton new/delete adaptor to 'bslma_Allocator' protocol.
//
//@CLASSES:
//  bslma_NewDeleteAllocator: support new/delete-style allocation/deallocation
//
...
// 'bslma_NewDeleteAllocator', that implements the 'bslma_Allocator' protocol
...
//                       deallocate
//..
// The essential purpose of this component is to facilitate the default use
// of global 'new' and 'delete' in all components that accept a user-supplied
// allocator derived from 'bslma_Allocator' (see 'bslma_default').  Hence, the
// global 'operator new' and 'operator delete' functions are wrapped within
// concrete methods of a derived 'bslma_NewDeleteAllocator' class.  A
// 'static' (factory) method of 'bslma_NewDeleteAllocator' can be used to
// obtain a unique (singleton) 'bslma_NewDeleteAllocator' object for the given
// process, and whose lifetime is guaranteed to exceed any possibility of its
// use.  Note that the standard also requires the global 'operator new' to
// return maximally-aligned memory, which is a stricter post condition than
// the natural-alignment requirement imposed by the base-class contract, or
// than is provided by many other concrete implementations.
//
///Thread Safety
///-------------
...
// singleton 'bslma_NewDeleteAllocator' can also be safely created/accessed
// concurrently (from multiple threads) via either the 'static' 'singleton'
// or 'allocator' (factory) methods.  Moreover, the underlying (native)
...
// created using a 'bslma_NewDeleteAllocator' will *not* invoke the
...
class bslma_NewDeleteAllocator : public bslma_Allocator {
    // This class defines a concrete mechanism that adapts the system-supplied
    // (native) global 'operator new' and 'operator delete' to the
    // 'bslma_Allocator' protocol.  The class method 'singleton' returns a
    // process-wide unique object of this class whose lifetime is guaranteed
    // to extend from the first call to 'singleton' until the program
    // terminates.  A second class method, 'allocator', allows for
    // conveniently replacing a "null" allocator with this singleton object.
    // Note that this entire class should generally not be used directly
    // by typical clients (see 'bslma_default' for more information).

    // NOT IMPLEMENTED
    bslma_NewDeleteAllocator(const bslma_NewDeleteAllocator&);
    bslma_NewDeleteAllocator& operator=(const bslma_NewDeleteAllocator&);
  ...
  public:
    ...
    static bslma_NewDeleteAllocator& singleton();
        ...
        // call of this method until the program terminates.  Note that
        // this method should generally not be used directly by typical
        // clients (see 'bslma_default' for more information).
    ...
    static bslma_Allocator *allocator(bslma_Allocator *basicAllocator);
        ...
        //..
        // Also note that if a 'bslma_NewDeleteAllocator' object is supplied,
        // it is owned by the class and must NOT be deleted.  Finally note
        // that this method should generally not be called directly by
        // typical clients (see 'bdema_default' for more information).

    // CREATORS
    bslma_NewDeleteAllocator();
        // Create a ("stateless") new-delete-allocator object that wraps the
        // global native 'operator new' and 'operator delete' functions in
        // order to supply memory via the 'bslma_Allocator' protocol.  Note
        // that all objects of this class share the same underlying resource;
        // hence, this constructor should generally not be invoked directly by
        // clients; instead, consider using the 'static' 'singleton' or
        // 'allocator' (factory) methods, or -- better -- the appropriate
        // ones in 'bslma_Default' (see 'bslma_default' for more information).

    virtual ~bslma_NewDeleteAllocator();
    ...
    virtual void *allocate(size_type size);
        ...
        // *not* called when 'size' is 0 (in order to avoid having to acquire
        // a lock, and potential contention in multi-threaded programs).
    ...
    virtual void deallocate(void *address);
        ...
        // order to avoid having to acquire a lock, and potential contention
...
// CLASS METHODS
inline
bslma_Allocator *bslma_NewDeleteAllocator::allocator(
                                               bslma_Allocator *basicAllocator)
{
...
// MANIPULATORS
inline
void bslma_NewDeleteAllocator::deallocate(void *address)
{
