...
// CREATORS
// [ 2] MovableTestType();
// [ 3] MovableTestType(int data);
// [ 7] MovableTestType(const MovableTestType& original);
// [ 2] ~MovableTestType();
//
// MANIPULATORS
// [ 9] MovableTestType& operator=(const MovableTestType& rhs);
...
//..
      ...
      case 2: {
        ...
        unsigned changed = u::numBitsChanged(xBuffer.address(),
                                             yBuffer.address(),
                                             sizeof(xBuffer));
        ASSERT(changed <= sizeof(void *) * 8 || !isDtorExecuted);
