...
// 'bdlqq::AtomicUtil' types and operations.  Since the provided classes are
...
// respective 'bdlqq::AtomicUtil' type and operation.
//
//-----------------------------------------------------------------------------
// bdlqq::AtomicInt
// --------------
...
// [ 6] int operator --();
// [ 6] int operator --(int);
// [ 3] bdlqq::AtomicInt& operator= (const bdlqq::AtomicInt& rhs);
// [ 2] bdlqq::AtomicInt& operator= (int value);
...
// [ 6] bsls::Types::Int64 operator --();
// [ 6] bsls::Types::Int64 operator --(int);
// [ 3] bdlqq::AtomicInt64& operator= (const bdlqq::AtomicInt64& rhs);
// [ 2] bdlqq::AtomicInt64& operator= (bsls::Types::Int64 value);
...
typedef bdlqq::AtomicInt                      AI;
typedef bdlqq::AtomicInt64                    AI64;
typedef bdlqq::AtomicPointer<APTestObj>       AP;
typedef bdlqq::AtomicPointer<const APTestObj> CAP;
typedef bdlqq::SpinLock                       SL;
...
public:
    struct Data
    {
        bdlqq::AtomicInt  flags[2];
        bdlqq::AtomicInt  turn;     // 0 or 1 - index into d_flags
    };
...
static bdlqq::AtomicInt64 transactionCount;
static bdlqq::AtomicInt64 successCount;
static bdlqq::AtomicInt64 failureCount;
...
class my_CountedHandleRep {
    bdlqq::AtomicInt    d_count;       // number of active references
...
class my_PtrStack {
    struct Node {
        TYPE                 *d_item;
        Node                 *d_next;
        bdlqq::AtomicUtil::Int  d_dirtyBit;
    };
    bdlqq::AtomicPointer<Node>  d_list_p;
    bdlqq::AtomicPointer<Node>  d_freeList_p;
...
typename my_PtrStack<TYPE>::Node* my_PtrStack<TYPE>::allocateNode()
{
    ...
    do {
        ...
        if (bdlqq::AtomicUtil::swapInt(&node->d_dirtyBit, 1)) {
            continue;
        }
        if (d_freeList_p.testAndSwap(node, node->d_next) == node) {
            break;
        }
        bdlqq::AtomicUtil::setInt(&node->d_dirtyBit, 0);
    } while (1);
    if (!node) {
        bslma::Allocator *ba = bslma::Default::allocator(d_allocator_p);
        node = new(*ba) Node();
        bdlqq::AtomicUtil::setInt(&node->d_dirtyBit, 1);
    }
    return node;
}
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:
      ...
      case 3: {
        ...
        //
        // Testing:
        //   bdlqq::AtomicInt(const bdlqq::AtomicInt& rhs);
        //   bdlqq::AtomicInt(int value);
        //   bdlqq::AtomicInt& operator= (const bdlqq::AtomicInt& rhs);
        //   bdlqq::AtomicInt64(const bdlqq::AtomicInt64& original);
        //   bdlqq::AtomicInt64(bsls::Types::Int64 value);
      ...
      case 2: {
        ...
        //   bdlqq::AtomicInt();
        //   ~bdlqq::AtomicInt()
        //   bdlqq::AtomicInt& operator= (int value);
        //   operator int() const;;
        //   bdlqq::AtomicInt64();
        //   ~bdlqq::AtomicInt64();
        //   bdlqq::AtomicInt64& operator= (bsls::Types::Int64 value);
        //   operator bsls::Types::Int64() const;
        //   bdlqq::AtomicPointer();
        //   bdlqq::AtomicPointer<T>& operator= (const T *value);
        //   ~bdlqq::AtomicPointer();
        //   T& operator*() const;
        //   T* operator->() const;
        //   operator T*() const;
        ...
        if (verbose)
            cout << "\nTesting 'bdlqq::AtomicInt64' Primary Manipulators"
                 << endl;
        {
            static const struct {
                int   d_lineNum;  // source line number
