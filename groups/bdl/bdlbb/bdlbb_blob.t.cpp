...
// contracts of each function, namely: - that 'bdlbb::BlobBuffer' is an in-core
// value-semantic type containing a
...
class UnknownFactoryException : public bsl::exception {
  ...
  public:
    UnknownFactoryException(const char *s)
    : d_what(s)
    {
    }
...
void loadBlob(bdlbb::Blob *blob, bsl::string& dataString)
{
    ...
    for (; bufferIdx < NUM_DATA_BUFFERS; ++bufferIdx) {
        ...
        for (char *pc = buffer.data(), *end = pc + buffer.size();
             pc < end && dataCharIdx < DATA_LENGTH;
             ++pc, ++dataCharIdx) {
            *pc = data[dataCharIdx];
        }
    }
...
{
    bsl::shared_ptr<char> shptr((char *)d_allocator_p->allocate(d_bufferSize),
                                d_allocator_p);
...
// ways of manipulating a blob: the simplest lets the blob automatically manage
// the length, by using only 'prependBuffer', 'appendBuffer', and
// 'insertBuffer'.  Consider the following typical utilities (these utilities
// are to illustrate usage, they are not meant to be copy-pasted into
// application programs although they can provide a foundation for application
// utilities): ..
void prependProlog(bdlbb::Blob        *blob,
    ...
    // optionally specified 'allocator' to supply any memory (or the currently
    // installed default allocator if 'allocator' is 0).  The behavior is
    // undefined unless 'blob' points to an initialized 'bdlbb::Blob' instance.
...
void composeMessage(bdlbb::Blob                      *blob,
    ...
    // 'deleter' to destroy them.  Use the optionally specified 'allocator' to
    ...
    // message.  Note also that 'blob' need not have been created with a blob
    // buffer factory.  The behavior is undefined unless 'blob' points to an
    // initialized 'bdlbb::Blob' instance.
...
int timestampMessage(bdlbb::Blob *blob, bslma::Allocator *allocator = 0);
    ...
    // the optionally specified 'allocator' to supply memory, or the currently
    // installed default allocator if 'allocator' is 0.  The behavior is
    // undefined unless the specified 'blob' points to an initialized
    // 'bdlbb::Blob' instance with at least one data buffer.
    //..
...
int timestampMessage(bdlbb::Blob *blob, bslma::Allocator *allocator)
{
    ...
    bsl::memcpy(
        timestampBuffer.data(), bdexStream.data(), bdexStream.length());
...
int main(int argc, char *argv[])
{
    ...
    switch (test) {
      ...
      case 14: {
        ...
        // application, a better implementation that pools buffers is available
        // in the 'bdlbb_pooledblobbufferfactory' component.
        //
        ///Simple Blob Usage
///- - - - - - - - -
        // Blobs can be created just by passing a factory that is responsible
        // to allocate the 'bdlbb::BlobBuffer'.  The following simple program
        // illustrates how.
        //..
        {
...
//..
...

        {
            ...
            const char *const MSG[] = {
                "Here is the first piece", "A second piece", "trailer"};
            const int MSG_SIZES[]     = {static_cast<int>(bsl::strlen(MSG[0])),
            ...
            ASSERT(PROLOG_LENGTH + TIMESTAMP_LENGTH + MSG_LENGTH ==
                   blob.length());
      ...
      case 13: {
...

        {
            ...
            //          bslma::Allocator
            //          *allocator = bslma::Default::allocator();
      ...
      case 11: {
        ...
        if (verbose)
            cout << endl
                 << "moveDataBuffers & moveAndAppendDataBuffers" << endl
                 << "==========================================" << endl;
            ...
            for (int numBuffers1 = 0; numBuffers1 <= 3; ++numBuffers1) {
                for (int dataLength1 = 0;
                    ...
                    const int NUM_DATA_BUFFERS1 =
                        (dataLength1 + BUFFER_SIZE1 - 1) / BUFFER_SIZE1;
                    const int LAST_DATA_BUFFER_LENGTH1 =
                        0 == DATA_LENGTH1
                            ? 0
                            : DATA_LENGTH1 -
                                  ((NUM_DATA_BUFFERS1 - 1) * BUFFER_SIZE1);
                    ...
                    if (veryVerbose) {
                        ...
                        P_(DATA_LENGTH1);
                        P_(TOTAL_SIZE1)
                        P_(NUM_DATA_BUFFERS1) P(LAST_DATA_BUFFER_LENGTH1)
                    }
                    ...
                    ASSERT(LAST_DATA_BUFFER_LENGTH1 ==
                           X.lastDataBufferLength());

                    for (int bufferSize2 = 1; bufferSize2 <= 6;
                         bufferSize2 += 2) {
                        for (int numBuffers2 = 0; numBuffers2 <= 3;
                             ++numBuffers2) {
                            for (int dataLength2 = 0;
                                ...
                                const int TOTAL_SIZE2 =
                                    NUM_BUFFERS2 * BUFFER_SIZE2;
                                ...
                                ASSERT(NUM_DATA_BUFFERS2 ==
                                       Z1.numDataBuffers());
                                ASSERT(LAST_DATA_BUFFER_LENGTH2 ==
                                       Z1.lastDataBufferLength());
                                ...
                                ASSERT(NUM_DATA_BUFFERS1 ==
                                       Y1.numDataBuffers());
                                ASSERT(LAST_DATA_BUFFER_LENGTH1 ==
                                       Y1.lastDataBufferLength());
                                ...
                                //          const int
                                //          TOTAL_TOTAL1 = Z1.totalSize() + Y1.totalSize();
                                //          const int
                                //          TOTAL_TOTAL2 = Z2.totalSize() + Y2.totalSize();
                                ...
                                if (veryVerbose) {
                                    P_(BUFFER_SIZE1)
                                    P_(NUM_BUFFERS1)
                                    P_(DATA_LENGTH1) P_(TOTAL_SIZE1) P_(
                                        NUM_DATA_BUFFERS1)
                                        P_(Y2.lastDataBufferLength()) P_(
                                            BUFFER_SIZE2) P_(NUM_BUFFERS2)
                                            P_(DATA_LENGTH2) P_(TOTAL_SIZE2)
                                                P_(NUM_DATA_BUFFERS2) P_(
                                                    Z2.lastDataBufferLength())
                                }

                                ASSERT(NUM_BUFFERS1 - NUM_DATA_BUFFERS1 ==
                                       Y1.numBuffers());
                                ASSERT(Y1.numBuffers() * BUFFER_SIZE1 ==
                                       Y1.totalSize());
                                ...
                                ASSERT(Y2.numBuffers() * BUFFER_SIZE1 ==
                                       Y2.totalSize());
                                ASSERT(0 == Y2.length());
                                ...
                                if (0 == Z1.length()) {
                                    LOOP2_ASSERT(NUM_DATA_BUFFERS1,
                                                 Z1.numBuffers(),
                                                 NUM_BUFFERS2 +
                                                         NUM_DATA_BUFFERS1 ==
                                                     Z1.numBuffers());
                                ...
                                LOOP2_ASSERT(NUM_DATA_BUFFERS1,
                                             ...
                                             NUM_DATA_BUFFERS1 ==
                                                 Z1.numDataBuffers());
                                LOOP4_ASSERT(DATA_LENGTH1,
                                             DATA_LENGTH2,
                                             LAST_DATA_BUFFER_LENGTH1,
                                             Z1.lastDataBufferLength(),
                                             LAST_DATA_BUFFER_LENGTH1 ==
                                                 Z1.lastDataBufferLength());
                                LOOP2_ASSERT(data1Substr,
                                             z1Data,
                                             data1Substr == z1Data);

                                const int EXP_NUM_BUFFERS =
                                    bsl::max(NUM_BUFFERS2,
                                ...
                                LOOP4_ASSERT(DATA_LENGTH1,
                                             ...
                                             LAST_DATA_BUFFER_LENGTH1,
                                             Z2.lastDataBufferLength(),
                                             APPEND_LAST_DATA_BUFFER_LENGTH ==
                                                 Z2.lastDataBufferLength());
                                LOOP2_ASSERT(
                                    EXP_DATA, z2Data, EXP_DATA == z2Data);
                            }
                        }
                    }
                }
      ...
      case 10: {
        ...
        for (int bufferSize = 1; bufferSize <= 5; ++bufferSize)
            for (int numBuffers = 1; numBuffers <= 5; ++numBuffers)
                for (int dataLength = 1; dataLength <= bufferSize * numBuffers;
                     ++dataLength) {
                    bslma::TestAllocator         defaultAlloc(veryVeryVerbose);
                    bslma::DefaultAllocatorGuard guard(&defaultAlloc);
                    bslma::TestAllocator         ta(veryVeryVerbose);
                    ...
                    BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator)
                    {
                        ...
                        for (int i = 0; i < numBuffers; ++i) {
                            ...
                            ASSERT(BUFFER_SIZE == src.size());
                            ASSERT(
                               compareBuffersData(X, numBuffers, dstChar, i) );
                            ASSERT(
                                 compareBlobBufferData(X.buffer(i), srcChar) );
                ...
                for (int dataLength = 0; dataLength <= bufferSize * numBuffers;
                    ...
                    for (int prependSz = 1; prependSz <= 5; ++prependSz) {
                        ...
                        BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator)
                        {
                            ...
                            const int EXP_NUM_DATA_BUFFERS =
                                (dataLength + BUFFER_SIZE - 1) / BUFFER_SIZE;
                            const int EXP_NUM_BUFFERS = NUM_BUFFERS;
                            const int EXP_LAST_DB_LENGTH =
                                EXP_NUM_DATA_BUFFERS > 0
                                    ? DATA_LENGTH -
                                          (EXP_NUM_DATA_BUFFERS - 1) *
                                              BUFFER_SIZE
                                    : 0;
                            if (veryVerbose) {
                                T_;
                            ...
                            ASSERT(EXP_LAST_DB_LENGTH ==
                                   X.lastDataBufferLength());

                            SimpleBlobBufferFactory ifa(PREPEND_BUFFER_SIZE,
                                                        &ta);
                            bdlbb::BlobBuffer       buffer;
                            ifa.allocate(&buffer);
                            ...
                            ASSERT(DATA_LENGTH + PREPEND_BUFFER_SIZE ==
                                   X.length());
                            if (0 < DATA_LENGTH) {
                                ASSERT(EXP_NUM_DATA_BUFFERS + 1 ==
                                       X.numDataBuffers());
                                ASSERT(EXP_LAST_DB_LENGTH ==
                                       X.lastDataBufferLength());
                            }
                                ...
                                if (0 < PREPEND_BUFFER_SIZE) {
                                    ...
                                    ASSERT(EXP_NUM_DATA_BUFFERS ==
                                           X.numDataBuffers());
                                }
                                ASSERT(PREPEND_BUFFER_SIZE ==
                                       X.lastDataBufferLength());
                            }
                            ASSERT(EXP_NUM_BUFFERS + 1 == X.numBuffers());

                            // Repeat invariants after testing setLength in the
                            // presence of zero-sized buffers.
                ...
                for (int dataLength = 0; dataLength <= bufferSize * numBuffers;
                    ...
                    for (int appendSz = 1; appendSz <= 5; ++appendSz) {
                        ...
                        BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator)
                        {
                            ...
                            SimpleBlobBufferFactory ifa(APPEND_BUFFER_SIZE,
                                                        &ta);
                            bdlbb::BlobBuffer       buffer;
                            ifa.allocate(&buffer);
                            ...
                            ASSERT(DATA_LENGTH + APPEND_BUFFER_SIZE ==
                                   X.length());
                            ASSERT(EXP_NUM_BUFFERS + 1 == X.numBuffers());
                            if (0 < APPEND_BUFFER_SIZE) {
                                ASSERT(EXP_NUM_DATA_BUFFERS + 1 ==
                                       X.numDataBuffers());
                                ASSERT(APPEND_BUFFER_SIZE ==
                                       X.lastDataBufferLength());
                            }
                            else {
                                ASSERT(EXP_NUM_DATA_BUFFERS ==
      ...
      case 7: {
        ...
        if (verbose)
            cout << "ntTesting 'removeBuffer'" << endl;

        for (int bufferSize = 1; bufferSize <= 5; ++bufferSize)
            for (int numBuffers = 0; numBuffers <= 5; ++numBuffers)
                for (int dataLength = 0; dataLength <= bufferSize * numBuffers;
                     ++dataLength)
                    for (int removePos = 0; removePos < numBuffers;
                         ++removePos) {
                        bslma::TestAllocator defaultAlloc(veryVeryVerbose);
                        bslma::DefaultAllocatorGuard guard(&defaultAlloc);
                        bslma::TestAllocator         ta(veryVeryVerbose);
                        ...
                        BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator)
                        {
                            ...
                            if (REMOVE_POSITION == EXP_NUM_DATA_BUFFERS - 1) {
                                // Removing the last data buffer (variable
                                // number of data bytes, equal to
                                // EXP_LAST_DB_LENGTH).
                                ...
                                ASSERT(DATA_LENGTH - EXP_LAST_DB_LENGTH ==
                                       X.length());
                                ASSERT(EXP_NUM_DATA_BUFFERS - 1 ==
                                       X.numDataBuffers());
                            }
        ...
        if (verbose)
            cout << "ntTesting 'removeBuffers'" << endl;
                    ...
                    for (int removePos = 0; removePos < numBuffers;
                        ...
                        for (int numRemoveBuffers = 0;
                            ...
                            for (int trimBufferIdx = -1;
                                 trimBufferIdx <
                                 (dataLength + bufferSize - 1) / bufferSize -
                                     1;
                                 ++trimBufferIdx) {
                                                                ...
                                                                testAllocator )
                                {
                                    ...
                                    const int TRIM_DELTA =
                                        TRIM_BUFFER_IDX == -1
                                            ? 0
                                            : BUFFER_SIZE - 1;
                                    const int DATA_LENGTH = dataLength;
                                    const int TRIMMED_DATA_LENGTH =
                                        dataLength - TRIM_DELTA;
                                    const int NUM_BUFFERS     = numBuffers;
                                    const int REMOVE_POSITION = removePos;
                                    const int NUM_REMOVE_BUFFERS =
                                        numRemoveBuffers;
                                    const int NUM_DATA_BUFFERS =
                                        (dataLength + BUFFER_SIZE - 1) /
                                        BUFFER_SIZE;
                                    const int EXP_NUM_BUFFERS =
                                        NUM_BUFFERS - NUM_REMOVE_BUFFERS;
                                    ...
                                    typedef bdlbb::Blob     Obj;
                                    SimpleBlobBufferFactory fa(BUFFER_SIZE,
                                                               &ta);
                                    ...
                                    ASSERT(NUM_BUFFERS == X.numBuffers());
                                    ASSERT(NUM_DATA_BUFFERS ==
                                           X.numDataBuffers());
                                    ASSERT(EXP_LAST_DB_LENGTH ==
                                           X.lastDataBufferLength());
                                    checkNoAliasedBlobBuffers(X);

                                    const int LAST_DATA_BUFFER_IDX =
                                        X.numDataBuffers() - 1;

                                    for (int idx = REMOVE_POSITION;
                                         idx <
                                         REMOVE_POSITION + NUM_REMOVE_BUFFERS;
                                         ++idx) {
                                    ...
                                    mX.removeBuffers(REMOVE_POSITION,
                                                     NUM_REMOVE_BUFFERS);

                                    ASSERT(EXP_NUM_BUFFERS == X.numBuffers());
                                    ASSERT(EXP_DATA_BUFFERS ==
                                           X.numDataBuffers());
        ...
        if (verbose)
            cout << "ntTesting 'removeUnusedBuffers'" << endl;
                ...
                for (int dataLength = 0; dataLength <= bufferSize * numBuffers;
                    ...
                    BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator)
                    {
                        ...
                        const int EXP_LAST_DB_LENGTH =
                            EXP_NUM_DATA_BUFFERS > 0
                                ? DATA_LENGTH -
                                      (EXP_NUM_DATA_BUFFERS - 1) * BUFFER_SIZE
                                : 0;
                        if (veryVerbose) {
                            T_;
                            P_(BUFFER_SIZE);
      ...
      case 5: {
        ...
        for (int bufferSize = 1; bufferSize <= 5; ++bufferSize)
            for (int numBuffers = 0; numBuffers <= 5; ++numBuffers)
                for (int dataLength = 0; dataLength <= bufferSize * numBuffers;
                     ++dataLength)
                    for (int insertPos = 0; insertPos <= numBuffers;
                         ++insertPos)
                        for (int insertSz = 0; insertSz <= 5; ++insertSz) {
                                                                ...
                                                                testAllocator )
                            {
                                ...
                                SimpleBlobBufferFactory ifa(INSERT_BUFFER_SIZE,
                                                            &ta);
                                bdlbb::BlobBuffer       buffer;
                                ifa.allocate(&buffer);
                                ASSERT(INSERT_BUFFER_SIZE == buffer.size());

                                mX.insertBuffer(INSERT_POSITION,
                                                buffer);  // TEST HERE

                                const int INSERT_FLAG =
                                    INSERT_POSITION < EXP_NUM_DATA_BUFFERS;
                                ASSERT(DATA_LENGTH +
                                           INSERT_FLAG * INSERT_BUFFER_SIZE ==
                                       X.length());
                                ASSERT(EXP_LAST_DB_LENGTH ==
                                       X.lastDataBufferLength());
                                ASSERT(EXP_NUM_BUFFERS + 1 == X.numBuffers());
                                ...
                                // Repeat invariants after testing setLength in
                                // the presence of zero-sized buffers.
                ...
                for (int dataLength = 0; dataLength <= bufferSize * numBuffers;
                    ...
                    bslma::DefaultAllocatorGuard guard(&ta);
                    {
                        ...
                        LOOP3_ASSERT(bufferSize,
                                     numBuffers,
                                     dataLength,
                                     EXP_NUM_DATA_BUFFERS ==
                                         X.numDataBuffers());  // TEST HERE
                        LOOP3_ASSERT(
                            bufferSize,
                            numBuffers,
                            dataLength,
                            EXP_LAST_DB_LENGTH ==
                                X.lastDataBufferLength());  // TEST HERE
                        checkNoAliasedBlobBuffers(X);
                        if (veryVerbose) {
                            P(X.numDataBuffers());
      ...
      case 3: {
        ...
        for (int bufferSize = 1; bufferSize <= 5; ++bufferSize)
            for (int dataLength = 0; dataLength <= 5 * bufferSize;
                 ++dataLength) {
                bslma::TestAllocator         defaultAlloc(veryVeryVerbose);
                bslma::DefaultAllocatorGuard guard(&defaultAlloc);
                bslma::TestAllocator         ta(veryVeryVerbose);
                ...
                BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator)
                {
                    ...
                    LOOP2_ASSERT(bufferSize,
                                 dataLength,
                                 EXP_TOTAL_SIZE ==
                                     X.totalSize());  // TEST HERE
                    LOOP2_ASSERT(bufferSize,
                                 dataLength,
                                 EXP_NUM_BUFFERS ==
                                     X.numBuffers());  // TEST HERE
                    checkNoAliasedBlobBuffers(X);
                    if (veryVerbose) {
                        P_(X.length());
                        P_(X.numBuffers());
                    ...
                    for (int blobLength = 0; blobLength < EXP_LENGTH;
                         ++blobLength) {
                    ...
                    for (int blobLength = EXP_LENGTH + 1;
                         blobLength < 2 * EXP_LENGTH + 2;
                         ++blobLength) {
                        const int EXP_NUM_BUFFERS =
                            (blobLength + BUFFER_SIZE - 1) / BUFFER_SIZE;
                        const int EXP_TOTAL_SIZE =
                            EXP_NUM_BUFFERS * BUFFER_SIZE;
      ...
      case 2: {
        ...
        BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator)
        {
            ...
            bsl::shared_ptr<char> shptr2(
                (char *)ta.allocate(BUFFER_SIZE + 1), &ta, &ta);
            bdlbb::BlobBuffer b2(shptr2, BUFFER_SIZE + 1);
            ...
            Obj        mX(BUFFERS, NUM_BUFFERS, &fa, &ta);
            const Obj& X = mX;
            ASSERT(0 == X.length());
            ASSERT(TOTAL_SIZE == X.totalSize());
            ASSERT(NUM_BUFFERS == X.numBuffers());
      ...
      case 1: {
        ...
        //
        // Plan: Do a mini value-semantic 10 case driver for
        // 'bdlbb::BlobBuffer'.  Manipulate 'bdlbb::Blob' using a non-fixed
        // size blob buffer factory with 'setLength', 'insertBuffer',
        // 'removeBuffer', and 'appendBuffer'.  For each blob, assert the state
        // is as expected and check the validity of the buffers.
        //
        // Testing:
