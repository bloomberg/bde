...
static
void replenishImp(bdlmtt::AtomicPointer<LLink>       *nextList,
    ...
    // blockSize' and '1 <= numBlocks'.
{
    ...
    for (char *p = start; p < end; p += blockSize) {
        ...
        bdlmtt::AtomicUtil::initInt(&nextLink->d_refCount, 0 );
        nextLink->d_next_p = toLink(p + blockSize);
    }
    bdlmtt::AtomicUtil::initInt(&toLink(end)->d_refCount, 0 );
...
void Pool::replenish()
{
    replenishImp(reinterpret_cast<bdlmtt::AtomicPointer<LLink> *>(&d_freeList),
                 &d_blockList,
                 d_internalBlockSize,
                 d_chunkSize);
...
{
    BSLS_ASSERT(1 <= blockSize);
    BSLS_ASSERT(0 != maxBlocksPerChunk);
...
void *Pool::allocate()
{
    ...
    for (;;) {
        ...
        if (BSLS_PERFORMANCEHINT_PREDICT_UNLIKELY
                         (2 != bdlmtt::AtomicUtil::addIntNv(&p->d_refCount, 2))) {
            BSLS_PERFORMANCEHINT_UNLIKELY_HINT;
            for (int i = 0; i < 3; ++i) {
        ...
        //   - The following 'relaxedLoad()' will return the new 'freelist'
        ...
        //   - relaxedLoad() will return the "old" (!= p) and the CAS and thus
        ...
        const Link * volatile h = d_freeList.relaxedLoad();
        ...
        BSLS_PERFORMANCEHINT_UNLIKELY_HINT;
        for (;;) {
            int refCount = bdlmtt::AtomicUtil::getInt(p->d_refCount);

            if (refCount & 1) {
                if (refCount ==
                        bdlmtt::AtomicUtil::testAndSwapInt(
                            &p->d_refCount,
                            refCount,
                            refCount^1)) {
...
void Pool::deallocate(void *address)
{
    ...
    int refCount = bdlmtt::AtomicUtil::getIntRelaxed(p->d_refCount);
    for (;;) {
        if (BSLS_PERFORMANCEHINT_PREDICT_LIKELY(2 == refCount)) {
            refCount = bdlmtt::AtomicUtil::testAndSwapInt(&p->d_refCount, 2, 0);
            if (BSLS_PERFORMANCEHINT_PREDICT_LIKELY(2 == refCount)) {
                break;
            }
        }
        ...
        refCount = bdlmtt::AtomicUtil::testAndSwapInt(&p->d_refCount,
                                                   refCount,
                                                   refCount - 1);
    ...
    Link *old = d_freeList.relaxedLoad();
    for (;;) {
        p->d_next_p = old;
...
void Pool::reserveCapacity(int numBlocks)
{
    ...
    if (numBlocks > 0) {
        replenishImp(
                   reinterpret_cast<bdlmtt::AtomicPointer<LLink> *>(&d_freeList),
                   &d_blockList,
                   d_internalBlockSize,
                   numBlocks);
    }
