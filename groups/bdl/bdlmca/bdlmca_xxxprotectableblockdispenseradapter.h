// bdlmca_xxxprotectableblockdispenseradapter.h      -*-C++-*-
#ifndef INCLUDED_BDLMCA_XXXPROTECTABLEBLOCKDISPENSERADAPTER
#define INCLUDED_BDLMCA_XXXPROTECTABLEBLOCKDISPENSERADAPTER
...
//@PURPOSE: Provide thread-safe adapter for 'bdlma::ProtectableBlockDispenser'.
//
//@DEPRECATED: Do not use.
//
//@CLASSES:
//  bdlmca::ProtectableBlockDispenserAdapter: thread-enabled protocol adapter
//
//@SEE_ALSO: bdlma_xxxprotectableblockdispenser, bdlmca_xxxprotectablemultipool
//
//@AUTHOR: Henry Verschell (hverschell)
//
//@DESCRIPTION: [!DEPRECATED!] This component defines an adapter for the
// 'bdlma::ProtectableBlockDispenser' protocol that provides synchronization
// for operations on a dispenser supplied at construction using a mutex also
// supplied at construction.
//
///Thread Safety
///-------------
// 'bdlmca::ProtectableBlockDispenserAdapter' is *thread-enabled*, meaning any
...
// 'bdlmca::ProtectableBlockDispenserAdapter' to synchronize memory
...
//                             int                              objectSize,
//                             bdlma::ProtectableBlockDispenser *dispenser = 0);
...
//          // 'bdlma::NativeProtectableBlockDispenser::singleton()' is
//          // used.
//
//      ~ThreadEnabledProtectablePool();
...
//                                 int                              objectSize,
//                                 bdlma::ProtectableBlockDispenser *dispenser)
//
...
// 'bdlmca::ProtectableBlockDispenserAdapter' to prevent our (thread-enabled)
// pools from attempting simultaneous memory allocations from our, not
// necessarily thread-safe, 'bdlma::ProtectableBlockDispenser'.  Note that the
...
//      // optionally supplied both a 'bdlma::ProtectableBlockDispenser' and a
...
class ProtectableBlockDispenserAdapter :
                                       public bdlma::ProtectableBlockDispenser {
    // This class defines an implementation of the
    // 'bdlma::ProtectableBlockDispenser' protocol that "decorates" (wraps) a
    // concrete 'bdlma::ProtectableBlockDispenser' to ensure thread-safe access
    // to the decorated dispenser.  Note that this class is local because it
    // is used by the 'ProtectableMultipool' to ensure that allocations
    // on an underlying (non-thread-enabled) 'bdlma::ProtectableBlockDispenser'
    ...
    bdlma::ProtectableBlockDispenser *d_dispenser_p;  // dispenser (held, not
                                                     // owned)

    // NOT IMPLEMENTED
    ProtectableBlockDispenserAdapter(
                                const ProtectableBlockDispenserAdapter&);
    ProtectableBlockDispenserAdapter& operator=(
                                const ProtectableBlockDispenserAdapter&);
  ...
  public:
    ...
    ProtectableBlockDispenserAdapter(
        ...
        // the 'bdlma::NativeProtectableBlockDispenser::singleton' is used.

    virtual ~ProtectableBlockDispenserAdapter();
        // Destroy this thread-enabled dispenser adapter.

    // MANIPULATORS
    virtual bdlma::MemoryBlockDescriptor allocate(size_type numBytes);
    ...
    virtual void deallocate(const bdlma::MemoryBlockDescriptor& block);
    ...
    virtual int protect(const bdlma::MemoryBlockDescriptor& block);
        ...
        // concrete implementations of the 'bdlma::ProtectableBlockDispenser'
    ...
    virtual int unprotect(const bdlma::MemoryBlockDescriptor& block);
        ...
        // 'bdlma::ProtectableBlockDispenser' protocol may differ in the status
...
: d_mutex_p(mutex)
, d_dispenser_p(
      0 == blockDispenser ? &bdlma::NativeProtectableBlockDispenser::singleton()
                          : blockDispenser)
{
}
