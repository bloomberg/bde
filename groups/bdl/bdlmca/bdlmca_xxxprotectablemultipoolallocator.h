// bdlmca_xxxprotectablemultipoolallocator.h                              -*-C++-*-
#ifndef INCLUDED_BDLMCA_XXXPROTECTABLEMULTIPOOLALLOCATOR
#define INCLUDED_BDLMCA_XXXPROTECTABLEMULTIPOOLALLOCATOR
...
//
//@CLASSES:
//  bdlmca::ProtectableMultipoolAllocator: allocator from protectable pools
//
//@SEE_ALSO: bdlmca_xxxprotectablemultipool
//
//@AUTHOR: Henry Verschell (hverschell)
//
//@DESCRIPTION: This component provides an allocator,
// 'bdlmca::ProtectableMultipoolAllocator', that implements the
// 'bdlma::ManagedAllocator' protocol and provides a partially thread-enabled
...
// change the access protection of that managed memory.  A multipool allocation
// (or deallocation) request allocates memory from (or returns memory to) the
// appropriate internal pool, or else to a separately managed list of memory
// blocks.  The 'protect' method sets the access protection for all the memory
// managed by this allocator to a READ-ONLY state.  Writing to memory in the
// protected state will cause a segmentation violation.  The 'unprotect' method
// sets the access protection for all the memory managed by this allocator to a
...
// unless the allocator is in an unprotected state:
...
///Thread Safety
///-------------
// 'bdlmca::ProtectableMultipoolAllocator' is *partially* *thread-enabled*,
...
// 'bdlmca::ProtectableMultipoolAllocator'.  The example structures are contrived
...
//      bdlmca::ProtectableMultipoolAllocator pmpa(NUM_POOLS);
//..
...
class ProtectableMultipoolAllocator : public bdlma::ManagedAllocator {
    ...
    // undefined for 'allocate' and 'deallocate' unless this allocator is in an
    // unprotected state.  Requests for "large" memory chunks (larger than can
    // be accommodated by the largest individual pool) are handled directly by
    // the underlying dispenser.

    //
    // 'ProtectableMultipoolAllocator' is *partially* *thread-safe*,
    ...
    // DATA
    ProtectableMultipool d_multipool;  // held protectable multipool

    // NOT IMPLEMENTED
    ProtectableMultipoolAllocator(
                                   const ProtectableMultipoolAllocator&);
    ProtectableMultipoolAllocator& operator=(
                                   const ProtectableMultipoolAllocator&);
  ...
  public:
    ...
    virtual ~ProtectableMultipoolAllocator();
...
// CREATORS
inline
ProtectableMultipoolAllocator::ProtectableMultipoolAllocator(
                              int                              numMemoryPools,
...
// MANIPULATORS
inline
void ProtectableMultipoolAllocator::protect()
{
    d_multipool.protect();
}

inline
void ProtectableMultipoolAllocator::unprotect()
{
...
// ACCESSORS
inline
bool ProtectableMultipoolAllocator::isProtected() const
{
    return d_multipool.isProtected();
}

inline
int ProtectableMultipoolAllocator::numPools() const
{
    return d_multipool.numPools();
}

inline
int ProtectableMultipoolAllocator::maxPooledBlockSize() const
{
