// bdlmca_xxxpooledbufferchain.cpp   -*-C++-*-
...
// MANIPULATORS
void PooledBufferChain::append(const char *buffer, int numBytes)
{
...
char *PooledBufferChain::buffer(int index)
{
    char **base = d_first_p;
...
void PooledBufferChain::removeAll()
{
    while (d_first_p) {
...
void PooledBufferChain::setLength(int newLength)
{
    int bufSize       = bufferSize();
...
#if 0
    // PROPOSED ADDITION
    void import(bdlmca::PooledBufferChain *source,
                int                      numBytes);

namespace bdlmca {        // Append to this buffer chain the buffers containing the specified
...
void PooledBufferChain::import(PooledBufferChain *source,
                                     int                      numBytes)
{
    int bufSize = bufferSize();
    BSLS_ASSERT(source);
...
namespace bdlmca {
void PooledBufferChain::replace(int         offset,
                                      const char *arena,
                                      int         numBytes)
{
    BSLS_ASSERT(arena);
...
void PooledBufferChain::copyOut(char *arena,
                                      int   numBytes,
                                      int   offset) const
{
    BSLS_ASSERT(arena);
...
// ACCESSORS
const char *PooledBufferChain::buffer(int index) const
{
    char **base = d_first_p;
    for (int i = 0; i < index; ++i) {
        base = (char **)(void *)*base;
...
int PooledBufferChain::loadBuffers(const char **buffers,
                                         int          numBuffers,
                                         int          index) const
{
    int i;
...
void PooledBufferChain::loadBuffers(
...
void PooledBufferChainFactory::deleteObject(
                                               PooledBufferChain *object)
{
