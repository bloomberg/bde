...
//
//@CLASSES:
//   bdlcxxx::BitArray: vector-like, sequential container of boolean values
//
...
// or '1') of type 'bool'.  A 'bdlcxxx::BitArray' may be thought of as an
// arbitrary-precision 'unsigned int'.  This metaphor is used to motivate the
// rich set of "bitwise" operations on 'bdlcxxx::BitArray' objects provided by this
...
// supports "boolean" functionality unique to 'bdlcxxx::BitArray' (note that 'LSB'
...
//..
//  bdlcxxx::BitArray mA(128);
//  assert(0 == mA[13]);         // Ok
//  mA[13] = 'false';            // Error -- 'mA[13]' is not an lvalue.
//  mA.replace(13, 1);           // Ok
//
//  const bdlcxxx::BitArray& A = a;  // Ok
//  assert(1 == A[13]);          // Ok
...
// literal '5' (of type 'int') -- is used in a 'bdlcxxx::BitArray' method to
...
// method of the 'bdlcxxx::BitArray' is invoked:
//..
//  bdlcxxx::BitArray a(10);
//  assert(0 == a[5]);
...
//..
// Note that *all* of the non-creator methods of 'bdlcxxx::BitArray' provide the
...
// has myriad applications.  For example, a 'bdlcxxx::BitArray' can be used
...
//      bdlcxxx::BitArray      d_nullFlags;    // 'true' indicates i'th element is
//                                         // null
...
class BitArray {
  ...
  public:
    // TRAITS
    BSLALG_DECLARE_NESTED_TRAITS(BitArray,
    ...
    struct InitialCapacity {
        ...
        // example:
        //..
        //  BitArray x(BitArray::InitialCapacity(8));
        //..
        // defines a 'BitArray' object 'x' with an initial capacity of 8
    ...
    // CREATORS
    explicit BitArray(bslma::Allocator *basicAllocator = 0);
    explicit BitArray(int               initialLength,
                           bslma::Allocator *basicAllocator = 0);
    BitArray(int              initialLength,
                  bool             value,
                  bslma::Allocator *basicAllocator = 0);
    explicit BitArray(const InitialCapacity&  numBits,
                           bslma::Allocator       *basicAllocator = 0);
    BitArray(const InitialCapacity&  numBits,
                  int                     initialLength,
                  bslma::Allocator       *basicAllocator = 0);
    BitArray(const InitialCapacity&  numBits,
                  int                     initialLength,
    ...
    void append(const BitArray& srcArray, int srcIndex, int numBits);
    ...
    void insert(int dstIndex, const BitArray& srcArray);
...
BitArray operator<<(const BitArray& array, int numBits);
...
// FREE FUNCTIONS
void swap(bdlcxxx::BitArray& a, bdlcxxx::BitArray& b);

namespace bdlcxxx {    // Swap the values of the specified 'a' and 'b' objects.  This method
...
// PRIVATE CLASS METHODS
inline
int BitArray::arraySize(int numBits)
{
...
// MANIPULATORS
inline
void BitArray::set0(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_length);

    bdlb::BitstringUtil::set(&d_array.front(), index, false);
}

inline
void BitArray::set1(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_length);

    bdlb::BitstringUtil::set(&d_array.front(), index, true);
}

inline
void BitArray::andEqual(int index, bool value)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_length);
...
inline
void BitArray::orEqual(int index, bool value)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_length);
...
inline
void BitArray::xorEqual(int index, bool value)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_length);
...
inline
void BitArray::minusEqual(int index, bool value)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_length);
...
void BitArray::set(int index, bool value)
{
    ...
    bdlb::BitstringUtil::set(&d_array.front(), index, value);
}
...
inline
void BitArray::append(bool value, int numBits)
{
    BSLS_ASSERT_SAFE(0 <= numBits);
...
inline
void BitArray::replace(int index, bool value)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_length);
...
inline
void BitArray::reserveCapacity(int numBits)
{
    BSLS_ASSERT_SAFE(0 <= numBits);
...
inline
void BitArray::reserveCapacityRaw(int numBits)
{
    BSLS_ASSERT_SAFE(0 <= numBits);
          ...
          case 1: {
            ...
            if (d_length % BDEA_BITS_PER_INT
             && d_array[d_array.size() - 1] >>
                ...
                const int pos = d_length % BitArray::BDEA_BITS_PER_INT;
                if (pos) {
                    bdlb::BitUtil::replaceZero(
                                       &d_array[d_array.size() - 1],
                                       pos,
                                       BitArray::BDEA_BITS_PER_INT - pos);
                }
                stream.invalidate();
                return stream;                                        // RETURN
            }
...
bool BitArray::operator[](int index) const
{
    ...
    return bdlb::BitstringUtil::get(&d_array.front(), index);
}

inline
int BitArray::length() const
{
    return d_length;
}
...
namespace bdlcxxx {
inline
int BitArray::findBit0AtLargestIndex() const
{
    return find0AtLargestIndex();
}

inline
int BitArray::findBit0AtLargestIndexGE(int index) const
{
    return find0AtLargestIndexGE(index);
}

inline
int BitArray::findBit0AtLargestIndexGT(int index) const
{
    return find0AtLargestIndexGT(index);
}

inline
int BitArray::findBit0AtLargestIndexLE(int index) const
{
    return find0AtLargestIndexLE(index);
}

inline
int BitArray::findBit0AtLargestIndexLT(int index) const
{
    return find0AtLargestIndexLT(index);
}

inline
int BitArray::findBit0AtSmallestIndex() const
{
    return find0AtSmallestIndex();
}

inline
int BitArray::findBit0AtSmallestIndexGE(int index) const
{
    return find0AtSmallestIndexGE(index);
}

inline
int BitArray::findBit0AtSmallestIndexGT(int index) const
{
    return find0AtSmallestIndexGT(index);
}

inline
int BitArray::findBit0AtSmallestIndexLE(int index) const
{
    return find0AtSmallestIndexLE(index);
}

inline
int BitArray::findBit0AtSmallestIndexLT(int index) const
{
    return find0AtSmallestIndexLT(index);
}

inline
int BitArray::findBit1AtLargestIndex() const
{
    return find1AtLargestIndex();
}

inline
int BitArray::findBit1AtLargestIndexGE(int index) const
{
    return find1AtLargestIndexGE(index);
}

inline
int BitArray::findBit1AtLargestIndexGT(int index) const
{
    return find1AtLargestIndexGT(index);
}

inline
int BitArray::findBit1AtLargestIndexLE(int index) const
{
    return find1AtLargestIndexLE(index);
}

inline
int BitArray::findBit1AtLargestIndexLT(int index) const
{
    return find1AtLargestIndexLT(index);
}

inline
int BitArray::findBit1AtSmallestIndex() const
{
    return find1AtSmallestIndex();
}

inline
int BitArray::findBit1AtSmallestIndexGE(int index) const
{
    return find1AtSmallestIndexGE(index);
}

inline
int BitArray::findBit1AtSmallestIndexGT(int index) const
{
    return find1AtSmallestIndexGT(index);
}

inline
int BitArray::findBit1AtSmallestIndexLE(int index) const
{
    return find1AtSmallestIndexLE(index);
}

inline
int BitArray::findBit1AtSmallestIndexLT(int index) const
{
    return find1AtSmallestIndexLT(index);
}

inline
int BitArray::maxSupportedVersion()
{
...
inline
bsl::ostream& bdlcxxx::operator<<(bsl::ostream& stream, const BitArray& rhs)
{
