// bdlcfxxx_vfunc2.h                                                  -*-C++-*-
#ifndef INCLUDED_BDLCFXXX_VFUNC2
#define INCLUDED_BDLCFXXX_VFUNC2
...
//
//@CLASSES:
//   bdlcfxxx::Vfunc2: signature-specific function object (functor)
//
...
// returns void and takes two arguments of arbitrary type; it is invoked in a
// manner similar to that of following free function:
//..
//   void functionObject(arg1, arg2);
//..
...
// construction.
//
// The 'bdlcfxxx::Vfunc2' functor (envelope) holds a polymorphic, reference-counted,
...
// signature and (2) the supplied user data.  See the 'bdlcfxxxu_vfunc2' component
// for more information on populating 'bdlcfxxx::Vfunc2' function objects with
...
// button is pressed.  Futhermore, the implementor agrees to provide to the
// callback function two arguments at its invokation: a modifiable object of
...
//..
//   class MyGuiButton  {
//       bdlcfxxx::Vfunc2<MyGuiContext *, MyGuiLocation> d_callback;
//           // Functor to execute when button is pressed.
//
//     public:
//       MyGuiButton(const bdlcfxxx::Vfunc2<MyGuiContext *,
...
//   };
//
//   MyGuiButton::MyGuiButton(const bdlcfxxx::Vfunc2<MyGuiContext *,
//                            MyGuiLocation>& buttonPressCallback)
...
// 'bdlcfxxx::Vfunc2' class specifies the number of required arguments).  However,
...
//   }
//..
// The 'bdlcfxxx::Vfunc2<MyGuiContext, MyGuiLocation>' class used in 'MyGuiButton'
// class has a private member of an abstract class 'bdlcfxxxr_vfunc2', whose
// 'execute' method is called when the functor is being called.  The 'execute'
// method of the 'bdlcfxxxr_vfunc2' class, the functor invocation operator of the
// 'bdlcfxxx::Vfunc2' class, and the 'pressButton' method of 'MyGuiButton' class
...
// signature the user implements a concrete class derived from 'bdlcfxxxr_vfunc2'
// as follows:
//..
//   template <class F, class A1, class A2, class D1>
//   class FuncRep : public bdlcfxxxr::Vfunc2<A1, A2> {
...
// Conventionally, a function signature consists of output parameters, and then
// input parameters.  Since here we have two parameter lists (one passed in by
// the caller, and one passed in by the callee) we can follow this convention
// within each list only.  We chose to pass in first the required arguments
// (passed by the caller) and then the optional arguments (supplied by the
// callee).  This order allows a callee (more easily) add parameters at the end
// of the function parameter list.  The number of required parameters is a part
// of the 'MyGuiButton' interface, and hence will never change.
//
...
//   int globalCounter = 0;
//
//   bdlcfxxxr::Vfunc2<MyGuiContext *, MyGuiLocation>  *rep = new(myAllocator)
...
//
//   bdlcfxxx::Vfunc2<MyGuiContext *, MyGuiLocation> callbackFunctor(rep);
//
...
class Vfunc2 {
    ...
    //..
    //     (void) functionObject(arg1, arg2)    // functor calling signature
    //..
    // which takes two arguments and returns void.  Note that pointer (but not
    // reference) types are supported.

    bdlcfxxxr::Vfunc2<A1, A2> *d_rep_p;  // polymorphic functor representation

    // NOT IMPLEMENTED
    bool operator==(const Vfunc2&) const;
    bool operator!=(const Vfunc2&) const;

    template<class B1, class B2>
    bool operator==(const Vfunc2<B1, B2>&) const;
    template<class B1, class B2>
    bool operator!=(const Vfunc2<B1, B2>&) const;

  public:
    // TRAITS
    BSLALG_DECLARE_NESTED_TRAITS(Vfunc2,
    ...
    Vfunc2(bdlcfxxxr::Vfunc2<A1, A2> *representation);
        ...
        // components (see 'bdlcfxxxu_vfunc2') to create an appropriate
        // representation and load it into an existing functor.

    Vfunc2(const Vfunc2<A1, A2>& functionObject);
    ...
    ~Vfunc2();
        ...
        // the static 'deleteObject' method of 'bdlcfxxxr::Vfunc2' base class).

    // MANIPULATORS
    Vfunc2<A1, A2>& operator=(const Vfunc2<A1, A2>& functionObject);
        ...
        // 'deleteObject' method of 'bdlcfxxxr::Vfunc2' base class.
...
template <class A1, class A2>
inline Vfunc2<A1, A2>::Vfunc2(bdlcfxxxr::Vfunc2<A1, A2> *rep)
: d_rep_p(rep)
{
    if (d_rep_p) {
        d_rep_p->increment();
...
template <class A1, class A2>
inline Vfunc2<A1, A2>::Vfunc2(const Vfunc2& functor)
...
Vfunc2<A1, A2>&
Vfunc2<A1, A2>::operator=(const Vfunc2<A1, A2>& rhs)
{
...
template <class A1, class A2>
inline void Vfunc2<A1, A2>::operator()(const A1& argument1,
...
template <class A1, class A2>
inline bdlcfxxx::Vfunc2<A1, A2>::operator const void *() const
{
    return d_rep_p;
}
