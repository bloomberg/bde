...
//                              OVERVIEW
//                              --------
// This program tests the functionality of the 'bdlmtt::WriteLockGuard' class.
...
//
//-----------------------------------------------------------------------------
// bdlmtt::WriteLockGuard
// ===================
// [2] bdlmtt::WriteLockGuard();
// [2] ~bdlmtt::WriteLockGuard();
// [2] bdlmtt::WriteLockGuard::release();
// [2] bdlmtt::WriteLockGuard::ptr();
//
// bdlmtt::WriteLockGuardUnlock
// =====================
// [3] bdlmtt::WriteLockGuardUnlock();
// [3] ~bdlmtt::WriteLockGuardUnlock();
// [3] bdlmtt::WriteLockGuardUnlock::release();
// [3] bdlmtt::WriteLockGuardUnlock::ptr();
//
// bdlmtt::WriteLockGuardTryLock
// =====================
// [4] bdlmtt::WriteLockGuardTryLock();
// [4] ~bdlmtt::WriteLockGuardTryLock();
// [4] bdlmtt::WriteLockGuardTryLock::release();
// [4] bdlmtt::WriteLockGuardTryLock::ptr();
//-----------------------------------------------------------------------------
// [1] Ensure helper class 'my_RWLock' works as expected
// [5] INTERACTION BETW 'bdlmtt::WriteLockGuard' AND 'bdlmtt::WriteLockGuardUnlock'
// [6] DEPRECATED 'bdlmtt::LockWriteGuard'
// [7] USAGE EXAMPLES
//=============================================================================
...
struct my_RWLock {
    ...
    // conforms to the interface required by the 'bdlmtt::WriteLockGuard' class.
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:
      ...
      case 6: {
        ...
        //
        // Concern:
        //   Backwards compatibility: ensure that the 'bdlmtt::LockWriteGuard'
...

        {
            ...
            {
                bdlmtt::LockWriteGuard<my_RWLock> l1(&mX);
                if(verbose)  P(X.lockCount());
                ASSERT(1 == X.lockCount());
                {
                    const int PRE_LOCKED = 0;
                    bdlmtt::LockWriteGuard<my_RWLock> l2(&mX, PRE_LOCKED);
                    if(verbose)  P(X.lockCount());
                    ASSERT(2 == X.lockCount());
                    {
                    const int PRE_LOCKED = 1; mX.lockWrite();
                        bdlmtt::LockWriteGuard<my_RWLock> l3(&mX, PRE_LOCKED);
                        if(verbose)  P(X.lockCount());
      ...
      case 5: {
        // --------------------------------------------------------------------
        // INTERACTION BETWEEN 'bdlmtt::WriteLockGuard' AND
        // 'bdlmtt::WriteLockGuardUnlock'
        //
        // Concern:
        //   That 'bdlmtt::WriteLockGuard' and 'bdlmtt::WriteLockGuardUnlock'
        ...
        //   distinct 'bdlmtt::WriteLockGuard' *and* 'bdlmtt::WriteLockGuardUnlock'
        ...
        if (verbose)
            cout << "\tTesting interaction between "
                    "'bdlmtt::WriteLockGuard' and bdlmtt::WriteLockGuardUnlock'"
                 << endl
                 << "=============================="
                    "========================================"
                 << endl;

        {
            ...
            mX1.lockWrite();
            {
                bdlmtt::WriteLockGuard<my_RWLock> l1(&mX1);
                bdlmtt::WriteLockGuard<my_RWLock> l2(&mX2);
                ASSERT(2 == X1.lockCount());
                ASSERT(1 == X2.lockCount());

                {
                    bdlmtt::WriteLockGuardUnlock<my_RWLock> u1(&mX1);
                    bdlmtt::WriteLockGuardUnlock<my_RWLock> u2(&mX2);
      ...
      case 4: {
        // --------------------------------------------------------------------
        // TESTING CLASS 'bdlmtt::WriteLockGuardTryLock'
        //
        // Concern:
        //   That the basic functionality of the 'bdlmtt::WriteLockGuardTryLock'
        //   class template is correct.
        //
        // Plan:
        //   We begin by creating a series of nested
        //   'bdlmtt::WriteLockGuardTryLock' objects using a common 'my_RWLock'
        ...
        //   constructing a new 'bdlmtt::WriteLockGuard' and calling 'release'.
        ...
        //   object and that when the 'bdlmtt::WriteLockGuardTryLock' object is
        //   destroyed, it does not unlock the object.
        //
        //   Finally we test that a 'bdlmtt::WriteLockGuardTryLock' can be
        ...
        //   bdlmtt::WriteLockGuardTryLock();
        //   ~bdlmtt::WriteLockGuardTryLock();
        //   bdlmtt::WriteLockGuardTryLock::release();
        //   bdlmtt::WriteLockGuardTryLock::ptr();
        // --------------------------------------------------------------------
...

        {
            ...
            {
                bdlmtt::WriteLockGuardTryLock<my_RWLock> l1(&mX);
                if(verbose)  P(X.lockCount());
                ASSERT(0 == X.lockCount());
                ASSERT(0 == l1.ptr());
                {
                    bdlmtt::WriteLockGuardTryLock<my_RWLock> l2(&mX);
                    if(verbose)  P(X.lockCount());
                    ASSERT(1 == X.lockCount());
                    {
                        bdlmtt::WriteLockGuardTryLock<my_RWLock> l3(&mX, 10);
                        if(verbose)  P(X.lockCount());
                        ASSERT(2 == X.lockCount());
                    }
                    if(verbose)  P(X.lockCount());
            ...
            {
                bdlmtt::WriteLockGuardTryLock<my_RWLock> l1(&mX, 2);
        ...
        {
            bdlmtt::WriteLockGuardTryLock<my_RWLock> mX(0, 6);
        }
        {
            bdlmtt::WriteLockGuardTryLock<my_RWLock> mX(0, 6);
            mX.release();
      ...
      case 3: {
        // --------------------------------------------------------------------
        // TESTING CLASS 'bdlmtt::WriteLockGuardUnlock'
        //
        // Concern:
        //   That the basic functionality of the 'bdlmtt::WriteLockGuardUnlock'
        //   class template is correct.
        //
        // Plan:
        //   We begin by creating a series of nested
        //   'bdlmtt::WriteLockGuardUnlock' objects using a common 'my_RWLock'
        ...
        //
        //   Next, we verify that the 'release' function works properly by
        //   constructing a new 'bdlmtt::WriteLockGuardUnlock' and calling
        ...
        //   'bdlmtt::WriteLockGuardUnlock' object is destroyed, it does not
        //   unlock the object.
        //
        //   Finally we test that a 'bdlmtt::WriteLockGuardUnlock' can be created
        //   with a null lock, and that 'release' may be called on the guard.
        //
        // Testing:
        //   bdlmtt::WriteLockGuardUnlock();
        //   ~bdlmtt::WriteLockGuardUnlock();
        //   bdlmtt::WriteLockGuardUnlock::release();
        //   bdlmtt::WriteLockGuardUnlock::ptr();
        // --------------------------------------------------------------------
...

        {
            ...
            {
                bdlmtt::WriteLockGuardUnlock<my_RWLock> l1(&mX);
                if(verbose)  P(X.lockCount());
                ASSERT(-1 == X.lockCount());
                {
                    const int PRE_UNLOCKED = 0;
                    bdlmtt::WriteLockGuardUnlock<my_RWLock>
                                                         l2(&mX, PRE_UNLOCKED);
                    if(verbose)  P(X.lockCount());
                    ASSERT(-2 == X.lockCount());
                    {
                        const int PRE_UNLOCKED = 1; mX.unlock();
                        bdlmtt::WriteLockGuardUnlock<my_RWLock> l3(&mX,
                                                             PRE_UNLOCKED);
                        if(verbose)  P(X.lockCount());
                        ASSERT(-3 == X.lockCount());
                    }
        ...
        {
            bdlmtt::WriteLockGuardUnlock<my_RWLock> mX(0);
        }
        {
            bdlmtt::WriteLockGuardUnlock<my_RWLock> mX(0);
            mX.release();
      ...
      case 2: {
        // --------------------------------------------------------------------
        // TESTING CLASS 'bdlmtt::WriteLockGuard'
        //
        // Concern:
        //   That the basic functionality of the 'bdlmtt::WriteLockGuard' class
        //   template is correct.
        //
        // Plan:
        //   We begin by creating a series of nested 'bdlmtt::WriteLockGuard'
        ...
        //   supplied object and that when the 'bdlmtt::WriteLockGuard' object is
        //   destroyed, it does not unlock the object.
        //
        //   Finally we test that a 'bdlmtt::WriteLockGuard' can be created with
        //   a null lock, and that 'release' may be called on the guard.
        //
        // Testing:
        //   bdlmtt::WriteLockGuard();
        //   ~bdlmtt::WriteLockGuard();
        //   bdlmtt::WriteLockGuard::release();
        //   bdlmtt::WriteLockGuard::ptr();
        // --------------------------------------------------------------------
...

        {
            ...
            if (verbose)
                cout << "\t\t lock guard constructor & destructor" <<endl;
            {
                bdlmtt::WriteLockGuard<my_RWLock> l1(&mX);
                if(verbose)  P(X.lockCount());
                ASSERT(1 == X.lockCount());
                {
                    const int PRE_LOCKED = 0;
                    bdlmtt::WriteLockGuard<my_RWLock> l2(&mX, PRE_LOCKED);
                    if(verbose)  P(X.lockCount());
                    ASSERT(2 == X.lockCount());
                    {
                    const int PRE_LOCKED = 1; mX.lockWrite();
                        bdlmtt::WriteLockGuard<my_RWLock> l3(&mX, PRE_LOCKED);
                        if(verbose)  P(X.lockCount());
                        ASSERT(3 == X.lockCount());
                    }
                    if(verbose)  P(X.lockCount());
        ...
        {
            bdlmtt::WriteLockGuard<my_RWLock> mX(0);
        }
        {
            bdlmtt::WriteLockGuard<my_RWLock> mX(0);
            mX.release();
