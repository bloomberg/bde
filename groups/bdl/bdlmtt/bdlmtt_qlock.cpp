...
BSLS_IDENT_RCSID(bdlmtt_qlock_cpp,"$Id$ $CSID$")
...
typedef bdlmtt::Semaphore *SemaphorePtr;
...
typedef bdlmtt::ThreadUtil::Key TlsKey;
...
{
    bdlmtt::ThreadUtil::deleteKey(*key);
...
struct SemaphoreKeyGuard {
        ...
        // Release the owned TLS key object.
    {
        ...
        releaseSemaphoreObject(bdlmtt::ThreadUtil::getSpecific(*d_key_p));
...
extern "C" void deleteThreadLocalSemaphore(void *semaphore)
    ...
    // 'bdlmtt::Semaphore' pointer, or 0.  Note that this function is intended to
    // serve as a "destructor" callback for 'bdlmtt::ThreadUtil::createKey'.
    ...
    // Return the address of the semaphore unique to the calling thread.
{
    ...
    if (!s_semaphore) {
        s_semaphore = new (semaphoreAllocator()) bdlmtt::Semaphore();

        // Still need to store the object in the thread specific key to release
        // it properly on thread exit.
        bdlmtt::ThreadUtil::setSpecific(*getSemaphoreTLSKey(), s_semaphore);
    }
    ...
    SemaphorePtr sema = reinterpret_cast<SemaphorePtr>(
        bdlmtt::ThreadUtil::getSpecific(*key));

    if (!sema) {
        sema = new (semaphoreAllocator()) bdlmtt::Semaphore();
        bdlmtt::ThreadUtil::setSpecific(*key, sema);
    }
