...
//-----------------------------------------------------------------------------
// [2] bdlmtt::Barrier(int numThreads);
// [2] ~bdlmtt::Barrier();
// [3] void wait();
...
static bdlmtt::SpinLock coutLock;
...
extern "C" void *b(void *arg)
{
    ...
    for(int i=0; i<c->d_numIterations; i++) {
        ...
        new (mem) bdlmtt::Barrier(c->d_numThreads);
    }
    return 0;
}
...
{
    bsl::vector<bdlmtt::ThreadUtil::Handle>  hh;
    Control c(numIterations, numThreads);

    for (int i = 0; i<numThreads-1; ++i) {
        bdlmtt::ThreadUtil::Handle h;
        bdlmtt::ThreadUtil::create(&h, &a, (void *)&c);
        hh.push_back(h);
    }
    bdlmtt::ThreadUtil::Handle h;
    bdlmtt::ThreadUtil::create(&h, &b, (void *)&c);
    hh.push_back(h);

    for (int i = 0; i<numThreads; ++i) {
        bdlmtt::ThreadUtil::join(hh[i]);
    }
}
...
void case7 (bdlmtt::Barrier* barrier,
   ...
   for (int i = 0; i < numWaits; ++i) {
      ...
      while (state < ((i+1) * numThreads)) bdlmtt::ThreadUtil::yield();
   }
...
int main(int argc, char *argv[])
{
    ...
    switch (test) {
      ...
      case 8: {
        ...
        ASSERT(0 == bdlmtt::ThreadUtil::create(&dummy, detached, driver));
      ...
      case 7: {
        ...
        //
        // Concern: that bdlmtt::Barrier is reusable by the same thread group,
       ...
       bdlmtt::Barrier basicBarrier(NUM_THREADS);
       if (veryVerbose) {
      ...
      case 2: {
...

        {
            ...
            for (int i = 0; i < NUM_VALUES; ++i) {
                ...
                {
                    bdlmtt::Barrier x(NUM); const bdlmtt::Barrier &X = x;
                    LOOP_ASSERT(LINE, NUM == X.numThreads());
                }

                {
                    bdlmtt::Barrier x(NUM, bsls::SystemClockType::e_REALTIME);
                    const bdlmtt::Barrier &X = x;
                    LOOP_ASSERT(LINE, NUM == X.numThreads());
                }

                {
                    bdlmtt::Barrier x(NUM, bsls::SystemClockType::e_MONOTONIC);
                    const bdlmtt::Barrier &X = x;
                    LOOP_ASSERT(LINE, NUM == X.numThreads());
                }
            }
