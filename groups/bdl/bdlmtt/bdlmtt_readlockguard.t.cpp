...
//                              OVERVIEW
//                              --------
// This program tests the functionality of the 'bdlmtt::ReadLockGuard' class.  It
...
//
//-----------------------------------------------------------------------------
// bdlmtt::ReadLockGuard
// ===================
// [2] bdlmtt::ReadLockGuard();
// [2] ~bdlmtt::ReadLockGuard();
// [2] bdlmtt::ReadLockGuard::release();
// [2] bdlmtt::ReadLockGuard::ptr();
//
// bdlmtt::ReadLockGuardUnlock
// =====================
// [3] bdlmtt::ReadLockGuardUnlock();
// [3] ~bdlmtt::ReadLockGuardUnlock();
// [3] bdlmtt::ReadLockGuardUnlock::release();
// [3] bdlmtt::ReadLockGuardUnlock::ptr();
//
// bdlmtt::ReadLockGuardTryLock
// ======================
// [4] bdlmtt::ReadLockGuardTryLock();
// [4] ~bdlmtt::ReadLockGuardTryLock();
// [4] bdlmtt::ReadLockGuardTryLock::release();
// [4] bdlmtt::ReadLockGuardTryLock::ptr();
//-----------------------------------------------------------------------------
// [1] Ensure helper class 'my_RWLock' works as expected
// [5] INTERACTION BETW. 'bdlmtt::ReadLockGuard' AND 'bdlmtt::ReadLockGuardUnlock'
// [6] DEPRECATED 'bdlmtt::LockReadGuard'
// [7] USAGE EXAMPLES
//=============================================================================
...
struct my_RWLock {
    ...
    // conforms to the interface required by the 'bdlmtt::ReadLockGuard' class.
...
{
    bdlmtt::ReadLockGuard<my_RWLock> guard(rwlock);
    if (someCondition) {
        obj->someMethod();
        return;
    } else if (someOtherCondition) {
    ...
    // Return 0 if locking fails.
{
    ...
    bdlmtt::ReadLockGuardTryLock<my_RWLock> guard(rwlock, RETRIES);
...
static void safeAtomicUpdateFunc(my_Object *obj, my_RWLock *rwlock)
{
    ...
    bdlmtt::ReadLockGuard<my_RWLock> guard(rwlock, PRELOCKED);
    if (someUpgradeCondition) {
...
void f(my_RWLock *rwlock)
{
    ...
    {
        bdlmtt::ReadLockGuardUnlock<my_RWLock> guard(rwlock);
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:
      ...
      case 6: {
        ...
        //
        // Concern:
        //   Backwards compatibility: ensure that the 'bdlmtt::LockReadGuard'
...

        {
            ...
            {
                bdlmtt::LockReadGuard<my_RWLock> l1(&mX);
                if(verbose)  P(X.lockCount());
                ASSERT(1 == X.lockCount());
                {
                    const int PRE_LOCKED = 0;
                    bdlmtt::LockReadGuard<my_RWLock> l2(&mX, PRE_LOCKED);
                    if(verbose)  P(X.lockCount());
                    ASSERT(2 == X.lockCount());
                    {
                        const int PRE_LOCKED = 1; mX.lockRead();
                        bdlmtt::LockReadGuard<my_RWLock> l3(&mX, PRE_LOCKED);
                        if(verbose)  P(X.lockCount());
      ...
      case 5: {
        // --------------------------------------------------------------------
        // INTERACTION BETWEEN 'bdlmtt::ReadLockGuard' AND
        // 'bdlmtt::ReadLockGuardUnlock'
        //
        // Concern:
        //   That 'bdlmtt::ReadLockGuard' and 'bdlmtt::ReadLockGuardUnlock'
        ...
        //   distinct 'bdlmtt::ReadLockGuard' *and* 'bdlmtt::ReadLockGuardUnlock'
        ...
        if (verbose)
            cout << "\tTesting interaction between "
                    "'bdlmtt::ReadLockGuard' and bdlmtt::ReadLockGuardUnlock'"
                 << endl
                 << "=============================="
                    "========================================"
                 << endl;

        {
            ...
            mX1.lockRead();
            {
                bdlmtt::ReadLockGuard<my_RWLock> l1(&mX1);
                bdlmtt::ReadLockGuard<my_RWLock> l2(&mX2);
                ASSERT(2 == X1.lockCount());
                ASSERT(1 == X2.lockCount());

                {
                    bdlmtt::ReadLockGuardUnlock<my_RWLock> u1(&mX1);
                    bdlmtt::ReadLockGuardUnlock<my_RWLock> u2(&mX2);
      ...
      case 4: {
        // --------------------------------------------------------------------
        // TESTING CLASS 'bdlmtt::ReadLockGuardTryLock'
        //
        // Concern:
        //   That the basic functionality of the 'bdlmtt::ReadLockGuardTryLock'
        //   class template is correct.
        //
        // Plan:
        //   We begin by creating a series of nested
        //   'bdlmtt::ReadLockGuardTryLock' objects using a common 'my_RWLock'
        ...
        //   constructing a new 'bdlmtt::ReadLockGuard' and calling 'release'.
        ...
        //   object and that when the 'bdlmtt::ReadLockGuardTryLock' object is
        //   destroyed, it does not unlock the object.
        //
        //   Finally we test that a 'bdlmtt::ReadLockGuardTryLock' can be created
        //   with a null lock, and that 'release' may be called on the guard.
        //
        // Testing:
        //   bdlmtt::ReadLockGuardTryLock();
        //   ~bdlmtt::ReadLockGuardTryLock();
        //   bdlmtt::ReadLockGuardTryLock::release();
        //   bdlmtt::ReadLockGuardTryLock::ptr();
        // --------------------------------------------------------------------
...

        {
            ...
            {
                bdlmtt::ReadLockGuardTryLock<my_RWLock> l1(&mX);
                if(verbose)  P(X.lockCount());
                ASSERT(0 == X.lockCount());
                ASSERT(0 == l1.ptr());
                {
                    bdlmtt::ReadLockGuardTryLock<my_RWLock> l2(&mX);
                    if(verbose)  P(X.lockCount());
                    ASSERT(1 == X.lockCount());
                    {
                        bdlmtt::ReadLockGuardTryLock<my_RWLock> l3(&mX, 10);
                        if(verbose)  P(X.lockCount());
                        ASSERT(2 == X.lockCount());
                    }
                    if(verbose)  P(X.lockCount());
            ...
            {
                bdlmtt::ReadLockGuardTryLock<my_RWLock> l1(&mX, 2);
        ...
        {
            bdlmtt::ReadLockGuardTryLock<my_RWLock> mX(0, 6);
        }
        {
            bdlmtt::ReadLockGuardTryLock<my_RWLock> mX(0, 6);
            mX.release();
      ...
      case 3: {
        // --------------------------------------------------------------------
        // TESTING CLASS 'bdlmtt::ReadLockGuardUnlock'
        //
        // Concern:
        //   That the basic functionality of the 'bdlmtt::ReadLockGuardUnlock'
        //   class template is correct.
        //
        // Plan:
        //   We begin by creating a series of nested
        //   'bdlmtt::ReadLockGuardUnlock' objects using a common 'my_RWLock'
        ...
        //
        //   Next, we verify that the 'release' function works properly by
        //   constructing a new 'bdlmtt::ReadLockGuardUnlock' and calling
        ...
        //   'bdlmtt::ReadLockGuardUnlock' object is destroyed, it does not
        //   unlock the object.
        //
        //   Finally we test that a 'bdlmtt::ReadLockGuardUnlock' can be created
        //   with a null lock, and that 'release' may be called on the guard.
        //
        // Testing:
        //   bdlmtt::ReadLockGuardUnlock();
        //   ~bdlmtt::ReadLockGuardUnlock();
        //   bdlmtt::ReadLockGuardUnlock::release();
        //   bdlmtt::ReadLockGuardUnlock::ptr();
        // --------------------------------------------------------------------
...

        {
            ...
            {
                bdlmtt::ReadLockGuardUnlock<my_RWLock> l1(&mX);
                if(verbose)  P(X.lockCount());
                ASSERT(-1 == X.lockCount());
                {
                    const int PRE_UNLOCKED = 0;
                    bdlmtt::ReadLockGuardUnlock<my_RWLock> l2(&mX, PRE_UNLOCKED);
                    if(verbose)  P(X.lockCount());
                    ASSERT(-2 == X.lockCount());
                    {
                        const int PRE_UNLOCKED = 1; mX.unlock();
                        bdlmtt::ReadLockGuardUnlock<my_RWLock> l3(&mX,
                                                                PRE_UNLOCKED);
                        if(verbose)  P(X.lockCount());
                        ASSERT(-3 == X.lockCount());
                    }
        ...
        {
            bdlmtt::ReadLockGuardUnlock<my_RWLock> mX(0);
        }
        {
            bdlmtt::ReadLockGuardUnlock<my_RWLock> mX(0);
            mX.release();
      ...
      case 2: {
        // --------------------------------------------------------------------
        // TESTING CLASS 'bdlmtt::ReadLockGuard'
        //
        // Concern:
        //   That the basic functionality of the 'bdlmtt::ReadLockGuard' class
        //   template is correct.
        //
        // Plan:
        //   We begin by creating a series of nested 'bdlmtt::ReadLockGuard'
        ...
        //   supplied object and that when the 'bdlmtt::ReadLockGuard' object is
        //   destroyed, it does not unlock the object.
        //
        //   Finally we test that a 'bdlmtt::ReadLockGuard' can be created with a
        //   null lock, and that 'release' may be called on the guard.
        //
        // Testing:
        //   bdlmtt::ReadLockGuard();
        //   ~bdlmtt::ReadLockGuard();
        //   bdlmtt::ReadLockGuard::release();
        //   bdlmtt::ReadLockGuard::ptr();
        // --------------------------------------------------------------------
...

        {
            ...
            if (verbose)
                cout << "\t\t lock guard constructor & destructor" <<endl;
            {
                bdlmtt::ReadLockGuard<my_RWLock> l1(&mX);
                if(verbose)  P(X.lockCount());
                ASSERT(1 == X.lockCount());
                {
                    const int PRE_LOCKED = 0;
                    bdlmtt::ReadLockGuard<my_RWLock> l2(&mX, PRE_LOCKED);
                    if(verbose)  P(X.lockCount());
                    ASSERT(2 == X.lockCount());
                    {
                    const int PRE_LOCKED = 1; mX.lockRead();
                        bdlmtt::ReadLockGuard<my_RWLock> l3(&mX, PRE_LOCKED);
                        if(verbose)  P(X.lockCount());
                        ASSERT(3 == X.lockCount());
                    }
                    if(verbose)  P(X.lockCount());
        ...
        {
            bdlmtt::ReadLockGuard<my_RWLock> mX(0);
        }
        {
            bdlmtt::ReadLockGuard<my_RWLock> mX(0);
            mX.release();
