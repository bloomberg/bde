...
//                             TEST PLAN
//-----------------------------------------------------------------------------
// Testing the linear mechanics of 'bdlmtt::Once' and 'bdlmtt::OnceGuard' is
...
// 'enter', 'leave', and 'cancel' functionality of 'bdlmtt::Once', the
// 'bdlmtt::OnceGuard' class, and the 'BDLMTT_ONCE_DO' macro.  We round out the
// tests with a breathing test and usage example.
//-----------------------------------------------------------------------------
// [ 3] void bdlmtt::Once::cancel(OnceLock *onceLock);
// [ 2] bool bdlmtt::Once::enter(OnceLock *onceLock);
// [ 2] void bdlmtt::Once::leave(OnceLock *onceLock);
// [ 4] bdlmtt::OnceGuard::bdlmtt::OnceGuard(bdlmtt::Once *once);
// [ 4] bdlmtt::OnceGuard::~bdlmtt::OnceGuard(bdlmtt::Once *once);
// [ 5] void bdlmtt::OnceGuard::cancel();
// [ 4] bool bdlmtt::OnceGuard::enter();
// [ 4] bool bdlmtt::OnceGuard::isInProgress() const;
// [ 6] void bdlmtt::OnceGuard::leave();
// [ 7] void bdlmtt::OnceGuard::setOnce(bdlmtt::Once *once);
// [ 9] BDLMTT_ONCE_CANCEL
// [ 8] BDLMTT_ONCE_DO
//-----------------------------------------------------------------------------
// [ 1] BREATHING TEST
// [10] USAGE EXAMPLE
...
typedef bdlmtt::ThreadUtil       TU;
typedef bdlmtt::ThreadAttributes TA;
typedef bdlmtt::Barrier          BAR;
...
static bdlmtt::Once onceControl =  BDLMTT_ONCE_INITIALIZER;
...
struct my_TestRecord {
    ...
    int            d_id;          // Record ID
    bdlmtt::Once    *d_once_p;      // Shared once object
...
void *enterLeaveTest(void *ptr)
    // Thread function for testing 'bdlmtt::Once::enter' and 'bdlmtt::Once::leave'
{
...
void *enterCancelTest(void *ptr)
    // Thread function for testing 'bdlmtt::Once::cancel'
{
    my_TestRecord *testRec = static_cast<my_TestRecord*>(ptr);
    ASSERT(testRec != 0);
    ASSERT(testRec->d_barrier_p != 0);
...
void *guardTest(void *ptr)
    // Thread function for testing 'bdlmtt::OnceGuard'
{
    my_TestRecord *testRec = static_cast<my_TestRecord*>(ptr);
    ASSERT(testRec != 0);
    ASSERT(testRec->d_barrier_p != 0);
    ...
    {
        bdlmtt::OnceGuard onceGuard(testRec->d_once_p);
...
void *guardCancelTest(void *ptr)
    // Thread function for testing 'bdlmtt::OnceGuard::cancel'
{
    my_TestRecord *testRec = static_cast<my_TestRecord*>(ptr);
    ASSERT(testRec != 0);
    ASSERT(testRec->d_barrier_p != 0);
...
void *macroTest(void *ptr)
    // Thread function for testing the 'BDLMTT_ONCE_DO' macro
{
    my_TestRecord *testRec = static_cast<my_TestRecord*>(ptr);
    ASSERT(testRec != 0);
    ASSERT(testRec->d_barrier_p != 0);
...
void *macroCancelTest(void *ptr)
    // Thread function for testing the 'BDLMTT_ONCE_CANCEL' macro
{
    my_TestRecord *testRec = static_cast<my_TestRecord*>(ptr);
    ASSERT(testRec != 0);
    ASSERT(testRec->d_barrier_p != 0);
...
// 'bdlmtt::OnceGuard', and the use of 'BDLMTT_ONCE_DO'.  All of our singleton
...
// initialization to a simple 'BDLMTT_ONCE_DO' construct.  This implementation
...
// it uses the 'BDLMTT_ONCE_DO' macro to handle multiple entries to the
...
//
// If the 'BDLMTT_ONCE_DO' mechanism works for you, then you need not read any
...
// 'bdlmtt::Once'.  Each succeeding example will use higher-level facilities to
// produce a simpler implementations (though none as simple as the
// 'BDLMTT_ONCE_DO' example above).
//
...
// 'singleton1' function, below, uses 'bdlmtt::Once' to ensure that
...
// available to 'BDLMTT_ONCE_DO' when we get a fix for the MS VC++ 2003
...
// of the special type, 'bdlmtt::Once::OnceLock', which is a cookie created
// on each thread's stack and passed to the methods of 'bdlmtt::Once'.  First,
// we declare a static 'bdlmtt::Once' object as before, and also declare a
...
//..
// Our third implementation uses 'bdlmtt::OnceGuard' to simplify the previous
// implementation.  The 'bdlmtt::OnceGuard' hides (automates) the use of
// 'bdlmtt::Once::OnceLock'.  We begin as before, defining a static
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      ...
      case 9: {
        // --------------------------------------------------------------------
        // TESTING BDLMTT_ONCE_CANCEL
        //
        // Concerns:
        //   - Calling 'BDLMTT_ONCE_CANCEL' within a 'BDLMTT_ONCE_DO' construct
        //     will leave the one-time code region in a restartable state.
        //   - Exactly one thread that is waiting on the 'BDLMTT_ONCE_DO' will
        ...
        //     'BDLMTT_ONCE_DO' macro uses a hidden static 'bdlmtt::Once' object
      ...
      case 8: {
        // --------------------------------------------------------------------
        // TESTING BDLMTT_ONCE_DO
        //
        // Concerns:
        //   - 'BDLMTT_ONCE_DO' can fully automate use of 'bdlmtt::Once'.
        //   - 'BDLMTT_ONCE_DO' can be used twice in the same block.
        //
        // Plan:
        //   - Create an array of data records with one element per thread.
        //   - Create a function that uses 'BDLMTT_ONCE_DO' to enter the
        ...
        //     current state of the 'bdlmtt::OnceGuard' object.
        ...
        //     the same execution because the the 'BDLMTT_ONCE_DO' macro uses a
        //     hidden static 'bdlmtt::Once' object that cannot be reset.)
        //   - Create two, non-nested BDLMTT_ONCE_DO constructs within a
        ...
        //     once.
        //   - Repeat the previous test but with nested BDLMTT_ONCE_DO
        //     constructs.
        //
        // Testing:
        //      BDLMTT_ONCE_DO
      ...
      case 7: {
        ...
        //
        // Concerns:
        //    - After calling 'leave' on a 'bdlmtt::OnceGuard', it can be
        //      associated with a new 'bdlmtt::Once' object and re-used.
        //
        //
        // Plan:
        //    - Because a 'bdlmtt::OnceGuard' object is not itself shared
        //      among threads and because the underlying thread-safe access to
        //      the 'bdlmtt::Once' object has already been verified by other
        //      test cases, there is no need even to spawn threads in this
        //      test case.
        //    - Create a 'bdlmtt::Once' object.
        //    - Construct a 'bdlmtt::OnceGuard' using the 'bdlmtt::Once' object.
        ...
        //    - Create a second 'bdlmtt::Once' object.
        ...
        //    - Call 'leave' on the guard.
        //    - Verify that the guard 'isInProgress' now returns false.
        //    - Create a third 'bdlmtt::Once' object.
        //    - Call 'enter' and 'leave' on the third 'bdlmtt::Once' object to
        ...
        //      'bdlmtt::Once' object has already been used).
        //
        // Testing:
        //      void bdlmtt::OnceGuard::setOnce(bdlmtt::Once *once);
        // --------------------------------------------------------------------
        ...
        bdlmtt::Once once1 = BDLMTT_ONCE_INITIALIZER;
        bdlmtt::OnceGuard guard(&once1);
        ...
        bdlmtt::Once once2 = BDLMTT_ONCE_INITIALIZER;
        guard.setOnce(&once2);
        ASSERT(! guard.isInProgress());
        ASSERT(  guard.enter());
        ASSERT(  guard.isInProgress());
        ...
        bdlmtt::Once once3 = BDLMTT_ONCE_INITIALIZER;
        bdlmtt::Once::OnceLock onceLock;
      ...
      case 6: {
        ...
        //
        // Concerns:
        //    - Calling 'leave' on a 'bdlmtt::OnceGuard' will put the
        //      associated 'bdlmtt::Once' object into a "done" state.
        ...
        //    - Call 'leave' on the original 'bdlmtt::Once' object and verify
        //      that it returns false.
        //
        // Testing:
        //      void bdlmtt::OnceGuard::leave();
        // --------------------------------------------------------------------
        ...
        bdlmtt::Once once = BDLMTT_ONCE_INITIALIZER;
        bdlmtt::OnceGuard guard(&once);
        ASSERT(! guard.isInProgress());
        ASSERT(  guard.enter());
        ASSERT(  guard.isInProgress());
        guard.leave();
      ...
      case 5: {
        ...
        //
        // Concerns:
        //   - Calling 'cancel' on a 'bdlmtt::OnceGuard' object will leave the
        //     one-time code region in a restartable state.
        //   - Exactly one thread that is waiting on the 'bdlmtt::Once' will
        //     be admitted into the one-time region.
        //   - When any thread completes the one-time code, all other threads
        //     will be unblocked and will not execute the one-time code.
        //
        // Plan:
        //   - Create an array of data records with one element per thread.
        //   - Create a function that, using a 'bdlmtt::OnceGuard' on a shared
        //     'bdlmtt::Once' object, sets an entry in a shared array of
      ...
      case 4: {
        ...
        //
        // Concerns: That the 'bdlmtt::OnceGuard' can automate the 'leave'
        //   some of the functionality of 'bdlmtt::Once'.  In particular,
        //   'leave' is called automatically on destruction.
        //
        // Plan:
        //   - Create an array of data records with one element per thread.
        //   - Create a function that uses a 'bdlmtt::Once' guard to enter the
        //     one-time code, set a "winner" flag within a specified record
        //     and a shared "winner" flag, then destroys the guard by exiting
        //     the scope.
        //   - In the thread function, verify that 'isInProgress' reflects the
        ...
        //     set the "done" flag).
        //   - Verify that only one thread set its "winner" flag.
        //   - Repeat the test without resetting the bdlmtt::Once structure.
        //   - Verify that none of the threads are winners the second time.
        //   - Reset the 'bdlmtt::Once' object and repeat this test several times
        //     to maximize the chance of exposing unhandled race conditions.
        //   - Repeat the entire test with two separate 'bdlmtt::Once' objects
        //     such that half the threads access one object and the other half
        //     access the other object.
        //
        // Testing:
        //      bdlmtt::OnceGuard::bdlmtt::OnceGuard(bdlmtt::Once *once);
        //      bdlmtt::OnceGuard::~bdlmtt::OnceGuard(bdlmtt::Once *once);
        //      bool bdlmtt::OnceGuard::enter();
        //      bool bdlmtt::OnceGuard::isInProgress() const;
        // --------------------------------------------------------------------
        ...
        if (verbose) bsl::cout << "Testing with one 'bdlmtt::Once' object"
                               << bsl::endl;
        ...
        if (verbose) bsl::cout << "Testing with two 'bdlmtt::Once' objects"
                               << bsl::endl;
      ...
      case 3: {
        ...
        //
        // Concerns:
        //   - Calling 'cancel' on a 'bdlmtt::Once' object will leave the
        //     one-time code region in a restartable state.
        //   - Exactly one thread that is waiting on a 'bdlmtt::Once' will
        //     be admitted into the one-time region.
        //   - When any thread completes the one-time code, all other threads
        //     will be unblocked and will not execute the one-time code.
        //
        // Plan:
        //   - Create an array of data records with one element per thread.
        //   - Create a function that, using 'enter' on a shared 'bdlmtt::Once'
        ...
        //
        // Testing:
        //      void bdlmtt::Once::cancel(OnceLock *onceLock);
        // --------------------------------------------------------------------
      ...
      case 2: {
        ...
        //   'bdlmtt::Once' object using the 'enter' and 'leave' methods, the
        //   following occurs:
        ...
        //     change the behavior.)
        //   - If there are two ore more 'bdlmtt::Once' objects, then they are
        ...
        //   - Reset the 'bdlmtt::Once' object and repeat this test many times
        //     to maximize the chance of exposing unhandled race conditions.
        //   - Repeat the entire test with two separate 'bdlmtt::Once' objects
        //     such that half the threads access one object and the other half
        //     access the other object.
        //
        // Testing:
        //      bool bdlmtt::Once::enter(OnceLock *onceLock);
        //      void bdlmtt::Once::leave(OnceLock *onceLock);
        // --------------------------------------------------------------------
