// bdlmtt_readerwriterlock.t.cpp    -*-C++-*-
...
typedef bdlmtt::ReaderWriterLock Obj;
enum { NTHREADS = 5 };
...
void my_Condition::wait(bdlmtt::Mutex *mutex)
{
    d_mutex.lock();
...
int  my_Condition::timedWait(bdlmtt::Mutex *mutex,
...
class UserInfoCache {
    ...
    bdlmtt::ReaderWriterLock d_lock;
    InfoMap                d_infoMap;
  public:
    UserInfoCache();
    ~UserInfoCache();
...
{
    bdlmtt::ReaderWriterLock *mX = (Obj*) arg;
    ASSERT(mX);
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:
      case 11: {
                 ...
                 << endl;
        {
            ...
            for (int i = 0; i < NUM_THREADS; ++i) {
                bdlmtt::ThreadUtil::ThreadFunction  threadMain = (i % 2)
                                                             ? case11ThreadRW
                                                             : case11ThreadRO;
                int rc = bdlmtt::ThreadUtil::create(&workers[i],
                                                  bdlmtt::ThreadAttributes(),
                                                  threadMain,
                                                  &argument);
                LOOP_ASSERT(i, 0 == rc);
            }
      ...
      case 6: {
        ...
        //   a 'bdlmtt::ReaderWriterLock' object.  By integrating the code
      ...
      case 3: {
...

        {
            ...
            bdlmtt::ThreadUtil::Handle threadHandles[NTHREADS];
            bdlmtt::ThreadUtil::Handle writerHandle;
            bdlmtt::ThreadAttributes attributes;
            ...
            bdlmtt::ThreadUtil::create(&writerHandle, attributes,
            ...
            args.signalStop();
            bdlmtt::ThreadUtil::join(writerHandle);
            for (int i=0; i<NTHREADS; ++i) {
                bdlmtt::ThreadUtil::join(threadHandles[i]);
            }
        }
