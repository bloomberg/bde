...
int translatePriority(bdlmtt::ThreadAttributes::SchedulingPolicy policy,
                      bool                                     low)
{
    if (low) {
        return bdlmtt::ThreadUtil::getMinSchedulingPriority(policy);    // RETURN
    }
    else {
        return bdlmtt::ThreadUtil::getMaxSchedulingPriority(policy);    // RETURN
    }
}
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      ...
      case -1: {
        ...
        const Policy DF = bdlmtt::ThreadAttributes::BCEMT_SCHED_DEFAULT;
        const Policy SO = bdlmtt::ThreadAttributes::BCEMT_SCHED_OTHER;
        const Policy SF = bdlmtt::ThreadAttributes::BCEMT_SCHED_FIFO;
        const Policy SR = bdlmtt::ThreadAttributes::BCEMT_SCHED_RR;
        ...
        for (int i = 0; i < DATA_LEN; ++i) {
            ...
            bdlmtt::ThreadUtil::Handle handles[TC::NUM_THREADS];

            bdlmtt::ThreadAttributes notUrgentAttr;
            notUrgentAttr.setStackSize(1024 * 1024);
            notUrgentAttr.setInheritSchedule(0);
            notUrgentAttr.setSchedulingPolicy(POLICY);

            bdlmtt::ThreadAttributes urgentAttr(notUrgentAttr);

            if (NORM_PRI) {
                notUrgentAttr.setSchedulingPriority(
                                 bdlmtt::ThreadUtil::convertToSchedulingPriority(
            ...
            for ( ; numThreads < TC::NUM_THREADS; ++numThreads) {
                bdlmtt::ThreadAttributes *attr
                                      ...
                                      ? &notUrgentAttr
                                      : &urgentAttr;
                rc = bdlmtt::ThreadUtil::create(&handles[numThreads],
                                              *attr,
                                              fs[numThreads]);
            ...
            for (int j = 0; j < numThreads; ++j) {
                rc = bdlmtt::ThreadUtil::join(handles[j]);
