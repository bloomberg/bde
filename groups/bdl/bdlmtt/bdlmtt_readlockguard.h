// bdlmtt_readlockguard.h                                              -*-C++-*-
#ifndef INCLUDED_BDLMTT_READLOCKGUARD
#define INCLUDED_BDLMTT_READLOCKGUARD
...
//
//@CLASSES:
//  bdlmtt::ReadLockGuard: automatic locking-unlocking for read access
//  bdlmtt::ReadLockGuardUnlock: automatic unlocking-locking for read access
//  bdlmtt::ReadLockGuardTryLock: automatic non-blocking locking-unlocking for
//                              read access
//  bdlmtt::LockReadGuard: DEPRECATED
//
//@SEE_ALSO: bdlmtt_lockguard, bdlmtt_writelockguard, bdlmtt_rwmutex
//
...
// synchronization object can be any type (e.g., 'bdlmtt::ReaderWriterLock')
// that provides the following methods:
//..
//  void lockRead();
//  void unlock();
//..
// Both 'bdlmtt::ReadLockGuard' and 'bdlmtt::ReadLockGuardUnlock' implement the
// "construction is acquisition, destruction is release" idiom.  During
// construction, 'bdlmtt::ReadLockGuard' automatically calls 'lockRead' on the
// user-supplied object, and 'unlock' when it is destroyed (unless released).
// 'bdlmtt::ReadLockGuardUnlock' does the opposite -- it invokes the 'unlock'
// method when constructed and the 'lockRead' method when destroyed.
//
// A third type of guard, 'bdlmtt::ReadLockGuardTryLock', attempts to acquire a
...
// value to indicate success.  Rather, the 'bdlmtt::ReadLockGuardTryLock'
...
///--------------------------------
// Like all BDE proctor classes, each of the three 'bdlmtt::ReadLockGuard*'
...
//..
//  // 'guard' is an existing guard of type 'bdlmtt::ReadLockGuard<my_RLock>',
...
// a 'bdlmtt::ReadLockGuardTryLock' in the typical following fashion:
//..
...
// does 'bdlmtt::ReaderWriterLock' with the 'upgradeToWriteLock' function, for
// example), this can be safely used in conjunction with 'bdlmtt::ReadLockGuard',
...
// does 'bdlmtt::ReaderWriterLock' if the 'lockReadReserveWrite' function is used
...
//
// Instantiations of 'bdlmtt::ReadLockGuardUnlock' can be interleaved with
// instantiations of 'bdlmtt::ReadLockGuard' to create both critical sections and
// regions where the lock is released.
//..
//  void f(my_RWLock *lock)
//  {
//      bdlmtt::ReadLockGuard<my_RWLock> guard(lock);
//
//      // critical section here
//
//      {
//           bdlmtt::ReadLockGuardUnlock<my_RWLock> guard(lock);
//
...
class ReadLockGuardUnlock {
  ...
  private:
    ...
    ReadLockGuardUnlock<T>& operator=
                              (const ReadLockGuardUnlock<T>&);
...
class ReadLockGuardTryLock {
  ...
  private:
    ...
    ReadLockGuardTryLock<T>& operator=(
                               const ReadLockGuardTryLock<T>&);
...
template <class T>
inline
ReadLockGuard<T>::ReadLockGuard(T *lock, int preLocked)
: d_lock_p(lock)
{
...
template <class T>
inline
LockReadGuard<T>::LockReadGuard(T *lock, int preLocked)
...
// CREATORS
template <class T>
ReadLockGuardTryLock<T>::ReadLockGuardTryLock(T *lock, int retries)
: d_lock_p(0)
{
    if (lock) {
        while (retries--) {
