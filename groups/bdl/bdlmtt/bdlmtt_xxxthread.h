// bdlmtt_xxxthread.h                                                     -*-C++-*-
#ifndef INCLUDED_BDLMTT_XXXTHREAD
#define INCLUDED_BDLMTT_XXXTHREAD
...
//   None
//
//@SEE_ALSO: bdlmtt_condition, bdlmtt_mutex, bdlmtt_recursivemutex,
//  bdlmtt_threadattributes, bdlmtt_threadutil
//
//@AUTHOR: Sergey Tarsis (starsis)
//
//@DESCRIPTION: [!DEPRECATED!]  This component is deprecated.  The collection
// of classes previously provided by 'bdlmtt_xxxthread' have been properly factored
// into individual components (with 'bcemt_Attribute' also being renamed) as
// follows:
//..
//          Class                  Component
//  +----------------------+-----------------------------+
//  | bcemt_Attribute      | bdlmtt_threadattributes      |
//  | bdlmtt::Condition      | bdlmtt_condition             |
//  | bdlmtt::Mutex          | bdlmtt_mutex                 |
//  | bdlmtt::RecursiveMutex | bdlmtt_recursivemutex        |
//  | bdlmtt::ThreadUtil     | bdlmtt_threadutil            |
//  +----------------------+-----------------------------+
//..
...
///Thread Management
///-----------------
// The 'bdlmtt::ThreadUtil' class is a suite of pure procedures that provides the
...
// is created (with 'bdlmtt::ThreadUtil::create()').  A client can also
...
//..
// The thread manipulation functions in 'bdlmtt::ThreadUtil' take a thread handle
...
// have access to the 'bdlmtt::ThreadUtil::NativeHandle' type, which exposes the
...
// 'bdlmtt::ThreadAttributes' object to the 'bdlmtt::ThreadUtil::create' function.
// 'bdlmtt::ThreadAttributes' is a value-semantic type that enables clients to
...
//..
// Note that, with the exception of 'bdlmtt::ThreadAttributes::DetachedState',
...
//
///'bdlmtt::ThreadAttributes::DetachedState'
/// -  -  -  -  -  -  -  -  -  -  -
// A thread can be created in either a "joinable" or "detached" state, by
// setting the 'detachedState' attribute to
// 'bdlmtt::ThreadAttributes::BCEMT_CREATE_JOINABLE' or
// 'bdlmtt::ThreadAttributes::BCEMT_CREATE_DETACHED', respectively.  Any thread
...
// 'bdlmtt::ThreadUtil::join' function on the joinable thread's 'Handle':
//..
...
// 'bdlmtt::ThreadUtil::detach' method.
//
...
///Locking
///-------
// 'bdlmtt_xxxthread' supports two variants of a mutually exclusive lock (mutex):
// 'bdlmtt::Mutex' and 'bdlmtt::RecursiveMutex'.  These may be used directly by
// client programs.  In addition, they are used by other classes in the
// 'bdlmtt_xxxthread' component.
//
///'bdlmtt::Mutex'
///- - - - - - -
// 'bdlmtt::Mutex' is an efficient synchronization primitive that enables
// serialized access to shared resources.  In order to protect against broken
// data invariants, clients can use a 'bdlmtt::Mutex' to ensure exclusive access
// to an area of code or data that is shared among multiple threads within a
// single process.  'bdlmtt::Mutex' provides the following three manipulators:
//..
//  void lock();
//  int tryLock();
//  void unlock();
...
// without calling 'unlock' in between (i.e., 'bdlmtt::Mutex' is non-recursive).
//..
//  void f(bdlmtt::Mutex *mx)
//  {
//      mx->lock();
//      mx->unlock();
//      mx->lock();   // OK
...
//..
//  void f(bdlmtt::RecursiveMutex *rmx)
//  {
...
///-------------------------
// The 'bdlmtt::Condition' class implements the concept of a *condition*
...
// methods of a 'bdlmtt::Condition':
//..
//  int wait(bdlmtt::Mutex *mutex);
//  int timedWait(bdlmtt::Mutex *mutex, const bsls::TimeInterval& timeout);
//..
...
// construction of the 'bdlmtt::Condition' object.  By default, the clock type
// used by a 'bdlmtt::Condition' object is 'bsls::SystemClockType::e_REALTIME'.
...
//      bdlmtt::ThreadUtil::create(&handle, attributes, myThreadFunction, 0);
...
// manipulating the thread's 'bdlmtt::ThreadAttributes' constructor argument.
...
//
// The following snippets of code illustrate the use of 'bdlmtt::Condition' to
...
//      bdlmtt::Condition         d_cv;    // signals the existence of new work
//  };
//
...
//      my_WorkQueue queue;
//
//      bdlmtt::ThreadUtil::Handle mainHandle;
//      mainHandle = bdlmtt::ThreadUtil::self();
//
//      bdlmtt::ThreadAttributes attributes;
//      attributes.setDetachedState(
...
//      assert(0 == status);
//
//      assert(0== bdlmtt::ThreadUtil::isEqual(consumerHandle,producerHandle));
//      assert(0== bdlmtt::ThreadUtil::isEqual(mainHandle, producerHandle));
//      assert(0== bdlmtt::ThreadUtil::isEqual(mainHandle, consumerHandle));
//
//      status = bdlmtt::ThreadUtil::join(consumerHandle);
//      assert(0 == status);
//      void *retArg;
//      status = bdlmtt::ThreadUtil::join(producerHandle, &retArg);
//      assert(0 == status);
//      assert((my_WorkQueue*)retArg == &queue);
//      return 0;
//  }
//..
///Using 'bdlmtt::Mutex' to Add Thread-Safety
/// - - - - - - - - - - - - - - - - - - - -
...
// brevity.
//
// Next, use a 'bdlmtt::Mutex' object to render atomic the function calls of a
...
//  };
//..
// The implementation showcasing the use of the 'bdlmtt::Mutex' follows.
//..
//  // CREATORS
...
#define bcemt_Attribute bdlmtt::ThreadAttributes
#endif
