// bdlmtt_once.h                                                       -*-C++-*-
#ifndef INCLUDED_BDLMTT_ONCE
#define INCLUDED_BDLMTT_ONCE
...
//
//@CLASSES:
//        bdlmtt::Once: Gate-keeper for code executed only once per process
//   bdlmtt::OnceGuard: Guard class for safely using 'bdlmtt::Once'
//
//@SEE_ALSO: bdlmtt_qlock
//
//@AUTHOR: Pablo Halpern (phalpern)
//
//@DESCRIPTION: This component provides a pair of classes, 'bdlmtt::Once' and
// 'bdlmtt::OnceGuard', which give the caller a way to run a body of code
// exactly once within the current process, particularly in the presence of
// multiple threads.  This component also defines the macro 'BDLMTT_ONCE_DO',
...
// first use.
//
// The 'bdlmtt::Once' class is designed to be statically allocated and
// initialized using the 'BDLMTT_ONCE_INITIALIZER' macro.  Client code may use
// the 'bdlmtt::Once' object in one of two ways: 1) it may use the 'callOnce'
...
//
// A safer way to use the 'enter' and 'leave' methods of 'bdlmtt::Once' is to
// manage the 'bdlmtt::Once' object using a 'bdlmtt::OnceGuard' object constructed
// from the 'bdlmtt::Once' object.  Calling 'enter' on the 'bdlmtt::OnceGuard'
// object will call 'enter' on its associated 'bdlmtt::Once' object.
// If the call to 'enter' returns 'true', then the destructor for the guard
// will automatically call 'leave' on its associated 'bdlmtt::Once' object.
// The 'bdlmtt::OnceGuard' class is intended to be allocated on the stack (i.e.,
// as a local variable) so that it is automatically destroyed at the end of its
// enclosing block.  Thus, the to call 'leave' of the 'bdlmtt::Once' object is
// enforced by the compiler.
//
// An even easier way to use the facilities of this component is to use the
// 'BDLMTT_ONCE_DO' macro.  This macro behaves like an 'if' statement --
...
// time.  Thus, bracketing arbitrary code in a 'BDLMTT_ONCE_DO' construct is the
// easiest way to ensure that code will be executed only once for a program.
// The 'BDLMTT_ONCE_DO' behaves correctly even if there are 'return' statements
...
///Warning
///-------
// The 'BDLMTT_ONCE_DO' macro consists of a declaration and a 'for' loop.
// Consequently, the following is syntactically incorrect:
//..
//  if (xyz) BDLMTT_ONCE_DO { stuff() }
//..
// Also, a 'break' or 'continue' statement within a 'BDLMTT_ONCE_DO' construct
// terminates the 'BDLMTT_ONCE_DO', not a surrounding loop or 'switch'
// statement.  For example:
//..
//  switch (xyz) {
//    case 0: BDLMTT_ONCE_DO { stuff(); break; /* does not break case */ }
...
///Thread-Safety
///-------------
// Objects of the 'bdlmtt::Once' class are intended to be shared among threads
// and may be accessed and modified simultaneously in multiple threads by using
// the methods provided.  To allow static initialization, 'bdlmtt::Once' is a POD
...
//
// The 'bdlmtt::OnceGuard' objects are designed to be used only by their creator
// threads and are typically created on the stack.  It is not safe to use a
// 'bdlmtt::OnceGuard' by a thread other than its creator.
//
///Usage
///-----
...
// illustrating the two ways to directly use 'bdlmtt::Once', the use of
// 'bdlmtt::OnceGuard', and the use of 'BDLMTT_ONCE_DO'.  In each example, the
...
/// - - - - - - - - - -
// Our first implementation uses the 'BDLMTT_ONCE_DO' construct, the
// recommended way to use this component.  The function is a variation of the
// singleton pattern described by Scott Meyers, except that the 'BDLMTT_ONCE_DO'
...
//  }
//..
// The 'BDLMTT_ONCE_DO' mechanism suffices for most situations; however,
...
// facilities of 'bdlmtt::Once'.  The two following examples use progressively
// higher-level facilities to produce simpler singleton implementations (though
// none as simple as the 'BDLMTT_ONCE_DO' example above).
//
///Second Implementation
///- - - - - - - - - - -
// The next singleton function implementation directly uses the 'doOnce' method
// of 'bdlmtt::Once'.  We begin by declaring a simple function that does most of
...
// method of 'bdlmtt::Once' to ensure that 'singletonImp' is called by only one
// thread and that the result is visible to all threads.  We start by creating
// and initializing a static object of type 'bdlmtt::Once':
//..
//  #include <bdlf_bind.h>
//
//  const bsl::string& singleton1(const char *s)
//  {
//      static bdlmtt::Once once = BDLMTT_ONCE_INITIALIZER;
//..
...
// exception thrown from within 'singletonImp' will cause the 'bdlmtt::Once'
...
// 'bdlmtt::Once::OnceLock', created on each thread's stack and passed to the
// methods of 'bdlmtt::Once'.  First, we declare a static 'bdlmtt::Once' object as
...
//      static bsl::string *theSingletonPtr = 0;
//..
// Next, we define a local 'bdlmtt::Once::OnceLock' object and pass it to
// the 'enter' method:
//..
//      bdlmtt::Once::OnceLock onceLock;
...
///- - - - - - - - - - -
// Our final implementation, 'singleton3', uses 'bdlmtt::OnceGuard' to simplify
// the previous implementation by using 'bdlmtt::OnceGuard' to hide (automate)
// the use of 'bdlmtt::Once::OnceLock'.  We begin as before, defining a static
// 'bdlmtt::Once' object and a static 'bsl::string' pointer:
//..
...
//      static bsl::string *theSingletonPtr = 0;
//..
// We then declare a local 'bdlmtt::OnceGuard' object and associate it with the
// 'bdlmtt::Once' object before entering the one-time initialization region:
//..
//      bdlmtt::OnceGuard onceGuard(&once);
//      if (onceGuard.enter()) {
//          static bsl::string theSingleton(s);
//          theSingletonPtr = &theSingleton;
//      }
...
#define BDLMTT_ONCE_DO \
    BDLMTT_ONCE_DO_IMP(BDLMTT_ONCE_CAT(bcemt_doOnceObj, BCEMT_ONCE_UNIQNUM))
    ...
    //..
    //  BDLMTT_ONCE_DO { /* one-time code goes here */ }
    //..
    // Leaving a 'BDLMTT_ONCE_DO' construct via 'break', 'continue', or
    // 'return' will put the construct in a "done" state (unless
    // 'BDLMTT_ONCE_CANCEL' has been called) and will unblock all threads
    // waiting to enter the one-time region.  Note that a 'break' or 'continue'
    // within the one-time code will terminate only the 'BDLMTT_ONCE_DO'
    ...
    // within the same construct.  Only one call to 'BDLMTT_ONCE_DO' may appear
    // on a single source-code line in any code block.

#define BDLMTT_ONCE_CANCEL() bcemt_doOnceGuard.cancel()
    // This macro provides a way to cancel once processing within a
    // 'BDLMTT_ONCE_DO' construct.  It will not compile outside of a
    // 'BDLMTT_ONCE_DO' construct.  Executing this function-like macro will set
    // the state of the 'BDLMTT_ONCE_DO' construct to "not entered", possibly
    // unblocking a thread waiting to enter the one-time code region.  Note
    // that this macro does not exit the 'BDLMTT_ONCE_DO' construct (i.e.,
    // it does not have 'break' or 'return' semantics).

#define BDLMTT_ONCE_INITIALIZER { BDLMTT_QLOCK_INITIALIZER, { 0 } }

namespace bdlmtt {    // Use this macro to initialize an object of type 'Once'.  E.g.:
    //..
    //  Once once = BDLMTT_ONCE_INITIALIZER;
    //..
...
// concatenation.
#define BDLMTT_ONCE_CAT(X, Y) BDLMTT_ONCE_CAT_IMP(X, Y)
#define BDLMTT_ONCE_CAT_IMP(X, Y) X##Y

                        // -------------------------------------
                        // Implementation of BDLMTT_ONCE_DO Macro
                        // -------------------------------------
...
#define BDLMTT_ONCE_DO_IMP(doOnceObj)                                         \
    static BloombergLP::bdlmtt::Once doOnceObj = BDLMTT_ONCE_INITIALIZER;       \
    for (BloombergLP::bdlmtt::OnceGuard bcemt_doOnceGuard(&doOnceObj);         \
...
#define BDLMTT_ONCE__CAT(X, Y) BDLMTT_ONCE_CAT(X, Y)
#endif
