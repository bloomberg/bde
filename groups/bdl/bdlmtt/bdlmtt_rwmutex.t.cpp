...
typedef bdlmtt::RWMutex Obj;

//=============================================================================
//                 TEST PLAN: Positive test cases
//
// This is a breathing test only.  bdlmtt::RWMutex is a trivial wrapper which
...
// vs. the bdlmtt_readerwriterlock component.  Case -1 is a speed benchmark of
...
// bdlmtt_readerwriterlock component.  Case -3 checks the 'bias' of the POSIX
// RW mutex (a concern on IBM) and case -4 checks it for the
// bdlmtt_readerwriterlock component.
//
...
{
   LOCK*            d_lock;
   bdlmtt::Semaphore *d_startSema;
   bdlmtt::Barrier   *d_releaseBarrier;

   WriteThread(LOCK *lock = 0,
               bdlmtt::Semaphore *startSema = 0,
               bdlmtt::Barrier   *releaseBarrier = 0)
      : d_lock(lock),
...
{
   LOCK            *d_lock;
   bdlmtt::Barrier   *d_holdBarrier;
   bdlmtt::Barrier   *d_releaseBarrier;
   bdlmtt::Semaphore *d_doneSema;

   ReadThread(LOCK *lock = 0,
              bdlmtt::Barrier   *holdBarrier = 0,
              bdlmtt::Barrier   *releaseBarrier = 0,
              bdlmtt::Semaphore *doneSema = 0)
      : d_lock(lock),
                    ...
                    int numWriters, int numReaders)
{
   ...
   if (0 != bdlmtt::ThreadUtil::create(&hWriter, writerThread)) {
   ...
   PingPongReader<LOCK> readerThread(locks, &stop, &readerScore, 0,
                                     &startBarrier);
   if (0 != bdlmtt::ThreadUtil::create(&hWriter, writerThread) ||
       0 != bdlmtt::ThreadUtil::create(&hReader, readerThread)) {
      cout << "ERROR: Could not create a thread!! Failing test" << endl;
      return -4;
   }
   startBarrier.wait();
   ...
   stop = 1;
   bdlmtt::ThreadUtil::join(hWriter);
   bdlmtt::ThreadUtil::join(hReader);
   ...
   bdlmtt::Barrier startBarrier2(numReaders+2);
   stop = 0;
   bdlmtt::ThreadGroup allThreads;
...
int benchmarkWriterBias(bool* isWriterBias, LOCK* lock)
{
   ...
   detached.setDetachedState(bdlmtt::ThreadAttributes::BCEMT_CREATE_DETACHED);
   bdlmtt::ThreadUtil::Handle h;

   bdlmtt::Barrier   readHold(2), readRelease(2), writeRelease(2);
   bdlmtt::Semaphore writeStart;

   if (0 != bdlmtt::ThreadUtil::create
...
int benchmarkBiasFairness(bool* isFair, LOCK* lock)
{
   ...
   bdlmtt::Barrier   readHold(2), readRelease1(2), writeRelease1(2),
      writeRelease2(2);
   bdlmtt::Semaphore readStart, writeStart, readDone;
   ...
   ReadWaitThread<LOCK> readWaitThread(lock, &readStart,
                                       &rwThreadHasLock);
   if (0 != bdlmtt::ThreadUtil::create(&h, detached, readWaitThread)) {
      return -4;
   }
   readStart.wait();
   bdlmtt::ThreadUtil::yield();
...
void readerThread (bdlmtt::Barrier* start, bdlmtt::Barrier* end, Obj* mutex)
{
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:
      case -1: {
         ...
         bdlmtt::RWMutexImpl<bdlmtt::Platform::PosixThreads> lock;
#else
         bdlmtt::RWMutex lock;
#endif
      ...
      case 1: {
        if (verbose) cout << "Testing: breathing test" << endl
                          ...
                          << endl;
        Obj mutex;
        bdlmtt::Barrier startBarrier(3), endBarrier(3);
        ...
        if (0 != bdlmtt::ThreadUtil::create
           ...
           ASSERT(0 == bdlmtt::ThreadUtil::join(t1));
           ASSERT(0 == bdlmtt::ThreadUtil::join(t2));
        }
