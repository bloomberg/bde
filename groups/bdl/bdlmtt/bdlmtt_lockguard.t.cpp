...
//                              OVERVIEW
//                              --------
// This program tests the functionality of the 'bdlmtt::LockGuard' class.  It
...
// [2] release();
//
// bdlmtt::LockGuardUnlock
// =================
// [3] bdlmtt::LockGuardUnlock();
// [3] ~bdlmtt::LockGuardUnlock();
// [3] release();

// bdlmtt::LockGuardTryLock
// =================
// [4] bdlmtt::LockGuardTryLock();
// [4] ~bdlmtt::LockGuardTryLock();
// [4] release();
//-----------------------------------------------------------------------------
// [1] Ensure helper class 'my_Mutex' works as expected
// [5] INTERACTION BETW. 'bdlmtt::LockGuard' AND 'bdlmtt::LockGuardUnlock'
// [6] DEPRECATED 'bdlmtt::TryLockGuard' and 'bdlmtt::UnLockGuard'
// [7] USAGE EXAMPLES
//=============================================================================
...
struct my_Mutex {
    ...
    // conforms to the interface required by the bdlmtt::LockGuard class.
...
{
    bdlmtt::LockGuard<my_Mutex> guard(mutex);
    if (someCondition) {
        obj->someMethod();
        return;
    } else if (someOtherCondition) {
    ...
    // Return 0 if locking fails.
{
    ...
    bdlmtt::LockGuardTryLock<my_Mutex> guard(mutex, RETRIES);
...
void f(my_Mutex *mutex)
{
    ...
    {
        bdlmtt::LockGuardUnlock<my_Mutex> guard(mutex);
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:
      ...
      case 6: {
        ...
        //
        // Concern:
        //   That 'bdlmtt::TryLockGuard' and 'bdlmtt::UnLockGuard' are equivalent
                          ...
                          << endl;
        {
            ...
            {   // next will fail
                bdlmtt::TryLockGuard<my_Mutex> l1(&mX);
                if(verbose)  P(X.lockCount());
                ASSERT(0 == X.lockCount());
                ASSERT(0 == l1.ptr());
                {   // next will succeed
                    bdlmtt::TryLockGuard<my_Mutex> l2(&mX);
                    if(verbose)  P(X.lockCount());
                    ASSERT(1 == X.lockCount());
                    {   // next will succeed at second attempt
                        bdlmtt::TryLockGuard<my_Mutex> l3(&mX,10);
                        if(verbose)  P(X.lockCount());
            ...
            {
                bdlmtt::TryLockGuard<my_Mutex> l1(&mX, 2);
        ...
        }
        {
            ...
            {
                bdlmtt::UnLockGuard<my_Mutex> l1(&mX);
                if(verbose)  P(X.lockCount());
                ASSERT(-1 == X.lockCount());
                {
                    const int PRE_UNLOCKED = 0;
                    bdlmtt::UnLockGuard<my_Mutex> l2(&mX, PRE_UNLOCKED);
                    if(verbose)  P(X.lockCount());
                    ASSERT(-2 == X.lockCount());
                    {
                        const int PRE_UNLOCKED = 1; mX.unlock();
                        bdlmtt::UnLockGuard<my_Mutex> l3(&mX, PRE_UNLOCKED);
                        if(verbose)  P(X.lockCount());
      ...
      case 5: {
        // --------------------------------------------------------------------
        // INTERACTION BETWEEN 'bdlmtt::LockGuard' AND 'bdlmtt::LockGuardUnlock'
        //
        // Concern:
        //   That bdlmtt::LockGuard and bdlmtt::LockGuardUnlock interact together
        ...
        //   distinct 'bdlmtt::LockGuard' *and* 'bdlmtt::LockGuardUnlock' objects
        ...
        if (verbose)
            cout << "\tTesting interaction between "
                    "'bdlmtt::LockGuard' and bdlmtt::LockGuardUnlock'" << endl
                 << "=============================="
                    "========================================" << endl;

        {
            ...
            mX1.lock();
            {
                bdlmtt::LockGuard<my_Mutex> l1(&mX1);
                bdlmtt::LockGuard<my_Mutex> l2(&mX2);
                ASSERT(2 == X1.lockCount());
                ASSERT(1 == X2.lockCount());

                {
                    bdlmtt::LockGuardUnlock<my_Mutex> u1(&mX1);
                    bdlmtt::LockGuardUnlock<my_Mutex> u2(&mX2);
      ...
      case 4: {
        // --------------------------------------------------------------------
        // TESTING CLASS 'bdlmtt::LockGuardTryLock'
        //
        // Concern:
        //   That the basic functionality of the 'bdlmtt::LockGuardTryLock' class
        //   template is correct.
        //
        // Plan:
        //   We begin by creating a series of nested 'bdlmtt::LockGuardTryLock'
        ...
        //   constructing a new 'bdlmtt::LockGuard' and calling 'release'.  We
        ...
        //   object and that when the 'bdlmtt::LockGuardTryLock' object is
        //   destroyed, it does not unlock the object.
        //
        //   Finally we test that a 'bdlmtt::LockGuardTryLock' can be created
        //   with a null lock, and that 'release' may be called on the guard.
        //
        // Testing:
        //   bdlmtt::LockGuardTryLock();
        //   ~bdlmtt::LockGuardTryLock();
        //   bdlmtt::LockGuardTryLock::release();
        //   bdlmtt::LockGuardTryLock::ptr();
        // --------------------------------------------------------------------
...

        {
            ...
            if (verbose)
                cout << "\t\t trylock guard constructors & destructor" <<endl;
            {   // next will fail
                bdlmtt::LockGuardTryLock<my_Mutex> l1(&mX);
                if(verbose)  P(X.lockCount());
                ASSERT(0 == X.lockCount());
                ASSERT(0 == l1.ptr());
                {   // next will succeed
                    bdlmtt::LockGuardTryLock<my_Mutex> l2(&mX);
                    if(verbose)  P(X.lockCount());
                    ASSERT(1 == X.lockCount());
                    {   // next will succeed at second attempt
                        bdlmtt::LockGuardTryLock<my_Mutex> l3(&mX,10);
                        if(verbose)  P(X.lockCount());
                        ASSERT(2 == X.lockCount());
                    }
                    if(verbose)  P(X.lockCount());
            ...
            if (verbose)
                cout << "\t\t trylock guard release" << endl;
            {
                bdlmtt::LockGuardTryLock<my_Mutex> l1(&mX, 2);
        ...
        {
            bdlmtt::LockGuardTryLock<my_Mutex> mX(0, 6);
        }
        {
            bdlmtt::LockGuardTryLock<my_Mutex> mX(0, 6);
            mX.release();
      ...
      case 3: {
        // --------------------------------------------------------------------
        // TESTING CLASS 'bdlmtt::LockGuardUnlock'
        //
        // Concern:
        ...
        //
        // Plan:
        //   We begin by creating a series of nested 'bdlmtt::LockGuardUnlock'
        ...
        //
        //   Next, we verify that the 'release' function works properly by
        //   constructing a new 'bdlmtt::LockGuardUnlock' and calling 'release'.
        //   We verify that the returned pointer matches the value we supplied.
        //   We then verify that 'release' makes no attempt to unlock
        //   the supplied object and that when the 'bdlmtt::LockGuardUnlock'
        //   object is destroyed, it does not unlock the object.
        //
        //   Finally we test that a 'bdlmtt::LockGuardUnlock' can be created
        //   with a null lock, and that 'release' may be called on the guard.
        //
        // Testing:
        //   bdlmtt::LockGuardUnlock();
        //   ~bdlmtt::LockGuardUnlock();
        //   bdlmtt::LockGuardUnlock::release();
        //   bdlmtt::LockGuardUnlock::ptr();
        // --------------------------------------------------------------------
...

        {
            ...
            if (verbose)
                cout << "\t\t unlock guard constructor & destructor" <<endl;
            {
                bdlmtt::LockGuardUnlock<my_Mutex> l1(&mX);
                if(verbose)  P(X.lockCount());
                ASSERT(-1 == X.lockCount());
                {
                    const int PRE_UNLOCKED = 0;
                    bdlmtt::LockGuardUnlock<my_Mutex> l2(&mX, PRE_UNLOCKED);
                    if(verbose)  P(X.lockCount());
                    ASSERT(-2 == X.lockCount());
                    {
                        const int PRE_UNLOCKED = 1; mX.unlock();
                        bdlmtt::LockGuardUnlock<my_Mutex> l3(&mX, PRE_UNLOCKED);
                        if(verbose)  P(X.lockCount());
                        ASSERT(-3 == X.lockCount());
                    }
                    if(verbose)  P(X.lockCount());
        ...
        {
            bdlmtt::LockGuardUnlock<my_Mutex> mX(0);
        }
        {
            bdlmtt::LockGuardUnlock<my_Mutex> mX(0);
            mX.release();
      ...
      case 2: {
        // --------------------------------------------------------------------
        // TESTING CLASS 'bdlmtt::LockGuard'
        //
        // Concern:
        //   That the basic functionality of the 'bdlmtt::LockGuard' class
        //   template is correct.
        //
        // Plan:
        //   We begin by creating a series of nested 'bdlmtt::LockGuard'
        //   objects using a common 'my_Mutex' object.  With each new object
        //   we verify the the lock function was called.  As each object is
        //   destroyed, we verify that the unlock function is called.
        //
        //   Next, we verify that the 'release' function works properly by
        //   constructing a new 'bdlmtt::LockGuard' and calling 'release'.
        //   We verify that the returned pointer matches the value we supplied.
        //   We then verify that 'release' makes no attempt to unlock
        //   the supplied object and that when the 'bdlmtt::LockGuard' object is
        //   destroyed, it does not unlock the object.
        //
        //   Finally we test that a 'bdlmtt::LockGuard' can be created with a
        //   null lock, and that 'release' may be called on the guard.
        //
        // Testing:
        //   bdlmtt::LockGuard();
        //   ~bdlmtt::LockGuard();
        //   bdlmtt::LockGuard::release();
        //   bdlmtt::LockGuard::ptr();
        // --------------------------------------------------------------------
...

        {
            ...
            {
                bdlmtt::LockGuard<my_Mutex> l1(&mX);
                if(verbose)  P(X.lockCount());
                ASSERT(1 == X.lockCount());
                {
                    const int PRE_LOCKED = 0;
                    bdlmtt::LockGuard<my_Mutex> l2(&mX, PRE_LOCKED);
                    if(verbose)  P(X.lockCount());
                    ASSERT(2 == X.lockCount());
                    {
                        const int PRE_LOCKED = 1; mX.lock();
                        bdlmtt::LockGuard<my_Mutex> l3(&mX, PRE_LOCKED);
                        if(verbose)  P(X.lockCount());
                        ASSERT(3 == X.lockCount());
                    }
                    if(verbose)  P(X.lockCount());
        ...
        {
            bdlmtt::LockGuard<my_Mutex> mX(0);
        }
        {
            bdlmtt::LockGuard<my_Mutex> mX(0);
            mX.release();
