...
//
//@CLASSES:
//       bdlmtt::QLock: Small, statically-initializable intra-process mutex
//  bdlmtt::QLockGuard: Automatic locking-unlocking of bdlmtt::QLock
//
//@SEE_ALSO: bdlmtt_xxxthread, bcemt_atomictypes, bdlmtt_lockguard, bdlmtt_once
//
...
// functionality of the 'bdlmtt::QLock' class overlaps those of the
// 'bdlmtt::Mutex' and 'bdlmtt::SpinLock' classes, but with different usage and
...
// mutex.  A 'bdlmtt::Mutex' object cannot be initialized statically because
// some platforms (e.g., Windows XP) do not have a native
// statically-initializable mutex type.  A 'bdlmtt::QLock' object, in contrast
...
//
///The 'bdlmtt::QLockGuard' Class
///----------------------------
// A 'bdlmtt::QLock' is different from other locking classes such as
// 'bdlmtt::Mutex' and 'bdlmtt::SpinLock' in that it cannot be manipulated except
// through the auxiliary 'bdlmtt::QLockGuard' class.  The reason for this
...
// 'bdlmtt::QLockGuard' provides this extra storage efficiently on the stack.
//
// In typical usage, a 'bdlmtt::QLockGuard' is created as a local (stack)
...
// 'bdlmtt::QLockGuard' object.
//
// 'bdlmtt::QLockGuard' also provides the following manipulators typical of
// locking classes:
//..
...
// listed above are only pass-through operations on the shared 'bdlmtt::QLock'
// object.  In other words, upon return from calling 'lock' on a
// 'bdlmtt::QLockGuard' object, a thread has actually acquired the lock to the
// underlying 'bdlmtt::QLock'.
//
// Although it is only a proxy for the actual QLock, 'lock'/'unlock'/'tryLock'
// interface of 'bdlmtt::QLockGuard' allows it to be treated as though it were
// itself a lock.  In particular, it is possible to instantiate the
// 'bdlmtt::LockGuard' and 'bdlmtt::LockGuardUnlock' class templates using
// 'bdlmtt::QLockGuard'.  This layering of guard classes is useful for creating
...
// a 'bdlmtt::LockGuardUnlock' as follows:
//..
//  void Node::update()
//  {
//     bdlmtt::QLockGuard qguard(&d_qlock);  // 'd_qlock' is a 'bdlmtt::QLock'.
//     readLunarState();
//     if (d_moonIsFull) {
//         // Free lock while we sleep
//         bdlmtt::LockGuardUnlock<bdlmtt::QLockGuard> unlock(&qguard)
...
// without an intervening call to 'unlock' (i.e., 'bdlmtt::QLockGuard' is
// non-recursive).
//
///Usage
///-----
...
//
///Example 1: Using 'bdlmtt::QLock' to Implement a Thread-Safe Singleton
///- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
// corruption at worse.  To solve this problem, we use a 'bdlmtt::QLock' to
...
// 'bdlmtt::QLock' over 'bdlmtt::Mutex':
//..
//      static const bsl::string *singletonPtr = 0;
//      static bdlmtt::QLock qlock = BDLMTT_QLOCK_INITIALIZER;
//..
...
// 'bdlmtt::QLockGuard' object:
//..
//      bdlmtt::QLockGuard qlockGuard(&qlock);
//..
...
// holding a spinlock.  Again, the characteristics of 'bdlmtt::QLock' are
...
// processing.  See also 'bdlmtt_once'.)
//..
//      if (! singletonPtr) {
...
// 'bdlmtt::QLockGuard' will automatically unlock the QLock and allow another
...
namespace bdlmtt {class Semaphore;
}  // close package namespace

#define BDLMTT_QLOCK_INITIALIZER  { {0} }

namespace bdlmtt {    // Use this macro as the value for initializing an object of type
    // 'QLock'  For example:
    //..
    //  QLock mylock = BDLMTT_QLOCK_INITIALIZER;
    //..
...
class QLock_EventFlag {
    ...
    //
    // This class is an implementation detail of the 'bdlmtt_qlock', and must
