...
//-----------------------------------------------------------------------------
// MACROS
//: o BDLMTT_MUTEXASSERT_IS_LOCKED
//: o BDLMTT_MUTEXASSERT_IS_LOCKED_SAFE
//: o BDLMTT_MUTEXASSERT_IS_LOCKED_OPT
//-----------------------------------------------------------------------------
// [ 1] BREATHING TEST
// [ 4] USAGE EXAMPLE
...
// 'BDLMTT_MUTEXASSERT_IS_LOCKED*' family of assertions allows the programmers
...
// 'BDLMTT_MUTEXASSERT_IS_LOCKED*' macros.  We use the '...ISLOCKED_SAFE...'
...
struct TestCase3SubThread {
    bdlmtt::Mutex     *d_mutexToAssertOn;
    bdlmtt::Mutex     *d_mutexThatMainThreadWillUnlock;
...
void myHandler(const char *text, const char *file, int line)
{
    switch (mode) {
      case SAFE_MODE: {
        ASSERT(!bsl::strcmp("BDLMTT_MUTEXASSERT_IS_LOCKED_SAFE(&mutex)", text));
      } break;
      case NORMAL_MODE: {
        ASSERT(!bsl::strcmp("BDLMTT_MUTEXASSERT_IS_LOCKED(&mutex)",      text));
      } break;
      case OPT_MODE: {
        ASSERT(!bsl::strcmp("BDLMTT_MUTEXASSERT_IS_LOCKED_OPT(&mutex)",  text));
      } break;
      default: {
        ASSERT(0);
      }
    ...
    ASSERT(0 &&
              "BDLMTT_MUTEXASSERT_IS_LOCKED* failed wtih exceptions disabled");
    abort();
#endif
}
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      ...
      case 3: {
        ...
        bdlmtt::Mutex     mutexToAssertOn;
        bdlmtt::Mutex     mutexThatMainThreadWillUnlock;
        ...
        int sts = bdlmtt::ThreadUtil::create(&handle, functor);
        ASSERT(0 == sts);
        ...
        BDLMTT_MUTEXASSERT_IS_LOCKED_SAFE(&mutexToAssertOn);
        BDLMTT_MUTEXASSERT_IS_LOCKED(     &mutexToAssertOn);
        BDLMTT_MUTEXASSERT_IS_LOCKED_OPT( &mutexToAssertOn);
        ...
        sts = bdlmtt::ThreadUtil::join(handle);
        ASSERT(0 == sts);
      ...
      case 2: {
        // --------------------------------------------------------------------
        // TESTING 'BDLMTT_MUTEXASSERT_IS_LOCKED*'
        //
        // Concerns:
        //: 1 That 'BDLMTT_MUTEXASSERT_IS_LOCKED*' is never calling
        ...
        if (verbose) cout << "TESTING 'BDLMTT_MUTEXASSERT_IS_LOCKED*'\n"
                             "=====================================\n";
        ...
        if (veryVerbose) cout << "Plan 1: testing with mutex locked\n";
        {
            ...
            BDLMTT_MUTEXASSERT_IS_LOCKED_SAFE(&mutex);
            BDLMTT_MUTEXASSERT_IS_LOCKED(     &mutex);
            BDLMTT_MUTEXASSERT_IS_LOCKED_OPT( &mutex);
        }
      ...
      case -1: {
        // ------------------------------------------------------------------
        // TESTING 'BDLMTT_MUTEXASSERT_IS_LOCKED_OPT'
        // ------------------------------------------------------------------
