// bdlmtt_meteredmutex.t.cpp      -*-C++-*-

#include <bdlmtt_meteredmutex.h>
...
//                                   Overview
//                                   --------
// Testing bdlmtt::MeteredMutex is divided into 3 parts (apart from usage and
// breathing test).
...
typedef bdlmtt::MeteredMutex Obj;
const bsls::Types::Int64 NANOSECONDS_IN_ONE_MICRO_SECOND = 1000LL;
bdlmtt::Mutex printLock; // lock needed for non thread-safe macro (P, P_ etc)
...
bdlmtt::Barrier barrier5(NUM_THREADS5);
  ...
  void *strategy1(void *arg)
    {
        ...
        if (remainder == 1) {
            oddMutex.lock();
            ++oddCount;
            bdlmtt::ThreadUtil::microSleep(SLEEP_TIME5);
            oddMutex.unlock();
        }
        else {
            evenMutex.lock();
...
bdlmtt::Barrier barrier4(NUM_THREADS4);
Obj mutex4;
extern "C" {
...
Obj mutex3;
bdlmtt::Barrier barrier3(NUM_THREADS3);
...
extern "C" {
    void *timesTest(void *arg)
    {
        for (int i = 0; i < NUM_ACQUIRE; ++i) {
            ...
            mutex3.lock();
            bdlmtt::ThreadUtil::microSleep(SLEEP_TIME3);
            mutex3.unlock();
        }
        return NULL;
    }
...
extern "C" {
    void *mutexTest(void *arg)
    {
        ...
        ASSERT(state == VALID);
        state = INVALID;
        bdlmtt::ThreadUtil::microSleep(SLEEP_TIME2);
        state = VALID;
        mutex2.unlock();
        return NULL;
    }
