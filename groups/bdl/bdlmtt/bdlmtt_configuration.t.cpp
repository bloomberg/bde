...
//                              OVERVIEW
//                              --------
// This program tests the functionality of the 'bdlmtt::Configuration' class.
//
// Note that since this component is below 'bdlmtt::ThreadUtil', we cannot
// actually create any threads and verify stack sizes, so some testing of this
// component is done in 'bdlmtt_threadutil.t.cpp'.
//
//-----------------------------------------------------------------------------
// [1] Breathing Test
...
typedef bdlmtt::Configuration Obj;
    ...
    switch (test) { case 0:
      case 4: {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            ASSERT(bdlmtt::ThreadAttributes::BCEMT_UNSET_STACK_SIZE ==
      ...
      case 3: {
        ...
        const int nativeDefault =
                           bdlmtt::Configuration::nativeDefaultThreadStackSize();
        ASSERT(nativeDefault > 0);

        // Then, we verify that 'defaultThreadStackSize' is unset.

        ASSERT(bdlmtt::ThreadAttributes::BCEMT_UNSET_STACK_SIZE ==
                                bdlmtt::Configuration::defaultThreadStackSize());
        ...
        bdlmtt::Configuration::setDefaultThreadStackSize(newDefaultStackSize);

        // Finally, we verify that the default thread stack size has been set
        // to the value we specified:

        ASSERT(bdlmtt::Configuration::defaultThreadStackSize() ==
                                                          newDefaultStackSize);
        ASSERT(bdlmtt::Configuration::defaultThreadStackSize() != nativeDefault);
      }  break;
      case 2: {
        // --------------------------------------------------------------------
