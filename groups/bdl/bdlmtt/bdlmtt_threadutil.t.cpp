...
typedef bdlmtt::ThreadUtil       Obj;
typedef bdlmtt::ThreadAttributes Attr;
...
bsl::ostream& operator<<(bsl::ostream&                            stream,
                         bdlmtt::ThreadAttributes::SchedulingPolicy policy)
{
    switch (policy) {
#undef CASE
#define CASE(x) case bdlmtt::ThreadAttributes::x: stream << #x; break
    ...
    // and perform some work
{
    ...
    bdlmtt::ThreadUtil::create(&handle,
                             attributes,
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      case 15: {
        ...
        bdlmtt::ThreadUtil::Handle handles[2];
        int rc;
...
# error unrecognized platform
      ...
      case 10: {
        ...
        //
        // Concern:
        //   That bdlmtt::Configuration can really affect stack size.
        //
        // Plan:
        ...
        const int stackSize =
                       5 * bdlmtt::Configuration::nativeDefaultThreadStackSize();
        const int stackToUse =
                       4 * bdlmtt::Configuration::nativeDefaultThreadStackSize();
        bdlmtt::Configuration::setDefaultThreadStackSize(stackSize);
        ...
        if (verbose) Q(Test functor with no attributes);
        {
            ...
            ASSERT(func.d_stackToUse >
                      3 * bdlmtt::Configuration::nativeDefaultThreadStackSize());

            int rc = bdlmtt::ThreadUtil::create(&handle, func);
            ASSERT(0 == rc);
        ...
        if (verbose) Q(Test functor with default attributes);
        {
            ...
            bdlmtt::ThreadAttributes attr;
            int rc = bdlmtt::ThreadUtil::create(&handle, attr, func);
            ASSERT(0 == rc);
      ...
      case 7: {
        ...
        bdlmtt::ThreadUtil::Handle handle;
        bdlmtt::ThreadUtil::create(&handle, TC::TlsKeyTestFunctor(10));
        bdlmtt::ThreadUtil::join(handle);

        bdlmtt::ThreadUtil::create(&handle, TC::TlsKeyTestFunctor(20));
        bdlmtt::ThreadUtil::join(handle);

        bdlmtt::ThreadUtil::create(&handle, TC::TlsKeyTestFunctor(30));
        bdlmtt::ThreadUtil::join(handle);
      ...
      case 6: {
        ...
        bdlmtt::ThreadUtil::Handle handle;
        rc = bdlmtt::ThreadUtil::create(&handle, TC::CreateKeyTestFunctor(true));
        ASSERT(0 == rc);
        bdlmtt::ThreadUtil::join(handle);

#if defined(BDLMTT_PLATFORM_POSIX_THREADS) && !defined(BSLS_PLATFORM_OS_CYGWIN)
        ...
        bdlmtt::ThreadUtil::create(&handle, TC::CreateKeyTestFunctor(false));
        bdlmtt::ThreadUtil::join(handle);
    ...
    case 3: {
        ...
        // previously in the 'bdlmtt_xxxthread' component) compile and work
        // properly.
        // --------------------------------------------------------------------
    ...
    case 2: {
        ...
        bdlmtt::ThreadUtil::Handle validH1, validH2;
        bdlmtt::ThreadUtil::create(&validH1, myThreadFunction, 0);
        bdlmtt::ThreadUtil::create(&validH2, myThreadFunction, 0);
        bdlmtt::ThreadUtil::Handle validH1copy = validH1;

        ASSERT(1 == bdlmtt::ThreadUtil::isEqual(validH1, validH1));
        ASSERT(1 == bdlmtt::ThreadUtil::isEqual(validH1, validH1copy));
        ASSERT(0 == bdlmtt::ThreadUtil::isEqual(validH1, validH2));
        ASSERT(0 == bdlmtt::ThreadUtil::isEqual(
                                             bdlmtt::ThreadUtil::invalidHandle(),
                                             validH2));
        ASSERT(0 == bdlmtt::ThreadUtil::isEqual(
                                             bdlmtt::ThreadUtil::invalidHandle(),
                                             validH1copy));
        ASSERT(0 == bdlmtt::ThreadUtil::isEqual(validH1,
                                           bdlmtt::ThreadUtil::invalidHandle()));
        ASSERT(1 == bdlmtt::ThreadUtil::isEqual(
                                           bdlmtt::ThreadUtil::invalidHandle(),
                                           bdlmtt::ThreadUtil::invalidHandle()));

        bdlmtt::ThreadUtil::join(validH1);
        bdlmtt::ThreadUtil::join(validH2);
    }  break;
    case 1: {
        // --------------------------------------------------------------------
       ...
       bdlmtt::ThreadUtil::Handle handles[THREAD_COUNT], dummy;
       ...
       while ((THREAD_COUNT != joinableChecker.count() ||
              ...
              0 < --iterations)
       {
           bdlmtt::ThreadUtil::microSleep(100 * 1000);  // 100 msec
           bdlmtt::ThreadUtil::yield();
       }
      ...
      case -1: {
        ...
        if (defaultSizeString) {
            ...
            P(bdlmtt::Configuration::defaultThreadStackSize());
        }

        bdlmtt::ThreadUtil::Handle handle;
        if (stackSizeString) {
            bdlmtt::ThreadUtil::create(&handle, attr, &testCaseMinus1ThreadMain,
                                     0);
        }
        else {
            bdlmtt::ThreadUtil::create(&handle, &testCaseMinus1ThreadMain, 0);
        }
      ...
      case -2: {
        ...
        bdlmtt::ThreadUtil::Handle handle;
        int rc = bdlmtt::ThreadUtil::create(&handle, attr, &clearanceTest, 0);
        ASSERT(0 == rc);
        rc = bdlmtt::ThreadUtil::join(handle);
        ASSERT(0 == rc);
        ...
        for (clearanceTestAllocaSize = clearanceTestStackSize / 2;;
            ...
            clearanceTestState = CLEARANCE_TEST_START;
            rc = bdlmtt::ThreadUtil::create(&handle, attr, &clearanceTest, 0);
            ASSERT(0 == rc);
            rc = bdlmtt::ThreadUtil::join(handle);
            ASSERT(0 == rc);
      ...
      case -4: {
        ...
        bdlmtt::ThreadUtil::Handle handle;
        int rc = bdlmtt::ThreadUtil::create(&handle,
                                          attr,
                                          &secondClearanceTest,
                                          (void *) stackSize);
        ASSERT(0 == rc);
