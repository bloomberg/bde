...
//                                Overview
//                                --------
// The basic idea of testing bdlmtt::QLock and bdlmtt::QLock is to create
...
class MyTask {
  ...
  private:
    ...
    bdlmtt::ThreadAttributes    d_attr;
    TestFunc                  d_f;
    void                     *d_arg;
...
, d_barrier(0)
{
    d_attr.setDetachedState(bdlmtt::ThreadAttributes::BCEMT_CREATE_JOINABLE);
    d_attr.setStackSize (1024*128);

    for (int i=0; i < MAX_THREADS; ++i) {
        d_handles[i] = bdlmtt::ThreadUtil::invalidHandle();
    }

    if (veryVerbose) {
        bdlmtt::LockGuard<bdlmtt::Mutex> lg(&d_logMutex);
...
int MyTask::start(int numThreads)
{
    ...
    ASSERT(d_barrier == 0);
    d_barrier = new  bdlmtt::Barrier(numThreads);

    for (int i=0; i < MAX_THREADS && numThreads > 0; ++i)
    {
        if (bdlmtt::ThreadUtil::isEqual(d_handles[i],
                                      bdlmtt::ThreadUtil::invalidHandle())) {

            int rc = bdlmtt::ThreadUtil::create(&d_handles[i],
                                              d_attr,
                                              threadFunc,
                                              this);
            --numThreads;
            ...
            ASSERT(rc == 0);
            ASSERT(!bdlmtt::ThreadUtil::isEqual(
                                           d_handles[i],
                                           bdlmtt::ThreadUtil::invalidHandle()));
        }
    }
...
int MyTask::stop()
{
    for (int i=0; i < MAX_THREADS; ++i) {
        ...
        if (!bdlmtt::ThreadUtil::isEqual(d_handles[i],
            ...
            int rc = bdlmtt::ThreadUtil::join (d_handles[i], 0);
            ASSERT(rc == 0);
            d_handles[i] = bdlmtt::ThreadUtil::invalidHandle();
        }
    }
...
struct DataCase7 {
    bdlmtt::Barrier    *d_barrier;  // common barrier
...
void *testCase7(int threadNum, const MyTask& task)
{
     ...
     while(!flgExit) {
        ...
        // Lock own context
        bdlmtt::QLockGuard guard(data->d_myContext->d_qlock);
...
void *testCase6(int threadNum, const MyTask& task)
{
     ...
     {
        bdlmtt::QLockGuard guard(data->d_qlock);
...

     {
         ...
         //bdlmtt::ThreadUtil::microSleep(1000*250);
...
struct CaseData5 {
    int           d_numIter;
    int           d_numElements;
    bdlmtt::QLock  *d_qlocks;
    bdlmtt::Mutex  *d_mutexes;
    int          *d_slots;
};
...
void *testCase5_fn1(int threadNum, const MyTask& task)
{
    ...
    for (int i=0; i < data->d_numIter; ++i) {
        ...
        bdlmtt::QLockGuard guard(&data->d_qlocks[slotIndex]);
...
void *testCase5_fn2(int threadNum, const MyTask& task)
{
    ...
    for (int i=0; i < data->d_numIter; ++i) {
        ...
        bdlmtt::LockGuard<bdlmtt::Mutex> guard(&data->d_mutexes[slotIndex]);
...
void *testCase3a(int threadNum, const MyTask& task)
{
     ...
     for (int i=0; i < data->d_numIter; ++i) {
         ...
         // Critical region
         // set mutex-qlock and lock
         bdlmtt::LockGuard<bdlmtt::Mutex> guard(data->d_mutex);
...
// ----------------------------------------------------------------------------
static bdlmtt::QLock qMutex1  = BDLMTT_QLOCK_INITIALIZER;
void *testCase1(int threadNum, const MyTask& task)
{
    ...
    // Critical region
    {
        bdlmtt::QLockGuard qlock(&qMutex1);
        ...
        bdlmtt::ThreadUtil::microSleep(1000*10);
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      case 8: {
        // --------------------------------------------------------------------
        // Testing bdlmtt::QLock::initialize and bdlmtt::QLock::isLocked
        // --------------------------------------------------------------------
      ...
      case 7: {
        ...
        bdlmtt::QLock qlock1 = BDLMTT_QLOCK_INITIALIZER;
        bdlmtt::QLock qlock2 = BDLMTT_QLOCK_INITIALIZER;
        ...
        for (int i=0; i < 10; ++i) {
            ...
            bdlmtt::QLockGuard guard1(&qlock1, false);
            ASSERT(guard1.tryLock() == 0);

            bdlmtt::QLockGuard guard2(&qlock2, false);
      ...
      case 6: {
        ...
        bdlmtt::QLock qlock = BDLMTT_QLOCK_INITIALIZER;
        DataCase6   data;
        data.d_qlock = &qlock;
        data.d_slots.clear();
        ...
        for (int i=0; i < 10; ++i) {
            ...
            bdlmtt::QLockGuard guard(data.d_qlock, false);
      ...
      case 5: {
...

        {
            ...
            for (int i=0; i < MAX_THREADS; ++i) {
                ...
                data.d_mutexes = 0;
                data.d_qlocks = new bdlmtt::QLock [MAX_SLOTS];
...

       {
            ...
            for (int i=0; i < MAX_THREADS; ++i) {
                ...
                data.d_qlocks = 0;
                data.d_mutexes = new bdlmtt::Mutex [MAX_SLOTS];
                data.d_slots = new int [MAX_SLOTS];
      ...
      case 3: {
       ...
       // repeat the same test with bdlmtt_mutex
       {
            bsls::Stopwatch sw;
      ...
      case 2: {
        ...
        bdlmtt::ThreadUtil::create(&handle, testCase2, 0);

        const bsl::string& s1 = helloString();
        void              *s2 = 0;

        bdlmtt::ThreadUtil::join(handle, &s2);
      ...
      case 1: {
        ...
        for (int i=0; i < 10; ++i) {
            ...
            bdlmtt::QLockGuard guard(&qMutex1, false);
            ASSERT(guard.tryLock() == 0);
        }
