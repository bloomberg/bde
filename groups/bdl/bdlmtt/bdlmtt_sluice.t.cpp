...
void enterPostSleepAndWait(Obj *sluice, bdlmtt::Semaphore *sem)
{
    const void *token = sluice->enter();

    sem->post();

    bdlmtt::ThreadUtil::sleep(bsls::TimeInterval(2));
    sluice->wait(token);
}
    ...
    switch (test) {
      ...
      case 2: {
        ...
        for (int i = 0; i < NUM_ITERATIONS; ++i) {
            bdlmtt::Semaphore readySem;

            bdlmtt::ThreadUtil::Handle h;
            int rc = bdlmtt::ThreadUtil::create(&h,
