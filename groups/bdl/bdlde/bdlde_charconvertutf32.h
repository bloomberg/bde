...
struct CharConvertUtf32 {
    ...
    static int utf32ToUtf8(bsl::string           *dstString,
        ...
        // specify 'errorByte' to be substituted (if not 0) for invalid
        ...
        // one or more invalid words were encountered in the input.  The
    ...
    static int utf32ToUtf8(char                  *dstBuffer,
        ...
        // written is to be loaded.  Optionally specify 'numBytesWritten',
        // which (if not 0) indicates the location of the modifiable variable
        // into which the number of bytes written (including the null
        // terminator, if any) is to be loaded.  Optionally specify 'errorByte'
        // to be substituted (if not 0) for invalid encodings in the input
        // string.  Invalid encodings are illegal Unicode values (in the range
        ...
        // assumed to be in host byte order.  Return 0 on success and a
        // bit-wise OR of the masks defined by 'CharConvertStatus::Enum'
        // otherwise, where 'CharConvertStatus::k_INVALID_INPUT_BIT' will be
        // set if one or more invalid words were encountered in the input, and
        // 'CharConvertStatus::BDEDE_OUT_OF_SPACE_BIT' will be set if the
