...
struct IntManipulator {
    int operator()(int *value) const
    {
        return intAccessor(*value);
    }
    template <class t_TYPE>
...
struct StringAccessor {
    int operator()(const bsl::string& ) const
    {
        return 0;
    }
    template <class t_TYPE>
...
struct StringManipulator {
    int stringManipulator(bsl::string *) const
    {
        return 0;
    }
    template <class t_TYPE>
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:
      case 1: {
        ...
        BSLMF_ASSERT(bdlar::IsSimple<int>::value);

        int n = 5;

        bdlar::SimpleTypeRef ref(n);

        ASSERT(ref.typeId() == bdlar::e_INT_ID);
        ASSERT(ref.access(intAccessor) == 0);
        ASSERT(ref.manipulate(intManipulator) == 0);

        ASSERT(ref.access(stringAccessor) != 0);
        ASSERT(ref.manipulate(stringManipulator) != 0);

        n = 5;
        {
            bdlar::SimpleTypeConstRef cref(ref);

            ASSERT(cref.typeId() == bdlar::e_INT_ID);
            cref.access(intAccessor);
        }

        {
            bdlar::SimpleTypeConstRef cref(const_cast<const int &>(n));

            ASSERT(cref.typeId() == bdlar::e_INT_ID);
            cref.access(intAccessor);
        }

        // Test `bsl::vector<char>`
        {
            BSLMF_ASSERT(bdlar::IsSimple<bsl::vector<char> >::value);
            bsl::vector<char> value;
            bdlar::SimpleTypeRef ref(value);
        }
      } break;
      default: {
