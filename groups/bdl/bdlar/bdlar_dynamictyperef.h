...
class DynamicTypeRef {
  ...
  public:
    ...
    /// undefined if the referred object is null.
    AnyConstRef valueConstRef() const;
};
...
inline
AnyRef DynamicTypeRef::valueRef() const
{
...
inline
AnyConstRef DynamicTypeRef::valueConstRef() const
{
                                               ...
                                               t_MANIPULATOR&   manipulator)
{
    ...
    if (any.typeCategory() == bdlat_TypeCategory::e_CUSTOMIZED_TYPE_CATEGORY) {
        CustomizedTypeRef ref(any.objectAddress(), any.customizedTypeVtable());
...
inline
AnyConstRef DynamicTypeConstRef::valueConstRef() const
{
    return d_vtable_p->d_valueRef_fp(d_object_p);
}
...
int bdlat_typeCategoryAccessArray(const DynamicTypeConstRef& ref,
                                  t_ACCESSOR&                accessor)
{
    AnyConstRef any = ref.valueConstRef();
    if (any.typeCategory() == bdlat_TypeCategory::e_ARRAY_CATEGORY) {
