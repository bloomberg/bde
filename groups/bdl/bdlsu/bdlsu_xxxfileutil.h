...
//
//@DEPRECATED: Use 'bdlsu_filesystemutil' instead.
//
//@CLASSES:
//  bdlsu::FileUtil: struct which scopes file system utilities
//
//@SEE_ALSO: bdlsu_pathutil, bdlsu_filesystemutil
//
...
///-----------------------------------
// The 'bdlsu::FileUtil::read' and 'bdlsu::FileUtil::write' methods add no
...
///Usage
///-----
// The following usage examples demonstrate how to use the various functions
// provided by 'bdlsu::FileUtil' to perform operations on files and filesystems.
//
...
// CLASS METHODS
inline
int FileUtil::createDirectories(const bsl::string& path,
...
inline
FileUtil::FileDescriptor FileUtil::open(
                                               const bsl::string& path,
                                               bool               writableFlag,
                                               bool               existFlag,
                                               bool               appendFlag)
...
inline
bool FileUtil::exists(const bsl::string& path)
{
    return exists(path.c_str());
}

inline
bool FileUtil::isRegularFile(const bsl::string& path, bool followLinks)
{
    return isRegularFile(path.c_str(), followLinks);
}

inline
bool FileUtil::isDirectory(const bsl::string& path, bool followLinks)
{
...
inline
int FileUtil::remove(const bsl::string& path, bool recursive)
{
    return remove(path.c_str(), recursive);
}

inline
int FileUtil::rollFileChain(const bsl::string& path, int maxSuffix)
{
    return rollFileChain(path.c_str(), maxSuffix);
}

inline
int FileUtil::move(const bsl::string& oldPath,
...
inline
int FileUtil::setWorkingDirectory(const bsl::string& path)
{
    return setWorkingDirectory(path.c_str());
}

inline
FileUtil::Offset FileUtil::getFileSize(const bsl::string& path)
{
    return getFileSize(path.c_str());
}

inline
FileUtil::Offset FileUtil::getAvailableSpace(
                                                       const bsl::string& path)
{
