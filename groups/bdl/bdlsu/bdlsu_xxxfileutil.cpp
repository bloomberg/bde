...
BSLS_IDENT_RCSID(bdlsu_xxxfileutil_cpp,"$Id$ $CSID$")
...
int FileUtil::tryLock(FileDescriptor fd, bool lockWrite)
{
    return FilesystemUtil::tryLock(fd, lockWrite);
}

int FileUtil::unlock(FileDescriptor fd)
{
    return FilesystemUtil::unlock(fd);
}
...
const bdlsu::FileUtil::FileDescriptor bdlsu::FileUtil::INVALID_FD =
                                                          INVALID_HANDLE_VALUE;

namespace bdlsu {
FileUtil::FileDescriptor
FileUtil::open(const char *pathName,
                     bool        writableFlag,
                     bool        existFlag,
                     bool        appendFlag)
{
...
int FileUtil::close(FileDescriptor fd)
{
    return CloseHandle(fd) ? 0 : -1;
}

int FileUtil::remove(const char *fileToRemove, bool recursive)
{
...
FileUtil::Offset
FileUtil::seek(FileDescriptor         fd,
                     FileUtil::Offset offset,
                     int                    whence)
{
    switch (whence) {
...
int FileUtil::read(FileDescriptor  fd,
                         void           *buf,
                         int             numBytesToRead)
{
    BSLS_ASSERT(buf);
...
int FileUtil::write(FileDescriptor  fd,
                          const void     *buf,
                          int             numBytesToWrite)
{
    BSLS_ASSERT(buf);
...
int FileUtil::map(FileDescriptor           fd,
                        void                   **addr,
                        FileUtil::Offset   offset,
                        int                      len,
                        int                      mode)
{
    ...
    if (mode == MemoryUtil::BDESU_ACCESS_NONE) {
        return -1;                                                    // RETURN
    }
    mode &= 7;
    ...
    FileUtil::Offset maxLength = offset + len;
...
int FileUtil::unmap(void *addr, int)
{
    BSLS_ASSERT(addr);
...
int FileUtil::sync(char *addr, int numBytes, bool)  // 3rd arg is sync
{
    BSLS_ASSERT(0 != addr);
...
int FileUtil::lock(FileDescriptor fd, bool lockWrite)
{
...
int FileUtil::move(const char *oldName, const char *newName)
{
    BSLS_ASSERT(oldName);
    BSLS_ASSERT(newName);
...
bool FileUtil::exists(const char *pathName)
{
    BSLS_ASSERT(pathName);
...
int FileUtil::getLastModificationTime(bdlt::Datetime *time,
                                            const char    *path)
{
    BSLS_ASSERT(time);
    BSLS_ASSERT(path);
                        ...
                        const bdlf::Function<void(*)(const char*)>&  visitor)
{
    ...
    if (bsl::string::npos != dirName.find_first_of("*?")) {
        ...
        do {
            ...
            else if (0 != PathUtil::appendIfValid(&dirNamePath,
                                                         findData.cFileName)) {
...
bool FileUtil::isRegularFile(const char *path, bool)
{
    BSLS_ASSERT(path);
...
bool FileUtil::isDirectory(const char *path, bool)
{
    BSLS_ASSERT(path);
...
FileUtil::Offset FileUtil::getAvailableSpace(const char *path)
{
    BSLS_ASSERT(path);
    ...
    return static_cast<FileUtil::Offset>(avail.QuadPart);
}

FileUtil::Offset FileUtil::getAvailableSpace(FileDescriptor fd)
{
    typedef struct {
        union {
            LONG Status;
...
FileUtil::Offset FileUtil::getFileSize(const char *path)
{
    BSLS_ASSERT(path);
    ...
    return (((FileUtil::Offset)fileAttribute.nFileSizeHigh) << 32)
           | fileAttribute.nFileSizeLow;
}

FileUtil::Offset FileUtil::getFileSizeLimit()
{
    // TBD
...
const bdlsu::FileUtil::FileDescriptor bdlsu::FileUtil::INVALID_FD = -1;

namespace bdlsu {
FileUtil::FileDescriptor
FileUtil::open(const char *path,
                     bool        writableFlag,
                     bool        existFlag,
                     bool        appendFlag)
{
...
int FileUtil::close(FileDescriptor descriptor)
{
    return ::close(descriptor);
}

FileUtil::Offset
FileUtil::seek(FileDescriptor fd, Offset offset, int whence)
{
    switch (whence) {
#if defined(BSLS_PLATFORM_OS_FREEBSD) || defined(BSLS_PLATFORM_OS_DARWIN) \
 || defined(BSLS_PLATFORM_OS_CYGWIN)
...
int FileUtil::remove(const char *path, bool recursive)
{
   BSLS_ASSERT(path);
...
int  FileUtil::unmap(void *addr, int size)
{
    BSLS_ASSERT(addr);
    BSLS_ASSERT(0 <= size);
...
int FileUtil::sync(char *addr, int numBytes, bool sync)
{
    BSLS_ASSERT(0 != addr);
    BSLS_ASSERT(0 <= numBytes);
    BSLS_ASSERT(0 == numBytes % MemoryUtil::pageSize());
...
int FileUtil::move(const char *oldPath, const char *newPath)
{
    BSLS_ASSERT(oldPath);
    BSLS_ASSERT(newPath);
...
bool FileUtil::isRegularFile(const char *path, bool followLinks)
{
    BSLS_ASSERT(path);
...
bool FileUtil::isDirectory(const char *path, bool followLinks)
{
    BSLS_ASSERT(path);
...
int FileUtil::createDirectories(const char *nativePath,
                                      bool        leafIsDirectory)
{
    ...
    if (!leafIsDirectory && PathUtil::hasLeaf(path)) {
        PathUtil::popLeaf(&path);
    }

    while (PathUtil::hasLeaf(path)) {
        directoryStack.push_back(bsl::string());
        int rc = PathUtil::getLeaf(&directoryStack.back(), path);
        BSLS_ASSERT(0 == rc);
        PathUtil::popLeaf(&path);
    }

    while (!directoryStack.empty()) {
        PathUtil::appendRaw(&path,
...
int FileUtil::getWorkingDirectory(bsl::string *path)
{
    BSLS_ASSERT(path);
...
int FileUtil::setWorkingDirectory(const char *path)
{
    BSLS_ASSERT(path);
...
void FileUtil::findMatchingPaths(bsl::vector<bsl::string> *result,
                                       const char               *pattern)
{
    BSLS_ASSERT(result);
    BSLS_ASSERT(pattern);
    ...
    visitPaths(pattern,
               bdlf::BindUtil::bind(&pushBackWrapper,
                                   result, bdlf::PlaceHolders::_1));
}

int FileUtil::grow(FileDescriptor         fd,
                         FileUtil::Offset size,
                         bool                   reserve,
...
int FileUtil::rollFileChain(const char *path, int maxSuffix)
{
    BSLS_ASSERT(path);
