...
// as "weekend" days, (2) have the same valid range, (3) have the same days
...
//
// CREATORS
// [ 2] Calendar(bslma::Allocator *basicAllocator = 0);
// [12] Calendar(const Date& firstDate, lastDate, ba = 0);
// [27] Calendar(const bdlt::PackedCalendar& packedCalendar, ba = 0);
// [ 7] Calendar(const Calendar& original, ba = 0);
// [ 2] ~Calendar();
//
// MANIPULATORS
// [ 9] Calendar& operator=(const Calendar& rhs);
// [ 9] Calendar& operator=(const PackedCalendar& rhs);
...
// [18] void intersectBusinessDays(const Calendar& calendar);
// [18] void intersectBusinessDays(const PackedCalendar& calendar);
// [18] void intersectNonBusinessDays(const Calendar& calendar);
...
// [28] int getNextBusinessDay(Date *nextBusinessDay, const Date& date);
// [28] int getNextBusinessDay(Date *nBD, const Date& date, int nth);
...
// [29] int numBusinessDays(beginDate, endDate) const;
...
// [ 4] const PackedCalendar& packedCalendar() const;
...
typedef bdlt::Calendar       Obj;
typedef bdlt::PackedCalendar PackedObj;
...
//..
      ...
      case 29: {
        ...
        if (verbose) cout << "\nNegative Testing." << endl;
        {
            ...
            ASSERT_SAFE_PASS(X.numBusinessDays(bdlt::Date(2015,  1,  1),
                                               bdlt::Date(2015,  1, 31)));
            ASSERT_SAFE_FAIL(X.numBusinessDays(bdlt::Date(2014, 12, 31),
                                               bdlt::Date(2015,  1, 31)));
            ASSERT_SAFE_FAIL(X.numBusinessDays(bdlt::Date(2015,  1,  1),
                                               bdlt::Date(2015,  2,  1)));

            ASSERT_SAFE_PASS(X.numBusinessDays(bdlt::Date(2015,  1, 10),
                                               bdlt::Date(2015,  1, 10)));
            ASSERT_SAFE_PASS(X.numBusinessDays(bdlt::Date(2015,  1, 10),
                                               bdlt::Date(2015,  1, 11)));
            ASSERT_SAFE_FAIL(X.numBusinessDays(bdlt::Date(2015,  1, 11),
                                               bdlt::Date(2015,  1, 10)));
        }
      } break;
      case 28: {
      ...
      case 27: {
        ...
        for (int ti = 0; SPECS[ti]; ++ti) {
            ...
            {
                PackedObj mZ;  const PackedObj& Z = gg(&mZ, SPEC);
            ...
            }
            {
                ...
                Obj        mX(Z, reinterpret_cast<bslma::TestAllocator *>(0));
                const Obj& X = mX;
      ...
      case 26: {
        ...
        typedef bdlt::Calendar::WeekendDaysTransitionConstReverseIterator
                                                                      Iterator;
...

        {
            ...
            ASSERT((bsl::is_same<IterTraits::value_type,
                         bdlt::Calendar::WeekendDaysTransition>::value));

            ASSERT((bsl::is_same<IterTraits::difference_type,
                                 bsl::ptrdiff_t>::value));

            ASSERT((bsl::is_same<IterTraits::pointer,
                 const bdlt::Calendar::WeekendDaysTransition *>::value));
      ...
      case 25: {
        ...
        typedef bdlt::Calendar::WeekendDaysTransitionConstIterator
                                                                      Iterator;
      ...
      case 24: {
        ...
        typedef bdlt::Calendar::BusinessDayConstReverseIterator Iterator;
      ...
      case 22: {
        ...
        typedef bdlt::Calendar::HolidayCodeConstReverseIterator Iterator;
            ...
            for (int offset = 0; offset < X.length(); ++offset) {
                ...
                if (X.isHoliday(date)) {
...

                    {
                        ...
                        bdlt::Calendar::HolidayConstIterator holidayIter
...

        {
            ...
            ASSERT_SAFE_FAIL_RAW(X.rendHolidayCodes(Y.beginHolidays()));
            ...
            ASSERT_SAFE_FAIL_RAW(X.rbeginHolidayCodes(bdlt::Date(2014,
                                                                 1,
                                                                 16)));

            ASSERT_SAFE_PASS(X.rendHolidayCodes(bdlt::Date(2014, 1, 15)));
            ASSERT_SAFE_FAIL_RAW(X.rendHolidayCodes(bdlt::Date(2014, 1, 16)));
      ...
      case 21: {
...

        {
            ...
            ASSERT_SAFE_FAIL_RAW(X.endHolidayCodes(Y.beginHolidays()));
            ...
            ASSERT_FAIL_RAW(X.rbeginHolidayCodes(bdlt::Date(2014, 1, 16)));

            ASSERT_PASS(X.rendHolidayCodes(bdlt::Date(2014, 1, 15)));
            ASSERT_FAIL_RAW(X.rendHolidayCodes(bdlt::Date(2014, 1, 16)));
      ...
      case 20: {
...

        {
            ...
            ASSERT_SAFE_FAIL_RAW(X.rbeginHolidays(bdlt::Date(2014, 3, 6)));

            ASSERT_SAFE_PASS(X.rendHolidays(bdlt::Date(2014, 1, 6)));
            ASSERT_SAFE_FAIL_RAW(X.rendHolidays(bdlt::Date(2014, 3, 6)));
        }
      } break;
      case 19: {
        // -------------------------------------------------------------------
...

        {
            ...
            ASSERT_SAFE_FAIL_RAW(X.beginHolidays(bdlt::Date(2014, 3, 6)));

            ASSERT_SAFE_PASS(X.endHolidays(bdlt::Date(2014, 1, 6)));
            ASSERT_SAFE_FAIL_RAW(X.endHolidays(bdlt::Date(2014, 3, 6)));
        }
      } break;
      case 18: {
        // -------------------------------------------------------------------
        ...
        //   void intersectNonBusinessDays(const Calendar& calendar);
        //   void intersectNonBusinessDays(const PackedCalendar& calendar);
        //   void unionBusinessDays(const Calendar& calendar);
        //   void unionBusinessDays(const PackedCalendar& calendar);
        //   void unionNonBusinessDays(const Calendar& calendar);
        ...
        if (verbose) cout << "\nTesting 'intersectBusinessDays'." << endl;
        {
            ...
            for (int i = 0; i < NUM_DATA; ++i) {
                ...
                PackedObj mPX;   const PackedObj PX  = gg(&mPX, SPEC1);

                PackedObj mPY;   const PackedObj PY  = gg(&mPY, SPEC2);
        ...
        if (verbose) cout << "\nTesting 'intersectNonBusinessDays'." << endl;
        {
            ...
            for (int i = 0; i < NUM_DATA; ++i) {
                ...
                BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator) {
                    Obj mZ(X, &testAllocator);  const Obj& Z = mZ;
                    mZ.intersectNonBusinessDays(PY);
                    ASSERTV(LINE, EXP, Z, EXP == Z);
                } BSLMA_TESTALLOCATOR_EXCEPTION_TEST_END
                ...
                BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator) {
                    Obj mZ(Y, &testAllocator);  const Obj& Z = mZ;
                    mZ.intersectNonBusinessDays(PX);
                    ASSERTV(LINE, EXP, Z, EXP == Z);
                } BSLMA_TESTALLOCATOR_EXCEPTION_TEST_END
      ...
      case 17: {
        ...
        Obj mZ;  const Obj& Z = gg(&mZ, "@2012/01/01 120 0");
      ...
      case 15: {
        ...
        static const struct {
            ...
            { L_,     "@2012/1/15 5 3A", 2012,  1,  1, 2012,  1, 31,
                                                          "@2012/1/1 30 17A" },
            { L_,     "@2012/1/15 5 3A", 2012,  1,  1, 2015,  1, 31,
                                          "@2012/1/1 @2015/1/31 @2012/1/18A" },
      ...
      case 14: {
        ...
        if (verbose) cout << "\nNegative Testing." << endl;
        {
...

            {
                ...
                ASSERT_SAFE_FAIL_RAW(mX.addWeekendDays(wd));
            }
        }
      } break;
      case 13: {
      ...
      case 12: {
...

        {
            ...
            ASSERT_FAIL_RAW(Obj(bdlt::Date(2012, 1, 5),
                                bdlt::Date(2012, 1, 4)));
        }
      } break;
      case 11: {
      ...
      case 9: {
        ...
        for (int ti = 0; SPECS[ti]; ++ti) {
            ...
            PackedObj mPZZ(&scratch);  const PackedObj& PZZ = gg(&mPZZ, SPEC1);
            ...
            for (int tj = 0; SPECS[tj]; ++tj) {
...

                {
                    ...
                    LOOP4_ASSERT(ti, tj, X, PZ,
                                 (X.packedCalendar() == PZ) == (ti == tj));
                    ...
                    LOOP4_ASSERT(ti, tj,   X, PZ, X.packedCalendar() == PZ);
                    LOOP4_ASSERT(ti, tj, PZZ, PZ, PZZ == PZ);

                    LOOP4_ASSERT(ti, tj, &oa, X.allocator(),
                                 &oa == X.allocator());
                    LOOP4_ASSERT(ti, tj, &scratch, PZ.allocator(),
                                 &scratch == PZ.allocator());
      ...
      case 8: {
                                                                       ...
                                                                       << endl;
        {
            ...
            const Obj XX(X, &scratch);
                  ...
                  Obj mY(&oa);  const Obj& Y = gg(&mY, "@2000/1/1 100");
            const Obj YY(Y, &scratch);
...

        {
            ...
            for (int ti = 0; SPECS[ti]; ++ti) {
                ...
                for (char cfg = 'a'; cfg <= 'c'; ++cfg) {
                    ...
                    // Verify memory is always allocated.
      ...
      case 5: {
        ...
        //   ostream& operator<<(ostream&, const Calendar&);
        // --------------------------------------------------------------------
      ...
      case 4: {
        ...
        if (verbose) cout << "\nTesting 'packedCalendar'." << endl;
        {
            ...
            for (int ti = 0; SPECS[ti]; ++ti) {
                ...
                Obj       mX(&sa1);  const Obj& X = gg(&mX, SPEC);

                Obj       mY(&sa2);  const Obj& Y = gg(&mY, SPEC);

                PackedObj mEXP;      const PackedObj& EXP = gg(&mEXP, SPEC);

                LOOP_ASSERT(ti, X.packedCalendar()             == EXP);
                LOOP_ASSERT(ti, X.packedCalendar().allocator() == &sa1);

                LOOP_ASSERT(ti, Y.packedCalendar()             == EXP);
                LOOP_ASSERT(ti, Y.packedCalendar().allocator() == &sa2);
            }
        }
        ...
        if (verbose) cout << "\nTesting weekend methods." << endl;
        {
            ...
            ASSERT(bdlt::Calendar::WeekendDaysTransition(
                                                      bdlt::Date(1, 1, 1), wAU)
        ...
        if (verbose) cout << "\nNegative Testing." << endl;
        {
            ...
            ASSERT_SAFE_FAIL_RAW(X.holiday(-1));
            ASSERT_SAFE_PASS(X.holiday( 0));
            ASSERT_SAFE_PASS(X.holiday( 1));
            ASSERT_SAFE_FAIL_RAW(X.holiday( 2));

            ASSERT_SAFE_PASS(X.holidayCode(bdlt::Date(2012, 1, 6),  0));
            ASSERT_SAFE_FAIL_RAW(X.holidayCode(bdlt::Date( 100, 1, 6),  0));
            ASSERT_SAFE_FAIL_RAW(X.holidayCode(bdlt::Date(2012, 1, 5),  0));
            ASSERT_SAFE_FAIL_RAW(X.holidayCode(bdlt::Date(2012, 1, 6), -1));
            ASSERT_SAFE_FAIL_RAW(X.holidayCode(bdlt::Date(2012, 1, 6),  1));

            ASSERT_SAFE_PASS(X.isHoliday(bdlt::Date(2012, 1, 6)));
            ASSERT_SAFE_FAIL_RAW(X.isHoliday(bdlt::Date( 100, 1, 6)));
      ...
      case 3: {
        ...
        //   int ggg(CALENDAR *obj, const char *spec, bool vF);
        // --------------------------------------------------------------------
...
//----------v
        ...
        for (int ti = 0; ti < NUM_DATA; ++ti) {
            ...
            {
                PackedObj mY;  const PackedObj& Y = mY;
                LOOP_ASSERT(LINE, ggg(&mY, INPUT, false) == retCode);
                LOOP_ASSERT(LINE, X.packedCalendar() == Y);
            }
            ...
            if (retCode == -1) {
                ...
                {
                    PackedObj mZ;  const PackedObj& Z = gg(&mZ, INPUT);
                    LOOP_ASSERT(LINE, R.packedCalendar() == Z);
                }
        ...
        }
        {
...

            {
                ...
                {
                    PackedObj mY;  const PackedObj& Y = mY;
                    ASSERT(ggg(&mY, spec, false) == retCode);
                    ASSERT(X.packedCalendar() == Y);
                }
      ...
      case 2: {
        ...
        }
        {
            ...
            bsls::Types::Int64 suppliedAllocations = sa.numAllocations();
        ...
        if (verbose) cout << "\nNegative Testing." << endl;
        {
...

            {
                ...
                ASSERT_SAFE_FAIL_RAW(mX.addWeekendDay(bdlt::DayOfWeek::e_SUN));
            }
        }
      } break;
      case 1: {
