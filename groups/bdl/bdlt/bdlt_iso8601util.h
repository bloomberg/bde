...
struct Iso8601Util {
    // This 'struct' provides a namespace for ISO 8601 date/time conversion
    // functions.

    // TYPES
    enum {
        // Fixed lengths for ISO 8601-formatted date and time values:
        k_DATE_STRLEN       = 10,
        k_DATETZ_STRLEN     = 16,
    ...
    static int generate(char            *buffer,
        ...
        // buffer of size 'k_MAX_STRLEN + 1' is large enough to hold any
    ...
    static int generate(char              *buffer,
        ...
        // process-wide default value
        // 'Iso8601UtilConfiguration::useZAbbreviationForUtc' is used.  If
        // 'bufferLength' is larger than necessary to contain the string
        // representation of 'object', then a null terminator is appended to
        // the output.  The behavior is undefined unless '0 <= bufferLength'.
        // Note that, for each type of 'object', the return value is always the
    ...
    static int generateRaw(char              *buffer,
        ...
        // otherwise).  If 'useZAbbreviationForUtc' is not supplied, the
        // process-wide default value
        // 'Iso8601UtilConfiguration::useZAbbreviationForUtc' is used.  The
        // behavior is undefined unless 'buffer' holds enough characters.  Note
        // that, for each type of 'object', the return value is always the same
        // (as enumerated in the '_STRLEN' constants).  Also note that a buffer
        // of size 'k_MAX_STRLEN + 1' is large enough to hold any formatted
        // string, including any null terminator.  It is unspecified whether
        // the 'buffer' is null terminated on output or not.
