...
#undef U_TEST_FORMAT_ARG
      ...
      case 16: {
        ...
        {
            for (int i = 0; i < k_NUM_DDATA; ++i) {
                const int U_LINE   = DDATA[i].d_line;
                const int U_YEAR   = DDATA[i].d_year;
                const int U_MONTH  = DDATA[i].d_month;
                const int U_DAY    = DDATA[i].d_day;
                const int U_OFFSET = DDATA[i].d_offset;
                ...
                for (int j = 0; j < k_NUM_DDATA; ++j) {
                    const int V_LINE   = DDATA[j].d_line;
                    const int V_YEAR   = DDATA[j].d_year;
                    const int V_MONTH  = DDATA[j].d_month;
                    const int V_DAY    = DDATA[j].d_day;
                    const int V_OFFSET = DDATA[j].d_offset;
      ...
      case 6: {
        ...
        //
        // 10. The inequality operator's signature and return type are
        //     standard.
        //
        // Plan:
      ...
      case 5: {
        ...
        {
            typedef bsl::ostream& (Obj::*funcPtr)(bsl::ostream&, int, int)
                                                                         const;
