...
// [17] int validateAndSetTimeTz(const Time& localTime, int offset);
// [16] Time gmtTime() const;
#endif // BDE_OPENSOURCE_PUBLICATION -- pending deprecation
// ----------------------------------------------------------------------------
// [ 1] BREATHING TEST
// [19] bsl::string  bsl::format( "...", datetime);
// [19] bsl::wstring bsl::format(L"...", datetime);
// [20] USAGE EXAMPLE
...
#undef U_TEST_FORMAT_ARG
      ...
      case 17: {
        ...
        if (verbose) cout << "\nUse a table of distinct inputs." << endl;
        {
            ...
            for (int di = 0; di < NUM_DATA; ++di) {
                ...
                bdlt::TimeTz timeTz;
                ASSERTV(LINE,
                       IS_VALID == !timeTz.validateAndSetTimeTz(TIME, OFFSET));
      ...
      case 16: {
        ...
        {
            Obj mX(bdlt::Time(12, 0, 0, 0), 1);  const Obj& X = mX;

            ASSERTV(X.utcTime()   == X.gmtTime());
            ASSERTV(X.localTime() != X.gmtTime());
        }
      ...
      case 10: {
        ...
        for (int versionIndex = 0;
            ...
            }
            {
                ...
                if (1 == VERSION) {
                    ...
                    bdlt::Time t = T.localTime();
                    if (24 != t.hour()) {
                        t.setMicrosecond(X.localTime().microsecond());
                    }
                    mT = Obj(t, T.offset());
                }
                    ...
                    BSLX_TESTINSTREAM_EXCEPTION_TEST_BEGIN(in) {
                        ...
                        if (i < LOD1) {
                            ...
                            if (1 == VERSION) {
                                ...
                                bdlt::Time t = T1.localTime();
                                if (24 != t.hour()) {
                                    t.setMicrosecond(
                                                 X1.localTime().microsecond());
                                }
                                mT1 = Obj(t, T1.offset());
                            }
                            LOOP_ASSERT(i, &in == &rvIn1);
                            ...
                            if (1 == VERSION) {
                                ...
                                bdlt::Time t = T2.localTime();
                                if (24 != t.hour()) {
                                    t.setMicrosecond(
                                                 X2.localTime().microsecond());
                                }
                                mT2 = Obj(t, T2.offset());
                            }
                            LOOP_ASSERT(i, &in == &rvIn2);
        ...
        }
        {
            ...
            bdexStreamOut(out, bdlt::Time(0), 2);
            out.putInt32(SERIAL_Y);
      ...
      case 5: {
                             ...
                             "manipulators and left alignment." << endl;
        {
            ...
            for (int ti = 0; ti < NUM_DATA;  ++ti) {
                ...
                bsl::ostringstream output;
                output << bsl::left << bsl::setfill(FILL_CHAR) <<
                                                       bsl::setw(FORMAT_WIDTH);
                ASSERTV(LINE, &output == &X.print(output, IND, SPL));
                             ...
                             "manipulators and right alignment." << endl;
        {
            ...
            for (int ti = 0; ti < NUM_DATA;  ++ti) {
                ...
                bsl::ostringstream output;
                output << bsl::right << bsl::setfill(FILL_CHAR) <<
                                                       bsl::setw(FORMAT_WIDTH);
                ASSERTV(LINE, &output == &X.print(output, IND, SPL));
        ...
        if (verbose) cout << "\nTesting `operator<<`" << endl;
        {
            ...
            for (int ti = 0; ti < NUM_DATA;  ++ti) {
                ...
                // Left fill
                {
                    ...
                    output
                        << bsl::right << bsl::setfill(FILL_CHAR) <<
                                                       bsl::setw(FORMAT_WIDTH);
                ...
                // Right fill
                {
                    ...
                    output
                        << bsl::left << bsl::setfill(FILL_CHAR) <<
                                                       bsl::setw(FORMAT_WIDTH);
                    ASSERTV(LINE, &output == &(output << X));
