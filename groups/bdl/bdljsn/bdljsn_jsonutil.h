...
//   - `e_ONELINE`: render a human readable single-line format (e.g., for
//     logging)
//   - `e_PRETTY`: render a multi-line human readable format
// * `spacesPerLevel`: for the `e_PRETTY` style, the number of spaces added
//   for each additional nesting level (default 4)
// * `initialIndentationLevel`: for the `e_PRETTY` style, the number of sets
...
//                      resultString.find("\"\\/moon\\/sea-of-tranquility\""));
//  assert(bsl::string::npos ==
//                          resultString.find("\"/moon/sea-of-tranquility\""));
// ```
//
