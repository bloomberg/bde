...
/// non-`const` reference to the `stream`.  Note that this function is
/// frequently useful in debugging, but is not currently used in the
/// implementation.
...
bool NumberUtil::isIntegralNumber(const bsl::string_view& value)
{
    ...
    // `exponentBias` cannot be `INT64_MIN,` as that would require a string
    // with `INT64_MAX` characters).
