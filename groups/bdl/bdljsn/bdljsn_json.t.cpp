...
#undef TEST
      ...
      case 46: {
...
#ifdef BSLS_COMPILERFEATURES_SUPPORT_GENERALIZED_INITIALIZERS
        {
            ...
            ASSERT(2 == jo.size());
            ASSERT(jo["play"].isString());
            ASSERT(jo["chars"].isArray());

            jo.insert("KEY", 123);
            ASSERT(3 == jo.size());
            ASSERT(jo["KEY"].isNumber());
        }

#if defined(BSLS_LIBRARYFEATURES_HAS_CPP17_BASELINE_LIBRARY)
        // In general, there are several ways to initialize an object:
        //..
        //  Foo x(a, b, c);   -- a sequence of parameters inside parameters
        //  Foo x{a, b, c};   -- a sequence of parameters inside braces
        //  Foo x = y;
        //  Foo x = {a, b, c};
        //..
        //
        // 'Json'/'JsonObject'/'JsonArray' do not support all of these methods,
        // but I have put them all here in the test driver for completenes.
        // When they are not supported, then the tests are commented out,
        // ideally with a comment about why they are not supported.
        {
            using namespace std::literals::string_view_literals;

            bsl::string      s = "ABC";
            Json             j0;
            Json_Initializer ji("ABC");
        ...
        //  Json j6 ( "abc", 1L, false ); // Json does not have a sequence ctor
        //  Json j7 ( "abc", {1UL});      // Json does not have a sequence ctor
        //  Json j8 ( "abc", Json {1UL}); // Json does not have a sequence ctor

            Json j9 (s);
            ASSERT(j9.isString());
            ASSERT(s == j9.theString());
            }
            ...
            {
            // All of these constructors fail because the JSON constructors
            // are 'explicit'.
        //  Json j1 = 2;
        //  Json j2 = true;
        //  Json j3  = 2.4;
        ...
        //  JsonArray j1   (2, 3.5, false, "ABC"sv);
            ...
            ASSERT(4 == j2.size());
            ASSERT(j2[0].isNumber());
            ASSERT(j2[1].isNumber());
            ASSERT(j2[2].isBoolean());
            ASSERT(j2[3].isString());
            ASSERT(2     == j2[0]);
            ASSERT(3.5   == j2[1].asDouble());
            ASSERT(!        j2[2].theBoolean());
            ASSERT("ABC" == j2[3].theString());

            JsonArray j3 = {2, 3.5, false, "DEF"sv};
            ASSERT(4 == j3.size());
            ASSERT(j3[1].isNumber());
            ASSERT(j3[0].isNumber());
            ASSERT(j3[1].isNumber());
            ASSERT(j3[2].isBoolean());
            ASSERT(j3[3].isString());
            ASSERT(2     == j3[0]);
            ASSERT(3.5   == j3[1].asDouble());
            ASSERT(!        j3[2].theBoolean());
            ASSERT("DEF" == j3[3].theString());

            JsonArray j4  ({2, 3.5, false, "XYZ"sv});
            ASSERT(4 == j4.size());
            ASSERT(j4[2].isBoolean());
            ASSERT(j4[0].isNumber());
            ASSERT(j4[1].isNumber());
            ASSERT(j4[2].isBoolean());
            ASSERT(j4[3].isString());
            ASSERT(2     == j4[0]);
            ASSERT(3.5   == j4[1].asDouble());
            ASSERT(!        j4[2].theBoolean());
            ASSERT("XYZ" == j4[3].theString());
        ...
        //  JsonArray j6  (false);
            ...
            JsonArray j7  {false};
            ASSERT(1 == j7.size());
            ASSERT(j7[0].isBoolean());
            ASSERT(!j7[0].theBoolean());
        ...
        //  JsonArray j8 = false;
         ...
         // Nathan says we don't need to support this case. But we do anyway.
            ...
            ASSERT(1 == j1.size());
            ASSERT(j1.contains("ABC"sv));
            ASSERT(2.34 == j1["ABC"sv].asDouble());

            JsonObject j2   {"DEF"sv, 2.34};
            ASSERT(1 == j2.size());
            ASSERT(j2.contains("DEF"sv));
            ASSERT(2.34 == j2["DEF"sv].asDouble());

            JsonObject j3 = {"GHI"sv, 2.34};
            ASSERT(1 == j3.size());
            ASSERT(j3.contains("GHI"sv));
            ASSERT(2.34 == j3["GHI"sv].asDouble());

        //  Nathan says we don't need to support this case. But we do anyway.
            JsonObject j4  ({"ABC"sv, 2.34});
            ASSERT(1 == j4.size());
            ASSERT(j4.contains("ABC"sv));
            ASSERT(2.34 == j4["ABC"sv].asDouble());

            JsonObject j5  ({{"ABC"sv, 2.34}, {"DEF"sv, 5.67}});
            ASSERT(2 == j5.size());
            ASSERT(j5.contains("ABC"sv));
            ASSERT(2.34 == j5["ABC"sv].asDouble());
            ASSERT(j5.contains("DEF"sv));
            ASSERT(5.67 == j5["DEF"sv].asDouble());

            JsonObject j6 = {{"PDQ"sv, 2.34}, {"XYZ"sv, 5.67}};
            ASSERT(2 == j6.size());
            ASSERT(j6.contains("PDQ"sv));
            ASSERT(2.34 == j6["PDQ"sv].asDouble());
            ASSERT(j6.contains("XYZ"sv));
            ASSERT(5.67 == j6["XYZ"sv].asDouble());
            }
        ...
        //  Json j1   (2, 3.5, false, "ABC"sv);  // would use the sequence ctor
            ...
            Json j6 = JsonArray{2, 3.5, false, "ABC"sv};
            ASSERT(j6.isArray());    ASSERT(4 == j6.theArray().size());

            Json j7  (JsonObject{"ABC"sv, 2.34});
            ASSERT(j7.isObject());   ASSERT(1 == j7.theObject().size());

            Json j8 = JsonObject{"ABC"sv, 2.34};
            ASSERT(j8.isObject());   ASSERT(1 == j8.theObject().size());

        //  Json j9 ({ConvertibleTo<int>{}});       // Ambiguous
        //  Json jA ({1, ConvertibleTo<int>{}});    // two conversions
        //  Json jB[] = { ConvertibleTo<int>{} };   // two conversions
            }
            ...
            Json j9 (std::move(s));            // uses Json(STRING_TYPE &&)
...

        {
            ...
            j1.pushBack("ABC");
            ASSERT(1 == j1.size());
            ASSERT(j1[0].isString());
            ASSERT("ABC" == j1[0].theString());
      ...
      case 45: {
        ...
        Json_Initializer j0;
        ASSERT(j0.get_storage().is<JsonNull>());
        ASSERT(JsonNull() == j0.get_storage().the<JsonNull>());

        Json_Initializer j1(jNull);
        ASSERT(j1.get_storage().is<JsonNull>());
        ASSERT(JsonNull() == j1.get_storage().the<JsonNull>());

        Json_Initializer j2[] = { 2, 2L, 2LL };
        for (const auto& j: j2) {
            ASSERT(j.get_storage().is<long long>());
            ASSERT(2 == j.get_storage().the<long long>());
        }

        Json_Initializer j3[] = { 3U, 3UL, 3ULL };
        for (const auto& j: j3) {
            ASSERT(j.get_storage().is<unsigned long long>());
            ASSERT(3 == j.get_storage().the<unsigned long long>());
        }

        Json_Initializer j4(4.25f);
        ASSERT(j4.get_storage().is<double>());
        ASSERT(4.25 == j4.get_storage().the<double>());

        Json_Initializer j5(4.25);
        ASSERT(j5.get_storage().is<double>());
        ASSERT(4.25 == j5.get_storage().the<double>());

        Json_Initializer j6[] = { "ABC", abcString, bsl::string_view("ABC") };
        for (const auto& j: j6) {
            ASSERT(j.get_storage().is<bsl::string_view>());
            ASSERT("ABC" == j.get_storage().the<bsl::string_view>());
        }

        Json_Initializer j7 = { false, "DEF", 42 };
        ASSERT(j7.get_storage().is<std::initializer_list<Json_Initializer>>());

        Json_Initializer j8(jObj);
        ASSERT(j8.get_storage().is<const JsonObject *>());
        ASSERT(&jObj == j8.get_storage().the<const JsonObject *>());

        Json_Initializer j9(jArr);
        ASSERT(j9.get_storage().is<const JsonArray *>());
        ASSERT(&jArr == j9.get_storage().the<const JsonArray *>());

        Json_Initializer j10(jNum);
        ASSERT(j10.get_storage().is<const JsonNumber *>());
        ASSERT(&jNum == j10.get_storage().the<const JsonNumber *>());

        Json_Initializer j11(json);
        ASSERT(j11.get_storage().is<const Json *>());
        ASSERT(&json == j11.get_storage().the<const Json *>());
#endif
      } break;
      case 44: {
        // --------------------------------------------------------------------
      ...
      case 43: {
        ...
        //:   o Pass the value to the 'pushBack' method of a 'JsonArray'.
        //:   o Confirm that an ('Json') element has been added to the array.
        //:   o Compare the type and value of the added element to those of the
        //:     'pushBack' argument.
        //:
        ...
        //:
        //:   o Most of these  tests are encapsulated in function templates
        //:     'testEqualityOverloadsRef' and 'testEqualityOverloadsVal',
        //:     each having different initialization patterns.
        //:
        //:   o 'testEqualityOverloadsStr' handles the special case of
        //:     setting the default value of a 'const char *' to '""'.
        //:
        //:   o The tests of 'JsonNull' follow a significantly different
...
// ```
      ...
      case 26: {
...

        {
            ...
            //  the ephemeral JSONArray in the initializer_list uses the
            //  default allocator, but then that is deallocated and the
            //  supplied allocator is used
                ASSERTV(da2.numBlocksInUse(), 0 == da2.numBlocksInUse());
