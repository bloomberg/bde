...
// values that start from 0 and then increase sequentially from INT_MIN + 1.
//
...
//
// CLASS METHODS
// [ 3] ostream& print(ostream& s, Code val, int level = 0, int sPL = 4);
// [ 1] const char *toAscii(bdlaggxxx::ErrorCode::Code val);
//
// FREE OPERATORS
// [ 2] operator<<(ostream& s, bdlaggxxx::ErrorCode::Code val);
// ----------------------------------------------------------------------------
// [ 5] USAGE EXAMPLE
...
typedef bdlaggxxx::ErrorCode::Code Enum;
typedef bdlaggxxx::ErrorCode       Obj;

// ============================================================================
//                       GLOBAL CONSTANTS FOR TESTING
// ----------------------------------------------------------------------------

const int NUM_ENUMERATORS = 15;
...
// 'bdlaggxxx::ErrorCode' usage.
//
// First, we create a variable 'value' of type 'bdlaggxxx::ErrorCode::Code' and
// initialize it with the enumerator value
// 'bdlaggxxx::ErrorCode::BCEM_BAD_FIELDNAME':
//..
   bdlaggxxx::ErrorCode::Code value = bdlaggxxx::ErrorCode::BCEM_BAD_FIELDNAME;
//..
// Now, we store a pointer to its ASCII representation in a variable
// 'asciiValue' of type 'const char *':
//..
   const char *asciiValue = bdlaggxxx::ErrorCode::toAscii(value);
...
#undef NL
      ...
      case 2: {
        ...
        //
        // Testing:
        //   operator<<(ostream& s, bdlaggxxx::ErrorCode::Code val);
        // --------------------------------------------------------------------
        ...
        static const struct {
            ...
            //----    ----------------------          -----------------
            { L_,      Obj::BCEM_SUCCESS,           "SUCCESS"         },
            { L_,      Obj::BCEM_UNKNOWN_ERROR,     "UNKNOWN_ERROR"   },
            { L_,      Obj::BCEM_NOT_A_RECORD,      "NOT_A_RECORD"    },
            { L_,      Obj::BCEM_NOT_A_SEQUENCE,    "NOT_A_SEQUENCE"  },
            { L_,      Obj::BCEM_NOT_A_CHOICE,      "NOT_A_CHOICE"    },
            { L_,      Obj::BCEM_NOT_AN_ARRAY,      "NOT_AN_ARRAY"    },
            { L_,      Obj::BCEM_BAD_FIELDNAME,     "BAD_FIELDNAME"   },
            { L_,      Obj::BCEM_BAD_FIELDID,       "BAD_FIELDID"     },
            { L_,      Obj::BCEM_BAD_FIELDINDEX,    "BAD_FIELDINDEX"  },
            { L_,      Obj::BCEM_BAD_ARRAYINDEX,    "BAD_ARRAYINDEX"  },
            { L_,      Obj::BCEM_NOT_SELECTED,      "NOT_SELECTED"    },
            { L_,      Obj::BCEM_BAD_CONVERSION,    "BAD_CONVERSION"  },
            { L_,      Obj::BCEM_BAD_ENUMVALUE,     "BAD_ENUMVALUE"   },
            { L_,      Obj::BCEM_NON_CONFORMANT,    "NON_CONFORMANT"  },
            { L_,      Obj::BCEM_AMBIGUOUS_ANON,    "AMBIGUOUS_ANON"  },
      ...
      case 1: {
        ...
        // Testing:
        //   enum Enum { ... };
        //   const char *toAscii(bdlaggxxx::ErrorCode::Enum val);
        // -------------------------------------------------------------------
        ...
        ASSERT(0 == DATA[0].d_value);
        for (int ti = 1, value = INT_MIN + 1;
             ti < NUM_ENUMERATORS; ++ti, ++value) {
            const Enum VALUE = DATA[ti].d_value;

            if (veryVerbose) { T_; P_(ti); P(value); P(VALUE); }

            LOOP3_ASSERT(ti, value, VALUE, value == VALUE);
        }
