...
//
// CREATORS
// [ 8] bdlaggxxx::AggregateRaw();
// [18] bdlaggxxx::AggregateRaw(const bdlaggxxx::AggregateRaw& original);
// [ 8] ~bdlaggxxx::AggregateRaw();
//
// MANIPULATORS
// [10] bdlaggxxx::AggregateRaw& operator=(const bdlaggxxx::AggregateRaw& rhs);
// [11] void setDataType(bdlmxxx::ElemType::Type dataType);
// [11] void setData(void *data);
// [11] void setSchema(const bdlmxxx::Schema *schema);
// [11] void setRecordDef(const bdlmxxx::RecordDef *recordDef);
// [11] void setFieldDef(const bdlmxxx::FieldDef *fieldDef);
...
// [ 5] bdlt::Datetime asDatetime() const;
// [ 5] bdlt::DatetimeTz asDatetimeTz() const;
// [ 5] bdlt::Date asDate() const;
// [ 5] bdlt::DateTz asDateTz() const;
// [ 5] bdlt::Time asTime() const;
// [ 5] bdlt::TimeTz asTimeTz() const;
// [ 6] const bdlmxxx::ElemRef asElemRef() const;
...
// [ 4] bdlmxxx::ElemType::Type dataType() const;
// [11] const bdlmxxx::RecordDef& recordDef() const;
// [11] const bdlmxxx::RecordDef *recordConstraint() const;
// [  ] const bdlmxxx::EnumerationDef *enumerationConstraint() const;
// [ 4] const bdlmxxx::FieldDef *fieldDef() const;
// [11] const bdlmxxx::RecordDef *recordDefPtr() const;
// [11] const void *data() const;
// [11] const bdlmxxx::Schema *schema() const;
// [27] void swap(bdlaggxxx::AggregateRaw& other);
...
typedef bdlaggxxx::AggregateRaw   Obj;
typedef bdlaggxxx::ErrorAttributes Error;
typedef bdlaggxxx::ErrorCode      ErrorCode;
typedef bdlaggxxx::FieldSelector  FieldSelector;
...
void aSsErTAggError(const bdlaggxxx::AggregateRaw& agg,
                    const char *aggNm,
...
void *makeValuePtr(bdlmxxx::ElemType::Type type, TestAllocator *ta)
{
    void *data;
    switch (type) {
...
void destroyValuePtr(void *value, bdlmxxx::ElemType::Type type, TestAllocator *ta)
{
    switch (type) {
      case bdlmxxx::ElemType::BDEM_CHAR: {
...
int ggAggData(Obj *agg, const RecDef& record, bslma::Allocator *basicAllocator)
{
    ...
    EType::Type elemType = bdlmxxx::RecordDef::BDEM_CHOICE_RECORD ==
                                                            record.recordType()
    ...
    switch (elemType) {
      case bdlmxxx::ElemType::BDEM_LIST: {
        if (record.recordType() != bdlmxxx::RecordDef::BDEM_SEQUENCE_RECORD) {
            ASSERT(0);
            return -1;                                                // RETURN
        }
        bdlmxxx::List *list = new (*allocator) bdlmxxx::List(allocator);
      ...
      case bdlmxxx::ElemType::BDEM_TABLE: {
        ...
        bdlmxxx::Table *table = new (*allocator) bdlmxxx::Table(allocator);
        bdlmxxx::SchemaAggregateUtil::initTable(table, record);
        data = (void *) table;
      } break;
      case bdlmxxx::ElemType::BDEM_CHOICE: {
        if (record.recordType() != bdlmxxx::RecordDef::BDEM_CHOICE_RECORD) {
            ASSERT(0);
            return -1;                                                // RETURN
        }
        bdlmxxx::Choice *choice = new (*allocator) bdlmxxx::Choice(allocator);
      ...
      case bdlmxxx::ElemType::BDEM_CHOICE_ARRAY: {
        ...
        bdlmxxx::ChoiceArray *ca = new (*allocator) bdlmxxx::ChoiceArray(allocator);
        bdlmxxx::SchemaAggregateUtil::initChoiceArray(ca, record);
        data = (void *) ca;
      } break;
      default: {
        return -1;                                                    // RETURN
...
int destroyAggData(Obj *agg, bslma::Allocator *allocator)
{
    ...
    switch (elemType) {
      case bdlmxxx::ElemType::BDEM_LIST: {
        bdlmxxx::List *list = (bdlmxxx::List *) aggData;
        allocator->deleteObject(list);
      } break;
      case bdlmxxx::ElemType::BDEM_TABLE: {
        bdlmxxx::Table *table = (bdlmxxx::Table *) aggData;
        allocator->deleteObject(table);
      } break;
      case bdlmxxx::ElemType::BDEM_CHOICE: {
        bdlmxxx::Choice *choice = (bdlmxxx::Choice *) aggData;
        allocator->deleteObject(choice);
      } break;
      case bdlmxxx::ElemType::BDEM_CHOICE_ARRAY: {
        bdlmxxx::ChoiceArray *choiceArray = (bdlmxxx::ChoiceArray *) aggData;
...
static bool compareNillableElement(bdlaggxxx::AggregateRaw agg, const CERef& elemRef)
{
...
static void assignToNillableTable(bdlaggxxx::AggregateRaw *agg,
...
static bool compareNillableElement(bdlaggxxx::AggregateRaw agg,
                                   const CERef&      elemRef,
                                   int               index)
{
...
static bool compareNillableTable(bdlaggxxx::AggregateRaw agg, const CERef& elemRef)
{
...
void testCase30() {
        ...
        //
        // Plan:
        //: 1 Create a 'bdlaggxxx::AggregateRaw' object, 'mX', and assign a table to
        //:   it.
        //:
        ...
        mX.setData(&table);
        mX.setDataType(bdlmxxx::ElemType::BDEM_TABLE);
...
void testCase29() {
        ...
        LOOP_ASSERT(A.dataType(), bdlmxxx::ElemType::BDEM_CHOICE == A.dataType());
...
void testCase28() {
        ...
        //
        // Testing:
        //   void swap(bdlaggxxx::AggregateRaw& other);
        // --------------------------------------------------------------------
...
void testCase26() {
        ...
        for (int i = 1; i <= 4096; i <<= 1) {
            bdlmxxx::ChoiceArray ca1, ca2;
            Obj mX;
            mX.setDataType(bdlmxxx::ElemType::BDEM_CHOICE_ARRAY);
            mX.setData(&ca1);
        ...
        for (int i = 1; i <= 4096; i <<= 1) {
            ...
            Obj mX;
            mX.setDataType(bdlmxxx::ElemType::BDEM_INT_ARRAY);
            mX.setData(&sa1);
        ...
        if (verbose) cout << "Testing with a non-array type aggregate" << endl;
        {
            bdlmxxx::List list;
            Obj mX;
            mX.setDataType(bdlmxxx::ElemType::BDEM_LIST);
            mX.setData(&list);
...
void testCase25() {
        ...
        if (verbose) cout << "Testing with a table-type aggregate" << endl;
        {
            ...
            int nf = 0;
            for (int i = 1; i <= 4096; i <<= 1) {
                bdlmxxx::Table table1(&ta), table2(&aa);
                Obj mX, mY;
                mX.setDataType(bdlmxxx::ElemType::BDEM_TABLE);
                mX.setData(&table1);
                mX.setTopLevelAggregateNullness(&nf);
                mY.setDataType(bdlmxxx::ElemType::BDEM_TABLE);
                mY.setData(&table2);
                          ...
                          << endl;
        {
            ...
            int nf = 0;
            for (int i = 1; i <= 4096; i <<= 1) {
                bdlmxxx::ChoiceArray ca1(&ta), ca2(&aa);
                Obj mX, mY;
                mX.setDataType(bdlmxxx::ElemType::BDEM_CHOICE_ARRAY);
                mX.setData(&ca1);
                mX.setTopLevelAggregateNullness(&nf);
                mY.setDataType(bdlmxxx::ElemType::BDEM_CHOICE_ARRAY);
                mY.setData(&ca2);
                mY.setTopLevelAggregateNullness(&nf);
                          ...
                          << endl;
        {
            ...
            for (int i = 1; i <= 4096; i <<= 1) {
                ...
                mX.setData(&sa1);
                mX.setTopLevelAggregateNullness(&nf);
                mY.setDataType(bdlmxxx::ElemType::BDEM_INT_ARRAY);
                mY.setData(&sa2);
                mY.setTopLevelAggregateNullness(&nf);
        ...
        if (verbose) cout << "Testing with a non-array type aggregate" << endl;
        {
            ...
            int nf = 0;
            bdlmxxx::List list1(&la), list2(&aa);
            Obj mX, mY;
            mX.setDataType(bdlmxxx::ElemType::BDEM_LIST);
            mX.setData(&list1);
            mX.setTopLevelAggregateNullness(&nf);
            mY.setDataType(bdlmxxx::ElemType::BDEM_LIST);
            mY.setData(&list2);
            mY.setTopLevelAggregateNullness(&nf);
...
void testCase22() {
        ...
        bdlaggxxx::AggregateRaw mA2, &A2 = mA2;
...
void testCase21() {
        ...
        for (int i = 0; i < NUM_DATA; ++i) {
            ...
            if (ET::isScalarType(TYPE) && HAS_DEFAULT) {
                ...
                const bdlmxxx::ElemRef&   ELEM_REF   = A.asElemRef();
                ...
                const bdlmxxx::Table& T = A.asElemRef().theTable();
...
void testCase18() {
        ...
        //
        // Testing:
        //   bdlaggxxx::AggregateRaw(const bdlaggxxx::AggregateRaw&  other);
        // --------------------------------------------------------------------
...
void testCase15() {
                          ...
                          << bsl::endl;
        {
            ...
            for (int i = 0; i < NUM_DATA; ++i) {
                ...
                ASSERT(!rc);
                ASSERT(bdlmxxx::ElemType::BDEM_VOID == L.dataType());
...
void testCase9() {
        ...
        //   ostream& operator<<(ostream&, const bdlaggxxx::AggregateRaw&);
        // --------------------------------------------------------------------
...
void testCase6() {
        ...
        if (veryVerbose) { T_ cout << "Testing for aggregates" << endl; }
        {
            ...
            for (int i = 0; i < NUM_DATA; ++i) {
                ...
                const bdlmxxx::ElemRef ERY = Y.asElemRef();
...
void testCase3() {
        ...
        //   void setField(bdlaggxxx::AggregateRaw   *field,
            ...
            for (int i = 0; i < NUM_DATA; ++i) {
                ...
                // Test with invalid field value
                {
                    if (s.numRecords() > 1 || ET::BDEM_STRING != TYPE) {
                        bdlb::NullableValue<double> errValue(3.14);
...
void testCase1() {
        ...
        bdlmxxx::Table     table(ct);
        table.appendRow(list3);

        ct.clear();
        ct.push_back(ET::BDEM_STRING);
        ct.push_back(ET::BDEM_INT);
        bdlmxxx::Choice choice(ct);
...

        {
            ...
            {
                bslma::TestAllocator sa;
                bdlmxxx::List data1(list1, &sa);
                bdlmxxx::List data2(list2, &sa);
                Obj agg1;
                agg1.setSchema(schema.get());
