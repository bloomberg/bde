// bdlmxxx_choicearrayimp.h                                              -*-C++-*-
#ifndef INCLUDED_BDLMXXX_CHOICEARRAYIMP
#define INCLUDED_BDLMXXX_CHOICEARRAYIMP
...
//
//@CLASSES:
// bdlmxxx::ChoiceArrayImp: provide implementation of a 'bdem' choice array
//
//@SEE_ALSO: bdlmxxx_choice, bdlmxxx_choicearray, bdlmxxx_choiceheader, bdlmxxx_elemref
//
...
// This component is *used* *by* the 'bdlmxxx_choicearray' component.  A choice
...
// 'bdlmxxx::ChoiceArrayImp' is a list of items.
//
...
//..
// To illustrate, consider an item within a 'bdlmxxx::ChoiceArrayImp' having a
...
// type.
//
// For example, suppose we have an item at index 1 in a 'bdlmxxx::ChoiceArrayImp',
...
// 'bdlmxxx::ChoiceArrayItem' for specific details about assigning
// values to bdem choice objects.
//
///Thread Safety
///-------------
// It is safe to access separate 'bdlmxxx::ChoiceArrayImp' objects simultaneously
// in separate threads.  It is not safe to access a single
// 'bdlmxxx::ChoiceArrayImp' object simultaneously in separate threads.
//
///Usage
///-----
...
class ChoiceArrayImp {
    ...
    // in the terms of an array of 'ChoiceHeader' objects.
    //
    ...
    class CatalogContainer {
        ...
        DescriptorCatalog                        *d_catalog_p; // catalog (held
                                                               // not owned)
        AggregateOption::AllocationStrategy  d_allocationStrategy;
    ...
    bsl::vector<ChoiceHeader> d_headers;           // choice headers
    bsl::vector<int>               d_nullBits;          // null bits

    // FRIENDS
    friend bool operator==(const ChoiceArrayImp&,
                           const ChoiceArrayImp&);
  ...
  public:
    // TRAITS
    BSLALG_DECLARE_NESTED_TRAITS3(ChoiceArrayImp,
                                  bslalg::TypeTraitBitwiseMoveable,
                                  bslalg::TypeTraitUsesBslmaAllocator,
                                  bdlb::TypeTraitHasPrintMethod);

    // CLASS DATA
    static const Descriptor  s_choiceArrayAttr;
        // Descriptor for the 'ChoiceArrayImp' class.  Contains pointers
    ...
    void streamInChoiceArray(
                 ...
                 STREAM&                                stream,
                 int                                    version,
                 const DescriptorStreamIn<STREAM> *strmAttrLookupTbl,
    ...
    void streamOutChoiceArray(
                    ...
                    STREAM&                                 stream,
                    int                                     version,
                    const DescriptorStreamOut<STREAM> *strmAttrLookupTbl);
        // Call the 'bdexStreamOut' method through a generic interface.  A
        // pointer to an instantiation of this function is stored in the
        // streaming function lookup tables for each stream type.

    // CREATORS
    explicit ChoiceArrayImp(
    ...
    ChoiceArrayImp(
        ...
        // '0 <= selectionTypes[i] < ElemType::BDEM_NUM_TYPES'.

    ChoiceArrayImp(const ChoiceArrayImp&  original,
                        bslma::Allocator           *basicAllocator = 0);
    ChoiceArrayImp(
    ...
    ChoiceHeader& theModifiableItem(int itemIndex);
    ...
    void clear();
        ...
        // that this method is needed for 'bdlmxxx_functiontemplates'.
    ...
    void insertItem(int dstIndex, const ChoiceHeader& src);
    ...
    ElemRef makeSelection(int itemIndex, int selection);
    ...
    const ChoiceHeader& theItem(int itemIndex) const;
    ...
    ElemType::Type selectionType(int selection) const;
        // Return the element type corresponding to the specified 'selection'
        // in the types catalog or 'ElemType::BDEM_VOID' if 'selection'
    ...
    bdexStreamOut(
              STREAM&                                 stream,
              int                                     version,
              const DescriptorStreamOut<STREAM> *strmAttrLookupTbl) const;
...
// FREE OPERATORS
bool operator==(const ChoiceArrayImp& lhs,
                const ChoiceArrayImp& rhs);
...
inline
bsl::ostream& operator<<(bsl::ostream& stream, const ChoiceArrayImp& rhs);
...
bdlmxxx::ChoiceArrayImp::CatalogContainer::catalog()
{
    return d_catalog_p;
}
...
// ACCESSORS
inline
const bdlmxxx::ChoiceHeader::DescriptorCatalog *
bdlmxxx::ChoiceArrayImp::CatalogContainer::catalog() const
{
    return d_catalog_p;
}
...
// ACCESSORS
inline
bsl::size_t ChoiceArrayImp::capacityRaw() const
{
    return d_headers.capacity();
}

inline
int ChoiceArrayImp::length() const
{
...
// PRIVATE MANIPULATORS
inline
ChoiceHeader::DescriptorCatalog *ChoiceArrayImp::catalog()
{
...
template <class STREAM>
inline
void ChoiceArrayImp::streamInChoiceArray(
                  void                                  *obj,
                  STREAM&                                stream,
                  int                                    version,
                  const DescriptorStreamIn<STREAM> *strmAttrLookupTbl,
                  const Descriptor                 *const *attrLookupTbl)
{
    ...
    static_cast<ChoiceArrayImp *>(obj)->bdexStreamIn(stream,
                                                          version,
                                                          strmAttrLookupTbl,
                                                          attrLookupTbl);
}
...
template <class STREAM>
inline
void ChoiceArrayImp::streamOutChoiceArray(
                     const void                             *obj,
                     STREAM&                                 stream,
                     int                                     version,
                     const DescriptorStreamOut<STREAM> *strmAttrLookupTbl)
{
    ...
    static_cast<const ChoiceArrayImp *>(obj)->bdexStreamOut(
                                                            stream,
                                                            version,
                                                            strmAttrLookupTbl);
}
...
// MANIPULATORS
inline
ChoiceHeader& ChoiceArrayImp::theModifiableItem(int itemIndex)
{
...
inline
void ChoiceArrayImp::clear()
{
    reset();
}

inline
void ChoiceArrayImp::removeItems(int dstIndex, int numItems)
{
    ...
    bdlb::BitstringUtil::removeAndFill0(
                                 &d_nullBits.front(),
...
inline
void ChoiceArrayImp::removeItem(int dstIndex)
{
    BSLS_ASSERT_SAFE(0 <= dstIndex);
    ...
    switch (version) {  // Switch on the version (starting with 1).
      case 3: {
        ...
        for (int i = 0; i < numSelections; ++i) {
            ...
            if (!stream
             || (unsigned)elemType >= (unsigned)ElemType::BDEM_NUM_TYPES)
                                                                              {
                stream.invalidate();
                return stream;                                        // RETURN
            }
...
// ACCESSORS
inline
int ChoiceArrayImp::numSelections() const
{
    return (int)catalog()->size();
}

inline
const ChoiceHeader& ChoiceArrayImp::theItem(int itemIndex) const
{
    BSLS_ASSERT_SAFE(0 <= itemIndex);
    BSLS_ASSERT_SAFE(     itemIndex < length());
...
inline
int ChoiceArrayImp::selector(int itemIndex) const
{
    BSLS_ASSERT_SAFE(0 <= itemIndex);
    BSLS_ASSERT_SAFE(     itemIndex < length());
      ...
      case 3: {
        ...
        for (int i = 0; i < numSelections; ++i) {
            ElemType::bdexStreamOut(
                            stream,
                            (ElemType::Type) (*catalogPtr)[i]->d_elemEnum,
                            1);
        }
...
inline
bsl::ostream& bdlmxxx::operator<<(bsl::ostream& stream, const ChoiceArrayImp& rhs)
{
