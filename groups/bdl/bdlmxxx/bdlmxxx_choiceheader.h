...
//
//@CLASSES:
//  bdlmxxx::ChoiceHeader: core implementation of 'bdem' choice machinery
//
//@SEE_ALSO:
//  bdlmxxx_choiceimp, bdlmxxx_choicearrayimp, bdlmxxx_choicearrayitem
//
//@AUTHOR: Pablo Halpern (phalpern), Anthony Comerico (acomeric)
//
//@DESCRIPTION: This component defines a class, 'bdlmxxx::ChoiceHeader', which
// provides the data and machinery common to 'bdlmxxx::ChoiceImp',
// 'bdlmxxx::ChoiceArrayImp' and 'bdlmxxx::ChoiceArrayItem' (choice objects).  Clients
// should not use this class directly but should use one of the choice classes
// instead.
//
// A choice header object stores a list of 'bdlmxxx::Descriptor' values and can
...
// management and destruction of its stored value.  'bdlmxxx::ChoiceHeader'
...
// component.
//..
//  Descriptor Catalog - The descriptor catalog is a set of 'bdlmxxx::Descriptor'
...
//  selected descriptor.  'bdlmxxx::ChoiceHeader' is responsible for the
...
//..
// To illustrate, consider a 'bdlmxxx::ChoiceHeader' with a 'BDEM_BOOL' selection.
...
//
// Note that a 'bdlmxxx::ChoiceHeader' may contain a sub-object of type
// 'bdlmxxx::ChoiceHeader', though the top-level 'bdlmxxx::ChoiceHeader' does not know
// its own nullness; the 'flags' method will correctly report the nullness of
// the nested choice sub-object.
//
///Thread Safety
///-------------
// It is safe to access separate 'bdlmxxx::ChoiceHeader' objects simultaneously in
// separate threads.  It is not safe to access a single 'bdlmxxx::ChoiceHeader'
...
class ChoiceHeader {
    ...
    // 'bdexStreamIn' and 'bdexStreamOut'.  A 'ChoiceHeader' can thus be
    // streamed to an older program provided the features being used are
    // compatible with that version of the object.

  public:
    // TRAITS
    BSLALG_DECLARE_NESTED_TRAITS(ChoiceHeader,
                                 bslalg::TypeTraitBitwiseMoveable);

    // PUBLIC TYPES
    typedef bsl::vector<const Descriptor *> DescriptorCatalog;

    // PUBLIC CONSTANTS
    enum {
        // Enumerate valid indices into the 'ChoiceHeader' flags.
  ...
  private:
    ...
    int                                       d_flags;      // null bit and
                                                            // future use.
    AggregateOption::AllocationStrategy  d_allocMode;  // alloc mode
    ...
    union {
        ...
        // 'Choice', 'ChoiceArray', and 'Table', which are
  ...
  public:
    ...
    static const Descriptor  s_choiceItemAttr;
        // Attributes of the 'ChoiceHeader' class.  Contains null pointers
        // for construction, destruction, and assignment functions, but has
        // functionality for the equality test, printing, etc.

    // CREATORS
    ChoiceHeader(const DescriptorCatalog                  *catalogPtr,
    ...
    ChoiceHeader(const ChoiceHeader&                 original,
                      AggregateOption::AllocationStrategy allocMode
                                    = AggregateOption::BDEM_PASS_THROUGH);
    ...
    // MANIPULATORS
    ChoiceHeader& operator=(const ChoiceHeader& rhs);
    ...
    const Descriptor *selectionDescriptor(int index) const;
        ...
        // and the descriptor corresponding to 'ElemType::BDEM_VOID'
    ...
    AggregateOption::AllocationStrategy allocMode() const;
...
// FREE OPERATORS
bool operator==(const ChoiceHeader& lhs, const ChoiceHeader& rhs);
...
inline
bsl::ostream& operator<<(bsl::ostream& stream, const ChoiceHeader& rhs);
...
// PRIVATE MANIPULATORS
template <class STREAM>
void ChoiceHeader::streamInSelection(
                     STREAM&                                stream,
                     int                                    version,
                     const DescriptorStreamIn<STREAM> *strmAttrLookup,
                     const Descriptor                 *const attrLookup[])
{
    const Descriptor *descriptor = (*d_catalog_p)[d_selector];
    ElemType::Type    type = (ElemType::Type) descriptor->d_elemEnum;
    const DescriptorStreamIn<STREAM> *elemStrmAttr =
                                                   &strmAttrLookup[(int) type];

    version = ElemType::isAggregateType(type) ? version : 1;
...
// PRIVATE ACCESSORS
template <class STREAM>
void ChoiceHeader::streamOutSelection(
                  STREAM&                                 stream,
                  int                                     version,
                  const DescriptorStreamOut<STREAM> *strmAttrLookup) const
{
...
inline
void ChoiceHeader::clearNullnessBit()
{
    d_flags &= ~(1 << ChoiceHeader::BDEM_NULLNESS_FLAG);
}

inline
void ChoiceHeader::setNullnessBit()
{
    d_flags |= (1 << ChoiceHeader::BDEM_NULLNESS_FLAG);
}

inline
int& ChoiceHeader::flags()
{
    return d_flags;
}
...
// ACCESSORS
inline
bool ChoiceHeader::isSelectionNull() const
{
    return d_flags & (1 << ChoiceHeader::BDEM_NULLNESS_FLAG);
}

inline
int ChoiceHeader::numSelections() const
{
    return (int)d_catalog_p->size();
}

inline
int ChoiceHeader::selector() const
{
    return d_selector;
}

inline
const void *ChoiceHeader::selectionPointer() const
{
    return const_cast<ChoiceHeader *>(this)->selectionPointer();
}

inline
const int& ChoiceHeader::flags() const
{
    return d_flags;
}

inline
const ChoiceHeader::DescriptorCatalog *
ChoiceHeader::catalog() const
{
    return d_catalog_p;
}

inline
bslma::Allocator *ChoiceHeader::allocator() const
{
    return d_catalog_p->get_allocator().mechanism();
}

inline
AggregateOption::AllocationStrategy ChoiceHeader::allocMode() const
{
    return d_allocMode;
}
...
inline
bsl::ostream& bdlmxxx::operator<<(bsl::ostream& stream, const ChoiceHeader& rhs)
{
