...
//
//@CLASSES:
//  bdlmxxx::Row: indexable, heterogeneous, streamable sequence of elements
//
//@SEE_ALSO: bdlmxxx_list, bdlmxxx_table, bdlmxxx_elemref, bdlmxxx_elemtype
//
//@AUTHOR: Pablo Halpern (phalpern)    (originally by Dan Glaser)
//
//@DESCRIPTION: This component implements a container class, 'bdlmxxx::Row', that
// can hold an arbitrary sequence of elements of any of the 'bdem' types
// enumerated in 'bdlmxxx::ElemType' (see the table below).
//
///Logical Rows and 'bdlmxxx::Row'
///---------------------------
// A 'bdlmxxx::Row' object is designed to represent the *common* aspects of the
// data structure of the proper value-semantic 'bdem' aggregate containers,
// 'bdlmxxx::List' and 'bdlmxxx::Table'.  In the 'bdem' design, a list is exactly one
...
// language.  Instead, we use 'bdlmxxx::Row' as a concrete representation of an
// arbitrary logical row, and give to 'bdlmxxx::Row' several properties that might
// at first seem unfortunate, but upon closer examination turn out to be
// practical and useful.
//
// Due to its specific intended use, 'bdlmxxx::Row' has a relatively limited
...
// types) of the row.  Moreover, 'bdlmxxx::Row' is not a value-semantic type, but
...
// to cast a pointer to 'bdlmxxx::RowData' to a 'bdlmxxx::Row *'), nor is there a
...
//
// With this intentionally limited interface, 'bdlmxxx::Row' serves as a useful
// proxy for the logical row of either a list or a table; 'bdlmxxx::Row' appears
// (in name only) frequently in the signatures of 'bdlmxxx::List' and 'bdlmxxx::Table',
...
// between 'bdlmxxx::ElemRef' as a proxy for any one 'bdem' element (type and
// value) and 'bdlmxxx::Row' as a proxy for any one *sequence* of 'bdem' elements
// (types and values).  'bdlmxxx::Row' may be regarded as a "reference semantic"
...
// 'bdlmxxx_elemref' component for a discussion of reference semantics.
//
...
// and their respective 'bdlmxxx::ElemType' enumerators (with the 'BDEM_' prefix
...
//                   DATETIME_ARRAY     bsl::vector<bdlt::Datetime>
//                   DATETIMETZ_ARRAY   bsl::vector<bdlt::DatetimeTz>
//                   DATE_ARRAY         bsl::vector<bdlt::Date>
//                   DATETZ_ARRAY       bsl::vector<bdlt::DateTz>
//                   TIME_ARRAY         bsl::vector<bdlt::Time>
//                   TIMETZ_ARRAY       bsl::vector<bdlt::TimeTz>
//
//  (heterogeneous)
//       AGGREGATE:  CHOICE             bdlmxxx::Choice
//                   CHOICE_ARRAY       bdlmxxx::ChoiceArray
//                   LIST               bdlmxxx::List
//                   TABLE              bdlmxxx::Table
//..
...
// list) or a homogeneous sequence (in the case of a table) of 'bdlmxxx::Row'
...
//..
// To illustrate, consider a 'BDEM_BOOL' element within a 'bdlmxxx::Row'.  The
...
// type.
//
// For example, suppose we have a 'bdlmxxx::Row', 'myRow', whose first element is
// of type 'BDEM_STRING':
//..
...
// the 'bdlmxxx::ElemRef' that is returned by 'operator[]' can be used to assign
...
//      0, 0,
//      &bdlmxxx::Properties::s_intAttr,         /* BDEM_INT    == 2 */
//      0, 0,
//      &bdlmxxx::Properties::s_doubleAttr,      /* BDEM_DOUBLE == 5 */
...
class Row {
    ...
    // of, an aggregate container (e.g., 'List' or 'Table').
    //
    ...
    // suppressed.  In particular, the only way to create a 'Row' object
    // is to cast the address of a 'RowData' object to 'Row *'.
    // Consequently, the 'sizeof' operator does not reflect the actual size of
    // of a 'Row', which may vary with the number of objects contained.
    // This behavior supports the effective use of 'Row' as a proxy for
    // the logical rows of lists and tables.

    // DATA
    RowData d_rowData;  // holds all items contributing to size and
                             // alignment
  ...
  public:
    ...
    Row& replaceValues(const Row& other);
...
bsl::ostream& operator<<(bsl::ostream& stream, const Row& row);
...
// PRIVATE ACCESSORS
inline
const void *Row::elemData(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
// CLASS METHODS
inline
int Row::maxSupportedBdexVersion()
{
    return 3;
}
...
// MANIPULATORS
inline
Row& Row::replaceValues(const Row& other)
{
    if (this != &other) {
        const RowData& srcRowData = (const RowData&) other;
...
inline
void Row::makeAllNull()
{
    d_rowData.makeAllNull();
}

inline
void Row::makeNull(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
template <class STREAM>
inline
STREAM& Row::bdexStreamIn(STREAM& stream, int version)
{
...
inline
ElemRef Row::operator[](int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bool& Row::theModifiableBool(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
char& Row::theModifiableChar(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
short& Row::theModifiableShort(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
int& Row::theModifiableInt(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bsls::Types::Int64& Row::theModifiableInt64(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
float& Row::theModifiableFloat(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
double& Row::theModifiableDouble(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bsl::string& Row::theModifiableString(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bdlt::Datetime& Row::theModifiableDatetime(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bdlt::DatetimeTz& Row::theModifiableDatetimeTz(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bdlt::Date& Row::theModifiableDate(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bdlt::DateTz& Row::theModifiableDateTz(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bdlt::Time& Row::theModifiableTime(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bdlt::TimeTz& Row::theModifiableTimeTz(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bsl::vector<bool>& Row::theModifiableBoolArray(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bsl::vector<char>& Row::theModifiableCharArray(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bsl::vector<short>& Row::theModifiableShortArray(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bsl::vector<int>& Row::theModifiableIntArray(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bsl::vector<bsls::Types::Int64>& Row::theModifiableInt64Array(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bsl::vector<float>& Row::theModifiableFloatArray(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bsl::vector<double>& Row::theModifiableDoubleArray(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bsl::vector<bsl::string>& Row::theModifiableStringArray(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bsl::vector<bdlt::Datetime>& Row::theModifiableDatetimeArray(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bsl::vector<bdlt::DatetimeTz>& Row::theModifiableDatetimeTzArray(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bsl::vector<bdlt::Date>& Row::theModifiableDateArray(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bsl::vector<bdlt::DateTz>& Row::theModifiableDateTzArray(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bsl::vector<bdlt::Time>& Row::theModifiableTimeArray(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bsl::vector<bdlt::TimeTz>& Row::theModifiableTimeTzArray(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
Choice& Row::theModifiableChoice(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
ChoiceArray& Row::theModifiableChoiceArray(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
List& Row::theModifiableList(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
Table& Row::theModifiableTable(int index)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
ElemType::Type Row::elemType(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
bool Row::isAnyNull() const
{
    return d_rowData.isAnyInRangeNull(0, length());
}

inline
bool Row::isAnyNonNull() const
{
...
inline
bsl::ostream&
Row::print(bsl::ostream& stream, int level, int spacesPerLevel) const
{
...
template <class STREAM>
inline
STREAM& Row::bdexStreamOut(STREAM& stream, int version) const
{
...
inline
ConstElemRef Row::operator[](int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const bool& Row::theBool(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const char& Row::theChar(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const short& Row::theShort(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const int& Row::theInt(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const bsls::Types::Int64& Row::theInt64(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const float& Row::theFloat(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const double& Row::theDouble(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const bsl::string& Row::theString(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const bdlt::Datetime& Row::theDatetime(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const bdlt::DatetimeTz& Row::theDatetimeTz(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const bdlt::Date& Row::theDate(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const bdlt::DateTz& Row::theDateTz(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const bdlt::Time& Row::theTime(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const bdlt::TimeTz& Row::theTimeTz(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const bsl::vector<bool>& Row::theBoolArray(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const bsl::vector<char>& Row::theCharArray(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const bsl::vector<short>& Row::theShortArray(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const bsl::vector<int>& Row::theIntArray(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const bsl::vector<bsls::Types::Int64>& Row::theInt64Array(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const bsl::vector<float>& Row::theFloatArray(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const bsl::vector<double>& Row::theDoubleArray(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const bsl::vector<bsl::string>& Row::theStringArray(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const bsl::vector<bdlt::Datetime>& Row::theDatetimeArray(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
Row::theDatetimeTzArray(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const bsl::vector<bdlt::Date>& Row::theDateArray(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const bsl::vector<bdlt::DateTz>& Row::theDateTzArray(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const bsl::vector<bdlt::Time>& Row::theTimeArray(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const bsl::vector<bdlt::TimeTz>& Row::theTimeTzArray(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const Choice& Row::theChoice(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const ChoiceArray& Row::theChoiceArray(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const List& Row::theList(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
inline
const Table& Row::theTable(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(index < d_rowData.length());
...
// FREE OPERATORS
inline
bool bdlmxxx::operator==(const Row& lhs, const Row& rhs)
{
...
inline
bsl::ostream& bdlmxxx::operator<<(bsl::ostream& stream, const Row& row)
{
