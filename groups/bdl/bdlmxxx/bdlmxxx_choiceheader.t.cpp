...
// STATIC DATA MEMBERS
// [12] static const bdlmxxx::Descriptor s_choiceItemAttr;

// PRIVATE ACCESSORS
// [ 9] int bdexMinorVersion() const;

// CREATORS
// [ 4] bdlmxxx::ChoiceHeader(Catalog* catalog, Mode allocMode);
// [ 7] bdlmxxx::ChoiceHeader(const bdlmxxx::ChoiceHeader& orig, Mode allocMode);
// [ 4] ~bdlmxxx::ChoiceHeader();
...
// [ 4] const bdlmxxx::Descriptor *selectionDescriptor(int index) const;
...
#define P(X) bsl::cout << #X " = " << (X) << bsl::endl;
                                                   // Print ID and value.
#define PV(X) bsl::cout << #X " = " << bdlb::PrintMethods::print(bsl::cout, X)\
                        << bsl::endl;
...
typedef bdlmxxx::ChoiceHeader                    Obj;
...
//   LargeString      - sizeof(bdlmxxx::ChoiceHeader)-byte object without
//                      allocator trait
//   LargeStringAlloc - sizeof(bdlmxxx::ChoiceHeader)-byte object with
//                      allocator trait
...
const int LFP = sizeof(bdlmxxx::ChoiceHeader); // Large Footprint
...
// a 'bdlmxxx::ChoiceHeader' which can store either a 'double' or a 'string'.  The
...
//..
      ...
      bdlmxxx::ChoiceHeader::DescriptorCatalog catalog;
      catalog.push_back(&bdlmxxx::Properties::s_doubleAttr);
      catalog.push_back(&bdlmxxx::Properties::s_stringAttr);
      catalog.push_back(&bdlmxxx::Properties::s_stringAttr);

      bdlmxxx::ChoiceHeader outMessage(&catalog);
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      ...
      case 12: {
        ...
        //   1. 'd_elemEnum == bdlmxxx::ElemType::BDEM_CHOICE_ARRAY_ITEM'
        //   2. 'd_size == sizeof(bdlmxxx::ChoiceHeader)'
        //   3. 'd_alignment ==
        //                   bsls::AlignmentFromType<bdlmxxx::ChoiceHeader>::VALUE'
        ...
        //   7. Function 'print' calls the 'bdlmxxx::ChoiceImp::print'.
        //
        // Plan:
        ...
        //
        // Testing:
        //   static const bdlmxxx::Descriptor s_choiceItemAttr;
        // --------------------------------------------------------------------

        if (verbose) bsl::cout << "\nTesting 's_choiceItemAttr' descriptor"
                               << "\n====================================="
                               << bsl::endl;
        const bdlmxxx::Descriptor& Desc = Obj::s_choiceItemAttr;

        ASSERT(bdlmxxx::ElemType::BDEM_CHOICE_ARRAY_ITEM == Desc.d_elemEnum);
            ...
            for (int i = 0; i < NUM_DATA; ++i) {
                ...
                for (int j = 0; j < LEN; ++j) {
                  ...
                  BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator) {
                    ...
                    bdlma::SequentialAllocator  seqAlloc(&testAllocator);
      ...
      case 9: {
        ...
        //   is confirmed using the specially instrumented 'bdlxxxx::TestInStream'
        //   and a pair of standard macros, 'BEGIN_BDEX_EXCEPTION_TEST' and
        //   'END_BDEX_EXCEPTION_TEST', which configure the 'bdlxxxx::TestInStream'
...
// TBD the following is *not* a test for backward-compatibility
                               ...
                               << bsl::endl;
        {
            ...
            for (int k = 0; k < 4; ++k) {
                ...
                for (int i = 0; i < NUM_DATA; ++i) {
                    ...
                    bdlxxxx::TestInStream is(INPUT, LENGTH);
                ...
                for (int i = 0; i < NUM_DATA; ++i) {
                    ...
                    for (int j1 = 0; j1 < LEN; ++j1) {
                        ...
                        // Stream from an empty and invalid stream
                        {
                            ...
                            bdlxxxx::TestInStream is;  // Empty stream
                            ...
                            bdlxxxx::TestInStream is2(os.data(), os.length());
      ...
      case 8: {
        ...
        //
        // Testing:
        //   bdlmxxx::ChoiceHeader& operator=(const bdlmxxx::ChoiceHeader& rhs);
        // --------------------------------------------------------------------
      ...
      case 7: {
        ...
        //
        // Testing:
        //   bdlmxxx::ChoiceHeader(const bdlmxxx::ChoiceHeader& original);
        // --------------------------------------------------------------------
      ...
      case 5: {
        ...
        //                       int           level = 0,
        //                       int           spacesPerLevel = 4) const;
        //   ostream& operator<<(ostream&, const bdlmxxx::ChoiceHeader&);
        //
        // --------------------------------------------------------------------
      ...
      case 4: {
        ...
        //
        // Plan:
        //   1. Create a 'bdlmxxx::ChoiceHeader' object from a types catalog
        ...
        //
        // Testing:
        //   bdlmxxx::ChoiceHeader(const DescriptorCatalog       *catalogPtr,
        //                     AggOption::AllocationStrategy  allocMode);
        //   ~bdlmxxx::ChoiceHeader();
        //   void *selectionPointer();
        //   int flags();
        //   int numSelections() const;
        //   int selector() const;
        //   const bdlmxxx::Descriptor *selectionDescriptor(int index) const;
      ...
      case 1: {
        ...
        //   bdlmxxx::ChoiceHeader object are as expected, check that memory is
        //   being used as expected, and (if in verbose mode), print the value
        //   of the created or modified object.
        //   0. Construct a types catalog (vector<bdlmxxx::ElemType::Type>)
        ...
        bdlmxxx::ChoiceHeader::DescriptorCatalog catalog(&t1);
        const int INT_IDX = catalog.size();
        catalog.push_back(&bdlmxxx::Properties::s_intAttr);
        ...
        bdlmxxx::ChoiceHeader choice1(&catalog);
        ASSERT(&bdlmxxx::Properties::s_voidAttr ==
        ...
        ASSERT(!choice1.isSelectionNull());
        ASSERT(&bdlmxxx::Properties::s_intAttr ==
               choice1.selectionDescriptor(choice1.selector()));
        ...
        *sp = "Hello world";
        ...
        bdlmxxx::ChoiceHeader choice3(choice2);
        ASSERT(choice2.selector() == STR_IDX);
