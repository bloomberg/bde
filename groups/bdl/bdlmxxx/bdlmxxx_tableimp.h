...
//
//@CLASSES:
//  bdlmxxx::TableImp: provides implementation of the 'bdem' table
//
//@SEE_ALSO: bdlmxxx_table
//
...
// component is *used* *by* the 'bdlmxxx::Table' component.  Clients should use
// 'bdlmxxx::Table', and should not use this component directly.
//
///'bdem' Null States
///------------------
...
// type.
//
// For example, suppose we have a 'bdlmxxx::TableImp', 'myTableImp', whose first
// element is
// of type 'BDEM_STRING':
//..
...
class TableImp {
    ...
    // 'bdlmxxx_table' object is typically configured at construction to have its
    ...
    // the 'bdlmxxx_list' class implementation.  Clients should use 'bdlmxxx_list',
    ...
    RowLayout                  *d_rowLayout_p;
                                                 // row layout (owned)

    bdlma::Pool                       d_rowPool;  // memory pool

    bsl::vector<RowData *>      d_rows;     // data rows (owned)

    bsl::vector<int>                 d_nullBits; // nullness bits for rows

    // FRIENDS
    friend bool operator==(const TableImp&, const TableImp&);

  private:
    // NOT IMPLEMENTED
    TableImp(const TableImp&);

  public:
    // TRAITS
    BSLMF_NESTED_TRAIT_DECLARATION_IF(TableImp,
                                bslmf::IsBitwiseMoveable,
                                bslmf::IsBitwiseMoveable<RowData>::value);
    BSLMF_NESTED_TRAIT_DECLARATION(TableImp, bslma::UsesBslmaAllocator);

    // CLASS DATA
    static const Descriptor  s_tableAttr;
        // Attributes of the 'TableImp' class.  Contains pointers to
    ...
    TableImp(
        ...
        // 'bdlmxxx_aggregateoption'.)  Optionally specify the 'initialMemorySize'
    ...
    TableImp(
        ...
        // for getting the 'Descriptor' values corresponding to the
        // various 'ElemType' values.  (The meanings of the various
    ...
    // MANIPULATORS
    TableImp& operator=(const TableImp& rhs);
    ...
    void clear();
        ...
        // 'removeAll', which is needed for 'bdlmxxx_functiontemplates'.

    RowData& insertRow(int dstRowIndex, const RowData& srcRow);
    ...
    void makeAllNull();
        ...
        // corresponding unset value for its element type (see 'bdltuxxx_unset').
    ...
    void makeColumnNull(int columnIndex);
        ...
        // corresponding element type (see 'bdltuxxx_unset').
    ...
    void reserveRaw(bsl::size_t numRows);
      ...
      void reset(const ElemType::Type     columnTypes[],
                 int                           numColumns,
                 const Descriptor *const *attrLookupTbl);
    ...
    void setColumnValue(int columnIndex, const void *value);
        ...
        // 'ElemType::BDEM_SHORT_ARRAY', then 'value' must point to an
    ...
    RowData& theModifiableRow(int rowIndex);
    ...
    const RowData& theRow(int rowIndex) const;
...
// FREE OPERATORS
bool operator==(const TableImp& lhs, const TableImp& rhs);
...
void TableImp_enableGeometricMemoryGrowth();
    // Enable geometric memory growth, upon insertion, for 'TableImp'
...
void TableImp_disableGeometricMemoryGrowth();
    // Disable geometric memory growth, upon insertion, for 'TableImp'
    // objects in the current process.  By default, memory is obtained on
    // an "as needed" basis.  Note that this method is provided,
    // *temporarily*, as part of an overall transition strategy to move toward
    // geometric memory growth being the default, and eventually the only
...
// ACCESSORS
inline
int TableImp::numColumns() const
{
    return d_rowLayout_p->length();
}

inline
int TableImp::numRows() const
{
                         ...
                         const Descriptor           *const attrLookup[])
{
    ...
    static_cast<TableImp *>(obj)->bdexStreamInImp(stream,
                                                       version,
                                                       strmAttrLookup,
                                                       attrLookup);
}
                        ...
                        const DescriptorStreamOut<STREAM> *strmAttrLookup)
{
    ...
    static_cast<const TableImp *>(obj)->bdexStreamOutImp(stream,
                                                              version,
                                                              strmAttrLookup);
}
...
inline
void TableImp::makeColumnNull(int columnIndex)
{
...
inline
void TableImp::reserveMemory(int numBytes)
{
...
inline
void TableImp::swapRows(int rowIndex1, int rowIndex2)
{
    ...
    RowData *tmp = d_rows[rowIndex1];
    d_rows[rowIndex1] = d_rows[rowIndex2];
    d_rows[rowIndex2] = tmp;

    bdlb::BitstringUtil::swapRaw(&d_nullBits.front(),
                                rowIndex1,
                                &d_nullBits.front(),
                                rowIndex2,
                                1);
...
inline
RowData& TableImp::theModifiableRow(int rowIndex)
{
    BSLS_ASSERT_SAFE(0 <= rowIndex);
    BSLS_ASSERT_SAFE(     rowIndex < numRows());

    bdlb::BitstringUtil::set(&d_nullBits.front(), rowIndex, false);
    return *d_rows[rowIndex];
}

inline
void TableImp::setColumnValueRaw(int columnIndex, const void *value)
{
    BSLS_ASSERT_SAFE(0 <= columnIndex);
    BSLS_ASSERT_SAFE(     columnIndex < numColumns());
...
inline
void TableImp::setColumnValue(int columnIndex, const void *value)
{
    BSLS_ASSERT_SAFE(0 <= columnIndex);
    BSLS_ASSERT_SAFE(     columnIndex < numColumns());
    BSLS_ASSERT_SAFE(value);
...
template <class STREAM>
STREAM& TableImp::bdexStreamInImp(
                      STREAM&                                stream,
                      int                                    version,
                      const DescriptorStreamIn<STREAM> *strmAttrLookup,
                      const Descriptor                 *const *attrLookup)
    ...
    switch (version) {  // Switch on the schema version (starting with 1).
      ...
      case 1: {
        ...
        for (int i = 0; i < numRows; ++i) {
            ...
            bslma::RawDeleterProctor<RowData, bdlma::Pool> rowProctor(
                                                                   tempRowPtr,
                                                                   &d_rowPool);
            tempRowPtr->bdexStreamInImp(stream,
                                        version,
...
// ACCESSORS
inline
ElemType::Type TableImp::columnType(int columnIndex) const
{
    BSLS_ASSERT_SAFE(0 <= columnIndex);
    BSLS_ASSERT_SAFE(     columnIndex < numColumns());
...
inline
const RowData& TableImp::theRow(int rowIndex) const
{
    BSLS_ASSERT_SAFE(0 <= rowIndex);
    BSLS_ASSERT_SAFE(     rowIndex < numRows());
...
template <class STREAM>
STREAM& TableImp::bdexStreamOutImp(
                  STREAM&                                 stream,
                  int                                     version,
                  const DescriptorStreamOut<STREAM> *strmAttrLookup) const
{
    ...
    switch (version) {  // Switch on the schema version (starting with 1).
      case 3: {
        ...
        while (numRows) {
            ...
            for (int i = 0; i < stride; ++i) {
                if (!(bitmap & bit)) {
                    const RowData *rowPtr = d_rows[rowIndex];
