...
//
//@CLASSES:
//  bdlmxxx::ListImp: heterogeneous sequence, managed by 'bdlmxxx::List' (q.v.)
//
//@SEE_ALSO: bdlmxxx_list
//
//@AUTHOR: Pablo Halpern (phalpern)    (originally by Dan Glaser)
//
//@DESCRIPTION: This component implements a 'bdem' list, a heterogeneous
// sequence of 'bdem' elements.  This component is *used* *by* the 'bdlmxxx_list'
// component.  Clients should use 'bdlmxxx_list', and should not use this
// component directly.
//
///'bdem' Null States
///------------------
...
// type.
//
// For example, suppose we have a 'bdlmxxx::ListImp', 'myListImp', whose first
...
class ListImp {
    ...
    // class is *used* *by* the 'bdlmxxx_list' class implementation.  Clients
    // should use 'bdlmxxx_list', and should not use this class in place.

    // DATA
    // NOTE: Do *NOT* change the order of the data members.  The implementation
    // relies on them being declared in this order.
    AllocatorManager    d_allocatorManager;
                                            // object storing the allocation
                                            // strategy and custom allocator

    RowLayout          *d_rowLayout_p; // row layout (owned)

    RowData            *d_rowData_p;   // holds all items contributing to
                                            // size and alignment (owned)

    // FRIENDS
    friend bool operator==(const ListImp&, const ListImp&);

  private:
    // PRIVATE MANIPULATORS
    void init();
    void init(const ElemType::Type *elementTypes,
              int                        numElements,
              const Descriptor     *const *attrLookupTbl);
    void init(const RowData& srcRowData);
        // TBD: Doc
  ...
  public:
    ...
    // TRAITS
    BSLMF_NESTED_TRAIT_DECLARATION_IF(ListImp,
                                bslmf::IsBitwiseMoveable,
                                bslmf::IsBitwiseMoveable<RowData>::value);
    BSLMF_NESTED_TRAIT_DECLARATION(ListImp, bslma::UsesBslmaAllocator);
    ...
    // PUBLIC CLASS DATA
    static const Descriptor  s_listAttr;
        // Attributes of the 'ListImp' class.  Contains pointers to
    ...
    // CREATORS
    explicit
    ListImp(AggregateOption::AllocationStrategy  allocationStrategy,
    ...
    ListImp(AggregateOption::AllocationStrategy  allocationStrategy,
        ...
        // 'bdlmxxx_aggregateoption'.)  Optionally specify an 'initialMemorySize'
    ...
    ListImp(const ElemType::Type                *elementTypes,
                 int                                       numElements,
                 const Descriptor *const             *attrLookupTbl,
    ...
    ListImp(const ElemType::Type                *elementTypes,
        ...
        // 'bdltuxxx_unset').  Optionally specify a memory allocation strategy
    ...
    ListImp(const RowData&                       original,
                 AggregateOption::AllocationStrategy  allocationStrategy,
                 bslma::Allocator                         *basicAllocator = 0);
    ListImp(const ListImp&                       original,
                 AggregateOption::AllocationStrategy  allocationStrategy,
                 bslma::Allocator                         *basicAllocator = 0);
    ListImp(const RowData&                       original,
                 AggregateOption::AllocationStrategy  allocationStrategy,
    ...
    ListImp(const ListImp&                       original,
        ...
        // implicit conversion of a 'Row' to a 'List' is deliberately
    ...
    // MANIPULATORS
    ListImp& operator=(const RowData& rhs);
    ListImp& operator=(const ListImp& rhs);
    ...
    void *insertNullElement(int dstIndex, const Descriptor *elemAttr);
    ...
    void insertNullElements(int                        dstIndex,
        ...
        // corresponding element types (see 'bdltuxxx_unset').
    ...
    ListImp& insertList(int dstIndex, const RowData& srcRow);
    ...
    void clear();
        ...
        // 'bdlmxxx_functiontemplates'.

    void replaceElement(int index, const ConstElemRef& srcElem);
    ...
    void resetElement(int index, const Descriptor *elemAttr);
        ...
        // (see 'bdltuxxx_unset').  Also note that, absent aliasing, the behavior
    ...
    void reset(const ElemType::Type *elementTypes,
               int                        numElements,
               const Descriptor     *const *attrLookupTbl);
        ...
        // (see 'bdltuxxx_unset').  Also note that the behavior of this method is
        // the same as:
        //..
        //  removeAll();
    ...
    bdexStreamInImp(STREAM&                                stream,
        ...
        // to a descriptor (see bdlmxxx_descriptor).
        //
    ...
    int size() const;
        ...
        // is needed for 'bdlmxxx_functiontemplates'.

    const RowData& rowData() const;
        // Return a reference to the non-modifiable heterogeneous row of
        // element values managed by this list.  Note that a
        // 'RowData   ' object does not permit the number or types of
...
// FREE OPERATORS
bool operator==(const ListImp& lhs, const ListImp& rhs);
                     ...
                     const Descriptor                 *const attrLookup[])
{
    ...
    static_cast<ListImp *>(obj)->bdexStreamInImp(stream,
                                                      version,
                                                      strmAttrLookup,
                                                      attrLookup);
}
                        ...
                        const DescriptorStreamOut<STREAM> *strmAttrLookup)
{
    ...
    static_cast<const ListImp *>(obj)->bdexStreamOutImp(stream,
                                                             version,
                                                             strmAttrLookup);
}
...
// MANIPULATORS
inline
ListImp& ListImp::operator=(const ListImp& rhs)
{
    if (this != &rhs) {
...
inline
void ListImp::reserveMemory(int numBytes)
{
...
inline
void ListImp::removeElement(int index)
{
...
inline
void ListImp::removeElements(int startIndex, int numElements)
{
...
inline
void ListImp::resetElement(int index, const Descriptor *elemAttr)
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(     index < length());
...
inline
void ListImp::reset(const ElemType::Type *elementTypes,
                         int                        numElements,
                         const Descriptor     *const *attrLookupTbl)
{
    BSLS_ASSERT_SAFE(0 <= numElements);
...
inline
void ListImp::compact()
{
    ListImp tmpList(*this,
                         d_allocatorManager.allocationStrategy(),
                         d_allocatorManager.originalAllocator());
    bdlimpxxx::BitwiseCopy<ListImp>::swap(this, &tmpList);
}

inline
void ListImp::swap(int index1, int index2)
{
...
template <class STREAM>
STREAM& ListImp::bdexStreamInImp(
                     STREAM&                                stream,
                     int                                    version,
                     const DescriptorStreamIn<STREAM> *strmAttrLookup,
                     const Descriptor                 *const attrLookup[])
{
    switch (version) {  // Switch on the schema version (starting with 1).
      ...
      case 2:                                                   // FALL THROUGH
      case 1: {
        RowLayout layout(d_allocatorManager.internalAllocator());
        ...
        bdlimpxxx::BitwiseCopy<RowLayout>::swap(d_rowLayout_p, &layout);

        RowData rowData(d_rowLayout_p,
                             d_allocatorManager.allocationStrategy(),
        ...
        bdlimpxxx::BitwiseCopy<RowData>::swap(d_rowData_p, &rowData);
      } break;
      default: {
        stream.invalidate();
      }
...
inline
RowData& ListImp::rowData()
{
    return *d_rowData_p;
}

inline
ElemType::Type ListImp::elemType(int index) const
{
    BSLS_ASSERT_SAFE(0 <= index);
    BSLS_ASSERT_SAFE(     index < length());
...
inline
const RowData& ListImp::rowData() const
{
    return *d_rowData_p;
}
...
inline
bsl::ostream& ListImp::print(bsl::ostream& stream,
                                  int           level,
...
// FREE OPERATORS
inline
bool bdlmxxx::operator==(const ListImp& lhs, const ListImp& rhs)
{
