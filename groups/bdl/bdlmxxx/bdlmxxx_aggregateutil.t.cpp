...
//  tolerances.  These tests are substantially taken from 'bdlmxxx_aggregate'.
...
//  'bdlimpxxx_fuzzy'.  To adequately test the various parameter-passing
...
static const char TYPE_CHARS[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdef";
    // letters that correspond to the bdem::ElemTypes, retrieval in
...
static char indexToTypeChar(int index)
    ...
    // '0 <= index < bdlmxxx::ElemType::BDEM_NUM_TYPES'.
{
    ASSERT(0 <= index && index < bdlmxxx::ElemType::BDEM_NUM_TYPES);
...
static const bdlmxxx::Table                   *G_VALUES_V[3];
static const bool                         *G_VALUES_W[3];
static const bsl::vector<bool>            *G_VALUES_X[3];
static const bdlmxxx::Choice                  *G_VALUES_Y[3];
static const bdlmxxx::ChoiceArray             *G_VALUES_Z[3];  // { &XZ, &YZ, &UZ }
static const bdlt::DatetimeTz              *G_VALUES_a[3];  // { &Xa, &Ya, &Ua }
static const bdlt::DateTz                  *G_VALUES_b[3];  // { &Xb, &Yb, &Ub }
static const bdlt::TimeTz                  *G_VALUES_c[3];  // { &Xc, &Yc, &Uc }
static const bsl::vector<bdlt::DatetimeTz> *G_VALUES_d[3];
static const bsl::vector<bdlt::DateTz>     *G_VALUES_e[3];
static const bsl::vector<bdlt::TimeTz>     *G_VALUES_f[3];
...
static void appendElement(bdlmxxx::List *address, char val, char type)
...
#if 0
static void appendElement(bdlmxxx::List *address, int typeIdx, int valIdx)
{
...
struct Assertions {
    char assertion1[sizeof(int) == sizeof(bdlmxxx::ElemType::Type)];
};

void getElemTypes(bsl::vector<bdlmxxx::ElemType::Type> *types, const bdlmxxx::Row& row)
...
inline
void getElemTypes(bsl::vector<bdlmxxx::ElemType::Type> *types,
                  const bdlmxxx::List&                  list)
...
{
    bsl::vector<bdlmxxx::ElemType::Type> elemTypes;
    ...
    for (int i = 0; i < length; i++) {
        bdlmxxx::ConstElemRef ref = row[i];
...
static int ggg(bdlmxxx::List *address, const char *spec, int showErrorFlag)
...
bdlmxxx::List g(const char *spec)
...
//-----------------------------------------------------------------------------
// The 'h' family of functions generate a 'bdlmxxx::Table' object for testing.
// They, along with their helpers, interpret a given 'spec' (from left to
...
// the 'columnTypes' of the input 'bdlmxxx::Table'.  When a complete type field is
...
static int hhh(bdlmxxx::Table *address, const char *spec, int showErrorFlag)
    // Configure the modifiable 'bdlmxxx::Table' at the specified 'address'
    // according to the specified 'spec'.  Report errors to 'cout' only if the
    // specified 'showErrorFlag' is non-zero.  Return 0 on success, and a
    // unique non-zero global error code otherwise.  Note that this function
    // uses only the primary manipulators of the test plan.  Note also that
...
bdlmxxx::Table& hh(bdlmxxx::Table *address, const char *spec)
    // Configure the modifiable 'bdlmxxx::Table' at the specified 'address'
...
bdlmxxx::Table h(const char *spec)
    // Return by value a new 'bdlmxxx::Table' object corresponding to the specified
    // 'spec'.
{
    bdlmxxx::Table object;
...
// 'bdlmxxx::AggregateUtil' functions under test.
...
bool elExact(const bdlmxxx::List& lhs, const bdlmxxx::List& rhs)
{
...
bool ecExact(const bdlmxxx::Choice& lhs, const bdlmxxx::Choice& rhs)
{
    return eExact(lhs.selection(), rhs.selection());
}

bool etExact(const bdlmxxx::Table& lhs, const bdlmxxx::Table& rhs)
{
...
bool ecaExact(const bdlmxxx::ChoiceArray& lhs, const bdlmxxx::ChoiceArray& rhs)
{
    for (int idx = 0; idx < lhs.length(); ++idx) {
...
bool elApprox2(const bdlmxxx::List& lhs, const bdlmxxx::List& rhs)
{
    for (int idx = 0; idx < lhs.length(); ++idx) {
...
bool ecApprox2(const bdlmxxx::Choice& lhs, const bdlmxxx::Choice& rhs)
{
    return eApprox2(lhs.selection(), rhs.selection());
}

bool etApprox2(const bdlmxxx::Table& lhs, const bdlmxxx::Table& rhs)
{
    for (int rIdx = 0; rIdx < lhs.numRows(); ++rIdx) {
        for (int cIdx = 0; cIdx < lhs.numColumns(); ++cIdx) {
...
bool ecaApprox2(const bdlmxxx::ChoiceArray& lhs, const bdlmxxx::ChoiceArray& rhs)
{
    for (int idx = 0; idx < lhs.length(); ++idx) {
...
bool elApprox3(const bdlmxxx::List& lhs, const bdlmxxx::List& rhs, double dRel)
{
...
bool ecApprox3(const bdlmxxx::Choice& lhs,
               const bdlmxxx::Choice& rhs,
               double             dRel)
{
    return eApprox3(lhs.selection(), rhs.selection(), dRel);
}

bool etApprox3(const bdlmxxx::Table& lhs, const bdlmxxx::Table& rhs, double dRel)
{
    for (int rIdx = 0; rIdx < lhs.numRows(); ++rIdx) {
        for (int cIdx = 0; cIdx < lhs.numColumns(); ++cIdx) {
...
bool ecaApprox3(const bdlmxxx::ChoiceArray& lhs,
                const bdlmxxx::ChoiceArray& rhs,
                double                  dRel)
{
    for (int idx = 0; idx < lhs.length(); ++idx) {
...
bool elApprox4(const bdlmxxx::List& lhs,
...
bool ecApprox4(const bdlmxxx::Choice& lhs,
               const bdlmxxx::Choice& rhs,
               double             dRel,
               double             dAbs)
{
...
bool etApprox4(const bdlmxxx::Table& lhs,
               const bdlmxxx::Table& rhs,
               double            dRel,
               double            dAbs)
{
    for (int rIdx = 0; rIdx < lhs.numRows(); ++rIdx) {
...
bool ecaApprox4(const bdlmxxx::ChoiceArray& lhs,
                const bdlmxxx::ChoiceArray& rhs,
                double                  dRel,
                double                  dAbs)
{
...
bool elApprox5(const bdlmxxx::List& lhs,
               const bdlmxxx::List& rhs,
               double           dRel,
               double           dAbs,
               double           fRel)
...
bool ecApprox5(const bdlmxxx::Choice& lhs,
               const bdlmxxx::Choice& rhs,
               double             dRel,
               double             dAbs,
               double             fRel)
...
bool etApprox5(const bdlmxxx::Table& lhs,
               const bdlmxxx::Table& rhs,
               double            dRel,
               double            dAbs,
               double            fRel)
...
bool ecaApprox5(const bdlmxxx::ChoiceArray& lhs,
                const bdlmxxx::ChoiceArray& rhs,
                double                  dRel,
                double                  dAbs,
                double                  fRel)
...
bool elApprox6(const bdlmxxx::List& lhs,
               const bdlmxxx::List& rhs,
               double           dRel,
               double           dAbs,
               double           fRel,
...
bool ecApprox6(const bdlmxxx::Choice& lhs,
               const bdlmxxx::Choice& rhs,
               double             dRel,
               double             dAbs,
               double             fRel,
...
bool etApprox6(const bdlmxxx::Table& lhs,
               const bdlmxxx::Table& rhs,
               double            dRel,
               double            dAbs,
               double            fRel,
...
bool ecaApprox6(const bdlmxxx::ChoiceArray& lhs,
                const bdlmxxx::ChoiceArray& rhs,
                double                  dRel,
                double                  dAbs,
                double                  fRel,
...
int main(int argc, char *argv[])
{
    ...
    //
    // For each of the 22 'bdlmxxx::ElemType' types 't', make a set of three
    ...
    const bdlmxxx::Choice                UY;
    const bdlmxxx::ChoiceArray           UZ;

    const bdlt::DatetimeTz               Ua;
    const bdlt::DateTz                   Ub;
    const bdlt::TimeTz                   Uc;
    const bsl::vector<bdlt::DatetimeTz>  Ud;
    const bsl::vector<bdlt::DateTz>      Ue;
    const bsl::vector<bdlt::TimeTz>      Uf;
    ...
    bsl::vector<bdlmxxx::ElemType::Type> XUElemTypes;
    getElemTypes(&XUElemTypes, XU);
    bdlmxxx::Table XV_(XUElemTypes, Z);
    ...
    bsl::vector<bdlmxxx::ElemType::Type> XYElemTypes;
    getElemTypes(&XYElemTypes, XU);
    bdlmxxx::Choice XY_(XYElemTypes, Z);
    const bdlmxxx::Choice& XY = XY_;
    XY_.makeSelection(0).theModifiableInt() = XC;

    bsl::vector<bdlmxxx::ElemType::Type> XZElemTypes;
    getElemTypes(&XZElemTypes, XU);
    bdlmxxx::ChoiceArray XZ_(XZElemTypes, Z);
    const bdlmxxx::ChoiceArray& XZ = XZ_;
    ...
    const bdlt::DatetimeTz            Xa(XH, 10);
    const bdlt::DateTz                Xb(XI, 10);
    const bdlt::TimeTz                Xc(XJ, 10);

    bsl::vector<bdlt::DatetimeTz>        Xd_;
    const bsl::vector<bdlt::DatetimeTz>& Xd = Xd_;
    Xd_.push_back(Xa);
    Xd_.push_back(Ua);

    bsl::vector<bdlt::DateTz>            Xe_;
    const bsl::vector<bdlt::DateTz>&     Xe = Xe_;
    Xe_.push_back(Xb);
    Xe_.push_back(Ub);

    bsl::vector<bdlt::TimeTz>            Xf_;
    const bsl::vector<bdlt::TimeTz>&     Xf = Xf_;
    Xf_.push_back(Xc);
    Xf_.push_back(Uc);
    ...
    bsl::vector<bdlmxxx::ElemType::Type> YUElemTypes;
    getElemTypes(&YUElemTypes, YU);
    bdlmxxx::Table YV_(&YUElemTypes[0],
                   YUElemTypes.size(),
                   Z);
    ...
    bsl::vector<bdlmxxx::ElemType::Type> YYElemTypes;
    getElemTypes(&YYElemTypes, YU);
    bdlmxxx::Choice YY_(YYElemTypes, Z);
    const bdlmxxx::Choice& YY = YY_;
    YY_.makeSelection(1).theModifiableString() = YG;

    bsl::vector<bdlmxxx::ElemType::Type> YZElemTypes;
    getElemTypes(&YZElemTypes, YU);
    bdlmxxx::ChoiceArray YZ_(YZElemTypes, Z);
    const bdlmxxx::ChoiceArray& YZ = YZ_;
    ...
    const bdlt::DatetimeTz            Ya(YH, 10);
    const bdlt::DateTz                Yb(YI, 10);
    const bdlt::TimeTz                Yc(YJ, 10);

    bsl::vector<bdlt::DatetimeTz>        Yd_;
    const bsl::vector<bdlt::DatetimeTz>& Yd = Yd_;
    Yd_.push_back(Ya);
    Yd_.push_back(Ua);

    bsl::vector<bdlt::DateTz>            Ye_;
    const bsl::vector<bdlt::DateTz>&     Ye = Ye_;
    Ye_.push_back(Yb);
    Ye_.push_back(Ub);

    bsl::vector<bdlt::TimeTz>            Yf_;
    const bsl::vector<bdlt::TimeTz>&     Yf = Yf_;
    Yf_.push_back(Yc);
    Yf_.push_back(Uc);
...
//..
      ...
      case 3: {
        ...
        //   'operator==', and so the 'bdlmxxx_aggregate' test is borrowed.
        //
        // Plan:
      ...
      case 2: {
        ...
        //   'bdlmxxx_aggregate' test is borrowed.  An extra test varies each
      ...
      case 1: {
        ...
        //   'bdlmxxx::AggregateUtil' using lists, tables, choices and choice
