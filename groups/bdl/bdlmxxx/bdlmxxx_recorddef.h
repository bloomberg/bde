...
//
//@CLASSES:
//  bdlmxxx::RecordDef: record definition comprising a field-definition sequence
//
//@SEE_ALSO: bdlmxxx_fielddef, bdlmxxx_schema
//
//@AUTHOR: Tom Marshall, Henry Verschell
//
//@DESCRIPTION: This component implements a collaborative type,
// 'bdlmxxx::RecordDef', that is an integral part of a fully-value semantic schema
// object.  'bdem::RecodDef' objects serve as metadata describing the
// structure of associated records.  Each 'bdlmxxx::RecordDef' object contains a
...
// 'bdlmxxx::RecordDef' directly, but instead obtain a record definition from a
// 'bdlmxxx::Schema' (see 'bdlmxxx_schema' for more information).  A record
...
// definition, and, as such, are accessed directly via the 'bdlmxxx::RecordDef'
// interface.
//
///Record Type
///-----------
...
// 'bdlmxxx::RecordDef' directly, but instead obtain one from a schema (see
// 'bdlmxxx_schema').
//
///Schema Description
///- - - - - - - - - -
...
///- - - - - - - - - - - - - - - - - - - - - - - - - - - -
// This section demonstrates how to create a 'bdem::RecordRef' (and
// 'bdlmxxx::EnumerationDef') in isolation -- this is *not* the recommended usage
// of 'bdlmxxx::RecordDef' (or 'bdlmxxx::EnumerationDef'): obtain 'bdlmxxx::RecordDef'
// objects from a schema (see 'bdlmxxx_schema').  For the purpose of this usage
// example, we define a dummy "stand-in" type for 'bdlmxxx::Schema'.  This type is
// used *in* *name* *only* by 'bdlmxxx::RecordDef', so no particular definition is
// necessary:
//..
...
//..
//
///Manipulating and Accessing a 'bdlmxxx::RecordDef'
///- - - - - - - - - - - - - - - - - - - - - - -
...
// definitions from a schema (see 'bdlmxxx_schema').
//
...
//..
// Next we add a set of values to 'ccTypeEnumDef' (see 'bdlmxxx_enumerationdef'
...
// the field "quantity" we create a 'bdlmxxx::FieldDefAttributes' object that we
...
class RecordDef {
    ...
    // clients should *not* construct a 'RecordDef' directly, but instead
    // obtain a record definition from a schema (see 'bdlmxxx_schema' for more
  ...
  private:
    ...
    // DATA
    const Schema            *d_schema_p;     // address of parent schema
    ...
    bsl::vector<FieldDef *>  d_fieldDefs;    // vector of contained fields
  ...
  private:
    ...
    FieldDef *append(const FieldDefAttributes&  attributes,
        ...
        // 'CONSTRAINT_TYPE' must be either 'EnumerationDef' or
        // 'RecordDef'.  Return the address of the newly-created,
  ...
  private:
    // NOT IMPLEMENTED
    RecordDef(const RecordDef&);
    RecordDef& operator=(const RecordDef&);

  public:
    // TYPES
    struct LowPrecedenceEnumPtr {
        // Use instead of a 'EnumerationDef *' in an overloaded
        // function signature to avoid overloading ambiguities between
        // 'EnumerationDef *' and 'RecordDef *'
    ...
    // CLASS METHODS
    static bool isValidConstraint(const EnumerationDef& constraint,
                                  ElemType::Type        type);
        // Return 'true' if the specified 'type' is either 'INT', 'STRING',
        // 'INT_ARRAY', or 'STRING_ARRAY', and 'false' otherwise.

    static bool isValidConstraint(const RecordDef& constraint,
                                  ElemType::Type   type);
    ...
    RecordDef(const Schema         *schema,
        ...
        // generally *not* attempt to construct a 'RecordDef' directly,
        // but instead should obtain one from a schema (see 'bdlmxxx_schema' for
        // more information).
    ...
    const FieldDef *appendField(
        ...
        //
        // DEPRECATED: Use
        // 'appendField(const FieldDefAttributes&, const char *, int)'
        // instead.

    // ACCESSORS
    const FieldDef& field(int index) const;
    ...
    const FieldDef *lookupField(const char *name) const;
    ...
    const FieldDef *lookupField(int id) const;
        // Return the address of the (non-modifiable) field definition having
    ...
    const char *recordName() const;
        ...
        // definition within a 'bdem' schema (see 'bdlmxxx_schema').
...
// PRIVATE ACCESSORS
inline
bslma::Allocator *RecordDef::writeOnceAllocator() const
{
...
// CLASS METHODS
inline
bool RecordDef::isValidConstraint(const EnumerationDef&,
                                       ElemType::Type        type)
{
...
// MANIPULATORS
inline
const FieldDef *RecordDef::appendField(ElemType::Type  type,
                                                 const char          *name,
                                                 int                  id)
{
    FieldDefAttributes fielddefattributes(type);
...
// ACCESSORS
inline
const FieldDef& RecordDef::field(int index) const
{
...
inline
int RecordDef::fieldId(int index) const
{
    BSLS_ASSERT_SAFE(    0 <= index);
    BSLS_ASSERT_SAFE(index <  numFields());
...
inline
const FieldDef *RecordDef::lookupField(const char *name) const
{
...
inline
const FieldDef *RecordDef::lookupField(int id) const
{
    if (d_fieldIds.empty()) {
...
inline
int RecordDef::fieldIndex(const char *name) const
{
    BSLS_ASSERT_SAFE(name);
...
inline
int RecordDef::fieldIndex(int id) const
{
    if (d_fieldIds.empty()) {
...
inline
const char *RecordDef::fieldName(int index) const
{
    BSLS_ASSERT_SAFE(    0 <= index);
    BSLS_ASSERT_SAFE(index <  numFields());
...
inline
bool RecordDef::hasFieldIds() const
{
    return !d_fieldIds.empty();
}

inline
int RecordDef::length() const
{
    return static_cast<int>(d_fieldDefs.size());
}

inline
int RecordDef::numAnonymousFields() const
{
    return static_cast<int>(d_fieldDefs.size() - d_fieldNames.size());
}

inline
int RecordDef::numFields() const
{
    return static_cast<int>(d_fieldDefs.size());
}

inline
int RecordDef::recordIndex() const
{
    return d_recordIndex;
}

inline
const char *RecordDef::recordName() const
{
    return d_recordName_p;
}

inline
RecordDef::RecordType RecordDef::recordType() const
{
    return d_recordType;
}

inline
const Schema& RecordDef::schema() const
{
    return *d_schema_p;
}
