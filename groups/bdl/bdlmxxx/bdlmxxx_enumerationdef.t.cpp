// bdlmxxx_enumerationdef.t.cpp                  -*-C++-*-
...
///- - - - - - - - - - - - - - - - - - - - - - - - - - - -
// The following section demontrates how to create a 'bdlmxxx::EnumerationDef' in
// isolation -- this is *not* the recommended usage of 'bdlmxxx::EnumerationDef'.
// A 'bdlmxxx::RecordDef' is meant to be used in conjuction with the types defined
// in 'bdlmxxx_schema'.  Clients should not create a 'bdlmxxx::EnumerationDef'
// directly, but obtain one from a 'bdlmxxx::Schema'.  For the purpose of this
// usage example, we define a dummy stand-in type for 'bdlmxxx::Schema'.  This
// type is used in name-only by 'bdlmxxx::EnumerationDef', so its definition is
// not necessary:
//..
//  namespace BloombergLP {
//
