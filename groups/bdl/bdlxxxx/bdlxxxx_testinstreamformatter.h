...
//
//@CLASSES:
//   bdlxxxx::TestInStreamFormatter: byte-array-based input stream class
//
//@AUTHOR: Lea Fester
//
//@SEE_ALSO: bdlxxxx_testoutstream
//
//@DESCRIPTION: This component implements the 'bdlxxxx::InStream' protocol as a
...
// component, 'bdlxxxx::TestOutStream'.  The supported types and required content
...
///Version Checking
///----------------
// By default, 'bdlxxxx::TestInStreamFormatter' assumes that the first unit of
...
// on a 'bdlxxxx::TestInStreamFormatter', the stream checks for the existence of
...
///Usage
///-----
// This 'bdlxxxx::TestInStreamFormatter' component (in conjunction with the
// companion 'bdlxxxx_testoutstream' component) is most useful in testing the
...
// stream-in operation.  The macros assume that a 'bdlxxxx::TestInStreamFormatter'
...
//      //     'bdlxxxx::TestInStreamFormatter' object to throw an exception for
...
//          const int NUM_EXTEND = sizeof EXTEND / sizeof *EXTEND;
//
//          bdlsb::FixedMemInStreamBuf sb((char *)"", 0);
//          bdlxxxx::TestInStreamFormatter testInStream(&sb);
//          testInStream.setSuppressVersionCheck(1);
//
//          int oldLen = -1;
...
class TestInStreamFormatter {
  ...
  private:
    ...
    // 'FieldCode::INT8', then set the specified '*validFlag_p' to zero
    ...
    // NOT DEFINED
    TestInStreamFormatter(const TestInStreamFormatter&);
    TestInStreamFormatter& operator=(const TestInStreamFormatter&);

    bool operator==(const TestInStreamFormatter&) const;
    bool operator!=(const TestInStreamFormatter&) const;

  public:
    // CREATORS
    TestInStreamFormatter(bsl::streambuf *streamBuffer);
    ...
    TestInStreamFormatter& getString(bsl::string& str);
    ...
    TestInStreamFormatter& getVersion(int& variable);
...
// MANIPULATORS
inline
void TestInStreamFormatter::setQuiet(int flag)
{
    d_quietFlag = !!flag;
}

inline
void TestInStreamFormatter::setSuppressVersionCheck(int flag)
{
    d_suppressVersionCheckFlag = !!flag;
}

inline
void TestInStreamFormatter::setInputLimit(int limit)
{
    d_inputLimit = limit;
}

inline
void TestInStreamFormatter::invalidate()
{
    d_validFlag = 0;
}

inline
void TestInStreamFormatter::seek(int offset)
{
...
inline
void TestInStreamFormatter::reset()
{
    d_validFlag = 1;
...
inline
void TestInStreamFormatter::reload(const char *buffer, int numBytes)
{
...
// ACCESSORS
inline
bdlxxxx::TestInStreamFormatter::operator const void *() const
{
...
namespace bdlxxxx {
inline
int TestInStreamFormatter::isQuiet() const
{
    return d_quietFlag;
}

inline
int TestInStreamFormatter::isSuppressVersionCheck() const
{
    return d_suppressVersionCheckFlag;
}

inline
int TestInStreamFormatter::inputLimit() const
{
    return d_inputLimit;
}
}  // close package namespace
