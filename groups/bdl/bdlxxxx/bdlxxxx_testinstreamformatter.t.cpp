...
#include <bdlxxxx_testoutstreamformatter.h>                 // for testing only
...
//                                 Overview
//                                 --------
// We are testing a "test" implementation of the 'bdlxxxx::InStream' protocol.
...
// 'bdlxxxx::OutStream' protocol.  The "informational data" allows an object to
...
//-----------------------------------------------------------------------------
// [ 3] bdlxxxx::TestInStreamFormatter();
// [ 3] bdlxxxx::TestInStreamFormatter(const char *buffer, int numBytes);
// [ 3] ~bdlxxxx::TestInStreamFormatter();
// [ 3] setQuiet(int flag);
...
// right to configure a 'bdlxxxx::TestOutStreamFormatter' object according to a
...
// bdlxxxx::TestOutStreamFormatter::put* method), each digit N serves as an index
// into VX.  Thus, the spec 'G2' invokes 'putInt32' with the value VG[2].
//
// For each lowercase letter (corresponding to an array
// bdlxxxx::TestOutStreamFormatter::put* method), each digit N indicates the first
...
// -----------  ---------------------------------------------------------------
// ""           Has no effect; leaves the bdlxxxx::TestInStreamFormatter empty.
...
int g(bdlxxxx::TestOutStreamFormatter* o, const char* spec) {
...
// bdlxxxx::TestInStreamFormatter is displayed only in "veryVerbose" mode.
//-----------------------------------------------------------------------------
    ...
    // extracted array to the values in the specified 'origVal' array.
{
    ...
    for (int i = 0; i < numTest; ++i)
    {
        ...
        bdlxxxx::TestOutStreamFormatter o(&osb);
        int res = g(&o, spec);
        LOOP2_ASSERT(line, i, 1 == res);
        bdlsb::FixedMemInStreamBuf sb(const_cast<char *>(osb.data()),
                                                             osb.length());
        bdlxxxx::TestInStreamFormatter x(&sb);
...
// created and initialized with the bdlxxxx::TestOutStreamFormatter::put* of the
...
void testGetArrayInputLimit(const InputLimitTestTable *data,
                            ...
                            int                        numArrayElments,
                            FuncPtr                    getFunc,
                            bdlxxxx::FieldCode::Type       dataType)
    // Iterate over the specified 'data' test vectors 'numData' times.  In each
    // iteration, create a test input stream bdlxxxx::TestInStreamFormatter
    // initialized with the bdlxxxx::TestOutStreamFormatter::put* of the generator
    ...
    // bdlxxxx::TestInStreamFormatter contains the specified 'dataType'.
{
    ...
    for (int novers = 0; novers < 2; ++novers) {
        if (veryVerbose) cout << "[ "
                              << (novers ? "With bdlxxxx::TestOutStreamFormatter"
                                         : "With")
                              << " version check ]" << endl;
...
void testGetScalarInputLimit(const InputLimitTestTable *data,
    ...
    // the exception bdlxxxx::TestInStreamFormatter contains the specified
    // 'dataType'.
{
#ifdef BDE_BUILD_TARGET_EXC
    // int verbose = globalVerbose;
...
int hhh(my_DoubleArray *object, const char *spec, int verboseFlag = 1)
    // Configure the specified 'bdlxxxx::TestInStreamFormatter' according to the
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:
      case 30: {
        ...
        //   following test plan.  Note that the 'bdlxxxx::TestInStreamFormatter'
        //   and 'bdlxxxx::TestOutStreamFormatter' components are used to test the
        ...
        //
        // Testing:
        //   bdlxxxx::TestInStreamFormatter& bdexStreamIn(stream, version);
        //   bdlxxxx::TestOutStreamFormatter& bdexStreamOut(stream, version) const;
        // --------------------------------------------------------------------
                             ...
                             " (valid) 'streamIn' functionality." << endl;
        {
            ...
            bdlsb::MemOutStreamBuf osb;
            bdlxxxx::TestOutStreamFormatter out(&osb);
            X.bdexStreamOut(out, VERSION);

            const char *const OD  = osb.data();
            const int         LOD = osb.length();

            bdlsb::FixedMemInStreamBuf sb(const_cast<char *>(OD), LOD);
            bdlxxxx::TestInStreamFormatter in(&sb);  ASSERT(in);
            ASSERT(sb.length() > 0);
        ...
        if (verbose) cout << "\tOn valid, non-empty stream data." << endl;
        {
            ...
            for (int ui = 0; SPECS[ui]; ++ui) {
                ...
                for (int uj = 0; uj < NUM_EXTEND; ++uj) {
                    ...
                    bdlsb::FixedMemInStreamBuf sb(const_cast<char *>(OD), LOD);
                    bdlxxxx::TestInStreamFormatter testInStream(&sb);
            ...
            "\tOn incomplete (but otherwise valid) data." << endl;
        {
            ...
            for (int i = 0; i < LOD; ++i) {
                bdlsb::FixedMemInStreamBuf sb(const_cast<char *>(OD), i);
                bdlxxxx::TestInStreamFormatter testInStream(&sb);
                bdlxxxx::TestInStreamFormatter& in = testInStream;
        ...
        if (verbose) cout << "\t\tBad version." << endl;
        {
            ...
            my_DoubleArray t(X);   ASSERT(W != t);
                                   ASSERT(X == t);    ASSERT(Y != t);
            bdlsb::FixedMemInStreamBuf sb(const_cast<char *>(OD), LOD);
            bdlxxxx::TestInStreamFormatter in(&sb);
      ...
      case 29: {
        ...
        if (verbose) cout << "\nTesting 'getVersion'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                       (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType&);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getVersion;
    ...
    { L_, "A0 C0 T1 C234",  0,     5,      { VC[0] }                         },
      ...
      case 28: {
        // --------------------------------------------------------------------
        // RELOAD TEST:
        //   Create 'bdlxxxx::TestInStreamFormatter' objects containing data of
 ...
 //-------------v
      ...
      case 27: {
...

        {
            ...
            bdlsb::FixedMemInStreamBuf sb(localBuf, 2);
            bdlxxxx::TestInStreamFormatter x(&sb);
      ...
      case 25: {
        // --------------------------------------------------------------------
        // SEEK/RESET TEST:
        //   Create a 'bdlxxxx::TestInStreamFormatter' object initialized with
      ...
      case 24: {
        ...
        if (verbose) cout << "\nTesting 'getArrayFloat64'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                       (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType*, int);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getArrayFloat64;
  ...
  { L_, "A0 l1 q2 l3",  0,      0,        3,      { 1, 2, 3 },   { 1, 0, 0 } },
      ...
      case 23: {
        ...
        if (verbose) cout << "\nTesting 'getArrayFloat32'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                       (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType*, int);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getArrayFloat32;
  ...
  { L_, "A0 k1 q2 k3",  0,      0,        3,      { 1, 2, 3 },   { 1, 0, 0 } },
      ...
      case 22: {
        ...
        if (verbose) cout << "\nTesting 'getArrayInt64'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                     (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType*, int);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getArrayInt64;
  ...
  { L_, "A0 i1 q2 i3",  0,      0,        3,      { 1, 2, 3 },   { 1, 0, 0 } },
        ...
        if (verbose) cout << "\nTesting 'getArrayUint64'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                      (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType*, int);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getArrayUint64;
  ...
  // read from empty stream
  { L_, "",             0,      0,        1,      { 1 },         { 0 }       },
  // stream with bdlxxxx::TestOutStreamFormatter version
  { L_, "j1",           0,      0,        1,      { 1 },         { 0 }       },
  // valid streams with bdlxxxx::TestOutStreamFormatter version and version check
  // turned off
  { L_, "j0",           1,      1,        1,      { 0 },         { 0 }       },
  { L_, "j1",           1,      1,        1,      { 1 },         { 1 }       },
  { L_, "j2",           1,      1,        1,      { 2 },         { 2 }       },
  ...
  { L_, "A0 j1 q2 j3",  0,      0,        3,      { 1, 2, 3 },   { 1, 0, 0 } },
      ...
      case 21: {
        ...
        if (verbose) cout << "\nTesting 'getArrayInt56'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                     (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType*, int);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getArrayInt56;
            const ElemType *V_ORIG = VI;        // base-comparison array
  ...
  { L_, "A0 s1 q2 s3",  0,      0,        3,      { 1, 2, 3 },   { 1, 0, 0 } },
        ...
        if (verbose) cout << "\nTesting 'getArrayUint56'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                        (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType*, int);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getArrayUint56;
            const ElemType *V_ORIG = VJ;        // base-comparison array
  ...
  { L_, "A0 t1 q2 t3",  0,      0,        3,      { 1, 2, 3 },   { 1, 0, 0 } },
      ...
      case 20: {
        ...
        if (verbose) cout << "\nTesting 'getArrayInt48'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                        (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType*, int);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getArrayInt48;
            const ElemType *V_ORIG = VI;        // base-comparison array
  ...
  { L_, "A0 q1 i2 q3",  0,      0,        3,      { 1, 2, 3 },   { 1, 0, 0 } },
        ...
        if (verbose) cout << "\nTesting 'getArrayUint48'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                        (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType*, int);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getArrayUint48;
            const ElemType *V_ORIG = VJ;        // base-comparison array
  ...
  { L_, "A0 r1 q2 r3",  0,      0,        3,      { 1, 2, 3 },   { 1, 0, 0 } },
      ...
      case 19: {
        ...
        if (verbose) cout << "\nTesting 'getArrayInt40'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                        (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType*, int);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getArrayInt40;
            const ElemType *V_ORIG = VI;        // base-comparison array
  ...
  { L_, "A0 o1 q2 o3",  0,      0,        3,      { 1, 2, 3 },   { 1, 0, 0 } },
        ...
        if (verbose) cout << "\nTesting 'getArrayUint40'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                        (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType*, int);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getArrayUint40;
            const ElemType *V_ORIG = VJ;        // base-comparison array
  ...
  { L_, "A0 p1 q2 p3",  0,      0,        3,      { 1, 2, 3 },   { 1, 0, 0 } },
      ...
      case 18: {
        ...
        if (verbose) cout << "\nTesting 'getArrayInt32'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                        (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType*, int);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getArrayInt32;
  ...
  { L_, "A0 g1 q2 g3",  0,      0,        3,      { 1, 2, 3 },   { 1, 0, 0 } },
        ...
        if (verbose) cout << "\nTesting 'getArrayUint32'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                        (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType*, int);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getArrayUint32;
  ...
  { L_, "A0 h1 q2 h3",  0,      0,        3,      { 1, 2, 3 },   { 1, 0, 0 } },
      ...
      case 17: {
        ...
        if (verbose) cout << "\nTesting 'getArrayInt24'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                        (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType*, int);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getArrayInt24;
            const ElemType *V_ORIG = VG;        // base-comparison array
  ...
  { L_, "A0 m1 q2 m3",  0,      0,        3,      { 1, 2, 3 },   { 1, 0, 0 } },
        ...
        if (verbose) cout << "\nTesting 'getArrayUint24'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                        (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType*, int);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getArrayUint24;
            const ElemType *V_ORIG = VH;        // base-comparison array
  ...
  { L_, "A0 n1 q2 n3",  0,      0,        3,      { 1, 2, 3 },   { 1, 0, 0 } },
      ...
      case 16: {
        ...
        if (verbose) cout << "\nTesting 'getArrayInt16'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                        (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType*, int);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getArrayInt16;
  ...
  { L_, "A0 e1 q2 e3",  0,      0,        3,      { 1, 2, 3 },   { 1, 0, 0 } },
        ...
        if (verbose) cout << "\nTesting 'getArrayUint16'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                        (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType*, int);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getArrayUint16;
  ...
  { L_, "A0 f1 q2 f3",  0,      0,        3,      { 1, 2, 3 },   { 1, 0, 0 } },
      ...
      case 15: {
        ...
        if (verbose) cout << "\nTesting 'getArrayInt8' w/ 'char*'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                        (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType*, int);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getArrayInt8;
  ...
  { L_, "A0 b1 q2 b3",  0,        0,      3,      { 1, 2, 3 },   { 1, 0, 0 } },
        ...
        if (verbose) cout << "\nTesting 'getArrayUint8' w/ 'char*'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                        (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType*, int);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getArrayUint8;
  ...
  { L_, "A0 c1 q2 c3",  0,        0,      3,      { 1, 2, 3 },   { 1, 0, 0 } },
      ...
      case 14: {
        ...
        if (verbose) cout << "\nTesting 'getFloat64'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                             (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType&);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getFloat64;
    ...
    { L_, "A0 L0 T1 L234",  0,     5,      { VL[0] }                         },
      ...
      case 13: {
        ...
        if (verbose) cout << "\nTesting 'getFloat32'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                             (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType&);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getFloat32;
    ...
    { L_, "A0 K0 T1 K234",  0,     5,      { VK[0] }                         },
      ...
      case 12: {
        ...
        if (verbose) cout << "\nTesting 'getInt64'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                             (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType&);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getInt64;
    ...
    { L_, "A0 I0 T1 I234",  0,     5,      { VI[0] }                         },
        ...
        if (verbose) cout << "\nTesting 'getUint64'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                             (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType&);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getUint64;
    ...
    { L_, "A0 J0 T1 J234",  0,     5,      { VJ[0] }                         },
      ...
      case 11: {
        ...
        if (verbose) cout << "\nTesting 'getInt56'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                             (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType&);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getInt56;
    ...
    { L_, "A0 S0 T1 S234",  0,     5,      { VS[0] }                         },
        ...
        if (verbose) cout << "\nTesting 'getUint56'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                             (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType&);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getUint56;
    ...
    { L_, "A0 T0 J1 T234",  0,     5,      { VT[0] }                         },
      ...
      case 10: {
        ...
        if (verbose) cout << "\nTesting 'getInt48'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                             (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType&);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getInt48;
    ...
    { L_, "A0 Q0 T1 Q234",  0,     5,      { VQ[0] }                         },
        ...
        if (verbose) cout << "\nTesting 'getUint48'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                             (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType&);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getUint48;
    ...
    { L_, "A0 R0 T1 R234",  0,     5,      { VR[0] }                         },
      ...
      case 9: {
        ...
        if (verbose) cout << "\nTesting 'getInt40'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                             (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType&);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getInt40;
    ...
    { L_, "A0 O0 T1 O234",  0,     5,      { VO[0] }                         },
        ...
        if (verbose) cout << "\nTesting 'getUint40'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                             (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType&);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getUint40;
    ...
    { L_, "A0 P0 T1 P234",  0,     5,      { VP[0] }                         },
      ...
      case 8: {
        ...
        if (verbose) cout << "\nTesting 'getInt32'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                             (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType&);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getInt32;
    ...
    { L_, "A0 G0 T1 G234",  0,     5,      { VG[0] }                         },
        ...
        if (verbose) cout << "\nTesting 'getUint32'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                             (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType&);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getUint32;
    ...
    { L_, "A0 H0 T1 H234",  0,     5,      { VH[0] }                         },
      ...
      case 7: {
        ...
        if (verbose) cout << "\nTesting 'getInt24'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                             (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType&);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getInt24;
    ...
    { L_, "A0 M0 T1 M234",  0,     5,      { VM[0] }                         },
        ...
        if (verbose) cout << "\nTesting 'getUint24'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                             (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType&);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getUint24;
    ...
    { L_, "A0 N0 T1 N234",  0,     5,      { VN[0] }                         },
      ...
      case 6: {
        ...
        if (verbose) cout << "\nTesting 'getInt16'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                             (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType&);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getInt16;
    ...
    { L_, "A0 E0 T1 E234",  0,     5,      { VE[0] }                         },
        ...
        if (verbose) cout << "\nTesting 'getUint16'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                             (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType&);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getUint16;
    ...
    { L_, "A0 F0 T1 F234",  0,     5,      { VF[0] }                         },
      ...
      case 5: {
        ...
        if (verbose) cout << "\nTesting 'getInt8' w/ 'signed char'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                    (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType&);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getInt8;
    ...
    { L_, "A0 B0 T1 B234",  0,     5,      { VB[0] }                         },
        ...
        if (verbose) cout << "\nTesting 'getUint8' w/ 'char'." << endl;
        {
            ...
            typedef bdlxxxx::TestInStreamFormatter&
                     (bdlxxxx::TestInStreamFormatter::*FuncPtr)(ElemType&);
            const FuncPtr FUNC = &bdlxxxx::TestInStreamFormatter::getUint8;
    ...
    { L_, "A0 C0 T1 C234",  0,     5,      { VC[0] }                         },
      ...
      case 4: {
        ...
        for (i = 0; i < 3; ++i) {
            ...
            bdlsb::FixedMemInStreamBuf sb2(const_cast<char *>(osb.data()),
                                          osb.length());
            bdlxxxx::TestInStreamFormatter x2(&sb2);
            LOOP_ASSERT(i, x2);
        ...
        for (i = 0; i < 5; ++i) {
            ...
            char junk;
            bdlsb::FixedMemInStreamBuf sb(&junk, 0);
            bdlxxxx::TestInStreamFormatter x(&sb);
            int j;
            LOOP_ASSERT(i, x.length() == 0);
            LOOP_ASSERT(i, x.cursor() == 0);

            // test bdlxxxx::TestInStreamFormatters of variable lengths
            bdlsb::MemOutStreamBuf osb;
            bdlxxxx::TestOutStreamFormatter o(&osb);    o.putInt8(VERSION);
      ...
      case 3: {
        ...
        // Testing
        //   bdlxxxx::TestInStreamFormatter();
        //   bdlxxxx::TestInStreamFormatter(const char *buffer, int numBytes);
        //   ~bdlxxxx::TestInStreamFormatter();   // by purify
        //   setQuiet(int flag);
...
// instead, then cast the table to ScalarTestTable<char>*, below.
    ...
    { L_, "A0 A0 L1 A234",  0,     5,      { VA[0] }                         }
        ...
        for (int i = 0; i < 5; ++i) {
            // test default bdlxxxx::TestInStreamFormatters
            char localBuf[4]; // g++ 2.95 crashes if size is < 4
            bdlsb::FixedMemInStreamBuf sb2(localBuf, 2);
            bdlxxxx::TestInStreamFormatter x(&sb2);
            LOOP_ASSERT(i, x);
            x.invalidate();
            LOOP_ASSERT(i, !x);
            ...
            bdlsb::FixedMemInStreamBuf sb(const_cast<char *>(osb.data()),
                                         osb.length());
            bdlxxxx::TestInStreamFormatter x2(&sb);
            if (veryVerbose) { P(x2) }
            LOOP_ASSERT(i, x2);
            x2.invalidate();
            LOOP_ASSERT(i, !x2);
      ...
      case 2: {
        ...
        if (verbose) cout << "\nTesting empty spec." << endl;
        {
            const char *SPEC = "";
            bdlsb::MemOutStreamBuf osb2;
            const bdlxxxx::TestOutStreamFormatter X(&osb2);
            bdlsb::MemOutStreamBuf osb;
            bdlxxxx::TestOutStreamFormatter o(&osb);
            int res = g(&o, SPEC);
            ASSERT(1 == res);
        ...
        if (verbose) cout << "\nTesting spec w/ single operation." << endl;
        {
            ...
            if (veryVerbose) cout << "\tSPEC : \"" << SPEC << '"' << endl;
            bdlsb::MemOutStreamBuf osb2;
            bdlxxxx::TestOutStreamFormatter mX(&osb2);
            mX.putInt8(VA[0]);
            mX.putInt8(VA[1]);
            mX.putInt8(VA[2]);
      ...
      case 1: {
        // --------------------------------------------------------------------
        // BREATHING TEST:
        //   Create 'bdlxxxx::TestInStreamFormatter' bdlxxxx::TestInStreamFormatter
        //   containing various data.
        //   Exercise these bdlxxxx::TestInStreamFormatter using appropriate
        //   input methods and primary manipulators, then verify the state of
        //   the resulting bdlxxxx::TestInStreamFormatters using basic accessors.
        //
        // Testing:
        ...
        if (verbose) cout <<
                 "\nCreate bdlxxxx::TestInStreamFormatter x2 w/ an initial value."
                          << endl;
        int i;
        bdlsb::MemOutStreamBuf osb;
        bdlxxxx::TestOutStreamFormatter o(&osb);
        ...
        char localBuf[4]; // g++ 2.95 crashes if size is < 4
        bdlsb::FixedMemInStreamBuf sb2(localBuf, 2);
        bdlxxxx::TestInStreamFormatter x1(&sb2);
