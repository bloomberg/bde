...
//-----------------------------------------------------------------------------
// [1] bdlxxxx::TestInStreamException(typeCode);
// [1] ~bdlxxxx::TestInStreamException();
// [1] bdlxxxx::FieldCode::dataType() const;
//-----------------------------------------------------------------------------
...
my_TestInStream& my_TestInStream::getInt32(int& variable)
{
    ...
    if (0 <= d_inputLimit) {
        --d_inputLimit;
        if (0 > d_inputLimit) {
            throw bdlxxxx::TestInStreamException(bdlxxxx::FieldCode::BDEX_INT32);
        }
    }
#endif
    if (d_validFlag &&
        (d_validFlag = d_cursor + SIZEOF_INT32 <= d_length)) {
        bdlxxxx::ByteStreamImpUtil::getInt32(&variable, &d_buffer[d_cursor]);
...
my_TestInStream& my_TestInStream::getInt8(char& variable)
{
    ...
    if (0 <= d_inputLimit) {
        --d_inputLimit;
        if (0 > d_inputLimit) {
            throw bdlxxxx::TestInStreamException(bdlxxxx::FieldCode::BDEX_INT8);
        }
    }
#endif
    if (d_validFlag &&
        (d_validFlag = d_cursor + SIZEOF_INT8 <= d_length)) {
        bdlxxxx::ByteStreamImpUtil::getInt8(&variable, &d_buffer[d_cursor]);
...
my_TestInStream& my_TestInStream::getArrayInt16(short *array, int length)
{
    ...
    if (0 <= d_inputLimit) {
        --d_inputLimit;
        if (0 > d_inputLimit) {
            throw bdlxxxx::TestInStreamException(bdlxxxx::FieldCode::BDEX_INT16);
        }
    }
#endif
    if (d_validFlag &&
        (d_validFlag = d_cursor + SIZEOF_INT16 * length <= d_length)) {
        bdlxxxx::ByteStreamImpUtil::getArrayInt16(array, &d_buffer[d_cursor],
                                              length);
   ...
   //----  -------  ----------------------  ------  --------------
    ...
    { L_,    19,    "\x01\x00\x00\x00\x05"
      ...
      case 1: {
        // --------------------------------------------------------------------
        // BASIC TEST
        //   Create 'bdlxxxx::TestInStreamException' objects with varying initial
        //   values.  Verify that each object contains the expected value using
        //   basic accessor 'dataType'.
        //
        // Testing:
        //   bdlxxxx::TestInStreamException(typeCode);
        //   ~bdlxxxx::TestInStreamException();
        //   bdlxxxx::FieldCode::Type dataType() const;
        // --------------------------------------------------------------------
        ...
        if (verbose) cout << "Testing ctor and accessor." << endl;
        {
            ...
            for (int i = 0; i < NUM_DATA; ++i) {
                const bdlxxxx::TestInStreamException X(DATA[i]);
