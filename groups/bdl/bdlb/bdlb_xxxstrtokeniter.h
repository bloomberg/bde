...
//
//@CLASSES:
//   bdlb::StrTokenIter: token iterator for a non-modifiable string
//
//@SEE_ALSO: bdlb_xxxstringref, bdlb_strtokenrefiter
//
...
//
// Note that the 'bdlb_strtokenrefiter' component provides a more efficient
...
// 'bdlb::StrTokenRefIter' return 'bdlb::StringRef' and those of
// 'bdlb::StrTokenIter' return 'const char *'.
//
///Definitions
///-----------
...
//    {
//        int numFields = 0;
//        for (bdlb::StrTokenIter it(dateString, " \t", "/"); it; ++it) {
//            ++numFields;
//            if (isVerbose) bsl::cout << it() << ':';
//        }
//        if (isVerbose) bsl::cout << bsl::endl;
...
//    {
//        bsl::ostrstream ostr;
//        bdlb::StrTokenIter it(input, softDelims, hardDelims);
...
//    {
//        enum { GOOD = 0, BAD = 1 };
//        bdlb::StrTokenIter it(input, " \t", "/"); // construct iterator
//                                                        // outside 'for' loop
//        for (; it; ++it) {
...
//                           "small lazy dog.\n";
//
//      for (bdlb::StrTokenIter it1(string, "\n"); it1; ++it1) {
//          bsl::cout << '@';
//          for (bdlb::StrTokenIter it2(it1(), " "); it2; ++it2) {
//              bsl::cout << it2() << ':';
//          }
//      }
//..
...
class StrTokenIter {
    ...
    // 'const char *' string.  Note that the 'StrTokenRefIter' class
    // provides a more efficient means for tokenizing a string via
    // 'StringRef', and its use should be preferred over this class.
    ...
    // NOT IMPLEMENTED
    StrTokenIter(const StrTokenIter&);
    StrTokenIter& operator=(const StrTokenIter&);

    bool operator==(const StrTokenIter&) const;
    bool operator!=(const StrTokenIter&) const;

  public:
    // CREATORS
    StrTokenIter(const char       *input,
                       const char       *softDelims,
...
//ACCESSORS
inline
bdlb::StrTokenIter::operator const void *() const
{
...
namespace bdlb {
inline
const char *StrTokenIter::operator()() const
{
...
inline
StringRef StrTokenIter::tokenRef() const
{
    BSLS_ASSERT_SAFE(d_string_p);

    const char *end = d_string_p - d_delimBuf_p->size();

    return StringRef(end - d_tokenBuf.size(), end);
}

inline
const char *StrTokenIter::delimiter() const
{
    return d_delimBuf_p->c_str();
}

inline
const char *StrTokenIter::previousDelimiter() const
{
