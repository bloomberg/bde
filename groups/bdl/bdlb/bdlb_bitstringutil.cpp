...
enum { BITS_PER_INT = bdlb::BitUtil::BDES_BITS_PER_INT };
    ...
    // 'dstPos + numBits <= BITS_PER_INT'.
{
    ...
    const int bits1 = (unsigned int)
                              (int1 & bdlb::BitUtil::oneMask(pos1, numBits))
                                                                     >> pos1;
    const int bits2 = (unsigned int)
                              (int2 & bdlb::BitUtil::oneMask(pos2, numBits))
                                                                     >> pos2;
    return bits1 == bits2;
}
    ...
    // 'int2 + numBits <= BITS_PER_INT'.
{
    ...
    const int bits1 = bdlb::BitstringUtil::get(int1, index1, numBits);
    const int bits2 = bdlb::BitstringUtil::get(int2, index2, numBits);
    bdlb::BitUtil::replaceValue(int1, index1, bits2, numBits);
    bdlb::BitUtil::replaceValue(int2, index2, bits1, numBits);
}
...
void BitstringUtil::andEqual(int       *dstBitstring,
                                  int        dstIndex,
                                  const int *srcBitstring,
                                  int        srcIndex,
                                  int        numBits)
...
void BitstringUtil::copy(int       *dstBitstring,
                              int        dstIndex,
                              const int *srcBitstring,
                              int        srcIndex,
                              int        numBits)
{
    ...
    if (requiresAliasedOper(dstBitstring,
                            ...
                            srcIndex,
                            numBits)) {
        Move<bdlb::BitUtil::replaceValue, Bitwise::setEq>::right(dstBitstring,
                                                                dstIndex,
                                                                srcBitstring,
                                                                srcIndex,
                                                                numBits);
        ...
        Move<bdlb::BitUtil::replaceValue, Bitwise::setEq>::left(dstBitstring,
                                                               dstIndex,
                                                               srcBitstring,
                                                               srcIndex,
                                                               numBits);
...
void BitstringUtil::copyRaw(int       *dstBitstring,
                                 int        dstIndex,
                                 const int *srcBitstring,
                                 int        srcIndex,
                                 int        numBits)
...
void BitstringUtil::insertRaw(int *bitstring,
                                   int  length,
                                   int  dstIndex,
                                   int  numBits)
{
    ...
    Move<bdlb::BitUtil::replaceValue, Bitwise::setEq>::right(bitstring,
                                                            dstIndex + numBits,
                                                            bitstring,
                                                            dstIndex,
                                                            length - dstIndex);
...
void BitstringUtil::minusEqual(int       *dstBitstring,
                                    int        dstIndex,
                                    const int *srcBitstring,
                                    int        srcIndex,
                                    int        numBits)
...
void BitstringUtil::orEqual(int       *dstBitstring,
                                 int        dstIndex,
                                 const int *srcBitstring,
                                 int        srcIndex,
                                 int        numBits)
...
void BitstringUtil::remove(int *bitstring,
                                int  length,
                                int  index,
                                int  numBits)
{
    ...
    if (numBits > 0) {
        ...
        Move<bdlb::BitUtil::replaceValue, Bitwise::setEq>::left(bitstring,
                                                               index,
                                                               bitstring,
                                                               index + numBits,
                                                               remBits);
...
void BitstringUtil::set(int *bitstring, int index, bool value)
{
    BSLS_ASSERT(bitstring);
    BSLS_ASSERT(0 <= index);
    ...
    if (value) {
        bitstring[idx] |= bdlb::BitUtil::eqMask(pos);
    }
    else {
        bitstring[idx] &= bdlb::BitUtil::neMask(pos);
    }
}

void BitstringUtil::set(int  *bitstring,
                             int   index,
                             bool  value,
                             int   numBits)
{
    ...
    if (value) {
        bitstring[idx] |= bdlb::BitUtil::oneMask( pos, numOfBits);
    }
    else {
        bitstring[idx] &= bdlb::BitUtil::zeroMask(pos, numOfBits);
    }
    ...
    const int VALUE = value ? bdlb::BitUtil::oneMask(0, BITS_PER_INT) : 0;
    while (numBits > BITS_PER_INT) {
        bitstring[idx] = VALUE;
        numBits -= BITS_PER_INT;
        ++idx;
    ...
    if (value) {
        bitstring[idx] |= bdlb::BitUtil::oneMask( 0, numBits);
    }
    else {
        bitstring[idx] &= bdlb::BitUtil::zeroMask(0, numBits);
    }
}

void BitstringUtil::swapRaw(int *bitstring1,
                                 int  index1,
                                 int *bitstring2,
                                 int  index2,
                                 int  numBits)
...
void BitstringUtil::toggle(int *bitstring, int index, int numBits)
{
    BSLS_ASSERT(0 <= index);
    BSLS_ASSERT(0 <= numBits);
    ...
    if (pos) {
        ...
        if (num >= numBits) {
            Oper<bdlb::BitUtil::replaceValue>::operBits(
                                                      &bitstring[idx],
                                                      pos,
                                                      (~bitstring[idx]) >> pos,
                                                      numBits);
        ...
        Oper<bdlb::BitUtil::replaceValue>::operBits(&bitstring[idx],
                                                   pos,
                                                   (~bitstring[idx]) >> pos,
                                                   num);
        numBits -= num;
...
void BitstringUtil::xorEqual(int       *dstBitstring,
                                  int        dstIndex,
                                  const int *srcBitstring,
                                  int        srcIndex,
                                  int        numBits)
{
    ...
    if (requiresAliasedOper(dstBitstring,
                            ...
                            srcIndex,
                            numBits)) {
        Move<bdlb::BitUtil::xorEqual, Bitwise::xorEq>::right(dstBitstring,
                                                            dstIndex,
                                                            srcBitstring,
                                                            srcIndex,
                                                            numBits);
        ...
        Move<bdlb::BitUtil::xorEqual, Bitwise::xorEq>::left(dstBitstring,
                                                           dstIndex,
                                                           srcBitstring,
                                                           srcIndex,
                                                           numBits);
                                        ...
                                        int            bitsPerUnit)
{
    ...
    stream << '[';
    if (0 == numBits) {
        Print::newlineAndIndent(stream, level, spacesPerLevel);
        stream << ']';
        if (spacesPerLevel >= 0) {
            stream << '\n';
        }
    ...
    Print::newlineAndIndent(stream, levelPlus1, spacesPerLevel);
...
bool BitstringUtil::areEqual(const int *bitstring1,
                                  int        index1,
...
bool BitstringUtil::isAny0(const int *bitstring,
                                int        index,
                                int        numBits)
{
    BSLS_ASSERT(bitstring);
    ...
    if (~bitstring[idx] & bdlb::BitUtil::oneMask(pos, numOfBits)) {
        return true;                                                  // RETURN
    }
    ...
    while (numBits > BITS_PER_INT) {
        if (bdlb::BitUtil::isAnySetZero(bitstring[idx])) {
            return true;                                              // RETURN
        }
        ++idx;
        numBits -= BITS_PER_INT;
    ...
    return ~bitstring[idx] & bdlb::BitUtil::oneMask(0, numBits);
}

bool BitstringUtil::isAny1(const int *bitstring,
                                int        index,
                                int        numBits)
{
    BSLS_ASSERT(bitstring);
    ...
    while (numBits > BITS_PER_INT) {
        if (bdlb::BitUtil::isAnySetOne(bitstring[idx])) {
            return true;                                              // RETURN
        }
        ++idx;
        numBits -= BITS_PER_INT;
...
int BitstringUtil::find0AtLargestIndex(const int *bitstring, int length)
{
    BSLS_ASSERT(bitstring);
    BSLS_ASSERT(0 <= length);
    ...
    if (length) {
        ...
        const int rv     = bdlb::BitUtil::find0AtLargestIndex(
                                  bdlb::BitUtil::setOne(bitstring[idx],
                                                       endPos,
                                                       BITS_PER_INT - endPos));
        if (-1 != rv) {
...
int BitstringUtil::find0AtLargestIndexGE(const int *bitstring,
                                              int        length,
                                              int        index)
{
    BSLS_ASSERT(bitstring);
    ...
    if (length) {
        ...
        if (len == idx) {
            ...
            int       rv  = bdlb::BitUtil::find0AtLargestIndexGE(
                                      bdlb::BitUtil::setOne(bitstring[len],
                                                           pos,
                                                           BITS_PER_INT - pos),
                                      index % BITS_PER_INT);
            if (-1 != rv) {
            ...
            const int pos = (length - 1) % BITS_PER_INT + 1;
            int       rv  = bdlb::BitUtil::find0AtLargestIndex(
                                     bdlb::BitUtil::setOne(bitstring[len],
                                                          pos,
                                                          BITS_PER_INT - pos));
            if (-1 != rv) {
...
int BitstringUtil::find0AtLargestIndexGT(const int *bitstring,
                                              int        length,
                                              int        index)
{
    BSLS_ASSERT(bitstring);
    ...
    if (length) {
        ...
        if (len == idx) {
            const int pos = (length - 1) % BITS_PER_INT + 1;
            int       rv  = bdlb::BitUtil::find0AtLargestIndexGT(
                                      bdlb::BitUtil::setOne(bitstring[len],
                                                           pos,
                                                           BITS_PER_INT - pos),
                                      index % BITS_PER_INT);
...
int BitstringUtil::find0AtLargestIndexLE(const int *bitstring, int index)
{
    BSLS_ASSERT(0 <= index);

    const int idx = index / BITS_PER_INT;
    const int rv  = bdlb::BitUtil::find0AtLargestIndexLE(bitstring[idx],
                                                        index % BITS_PER_INT);
    if (-1 != rv) {
        return idx * BITS_PER_INT + rv;                               // RETURN
    }
...
int BitstringUtil::find0AtLargestIndexLT(const int *bitstring, int index)
{
    BSLS_ASSERT(bitstring);
    BSLS_ASSERT(0 <= index);

    const int idx = index / BITS_PER_INT;
    const int rv  = bdlb::BitUtil::find0AtLargestIndexLT(bitstring[idx],
                                                        index % BITS_PER_INT);
    if (-1 != rv) {
        return idx * BITS_PER_INT + rv;                               // RETURN
    }
...
int BitstringUtil::find0AtSmallestIndex(const int *bitstring, int length)
{
    BSLS_ASSERT(bitstring);
    BSLS_ASSERT(0 <= length);
    ...
    if (length) {
        ...
        int rv = bdlb::BitUtil::find0AtSmallestIndex(
                                  bdlb::BitUtil::setOne(bitstring[len],
                                                       length,
                                                       BITS_PER_INT - length));
        if (rv < BITS_PER_INT) {
...
int BitstringUtil::find0AtSmallestIndexGE(const int *bitstring,
                                               int        length,
                                               int        index)
{
    BSLS_ASSERT(bitstring);
    ...
    if (length) {
        ...
        if (len == idx) {
            const int pos = (length - 1) % BITS_PER_INT + 1;
            int       rv  = bdlb::BitUtil::find0AtSmallestIndexGE(
                                      bdlb::BitUtil::setOne(bitstring[len],
                                                           pos,
                                                           BITS_PER_INT - pos),
                                      index % BITS_PER_INT);
            ...
            const int pos = (length - 1) % BITS_PER_INT + 1;
            rv = bdlb::BitUtil::find0AtSmallestIndex(
                                     bdlb::BitUtil::setOne(bitstring[len],
                                                          pos,
                                                          BITS_PER_INT - pos));
            if (rv < BITS_PER_INT) {
...
int BitstringUtil::find0AtSmallestIndexGT(const int *bitstring,
                                               int        length,
                                               int        index)
{
    BSLS_ASSERT(bitstring);
    ...
    if (length) {
        ...
        if (len == idx) {
            const int pos = (length - 1) % BITS_PER_INT + 1;
            int       rv  = bdlb::BitUtil::find0AtSmallestIndexGT(
                                      bdlb::BitUtil::setOne(bitstring[len],
                                                           pos,
                                                           BITS_PER_INT - pos),
                                      index % BITS_PER_INT);
...
int BitstringUtil::find0AtSmallestIndexLE(const int *bitstring, int index)
{
    BSLS_ASSERT(bitstring);
    BSLS_ASSERT(0 <= index);
...
int BitstringUtil::find0AtSmallestIndexLT(const int *bitstring, int index)
{
    BSLS_ASSERT(bitstring);
    BSLS_ASSERT(0 <= index);
...
int BitstringUtil::find1AtLargestIndex(const int *bitstring, int length)
{
    BSLS_ASSERT(bitstring);
    BSLS_ASSERT(0 <= length);
    ...
    if (length) {
        const int idx    = (length - 1) / BITS_PER_INT;
        const int endPos = (length - 1) % BITS_PER_INT + 1;
        const int rv     = bdlb::BitUtil::find1AtLargestIndex(
                                 bdlb::BitUtil::setZero(bitstring[idx],
                                                       endPos,
                                                       BITS_PER_INT - endPos));
        if (-1 != rv) {
            return idx * BITS_PER_INT + rv;                           // RETURN
...
int BitstringUtil::find1AtLargestIndexGE(const int *bitstring,
                                              int        length,
                                              int        index)
{
    BSLS_ASSERT(bitstring);
    ...
    if (length) {
        ...
        if (len == idx) {
            const int pos = (length - 1) % BITS_PER_INT + 1;
            int       rv  = bdlb::BitUtil::find1AtLargestIndexGE(
                                     bdlb::BitUtil::setZero(bitstring[len],
                                                           pos,
                                                           BITS_PER_INT - pos),
                                     index % BITS_PER_INT);
            if (-1 != rv) {
            ...
            const int pos = (length - 1) % BITS_PER_INT + 1;
            int       rv  = bdlb::BitUtil::find1AtLargestIndex(
                                    bdlb::BitUtil::setZero(bitstring[len],
                                                          pos,
                                                          BITS_PER_INT - pos));
            if (-1 != rv) {
...
int BitstringUtil::find1AtLargestIndexGT(const int *bitstring,
                                              int        length,
                                              int        index)
{
    BSLS_ASSERT(bitstring);
    ...
    if (length) {
        ...
        if (len == idx) {
            const int pos = (length - 1) % BITS_PER_INT + 1;
            int       rv  = bdlb::BitUtil::find1AtLargestIndexGT(
                                     bdlb::BitUtil::setZero(bitstring[len],
                                                           pos,
                                                           BITS_PER_INT - pos),
                                     index % BITS_PER_INT);
...
int BitstringUtil::find1AtLargestIndexLE(const int *bitstring, int index)
{
    BSLS_ASSERT(bitstring);
    BSLS_ASSERT(0 <= index);

    const int idx = index / BITS_PER_INT;
    const int rv  = bdlb::BitUtil::find1AtLargestIndexLE(bitstring[idx],
                                                        index % BITS_PER_INT);
    if (-1 != rv) {
        return idx * BITS_PER_INT + rv;                               // RETURN
    }
...
int BitstringUtil::find1AtLargestIndexLT(const int *bitstring, int index)
{
    BSLS_ASSERT(bitstring);
    BSLS_ASSERT(0 <= index);

    const int idx = index / BITS_PER_INT;
    const int rv  = bdlb::BitUtil::find1AtLargestIndexLT(bitstring[idx],
                                                        index % BITS_PER_INT);
    if (-1 != rv) {
        return idx * BITS_PER_INT + rv;                               // RETURN
    }
...
int BitstringUtil::find1AtSmallestIndex(const int *bitstring, int length)
{
    BSLS_ASSERT(bitstring);
    BSLS_ASSERT(0 <= length);
    ...
    if (length) {
        ...
        length = (length - 1) % BITS_PER_INT + 1;
        int rv  = bdlb::BitUtil::find1AtSmallestIndex(
                                 bdlb::BitUtil::setZero(bitstring[len],
                                                       length,
                                                       BITS_PER_INT - length));
        if (rv < BITS_PER_INT) {
...
int BitstringUtil::find1AtSmallestIndexGE(const int *bitstring,
                                               int        length,
                                               int        index)
{
    BSLS_ASSERT(bitstring);
    ...
    if (length) {
        ...
        if (len == idx) {
            const int pos = (length - 1) % BITS_PER_INT + 1;
            int       rv  = bdlb::BitUtil::find1AtSmallestIndexGE(
                                     bdlb::BitUtil::setZero(bitstring[len],
                                                           pos,
                                                           BITS_PER_INT - pos),
                                     index % BITS_PER_INT);
            ...
            const int pos = (length - 1) % BITS_PER_INT + 1;
            rv = bdlb::BitUtil::find1AtSmallestIndex(
                                    bdlb::BitUtil::setZero(bitstring[len],
                                                          pos,
                                                          BITS_PER_INT - pos));
            if (rv < BITS_PER_INT) {
...
int BitstringUtil::find1AtSmallestIndexGT(const int *bitstring,
                                               int        length,
                                               int        index)
{
    BSLS_ASSERT(bitstring);
    ...
    if (length) {
        ...
        if (len == idx) {
            const int pos = (length - 1) % BITS_PER_INT + 1;
            int       rv  = bdlb::BitUtil::find1AtSmallestIndexGT(
                                     bdlb::BitUtil::setZero(bitstring[len],
                                                           pos,
                                                           BITS_PER_INT - pos),
                                     index % BITS_PER_INT);
...
int BitstringUtil::find1AtSmallestIndexLE(const int *bitstring, int index)
{
    BSLS_ASSERT(bitstring);
    BSLS_ASSERT(0 <= index);
...
int BitstringUtil::find1AtSmallestIndexLT(const int *bitstring, int index)
{
    BSLS_ASSERT(bitstring);
    BSLS_ASSERT(0 <= index);
...
bool BitstringUtil::get(const int *bitstring, int index)
{
    BSLS_ASSERT(bitstring);
    BSLS_ASSERT(0 <= index);
    ...
    return bitstring[idx] & bdlb::BitUtil::eqMask(pos);
}

int BitstringUtil::get(const int *bitstring, int index, int numBits)
{
    BSLS_ASSERT(bitstring);
    BSLS_ASSERT(0 <= index);
    BSLS_ASSERT(0 <= numBits);
    ...
    int result    = ((unsigned int) (bitstring[idx]
                              & bdlb::BitUtil::oneMask(pos, numOfBits))) >> pos;
    ...
    result |= (bitstring[idx + 1]
                                & bdlb::BitUtil::oneMask(0, numBits)) << numRem;
    return result;
}

int BitstringUtil::num0(const int *bitstring, int index, int numBits)
{
    BSLS_ASSERT(bitstring);
    BSLS_ASSERT(0 <= index);
    BSLS_ASSERT(0 <= numBits);
    ...
    int numSet = bdlb::BitUtil::numSetZero(bitstring[idx]
                                       | bdlb::BitUtil::zeroMask(pos, remBits));
    ...
    numSet += accumulate<bdlb::BitUtil::numSetZero>(&bitstring[idx + 1],
                                                   numInts);

    if (remBits) {
        numSet += bdlb::BitUtil::numSetZero(bitstring[idx + numInts + 1]
                              | bdlb::BitUtil::oneMask(remBits,
...
int BitstringUtil::num1(const int *bitstring, int index, int numBits)
{
    BSLS_ASSERT(bitstring);
    BSLS_ASSERT(0 <= index);
    BSLS_ASSERT(0 <= numBits);
    ...
    int numSet = bdlb::BitUtil::numSetOne(bitstring[idx]
                                     & bdlb::BitUtil::oneMask(pos, remBits));
    ...
    numSet += accumulate<bdlb::BitUtil::numSetOne>(&bitstring[idx + 1],
                                                  numInts);

    if (remBits) {
        numSet += bdlb::BitUtil::numSetOne(bitstring[idx + numInts + 1]
                             & bdlb::BitUtil::zeroMask(remBits,
                                                      BITS_PER_INT - remBits));
    }
