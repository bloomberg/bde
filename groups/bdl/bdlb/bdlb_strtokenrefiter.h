...
///Usage
///-----
// The following usage of 'bdlb::StrTokenRefIter' mimics the standard C library
// function 'strtok' on a non-modifiable 'const char *' string:
//..
//    void listTokens(const char *input, const char *delims, bool isVerbose)
//    {
//        for (bdlb::StrTokenRefIter it(input, delims); it; ++it) {
...
//    {
//        int numFields = 0;
//        for (bdlb::StrTokenRefIter it(dateString, " \t", "/"); it; ++it) {
//            ++numFields;
//            if (isVerbose) bsl::cout << it() << ':';
//        }
//        if (isVerbose) bsl::cout << bsl::endl;
...
//    {
//        bsl::ostrstream ostr;
//        bdlb::StrTokenRefIter it(input, softDelims, hardDelims);
...
class StrTokenRefIter {
  ...
  private:
    // NOT IMPLEMENTED
    StrTokenRefIter(const StrTokenRefIter&);
    StrTokenRefIter& operator=(const StrTokenRefIter&);

    bool operator==(const StrTokenRefIter&) const;
    bool operator!=(const StrTokenRefIter&) const;

  public:
    // CREATORS
    StrTokenRefIter(const char *input, const char *softDelims);
...
// ACCESSORS
inline
bdlb::StrTokenRefIter::operator const void *() const
{
