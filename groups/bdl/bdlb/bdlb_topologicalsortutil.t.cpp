...
class NoDetachedNodeIterator {
  ...
  public:
    ...
    typedef typename Traits::value_type value_type;
        // This iterator is a stand-in for the base type, hence it has the same
        // 'value_type'.
    ...
    void advanceOne()
    {
        ...
        while (MappingType::from(*d_actual) == MappingType::to(*d_actual)) {
            ++d_actual;
            if (d_actual == d_end) {
                return;                                               // RETURN
            }
    ...
    reference operator*() const
        // Do nothing and return a reference to the actual underlying element.
    {
        return *d_actual;
    }
...
// elements of the input.  See 'NoDetachedNodeIterator' before 'main' (local
// classes cannot be templates in C++03).
//
