...
// CREATORS
// [ 2] bdlb::StringRef();
// [ 2] bdlb::StringRef(const char *begin, const char *end);
// [ 2] bdlb::StringRef(const char *begin, int length);
// [ 2] bdlb::StringRef(const char *begin);
// [ 2] bdlb::StringRef(const bsl::string& begin);
// [ 2] bdlb::StringRef(const native_std::string& begin);
// [ 2] bdlb::StringRef(const bdlb::StringRef& original);
// [ 2] ~bdlb::StringRef();
//
// MANIPULATORS
// [ 2] bdlb::StringRef& operator=(const bdlb::StringRef& rhs);
// [ 6] void clear();
...
struct StringProducer {
    ...
    {
        return bsl::string("woof");
    }
};

struct StringRefProducer {
    operator bdlb::StringRef() const
    {
...
// the string that is bound to a specified 'bdlb::StringRef'.  The function
// delegates the work to the 'bsl::count' STL algorithm.  This is made possible
// by the STL-compliant random access iterators provided by 'bdlb::StringRef'
...
// 'bdlb::StringRef' argument.  First we verify tolerance of null pointers and
// 'bdlb::StringRef' objects that are unbound:
//..
...
//..
// In the following, we illustrate use of the 'assign' method to bind 'line' to
...
//..
    ...
    ASSERT(0 == bsl::strncmp("There is nobody wise enough\n",
                             line.data(), line.length()));
//..
// Next we call 'getNumBlanks' with an 'bsl::string' initialized to the
...
//..
      ...
      case 10: {
        ...
        //
        // Concern:
        //   That the free operators work under certain circumstances.  We did
        //   this component just inheriting the free operators from the base
        //   'bslstl::StringRef' class, but we found that when not using
        //   namespace bsl, we didn't have access to them, and furthermore,
        //   since they are template functions they weren't found when a
        //   conversion operator was involved.  So here added the free
        //   operators back.
        //
        // Plan:
        //   We invoke numerous free operators when not using namespace bsl,
        //   observe that we compile and work.
        // --------------------------------------------------------------------

        if (verbose) cout << "TESTING FREE OPERATORS\n"
                             "======================\n";

        StringProducer    sProd;
        StringRefProducer srProd;
      ...
      case 9: {
        // --------------------------------------------------------------------
        // TESTING: conversions from 'bslstl::StringRef'
        //
        // Concerns:
        //   That conversions to and from 'bdlb::StringRef' all work
        //   properly. We are concerned with 3 other types:
        //
        //: 1 'bslstl::StringRrf'
        //:
        //: 2 'bsl::string'
        //:
        //: 3 'native_std::string'
        //   Note that all testing here is redundant with other tests.
        //
        // Plan:
        //: 1 Assign through 'operator=' to all 4 types from
        //:   'bslstl::StringRef'.
        //:
        //: 2 Assign through 'operator=' to all 4 types with casting from
        //:   'bslstl::StringRef'.
        //:
        //: 3 Assign through 'operator=' to all 4 types from
        //:   'bdlb::StringRef'.
        //:
        //: 4 Assign through 'operator=' to all 4 types with casting from
        //:   'bdlb::StringRef'.
        //:
        //: 5 Assign through 'operator=' to 'bdlb::StringRef' from all 4
        //:   types.
        //:
        //: 6 Assign through 'assign' to 'bdlb::StringRef' from all 4 types.
        //
        // --------------------------------------------------------------------

        if (verbose) cout << "Construct our 'bslstl::StringRef\n";

        const bsl::string coreStr = "woof";
        const bslstl::StringRef srRhs(coreStr);
        bslstl::StringRef sr(coreStr);

        if (verbose) cout << "Construct things starting with 'sr'\n";

        bsl::string str(srRhs);
        ASSERT("woof" == str);

        native_std::string strN(srRhs);
        ASSERT("woof" == strN);

        const bdlb::StringRef dsrRhs(sr);
        bdlb::StringRef dsr(str);
        bdlb::StringRef dsrN(strN);
        bdlb::StringRef dsrC(dsrRhs);
        ASSERT("woof" == dsr);
        ASSERT("woof" == dsrRhs);
        ASSERT("woof" == dsrN);
        ASSERT("woof" == dsrC);
        ...
        if (verbose) cout << "Assign through '=' to things from 'srRhs'\n";
        {
            str = srRhs;
            ASSERT("woof" == str);
            str.clear();
        ...
        if (verbose) cout << "Assign to things from 'sr' with cast\n";
        {
            str = static_cast<bsl::string>(srRhs);
            ASSERT("woof" == str);
            str.clear();

            strN = static_cast<native_std::string>(srRhs);
            ASSERT("woof" == strN);
            strN.clear();

            dsr = static_cast<bdlb::StringRef>(srRhs);
            ASSERT("woof" == dsr);
            dsr.unbind();
        }

        if (verbose) cout << "Assign through '=' to things from 'dsrRhs'\n";
        {
            str = dsrRhs;
            ASSERT("woof" == str);
            str.clear();
        ...
        if (verbose) cout << "Assign to things from 'dsrRhs' with cast\n";
        {
            str = static_cast<bsl::string>(dsrRhs);
            ASSERT("woof" == str);
            str.clear();

            strN = static_cast<native_std::string>(dsrRhs);
            ASSERT("woof" == strN);
            strN.clear();

            dsr = static_cast<bdlb::StringRef>(dsrRhs);
            ASSERT("woof" == dsr);
        }

        if (verbose) cout << "Assign through '=' to 'dsr' from things\n";
        {
            dsr = srRhs;
            ASSERT("woof" == dsr);
            dsr.unbind();
        ...
        if (verbose) cout << "Assign through 'assign' to 'dsr' from things\n";
        {
            dsr.assign(srRhs);
            ASSERT("woof" == dsr);
            dsr.unbind();
      ...
      case 8: {
        ...
        //
        // Testing:
        //   bsl::hash<BloombergLP::bdlb::StringRef>
        // --------------------------------------------------------------------
        ...
        static const struct {
          ...
          //{ L_,  "B"                          },
            ...
            // $ cat /etc/passwd | grep -i bde | BEGIN{FS=":"} { print $5 } |
        ...
        map<Obj, bsl::size_t> hash_results;
        map<bsl::size_t, int> hash_value_counts;
        ...
        for (int ti = 0; ti < NUM_DATA; ++ti) {
            ...
            bsl::size_t hash_value = hash_function(o);
        ...
        for (map<bsl::size_t, int>::iterator i  = hash_value_counts.begin();
                                             i != hash_value_counts.end();
                                             ++i) {
      ...
      case 6: {
                 ...
                 << endl;
        {
          ...
          ASSERT(NES.isEmpty() == 0);
          ASSERT(NES.length()  != 0);
          ASSERT(NES.begin()   != NES.end());

          nes.clear();

          ASSERT(NES.isEmpty() == 1);
          ASSERT(NES.length()  == 0);
          ASSERT(NES.begin()   == NES.end());
                 ...
                 << endl;
        {
          ...
          Obj x1(EMPTY_STRING, EMPTY_STRING + bsl::strlen(EMPTY_STRING));
          const Obj& X1 = x1;
          ASSERT(X1.isEmpty() == 1);
          ASSERT(X1.length()  == 0);
          ASSERT(X1.begin()   == X1.end());
          ASSERT(X1.begin()   == EMPTY_STRING);
          ASSERT(X1.end()     == EMPTY_STRING + bsl::strlen(EMPTY_STRING));

          x1.assign(NON_EMPTY_STRING,
                    NON_EMPTY_STRING + bsl::strlen(NON_EMPTY_STRING));
                 ...
                 << endl;
        {
          ...
          x1.assign(NON_EMPTY_STRING, bsl::strlen(NON_EMPTY_STRING));
                 ...
                 << endl;
        {
          ...
          x1.assign(NULL_STRING);

          ASSERT(X1.isEmpty() == 1);
          ASSERT(X1.length()  == 0);
          ASSERT(X1.begin()   == X1.end());
          ASSERT(X1.begin()   == NULL_STRING);
          ASSERT(X1.end()     == NULL_STRING);
          ...
          x2.assign(NULL_STRING);

          ASSERT(X2.isEmpty() == 1);
          ASSERT(X2.length()  == 0);
          ASSERT(X2.begin()   == X2.end());
          ASSERT(X2.begin()   == NULL_STRING);
          ASSERT(X2.end()     == NULL_STRING);

          // Empty string to non-empty string
          Obj x3(EMPTY_STRING, EMPTY_STRING + bsl::strlen(EMPTY_STRING));
          const Obj& X3 = x3;
          ASSERT(X3.isEmpty() == 1);
          ASSERT(X3.length()  == 0);
          ASSERT(X3.begin()   == X3.end());
          ASSERT(X3.begin()   == EMPTY_STRING);
          ASSERT(X3.end()     == EMPTY_STRING + bsl::strlen(EMPTY_STRING));
      ...
      case 4: {
        ...
        //
        // Testing:
        //   operator<<(ostream&, const bdlb::StringRef& string);
        // --------------------------------------------------------------------
...

        {
          ...
          if (veryVerbose) bsl::cout << "\tEXPECTED FORMAT: "
                                     << static_cast<bsl::string>(ES)
                                     << bsl::endl;
          ostrstream out(buf, SIZE);  out << ES << ends;
          if (veryVerbose) bsl::cout << "\tACTUAL FORMAT:   "
                                     << buf << bsl::endl;

          const int ESSZ = ES.length() + 1;
          ASSERT(ESSZ < SIZE);           // Check buffer is large enough.
          ASSERT(XX  == buf[SIZE - 1]);  // Check for overrun.
          ASSERT(0   == memcmp(buf, ES.data(), ESSZ - 1));
          ASSERT(0   == memcmp(buf + ESSZ, CTRL_BUF + ESSZ, SIZE - ESSZ));
          ...
          if (veryVerbose) bsl::cout << "\tEXPECTED FORMAT: "
                                     << NON_EMPTY_STRING << bsl::endl;
          ostrstream nesOut(buf, SIZE);  nesOut << NES << ends;
          if (veryVerbose) bsl::cout << "\tACTUAL FORMAT:   "
                                     << buf << bsl::endl;
          ...
          ASSERT(XX   == buf[SIZE - 1]);  // Check for overrun.
          ASSERT(0    == memcmp(buf, NES.data(), NESSZ - 1));
          ASSERT(0    == memcmp(buf + NESSZ, CTRL_BUF + NESSZ, SIZE-NESSZ));
          ...
          if (veryVerbose) bsl::cout << "\tEXPECTED FORMAT: "
                                     ...
                                     << bsl::endl;
          ostrstream nessOut(buf, SIZE);  nessOut << NESS << ends;
          if (veryVerbose) bsl::cout << "\tACTUAL FORMAT:   "
                                     << buf << bsl::endl;
          ...
          ASSERT(XX   == buf[SIZE - 1]);  // Check for overrun.
          ASSERT(0    == memcmp(buf, NESS.data(), NESSSZ - 1));
          ASSERT(0    == memcmp(buf + NESSZ, CTRL_BUF + NESSZ, SIZE-NESSZ));
        }
      } break;
      case 3: {
                          ...
                          << endl;
        {
          ...
          ASSERT(ES.length()  == (int)bsl::strlen(EMPTY_STRING));
          ASSERT(ES.isEmpty() == true);
          ...
          // NON-EMPTY STRING
          Obj nes(NON_EMPTY_STRING);  const Obj& NES = nes;
          int LEN = bsl::strlen(NON_EMPTY_STRING);
...
// TBD negative test
      ...
      case 2: {
        ...
        // Testing:
        //   bdlb::StringRef();
        //   bdlb::StringRef(const char *begin, const char *end);
        //   bdlb::StringRef(const char *begin, int length);
        //   bdlb::StringRef(const char *begin);
        //   bdlb::StringRef(const bsl::string& begin);
        //   bdlb::StringRef(const native_std::string& begin);
        //   bdlb::StringRef(const bdlb::StringRef& original);
        //   ~bdlb::StringRef();
        //   bdlb::StringRef& operator=(const bdlb::StringRef& rhs);
        // --------------------------------------------------------------------
        ...
        if (verbose) cout << "\nTesting other constructors"
                          << "\n= = = = = = = = = = = = = " << endl;
        if (veryVerbose)
            cout << "\nbdeut_StringRef(const char *begin, const char *end)"
                 << "\n=  =  =  =  =  =  =  =  =  =  =  =  =  =  =  =  =  "
                 << endl;
        {
          // Empty string
        ...
        if (veryVerbose)
            cout << "\nbdeut_StringRef(const char *begin, int length)"
                 << "\n=  =  =  =  =  =  =  =  =  =  =  =  =  =  =  ="
                 << endl;

        {
          // Empty string
          Obj x1(EMPTY_STRING, bsl::strlen(EMPTY_STRING));  const Obj& X1 = x1;
          ASSERT(X1.isEmpty() == 1);
          ASSERT(X1.length()  == 0);
          ASSERT(X1.begin()   == X1.end());
          ASSERT(X1.begin()   == EMPTY_STRING);
          ...
          // Non-empty string
          Obj x2(NON_EMPTY_STRING, bsl::strlen(NON_EMPTY_STRING));
          const Obj& X2 = x2;
          ASSERT(X2.isEmpty() == 0);
          ASSERT(X2.length()  == 30);
          ASSERT(X2.begin()   != X2.end());
        ...
        if (veryVerbose)
            cout << "\nbdeut_StringRef(const char *begin)"
                 << "\n=  =  =  =  =  =  =  =  =  =  =  ="
                 << endl;

        {
          // Null string
          Obj x(NULL_STRING);  const Obj& X = x;
          ASSERT(X.isEmpty() == 1);
          ASSERT(X.length()  == 0);
          ASSERT(X.begin()   == X.end());
          ASSERT(X.begin()   == NULL_STRING);
          ASSERT(X.end()     == NULL_STRING);
        ...
        if (veryVerbose)
            cout << "\nbdeut_StringRef(const bsl::string& begin)"
                 << "\n=  =  =  =  =  =  =  =  =  =  =  =  =  = "
                 << endl;
        ...
        if (veryVerbose)
            cout << "\nbdeut_StringRef(const native_std::string& begin)"
                 << "\n  =  =  =  =  =  =  =  =  =  =  =  =  =  =  =  ="
                 << endl;

        {
          // Empty string
          native_std::string emptyString(EMPTY_STRING);
          Obj x1(emptyString);  const Obj& X1 = x1;
          ASSERT(X1.isEmpty()   == 1);
          ASSERT(X1.length()    == 0);
          ASSERT(X1.begin()     == X1.end());

// TBD: When built under optimized mode on IBM, the following assert fails.
...
// assembly code to verify what is happening.
        ...
        if (verbose) cout << "\nTesting copy constructor"
                          << "\n= = = = = = = = = = = = " << endl;
        if (veryVerbose)
            cout << "\nbdeut_StringRef(const bdlb::StringRef& original)"
                 << "\n=  =  =  =  =  =  =  =  =  =  =  =  =  =  =  =  "
                 << endl;
        {
          // Empty string
          ...
          ASSERT(XC1.isEmpty() == 1);
          ASSERT(XC1.length()  == 0);
          ASSERT(XC1.begin()   == XC1.end());
          ASSERT(XC1.begin()   == EMPTY_STRING);
          ASSERT(XC1.end()     == EMPTY_STRING + bsl::strlen(EMPTY_STRING));
