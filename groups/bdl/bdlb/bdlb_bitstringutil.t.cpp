    ...
    // least 'index + bsl::strlen(ascii)' bits.
{
    ...
    while (numChars > 0) {
        ...
        bdlb::BitUtil::replaceBitValue(intPtr, pos, currValue - '0');
        ++pos;
        --numChars;
    }
}
   ...
   public:
     ...
     void IntArray::resizeNullnessBitsArray()
     {
         ...
         if (d_nullBitsArrayLen > 0) {
             bdlb::BitstringUtil::copyRaw(newBitArray,
                                         0,
                                         d_nullBits_p,
                                         0,
     ...
     int& IntArray::operator[](int index)
     {
         ...
         bdlb::BitstringUtil::set(d_nullBits_p, index, false);
     ...
     void IntArray::insertInt(int index, int value)
     {
         ...
         bdlb::BitstringUtil::insert0(d_nullBits_p,
                                         length() - 1,
                                         index,
                                         1);
     }
     ...
     void IntArray::insertNullInt(int index)
     {
         ...
         bdlb::BitstringUtil::insert1(d_nullBits_p,
                                         length() - 1,
                                         index,
                                         1);
     }
     ...
     void IntArray::makeNull(int index)
     {
         ...
         bdlb::BitstringUtil::set(d_nullBits_p, index, true);
     }
     ...
     {
         ASSERT(0 <= index && index < length());
         return bdlb::BitstringUtil::get(d_nullBits_p, index);
     }
...
//..
    ...
    for (int i = 2 /* first Sat */; i < 366; i += 7) {
        bdlb::BitstringUtil::set(weekends, i, 1);
        bdlb::BitstringUtil::set(weekends, i+1, 1);
    }
//..
// Now, we can easily use 'bdeu_bitstreamutil' methods to find days of
// interest.  For example:
//..
    int firstWeekendDay = bdlb::BitstringUtil::find1AtSmallestIndex(
                                                                  weekends,
                                                                  365 + 1);
    int lastWeekendDay  = bdlb::BitstringUtil::find1AtLargestIndex(
                                                                  weekends,
                                                                  365 + 1);
//..
...
//..
    ...
    bdlb::BitstringUtil::set(holidays, NEW_YEARS_DAY,    1);
    bdlb::BitstringUtil::set(holidays, MLK_DAY,          1);
    bdlb::BitstringUtil::set(holidays, PRESIDENTS_DAY,   1);
    bdlb::BitstringUtil::set(holidays, GOOD_FRIDAY,      1);
    bdlb::BitstringUtil::set(holidays, MEMORIAL_DAY,     1);
    bdlb::BitstringUtil::set(holidays, INDEPENDENCE_DAY, 1);
    bdlb::BitstringUtil::set(holidays, LABOR_DAY,        1);
    bdlb::BitstringUtil::set(holidays, THANKSGIVING,     1);
    bdlb::BitstringUtil::set(holidays, CHRISTMAS,        1);
...
// if any:
//..
    int firstHolidayOfQ3 = bdlb::BitstringUtil::find1AtSmallestIndexGT(holidays,
                                                                      Q4,
                                                                      Q3);
...
//..
    ...
    int isOffMay07 = bdlb::BitstringUtil::get(allDaysOff, MAY +  7);
    int isOffMay08 = bdlb::BitstringUtil::get(allDaysOff, MAY +  8);
    int isOffMay09 = bdlb::BitstringUtil::get(allDaysOff, MAY +  9);
    int isOffMay10 = bdlb::BitstringUtil::get(allDaysOff, MAY + 10);
    int isOffXmas  = bdlb::BitstringUtil::get(allDaysOff, CHRISTMAS);
    ...
    case 5: {
     ...
     "11110000 11110000 11110000 11110000 11110000 11110000 11110000 11110000",
      ...
      for (int i = 0; i < NUM_DATA; ++i) {
          ...
          exp = bdlb::BitUtil::oneMask(0, NUM_BITS);
          LOOP_ASSERT(LINE, exp == BSU::get(bits, INDEX, NUM_BITS));
