...
#include <bdlimpxxx_fuzzy.h>                // for testing only
...
//                              Overview
//                              --------
// This driver tests parsing and generating functions of 'bdlpuxxx::TypesParser'.
// Each parsing function has different constrains, which are described in the
// function documentation in 'bdlpuxxx_typesparser.h'.  However, each of the
// parsed strings representing basic types may be preceded by arbitrary
// <WHITESPACE>, which is also defined in 'bdlpuxxx_typesparser.h'.  Array types
...
#define INVOKE_testParseErrorInput(TNAME, FNAME) \
    testParseErrorInput<TNAME>( \
                   bdlpuxxx::TypesParser::parse##FNAME, \
...
#define INVOKE_testParseArray(TNAME, FNAME) \
    testParseArray<TNAME, SampleValuesParse##FNAME>( \
                   bdlpuxxx::TypesParser::parse##FNAME##Array, \
                   DATA, \
                   VALUES, \
                   DATA_ERRORS, \
                   NUM_DATA_ERRORS, \
...
#define INVOKE_testGenerateR(TNAME, FNAME) \
    testGenerate<TNAME, const TNAME&, SampleValuesGenerate##FNAME>( \
                   bdlpuxxx::TypesParser::generate##FNAME, \
                   bdlpuxxx::TypesParser::generate##FNAME##Raw, \
                   bdlpuxxx::TypesParser::parse##FNAME, \
                   (int (*)(const char **, TNAME *, const char *, int))0, \
                   DATA, \
                   VALUES, \
...
#define INVOKE_testGenerateArray(TNAME, FNAME) \
    testGenerateArray<TNAME, SampleValuesGenerate##FNAME>( \
                   bdlpuxxx::TypesParser::generate##FNAME##Array, \
                   bdlpuxxx::TypesParser::generate##FNAME##ArrayRaw, \
                   bdlpuxxx::TypesParser::parse##FNAME##Array, \
                   DATA, \
                   VALUES, \
                   __LINE__)
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      case 29: {
        ...
        bsl::vector<bdlt::DatetimeTz> VALUES(testAllocator_p);
        VALUES.reserve(NUM_DATA);
        ...
        for (int k = 0; k < NUM_DATA; ++k) {
            ...
            const bdlt::Datetime VALUEDT(YEAR,
                                        MONTH,
                                        DAY,
                                        HOUR,
                                        MINUTE,
                                        SECOND,
                                        MILLI);
            const bdlt::DatetimeTz VALUE(VALUEDT, OFFSET);

            VALUES.push_back(VALUE);
        }

        INVOKE_testGenerateR(bdlt::DatetimeTz, DatetimeTz);
        INVOKE_testGenerateArray(bdlt::DatetimeTz, DatetimeTz);
      } break;
      case 28: {
        // --------------------------------------------------------------------
        ...
        bsl::vector<bdlt::TimeTz> VALUES(testAllocator_p);
        VALUES.reserve(NUM_DATA);
        ...
        for (int k = 0; k < NUM_DATA; ++k) {
            ...
            const bdlt::Time VALUET(HOUR, MINUTE, SECOND, MILLI);
            const bdlt::TimeTz VALUE(VALUET, OFFSET);

            VALUES.push_back(VALUE);
        }

        INVOKE_testGenerateR(bdlt::TimeTz, TimeTz);
        INVOKE_testGenerateArray(bdlt::TimeTz, TimeTz);
      } break;
      case 27: {
        // --------------------------------------------------------------------
        ...
        bsl::vector<bdlt::DateTz> VALUES(testAllocator_p);
        VALUES.reserve(NUM_DATA);
        ...
        for (int k = 0; k < NUM_DATA; ++k) {
            ...
            const bdlt::Date VALUED(YEAR, MONTH, DAY);
            const bdlt::DateTz VALUE(VALUED, OFFSET);

            VALUES.push_back(VALUE);
        }

        INVOKE_testGenerateR(bdlt::DateTz, DateTz);
        INVOKE_testGenerateArray(bdlt::DateTz, DateTz);
      } break;
      case 26: {
        // --------------------------------------------------------------------
        ...
        bsl::vector<bdlt::DatetimeTz> VALUES(&testAllocator);
        VALUES.reserve(NUM_DATA);
        ...
        INVOKE_testParseErrorInput(bdlt::DatetimeTz, DatetimeTz);

        INVOKE_testParseValidInput(bdlt::DatetimeTz, DatetimeTz);

        INVOKE_testParseArray(bdlt::DatetimeTz, DatetimeTz);
      } break;
      case 25: {
        // --------------------------------------------------------------------
        ...
        bsl::vector<bdlt::TimeTz> VALUES(&testAllocator);
        VALUES.reserve(NUM_DATA);
        ...
        for (int ti = 0; ti < NUM_DATA ; ++ti) {
            ...
            const bdlt::Time VALUET(HOUR, MINS, SECS, MILLI);
            const bdlt::TimeTz VALUE(VALUET, OFFSET);
        ...
        INVOKE_testParseErrorInput(bdlt::TimeTz, TimeTz);

        INVOKE_testParseValidInput(bdlt::TimeTz, TimeTz);

        INVOKE_testParseArray(bdlt::TimeTz, TimeTz);
      } break;
      case 24: {
        // --------------------------------------------------------------------
        ...
        bsl::vector<bdlt::DateTz> VALUES(&testAllocator);
        VALUES.reserve(NUM_DATA);
        ...
        INVOKE_testParseErrorInput(bdlt::DateTz, DateTz);

        INVOKE_testParseValidInput(bdlt::DateTz, DateTz);

        INVOKE_testParseArray(bdlt::DateTz, DateTz);
      } break;
      case 23: {
        // --------------------------------------------------------------------
        ...
        bsl::vector<bdlt::Time> VALUES(testAllocator_p);
        VALUES.reserve(NUM_DATA);
        ...
        for (int k = 0; k < NUM_DATA; ++k) {
            ...
            const bdlt::Time VALUE(HOUR, MINUTE, SECOND, MILLI);

            VALUES.push_back(VALUE);
        }

        INVOKE_testGenerateR(bdlt::Time, Time);
        INVOKE_testGenerateArray(bdlt::Time, Time);
      } break;
      case 22: {
        // --------------------------------------------------------------------
...
// [20] generateIntegerLocatorRaw(b_ca *, int val, int level, int spaces);
      ...
      case 15: {
        ...
        SampleValuesGenerateDatetime DATA[] = {
            ...
            { L_,    1,  1,  1, 24,   0,   0,   0, "0001/01/01 24:00:00.000" },
        };
        const int NUM_DATA = sizeof DATA / sizeof *DATA;

        bsl::vector<bdlt::Datetime> VALUES(testAllocator_p);
        VALUES.reserve(NUM_DATA);
        ...
        for (int k = 0; k < NUM_DATA; ++k) {
            ...
            const bdlt::Datetime VALUE(YEAR,
                                      MONTH,
                                      DAY,
                                      HOUR,
                                      MINUTE,
        ...
        INVOKE_testGenerateR(bdlt::Datetime, Datetime);
        INVOKE_testGenerateArray(bdlt::Datetime, Datetime);
      } break;
      case 14: {
        // --------------------------------------------------------------------
        ...
        bsl::vector<bdlt::Date> VALUES(testAllocator_p);
        VALUES.reserve(NUM_DATA);
        ...
        INVOKE_testGenerateR(bdlt::Date, Date);
        INVOKE_testGenerateArray(bdlt::Date, Date);
      } break;
      case 13: {
        // --------------------------------------------------------------------
      ...
      case 12: {
...

        {
            ...
            for (int ti = 0; ti < NUM_DATA ; ++ti) {
                ...
                bdlpuxxx::TypesParser::stripNull(&result);
      ...
      case 11: {
        ...
        bsl::vector<bdlt::Time> VALUES(&testAllocator);
        VALUES.reserve(NUM_DATA);
        ...
        for (int ti = 0; ti < NUM_DATA ; ++ti) {
            ...
            const bdlt::Time VALUE(HOUR, MINS, SECS, MILLI);
            VALUES.push_back(VALUE);
        }
        ASSERT(NUM_DATA == VALUES.size());

        INVOKE_testParseErrorInput(bdlt::Time, Time);

        INVOKE_testParseValidInput(bdlt::Time, Time);

        INVOKE_testParseArray(bdlt::Time, Time);
      } break;
      case 10: {
        // TBD
// [10] parseString(cchar **endPos, bStr *res, cchar *in);
...
// [ 8] parseIntegerLocator(cchar **endPos, int *result, cchar *inputString);
      ...
      case 3: {
        ...
        const bdlt::Datetime INITIAL_VALUE_1(2000, 1, 1, 1, 1, 1, 1);
        const bdlt::Datetime INITIAL_VALUE_2(1000, 1, 1, 1, 1, 0, 0);
        ...
        bsl::vector<bdlt::Datetime> VALUES(&testAllocator);
        VALUES.reserve(NUM_DATA);
        ...
        INVOKE_testParseErrorInput(bdlt::Datetime, Datetime);

        INVOKE_testParseValidInput(bdlt::Datetime, Datetime);

        INVOKE_testParseArray(bdlt::Datetime, Datetime);
      } break;
      case 2: {
        // --------------------------------------------------------------------
        // TESTING PARSE DATE
        ...
        bsl::vector<bdlt::Date> VALUES(&testAllocator);
        VALUES.reserve(NUM_DATA);
        ...
        INVOKE_testParseErrorInput(bdlt::Date, Date);

        INVOKE_testParseValidInput(bdlt::Date, Date);

        INVOKE_testParseArray(bdlt::Date, Date);
      } break;
      case 1: {
        // --------------------------------------------------------------------
        // TESTING PARSE CHAR
