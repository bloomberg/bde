...
//                              Overview
//                              --------
// 'baetzo_ZoneinfoUtil' is a utility for performing operations using a
// 'baetzo_Zoneinfo' value.  This test driver tests each implemented utility
...
// [ 2] bool isWellFormed(const baetzo_Zoneinfo& timeZone);
// ----------------------------------------------------------------------------
// [ 1] BREATHING TEST
// [ 5] USAGE EXAMPLE
...
//-----------------------------------------------------------------------------
typedef baetzo_ZoneinfoUtil                      Obj;
typedef baetzo_Zoneinfo                          Tz;
typedef baetzo_Zoneinfo::TransitionConstIterator TzIt;
    ...
    // " *-?HH:MM:SS.mmm".
{
    ...
    bdet_Time time;
    int rc = bdepu_Iso8601::parse(&time, iso8601Value, length);
...
int main(int argc, char *argv[])
{
    ...
    const bdetu_Epoch::TimeT64 MAX_DATETIME =
                         toTimeT(bdet_Datetime(9999, 12, 31, 23, 59, 59, 999));

    const Validity::Enum U = baetzo_LocalTimeValidity::BAETZO_VALID_UNIQUE;
    const Validity::Enum A = baetzo_LocalTimeValidity::BAETZO_VALID_AMBIGUOUS;
    const Validity::Enum I = baetzo_LocalTimeValidity::BAETZO_INVALID;
...
//..
     ...
     case 5: {
        ...
        }
        {
            ...
            const bdet_Datetime TRANS_TIME(2000, 1, 1);
            ...
            for (int i = 0; i < NUM_DIFFERENCES; ++i) {
                ...
                bdet_Datetime minTime = PRE_TRANSITION < POST_TRANSITION
                                        ? PRE_TRANSITION
                                        : POST_TRANSITION;
                bdet_Datetime maxTime = PRE_TRANSITION > POST_TRANSITION
                                        ? PRE_TRANSITION
                                        : POST_TRANSITION;
                ...
                for (int j = 0; j < NUM_EPSILONS; ++j) {
                    ...
                    bdet_Datetime TEST_PRE(PRE_TRANSITION);
                    bdet_Datetime TEST_POST(POST_TRANSITION);
                    TEST_PRE.addMilliseconds(ADJUST);
                    TEST_POST.addMilliseconds(ADJUST);

                    bdet_Datetime VALUES[2] = { TEST_PRE, TEST_POST };

                    for (int k = 0; k < 2; ++k) {
                        const bdet_Datetime& VALUE = VALUES[k];
                        ...
                        if (VALUE < minTime || VALUE >= maxTime) {
                            ...
                            // time.
                            bdet_Datetime utcTime1(VALUE), utcTime2(VALUE);
                            utcTime1.addSeconds(
                                -resultIt1->descriptor().utcOffsetInSeconds());
                            utcTime2.addSeconds(
        ...
        }
        {
            ...
            for (int i = 1; i < NUM_DATA; ++i) {
                const bdet_Datetime UTC_TRANSITION(
                ...
                bdet_Datetime PRE_TRANSITION(UTC_TRANSITION);
                PRE_TRANSITION.addMinutes(prevOffset);
                bdet_Datetime POST_TRANSITION(UTC_TRANSITION);
                ...
                bsl::vector<bdetu_Epoch::TimeT64> TEST_OFFSETS(Z);
                for (int j = 0; j < NUM_EPSILONS; ++j) {
                ...
                for (int j = 0; j < TEST_OFFSETS.size(); ++j) {
                    bdet_Datetime VALUE(UTC_TRANSITION);
        ...
        }
        {
            ...
            const bdet_Datetime TEST_TIMES[] = {
                bdet_Datetime(1, 1, 1),
                bdet_Datetime(1, 1, 2),
                bdet_Datetime(2, 1, 1),
                bdet_Datetime(1000, 5, 31),
                bdet_Datetime(2011, 3, 13, 7),
                bdet_Datetime(2012, 1, 1),
                bdet_Datetime(9999, 1, 1),
                bdet_Datetime(9999, 3, 1),
            ...
            for (int i = 0; i < NUM_TEST_TIMES; ++i) {
                const bdet_Datetime TEST_TIME = TEST_TIMES[i];
      ...
      case 4: {
        ...
        //   'baetzo_Zoneinfo::findTransitionForUtcTime' correctly finds that
                          ...
                          << "================================" << endl;
        {
            ...
            for (int i = 0; i < NUM_DATA; ++i) {
                ...
                for (int offset = -5; offset < 6; ++offset) {
                    bdet_Datetime value(toDatetime(DATA[i].d_transitionTime));
                    const bdet_Datetime& VALUE = value;

                    if (value == bdet_Datetime(1, 1, 1) && offset < 0) {
                        continue;
                    }
        ...
        }
        {
            ...
            for (int i = 0; i < NUM_DATA; ++i) {
                ...
                const bdet_Datetime TIME(2000, 1, 1);
                bdetu_Epoch::TimeT64 TIME_T = toTimeT(TIME);

                Tz tz(Z); const Tz& TZ = tz;
                tz.addTransition(MIN_DATETIME, Desc(0, false, ""));
                tz.addTransition(TIME_T, Desc(OFFSET * 60, false, ""));

                bdet_Datetime expLocal = TIME;
      ...
      case 3: {
        ...
        //   'baetzo_Zoneinfo' meets the definition of a "well-defined' value.
        //
        // Concerns:
        ...
        //:     the minimum representable 'bdet_Datetime' value.
        //:
        ...
        //:
        //:     2 If the newly added transition is at 'bdet_Datetime(1, 1, 1)'
        ...
        baetzo_LocalTimeDescriptor desc(0, false, "junk", Z);
        enum {
        ...
        }
        {
            ...
            const bdet_Datetime DATA[] = {
                bdet_Datetime(1, 1, 1),
                bdet_Datetime(1, 1, 2),
                bdet_Datetime(2, 1, 1),
                bdet_Datetime(9999, 12, 31, 23, 59, 59, 999),
                bdet_Datetime(1, 1, 1),
                bdet_Datetime(9999,  1,  1, 1),
                bdet_Datetime(1000,  1,  1, 1),
                bdet_Datetime(2011,  3, 13, 7),
            };
            const int NUM_DATA = sizeof DATA / sizeof *DATA;
            ...
            for (int i = 0; i < NUM_DATA; ++i) {
                ...
                for (int j = 0; j < NUM_DATA; ++j) {
                    ...
                    if (bdet_Datetime(1, 1, 1) == DATA[INDEX]) {
                        VALID = true;
                    }
                    bdetu_Epoch::TimeT64 timeT = toTimeT(DATA[INDEX]);
        ...
        { L_, "-23:59:59.000", " 23:59:59.000", 1440, "-23:59:59.000", false },
            ...
            for (int i = 0; i < NUM_DATA; ++i) {
                ...
                const bdetu_Epoch::TimeT64 T0 = MIN_DATETIME;
                const bdetu_Epoch::TimeT64 T1 = toTimeT(
                                                    bdet_Datetime(1000, 1, 1));
                const bdetu_Epoch::TimeT64 T2 = T1 + (INTERVAL_M * 60);
                ...
                if (veryVeryVerbose) {
                    P(bdetu_Epoch::convertFromTimeT64(T0));
                    P(bdetu_Epoch::convertFromTimeT64(MIN_DATETIME));
                    P_(T0); P_(T1); P(T2);
                    P_(UTC_OFFSET_T0_S);
                    P_(UTC_OFFSET_T1_S);
                    P(UTC_OFFSET_T2_S);
        ...
        }
        {
            ...
            Tz tz(Z); const Tz& TZ = tz;
            tz.addTransition(toTimeT(bdet_Datetime(1000, 1, 1)),
                             Desc(1439 * 60, 0, "A"));
            tz.addTransition(toTimeT(bdet_Datetime(1000, 1, 1, 1)),
                             Desc(-1439 * 60, 0, "B"));
            ASSERT(false == Obj::isWellFormed(TZ));
            tz.addTransition(toTimeT(bdet_Datetime(1, 1, 1)),
                             Desc(0, 0, "C"));
            ASSERT(false == Obj::isWellFormed(TZ));
        }
      } break;
        ...
        // Testing:
        //   int toOffsetInMilliseconds(const char *iso8601Value)
        //   void addTransitions(baetzo_Zoneinfo *, TransDescription *, int);
        // --------------------------------------------------------------------
      case 2: {
      ...
      case 1: {
        ...
        newYork.setIdentifier("America/New_York");
        newYork.addTransition(toTimeT(bdet_Datetime(1, 1, 1)), est);
        static const bdet_Datetime TRANSITION_TIMES[] = {
            bdet_Datetime(2007,  3, 11, 7),
            bdet_Datetime(2007, 11,  4, 6),
            bdet_Datetime(2008,  3,  9, 7),
        ...
        for (int ti = 0; ti < NUM_DATA_UTC; ++ti) {
                ...
                bdet_Datetime x(YEAR, MONTH, DAY, HOUR, MINUTE, SECOND);
                const bdet_Datetime& X = x;

                bdet_DatetimeTz y;
                const bdet_DatetimeTz& Y = y;

                baetzo_Zoneinfo::TransitionConstIterator r;
                const baetzo_Zoneinfo::TransitionConstIterator& R = r;
        ...
        for (int ti = 0; ti < NUM_DATA_LOCAL; ++ti) {
                ...
                baetzo_Zoneinfo::TransitionConstIterator y;
                const baetzo_Zoneinfo::TransitionConstIterator& Y = y;
                baetzo_Zoneinfo::TransitionConstIterator z;
                const baetzo_Zoneinfo::TransitionConstIterator& Z = z;
                baetzo_LocalTimeValidity::Enum validity;
        ...
        }
        {
            ...
            }
            {
                ...
                Tz mX(Z); const Tz& X = mX;
                baetzo_LocalTimeDescriptor desc(0, true, "", Z);

                bdet_Datetime invalid1(2000, 1, 1);
                bdet_Datetime invalid2(1, 1, 2);
                bdet_Datetime valid(1, 1, 1);
            ...
            }
            {
                ...
                Tz mX(Z); const Tz& X = mX;
                baetzo_LocalTimeDescriptor desc(0, true, "", Z);
                bdet_Datetime initial(1, 1, 1);
                bdet_Datetime t0(2000, 1, 1);
                bdet_Datetime t1(2000, 1, 2);
                bdet_Datetime t1Plus1Hour(2000, 1, 2, 1);
                bdet_Datetime t2(2001, 1, 1);
