...
// The 'baetzo_ZoneinfoTransition' is an unconstrained in-core (value-semantic)
...
//
// The 'baetzo_Zoneinfo' is a value semantic type containing, an 'identifier'
// string, a sequence of transitions (specified by
// 'baetzo_ZoneinfoTransition'), and the 'baetzo_LocalTimeDescriptor'
...
//
// ACCESSORS
// [ 4] const baetzo_LocalTimeDescriptor& descriptor() const;
// [ 4] bdetu_Epoch::TimeT64 utcTime() const;
//
// [ 6] ostream& print(ostream& s, int level = 0, int sPL = 4) const;
//
// FREE OPERATORS
// [ 5] bool operator==(const baetzo_ZoneinfoTransition& lhs, rhs);
// [ 5] bool operator!=(const baetzo_ZoneinfoTransition& lhs, rhs);
// [ 7] bool operator<(const baetzo_ZoneinfoTransition& lhs, rhs);
...
//
// CREATORS
// [ 2] explicit baetzo_Zoneinfo(bslma_Allocator *basicAllocator = 0);
// [11] baetzo_Zoneinfo(const baetzo_Zoneinfo&  original, *bA);
//
// MANIPULATORS
// [13] baetzo_Zoneinfo& operator=(const baetzo_Zoneinfo& rhs);
// [ 2] void addTransition(TimeT64 time, const baetzo_LTD& d);
// [ 9] void setIdentifier(const bdeut_StringRef& identifier);
// [12] void swap(baetzo_Zoneinfo& other);
...
//
// FREE OPERATORS
// [ 8] bool operator==(const baetzo_Zoneinfo& lhs, rhs);
// [ 8] bool operator!=(const baetzo_Zoneinfo& lhs, rhs);
// [10] bsl::ostream& operator<<(ostream& stream, const baetzo_Zoneinfo&);
//
// FREE FUNCTIONS
// [12] void swap(baetzo_Zoneinfo& first, baetzo_Zoneinfo& second);
// ---------------------------------------------------------------------------
// [ 1] BREATHING TEST: 'baetzo_Zoneinfo', 'baetzo_ZoneinfoTransition'
// [15] USAGE EXAMPLE
...
typedef baetzo_Zoneinfo              Obj;
typedef baetzo_ZoneinfoTransition    Transition;
typedef Obj::TransitionConstIterator TransitionConstIter;

typedef baetzo_LocalTimeDescriptor   Descriptor;


typedef bdetu_Epoch::TimeT64         TimeT64;
...
// TBD: REMOVE THIS after it is moved to the test allocator.
// TBD: change the name to 'bslma_TestAllocatorMonitor'.
...
int ggg(Obj *object, const char *spec, int vF = 1)
    ...
    // the primary manipulator function.  Optionally specify a zero 'vF' to
...
//..
      ...
      case 14: {
        // --------------------------------------------------------------------
        // 'baetzo_Zoneinfo' 'findTransitionForUtcTime'
        ...
        //:     a 'bdet_Datetime' corresponding to the 'utcTime' in that row.
        //:
        ...
        for (int ti = 0; ti < NUM_DATA; ++ti) {
            ...
            const bdet_Datetime DT     = bdetu_Epoch::convertFromTimeT64(UTC);
        ...
        if (verbose) cout << "\nNegative Testing." << endl;
        {
            ...
            if (veryVerbose) cout << "\tWith no transitions" << endl;
            {
                ...
                const bdet_Datetime DT = bdetu_Epoch::convertFromTimeT64(1);
      ...
      case 13: {
        ...
        //:
        //: 2 Construct a 'bslma_TestAllocator' object and install it as the
        //:   default allocator (note that a ubiquitous test allocator is
        //:   already installed as the global allocator).
        //:
        ...
        //:
        //:     1 Construct a 'bslma_TestAllocator' object, 'oa'.
        //:
        ...
        //:
        //:       1 If the number of transitions in the the source object
        //:         are greater than the number of transitions in the target
        //:         object then the target DOES allocate memory from its
        //:         object allocator only (irrespective of the specific number
        //:         of allocations or the total amount of memory allocated);
        //:         also cross check with what is expected for 'mX' and 'Z'.
        //:
        //:       2 No additional memory is allocated by the source object.
        //:
        ...
        //
        // Testing:
        //   operator=(const baetzo_Zoneinfo& rhs);
        // --------------------------------------------------------------------
        ...
        for (int ti = 0; ti < NUM_DATA; ++ti) {
            ...
            const int NT1 = Z.numTransitions();
            ...
            for (int tj = 0; tj < NUM_DATA; ++tj) {
...

                {
                    ...
                    const int NT2 = X.numTransitions();
      ...
      case 11: {
        ...
        //:
        //:     1 Construct three 'bslma_TestAllocator' objects and install
        ...
        //
        // Testing:
        //   baetzo_Zoneinfo(const baetzo_Zoneinfo& o, *bA = 0);
        // --------------------------------------------------------------------
      ...
      case 10: {
        // --------------------------------------------------------------------
        // 'baetzo_Zoneinfo' 'setIdentifier' & 'identifier'
        ...
        //
        // Testing:
        //   void setIdentifier(const bdeut_StringRef& identifier);
      ...
      case 9: {
        // --------------------------------------------------------------------
        // 'baetzo_Zoneinfo' EQUALITY OPERATORS
        ...
        //
        // Testing:
        //   bool operator==(const baetzo_Zoneinfo& lhs, rhs);
        //   bool operator!=(const baetzo_Zoneinfo& lhs, rhs);
        // --------------------------------------------------------------------

        if (verbose) cout << endl
                          << "'baetzo_Zoneinfo' EQUALITY OPERATORS" << endl
                          << "====================================" << endl;
        ...
        for (int ti = 0; ti < NUM_DATA ; ++ti) {
            ...
            for (int tj = 0; tj < NUM_DATA ; ++tj) {
                ...
                for (char cfg = 'a'; cfg <= 'b'; ++cfg) {
                    ...
                    bslma_TestAllocator pa("p", veryVeryVeryVerbose);
                    bslma_TestAllocator qa("q", veryVeryVeryVerbose);

                    // Map allocators above to objects 'X' and 'Y' below.

                    bslma_TestAllocator& xa = pa;
                    bslma_TestAllocator& ya = 'a' == CONFIG ? pa : qa;
                    ...
                    bslma_TestAllocatorMonitor pam(pa), qam(qa);
      ...
      case 8: {
        // --------------------------------------------------------------------
        // 'baetzo_Zoneinfo' PRINT AND OUTPUT OPERATOR
        ...
        //   operator<<(ostream& s, const baetzo_Zoneinfo& d);
        // --------------------------------------------------------------------
...
#undef SP
                          ...
                          << endl;
        {
            for (int ti = 0; ti < NUM_DATA; ++ti) {
                ...
                bslma_TestAllocator oa("supplied", veryVeryVeryVerbose);
      ...
      case 7: {
        // --------------------------------------------------------------------
        // 'baetzo_ZoneinfoTransition' 'operator<'
        ...
        //
        // Testing:
        //   bool operator<(const baetzo_ZoneinfoTransition& lhs, rhs);
        // --------------------------------------------------------------------

        if (verbose) cout << endl
                          << "'baetzo_ZoneinfoTransition' 'operator<'" << endl
                          << "=======================================" << endl;
        ...
        if (verbose) cout << "\nTesting 'operator<'" << endl;
        {
            for (int ti = 0; ti < NUM_DATA ; ++ti) {
                ...
                for (int tj = 0; tj < NUM_DATA ; ++tj) {
                    ...
                    bslma_TestAllocatorMonitor oam1(oa1), oam2(oa2);
      ...
      case 6: {
        // --------------------------------------------------------------------
        // 'baetzo_ZoneinfoTransition' EQUALITY OPERATORS
        //   Ensure that '==' and '!=' are the operational definition of value.
        //
        // Concerns:
        //: 1 Two objects, 'X' and 'Y', compare equal if and only if each of
        ...
        if (verbose) cout << endl
                         << "'baetzo_ZoneinfoTransition' EQUALITY OPS" << endl
                         << "========================================" << endl;
      ...
      case 5: {
        // --------------------------------------------------------------------
        // 'baetzo_ZoneinfoTransition' PRINT AND OUTPUT OPERATOR
        //   Ensure that the value of the object can be formatted appropriately
        //   on an 'ostream' in some standard, human-readable form.
        //
        // Concerns:
        ...
        // Testing:
        //   ostream& print(ostream& s, int level = 0, sPL = 4) const;
        //   operator<<(ostream& s, const baetzo_ZoneinfoTransition& d);
        // --------------------------------------------------------------------
...
#undef SP
      ...
      case 4: {
        // --------------------------------------------------------------------
        // 'baetzo_ZoneinfoTransition' ACCESSORS
      ...
      case 2: {
        ...
        //
        // Testing:
        //   baetzo_Zoneinfo(bslma_Allocator *bA = 0);
        //   void addTransition(TimeT64 time, const baetzo_LTD& d);
        // --------------------------------------------------------------------
                          ...
                          << endl;
        {
            for (char cfg = 'a'; cfg <= 'c'; ++cfg) {
                ...
                // The default-constructed 'set' data member allocates a block.
      ...
      case 1: {
        // --------------------------------------------------------------------
        // BREATHING TEST: 'baetzo_Zoneinfo' & 'baetzo_ZoneinfoTransition'
        //
        // Concerns:
        ...
        if (verbose) cout << endl
                        << "BREATHING TEST: baetzo_ZoneinfoTransition" << endl
                        << "=========================================" << endl;

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // Values for testing

        const bdet_Datetime FIRST_DATETIME(1, 1, 1, 0, 0, 0);
        const TimeT64 FIRST_DATETIME64 =
                      bdetu_Epoch::convertToTimeT64(FIRST_DATETIME);
        ...
        const baetzo_LocalTimeDescriptor lta_a(OA_A, IDA_A, DA_A);
        const baetzo_LocalTimeDescriptor ltb_a(OB_A, IDB_A, DB_A);
        const baetzo_LocalTimeDescriptor ltc_a(OC_A, IDC_A, DC_A);

        const baetzo_LocalTimeDescriptor& LTA_A =  lta_a;
        const baetzo_LocalTimeDescriptor& LTB_A =  ltb_a;
        const baetzo_LocalTimeDescriptor& LTC_A =  ltc_a;

        const bdet_Datetime TA_A(1970,  1,  1,  0,  0,  0);
        const bdet_Datetime TB_A(2000,  3, 15,  3, 10, 15);
        const bdet_Datetime TC_A(2038,  1, 19,  3, 14,  7);

        const TimeT64 TTA_A =
                                bdetu_Epoch::convertToTimeT64(TA_A);
        const TimeT64 TTB_A =
                                bdetu_Epoch::convertToTimeT64(TB_A);
        const TimeT64 TTC_A =
                                bdetu_Epoch::convertToTimeT64(TC_A);
        ...
        baetzo_LocalTimeDescriptor lta_b(OA_B, IDA_B, DA_B);
        baetzo_LocalTimeDescriptor ltb_b(OB_B, IDB_B, DB_B);

        const baetzo_LocalTimeDescriptor& LTA_B =  lta_b;
        const baetzo_LocalTimeDescriptor& LTB_B =  ltb_b;

        const bdet_Datetime TA_B(2010,  3, 14,  2,  0,  0);
        const bdet_Datetime TB_B(2010, 11,  9,  2,  0,  0);

        const TimeT64 TTA_B = bdetu_Epoch::convertToTimeT64(TA_B);
        const TimeT64 TTB_B = bdetu_Epoch::convertToTimeT64(TB_B);
        ...
        baetzo_LocalTimeDescriptor lta_c(OA_C, IDA_C, DA_C);

        const baetzo_LocalTimeDescriptor& LTA_C = lta_c;

        const bdet_Datetime TA_C(2010,  3, 14,  2,  0,  0);

        const TimeT64 TTA_C = bdetu_Epoch::convertToTimeT64(TA_C);
