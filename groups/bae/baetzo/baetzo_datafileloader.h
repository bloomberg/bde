// baetzo_datafileloader.h                                            -*-C++-*-
#ifndef INCLUDED_BAETZO_DATAFILELOADER
#define INCLUDED_BAETZO_DATAFILELOADER
...
//@PURPOSE: provide a concrete 'baetzo_Loader' for Zoneinfo binary files
//
//@CLASSES:
//  baetzo_DataFileLoader: concrete 'baetzo_Loader' for Zoneinfo binary data
//
//@SEE_ALSO: 'baetzo_zoneinfobinaryreader', 'baetzo_zoneinfoutil'
//
//@AUTHOR: Stefano Pacifico (spacifico1), Henry Verschell (hvershell)
//
//@DESCRIPTION: This component provides a mechanism, 'baetzo_DataFileLoader',
// that is a concrete implementation of the 'baetzo_Loader' protocol for
// loading, into a 'baetzo_Zoneinfo' object, the properties of a time zone
...
//                 loadTimeZone
//..
// A 'baetzo_DataFileLoader' is supplied a file-system location using the
...
// zone.  Accordingly, 'baetzo_DataFileLoader' provides a method that, given a
...
// 'baetzo_Zoneinfo' object, the data from that file.
//
...
// component).  See 'baetzo_zoneinfobinaryreader' for more information about
...
///Thread Safety
///-------------
// 'baem_DataFileLoader' is *const* *thread-safe*, meaning that accessors may
// be invoked concurrently from different threads, but it is not safe to access
// or modify a 'baetzo_DataFileLoader' in one thread while another thread
// modifies the same object.
//
///Usage
///-----
// The following examples illustrate how to use a 'baetzo_DataFileLoader' to
...
//  const char *TEST_FILE      = "test/Asia/Bangkok";
//  #endif
//  int rc = bdesu_FileUtil::createDirectories(TEST_DIRECTORY, true);
//  assert(0 == rc);
//..
...
// 'test' directory.
//
///Example 2: Using a 'baetzo_DataFileLoader' to Load a Zoneinfo File
/// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// In this example we demonstrate how to use a 'baetzo_DataFileLoader' to load
// a time-zone data file into a 'baetzo_Zoneinfo' object.  We start by creating
// a 'baetzo_DataFileLoader' object, 'loader', and configure it with the
...
//..
// Now we create a 'baetzo_Zoneinfo' object, 'timeZone', and load it using
// 'loader':
//..
...
//..
//  int rc = bdesu_FileUtil::remove(TEST_DIRECTORY, true);
//  assert(0 == rc);
//..
...
class baetzo_DataFileLoader : public baetzo_Loader {
    // This component provides a concrete implementation of the
    // 'baetzo_DataLoader' protocol for loading, into a 'baetzo_Zoneinfo', the
  ...
  private:
    // NOT IMPLEMENTED
    baetzo_DataFileLoader(const baetzo_DataFileLoader&);
    baetzo_DataFileLoader& operator=(const baetzo_DataFileLoader&);
...
// ACCESSORS
inline
bool  baetzo_DataFileLoader::isRootPathPlausible() const
{
