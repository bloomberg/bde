...
//                              Overview
//                              --------
// The component under test provides 'baetzo_TestLoader', a class with a simple
// (easily verified) implementation of the of the 'baetzo_Loader', which is
...
// known 'baetzo_Zoneinfo' objects, the 'loadTimeZone' method -- the sole,
...
// loader.
//
// There are two broad concerns in the testing of the 'baetzo_TestLoader'
// class:
//: o Does it properly implement the 'baetzo_Loader' protocol (i.e., the
//:   'loadTimeZone' method)?
//: o Is it a proper (albeit simple) container of 'baetzo_Zoneinfo' objects?
...
//
// Although 'baetzo_TestLoader' is a container, it is limited functionality, so
...
//
// MANIPULATORS
// [ 3] void setTimeZone(const baetzo_Zoneinfo& tz);
// [ 3] int setTimeZone(const char *tzId, const char *data, int count);
// [ 3] int loadTimeZone(baetzo_Zoneinfo *result, const char *tzId);
//
// ACCESSORS
// [ 4] ostream& print(ostream& s, int level = 0, int sPL = 4) const;
//
// FREE OPERATORS
// [ 4] operator<<(ostream& s, const baetzo_TestLoader& tl);
// ----------------------------------------------------------------------------
// [ 1] BREATHING TEST
// [ 5] USAGE EXAMPLE
...
//..
      ...
      case 4: {
        ...
        //:
        //: 2 Create several 'baetzo_TestLoader' objects, each set with a
        //:   different number number, [ 0 .. 3 ], of 'baetzo_Zoneinfo'
        ...
        //:   1 Prepare two 'bael_TestLoader' objects: one empty, the other set
        //:     with a 'bael_Zoneinfo' object.
        ...
        //:       the 'print' method of the 'baetzo_Zoneinfo' object (with
        ...
        //   operator<<(ostream& s, const baetzo_TestLoader& tl);
        // --------------------------------------------------------------------
...

        {
            ...
            baetzo_Zoneinfo newYork(Z); newYork.setIdentifier(nyId);
            baetzo_Zoneinfo london(Z);   london.setIdentifier(londonId);
            baetzo_Zoneinfo tokyo(Z);     tokyo.setIdentifier(tokyoId);

            // Add some bogus data
            baetzo_LocalTimeDescriptor     nyType(-10, true, "NYTIME",     Z);
            baetzo_LocalTimeDescriptor londonType( 10, true, "LONDONTIME", Z);
            baetzo_LocalTimeDescriptor  tokyoType( 10, true, "TOKYOTIME",  Z);

            const bdet_Datetime NY_DATETIME     = bdet_Datetime(2009, 2, 1, 1);
...
#undef SP
      ...
      case 3: {
        ...
        //:   'baetzo_Zoneinfo' object; for one time-zone identifier create no
        //:   corresponding 'baetzo_Zoneinfo' object.
        //:   1 Create a default (empty) 'baetzo_TestLoader'.
        //:   2 Using the 'setTimeZone' method, insert each of the
        //:     'baetzo_Zoneinfo' objects into the 'baetzo_TestLoader' object.
        ...
        //:     (C-2)
        //:   4 Create an additional 'baetzo_Zoneinfo' object with a time-zone
        ...
        //:     'baetzo_Zoneinfo' object has been changed.  (C-3)
        //:
        ...
        //:   (C-7)
        //
        // Testing: void setTimeZone(const baetzo_Zoneinfo& tz); int
        //   setTimeZone(const char *tzId, const char *data, int count); int
        //   loadTimeZone(baetzo_Zoneinfo *result, const char *tzId);
        // --------------------------------------------------------------------
...

        {
            ...
            // ---------------------------------------------------------
            // Add a 'baetzo_Zoneinfo' objects via the object interface.
            // ---------------------------------------------------------
            ...
            // ---------------------------------------------------------------
            // Create, 'newYorkPrime', a 'bael_Zoneinfo' object, an object
            // with the same time-zone identifier as the 'newYork' object,
            // but different attributes otherwise.
            // ---------------------------------------------------------------

            baetzo_Zoneinfo newYorkPrime(Z); newYorkPrime.setIdentifier(
                                                         newYork.identifier());
            baetzo_LocalTimeDescriptor nyTypePrime(-11, false, "NYMINUTE",  Z);
...

        {
            ...
            // -------------------------------------------------------
            // Add 'baetzo_Zoneinfo' objects via the binary interface.
            // -------------------------------------------------------
      ...
      case 2: {
        ...
        //   create an 'bael_TestLoader' object, add 'bael_Zoneinfo' objects,
        //   access added 'bael_Zoneinfo' objects, and use the destructor to
        //   destroy the 'bael_TestLoader' object safely.  Although the
        ...
        //
        // Plan:
        //: 1 Create a 'bael_Zoneinfo' object.
        //:
        ...
        //:     previously created 'baetzo_Zoneinfo' object.  Use the (as yet
        ...
        //:     allocator.  (C-4, 7)
        //:
        //:   7 Destroy the 'baetzo_TestLoader' object and verify that all
        ...
        for (char cfg = 'a'; cfg <= 'c'; ++cfg) {
            ...
            // -------------------------------
            // Add a 'baetzo_Zoneinfo' object.
            // -------------------------------
            mX.setTimeZone(newYork);

            baetzo_Zoneinfo timeZone(Z);
            int             rc;
            rc = mX.loadTimeZone(&timeZone, badId);
            ASSERT(baetzo_ErrorCode::BAETZO_UNSUPPORTED_ID
                           == rc);
            rc = mX.loadTimeZone(&timeZone, nyId);
            ASSERT(0       == rc);
            ASSERT(newYork == timeZone);
      ...
      case 1: {
        ...
        baetzo_Loader& loader = x;

        baetzo_Zoneinfo value(Z);
