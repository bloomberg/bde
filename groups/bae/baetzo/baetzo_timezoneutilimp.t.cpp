...
//                      GLOBAL TYPEDEFS
//-----------------------------------------------------------------------------
typedef baetzo_TimeZoneUtilImp                   Obj;
typedef baetzo_LocalTimeDescriptor               Descriptor;
typedef baetzo_LocalTimeValidity                 Validity;
typedef baetzo_Zoneinfo::TransitionConstIterator Iterator;
typedef baetzo_ErrorCode                         Err;
typedef baetzo_DstPolicy                         Dst;
//=============================================================================
//                      GLOBAL CONSTANTS
//-----------------------------------------------------------------------------
...
//                                 HELPER FUNCTIONS
//-----------------------------------------------------------------------------
static bdet_Datetime fromTimeT(bdetu_Epoch::TimeT64 value)
    // Return the 'bdet_Datetime' representation of the interval in seconds
    // from UNIX epoch time of the specified 'value'.  Note that this method is
    // shorthand for 'bdetu_Epoch::convertFromTimeT64'.
{
    return bdetu_Epoch::convertFromTimeT64(value);
}

static bdetu_Epoch::TimeT64 toTimeT(const bdet_Datetime& value)
...
static bdet_Datetime fromIso8601 (const char *iso8601TimeString)
...
static void addTransitions(baetzo_Zoneinfo             *result,
...
int main(int argc, char *argv[])
{
    ...
    baetzo_Zoneinfo allDstTimeZone(Z);
    {
    ...
    baetzo_Zoneinfo oldDstTimeZone(Z);
    {
        static const TransitionDescription DESC[] = {
            // LINE  TRANSITION            OFF  DESC     DST
            // ---   --------------------  ---- -------  -----
    ...
    baetzo_TestLoader badLoader(Z);
    baetzo_ZoneinfoCache badCache(&badLoader, Z);
    baetzo_DefaultZoneinfoCache::setDefaultCache(&badCache);
...
//
///Prologue: Initializing a Example 'baetzo_ZoneinfoCache' object.
///- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
// cache containing only data for New York loaded through a 'baetzo_TestLoader'
// object.  Note that, in general, clients should use data from an external
// data source (see 'baetzo_datafileloader').
//
...
// 'baetzo_zoneinfoutil'):
//..
    newYork.addTransition(bdetu_Epoch::convertToTimeT64(
                                                       bdet_Datetime(1, 1, 1)),
                          est);
//..
...
//..
      ...
      case 6: {
        ...
        if (verbose) cout << "\nTesting an invalid time zone id." << endl;
        {
            ...
            bdet_Datetime VALID_INPUT(2010, 1, 1, 12, 0);
        ...
        if (verbose) cout << "\nTesting an invalid loader." << endl;
        {
            ...
            baetzo_DataFileLoader bogusLoader(Z);
            bogusLoader.configureRootPath("STEFANOBOGUSPATH");
            baetzo_ZoneinfoCache bogusCache(&bogusLoader, Z);
...
// Time zone with 1 transition (2 descriptors) (Riyadh)
 ...
 { __LINE__,  RY, "1950-12-31T20:53:09.000", "1949-12-31T20:53:08",
        ...
        for (int i = 0; i < NUM_VALUES; ++i) {
            ...
            const bdet_Datetime   INPUT_TIME   =
                                     fromIso8601(VALUES[i].d_input);
            const bdet_Datetime   EXP_START    =
                                     fromIso8601(VALUES[i].d_expectedStart);
            const bdet_Datetime   EXP_END      =
            ...
            const bdet_Datetime& START = result.utcStartTime();
            const bdet_Datetime& END   = result.utcEndTime();
      ...
      case 5: {
        ...
        for (int i = 0; i < NUM_VALUES; ++i) {
            ...
            const baetzo_Zoneinfo *currentTimeZone =
                                        testCache.lookupZoneinfo(TIME_ZONE_ID);

            ASSERT(currentTimeZone);

            const bdet_Datetime FIRST(1, 1, 1);
            const bdet_Datetime LAST(9999, 12, 31, 23, 59, 59, 999);
        ...
        if (verbose) cout << "\nNegative Testing." << endl;
        {
                             ...
                             "\t'createLocalTimePeriod' class method " << endl;
            {
                ...
                const baetzo_Zoneinfo BADZI;
                const baetzo_Zoneinfo *NYZI   = testCache.lookupZoneinfo(NY);
      ...
      case 4: {
        ...
        if (verbose) cout << "\nTesting an invalid loader." << endl;
        {
            ...
            LogVerbosityGuard guard(true);
            const bdet_Datetime VALID_INPUT(2010, 1, 1, 12, 0);
...
// Time zone with sequential DS transitions (Saigon)
 ...
 { __LINE__, SA, "1912-05-01T00:30:00",  DD, VI, "1912-04-30T23:30:00+07:00" }
        ...
        for (int i = 0; i < NUM_VALUES; ++i) {
            ...
            bdet_DatetimeTz  expResultTz;
            ASSERT(0 == bdepu_Iso8601::parse(&inputTime,
                                             inputStr.c_str(),
                                             inputStr.size()));
            ASSERT(0 == bdepu_Iso8601::parse(&expResultTz,
                                             expResultStr.c_str(),
                                             expResultStr.size()));
            baet_LocalDatetime expLocalTime(expResultTz, TIME_ZONE_ID, Z);
            ...
            bdet_DatetimeTz resultResolveTz;
        ...
        if (verbose) cout << "\nNegative Testing." << endl;
        {
            ...
            {
                const bdet_Datetime   VALID_INPUT(2011, 04, 10);
                const baetzo_Zoneinfo BAD;
                const baetzo_Zoneinfo *NYZI = testCache.lookupZoneinfo(NY);
 ...
 { __LINE__,  OLDDST, "2001-01-01T03:59:59.999", DS,
     "2001-01-01T03:59:59.999+01:00", VU, "2001-01-01T00:00:00",  3600, false,
        ...
        for (int i = 0; i < NUM_VALUES; ++i) {
            ...
            bdet_Datetime    expUtcTransitionTime;
            ASSERT(0 == bdepu_Iso8601::parse(&inputTime,
                                             inputStr.c_str(),
                                             inputStr.size()));
            ASSERT(0 == bdepu_Iso8601::parse(&expResultTime,
                                             expResultStr.c_str(),
                                             expResultStr.size()));
            ASSERT(0 == bdepu_Iso8601::parse(&expUtcTransitionTime,
            ...
            const bdet_Datetime resultTransitionTime =
      ...
      case 2: {
        ...
        //:   'baetzo_ZoneinfoUtil::convertUtcToLocalTime'.
        //:
...
// Time zone with multiple sequential STD transitions (Saigon)
 ...
 { __LINE__,  SA, "1912-04-30T17:01:00.000", "1912-05-01T01:01:00.000+08:00" },
            ...
            for (int i = 0; i < NUM_VALUES; ++i) {
                ...
                baetzo_ZoneinfoUtil::convertUtcToLocalTime(&resultTzRaw,
                                                           &iterator,
                                                            inputTime,
                                                           *currentTimeZone);
        ...
        if (verbose) cout << "\nNegative Testing." << endl;
        {
            ...
            {
                bdet_DatetimeTz result;
                bdet_Datetime   utcTime(2011, 04, 10);
 ...
 { __LINE__, SA, "1912-05-01T00:30:00",   DD, "1912-04-30T23:30:00+07:00" }
            ...
            for (int i = 0; i < NUM_VALUES; ++i) {
                ...
                ASSERT(0 == bdepu_Iso8601::parse(&expResultTime,
                                                 expResultStr.c_str(),
                                                 expResultStr.size()));
                baet_LocalDatetime expLocalTime(expResultTime, timeZoneId, Z);
        ...
        }
        {
            ...
            for (int i = 0; i < NUM_VALUES; ++i) {
                ...
                bdet_Datetime input, start, end;
                ASSERT(0 == bdepu_Iso8601::parse(&input,
                                                 inputStr.c_str(),
                                                 inputStr.size()));
