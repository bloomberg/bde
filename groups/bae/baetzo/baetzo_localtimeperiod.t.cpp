...
// ----------------------------------------------------------------------------
// CLASS METHODS
// [11] bool isValidUtcStartAndEndTime(const bdet_Datetime& start, end);
//
// CREATORS
// [ 2] baetzo_LocalTimePeriod(bslma_Allocator *bA = 0);
// [ 3] baetzo_LocalTimePeriod(const LTD& d, const Dt& s, e, Al *bA = 0);
// [ 7] baetzo_LocalTimePeriod(const baetzo_LTD& o, Al *ba = 0);
// [ 2] ~baetzo_LocalTimePeriod();
//
// MANIPULATORS
// [ 9] operator=(const baetzo_LocalTimePeriod& rhs);
// [ 2] void setDescriptor(const baetzo_LocalTimeDescriptor& value);
// [ 2] void setUtcStartAndEndTime(const bdet_Datetime& start, end);
//
// [ 8] void swap(baetzo_LocalTimePeriod& other);
//
// ACCESSORS
// [ 4] bslma_Allocator *allocator() const;
// [ 4] const baetzo_LocalTimePeriod& descriptor() const;
// [ 4] const bdet_Datetime& utcStartTime() const;
// [ 4] const bdet_Datetime& utcEndTime() const;
//
// [ 5] ostream& print(ostream& s, int level = 0, int sPL = 4) const;
//
// FREE OPERATORS
// [ 6] bool operator==(const baetzo_LocalTimePeriod& lhs, rhs);
// [ 6] bool operator!=(const baetzo_LocalTimePeriod& lhs, rhs);
// [ 5] operator<<(ostream& s, const baetzo_LocalTimePeriod& d);
//
// FREE FUNCTIONS
// [ 8] void swap(baetzo_LocalTimePeriod& a, b);
// ----------------------------------------------------------------------------
// [ 1] BREATHING TEST
// [12] USAGE EXAMPLE
...
typedef baetzo_LocalTimeDescriptor T1;  // 'descriptor'
typedef bdet_Datetime              T2;  // 'utcStartTime'
typedef bdet_Datetime              T3;  // 'utcEndTime'
...
//..
      ...
      case 11: {
        ...
        //:     the boundaries.  Treat the default value ('bdet_Datetime()') as
        ...
        //
        // Testing:
        //   bool isValidUtcStartAndEndTime(const bdet_Datetime& start, end);
        // --------------------------------------------------------------------
        ...
        if (verbose) cout <<
           "\nAssign the address of 'isValidUtcStartAndEndTime' to a variable."
                                                                       << endl;
        {
            typedef bool (*freeFuncPtr)(const bdet_Datetime&,
                                        const bdet_Datetime&);
        ...
        const bdet_Datetime      defaultDt;
        const bdet_Datetime       earliest(   1,  1,  1,  1,  0,  0, 000);
        const bdet_Datetime almostEarliest(   1,  1,  1,  1,  0,  0, 001);
        const bdet_Datetime           some(2010,  4, 15,  8);
        const bdet_Datetime         latest(9999, 12, 31, 23, 59, 59, 999);
        const bdet_Datetime   almostLatest(9999, 12, 31, 23, 59, 59, 998);
      ...
      case 9: {
        ...
        //
        // Testing:
        //   operator=(const baetzo_LocalTimePeriod& rhs);
        // --------------------------------------------------------------------
      ...
      case 8: {
        ...
        //   void swap(baetzo_LocalTimePeriod& a, b);
        // --------------------------------------------------------------------
      ...
      case 6: {
        ...
        const T2 A2 = bdet_Datetime(2010,  3, 14, 7);  // baseline value
        const T2 B2 = bdet_Datetime(2010,  3, 14, 8);

        // Attribute 3 Values: 'utcEndTime'

        const T3 A3 = bdet_Datetime(2010, 11,  7, 6);  // baseline value
        const T3 B3 = bdet_Datetime(2010, 11,  7, 7);
      ...
      case 5: {
        ...
        const Obj A(baetzo_LocalTimeDescriptor(-7, true,  "MDT"),
                    bdet_Datetime(2010,  4, 15,  8),
                    bdet_Datetime(2010, 12,  8,  9));

        const Obj B(baetzo_LocalTimeDescriptor( 2, false, "IDT"),
                    bdet_Datetime(2011,  5, 16,  9),
                    bdet_Datetime(2012,  1,  9, 10));
...
#undef SP
      ...
      case 4: {
        ...
        //   const baetzo_LocalTimePeriod& descriptor() const;
        ...
        const T1 D1 = baetzo_LocalTimeDescriptor();  // 'descriptor'
        const T2 D2 = bdet_Datetime();               // 'utcStartTime'
        const T3 D3 = bdet_Datetime();               // 'utcEndTime'

        // -------------------------------------------------------
        // 'A' values: Should cause memory allocation if possible.
        // -------------------------------------------------------

        const T1 A1 = baetzo_LocalTimeDescriptor(
                                                UTC_MIN,
                                                true,
                                                "a_" SUFFICIENTLY_LONG_STRING);
        const T2 A2 = bdet_Datetime(2010,  4, 15,  8);
        const T3 A3 = bdet_Datetime(2010, 12,  8,  9);
      ...
      case 3: {
        ...
        //
        // Testing:
        //   baetzo_LocalTimePeriod(const LTD& d, const Dt& s, e, Al *bA = 0);
        // --------------------------------------------------------------------
        ...
        if (verbose) cout << "\nTesting with injected exceptions." << endl;
        {
            ...
            for (int ti = 0; ti < NUM_DATA; ++ti) {
                const int                         LINE  = DATA[ti].d_line;
                const char                        MEM   = DATA[ti].d_mem;
                const baetzo_LocalTimeDescriptor& DESCR =
        ...
        if (verbose) cout << "\nNegative Testing." << endl;
        {
            ...
            const baetzo_LocalTimeDescriptor defaultLTD;
            const bdet_Datetime              defaultDt;
            const bdet_Datetime               someDt(2010,  4, 15,  8);
            const bdet_Datetime              earlyDt(2011,  6, 16,  9);
            const bdet_Datetime              laterDt(2012,  1, 10, 11);
      ...
      case 2: {
        ...
        //
        // Testing:
        //   baetzo_LocalTimePeriod(bslma_Allocator *bA = 0);
        //   ~baetzo_LocalTimePeriod();
        //   void setDescriptor(const baetzo_LocalTimeDescriptor& value);
        //   void setUtcStartAndEndTime(const bdet_Datetime& start, end);
        // --------------------------------------------------------------------
        ...
        const T1 B1 = baetzo_LocalTimeDescriptor(UTC_MAX, false, "ETD");
        const T2 B2 = bdet_Datetime(2011,  6, 16,  9);
        const T3 B3 = bdet_Datetime(2012,  1, 10, 11);
        ...
        if (verbose) cout << "\nNegative Testing." << endl;
        {
            ...
            if (veryVerbose) cout << "\tutcOffsetInSeconds" << endl;
            {
                ...
                const bdet_Datetime  someDatetime(2010,  4, 15,  8);
                const bdet_Datetime earlyDatetime(2011,  6, 16,  9);
                const bdet_Datetime laterDatetime(2012,  1, 10, 11);
      ...
      case 1: {
        ...
        const T1 D1 = baetzo_LocalTimeDescriptor();     // default value
        const T1 A1 = baetzo_LocalTimeDescriptor(-4 * 60 * 60, true, "EDT");

        // Attribute 2 Values: 'utcStartTime'

        const T2 D2 = bdet_Datetime();                  // default value
        const T2 A2 = bdet_Datetime(2010,  3, 14, 7);

        // Attribute 3 Values: 'utcEndTime'

        const T3 D3 = bdet_Datetime();                  // default value
        const T3 A3 = bdet_Datetime(2010, 11,  7, 6);
