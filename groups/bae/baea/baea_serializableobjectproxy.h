...
//
//@CLASSES:
//           baea_SerializableObjectProxy: proxy for autogenerated types
...
// 'baea_SerializableObjectProxy', that serves as a proxy that encapsulate a
// 'bdeat'-compliant object with a common interface providing the
// functionalities required by 'bdeat' encoders and decoders.  It exposes all
// the accessors and manipulators required by 'bdeat'-based encoders/decoders
// (such as 'bdem_berencoder', 'bdem_berdecoder', and similar encoders/decoders
// for XML) to the object represented by this proxy.  It provides set of "load"
// methods for configuring a proxy object to represent the intended
// 'bdeat'-compliant object so that it can be used for either encoding or
// decode (but not both).  However, clients should use a higher level component
// (e.g., 'baea_serializableobjectproxyutil') to configure the proxy object.
//
// The encoders/decoders are templatized on the objects they are used on, which
// can significantly increase the code size if there are many different types
// of objects.  This component reduces the size of executable code associated
// with encoding/decoding autogenerated objects by providing a proxy type such
// that the encoder/decoder implementations are template-expanded in terms of
// only 'baea_SerializableObjectProxy' instead of all the autogenerated types.
// Rather than encoding or decoding 'bdeat'-compliant object directly, a client
// will create and configure a proxy to represent that object, and then
// encode/decode the proxy.
//
...
// category contains an object of another type and imposes certain restrictions
...
// Customized type, but it can be use to represent its underlying base type.
//
...
// but such types cannot be used with this component.  'bas_codegen.pl' emits
// Customized types having conversion methods that return 'const' references to
// their underlying value and are thus compatible with this component.
//
...
//:   of the target variable, whereas the BER and XML encoders automatically
//:   do.  As a result, users of 'baea_SerializableObjectProxy' should manually
//:   reset the target variable before decoding.
//:
//: o When decoding, 'baea_SerializableObjectProxy' represents the underlying
//:   object of the Customized type.  Therefore, it imposes the restrictions of
//:   the Customized types (e.g., length-limited strings).
//:
//: o A particular 'baea_SerializableObjectProxy' object can be used for either
//:   encoding or decoding, but not both.
//:
//: o 'baea_SerializableObjectProxy' does not support BDEX.  It provides
//:   standard BDEX accessor functions for integration purposes, but these
//:   methods will fail (indicating encoding/decoding failure) at runtime if
//:   used.
//:
...
//  };
//..
// Then we define an instance of 'MyChoiceType', 'myChoice', and an initially
...
//..
// Now we invoke the 'loadChoice' method, assuming we have a function named
// 'makeEncodeProxy' configures another 'baea_SerializableObjectProxy' object
// to represent the selection (i.e., 'myChoice.d_mySequence').
//..
...
// supplied to its "load" method to configure that new object, and
...
// (e.g., baea_SerializableObjectProxyUtil) recursively instantiates function
// templates for all the types that the encoder/decoder may encounter; and at
// runtime, applying an accessor or manipulator to a
// baea_SerializableObjectProxy results in the recursive invocation of that
// accessor or manipulator on temporary 'baea_SerializableObjectProxy' objects
// created on the stack.  Note that 'baea_SerializableObjectProxy' objects are
// PODs so these temporary objects are relatively inexpensive to create and
// destroy.
//
...
// that configure a proxy object for its element.  Note that in this case,
...
//                     int                                  attributeId)
//     // Configure the specified 'proxy' to represent the 'status' object of
//     // the 'Test::Simple' object represented by the specified 'object'.  The
...
//  test::Simple result;
//..
// Then, instead of decoding the XML string directly, we create a proxy
// object for 'result'.
//..
...
// to represent 'result'.  Note that this is done for demonstration purpose.
...
struct baea_SerializableObjectProxyFunctions {
    ...
    typedef void (*Loader)(baea_SerializableObjectProxy *proxy, void *object);
        // This 'typedef' is an alias for a function that configure the
    ...
    typedef void (*SelectionLoader)(
        ...
        // This 'typedef' is an alias for a function that configure the
        // specified 'proxy' with the specified 'object and load the current
    ...
    typedef int (*Chooser)(void *object, int selectionId);
        ...
        // of the specified Choice object with the specified 'selectionId'.

    typedef void (*Resizer)(void         *object,
                            void        **newBeginAddress,
                            bsl::size_t   newSize);
        // This 'typedef' is an alias for a function that load, into the
...
struct baea_SerializableObjectProxy_NullableAdapter {
    ...
    // 'baea_SerializableObjectProxy' but only exposes the Nullable trait
    // (through the implementation of the 'bdeat' functions overloads), and not
    // any other traits.  'baea_SerializableObjectProxy' exposes multiple
    // traits, and is, therefore, considered to be a Dynamic type (see
    // 'bdeat_typecategory' for how Dynamic type is defined).
    // 'baea_SerializableObjectProxy_NullableAdapter' is required for
    // integration with at least the BER decoder, which cannot handle a
...
class baea_SerializableObjectProxy {
    ...
    // 'baea_SerializableObjectProxy' exposes the minimal interface required by
  ...
  private:
    ...
    int loadSequenceElementProxy(
        ...
        // Sequence object (represented by this proxy) contains such element.
        // Return 0 on success, and a non-zero value otherwise.  The behavior
        // is undefined unless this object represents a Sequence object (i.e.,
    ...
    int loadSequenceElementProxy(
        ...
        // (represented by this proxy) contains such element.  Return 0 on
  ...
  public:
    ...
    int maxSupportedBdexVersion() const;
        // Return -1.  'baea_SerializableObjectProxy' does not support BDEX.
        // This method is provided for integration with frameworks that assume
        // the complete 'bas_codegen' API, which is larger than the API
        // required by 'bdeat'-based codecs.

    template <class STREAM>
    STREAM& bdexStreamIn(STREAM& stream, int);
        // Mark the specified 'stream' invalid and return 'stream'.
        // 'baea_SerializableObjectProxy' does not support BDEX.  This method
        // is provided for integration with frameworks that assume the complete
    ...
    int choiceMakeSelection(const char *selectionName,
        ...
        // Change the selection of the Choice object represented by this proxy
        // to the selection having the specified 'selectionName' of the
        // specified 'selectionNameLength' by invoking the chooser function
        // supplied to the 'loadChoiceForDecoding' method if the object
        // (represented by this proxy) such selection; return the result of
        // that invocation on success, and a non-zero value otherwise.  The
        // behavior is undefined unless this object represents a Choice object
        // for decoding (i.e., 'loadChoiceForDecoding' was called).

    template<typename MANIPULATOR>
    int choiceManipulateSelection(MANIPULATOR& manipulator);
        // Invoke the specified 'manipulator' on a proxy object representing
        // the current selection of the Choice object represented by this proxy
        // (populated by the 'loader' function supplied to the
        // 'loadChoiceForDecoding' method), and return the result of that
    ...
    int arrayManipulateElement(MANIPULATOR& manipulator, int index);
        // Invoke the specified 'manipulator' on a proxy object representing
        // the element at the specified 'index' in the Array object represented
        // by this proxy (populated by the 'loader' function supplied to the
        // 'loadArrayForDecoding' method), and return the result of that
        // invocation.  'MANIPULATOR' shall be a functor providing methods that
        // can be called as if it had the following signatures:
        //..
        //  int operator()(const baea_SerializableObjectProxy&);
    ...
    int sequenceManipulateAttribute(MANIPULATOR& manipulator, int attributeId);
        // Invoke the specified 'manipulator' on a proxy object representing
        // the element with the specified 'attributeId' (populated by the
        // loader function supplied at the 'loadSequence' method) if the
        // Sequence object (represented by this proxy) contains such element;
        // return the result of that invocation on success, and a non-zero
        // value otherwise.  'MANIPULATOR' shall be a functor providing methods
    ...
    int sequenceManipulateAttribute(MANIPULATOR&  manipulator,
        ...
        // 'nameLength' (populated by the loader function supplied at the
        // 'loadSequence' method) if the Sequence object (represented by this
        // proxy) contain such element; return the result of that invocation on
        // success, and a non-zero value otherwise.  'MANIPULATOR' shall be a
    ...
    int sequenceManipulateAttributes(MANIPULATOR& manipulator);
        ...
        // the 'loader' function supplied at the 'loadSequence' method), each
    ...
    int enumFromString(const char *stringValue, int stringLength);
        ...
        // the 'stringSetter' supplied at the 'loadEnumerationForDecoding'
    ...
    void loadChoiceForEncoding(
        ...
        // current selection, with the information about the selection
        // represented by the specified 'selectionInfo', such that it can be
        // used for encoding; specify a 'loader' function to configure
        // another 'baea_SerializableObjectProxy' object to represent the
        // selection of the Choice object (i.e.,  'object').  The behavior is
        // undefined unless 'object' refers the selection of a Choice object.
    ...
    baea_SerializableObjectProxyFunctions::Chooser          chooser);
        ...
        // (having the specified 'numSelections' and the information about
        // choices of the object represented by the specified
        // 'selectionInfoArray') so that it can be used for decoding; specify a
        // 'loader' function to configure another
        // 'baea_SerializableObjectProxy' object to represent the current
        // selection of the Choice object; specify a 'chooser' function to
        // change the selection of 'choice'.  The behavior is undefined unless
        // 'choice' refers to a Choice object.
    ...
    void loadSequence(
        ...
        // having the specified 'className' (and having the specified
        // 'numAttributes' and the attributes information represented by the
        // specified 'attributeInfo') so that it can be used for encoding or
        // decoding; specify a 'loader' function to configure another
    ...
    void loadArrayForEncoding(
        ...
        // if empty), so that it can be used for encoding; specify a 'loader'
        // function to configure another 'baea_SerializableObjectProxy' to
        // represent an element in the Array object.  The behavior is
        // undefined unless the Array object is of type 'bsl::vector' (see
    ...
    void loadArrayForDecoding(
        ...
        // address (0 if empty) so that it can be used for decoding; specify a
        // 'loader' function to configure another
        // 'baea_SerializableObjectProxy' to represent an element in the Array
        // object; specify a 'resizer' is a function to change the size of the
        // Array object.  The behavior is undefined unless 'object' refers to a
        // 'bsl::vector' (see "Restrictions On Proxied Types" in the
        // @DESCRIPTION section).
    ...
    void loadNullableForEncoding(
        ...
        // having a null value if 'object' is 0; specify a 'loader' a function
        // to configure another 'baea_SerializableObjectProxy' to represent the
        // value of the Nullable object, or it is ignored if 'object' is 0.
    ...
    void loadNullableForDecoding(
        ...
        // that it can be used for decoding; specify a 'loader' function to to
        // configure another 'baea_SerializableObjectProxy' to represent the
        // *contained* object within the nullable value; specified a
        // 'valueMaker' function to will make 'object' non-null; and specify a
        // 'valueFetcher' function to return the address of the contained
        // object.  The behavior is undefined unless 'object' is the address of
        // an object of type 'bdeut_NullableAllocatedValue' or
        // 'bdeut_NullableValue'.
    ...
    const void *object() const;
        ...
        // (e.g., 'loadSequence').  The meaning of this address depends on the
    ...
    int choiceAccessSelection(ACCESSOR& accessor) const;
        // Invoke the specified 'accessor' on a proxy object representing the
        // current selection of the Choice object represented by this proxy
        // (populated by the 'loader' function supplied to the
        // 'loadChoiceForEncoding' method), and return the result of that
    ...
    int sequenceAccessAttribute(ACCESSOR& accessor, int attributeId) const;
        // Invoke the specified 'accessor' on a proxy object representing
        // the element with the specified 'attributeId' (populated by the
        // loader function supplied at the 'loadSequence' method) if the
        // Sequence object (represented by this proxy) contains such element;
        // return the result of that invocation on success, and a non-zero
        // value otherwise.  'ACCESSOR' shall be a functor providing methods
        // that can be called as if it had the following signatures:
        //..
        //  int operator()(const baea_SerializableObjectProxy&,
        //                 const bdeat_AttributeInfo&);
...
inline
int baea_SerializableObjectProxy::maxSupportedBdexVersion() const
{
    return -1;
}
...
template <class STREAM>
inline
STREAM& baea_SerializableObjectProxy::bdexStreamIn(STREAM& stream, int)
{
    stream.invalidate();
    return stream;
}
...
template <class STREAM>
inline
STREAM& baea_SerializableObjectProxy::bdexStreamOut(STREAM& stream, int) const
{
    stream.invalidate();
    return stream;
}
