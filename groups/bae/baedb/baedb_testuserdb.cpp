...
int baedb_TestUserDb::verifyPasswordByLogin(const char *password,
                                            int         passwordLength,
                                            const char *login,
                                            int         loginLength) const
{
    ...
    const bsl::string loginString(login, loginLength);
    const bsl::string passwordString(password, passwordLength);
    const int         len = numRecords();
    ...
    for (int i = 0; i < len; ++i) {
        ...
        if (bdeu_String::areEqualCaseless(record.theString(USER_FIELD_LOGIN),
                                          loginString)) {
            if (record.theString(USER_FIELD_PASSWORD) == passwordString) {
                return baedb_UserDb::SUCCESS;
            }
            else {
                return baedb_UserDb::INVALID_PASSWORD;
            }
        }
    }
...
int baedb_TestUserDb::verifyPasswordByUserNumber(const char *password,
                                                 int         passwordLength,
                                                 int         userNumber) const
{
    BSLS_ASSERT(password);
...
int baedb_TestUserDb::verifyPasswordByUUID(const char *password,
                                           int         passwordLength,
                                           int         uuid) const
{
    BSLS_ASSERT(password);
