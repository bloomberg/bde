...
struct XmlEl {
    ...
    baexml_Reader::NodeType d_typeExpected;
    const char             *d_text;
                     ...
                     int         elementCount)
{
    ...
    for (i = 0; ; ++i) {
        ...
        baexml_Reader::NodeType nt = reader.nodeType ();
...
XmlEl table91[] =
{
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_XML_DECLARATION ,
          "<?xml version='1.0' encoding='UTF-8'?>" , 0 },
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_WHITESPACE, "\n" , 0},
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_ELEMENT, "<RootElement>", 0 },

    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_ELEMENT, "<Elem1>" , 0 },
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_TEXT, "&lt;&amp;&apos;&quot;&gt;",
                                          "<&'\">" },
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_END_ELEMENT, "</Elem1>" , 0 },

    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_ELEMENT, "<Elem2>" , 0 },
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_TEXT,
           "&#127;&#x7e;&#128;&#x81;&#2047;&#2048;&#x801;"
           "&#65535;&#x10000;&#x10001;&#x10FFFF;",
           "\x7F\x7e\xC2\x80\xC2\x81\xDF\xBF\xE0\xA0\x80\xE0\xA0\x81"
           "\xEF\xBF\xBF\xF0\x90\x80\x80\xF0\x90\x80\x81\xF4\x8F\xBF\xBF" },
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_END_ELEMENT, "</Elem2>" , 0 },

    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_END_ELEMENT, "</RootElement>", 0 },
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_WHITESPACE,   "\n", 0 }
};
...
XmlEl table81[] =
{
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_XML_DECLARATION ,
          "<?xml version='1.0' encoding='UTF-8'?>" , 0 },
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_WHITESPACE, "\n" , 0},
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_ELEMENT,
       "<directory-entry xmlns:dir='http://bloomberg.com/schemas/directory'>",
                                                                           0 },
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_WHITESPACE,   "\n     ", 0 },
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_ELEMENT, "<name>" , 0 },
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_TEXT, "John Smith", 0 },
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_END_ELEMENT, "</name>" , 0 },
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_WHITESPACE,   "\n     ", 0 },
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_ELEMENT,
                                                "<phone dir:phonetype='cell'>",
                                                                           0 },
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_TEXT, "212-318-2000", 0 },
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_END_ELEMENT, "</phone>" , 0 },
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_WHITESPACE,   "\n     ", 0 },
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_ELEMENT, "<address/>" , 0 },
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_WHITESPACE,   "\n", 0 },
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_END_ELEMENT,
           "</directory-entry>", 0 },
    {0, 0, baexml_Reader::BAEXML_NODE_TYPE_WHITESPACE,   "\n", 0 }
};
...
int advancePastWhiteSpace(baexml_Reader& reader) {
    ...
    do {
        ...
        cout << baexml_Reader::nodeTypeAsString(baexml_Reader::NodeType(type))
             << endl;
        if (value) P(value)
    } while(0 == rc &&
...
static void  processNode(baexml_Reader *reader)
{
...
static int parseAndProcess(baexml_Reader *reader)
{
    int rc;
    ...
    if (rc < 0)
    {
        const baexml_ErrorInfo& errInfo = reader->errorInfo();
...
//     rc = advancePastWhiteSpace(reader);
    ASSERT( reader.nodeType() == baexml_Reader::BAEXML_NODE_TYPE_ELEMENT);
    LOOP_ASSERT(reader.nodeValue(), !bsl::strcmp(reader.nodeValue(), "Tom & Jerry #'\"<>&"));
...
void readHeader(Obj& reader)
{
    ...
    rc = advancePastWhiteSpace(reader);
    ASSERT( reader.nodeType() == baexml_Reader::BAEXML_NODE_TYPE_COMMENT);
...
int main(int argc, char *argv[])
{
    ...
    switch (test)
    {
      ...
      case 10: {
        ...
        // USAGE EXAMPLE
        //
        // This is a implementation of the 'baexml_Reader' interface.
        //
        // Concerns:
        ...
        //
        // Testing:
        //   baexml_MiniReader();  // Constructor
        //   ~baexml_MiniReader(); // Destructor
      ...
      case 9: {
        ...
        //
        // Concerns:
        //   That baexml_MiniReader performs properly reference processing
        //
        // Plan:
      ...
      case 8: {
        ...
        //
        // Concerns:
        //   That baexml_MiniReader reports correct start and end positions
        //   each node.
        //
        // Plan:
        //   Create an Xml document from array of XmlEl elements.
      ...
      case 7: {
        ...
        //
        // Concerns:
        //   That baexml_MiniReader operate correctly with xml strings
        //   (buffers) with many attributes.  The strings in the test vectors
        //   all contain a *single element* with different number of
        //   attributes.
        //
        // Plan:
        //   Construct a 'baexml_MiniReader' reader capable of buffering
        ...
        for (int idx = 0; idx < NUM_DATA; ++idx) {
          ...
          LOOP_ASSERT(LINE,
                    reader.nodeType() == baexml_Reader::BAEXML_NODE_TYPE_NONE);
      ...
      case 5: {
        ...
        //   'baexml_MiniReader'.  The 'bufferSize' is used to perturb the
        ...
        for (int idx = 0; idx < NUM_DATA; ++idx) {
          ...
          rc = advancePastWhiteSpace(reader);
          LOOP_ASSERT(LINE,
               reader.nodeType()==baexml_Reader::BAEXML_NODE_TYPE_END_ELEMENT);
          LOOP_ASSERT(LINE, !bsl::strcmp(reader.nodeName(), "xs:schema"));
      ...
      case 4: {
        ...
        //
        // Plan:
        //  Create and open a 'baexml_MiniReader' with strings containing
        ...
        if (verbose) bsl::cout << "Using CDATA."   << bsl::endl
                               << "-  -  -  - -\n" << bsl::endl;
        {
          static const char xmlStr[] =
            ...
            "    bdem:package='bascfg'>"
            "<Node0>"
            "   <Element3>Hello<![CDATA[Tom & Jerry #'\"<>&]]>World</Element3>"
            "</Node0>\n"
            "</xs:schema>";
          ...
          rc = advancePastWhiteSpace(reader);
          ASSERT(0 > rc);
          const baexml_ErrorInfo& errorInfo = reader.errorInfo();
      ...
      case 3: {
        ...
        //   * Open an 'baexml_MiniReader' with the xml string and pass the
      ...
      case 2: {
        ...
        // BOOTSTRAP TEST
        // Concerns:
        //   1. The 'baexml_MiniReader's' constructors work properly:
        ...
        if (verbose)
          bsl::cout << "\nTesting 'baexml_MiniReader(*bA)' ctor" << bsl::endl;
        ...
        if (verbose)
          bsl::cout << "\nTesting 'baexml_MiniReader(bufSize, *bA)' ctor"
                    << bsl::endl;
      ...
      case -1: {
        // --------------------------------------------------------------------
        // baexml_MiniReader INTERACTIVE TEST
        //
