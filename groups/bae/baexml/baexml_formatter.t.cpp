...
//                              Overview
//                              --------
// Each of baexml_Formatter's the manipulators is tested in its own test case
  ...
  public:
    ...
    ScalarData(const bdet_Datetime& d)
        : d_type(Et::BDEM_DATETIME),
          d_datetime(d)
        {}
    ScalarData(const bdet_Date& d) : d_type(Et::BDEM_DATE), d_date(d) {}
    ScalarData(const bdet_Time& t) : d_type(Et::BDEM_TIME), d_time(t) {}
    void addAttribute(const bsl::string&  attrName,
                      baexml_Formatter   *formatter) const;
...
//..
      ...
      case 22: {
        ...
        baexml_Formatter mY(bsl::cout.rdbuf(), 0, 4, 5);
        ...
        baexml_Formatter mZ(bsl::cout.rdbuf(), 0, 4, -1);
        ...
        bsl::cout << "Wrap Column 0" << bsl::endl;
        baexml_Formatter mX1(bsl::cout.rdbuf(), 0, 4, 0);
        mX1.addElementAndData("farm", "Frances' Orchard"); // ' is escaped

        bsl::cout << "Wrap Column 5" << bsl::endl;
        baexml_Formatter mX2(bsl::cout.rdbuf(), 0, 4, 5);
        mX2.addElementAndData("farm", "Frances' Orchard"); // ' is escaped

        bsl::cout << "Wrap Column -1" << bsl::endl;
        baexml_Formatter mA(bsl::cout.rdbuf(), 0, 4, -1);
...
//..
    {
        ...
        baexml_Formatter formatter(outStream);
...
//..
    {
        bsl::ostringstream outStream;
        baexml_Formatter formatter(outStream, 0, 4, 40);
...
//..
      ...
      case 15: {
        ...
        // --------------------------------------------------------------------
          ...
          static struct {
              ...
              { L_, "Datetime", bdet_Datetime(1, 1, 1, 0, 0, 0, 0),
                    "0001-01-01T00:00:00.000" },
              { L_, "Datetime", bdet_Datetime(2005, 1, 22, 23, 59, 59, 999),
                    "2005-01-22T23:59:59.999" },
              { L_, "Date", bdet_Date(), "0001-01-01" },
              { L_, "Date", bdet_Date(2005, 1, 22), "2005-01-22" },
              { L_, "Time", bdet_Time(0, 0, 0, 1), "00:00:00.001" },
              { L_, "Time", bdet_Time(), "24:00:00.000" },
          };
          enum { DATA_SIZE = sizeof DATA / sizeof *DATA };
          bsl::ostringstream ss;
      ...
      case -3: {
          // Performance test
          // Usage: baexml_formatter.t.dbg_exc_mt -3 <size> <reps> <verbose>
          ...
          bdesb_MemOutStreamBuf output;

          for (int i = 0; i < reps; ++i) {
              output.reset();
              output.reserveCapacity(msgSize + 100);
              baexml_Formatter formatter(&output, 0, 0, -1);
              formatter.addHeader("UTF-8");
              formatter.openElement("root");
