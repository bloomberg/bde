// bael_predicateset.t.cpp    -*-C++-*-

#include <bael_predicateset.h>

#include <bael_attributecontainerlist.h>        // for testing 'evaluate'
#include <bael_defaultattributecontainer.h>     // for testing 'evaluate'
...
//-----------------------------------------------------------------------------
// [14] static int hash(const bael_PredicateSet&, int size);
// [10] static int maxSupportedBdexVersion();
// [ 2] bael_PredicateSet(bslma_Allocator *basicAllocator = 0);
// [ 7] bael_PredicateSet(const bael_PredicateSet&, bdema_Alct * = 0)
// [ 2] ~bael_PredicateSet();
// [ 2] int addPredicate(const bael_Predicate& predicate);
// [ 2] int removePredicate(const bael_Predicate& predicate);
// [12] void removeAllPredicates();
// [ 9] const bael_PredicateSet& operator=(const bael_PS& other)
// [10] template <class STREAM> STREAM& bdexStreamIn(STREAM&, int)
// [ 4] int numPredicates() const;
// [13] bool evaluate(const bael_AttributeSet& context) const;
// [ 4] bool isMember(const bael_Predicate&) const;
...
// [ 6] bool operator==(const bael_PS& lhs, const bael_PS& rhs)
// [ 6] bool operator!=(const bael_PS& lhs, const bael_PS& rhs)
...
bael_Predicate A0("", "12345678");
bael_Predicate A1("", 12345678);
bael_Predicate A2("", (Int64)12345678);
bael_Predicate A3("uuid", "12345678");
bael_Predicate A4("uuid", 12345678);
bael_Predicate A5("uuid", (Int64)12345678);
bael_Predicate A6("UUID", "12345678");
bael_Predicate A7("UUID", 12345678);
bael_Predicate A8("UUID", (Int64)12345678);
...
//-----------------------------------------------------------------------------
// The 'g' family of functions generate a 'bael_PredicateSet' object for
...
static Obj& gg(Obj *obj, const char *spec)
{
    ...
    while (*spec) {
        ...
        switch (*spec) {
          case 'i': {
            ++spec;
            bael_Predicate attr(name, *spec - '0');
            obj->addPredicate(attr);
          } break;
          case 'I': {
            ++spec;
            bael_Predicate attr(name, (Int64)*spec - '0');
            obj->addPredicate(attr);
          } break;
          default: {
            string value;
            value = *spec;
            bael_Predicate attr(name, value.c_str());
            obj->addPredicate(attr);
          }
        }
        ++spec;
...
// gg function for 'bael_AttributeSet'; used for testing 'evaluate' method
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      case 15: {
        ...
        ASSERT(1 == predicateSet.addPredicate(bael_Predicate("uuid",
                                                             3133246)));

        ASSERT(true == predicateSet.isMember(p1));
        ASSERT(true == predicateSet.isMember(bael_Predicate("uuid", 3133246)));

        ASSERT(0 == predicateSet.addPredicate(bael_Predicate("uuid",
                                                             3133246)));
      ...
      case 14: {
        ...
        //
        // Testing:
        //   static int hash(const bael_PredicateSet&, int size);
        // --------------------------------------------------------------------
      ...
      case 13: {
        ...
        //
        // Plan:
        //   Specify a set of pairs of a 'bael_PredicateSet' object and a
        //   'bael_AttributeSet' object.  For each pair, verify that the
        //   'evaluate' method returns the expected value.
        //
        // Testing:
        //   bool evaluate(const bael_AttributeSet& context) const;
        // --------------------------------------------------------------------
        ...
        for (int i = 0; i < NUM_DATA; ++i) {
            ...
            bael_DefaultAttributeContainer mY;
            const bael_DefaultAttributeContainer& Y = mY;
            LOOP_ASSERT(LINE, &mY == &hh(&mY, DATA[i].d_attributeSet));

            if (veryVerbose) { P_(X); P(Y); }

            bael_AttributeContainerList list;
            const bael_AttributeContainerList& LIST = list;
            list.pushFront(&Y);
      ...
      case 12: {
        ...
        //   'bael_PredicateSet' object.
        //
        // Plan:
        //   Specify a set S of test vectors.  For each element in S,
        //   construct the corresponding 'bael_PredicateSet' object x using
        ...
        for (int j = 0; j < NUM_SPECS; ++j) {
            ...
            for (bael_PredicateSet::const_iterator iter = Y.begin();
                 iter != Y.end();
                 ++iter) {
      ...
      case 11: {
        ...
        //
        // Plan:
        //   Construct an array consisting of 'bael_Predicate' objects having
        //   distinct values.  For each n in [0 .. N] where N is the maximum
        //   number of predicates tested, create an empty 'bael_PredicateSet'
        ...
        const bael_Predicate PREDS[] = { A0, A1, A2, A3, A4, A5, A6, A7, A8 };
      ...
      case 10: {
        ...
        Obj V1;
        Obj V2(V1);  V2.addPredicate(A1);
        Obj V3(V2);  V3.addPredicate(A2);
        Obj V4(V3);  V4.addPredicate(A3);
        Obj V5(V4);  V5.addPredicate(A4);
        Obj V6(V5);  V6.addPredicate(A5);
        Obj V7(V6);  V7.addPredicate(A6);
        Obj V8(V7);  V8.addPredicate(A7);
        Obj V9(V8);  V9.addPredicate(A8);
        ...
        if (verbose) cout <<
            "\nThorough test of stream functions." << endl;
        {
        ...
        Obj mX; mX.addPredicate(A1); mX.addPredicate(A2);
        const Obj& X = mX;                   // original (control) value
        Obj mY; mY.addPredicate(A3); mY.addPredicate(A4);
        ...
        if (verbose) cout << "\t\tBad number of predicates." << endl;
        {
            Out out;
            out.putInt32(-1);       // number of predicates can't be negative
            const char *const OD  = out.data();
            const int         LOD = out.length();
        ...
        if (verbose) cout << "\t\tBad predicate." << endl;
        {
            Out out;
            out.putInt32(1);          // indicating one predicate
            out.putString("uuid");    // predicate name is "uuid"
      ...
      case 9: {
        ...
        //
        // Testing:
        //   const bael_PredicateSet& operator=(const bael_PS& other)
        // --------------------------------------------------------------------
        if (verbose) cout << "\nTesting Assignment Operator"
                          << "\n===========================" << endl;
      ...
      case 7: {
        ...
        //
        // Testing:
        //   bael_PredicateSet(const bael_PredicateSet&, bdema_Alct * = 0)
        // --------------------------------------------------------------------
        if (verbose) cout << "\nTesting Copy Constructor"
                          << "\n========================" << endl;
     ...
     case 4: {
        ...
        //
        // Testing:
        //   bool isMember(const bael_Predicate&) const;
        ...
        for (int n = 0; n < NUM_NAMES; ++n) {
            ...
            for (int v = 0; v <= '9' - '0'; ++v) {
                ...
                LOOP2_ASSERT(n,
                             v,
                             (v < n) == X.isMember(
                                  bael_Predicate(NAMES[n], sValue.c_str())));

                int int32Value = v;
                LOOP2_ASSERT(n,
                             v,
                             (v < n) == X.isMember(
                                  bael_Predicate(NAMES[n], int32Value)));

                Int64 int64Value = v;
                LOOP2_ASSERT(n,
                             v,
                             (v < n) == X.isMember(
                                  bael_Predicate(NAMES[n], int64Value)));
            }
        }
      ...
      case 3: {
        ...
        //   bael_AttributeSet& hh(bael_AttributeSet *obj, const char *spec);
        // --------------------------------------------------------------------
        if (verbose) cout << endl
      ...
      case 2: {
        ...
        //
        // Testing:
        //   bael_PredicateSet(bslma_Allocator *basicAllocator = 0);
        //   int addPredicate(const bael_Predicate& predicate);
        //   int removePredicate(const bael_Predicate& predicate);
        //   ~bael_PredicateSet();
        // --------------------------------------------------------------------
