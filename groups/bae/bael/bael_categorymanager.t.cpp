...
//-----------------------------------------------------------------------------
// 'bael_Category' private methods, tested indirectly:
// [ 2] bael_Category(const char *name, int, int, int, int, *ba = 0);
// [ 2] ~bael_Category();
//
// 'bael_Category' public interface:
...
// [ 3] const bael_ThresholdAggregate& thresholdLevels() const;
...
// [13] bael_Category *addCategory(Holder *, const char *, int, int, int, int);
// [ 3] bael_Category *lookupCategory(const char *name);
// [13] bael_Category *lookupCategory(Holder *, const char *name);
// [13] void resetCategoryHolders();
// [ 5] bael_Category *setThresholdLevels(*name, int, int, int, int);
// [10] int addRule(const bael_Rule& rule);
// [11] int addRules(const bael_RuleSet& ruleSet);
// [10] int removeRule(const bael_Rule& rule);
// [11] int removeRules(const bael_RuleSet& ruleSet);
// [10] void removeAllRules();
// [15] bcemt_Mutex& rulesetMutex();
// [ 8] const bael_Category& operator[](int index) const
// [ 3] const bael_Category *lookupCategory(const char *name) const;
// [ 3] int length() const;
// [10] int ruleSequenceNumber() const;
// [15] bael_RuleSet& ruleSet() const;
//
// 'bael_CategoryManagerIter' public interface:
// [ 6] bael_CategoryManagerIter(const bael_CategoryManager& cm)
// [ 6] ~bael_CategoryManagerIter();
// [ 6] void operator++();
// [ 6] operator const void *() const;
// [ 6] const bael_Category& operator()() const;
//
// 'bael_CategoryManagerManip' public interface:
// [ 7] bael_CategoryManagerManip(bael_CategoryManager *cm);
// [ 7] ~bael_CategoryManagerManip();
// [ 7] void advance();
// [ 7] bael_Category& operator()();
// [ 7] operator const void *() const;
//
// 'bael_CategoryHolder' public interface:
// [12] void reset();
// [12] void setCategory(const bael_Category *category);
// [12] void setThreshold(int threshold);
// [12] void setNext(bael_CategoryHolder *holder);
// [12] const bael_Category *category() const;
// [12] int threshold() const;
// [12] bael_CategoryHolder *next() const;
//-----------------------------------------------------------------------------
// [ 1] BREATHING TEST
// [ 2] BASIC CONSTRUCTORS AND PRIMARY MANIPULATORS (BOOTSTRAP)
// [ 2] BOOTSTRAP: bael_Category *addCategory(*name, int, int, int, int);
// [ 9] MT-SAFETY
...
struct my_ThreadParameters {
    ...
    bcemt_Barrier *d_barrier_p;  // coordinates activity between threads (held)
...
extern "C" void *ruleThreadTest(void *args)
{
    ...
    bcemt_Barrier&  barrier = *((RuleThreadTestArgs *)args)->d_barrier;
    const Obj&      MX      = mX;
    ...
    for (int i = 0; i < NUM_NAMES; ++i) {
        ...
        uniqueStream << "ID" << i <<  bcemt_ThreadUtil::selfIdAsInt();
        ...
        // thread (and removed).
        bael_Rule rule1(NAMES[i], 255, 255, 255, 255);
        bael_Rule rule2(NAMES[i], 255, 255, 255, 255);
        ...
        for (int j = 0; j < NUM_NAMES; ++j) {
            ...
            ASSERT(exp == bdes_BitUtil::isSetOne(entry->relevantRuleMask(),
                                                 ruleId1));
            ASSERT(exp == bdes_BitUtil::isSetOne(entry->relevantRuleMask(),
                                                 ruleId3));
            ASSERT(!bdes_BitUtil::isSetOne(entry->relevantRuleMask(),ruleId4));
        }
        ...
        ASSERT(bdes_BitUtil::isSetOne(entry->relevantRuleMask(), ruleId4));
        {
            bcemt_LockGuard<bcemt_Mutex> guard(&mX.rulesetMutex());
            int seqNumber = MX.ruleSequenceNumber();
            const bael_RuleSet& rules = MX.ruleSet();
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      case 16: {
        ...
        for (int i = 0; i < NUM_CATEGORIES; ++i) {
            const bael_Category *category = cm.lookupCategory(myCategories[i]);
            out << "[ " << myCategories[i]
        ...
        for (int i = 0; i < NUM_CATEGORIES; ++i) {
            bael_Category *category = cm.lookupCategory(myCategories[i]);
        ...
        for (int i = 0; i < cm.length(); ++i) {
            const bael_Category&   category = cm[i];
      ...
      case 14: {
        ...
        //
        // Plan:
        //    Create a set of 'bael_ThresholdAggregate' objects containing a
      ...
      case 13: {
        ...
        //
        // Concerns:
        //   The 'addCategory' function taking a bael_CategoryHolder object
        ...
        //
        // Testing:
        //   bael_Category *addCategory(Holder *, name, int, int, int, int);
        //   bael_Category *lookupCategory(Holder *, const char *name);
      ...
      case 12: {
        ...
        //   a bael_CategoryHolder object.  Statically initialize an object X
        ...
        if (verbose) cout << endl << "Test bael_CategoryHolder" << endl
                                  << "========================" << endl;
     ...
     {  L_,  INT_MIN,  INT_MIN,  INT_MIN,  INT_MIN, "1234",    (Holder*) 1024}
      ...
      case 11: {
        ...
        //
        // Testing:
        //  int addRules(const bael_RuleSet& );
        //  int removeRules(const bael_RuleSet& );
        // --------------------------------------------------------------------
        ...
        bael_RuleSet::MaskType endMask = bdes_BitUtil::setOne(0, 0, NUM_NAMES);
        for (bael_RuleSet::MaskType mask = 0; mask <= endMask; ++mask) {
            int seqNumber = MX.ruleSequenceNumber();
            bael_RuleSet rules(&ta);
            for (int i= 0; i < NUM_NAMES; ++i) {
                if (bdes_BitUtil::isSetOne(mask, i)) {
                    bael_Rule rule(NAMES[i], LEVELS[i][0], LEVELS[i][1],
                                             LEVELS[i][2], LEVELS[i][3]);
                    rules.addRule(rule);
                }
            }
            ...
            ASSERT(bdes_BitUtil::numSetOne(mask) ==  MX.ruleSet().numRules());
            if (mask != 0) {
                ASSERT(seqNumber < MX.ruleSequenceNumber());
            ...
            for (int i = 0; i < NUM_NAMES; ++i) {
                if (bdes_BitUtil::isSetOne(mask, i)) {
                    ...
                    for (int j = 0; j < NUM_NAMES; ++j) {
                        ...
                        ASSERT(isSet == bdes_BitUtil::isSetOne(
            ...
            ASSERT(bdes_BitUtil::numSetOne(endMask & ~mask) ==
                   mX.addRules(allRules));
            ASSERT(bdes_BitUtil::numSetOne(endMask) ==mX.ruleSet().numRules());
            ...
            ASSERT(bdes_BitUtil::numSetOne(mask) == mX.removeRules(rules));
            ASSERT(bdes_BitUtil::numSetZero(~endMask | mask) ==
                                            MX.ruleSet().numRules());
            ...
            for (int i = 0; i < NUM_NAMES; ++i) {
                if (bdes_BitUtil::isSetZero(mask, i)) {
                    bael_Rule rule(NAMES[i], LEVELS[i][0], LEVELS[i][1],
                                             LEVELS[i][2], LEVELS[i][3]);
                    int ruleId = MX.ruleSet().ruleId(rule);
                    ASSERT(0 <= ruleId);
      ...
      case 10: {
        ...
        //   and the relevant rule masks for the affected bael_Category
        //   objects.
        //
        // Plan:
        ...
        // Testing:
        //   int addRules(const bael_RuleSet& );
        //   int removeRule(const bael_Rule& );
        //   void removeAllRules();
        // --------------------------------------------------------------------
        ...
        int seqNumber = MX.ruleSequenceNumber();
        for (int i = 0; i < NUM_NAMES; ++i) {
            bael_Rule rule1(NAMES[i], LEVELS[i][0], LEVELS[i][1],
                                      LEVELS[i][2], LEVELS[i][3]);
            ...
            for (int j = 0; j < NUM_NAMES; ++j) {
                bool  isSet = i == j;
                const Entry *cat = MX.lookupCategory(NAMES[j]);
                ASSERT(isSet == bdes_BitUtil::isSetOne(cat->relevantRuleMask(),
                                                       ruleId1));
            }

            bael_Rule rule2(NAMES[i], LEVELS[i][0], LEVELS[i][1],
                                      LEVELS[i][2], LEVELS[i][3]);
            bael_Predicate p2("A", 1);
            ...
            for (int j = 0; j < NUM_NAMES; ++j) {
                ...
                ASSERT(isSet == bdes_BitUtil::isSetOne(cat->relevantRuleMask(),
                                                       ruleId1));
                ASSERT(false == bdes_BitUtil::isSetOne(cat->relevantRuleMask(),
                                                       ruleId2));
            }
        }
      ...
      case 9: {
        ...
        for (int i = 0; i < NUM_THREADS; ++i) {
            ...
            bcemt_ThreadUtil::ThreadFunction action = (i < NUM_W_THREADS)
                                                      ? case9ThreadW
                                                      : case9ThreadQ;
            ASSERT(0 == bcemt_ThreadUtil::create(&threads[i].d_handle,
                                                 action,
                                                 &threads[i].d_params));
        }

        for (int i = 0; i < NUM_THREADS; ++i) {
            ASSERT(0 == bcemt_ThreadUtil::join(threads[i].d_handle));
        }
      ...
      case 8: {
        ...
        //
        // Plan:
        //   Create a modifiable 'bael_CategoryManager' object, 'mX', and a
        ...
        //   threshold level values on 'bael_Category' objects obtained from
        //   'mX'.  Verify that the values are changed using const
        //   'bael_Category' objects obtained from 'X'.  Change the threshold
        ...
        for (int i = 0; i < X.length(); ++i) {
            const bael_Category& category = X[i];
      ...
      case 7: {
        // --------------------------------------------------------------------
        // TESTING MANIPULATOR (bael_CategoryManagerManip)
        //
        // Concerns:
        ...
        //
        // Testing:
        //   bael_CategoryManagerManip(bael_CategoryManager *cm);
        ...
        if (verbose) cout << endl << "Test bael_CategoryManagerManip" << endl
                                  << "==============================" << endl;
      ...
      case 6: {
        ...
        //
        // Testing:
        //   bael_CategoryManagerIter(const bael_CategoryManager& cm)
      ...
      case 5: {
        ...
        //   bael_Category *addCategory(const char *name, int, int, int, int);
        //   bael_Category *setThresholdLevels(*name, int, int, int, int);
        // --------------------------------------------------------------------
        ...
        for (int i = 0; i < NUM_DATA; ++i) {
            ...
            {
                const int r = bael_Category::areValidThresholdLevels(
                                                             RECORD_LEVEL,
                                                             PASS_LEVEL,
                                                             TRIGGER_LEVEL,
                                                             TRIGGERALL_LEVEL);
      ...
      case 3: {
        ...
        //
        //   bael_Category *lookupCategory(const char *name);
        //   const bael_Category *lookupCategory(const char *name) const;
        //   int length() const;
        // --------------------------------------------------------------------
      ...
      case 1: {
        ...
        //
        // Plan:
        //   First, create a 'bael_CategoryManager' instance.  Add a category
