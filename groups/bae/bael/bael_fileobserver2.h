...
// use the local time offset in effect at construction.
//
// The 'enablePublishInLocalTime' method will not have the intended effect if
// the 'setLogFileFunctor' method has been called to install an alternate
// logging functor from the default.  However, if the functor passed to the
// 'setLogFileFunctor' method is provided by a 'bael_RecordStringFormatter'
// object, the timestamp attribute of records can be configured to be written
// in local time or UTC time as desired.  (See the component-level
// documentation of 'bael_recordstringformatter' for more information on
// configuring the format of log records.)
//
