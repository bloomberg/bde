// bael_recordobserverobserver.h                 -*-C++-*-
#ifndef INCLUDED_BAEL_RECORDOBSERVERADAPTER
#define INCLUDED_BAEL_RECORDOBSERVERADAPTER
...
//@PURPOSE: Provide an adapter to adapt bael_Observer object to bael_RecordObserver 
//          object.
//
//@CLASSES:
//  bael_RecordObserverAdapter: adapter class for adapting bael_Observer object to 
//                              bael_RecordObserver object.
//
//@SEE_ALSO: bael_observer, bael_recordobserver
//
//@AUTHOR: Shijin Kong (skong)
//
...
class bael_Context;
                           ...
                           // ================================
                           // class bael_RecordObserverAdapter
                           // ================================

class bael_RecordObserverAdapter : public bael_RecordObserver {
    // This class provides a protocol for receiving and processing log record
    // output.
    
    // DATA
    bael_Observer         *d_observer_p;   // the object that needs to be adapted

  public:
    // CREATORS
    explicit bael_RecordObserverAdapter(bael_Observer *observer); 

    virtual ~bael_RecordObserverAdapter();
        // Destroy this observer.

    // MANIPULATORS
    void publish(const bcema_SharedPtr<const bael_Record>& record, 
                 const bael_Context& context);
        // Process the specified shared pointer of log 'record' having the specified 
        // publishing 'context'.
...
// ACCESSORS
inline
bael_Observer *bael_RecordObserverAdapter::observer()
{
    return d_observer_p;
}
