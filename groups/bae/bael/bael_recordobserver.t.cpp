...
struct ObserverTest : bsls_ProtocolTestImp<bael_RecordObserver> {
    void publish(const bcema_SharedPtr<const bael_Record>&, const bael_Context&)  
    { 
        markDone(); 
    }
};
...
class my_OstreamObserver : public bael_RecordObserver {
    ostream& d_stream;
...
void my_OstreamObserver::publish(const bcema_SharedPtr<const bael_Record>&  record,
                                 const bael_Context& context)
{
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      case 2: {
...

        {
                ...
                cout << "Publish a single message (a sequence of 1)." << endl;
            {
                ...
                fixed.setThreadID(0);
                myObserver.publish(
                          bcema_SharedPtr<const bael_Record>(
      ...
      case 1: {
        ...
        // PROTOCOL TEST:
        // Concerns:
        //   We must ensure that (1) a subclass of the 'bael_RecordObserver' class
        //   compiles and links when all virtual functions are defined, and
        //   (2) the functions are in fact virtual.
        //
        // Plan:
        //   Construct an object of a class derived from 'bael_RecordObserver' and
        //   bind a 'bael_RecordObserver' reference to the object.  Using the base
        ...
        BSLS_PROTOCOLTEST_ASSERT(t, publish(
                                bcema_SharedPtr<const bael_Record>(
