...
struct BaseReaderHandler {
    ...
    void String(const Ch* str, size_t length, bool copy) { Default(); }
    void StartObject() { Default(); }
    void EndObject(size_t memberCount) { Default(); }
    void StartArray() { Default(); }
    void EndArray(size_t elementCount) { Default(); }
};
...
struct Visitor {
    ...
    int operator() (TYPE *, INFO_TYPE&) {
        return 0;
    };

    template <class INFO_TYPE>
    int operator() (bsl::string *object, INFO_TYPE&) {
        *object = d_value;
        cout << "visited node with value: " << d_value << endl;
...
struct TestReaderHandler {
    ...
    bsl::vector<bsl::string> d_attributes;
    TYPE *d_object;
    bool d_waitingForName;


    TestReaderHandler(TYPE *object)
    : d_object(object)
    , d_waitingForName(false)
    {
    }

    void Default() {}
    void Null() { Default(); }
    void Bool(bool b) { (void) b; Default(); }
    void Int(int i) { (void) i; Default(); }
    void Uint(unsigned i) { (void) i; Default(); }
    void Int64(int64_t i) { (void) i; Default(); }
    void Uint64(uint64_t i) { (void) i; Default(); }
    void Double(double d) { (void) d; Default(); }
    void String(const Ch* str, size_t length, bool copy) {
        cout << "String: " << str << endl;
        if (d_waitingForName) {
            d_attributes.push_back(str);
            d_waitingForName = false;
        }
        else {
            Visitor v;
            v.d_value = str;
            d_object->manipulateAttribute(v, d_attributes.back().c_str(), d_attributes.back().length());
            d_waitingForName = true;
        }
    }
    void StartObject() { d_waitingForName = true; }
    void EndObject(size_t memberCount) { (void) memberCount; d_waitingForName = false; }
    void StartArray() { Default(); }
    void EndArray(size_t elementCount) { (void) elementCount; Default(); }
};
...
int main(int argc, char *argv[])
{
    ...
    rapidjson::GenericInsituStringStream iss(jsonText);
