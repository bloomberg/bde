...
// value-semantic objects in JSON format.  The 'decode' methods are function
// templates that will decode any object that meets the requirements of a
// sequence, choice, or array object as defined in the
// 'bdeat_sequencefunctions', 'bdeat_choicefunctions', and
// 'bdeat_arrayfunctions' components.  These generic frameworks provide a
...
// this component.
//
// Clients can invoke one of a set of 'decode' method templates that decode a
// specified value-semantic object from a specified stream or other input
// source.  The caller may specify the input for 'decode' as an 'bsl::istream',
// an 'bsl::streambuf', or any implementation of both.
//
// Although the JSON format is very useful for debugging and for conforming to
...
// all the classes in the 'test_messages' components in the current directory.
//
...
//
//      assert(0 == decoder.decode(os, &bob));
//
//      assert(ss);
//      assert("Bob"         == bob.name());
...
class baejsn_Decoder {
    ...
    int skipUnknownElement(const bslstl::StringRef& elementName);
        ...
        // the data corresponding to it and advancing the parser to the next
...
int baejsn_Decoder::decodeImp(TYPE *value, bdeat_TypeCategory::Choice)
{
    ...
    if (baejsn_Parser::BAEJSN_ELEMENT_NAME == d_parser.tokenType()) {
        ...
        rc = d_parser.advanceToNextToken();
        if (rc) {
            d_logStream << "Could not decode sequence, error reading "
