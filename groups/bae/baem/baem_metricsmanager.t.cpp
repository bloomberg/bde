...
// recorded and published.  The 'baem_MetricsManager' provides to general
// classes of methods:
//
// 1) A 'baem_MetricsManager' serves as an organized container of callback
// functors and 'baem_Publisher' objects.   Methods are provided to add and
...
// [ 6]  CallbackHandle registerCollectionCallback(const baem_Category *,
//                                           RecordsCollectionCallback  );
// [16]  int removeCollectionCallback(CallbackHandle );
// [ 7]  int addGeneralPublisher(bcema_SharedPtr<baem_Publisher>& );
// [18]  int addSpecificPublisher(const char *,
//                                bcema_SharedPtr<baem_Publisher>& );
// [ 7]  int addSpecificPublisher(const baem_Category *,
//                                bcema_SharedPtr<baem_Publisher>&  );
// [ 7]  int removePublisher(baem_Publisher *);
// [24]  int removePublisher(const bcema_SharedPtr<baem_Publisher>& );
// [ 5]  baem_CollectorRepository& collectorRepository();
// [ 5]  baem_MetricRegistry& metricRegistry();
// [22]  void collectSample(baem_MetricSample              *
//                          bsl::vector<baem_MetricRecord> *
//                          bool                            );
// [22]  void collectSample(baem_MetricSample              *,
//                          bsl::vector<baem_MetricRecord> *,
//                          const baem_Category            *[],
//                          int                             ,
//                          bool                            );
// [10]  void publish(const baem_Category  *, const bdet_TimeInterval& );
// [ 8]  void publish(const baem_Category *[], int, const bdet_TimeInterval& );
// [ 9]  void publish(const bsl::set<const baem_Category *>& ,
//                    const bdet_TimeInterval&               );
// [11]  void publishAll(const bdet_TimeInterval& );
// [12]  void publishAll(const bsl::set<const baem_Category *>& ,
//                       const bdet_TimeInterval& );
// [15]  void setCategoryEnabled(const baem_Category *, bool );
// [15]  void setCategoryEnabled(const char *, bool );
// [15]  void setAllCategoriesEnabled(bool );
// ACCESSORS
// [ 7]  int findGeneralPublishers(bsl::vector<baem_Publisher *> *) const;
// [17]  int findSpecificPublishers(bsl::vector<baem_Publisher *> *,
//                                  const char *        ) const;
// [ 7]  int findSpecificPublishers(bsl::vector<baem_Publisher *> *,
//                                  const baem_Category           *) const;
// [ 5]  const baem_CollectorRepository& collectorRepository() const;
...
typedef baem_MetricsManager            Obj;
...
bool withinWindow(const bdet_DatetimeTz&   localTime,
                  const bdet_Datetime&     utcExpectedTime,
                  int                      windowMs)
...
inline
const baem_Category *firstCategory(const baem_MetricSampleGroup& group)
    // Return the 'baem_Category' for the first record in 'group'.  'ASSERT'
    ...
    // behavior is undefined in 'group.numRecords()' is 0.
{
    ...
    const baem_MetricRecord *record = group.records();
    const baem_Category *value = record->metricId().category();
...
class TestCallback {
    ...
    // 'baem_MetricsManager::MetricsCollectionCallback'.  A 'TestCallback' is
    // supplied a 'baem_MetricId' at construction that determines the id of the
    ...
    // a 'bdef_Function' object matching the
    // 'baem_MetricsManager::MetricsCollectionCallback' signature that, on
  ...
  public:
    ...
    // MANIPULATORS
    void recordMetrics(bsl::vector<baem_MetricRecord> *records,
                       bool                            resetFlag);
        // Increment 'invocations()' and append to the specified 'records' a
        // 'baem_MetricRecord' containing the 'baem_MetricId' or matching the
        // 'baem_MetricRecord' supplied at construction.

    baem_MetricRecord& record();
        // Return a reference to the modifiable 'baem_MetricRecord' object
        // that will be appended on a call to 'recordMetrics'.

    baem_MetricsManager::RecordsCollectionCallback function();
        // Return a 'bdef_Function' of type
        // 'baem_MetricsManager::RecordsCollectionCallback' that will invoke
    ...
    // ACCESSORS
    const baem_MetricRecord& record() const;
        // Return a reference to the non-modifiable 'baem_MetricRecord' object
        // that will be appended on a call to 'recordMetrics'.

    baem_MetricId metricId() const;
        // Return a 'baem_MetricId' object identifying the 'baem_MetricRecord'
        // that will be appended on a call to 'recordMetrics'.
...
// MANIPULATORS
void TestCallback::recordMetrics(bsl::vector<baem_MetricRecord> *records,
                                 bool                            resetFlag)
{
    d_reset = resetFlag;
    ++d_numInvocations;
...
inline
baem_MetricRecord& TestCallback::record()
{
    return d_record;
}
...
// ACCESSORS
inline
const baem_MetricRecord& TestCallback::record() const
{
    return d_record;
}

inline
baem_MetricId TestCallback::metricId() const
{
    return d_record.metricId();
}

inline
baem_MetricsManager::RecordsCollectionCallback TestCallback::function()
{
    baem_MetricsManager::RecordsCollectionCallback cb =
...
class TestPublisher : public baem_Publisher {
    ...
    // sorted order) of the last published 'baem_MetricSample' object.  A
...
void ConcurrencyTest::execute()
{
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        bsl::vector<const baem_Category *> allCategories(Z);
        for (int i = 0; i < NUM_CATEGORIES; ++i) {
            allCategories.push_back(registry.getCategory(CATEGORIES[i]));
        }

        TestPublisher gPub(Z), aPub(Z), bPub(Z), s1Pub(Z), s2Pub(Z);
        PubPtr gPub_p(&gPub, bcema_SharedPtrNilDeleter(), Z);
        PubPtr aPub_p(&aPub, bcema_SharedPtrNilDeleter(), Z);
        PubPtr bPub_p(&bPub, bcema_SharedPtrNilDeleter(), Z);
        PubPtr s1Pub_p(&s1Pub, bcema_SharedPtrNilDeleter(), Z);
        PubPtr s2Pub_p(&s2Pub, bcema_SharedPtrNilDeleter(), Z);

        baem_MetricId idA  = registry.getId(A_VAL, S1);
        baem_MetricId idB  = registry.getId(B_VAL, S1);
        baem_MetricId idS1 = registry.getId(S1, S1);
        baem_MetricId idS2 = registry.getId(S2, S2);
        ...
        bsl::vector<baem_Publisher *> pubs(Z);
        ...
        ASSERT(Obj::BAEM_INVALID_HANDLE != kA);
        ASSERT(Obj::BAEM_INVALID_HANDLE != kB);
        ASSERT(Obj::BAEM_INVALID_HANDLE != kS1);
        ASSERT(Obj::BAEM_INVALID_HANDLE != kS2);
        ...
        LOOP2_ASSERT( bcemt_ThreadUtil::selfIdAsInt(),
                      s1Cb.invocations(),
                      2 == s1Cb.invocations());
        LOOP2_ASSERT( bcemt_ThreadUtil::selfIdAsInt(),
                      s2Cb.invocations(),
...
//..
    class EventHandler {
        ...
        // failures using 'baem_Collector' objects provided by a
        // 'baem_MetricsManager'.

        // PRIVATE DATA
        baem_Collector *d_eventMessageSize_p; // collect the message size
        baem_Collector *d_eventFailure_p;     // collect the number of failures
...
// 'baem_metric') rather than explicitly pass the address of 'manager'.
//..
...
// 'baem_MetricsManager::MetricsCollectionCallback'.
//..
    ...
    class EventHandlerWithCallback {
        ...
        // PRIVATE DATA
        bces_AtomicInt       d_numEvents;         // number of requests
     ...
     // ...
        ...
        void collectMetricsCb(bsl::vector<baem_MetricRecord> *records,
            ...
            // with the 'baem_MetricsManager::RecordsCollectionCallback'
            // function prototype.
...
//..
// In the destructor we used the 'baem_MetricsManager::CallbackHandle', stored
...
// 'baem_MetricsManager' that we will use to record and publish metric
// values.  We first create a 'baem_MetricsManager' object and a
...
// 'baem_publisher' component, in practice clients can use a standard
...
// assumes that an instance, 'manager', of the 'baem_MetricsManager' class has
...
//..
      ...
      case 24: {
        ...
        //
        // Testing:
        //   int removePublisher(const bcema_SharedPtr<baem_Publisher> );
        // --------------------------------------------------------------------
        ...
        PubPtr gtpSPtr(&gtp, bcema_SharedPtrNilDeleter(), 0);
        PubPtr tp1SPtr(&tp1, bcema_SharedPtrNilDeleter(), 0);
        PubPtr tp2SPtr(&tp2, bcema_SharedPtrNilDeleter(), 0);

        Obj mX(Z); const Obj& MX = mX;

        bsl::vector<baem_Publisher *> publisherVec(Z);
        ...
        publisherVec.clear();
        ASSERT(0 != mX.removePublisher(bcema_SharedPtr<baem_Publisher>()));
      ...
      case 23: {
        ...
        bsl::vector<bcema_SharedPtr<TestCallback> > callbacks(Z);
        for (int i = 0; i < NUM_CATEGORIES; ++i) {
            bcema_SharedPtr<TestCallback> cb;
      ...
      case 22: {
        ...
        //
        // Testing:
        //   void collectSample(baem_MetricSample              *,
        //                      bsl::vector<baem_MetricRecord> *,
        //                      bool                            );
        //   void collectSample(baem_MetricSample              *,
        //                      bsl::vector<baem_MetricRecord> *,
                              ...
                              << endl;
        {
            ...
            for (int i = 0; i < sample.numGroups(); ++i) {
                const baem_MetricSampleGroup& group = sample.sampleGroup(i);
            ...
            bsl::vector<baem_MetricRecord> records2(Z);
            ...
            for (int i = 0; i < NUM_CATEGORIES; ++i) {
                for (int j = 0; j < NUM_METRICS; ++j) {
                    ...
                    Collector *col = rep.getDefaultCollector(CATEGORIES[i],
                                                             METRICS[j]);
                    col->load(&record);
                    ASSERT(baem_MetricRecord(record.metricId()) == record);
                }
            }
        }
        ...
        do {
            // Set all the 'baem_Collector' objects to a known state and set
    ...
    case 21: {
...

        {
            ...
            PubPtr spub_p1(&tp1, bcema_SharedPtrNilDeleter(), Z);
            PubPtr spub_p2(&tp2, bcema_SharedPtrNilDeleter(), Z);
            ...
            for (int i = 0; i < NUM_CATEGORIES; ++i) {
                ...
                for (int j = 0; j < NUM_METRICS; ++j) {
                    ...
                    ASSERT(handle != Obj::BAEM_INVALID_HANDLE);
    ...
    case 19: {
        ...
        bsl::set<baem_Publisher *> generalPublishers;
        {
            ...
            BEGIN_BSLMA_EXCEPTION_TEST {
                for (int i = 0; i < NUM_PUBS; ++i) {
                    ...
                    PubPtr pub_p(TEST_PUBS[i], bcema_SharedPtrNilDeleter(), Z);
                    ...
                    mX.removePublisher(pub_p);
                    mX.addGeneralPublisher(pub_p);
                    generalPublishers.insert(pub_p.ptr());
                }
      ...
      case 18: {
        // --------------------------------------------------------------------
        // TESTING: 'addSpecificPublishers(const bdeut_StringRef&, ...);
        //
        // Concerns:
        ...
        //   'baem_Publisher' objects.  Then, for each category, invoke
        //   'findSpecificPublishers' and verify the publishers are found.
        //
        // Testing:
        //   int addSpecificPublishers(bsl::vector<baem_Publisher *> *,
        //                              const char *        ) const;
        // --------------------------------------------------------------------
        if (verbose) cout
            << endl
            << "TEST: addSpecificPublishers(const bdeut_StringRef&, ...)\n"
            << "========================================================\n";
        ...
        for (int i = 0; i < NUM_CATEGORIES; ++i) {
            ...
            bsl::vector<baem_Publisher *> results(Z);
            ASSERT(NUM_PUBLISHERS == MX.findSpecificPublishers(&results, CAT));

            bsl::vector<baem_Publisher *>::const_iterator expIt =
                                              results.begin();
      ...
      case 17: {
        // --------------------------------------------------------------------
        // TESTING: 'findSpecificPublishers(..., const bdeut_StringRef&);
        //
        // Concerns:
        ...
        // Plan:
        //   For each category in a set of unique categories, add a series of
        //   'baem_Publisher' objects.  Then, for each category, invoke the
        //   two 'findSpecificPublishers' variants and verify the results are
        //   equivalent.
        //
        // Testing:
        //   int findSpecificPublishers(bsl::vector<baem_Publisher *> *,
        //                              const char *                  ) const;
        // --------------------------------------------------------------------

        if (verbose) cout
            << endl
            << "TEST: findSpecificPublishers(...,const bdeut_StringRef&)\n"
            << "========================================================\n";
        ...
        for (int i = 0; i < NUM_CATEGORIES; ++i) {
            const char *CATEGORY = CATEGORIES[i];
            const Category *CAT = mX.metricRegistry().getCategory(CATEGORY);
            bsl::vector<baem_Publisher *> exp_results(Z);
            bsl::vector<baem_Publisher *> results(Z);
      ...
      case 16: {
        // --------------------------------------------------------------------
        // TESTING: 'registerCollectionCallback(const bdeut_StringRef&, ...);
        //
        // Concerns:
        ...
        for (int i = 0; i < NUM_CATEGORIES; ++i) {
            ...
            for (int j = 0; j < NUM_METRICS; ++j) {
                ...
                cbInfo.first = Obj::BAEM_INVALID_HANDLE;
                ...
                for (; it != callbacks.end(); ++it) {
                    ...
                    const int EXP = (info.first == Obj::BAEM_INVALID_HANDLE)
                                    ? 0 : 1;
      ...
      case 14: {
        ...
        //   each iteration:
        //   1. Set all the 'baem_Collector' objects to a known state, and set
        ...
        //
        // Testing:
        //   void publishAll(const bsl::set<const baem_Category *>& ,
        //                   const bdet_TimeInterval&);
        // --------------------------------------------------------------------
        ...
        do {
            ...
            for (int i = 0; i < NUM_CATEGORIES; ++i) {
                ...
                bsl::vector<baem_Publisher *> sPublishers(Z);
      ...
      case 13: {
        ...
        //
        // Concerns:
        //   That the 'publish' operations publish a 'baem_MetricSample' that
        ...
        bdet_TimeInterval creationTime = bdetu_SystemTime::now();
        ...
        bsl::map<const Category *, bdet_TimeInterval> lastPublicationTimes(Z);
        bsl::vector<const Category *> allCategories(Z);
        for (int i = 0; i < NUM_CATEGORIES; ++i) {
            const Category *CAT = registry.getCategory(CATEGORIES[i]);
            allCategories.push_back(CAT);
        ...
        do {
            ...
            // Publish the records.
            bcemt_ThreadUtil::sleep(bdet_TimeInterval(0, TIME_UNIT));
            bdet_TimeInterval publicationTime = bdetu_SystemTime::now();
            mX.publishAll();
            ...
            const baem_MetricSample& sample = gPub.lastSample();
            ...
            for (int i = 0; i < combIt.current().size(); ++i) {
                ...
                int  groupIndex = -1;
                for (int j = 0; j < sample.numGroups(); ++j) {
                    const baem_MetricSampleGroup& group =sample.sampleGroup(j);
      ...
      case 12: {
        // --------------------------------------------------------------------
        // TESTING: 'publishAll(const bsl::set<const baem_Category *>&, ...)
        //
        // Concerns:
        ...
        //   Perform a test for each possible combination of categories.  On
        //   each iteration:
        //   1. Set all the 'baem_Collector' objects to a known state.
        ...
        if (verbose) cout
            << endl
            << "TEST: void publishAll(bsl::set<const baem_Category *>&,...);\n"
            << "===========================================================\n";
        ...
        CombinationIterator<const Category *> combIt(allCategories, Z);
        do {
            // Set all the 'baem_Collector' objects to a known state.
            for (int i = 0; i < NUM_CATEGORIES; ++i) {
                for (int j = 0; j < NUM_METRICS; ++j) {
                    Collector *col =
                        repository.getDefaultCollector(CATEGORIES[i],
            ...
            bsl::set<const baem_Category *> excludedSet(Z);
            for (int i = 0; i < NUM_CATEGORIES; ++i) {
                if (!combIt.includesElement(i)) {
      ...
      case 11: {
        // --------------------------------------------------------------------
        // TESTING: 'publishAll(const bdet_TimeInterval& )
        //
        // Concerns:
        //   That 'publishAll' method calls the expected callbacks and invokes
        //   the expected publishers
        ...
        //   category.
        //
        //   Then set the 'baem_Collector' objects for each metric to a known
        ...
        //
        // Testing:
        //   void publishAll(const bdet_TimeInterval&);
        // --------------------------------------------------------------------

        if (verbose) cout
            << endl
            << "TEST: void publishAll(const bdet_TimeInterval& );\n"
            << "=================================================\n";
        ...
        ASSERT(bdet_TimeInterval(0, 0) < *gPub.lastElapsedTimes().begin());
      ...
      case 10: {
        // --------------------------------------------------------------------
        // TESTING: 'publish(const baem_Category *, ...)
        //
        // Concerns:
        ...
        //
        // Testing:
        //   void publish(const baem_Category *, const bdet_TimeInterval&);
        // --------------------------------------------------------------------

        if (verbose) cout
            << endl
            << "TEST: void publish(const baem_Category *, ...);\n"
            << "===============================================\n";
      ...
      case 9: {
        // --------------------------------------------------------------------
        // TESTING: 'publish(const bsl::set<const baem_Category *>&, ...)
        //
        // Concerns:
        //   That 'publish' method calls the expected callbacks and invokes the
        //   expected publishers
        ...
        //
        // Testing:
        //   void publish(const bsl::set<const baem_Category *>& ,
        //                 const bdet_TimeInterval&);
        // --------------------------------------------------------------------

        if (verbose) cout
            << endl
            << "TEST: void publish(bsl::set<const baem_Category *>&, ...);\n"
            << "===========================================================\n";
        ...
        do {
            ...
            bsl::set<const baem_Category *> categorySet(
      ...
      case 8: {
        ...
        //   categories.
        //
        //   That the 'publish' method delivers a 'baem_MetricSample',
        ...
        //   collected by 'baem_Collector' objects, and an equal number of
        //   metric names to be collected by
        //   'baem_MetricsManager::RecordCollectionCallback' functors.
        //
        //   Add to the 'baem_MetricManager' object under test: a number
        ...
        //
        // Testing:
        //   void publish(const baem_Category **,int,const bdet_TimeInterval&);
        // --------------------------------------------------------------------

        if (verbose) cout
            << endl
            << "TEST: void publish(const baem_Category **, int, ...)\n"
            << "====================================================\n";
        ...
        // Populate the 'baem_MetricsManager' object under test with
        ...
        do {
            ...
            for (int i = 0; i < NUM_CATEGORIES; ++i) {
                const Category *CAT = registry.getCategory(CATEGORIES[i]);
                bsl::vector<baem_Publisher *> specificPublishers(Z);
            ...
            //      correct.
            bsl::vector<baem_Publisher *> generalPublishers(Z);
            ASSERT(NUM_PUBLISHERS ==
      ...
      case 7: {
        ...
        //
        // Testing:
        //   int addGeneralPublisher(bcema_SharedPtr<baem_Publisher>& );
        //   int addSpecificPublisher(const baem_Category *category,
        //                            bcema_SharedPtr<baem_Publisher>&  );
        //   int removePublisher(const bcema_SharedPtr<baem_Publisher>& )
        //   void publish(const baem_Category *, const bdet_TimeInterval&  );
        //   void publishAll(const bdet_TimeInterval&  );
        //   int findGeneralPublishers(bsl::vector<baem_Publisher *> *) const;
        //   int findSpecificPublishers(bsl::vector<baem_Publisher *> *,
        //                              const baem_Category           *) const;
        // --------------------------------------------------------------------
        ...
        // ------------------- Verify 'general' publishers --------------------
        {
            ...
            for (int i = 0; i < NUM_PUBS; ++i) {
                ...
                for (int j = 0; j < NUM_CATEGORIES; ++j) {
                    ...
                    bsl::vector<baem_Publisher *> pubsFound(Z);
                ...
                bsl::set<baem_Publisher *> pubSet(Z);
            ...
            for (int i = 0; i < NUM_PUBS; ++i) {
                PubPtr pub_p(TEST_PUBS[i], bcema_SharedPtrNilDeleter(), Z);
                ASSERT(0 == mX.removePublisher(pub_p.ptr()));
                ASSERT(0 != mX.removePublisher(pub_p.ptr()));
                mX.publishAll();
                for (int j = 0; j < NUM_PUBS; ++j) {
        ...
        // ------------------- Verify 'specific' publishers -------------------
        {
            ...
            for (int i = 0; i < NUM_CATEGORIES; ++i) {
                ...
                mX.collectorRepository().getDefaultCollector(CATEGORIES[i],
                                                             "A");
                for (int j = 0; j < NUM_PUBS; ++j) {
                    PubPtr pub_p(TEST_PUBS[j], bcema_SharedPtrNilDeleter(), Z);
                    ASSERT(0 == mX.addSpecificPublisher(CAT, pub_p));
      ...
      case 6: {
        ...
        //
        // Plan:
        //   Create a map from 'baem_MetricId' to a '(CallbackHandle,
      ...
      case 4: {
...

        {
            ...
            for (int i = 0; i < NUM_IDS; ++i) {
                ...
                const baem_MetricRecord VAL(id, 2, 2, 2, 2);
                ...
                ASSERT(baem_MetricRecord(id) == TCB1.record());
                ASSERT(baem_MetricRecord(id) == TCB2.record());
                ...
                for (int j = 0; j < 10; ++j) {
                    bsl::vector<baem_MetricRecord> rec1(Z), rec2(Z), rec3(Z);
                    cb1(&rec1, true);
                    cb2(&rec2, true);
                    cb3(&rec3, true);
                    ...
                    ASSERT(baem_MetricRecord(id) == rec1.front());
      ...
      case 3: {
        ...
        baem_MetricId METRIC_A = mX.metricRegistry().getId("A", "A");
        baem_MetricId METRIC_B = mX.metricRegistry().getId("B", "B");
        baem_MetricId METRIC_C = mX.metricRegistry().getId("C", "C");

        const baem_Category *CAT_A = mX.metricRegistry().getCategory("A");
        const baem_Category *CAT_B = mX.metricRegistry().getCategory("B");
        const baem_Category *CAT_C = mX.metricRegistry().getCategory("C");

        TestCallback CB_A(METRIC_A, Z), CB_B(METRIC_B, Z), CB_C(METRIC_C, Z);
        TestPublisher PUB_A(Z), PUB_B(Z), PUB_ALL(Z);
        PubPtr PUBPTR_A(&PUB_A, bcema_SharedPtrNilDeleter(), Z);
        PubPtr PUBPTR_B(&PUB_B, bcema_SharedPtrNilDeleter(), Z);
        PubPtr PUBPTR_ALL(&PUB_ALL, bcema_SharedPtrNilDeleter(), Z);
        ...
        bsl::set<const baem_Category *> exclude(Z);
      ...
      case 2: {
        ...
        // Add general publishers.
        bsl::vector<baem_Publisher *> pubV(Z);
        ASSERT(0 == defaultAllocator.numBytesInUse());
      ...
      case 1: {
        ...
        PubPtr pA_1(&tpa_1, bcema_SharedPtrNilDeleter(), Z);
        PubPtr pA_2(&tpa_2, bcema_SharedPtrNilDeleter(), Z);
        PubPtr pB_1(&tpb_1, bcema_SharedPtrNilDeleter(), Z);
        PubPtr pB_2(&tpb_2, bcema_SharedPtrNilDeleter(), Z);
        PubPtr pC_1(&tpc_1, bcema_SharedPtrNilDeleter(), Z);
        PubPtr pC_2(&tpc_2, bcema_SharedPtrNilDeleter(), Z);
        ...
        bsl::vector<baem_Publisher *> pVec(Z);
        ASSERT(2 == MX.findSpecificPublishers(&pVec, CAT_A));
        ASSERT(2 == pVec.size());
        ASSERT(pA_1.ptr() == pVec[0] || pA_2.ptr() == pVec[0]);
        ASSERT(pA_1.ptr() == pVec[1] || pA_2.ptr() == pVec[1]);

        pVec.clear();
        ASSERT(2 == MX.findSpecificPublishers(&pVec, CAT_B));
        ASSERT(2 == pVec.size());
        ASSERT(pB_1.ptr() == pVec[0] || pB_2.ptr() == pVec[0]);
        ASSERT(pB_1.ptr() == pVec[1] || pB_2.ptr() == pVec[1]);

        pVec.clear();
        ASSERT(2 == MX.findSpecificPublishers(&pVec, CAT_C));
        ASSERT(2 == pVec.size());
        ASSERT(pC_1.ptr() == pVec[0] || pC_2.ptr() == pVec[0]);
        ASSERT(pC_1.ptr() == pVec[1] || pC_2.ptr() == pVec[1]);
        ...
        LOOP_ASSERT(*tpa_1.lastElapsedTimes().begin(),
            bdet_TimeInterval(0, 0) < *tpa_1.lastElapsedTimes().begin());
        ...
        LOOP_ASSERT(*tpa_2.lastElapsedTimes().begin(),
            bdet_TimeInterval(0, 0) < *tpa_2.lastElapsedTimes().begin());
        ...
        if (verbose) {
            cout << "\tVerify publish(const baem_Category **categories,...)\n";
        }
        ...
        const baem_Category *CATEGORIES[] = {CAT_A, CAT_B, CAT_C};
        const int  NUM_CATEGORIES = sizeof(CATEGORIES)/sizeof(*CATEGORIES);
        bcemt_ThreadUtil::microSleep(100000, 0);
        ...
        if (verbose) {
            cout << "\tVerify publishAll(bsl::set<const baem_Category *>&,)\n";
        }
        ...
        bsl::set<const baem_Category *> catSet;
        catSet.insert(CAT_C);
        ...
        bcemt_ThreadUtil::sleep(bdet_TimeInterval(0, TIME_UNIT));
        bdet_TimeInterval catATime  = bdetu_SystemTime::now();
        bdet_TimeInterval catAIntvl = catATime - startTime;
        mX.publish(CAT_A);

        bcemt_ThreadUtil::sleep(bdet_TimeInterval(0, TIME_UNIT));
        bdet_TimeInterval catBTime  = bdetu_SystemTime::now();
        bdet_TimeInterval catBIntvl = catBTime - startTime;
        mX.publish(CAT_B);
        ...
        bcemt_ThreadUtil::sleep(bdet_TimeInterval(0, TIME_UNIT));
        bdet_TimeInterval allTime  = bdetu_SystemTime::now();
        mX.publishAll();

        catAIntvl = allTime - catATime;
        catBIntvl = allTime - catBTime;
        bdet_TimeInterval catCIntvl = allTime - startTime;
