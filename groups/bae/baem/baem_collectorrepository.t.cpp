// baem_collectorrepository.t.cpp  -*-C++-*-
...
// [ 4]  void collectAndReset(bsl::vector<baem_MetricRecord> *,
//                            const baem_Category            *);
// [ 6]  baem_Collector *getDefaultCollector(const bdeut_StringRef& ,
//                                           const bdeut_StringRef& );
// [ 3]  baem_Collector *getDefaultCollector(const baem_MetricId& );
// [ 6]  baem_IntegerCollector *getDefaultIntegerCollector(
//                                               const bdeut_StringRef& ,
//                                               const bdeut_StringRef& );
// [ 3]  baem_IntegerCollector *getDefaultIntegerCollector(
//                                                  const baem_MetricId& );
// [ 5]  baem_Collector *addCollector(const bdeut_StringRef& ,
//                                    const bdeut_StringRef& );
// [ 2]  baem_Collector *addCollector(const baem_MetricId& metricId);
// [ 5]  baem_IntegerCollector *addIntegerCollector(
//                                               const bdeut_StringRef& ,
//                                               const bdeut_StringRef& );
// [ 2]  baem_IntegerCollector *addIntegerCollector(const baem_MetricId& );
// [ 2]  int getAddedCollectors(
//                      bsl::vector<baem_Collector *>        *,
//                      bsl::vector<baem_IntegerCollector *> *,
//                      const baem_MetricId&                  );
...
// ACCESSORS
// [ 2]  int getAddedCollectors(
//                      bsl::vector<const baem_Collector *>        *,
//                      bsl::vector<const baem_IntegerCollector *> *,
//                      const baem_MetricId&                        ) const;
...
typedef baem_CollectorRepository Obj;
typedef baem_Collector           Col;
typedef baem_IntegerCollector    ICol;
typedef baem_MetricId            Id;
typedef baem_MetricRecord        Rec;
typedef bcema_SharedPtr<Col>     ColSPtr;
typedef bcema_SharedPtr<ICol>    IColSPtr;
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        for (int j = 0; j < NUM_CATEGORIES; ++j) {
            ...
            for (int k = 0; k < NUM_IDS; ++k) {
                const char *ID = IDS[k];
                baem_MetricId id = reg.getId(CATEGORY, ID);
                Col  *col   = mX.getDefaultCollector(CATEGORY, ID);
                ICol *iCol  = mX.getDefaultIntegerCollector(CATEGORY, ID);
                Col  *col2  = mX.addCollector(CATEGORY, ID).ptr();
                ICol *iCol2 = mX.addIntegerCollector(CATEGORY, ID).ptr();
        ...
        for (int j = 0; j < NUM_CATEGORIES; ++j) {
            const char *CATEGORY = CATEGORIES[j];
            const baem_Category *category = REG.findCategory(CATEGORY);
            ...
            if (CATEGORY == S1 || CATEGORY == S2) {
                ...
                baem_MetricId idA = reg.getId(CATEGORY, "A");
                baem_MetricId idB = reg.getId(CATEGORY, "B");
...
{
    bdef_Function<void(*)()> job = bdef_BindUtil::bindA(d_allocator_p,
...
template <typename T>
bool sharedPtrLess(const bcema_SharedPtr<T>& lhs,
                   const bcema_SharedPtr<T>& rhs)
{
...
bool vectorEquals(const bsl::vector<T *>&                 lhs,
                  const bsl::vector<bcema_SharedPtr<T> >& rhs)
{
    ...
    bsl::vector<bcema_SharedPtr<T> > copyRhs(rhs);

    bsl::sort(copyLhs.begin(), copyLhs.end(), bsl::less<T *>());
    bsl::sort(copyRhs.begin(), copyRhs.end(), &sharedPtrLess<T>);

    typename bsl::vector<T *>::const_iterator lhsIt = copyLhs.begin();
    typename bsl::vector<bcema_SharedPtr<T> >::const_iterator rhsIt =
                                                             copyRhs.begin();
...
//..
// The exact format of the output is determined by the 'baem_MetricRecord'
...
//..
      ...
      case 8: {
        ...
        baem_CollectorRepository mX(&registry, &testAllocator);
        {
      ...
      case 7: {
        ...
        //   (across multiple instances of 'baem_Collector' and
        //   'baem_IntegerCollector' for the same metric).
        //
        // Plan:
        ...
        //
        // Testing:
        //    void collect(bsl::vector<baem_MetricRecord> *,
        //                 const baem_Category            *);
        // --------------------------------------------------------------------
        ...
        for (int i = 0; i < NUM_CATEGORIES; ++i) {
            ...
            for (int j = 0; j < NUM_CATEGORIES; ++j) {
                ...
                for (int k = 0; k < NUM_METRICS; ++k) {
                    baem_MetricId metric = reg.getId(CATEGORY, METRICS[k]);
                    for (int l = 0; l < NUM_COLS; ++l) {
                        Col *col   = mX.addCollector(metric).ptr();
                        ICol *iCol = mX.addIntegerCollector(metric).ptr();
                        if (i == j) {
            ...
            // Collect values from the category under test.
            {
                ...
                const baem_Category *category = reg.getCategory(CATEGORY);
                ...
                for (int j = 0; j < NUM_METRICS; ++j) {
                    baem_MetricId metric = reg.getId(CATEGORY,METRICS[j]);
                    ColSPtrVector cols;
                    IColSPtrVector iCols;
                    mX.getAddedCollectors(&cols, &iCols, metric);
                    for (int k = 0; k < cols.size(); ++k) {
                        baem_MetricRecord E(metric,  j, 2 * j, -j, j);
      ...
      case 6: {
        ...
        //   returns the address of a valid 'baem_Collector' object (or
        ...
        //   'baem_CollectorRepository' and use 'getDefaultCollector' and
        ...
        //
        // Testing:
        //   baem_Collector *getDefaultCollector(const bdeut_StringRef& ,
        //                                const bdeut_StringRef& );
...

        {
            ...
            for (int i = 0; i < NUM_METRICS; ++i) {
                ...
                for (int j = 0; j < NUM_ADDITIONAL; ++j) {
                    ASSERT(col  != mX.addCollector(CATEGORY, NAME).ptr());
                    ASSERT(iCol != mX.addIntegerCollector(id).ptr());
                    ASSERT(col  == mX.getDefaultCollector(id));
                    ASSERT(iCol == mX.getDefaultIntegerCollector(id));
                }
                ...
                for (int i = 0; i < NUM_ADDITIONAL; ++i) {
                    ASSERT(col  != colV[i].ptr());
                    ASSERT(iCol != iColV[i].ptr());
                }
            }
        }
        {
      ...
      case 5: {
        ...
        //   address of a new valid 'baem_Collector' object (or
        //   'baem_IntegerCollector').
        //
...

        {
            ...
            for (int i = 0; i < NUM_METRICS; ++i) {
                ...
                Col *col   = mX.addCollector(CATEGORY, NAME).ptr();
                ICol *iCol = mX.addIntegerCollector(CATEGORY, NAME).ptr();
      ...
      case 4: {
        ...
        //   correctly (across multiple instances of 'baem_Collector' and
        //   'baem_IntegerCollector' for the same metric).
        //
        // Plan:
        //   For an independent sequences of metric ids and categories, select
        ...
        //
        // Testing:
        //    void collectAndReset(bsl::vector<baem_MetricRecord> *,
        //                         const baem_Category            *);
        // --------------------------------------------------------------------
            ...
            // Collect values from the category under test.
            {
                ...
                for (int j = 0; j < NUM_METRICS; ++j) {
                    ...
                    for (int k = 0; k < cols.size(); ++k) {
                        baem_MetricRecord r1, r2;
                        cols[k]->load(&r1); iCols[k]->load(&r2);
                        ASSERT(baem_MetricRecord(metric) == r1);
                        ASSERT(baem_MetricRecord(metric) == r2);
                    }
                }
            }
      ...
      case 3: {
        ...
        // Concerns:
        //   That 'getDefaultCollector' (and 'getDefaultIntegerCollector')
        //   returns the valid address of the default baem_Collector' (or
        //   'baem_IntegerCollector') object.
        //
        ...
        //
        // Testing:
        //   baem_Collector *getDefaultCollector(const baem_MetricId& );
        //   baem_IntegerCollector *getDefaultIntegerCollector(
        //                                            const baem_MetricId& );
        // --------------------------------------------------------------------
        ...
        const int NUM_METRICS = sizeof METRICS / sizeof *METRICS;
        {
            ...
            for (int i = 0; i < NUM_METRICS; ++i) {
                ...
                cols[id].push_back(mX.addCollector(id).ptr());
                iCols[id].push_back(mX.addIntegerCollector(id).ptr());
        ...
        }
        {
            ...
            for (int i = 0; i < NUM_METRICS; ++i) {
                ...
                for (int j = 0; j < NUM_ADDITIONAL; ++j) {
                    ASSERT(col  != mX.addCollector(id).ptr());
                    ASSERT(iCol != mX.addIntegerCollector(id).ptr());
                    ASSERT(col  == mX.getDefaultCollector(id));
                    ASSERT(iCol == mX.getDefaultIntegerCollector(id));
                }
      ...
      case 2: {
        ...
        //                          addIntegerCollector(const baem_MetricId& );
        //   baem_MetricRegistry &registry();
        //   int getAddedCollectors(
        //            bsl::vector<bcema_SharedPtr<baem_Collector> >  *,
        //            bsl::vector<bcema_SharedPtr<baem_IntegerCollector> >  *,
        //            const baem_MetricId&                                   );
        ...
        }
        {
            ...
            for (int i = 0; i < NUM_METRICS; ++i) {
                ...
                Col *collector  = mX.addCollector(id).ptr();
                ICol *iCollector = mX.addIntegerCollector(id).ptr();
        ...
        }
        {
            ...
            ColSPtr dummyCol((Col *)0xDEAD, bcema_SharedPtrNilDeleter(), 0);
            IColSPtr dummyICol((ICol *)0xDEAD, bcema_SharedPtrNilDeleter(), 0);
            ...
            Obj mX(&reg, Z); const Obj& MX = mX;
            for (int i = 0; i < NUM_METRICS; ++i) {
                mX.addCollector(METRIC_AA).ptr();
                mX.addIntegerCollector(METRIC_AA).ptr();
      ...
      case 1: {
        ...
        Id METRIC_BB = reg.getId("B", "B");
        {
            ...
            if (veryVerbose) {
                cout << "\tTest getDefaultCollector(const baem_MetricId& )"
                     << bsl::endl;
            }
            ...
            if (veryVerbose) {
                cout << "\tTest addCollector(const baem_MetricId& )"
                     << bsl::endl;
            }
            Col *COL_AA_2 = mX.addCollector(METRIC_AA).ptr();
            Col *COL_AB_2 = mX.addCollector(METRIC_AB).ptr();
            Col *COL_BA_2 = mX.addCollector(METRIC_BA).ptr();
            Col *COL_BB_2 = mX.addCollector(METRIC_BB).ptr();
        ...
        }
        {
            Obj mX(&reg, Z); const Obj& MX = mX;
            if (veryVerbose) {
                cout << "\tTest getDefaultCollector(const bdeut_StringRef &, "
                     << "const bdeut_StringRef& )" << endl;
            }
        ...
        }
        {
            ...
            if (veryVerbose) {
                cout << "\tTest addIntegerCollector(const baem_MetricId& )"
                     << bsl::endl;
            }
            ICol *COL_AA_2 = mX.addIntegerCollector(METRIC_AA).ptr();
            ICol *COL_AB_2 = mX.addIntegerCollector(METRIC_AB).ptr();
            ICol *COL_BA_2 = mX.addIntegerCollector(METRIC_BA).ptr();
            ICol *COL_BB_2 = mX.addIntegerCollector(METRIC_BB).ptr();
        ...
        }
        {
            ...
            if (veryVerbose) {
                cout << "\tTest getDefaultIntegerCollector(const "
                     << "bdeut_StringRef &, const bdeut_StringRef& )"
                     << bsl::endl;
            }
        ...
        }
        {
            ...
            Col *col1 = mX.addCollector(METRIC_AA).ptr();
            Col *col2 = mX.addCollector(METRIC_AA).ptr();
            Col *col3 = mX.addCollector(METRIC_AA).ptr();

            mX.addCollector(METRIC_AB).ptr();

            ICol *icol1 = mX.addIntegerCollector(METRIC_AA).ptr();
            ICol *icol2 = mX.addIntegerCollector(METRIC_AA).ptr();
            ICol *icol3 = mX.addIntegerCollector(METRIC_AA).ptr();

            mX.addIntegerCollector(METRIC_AB).ptr();
            ...
            for (int i = 0; i < 3; ++i) {
                LOOP3_ASSERT(i, expCol[i], collectors[i].ptr(),
                                           expCol[i]  == collectors[i].ptr());
                LOOP3_ASSERT(i, expICol[i], intCollectors[i].ptr(),
                                        expICol[i] == intCollectors[i].ptr());
            }
        }
        {
            ...
            Col *col1 = mX.addCollector(METRIC_AA).ptr();
            Col *col2 = mX.addCollector(METRIC_AA).ptr();
            Col *col3 = mX.addCollector(METRIC_AB).ptr();
            Col *col4 = mX.addCollector(METRIC_AB).ptr();
            Col *ignored1 = mX.addCollector(METRIC_BA).ptr();

            ICol *icol1 = mX.addIntegerCollector(METRIC_AA).ptr();
            ICol *icol2 = mX.addIntegerCollector(METRIC_AA).ptr();
            ICol *icol3 = mX.addIntegerCollector(METRIC_AB).ptr();
            ICol *icol4 = mX.addIntegerCollector(METRIC_AB).ptr();
            ICol *ignored2 = mX.addIntegerCollector(METRIC_BA).ptr();
            ...
            bsl::sort(records.begin(), records.end(), recordLess);
            const baem_MetricRecord& a = records[0];
            const baem_MetricRecord& b = records[1];
