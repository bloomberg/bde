// btesos_tcptimedcbchannel.h    -*-C++-*-
#ifndef INCLUDED_BTESOS_TCPTIMEDCBCHANNEL
#define INCLUDED_BTESOS_TCPTIMEDCBCHANNEL
...
//
//@CLASSES:
//  btesos_TcpTimedCbChannel: non-blocking channel over TCP/IPv4 sockets
//
//@SEE_ALSO:  btesos_tcptimedcbacceptor btesos_tcptimedcbconnector
//
//@SEE_ALSO:  btesos_tcpcbchannel bteso_streamsocket
//
...
// operations are not required, 'btesos_tcpcbchannel' is preferred from
...
class btesos_TcpTimedCbChannel : public btesc_TimedCbChannel {
    ...
    bdef_Function<void (*)()>    d_readTimerFunctor;
    bdef_Function<void (*)()>    d_writeTimerFunctor;
  ...
  private:
    btesos_TcpTimedCbChannel(const btesos_TcpTimedCbChannel&); // not impl.
  ...
  public:
    ...
    int timedBufferedWritev(const btes_Ovec   *buffers,
                            int                       numBuffers,
                            const bdet_TimeInterval&  timeout,
                            const WriteCallback&      writeCallback,
                            int                       flags = 0);
    int timedBufferedWritev(const btes_Iovec  *buffers,
                            int                       numBuffers,
                            const bdet_TimeInterval&  timeout,
                            const WriteCallback&      writeCallback,
                            int                       flags = 0);
...
inline
bteso_StreamSocket<bteso_IPv4Address>
         *btesos_TcpTimedCbChannel::socket() const
{
    return d_socket_p;
}

inline
bteso_TimerEventManager *btesos_TcpTimedCbChannel::readEventManager() const {
    return d_rManager_p;
}

inline
bteso_TimerEventManager *btesos_TcpTimedCbChannel::writeEventManager() const {
    return d_wManager_p;
}
