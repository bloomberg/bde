...
//-----------------------------------------------------------------------------
// CREATORS
// [ 2] btesos_TcpCbChannel(..., *manager, ...);
// [ 5] btesos_TcpCbChannel(..., *rManager, *wManager, ...);
// [ 2] ~btesos_TcpCbChannel();
// MANIPULATORS
// [ 9] int read();
// [11] int readv();
// [ 6] int readRaw();
...
// [ 8] int write();
// [ 7] int writeRaw();
// [14] int writev(const btes_Iovec::Ovec *buffers, ...);
// [13] int writev(const btes_Iovec::Iovec *buffers, ...);
// [14] int writevRaw(const btes_Iovec::Ovec *buffers, ...);
// [13] int writevRaw(const btes_Iovec::Iovec *buffers, ...);
// [15] int bufferedWrite();
// [16] int bufferedWritev(const btes_Iovec::Ovec *buffers, ...);
// [16] int bufferedWritev(const btes_Iovec::Iovec *buffers, ...);
// [ 4] void cancelAll();
// [ 2] void cancelRead();
// [ 2] void cancelWrite();
...
//-----------------------------------------------------------------------------
static int
gg(btesos_TcpCbChannel *channel, Buffer *buffer,
                ...
                if ('r' == *(script+2)) {
                    if ('t' == *(script+3)) {  // rbrt
                        ...
                        btesc_CbChannel::BufferedReadCallback callback(
                                bdef_BindUtil::bind( &bufferedReadCallback
                                                   , _1, _2, _3
                                                   , rManager
                                                   , channel
                                                   , expStatus
            ...
            case 'r': {
                if ('t' == *(script+2)) {   // rrt
                    ...
                    ret = sscanf(script, "rr%d,%d,%d,%d", &readLen,
                                 &optFlag, &expStatus, &expAugStatus);
                    ASSERT(4 == ret);
                    callback = bdef_BindUtil::bind( &readCallback
                                                  , _1, _2
                                                  , rManager
                                                  , channel
                                                  , expStatus
            ...
            case 'v': {
                ...
                if ('r' == *(script+2)) {
                       if ('t' == *(script+3)) { // timedReadvRaw()
                           ...
                           callback = bdef_BindUtil::bind( &readvCallback
                                                         , _1, _2
                                                         , rManager
                                                         , channel
                                                         , expStatus
            ...
            case 'r':
                if ('t' == *(script+2)) {     // timedWriteRaw()
                    ...
                    ret = sscanf(script, "wr%d,%d,%d,%d",
                                 ...
                                 &expAugStatus);
                    ASSERT(4 == ret);         // Return 4 numbers.
                    callback = bdef_BindUtil::bind( &writeCallback
                                                  , _1, _2
                                                  , wManager
                                                  , channel
                                                  , expStatus
      ...
      case 17: {
       ...
       if (verbose) cout << "\nTesting Usage Example"
        ...
        bteso_TcpTimerEventManager rEventManager(
                                     ...
                                     &testAllocator);
        bteso_TcpTimerEventManager wEventManager(
                                     bteso_TcpTimerEventManager::BTESO_NO_HINT,
                                     &testAllocator);
        {
            ...
            btesos_TcpCbChannel channel(sSocket, &rEventManager,
            ...
            bdef_Function<void (*)(int, int)> callback1(
                    bdef_BindUtil::bind(&myReadCallback, _1, _2, readLen1, 0));
            ...
            bdef_Function<void (*)(int, int)> callback2(
                    bdef_BindUtil::bind(&myReadCallback, _1, _2, readLen2, 0));
            // Issue a read request:
...
// TBD FIX ME
        ...
        //
        // Testing:
        //   int bufferedWritev(const btes_Iovec::Ovec *buffers...);
        //   int bufferedWritev(const btes_Iovec::Iovec *buffers);
        // -------------------------------------------------------------------
  ...
  // alignment should be as the following statements:
        ...
        for (int i = 0; i < NUM_SCRIPTS; ++i) {
            ...
            // driver.
            {
                ...
                btesos_TcpCbChannel channel(sSocket, &eventManager,
                                            &testAllocator);
...
// TBD FIX ME
        ...
        //
        // Testing:
        //   int writevRaw(btes_Ovec);
        // -------------------------------------------------------------------
        if (verbose) cout << "\nTESTING 'writev(btes_Ovec)' METHOD"
                          << "\n=================================="
                          << "\nTESTING 'writevRaw(btes_Ovec)' METHOD"
                          << "\n====================================="
                          << endl;
   ...
   { // entry 13 Concern: if this channel can work correctly for new read
     ...
     {L_,  0,            0,      0,     0,        0,  NON_VEC,      ""      }
        ...
        for (int i = 0; i < NUM_SCRIPTS; ++i) {
            ...
            bteso_TcpTimerEventManager rEventManager(&testAllocator),
                                       wEventManager(&testAllocator);
            {
                // We should guarantee that the 'channel's destructor is
                // invoked before the corresponding 'streamSocket' destructor
     ...
     {L_,  0,              0,        0,            0,         0,     ""      }
        ...
        for (int i = 0; i < NUM_SCRIPTS; ++i) {
...

            {
                ...
                btesos_TcpCbChannel channel(sSocket, &eveManager,
                                        &testAllocator);
...
// TBD FIX ME
 ...
 // alignment should be as the following statements:
        ...
        for (int i = 0; i < NUM_SCRIPTS; ++i) {
            ...
            bteso_TcpTimerEventManager eventManager(&testAllocator);
...
// The normal alignment should be as the following statement:
            ...
            for (int i = 0; i < NUM_OBJS; ++i) {
...

                {
                    ...
                    btesos_TcpCbChannel channel(sSocket,
                ...
                factory.deallocate(sSocket);
                bteso_SocketImpUtil::close(OBJECTS[i].d_handle[0]);
                bteso_SocketImpUtil::close(OBJECTS[i].d_handle[1]);
            }
        }
...
// The normal alignment should be as the following statement:
            ...
            factory.deallocate(sSocket);
            factory.deallocate(cSocket);
            bteso_SocketImpUtil::close(socket[0]);
            bteso_SocketImpUtil::close(socket[1]);
        }
#endif
      } break;
      case 4: {
...
// TBD FIX ME
        ...
        //
        // Testing:
        //   btesos_TcpCbChannel(eventManager);
        //   ~btesos_TcpCbChannel();
        //   void cancelRead();
        //   void cancelWrite();
        //   void invalidateRead();
...
// The normal alignment should be as the following statement:
 ...
 // ----v The normal alignment should be as the following statement:
        ...
        for (int i = 0; i < NUM_CHANNELS; ++i) {
            ...
            factory.deallocate(sSocket);
            factory.deallocate(cSocket);
            bteso_SocketImpUtil::close(CHANNELS[i].d_handle[0]);
            bteso_SocketImpUtil::close(CHANNELS[i].d_handle[1]);
        }
      } break;
      default: {
