// btesos_tcpcbchannel.h         -*-C++-*-
#ifndef INCLUDED_BTESOS_TCPCBCHANNEL
#define INCLUDED_BTESOS_TCPCBCHANNEL
...
//
//@CLASSES:
//  btesos_TcpCbChannel: non-blocking non-timed channel over TCP/IPv4 sockets
//
//@SEE_ALSO:  btesos_tcptimedcbacceptor btesos_tcptimedcbconnector
//
//@SEE_ALSO:  btesos_tcptimedcbchannel bteso_streamsocket
//
//@SEE_ALSO:  btesc_timedcbchannel btesc_timedchannel
//
//@DESCRIPTION: This component provides a non-blocking communication channel
// over TCP/IPv4 sockets that adheres to 'btesc_TimedCbChannel' protocol.
...
// of the framework (see 'btesos_tcptimedcbacceptor' and
// 'btesos_tcptimedcbconnector' components).
//
///Thread-safety
///-------------
...
///=============
//..
// See 'btesos_tcptimedcbacceptor' component.
//..
...
#ifndef INCLUDED_BTESC_CBCHANNEL
...
class btesos_TcpCbChannel_RReg;  // Component local class declaration
class btesos_TcpCbChannel_WReg;  // Component local class declaration

                          // =========================
                          // class btesos_TcpCbChannel
                          // =========================

class btesos_TcpCbChannel : public btesc_CbChannel {
    ...
    bteso_TimerEventManager     *d_rManager_p;
    bteso_TimerEventManager     *d_wManager_p;
    ...
    bdef_Function<void (*)()>    d_bufferedReadFunctor;
    bdef_Function<void (*)()>    d_readFunctor;

    bdef_Function<void (*)()>    d_bufferedWriteFunctor;
    bdef_Function<void (*)()>    d_writeFunctor;
  ...
  private:
    btesos_TcpCbChannel(const btesos_TcpCbChannel&);           // not impl.
    btesos_TcpCbChannel& operator=(const btesos_TcpCbChannel); // not impl.

  public:
    // CREATORS
    btesos_TcpCbChannel(bteso_StreamSocket<bteso_IPv4Address> *sSocket,
    ...
    int bufferedWritev(const btes_Ovec  *buffers,
                       int                      numBuffers,
                       const WriteCallback&     writeCallback,
                       int                      flags = 0);
    int bufferedWritev(const btes_Iovec *buffers,
                       int                      numBuffers,
                       const WriteCallback&     writeCallback,
                       int                      flags = 0);
        // Initiate a non-blocking operation to write to this channel from the
    ...
    void setReadEventManager(bteso_TimerEventManager *manager);
    ...
    void setWriteEventManager(bteso_TimerEventManager *manager);
    ...
    bteso_TimerEventManager *readEventManager() const;
        // Return the address of an event manager used for monitoring for
        // incoming data on this channel.

    bteso_TimerEventManager *writeEventManager() const;
        // Return the address of an event manager used for monitoring for
...
inline
bteso_StreamSocket<bteso_IPv4Address> *btesos_TcpCbChannel::socket() const
{
    return d_socket_p;
}

inline
bteso_TimerEventManager *btesos_TcpCbChannel::readEventManager() const {
    return d_rManager_p;
}

inline
bteso_TimerEventManager *btesos_TcpCbChannel::writeEventManager() const {
    return d_wManager_p;
}
