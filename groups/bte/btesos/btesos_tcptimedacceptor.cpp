// btesos_tcptimedacceptor.cpp    -*-C++-*-
#include <btesos_tcptimedacceptor.h>

#include <bdes_ident.h>
BDES_IDENT_RCSID(btesos_tcptimedacceptor_cpp,"$Id$ $CSID$")
...
// CREATORS
btesos_TcpTimedAcceptor::btesos_TcpTimedAcceptor(
...
void btesos_TcpTimedAcceptor::deallocate(btesc_Channel *channel) {
    BSLS_ASSERT(channel);
...
btesc_Channel *btesos_TcpTimedAcceptor::allocate(int *status, int flags)
{
    BSLS_ASSERT(status);
...
btesc_TimedChannel *btesos_TcpTimedAcceptor::allocateTimed(int *status,
                                                          int flags)
{
    BSLS_ASSERT(status);
    if (d_isInvalidFlag || NULL == d_serverSocket_p) {
...
btesc_Channel *btesos_TcpTimedAcceptor::timedAllocate(
                                           int                      *status,
                                           const bdet_TimeInterval&  timeout,
                                           int                       flags)
{
    ...
    btesos_TcpChannel *channel =
        BloombergLP::timedAllocate<btesos_TcpChannel>(status, flags,
                                         d_serverSocket_p,
                                         &d_pool, timeout);
    if (channel) {
        bsl::vector<btesc_Channel*>::iterator idx =
...
btesc_TimedChannel *btesos_TcpTimedAcceptor::timedAllocateTimed(
                                            int                     *status,
                                            const bdet_TimeInterval& timeout,
                                            int                      flags)
{
    ...
    btesos_TcpTimedChannel *channel =
        BloombergLP::timedAllocate<btesos_TcpTimedChannel>(status, flags,
                                              d_serverSocket_p,
                                              &d_pool, timeout);
    if (channel) {
        bsl::vector<btesc_Channel*>::iterator idx =
...
int btesos_TcpTimedAcceptor::close()
{
...
int btesos_TcpTimedAcceptor::open(const bteso_IPv4Address& endpoint,
...
int btesos_TcpTimedAcceptor::setOption(int level, int option, int value)
{
...
int
btesos_TcpTimedAcceptor::getOption(int *result, int level, int option) const
{
    BSLS_ASSERT(!d_isInvalidFlag);
    BSLS_ASSERT(d_serverSocket_p);
