// btesos_tcptimedcbchannel.t.cpp                                     -*-C++-*-
...
#include <bteso_inetstreamsocketfactory.h>   // create a streamsocket opt.
...
// that adheres to 'btesc_CbChannel' protocol.  The basic plan for the
...
//-----------------------------------------------------------------------------
// CREATORS
// [ 2] btesos_TcpTimedCbChannel(..., *manager, ...);
// [ 5] btesos_TcpTimedCbChannel(..., *rManager, *wManager, ...);
// [ 2] ~btesos_TcpTimedCbChannel();
// MANIPULATORS
// [ 6] int read();
// [ 7] int timedRead();
// [14] int readv();
...
// [20] int writev(const btes_Iovec::Ovec *buffers, ...);
// [20] int writev(const btes_Iovec::Iovec *buffers, ...);
// [21] int timedWritev(const btes_Iovec::Ovec *buffers, ...);
// [21] int timedWritev(const btes_Iovec::Iovec *buffers, ...);
// [21] int writevRaw(const btes_Iovec::Ovec *buffers, ...);
// [21] int writevRaw(const btes_Iovec::Iovec *buffers, ...);
// [21] int timedWritevRaw(const btes_Iovec::Ovec *buffers, ...);
// [21] int timedWritevRaw(const btes_Iovec::Iovec *buffers, ...);
// [22] int bufferedWrite();
// [23] int timedBufferedWrite();
// [24] int bufferedWritev(const btes_Iovec::Ovec *buffers, ...);
// [24] int bufferedWritev(const btes_Iovec::Iovec *buffers, ...);
// [25] int timedBufferedWritev(const btes_Iovec::Ovec *buffers, ...);
// [25] int timedBufferedWritev(const btes_Iovec::Iovec *buffers, ...);
// [ 4] void cancelAll();
// [ 5] void cancelRead();
// [ 5] void cancelWrite();
...
// [ 4] bteso_StreamSocket<bteso_IPv4Address> *socket() const;
//-----------------------------------------------------------------------------
// [26] USAGE example
// [ 1] Breathing test
//=============================================================================
...
//-----------------------------------------------------------------------------
typedef btesos_TcpTimedCbChannel Obj;
...
struct Buffer { // for mixing I/O request, e.g. bufferRead and readvRaw request
    ...
    btes_Iovec *d_readIovecBuf;  // to read into this vector
    btes_Iovec *d_writeIovecBuf; // to be written to the channel.
    btes_Ovec  *d_writeOvecBuf;  // to be written to the channel.
};

btes_Iovec ioVec[MAX_VEC]; // global object to be used in the gg() for vector
                           // read or write
...
static int
gg(btesos_TcpTimedCbChannel *channel, Buffer *buffer,
   bteso_TcpTimerEventManager *rManager,
...
static int helpWrite(bteso_SocketHandle::Handle client, const char *buf,
                     int bufLen)
    ...
    // tested.  return 0 on success, non-zero otherwise.
{
  if (0 == buf || 0 == bufLen)
      ...
      int len = bteso_SocketImpUtil::write(client, buf, bufLen);
      if (veryVerbose) {
                              ...
                              const char *expData)
{
    ...
    if (vecBuffer)
    {
        switch (type) {
          case IOVECTOR: {
              btes_Iovec *vec = (btes_Iovec*) vecBuffer;
              int idx = 0;
...
static void bufferedReadCallback(const char                 *buf,
                                 int                         status,
                                 int                         augStatus,
                                 bteso_TcpTimerEventManager *eveManager,
...
//   wbv   - buffered writev,
//   wbvi  - buffered writev by using btes_Iovec buffer,
//   wbvo  - buffered writev by using btes_Ovec buffer,
//   wbvt  - timed buffered writev,
//   wbvit - timed buffered writev by using btes_Iovec buffer,
//   wbvot - timed buffered writev by using btes_Ovec buffer,
//   wr    - raw write,
//   wrt   - timed raw write,
//   wv    - writev,
//   wvi   - writev by using btes_Iovec buffer,
//   wvo   - writev by using btes_Ovec buffer,
//   wvit  - timed writev by using btes_Iovec buffer,
//   wvot  - timed writev by using btes_Ovec buffer,
//   wvir  - raw writev by using btes_Iovec buffer,
//   wvor  - raw writev by using btes_Ovec buffer,
//   wvirt - timed raw writev by using btes_Iovec buffer,
//   wvort - timed raw writev by using btes_Ovec buffer,
...
//     bdetu_SystemTime::now(), where the first one in the pair means the
...
static int
gg(btesos_TcpTimedCbChannel   *channel,
   Buffer                     *buffer,
   bteso_TcpTimerEventManager *rManager,
   bteso_TcpTimerEventManager *wManager,
   const char                 *script)
{
    ...
    switch (*script) {
      ...
      case 's': {
          ...
          ASSERT(1 == ret);    // Return the length to be written numbers.
          #ifdef BSLS_PLATFORM__OS_UNIX // unix systems
      ...
      case 'r': {
          ...
          switch (*(script+1)) {
            case 'b': {
                btesc_CbChannel::BufferedReadCallback callback;
                if ('r' == *(script+2)) {
                ...
                if ('r' == *(script+2)) {
                       if ('t' == *(script+3)) { // timedReadvRaw()
                           ret = sscanf(script, "rvrt%d,(%d,%d),%d,%d,%d",
                                     ...
                                     &expStatus, &expAugStatus);
                           ASSERT(6 == ret);     // Return 6 numbers.
                           btes_Iovec *buf = buffer->d_readIovecBuf;
      ...
      case 'w': {
          int writeLen = 0, optFlag = 0, milliSec = 0, nanoSec = 0,
              expStatus, expAugStatus;
          btesc_CbChannel::WriteCallback callback;
          switch (*(script+1)) {
            case 'r':
...
//-----------------------------------------------------------------------------
static inline
void adjustBufferSizes(bteso_SocketHandle::Handle sockFd, int bufferSize,
                       int watermark)
...
{
    int ret;
    ret = bteso_SocketOptUtil::setOption(sockFd,
...
static inline
void printBufferSizes(bteso_SocketHandle::Handle sockFd)
    // Print the buffer sizes and the watermarks of the specified 'sockFd'
    // socket handle.
{
    int sendSize = -1, recvSize = -1, sendWat = -1, recvWat = -1;
    int ret = -1;
    ret = bteso_SocketOptUtil::getOption(&recvSize, sockFd,
            bteso_SocketOptUtil::BTESO_SOCKETLEVEL,
            bteso_SocketOptUtil::BTESO_RECEIVEBUFFER);
    ASSERT(0 == ret);

    ret = bteso_SocketOptUtil::getOption(&sendSize, sockFd,
            bteso_SocketOptUtil::BTESO_SOCKETLEVEL,
            bteso_SocketOptUtil::BTESO_SENDBUFFER);
    ASSERT(0 == ret);

    ret = bteso_SocketOptUtil::getOption(&recvWat, sockFd,
            bteso_SocketOptUtil::BTESO_SOCKETLEVEL,
            SO_RCVLOWAT);
    ...
    ret = bteso_SocketOptUtil::getOption(&sendWat, sockFd,
            bteso_SocketOptUtil::BTESO_SOCKETLEVEL,
            SO_SNDLOWAT);
...
// #if !defined(BSLS_PLATFORM__OS_AIX) && !defined(BSLS_PLATFORM__OS_SOLARIS)
        ...
        // the channel.  The 'bteso_StreamSocket' object has a field of type
        // 'bteso_SocketHandle::Handle', whose value is set to the
        ...
        // the convenience of data reading in a test.
         bteso_StreamSocket<bteso_IPv4Address> *cSocket =
        ...
        bteso_TcpTimerEventManager rEventManager(
                                     ...
                                     &testAllocator);
        {
            ...
            // created 'bteso_StreamSocket' object, the
            // 'bteso_TcpTimerEventManager' object.

            btesos_TcpTimedCbChannel channel(sSocket, &rEventManager,
            ...
            bdef_Function<void (*)(int, int)> callback1(
                    bdef_BindUtil::bind(&myReadCallback, readLen1, 0));
            // We also want to set a timeout value which is relative to the
            // current system time.
            bdet_TimeInterval timer1 = bdetu_SystemTime::now();
            ...
            int optFlag2 = 0;
            char readBuf2[20] = "\0";
            bdef_Function<void (*)(int, int)> callback2(
                    bdef_BindUtil::bind(&myReadCallback, readLen2, 0));
            // We also want to set a timeout value which is relative to the
            // current system time.
            bdet_TimeInterval timer2 = bdetu_SystemTime::now();
            ...
            ret = ((bteso_TcpTimerEventManager*)
        ...
        // Any 'bteso_StreamSocket' objects should be deleted specifically.
...
// TBD FIX ME
        ...
        // --------------------------------------------------------------------
        // TESTING 'timedBufferedWritev(btes_Iovec)' FUNCTION:
        ...
        //
        // Testing:
        //   int timedBufferedWritev(btes_Iovec);
        // --------------------------------------------------------------------
        if (verbose) cout << "\nTesting 'timedBufferedWritev(btes_Iovec)'"
                          << "\n========================================="
                          << endl;
...
// alignment should be as the following statements:
        ...
        for (int i = 0; i < NUM_SCRIPTS; ++i) {
...

            {
                ...
                btesos_TcpTimedCbChannel channel(sSocket, &eveManager,
                                                 &testAllocator);
...
// TBD FIX ME
        ...
        // --------------------------------------------------------------------
        // TESTING 'bufferedWritev(btes_Iovec)' FUNCTION:
        //   Initiate a non-blocking operation to *atomically* write *up *to*
        //   the specified 'numBuffers' into this channel.
        //   The main concerns about the function are (1) if a write
        //   request can be finished right away when the channel is
        ...
        //
        // Testing:
        //   int bufferedWritev(btes_Iovec);
        // --------------------------------------------------------------------
        if (verbose) cout << "\nTesting 'bufferedWritev(btes_Iovec)'"
                          << "\n====================================" << endl;
...
// TBD FIX ME
#if !defined(BSLS_PLATFORM__OS_SOLARIS) && !defined(BSLS_PLATFORM__OS_LINUX)
        // --------------------------------------------------------------------
        // TESTING 'timedWritev(btes_Iovec)' FUNCTION:
        //   Initiate a non-blocking operation to *atomically* write *up *to*
        ...
        //
        // Testing:
        //   int timedWritev(btes_Iovec);
        // --------------------------------------------------------------------
        if (verbose) cout << "\nTesting 'timedWritev(btes_Iovec)'"
                          << "\n=================================" << endl;
...
// alignment should be as the following statements:
        ...
        for (int i = 0; i < NUM_SCRIPTS; ++i) {
            ...
            ret = bteso_SocketOptUtil::setOption(handles[0],
                      bteso_SocketOptUtil::BTESO_SOCKETLEVEL,
                      bteso_SocketOptUtil::BTESO_SENDLOWATER, WRITELOWWATER);
            //LOOP_ASSERT(i, 0 == ret);

            ret = bteso_SocketOptUtil::setOption(handles[0],
                      bteso_SocketOptUtil::BTESO_SOCKETLEVEL,
                      bteso_SocketOptUtil::BTESO_RECEIVELOWATER, READLOWWATER);
            // LOOP_ASSERT(i, 0 == ret);
...
// TBD FIX ME
#if !defined(BSLS_PLATFORM__OS_SOLARIS) && !defined(BSLS_PLATFORM__CPU_X86_64)
        // --------------------------------------------------------------------
        // TESTING 'writev(btes_Iovec)' FUNCTION:
        //   Initiate a non-blocking operation to *atomically* write *up *to*
        //   the specified 'numBuffer' into this channel.
        //   The main concerns about the function are (1) if a write
        //   request can be finished right away when the channel is
        ...
        //
        // Testing:
        //   int writev(btes_Iovec);
        // --------------------------------------------------------------------
        if (verbose) cout << "\nTesting 'writev(btes_Iovec)'"
                          << "\n============================" << endl;
...
// TBD FIX ME
#if !defined(BSLS_PLATFORM__OS_AIX) && !defined(BSLS_PLATFORM__OS_SOLARIS) \
                                    && !defined(BSLS_PLATFORM__CPU_X86_64)
        // --------------------------------------------------------------------
...
// alignment should be as the following statements:
        ...
        for (int i = 0; i < NUM_SCRIPTS; ++i) {
            ...
            bteso_TcpTimerEventManager eveManager(&testAllocator);
...
// TBD FIX ME
        ...
        //
        // Testing:
        //   btesos_TcpTimedCbChannel(..., rManager, wManager, ...);
        //   void cancelAll();
        //   void invalidate();
        ...
        {
            bteso_SocketHandle::Handle sock0[2], sock1[2], sock2[2], sock3[2];
            bteso_TcpTimerEventManager
                           rManager0(bteso_TcpTimerEventManager::BTESO_NO_HINT,
                                     &testAllocator),
                           rManager1(bteso_TcpTimerEventManager::BTESO_NO_HINT,
                                     &testAllocator),
                           rManager2(bteso_TcpTimerEventManager::BTESO_NO_HINT,
                                     &testAllocator),
                           rManager3(bteso_TcpTimerEventManager::BTESO_NO_HINT,
                                     &testAllocator),
                           wManager0(bteso_TcpTimerEventManager::BTESO_NO_HINT,
                                     &testAllocator),
                           wManager1(bteso_TcpTimerEventManager::BTESO_NO_HINT,
                                     &testAllocator),
                           wManager2(bteso_TcpTimerEventManager::BTESO_NO_HINT,
                                     &testAllocator),
                           wManager3(bteso_TcpTimerEventManager::BTESO_NO_HINT,
                                     &testAllocator);

            struct {
                int                               d_line;
                bteso_SocketHandle::Handle       *d_handle;
                bteso_TcpTimerEventManager       *d_rManager;
                bteso_TcpTimerEventManager       *d_wManager;
                const bteso_TcpTimerEventManager *d_exprManager;
                const bteso_TcpTimerEventManager *d_expwManager;
            } OBJECTS[] =
...
// The normal alignment should be as the following statement:
            ...
            for (int i = 0; i < NUM_OBJS; ++i) {
                ...
                ret = bteso_SocketOptUtil::setOption(OBJECTS[i].d_handle[0],
                      bteso_SocketOptUtil::BTESO_SOCKETLEVEL,
                      bteso_SocketOptUtil::BTESO_SENDBUFFER, BUF_LIMIT);
                LOOP_ASSERT(i, 0 == ret);

                ret = bteso_SocketOptUtil::setOption(OBJECTS[i].d_handle[1],
                      bteso_SocketOptUtil::BTESO_TCPLEVEL,
                      bteso_SocketOptUtil::BTESO_TCPNODELAY, 1);
                LOOP_ASSERT(i, 0 == ret);
...

                {
                    ...
                    btesos_TcpTimedCbChannel channel(sSocket,
                          ...
                          << "\n===============================" << endl;
        {
            ...
            struct {
                ...
                bteso_TcpTimerEventManager *d_rManager;
                bteso_TcpTimerEventManager *d_wManager;
                bteso_TcpTimerEventManager *d_exprManager;
                bteso_TcpTimerEventManager *d_expwManager;
            } OBJECTS[] =
...
// the number of channel objects to be created for test
            ...
            ret = bteso_SocketOptUtil::setOption(socket[0],
                      bteso_SocketOptUtil::BTESO_SOCKETLEVEL,
                      bteso_SocketOptUtil::BTESO_SENDBUFFER, BUF_LIMIT);
            ASSERT(0 == ret);
            ...
            ret = bteso_SocketOptUtil::setOption(socket[1],
                      bteso_SocketOptUtil::BTESO_SOCKETLEVEL,
                      bteso_SocketOptUtil::BTESO_SENDBUFFER, BUF_LIMIT);
            ASSERT(0 == ret);
...
// TBD FIX ME
        ...
        bteso_SocketHandle::Handle sock0[2], sock2[2];
        bteso_TcpTimerEventManager rManager0(
                                     bteso_TcpTimerEventManager::BTESO_NO_HINT,
                                     &testAllocator),
                                   rManager1(
                                     bteso_TcpTimerEventManager::BTESO_NO_HINT,
...
// #if !defined(BSLS_PLATFORM__OS_AIX) && !defined(BSLS_PLATFORM__OS_SOLARIS)
        ...
        //
        // Testing:
        //   btesos_TcpTimedCbChannel(eventManager);
...
// #if !defined(BSLS_PLATFORM__OS_AIX) && !defined(BSLS_PLATFORM__OS_SOLARIS)
        ...
        bteso_SocketHandle::Handle sock0[2];
        bteso_TcpTimerEventManager rManager0(&testAllocator),
 ...
 // The normal alignment should be as the following statement:
        ...
        for (int i = 0; i < NUM_CHANNELS; ++i) {
            ...
            ret = bteso_SocketOptUtil::setOption(CHANNELS[i].d_handle[0],
                      bteso_SocketOptUtil::BTESO_SOCKETLEVEL,
                      bteso_SocketOptUtil::BTESO_SENDBUFFER, BUF_LIMIT);

            ret = bteso_SocketOptUtil::setOption(CHANNELS[i].d_handle[1],
                      bteso_SocketOptUtil::BTESO_TCPLEVEL,
                      bteso_SocketOptUtil::BTESO_TCPNODELAY, 1);
            LOOP_ASSERT(i, 0 == ret);
