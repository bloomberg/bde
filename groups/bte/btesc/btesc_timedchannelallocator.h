// btesc_timedchannelallocator.h       -*-C++-*-
#ifndef INCLUDED_BTESC_TIMEDCHANNELALLOCATOR
#define INCLUDED_BTESC_TIMEDCHANNELALLOCATOR
...
//
//@CLASSES:
// btesc_TimedChannelAllocator: blocking stream channel allocator with timeout
//
//@SEE_ALSO: btesc_timedcbchannelallocator btemc_timedchannelallocator
//
...
// capability; the allocation itself also has timeout capability.  Each channel
// is an end point of a bi-directional stream-based communication connection to
// a peer; connection details, such as who the peer is, whether there is indeed
// one peer or multiple similar peers, and how the connection came to be, are
// not relevant to this channel-allocator protocol, and are therefore
// abstracted.
//
///Protocol Hierarchy
///------------------
// The interface hierarchy (defined by direct inheritance) of the
// 'btesc_TimedChannelAllocator' protocol is as follows:
//..
//                  ,---------------------------.
...
// operation was interrupted (e.g., via a timeout), the channel address is
// null, and 'status' is set to a non-negative value (see below), or (3)
// "error" -- an implementation-dependent error occurred, the channel address
// is null, and 'status' is less than -1.  The user may retry interrupted
// operations with a reasonable expectation of success.  An "error" status
// implies that the allocation is unlikely to succeed if retried, but does not
// *necessarily* invalidate the allocator.  The 'isInvalid' method may be used
// to confirm the occurrence of a permanent error.  An allocation can *never*
// succeed once an allocator is invalid.
//
...
//
// The concrete allocator will make a "best effort" to honor the timeout
...
// and Client sides of a 'my_Tick' reporting service.  Since each side of this
// service could potentially be a library component, we do not want to "bake"
// into either side the details of how connections will be established.  It is
// sufficient that, when a tick needs to be sent or received, a channel is
// obtained, the tick is transmitted, and the channel is returned to its
// allocator.  Note that this example serves to illustrate the use of the
// 'btesc_TimedChannelAllocator' and does not represent production-quality
// software.
//
///Server Side
///-----------
...
// server using just the 'btesc_TimedChannelAllocator' and 'btesc_TimedChannel'
// protocols.
//..
...
//      };
//      btesc_TimedChannelAllocator *d_acceptor_p; // incoming connections
...
//                      btesc_TimedChannelAllocator *acceptor)
...
//      int reportTicker();
//          // Call 'd_acceptor_p's 'timedAllocatedTimed' to establish a
//          // channel.
//  };
//
...
//                 d_acceptor->timedAllocatorTimed(&status, timeout);
...
//
//              char *buffer = 0;
//              int ret = newChannel->timedBufferedRead(&argStatus, &buffer,
//                                                      now + READ_TIME_LIMIT);
//              if (0 >= ret) {
...
//      btesos_TimedCbChannelAcceptor acceptor(address, &sf);
//
...
// demand.  We will use the 'btesc_TimedChannelAllocator' protocol to abstract
...
//      // single 'btesc_TimedChannel' and send them synchronously one by
//      // one to a peer (or similar peers) connected via channels provided via
//      // a 'btesc_TimedChannelAllocator'.  Both the output channel
...
//
//      btesc_TimedChannelAllocator *d_connector_p;  // outgoing connections
//      btesc_TimedChannel          *d_input_p;      // incoming packets
...
//                              btesc_TimedChannelAllocator *connector
//                              btesc_TimedChannel          *input,
//                              int                          inputSize);
...
//
//              btesc_Channel
//                    *newChannel = d_connector_p->timedAllocate(functor,
...
//      btesos_TimedCbChannelConnector connector(address, &sf);
//
...
// the use of 'btesc_TimedChannelAllocator' and do not represent
...
class btesc_TimedChannel;
                        ...
                        // =================================
                        // class btesc_TimedChannelAllocator
                        // =================================

class btesc_TimedChannelAllocator // : public btesc_ChannelAllocator
{
  ...
  public:
    ...
    btesc_Channel *timedAllocate(int                       *status,
        ...
        // operation, with a positive value if the interruption was due to an
        // asynchronous event and with a value less than -1 (indicating an
        ...
        // still be attempted, but the attempt will not block.  Note also that
        // a status value of -1 is not possible for this method.
    ...
    virtual btesc_TimedChannel
        ...
        // interrupted this operation, with a positive value if the
        // interruption was due to an asynchronous event, and with a value less
        // than -1 (indicating an error) otherwise; 'status' is not modified on
        // success.  A non-null channel address will remain valid until
        // deallocated explicitly (see 'deallocate').  An allocation that fails
        ...
        // Note also that a status value of -1 is not possible for this method.
