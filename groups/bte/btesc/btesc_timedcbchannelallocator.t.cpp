// btesc_timedcbchannelallocator.t.cpp        -*-C++-*-
...
//-----------------------------------------------------------------------------
// [ 1] ~btesc_TimedCbChannelAllocator();
...
//-----------------------------------------------------------------------------
class MyAllocator : public btesc_TimedCbChannelAllocator {
  ...
  public:
    ...
    int timedAllocate(const Callback&          callback,
                      const bdet_TimeInterval& timeout,
                      int                      flags = 0)
...
int main(int argc, char *argv[]) {
    ...
    switch (test) { case 0:
      ...
      case 1: {
        ...
        //   int allocateTimed(const TimedCallback& timedCallback,
        //                     int                  flags = 0);
        //   timedAllocateTimed(const TimedCallback&     timedCallback,
        //                      const bdet_TimeInterval& timeout,
        //                      int                      flags = 0);
        ...
        MyAllocator myA;
        btesc_TimedCbChannelAllocator& a = myA;
        ...
        if (verbose) cout << "\nTesting protocol interface" << endl;
        {
            ...
            btesc_TimedCbChannelAllocator::Callback myCb(&myCbFn);

            btesc_TimedCbChannelAllocator::TimedCallback myTimedCb(
                                                                 &myTimedCbFn);
            ...
            a.timedAllocate(myCb, myTimeout);
            ASSERT(2 == myA.fun()); ASSERT(0 == myA.flags());
            a.timedAllocate(myCb, myTimeout, myFlags);
            ASSERT(2 == myA.fun()); ASSERT(myFlags == myA.flags());

            a.allocateTimed(myTimedCb);
            ASSERT(3 == myA.fun()); ASSERT(0 == myA.flags());
            a.allocateTimed(myTimedCb, myFlags);
            ASSERT(3 == myA.fun()); ASSERT(myFlags == myA.flags());

            a.timedAllocateTimed(myTimedCb, myTimeout);
            ASSERT(4 == myA.fun()); ASSERT(0 == myA.flags());
            a.timedAllocateTimed(myTimedCb, myTimeout, myFlags);
            ASSERT(4 == myA.fun()); ASSERT(myFlags == myA.flags());
            ...
            a.invalidate();
            ASSERT(6 == myA.fun()); ASSERT(0 == myA.valid());
            ASSERT(1 == myA.isInvalid());
        }
