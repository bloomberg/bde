// btesc_timedcbchannel.t.cpp        -*-C++-*-

#include <btesc_timedcbchannel.h>
...
//-----------------------------------------------------------------------------
// [ 1] virtual ~btesc_TimedCbChannel()
// [ 1] virtual int read(...)
...
// [ 1] virtual int timedBufferedWrite(const btes_Iovec *buffers, ...)
// [ 1] virtual int bufferedWritev(const btes_Ovec *buffers, ...)
// [ 1] virtual int bufferedWritev(const btes_Iovec *buffers, ...)
// [ 1] virtual int timedBufferedWritev(const btes_Ovec *buffers, ...)
// [ 1] virtual int timedBufferedWritev(const btes_Iovec *buffers, ...)
...
//-----------------------------------------------------------------------------
class MyChannel : public btesc_TimedCbChannel {
  // Test class used to verify protocol.
    ...
    int d_fun;  // holds code describing (non-const) function:
                ...
                //   +17 timedWritevRaw(const btes_Ovec *, ...)
                //   +18 timedWritevRaw(const btes_Iovec *, ...)
                //   +19 bufferedWrite
                //   +20 timedBufferedWrite
                //   +21 bufferedWritev(const btes_Ovec *, ...)
                //   +22 bufferedWritev(const btes_Iovec *, ...)
                //   +23 timedBufferedWritev(const btes_Ovec *, ...)
                //   +24 timedBufferedWritev(const btes_Iovec *, ...)
                //   +25 cancelAll
                //   +26 cancelRead
                //   +27 cancelWrite
                //   +28 invalidate
...
int main(int argc, char *argv[]) {
    ...
    switch (test) { case 0:
      ...
      case 1: {
        ...
        //   'btesc_TimedCbChannel' class compiles and links when all
        ...
        //   virtual int timedBufferedWrite(const btes_Iovec *buffers, ...)
        //   virtual int bufferedWritev(const btes_Ovec *buffers, ...)
        //   virtual int bufferedWritev(const btes_Iovec *buffers, ...)
        //   virtual int timedBufferedWritev(const btes_Ovec *buffers, ...)
        //   virtual int timedBufferedWritev(const btes_Iovec *buffers, ...)
        ...
        if (verbose) cout << "\nTesting protocol interface" << endl;
        {
            ...
            btesc_TimedCbChannel::ReadCallback myRdCb(&myRdCbFn);

            btesc_TimedCbChannel::BufferedReadCallback myBufCb(&myBufRdCbFn);

            btesc_TimedCbChannel::WriteCallback myWrCb(&myWrCbFn);
            ...
            c.timedWritevRaw(static_cast<const btes_Ovec *>(0), 0, myTimeout,
                             myWrCb);
            ...
            c.timedWritevRaw(static_cast<const btes_Iovec *>(0), 0, myTimeout,
                             myWrCb);
            ...
            c.timedBufferedWritev(static_cast<const btes_Ovec *>(0), 0,
                                  myTimeout, myWrCb);
            ...
            c.timedBufferedWritev(static_cast<const btes_Iovec *>(0), 0,
                                  myTimeout, myWrCb);
