// btesc_timedcbchannel.h       -*-C++-*-
#ifndef INCLUDED_BTESC_TIMEDCBCHANNEL
#define INCLUDED_BTESC_TIMEDCBCHANNEL
...
//
//@CLASSES:
// btesc_TimedCbChannel: non-blocking stream-based channel protocol w/ timeout
//
//@SEE_ALSO: btesc_timedchannel btemc_timedcbchannel btemc_timedchannel
//
...
// 'btesc_TimedCbChannel' protocol is as follows:
//..
//                        ,--------------------.
...
//                           `---------------'
//..
// 'btesc_TimedCbChannel' adds a "timeout" capability for read and write
// methods.
//
...
// auxiliary data structures defined in the 'btes_iovec' component (see
// 'btes_iovec.h').
//
// The 'btes_Iovec' structure will be familiar to most Unix systems
...
//     }
//
//     void myWritevAndPrintStatusWhenAvailable(btesc_TimedCbChannel *channel)
...
// available to read and write data from and to a 'btesc_TimedCbChannel'; note
...
//
//              // Create a functor.
//              btesc_TimedCbChannel::BufferedReadCallback functor(
//                      bdef_BindUtil::bind(
//                                  &myPrintTick,
//                                  _1, _2, _3
...
//  }
//
//  void myPrintTickWhenAvailable(btesc_TimedCbChannel *channel)
...
//      const bdet_TimeInterval timeout = bdeu_SystemTime::now() + MAX_TIME;
//
//      // Create a functor.
//      btesc_TimedCbChannel::BufferedReadCallback functor;
//                  bdef_BindUtil::bind(
...
class btesc_TimedCbChannel : public btesc_CbChannel {
  ...
  private:
    btesc_TimedCbChannel& operator=(const btesc_TimedCbChannel&); // not impl.

  public:
    // CREATORS
    virtual ~btesc_TimedCbChannel();
        // Destroy this channel.

    // TYPES
    typedef bdef_Function<void (*)(int, int)> ReadCallback;
    ...
    typedef bdef_Function<void (*)(const char *, int, int)>
    ...
    typedef bdef_Function<void (*)(int, int)> WriteCallback;
    ...
    virtual int timedBufferedWritev(const btes_Iovec         *buffers,
                                    int                       numBuffers,
                                    const bdet_TimeInterval&  timeout,
                                    const WriteCallback&      writeCallback,
                                    int                       flags = 0)   = 0;
    virtual int timedBufferedWritev(const btes_Ovec          *buffers,
                                    int                       numBuffers,
                                    const bdet_TimeInterval&  timeout,
                                    const WriteCallback&      writeCallback,
                                    int                       flags = 0)   = 0;
