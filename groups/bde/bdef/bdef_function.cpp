...
BDES_IDENT_RCSID(bdef_function_cpp,"$Id$ $CSID$")
//
...
// them below.  In addition, the 'bdef_Function_Rep' constructors initialize
// their manager *after* the invocable creation (in-place or out-of-place)
// because when an exception is thrown by the 'bdef_Function_Rep' ctor in the
...
//
///SHORT SUMMARY OF TECHNIQUES USED IN 'bdef_function'
///---------------------------------------------------
...
// out-of-place.  Here it is used in the 'bdef_Function_Rep' class, where the
...
// template parameter of the target type.  Thus a 'bdef_Function' object can
// store many invocables of a parameterized 'FUNC' type, but 'FUNC' is not part
// of the 'bdef_Function' or 'bdef_Function_Rep' types.  The downside of this
// erasure of the 'FUNC' type is that it is impossible to retrieve the
// invocable from a 'bdef_Function' object unless one knows the 'FUNC' type a
...
//
// This technique is used twice in 'bdef_function': once for the representation
// and once for the invoker.  A notable feature of the 'bdef_Function_Rep'
...
// the 'FUNC' type.  In this way, 'bdef_Function_Rep' instances can manipulate
...
// the 'bdef_Function_Rep' constructors triggers the instantiation of these
// manager functions.
//
// Likewise, the invoker function pointer in the 'bdef_Function' class points
...
// 'bdef_Function_Rep' representation (of the 'FUNC' type) can be retrieved and
// invoked safely.  This mechanism relies on the fact that the function object
// representation 'd_rep' of the 'bdef_Function_Rep' type was initialized with
...
///Const-correctness:
///------------------
// 'const'-correctness is broken for 'bdef_function', as it is also in the
...
// 'bdef_Function_Rep' or a 'const_cast' in the 'invocable' member function for
...
//..
//
///Value-semantic for 'bdef_Function':
///-----------------------------------
// Unfortunately, comparison between 'bdef_Function' objects cannot be
// implemented "well", and therefore in not implemented.  The typical
// semantics requested for comparing two 'bdef_Function' objects 'f' and 'g'
// are:
//
...
// containers.  However, this is not implementable for 'bdef_Function' because
...
// into a 'bdef_Function' object: you will get an error at the assignment
...
///------------------------
// When assigning a 'FUNC' instance to a 'bdef_Function' object, if the
