...
//
//@CLASSES:
//   bdef_Vfunc4: signature-specific function object (functor)
//
...
//
// The 'bdef_Vfunc4' functor (envelope) holds a polymorphic, reference-counted,
...
// signature and (2) the supplied user data.  See the 'bdefu_vfunc4' component
// for more information on populating 'bdef_Vfunc4' function objects with
...
class bdef_Vfunc4 {
    ...
    // NOT IMPLEMENTED
    bool operator==(const bdef_Vfunc4&) const;
    bool operator!=(const bdef_Vfunc4&) const;

    template<class B1, class B2, class B3, class B4>
    bool operator==(const bdef_Vfunc4<B1, B2, B3, B4>&) const;
    template<class B1, class B2, class B3, class B4>
    bool operator!=(const bdef_Vfunc4<B1, B2, B3, B4>&) const;
  ...
  public:
    ...
    bdef_Vfunc4(bdefr_Vfunc4<A1, A2, A3, A4> *representation);
        ...
        // components (see 'bdefu_vfunc4') to create an appropriate
        // representation and load it into an existing functor.

    bdef_Vfunc4(const bdef_Vfunc4<A1, A2, A3, A4>& functionObject);
    ...
    ~bdef_Vfunc4();
        ...
        // the static 'deleteObject' method of 'bdefr_Vfunc4' base class).

    // MANIPULATORS
    bdef_Vfunc4<A1, A2, A3, A4>& operator=(
        const bdef_Vfunc4<A1, A2, A3, A4>& functionObject);
        ...
        // 'deleteObject' method of 'bdefr_Vfunc4' base class.
...
template <class A1, class A2, class A3, class A4>
inline bdef_Vfunc4<A1, A2, A3, A4>::bdef_Vfunc4()
: d_rep_p(0)
{
}
...
template <class A1, class A2, class A3, class A4>
inline
bdef_Vfunc4<A1, A2, A3, A4>::bdef_Vfunc4(
    bdefr_Vfunc4<A1, A2, A3, A4> *rep)
: d_rep_p(rep)
{
    if (d_rep_p) {
        d_rep_p->increment();
...
template <class A1, class A2, class A3, class A4>
inline bdef_Vfunc4<A1, A2, A3, A4>::bdef_Vfunc4(const bdef_Vfunc4& functor)
...
inline bdef_Vfunc4<A1, A2, A3, A4>::~bdef_Vfunc4()
{
    if (d_rep_p && 0 == d_rep_p->decrement()) {
        bdefr_Vfunc4<A1, A2, A3, A4>::deleteObject(d_rep_p);
    }
}

template <class A1, class A2, class A3, class A4> inline
bdef_Vfunc4<A1, A2, A3, A4>&
bdef_Vfunc4<A1, A2, A3, A4>::operator=(const bdef_Vfunc4<A1, A2, A3, A4>& rhs)
{
...
template <class A1, class A2, class A3, class A4>
inline void bdef_Vfunc4<A1, A2, A3, A4>::operator()(
                                                const A1& argument1,
                                                const A2& argument2,
                                                const A3& argument3,
...
template <class A1, class A2, class A3, class A4>
inline bdef_Vfunc4<A1, A2, A3, A4>::operator const void *() const
{
    return d_rep_p;
}
