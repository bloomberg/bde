...
//-----------------------------------------------------------------------------
// [ 2] bdec2_Map(bslma_Allocator *ba = 0);
// [  ] bdec2_Map(Hint, bslma_Allocator *ba = 0);
// [13] bdec2_Map(const InitialCapacity& ne, *ba = 0);
// [  ] bdec2_Map(const InitialCapacity& ne, Hint, *ba = 0);
// [ 7] bdec2_Map(const bdec2_Map& original, *ba = 0);
// [  ] bdec2_Map(const bdec2_Map& original, Hint, *ba = 0);
// [16] bdec2_Map(keys, values, int ne, *ba = 0);
// [  ] bdec2_Map(keys, values, int ne, Hint, *ba = 0);
// [ 2] ~bdec2_Map();
// [ 9] bdec2_Map& operator=(const bdec2_Map& rhs);
// [12] bsl::string *add(int key, const char *value);
// [ 4] char *lookup(int key);
// [12] int remove(int key);
// [ 2] void removeAll();
// [13] void reserveCapacity(int ne);
// [12] bsl::string *set(int key, const char *value);
// [10] bdex_InStream& streamIn(bdex_InStream& stream);
// [ 4] int isMember() const;
// [ 4] int length() const;
// [ 4] const char *lookup(int key) const;
...
//
// [ 6] operator==(const bdec2_Map&, const bdec2_Map&);
// [ 6] operator!=(const bdec2_Map&, const bdec2_Map&);
// [ 5] operator<<(ostream&, const bdec2_Map&);
// [10] operator>>(bdex_InStream&, bdec2_Map&);
// [10] operator<<(bdex_OutStream&, const bdec2_Map&);
//-----------------------------------------------------------------------------
// [14] bdec2_MapIter(const bdec2_Map&);
// [14] ~bdec2_MapIter();
// [14] operator++();
// [14] operator const void *() const;
// [14] Int operator()() const;
//-----------------------------------------------------------------------------
// [15] bdec2_MapManip(bdec2_Map *);
// [15] ~bdec2_MapManip();
// [15] advance();
// [15] remove();
...
// [ 3] int ggg(bdec2_Map *object, const char *spec, int vF = 1);
// [ 3] bdec2_Map& gg(bdec2_Map* object, const char *spec);
// [ 8] bdec2_Map   g(const char *spec);
...
typedef bdec2_Map<int, bsl::string, bdeimp_IntHash> Obj;
typedef bdec2_MapIter<int, bsl::string, bdeimp_IntHash> ObjIter;
typedef bdec2_MapManip<int, bsl::string, bdeimp_IntHash> ObjManip;
...
const bsl::string  S0 = bsl::string(V0), &SA = S0,
                   S1 = bsl::string(V1), &SB = S1,
                   S2 = bsl::string(V2), &SC = S2,
                   S3 = bsl::string(V3), &SD = S3,
                   S4 = bsl::string(V4), &SE = S4,
                   S5 = bsl::string(V5), &SF = S5,
                   S6 = bsl::string(V6), &SG = S6,
                   S7 = bsl::string(V7), &SH = S7,
                   S8 = bsl::string(V8), &SI = S8;
...
// added to the 'bdec2_Map' object.  A tilde ('~') indicates that
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      case 19: {
...

        {
            ...
            for (bdec2_MapIter<int, bsl::string, bdeimp_IntHash> it(s);
                                                                    it; ++it) {
    ...
    case 18: {
        // --------------------------------------------------------------------
        // Testing for undesired use of default allocator
        //
        //   Concerns:
        ...
        //     memory used by the default allocator has changed.
        // --------------------------------------------------------------------
        ...
        for (int i = 0; 6 > i; ++i) {
            mX.add(KEYS[i], bsl::string(VALUES[i]));
        }

        ASSERT(DEFAULT_BYTES == taDefault.numBytesInUse());
      } break;
    case 17: {
        // --------------------------------------------------------------------
        // TBD doc
        ...
        if (verbose) cout << endl
                          << "Testing Basic Accessors" << endl // TBD
                          << "=======================" << endl;

        if (verbose) cout << "\nTesting 'length' & 'isMember'" << endl; // TBD
        {
            const int SZ = 10;
            static const struct {
            ...
            for (int ti = 0; ti < NUM_DATA ; ++ti) {
                ...
                for (i = 0; i < NUM_KEYS; ++i) {
                    ...
                    {
                        bsl::string *xp = mX.lookupRaw(KEYS[i]);
                        bsl::string *yp = mY.lookupRaw(KEYS[i]);
                    ...
                    {
                        const int *xp = X.lookupKeyRaw(KEYS[i]);
                        const int *yp = Y.lookupKeyRaw(KEYS[i]);
                        LOOP2_ASSERT(LINE, i, ev == (0 != xp));
                        LOOP2_ASSERT(LINE, i, ev == (0 != yp));
      ...
      case 16: {
        // TBD doc
        //bdec2_Map<bsl::string> x(KEYS, VALUES, NUM_VALUES);
            ...
            for (int ti = 0; SPECS[ti]; ++ti) {
                ...
                for (int ei = 0; ei < NUM_EXTEND; ++ei) {
                    ...
                    for (ObjIter iter(X); iter; ++iter) {
                        ...
                        {
                            ObjIter mZ(iter);  const ObjIter& Z = mZ;
                            LOOP_ASSERT(ti, iter == Z);
                        }

                        {
                            ObjIter mZ(X);  const ObjIter& Z = mZ;
                            mZ = iter;
                            LOOP_ASSERT(ti, iter == Z);
                        }
                    }
      ...
      case 13: {
        ...
        //
        // Testing:
        //   bdec2_Map(const InitialCapacity& ne, *ba = 0);
        ...
        if (verbose) cout <<
            "\nTesting 'bdec2_Map(capacity, ba)' Constructor" << endl;
      ...
      case 12: {
        ...
        //
        // Testing:
        //   bsl::string *add(int key, const char *value);
        //   int remove(int key);
        //   bsl::string *set(int key, const char *value);
        // --------------------------------------------------------------------
...
#undef NL
      ...
      case 10: {
        ...
        //   operator>>(bdex_InStream&, bdec2_Map&);
        //   operator<<(bdex_OutStream&, const bdec2_Map&);
        // --------------------------------------------------------------------
      ...
      case 9: {
        ...
        //
        // Testing:
        //   bdec2_Map& operator=(const bdec2_Map& rhs);
        // --------------------------------------------------------------------
      ...
      case 7: {
        ...
        //
        // Testing:
        //   bdec2_Map(const bdec2_Map& original, *ba = 0);
        // --------------------------------------------------------------------
    ...
    case 6: { // TBD same key, different value tests
        ...
        //
        // Testing:
        //   operator==(const bdec2_Map&, const bdec2_Map&);
        //   operator!=(const bdec2_Map&, const bdec2_Map&);
        // --------------------------------------------------------------------
      ...
      case 5: {
        ...
        //
        // Testing:
        //   operator<<(ostream&, const bdec2_Map&);
        // --------------------------------------------------------------------
    ...
    case 4: {
        ...
        //   char *lookup(int key);
        //   const char *lookup(int key) const;
      ...
      case 3: {
        ...
        //
        // Testing:
        //   bdec2_Map& gg(bdec2_Map* object, const char *spec);
        //   int ggg(bdec2_Map *object, const char *spec, int vF = 1);
        //   void stretch(Obj *object, int size);
        //   void stretchRemoveAll(Obj *object, int size);
...
#define NL "\n"
      ...
      case 2: {
        ...
        if (verbose) cout << "\nTesting 'add' (bootstrap)." << endl;
        {
            ...
            const int BYTES_BEFORE = taDefault.numBytesInUse();
            mX.add(K0, S0);
            LOOP2_ASSERT(taDefault.numBytesInUse(), BYTES_BEFORE,
                                    taDefault.numBytesInUse() == BYTES_BEFORE);
        ...
        if (verbose) cout << "\nTesting 'removeAll'." << endl;
        {
            ...
            const int DEFAULT_BYTES_BEFORE = taDefault.numBytesInUse();
            const int TEST_BYTES_BEFORE    = testAllocator.numBytesInUse();
            mX.removeAll();
            ASSERT(taDefault.numBytesInUse() == DEFAULT_BYTES_BEFORE);
            ASSERT(testAllocator.numBytesInUse() <= TEST_BYTES_BEFORE);
