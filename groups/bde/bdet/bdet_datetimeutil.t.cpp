...
// appropriate.
// ----------------------------------------------------------------------------
// [ 2] void convertToTm(tm *result, const bdet_Datetime& datetime);
// [ 2] int convertFromTm(bdet_Datetime *result, const tm& timeStruct);
// ----------------------------------------------------------------------------
// [ 1] BREATHING TEST -- (developer's sandbox)
// [ 3] USAGE EXAMPLE -- Daylight-Saving Time
// ----------------------------------------------------------------------------
...
int main(int argc, char *argv[])
{
  ...
  //v-----------^
    ...
    bdet_DatetimeUtil::convertToTm(&result, INPUT);
  ...
  //^-----------v
        ...
        if (!switches[(int)'2']) {
            ...
            static const struct {
                ...
                //lin   year mon day hou min sec  s  year mon hou
                //---   ---- --- --- --- --- ---  -  ---- --- ---
                { L_,  -1900, 11, 31, 24,  0,  0, 1,    0,  0, 0  }, // year
                { L_,  -1900, 11, 31, 23, 59, 59, 1,    0,  0, 0  },
                { L_,  -1900,  0,  1,  0,  0,  0, 1,    0,  0, 0  },
                ...
                { L_,   8100, 11, 31, 24,  0,  0, 1,    0,  0,  0 }, // year
                { L_,   8100, 11, 31, 23, 59, 59, 1,    0,  0,  0 },
                { L_,   8100,  0,  1,  0,  0,  0, 1,    0,  0,  0 },

                    // *** Illegal dates are detected ***
                //lin   year mon day hou min sec  s  year mon hou
                //---   ---- --- --- --- --- ---  -  ---- --- ---
                { L_,    100, -1, 15,  0,  0,  0, 1,    0,  0,  0 }, // month
                { L_,    100,  0, 15,  0,  0,  0, 0, 2000,  1,  0 },
                { L_,    100, 11, 15,  0,  0,  0, 0, 2000, 12,  0 },
                { L_,    100, 12, 15,  0,  0,  0, 1,    0,  0,  0 },

                { L_,    100,  2,  0,  0,  0,  0, 1,    0,  0,  0 }, // day in
                { L_,    100,  2,  1,  0,  0,  0, 0, 2000,  3,  0 }, // March
                { L_,    100,  2, 31,  0,  0,  0, 0, 2000,  3,  0 },
                { L_,    100,  2, 32,  0,  0,  0, 1,    0,  0,  0 },

                { L_,    100,  3, 30,  0,  0,  0, 0, 2000,  4,  0 }, // day in
                { L_,    100,  3, 31,  0,  0,  0, 1,    0,  0,  0 }, // April

                { L_,    103,  1, 28,  0,  0,  0, 0, 2003,  2,  0 }, // Feb.
                { L_,    103,  1, 29,  0,  0,  0, 1,    0,  0,  0 }, // no-leap

                { L_,    104,  1, 29,  0,  0,  0, 0, 2004,  2,  0 }, // Feb.
                { L_,    100,  1, 30,  0,  0,  0, 1,    0,  0,  0 }, // leap-yr

                { L_,      0,  1, 28,  0,  0,  0, 0, 1900,  2,  0 }, // Feb.
                { L_,      0,  1, 29,  0,  0,  0, 1,    0,  0,  0 }, // century

                { L_,    500,  1, 29,  0,  0,  0, 0, 2400,  2,  0 }, // Feb.
                { L_,    500,  1, 30,  0,  0,  0, 1,    0,  0,  0 }, // 400-yr

                { L_,    100,  6, 15, -1, 12, 34, 1,    0,  0,  0 }, // hour
                { L_,    100,  6, 15,  0, 12, 34, 0, 2000,  7,  0 },
                { L_,    100,  6, 15, 23, 12, 34, 0, 2000,  7, 23 },
                { L_,    100,  6, 15, 24,  0, 34, 1,    0,  0,  0 },
                { L_,    100,  6, 15, 24,  0,  0, 0, 2000,  7, 24 },
                { L_,    100,  6, 15, 25,  0,  0, 1,    0,  0,  0 },

                { L_,    100,  6, 15, 12, -1, 34, 1,    0,  0,  0 }, // min
                { L_,    100,  6, 15, 12,  0, 34, 0, 2000,  7, 12 },
                { L_,    100,  6, 15, 12, 59, 34, 0, 2000,  7, 12 },
                { L_,    100,  6, 15, 12, 60, 34, 1,    0,  0,  0 },

                { L_,    100,  6, 15, 12, 34, -1, 1,    0,  0,  0 }, // sec
                { L_,    100,  6, 15, 12, 34,  0, 0, 2000,  7, 12 },
                { L_,    100,  6, 15, 12, 34, 59, 0, 2000,  7, 12 },
                { L_,    100,  6, 15, 12, 34, 60, 1,    0,  0,  0 },

                    // *** Leap seconds at the end of the year fail ***
                //lin   year mon day hou min sec  s  year mon hou
                //---   ---- --- --- --- --- ---  -  ---- --- ---
                { L_,    100, 11, 31, 23, 59, 59, 0, 2000, 12, 23 }, // sec
                { L_,    100, 11, 31, 23, 59, 60, 1,    0,  0,  0 },
                { L_,    100, 11, 31, 23, 59, 61, 1,    0,  0,  0 },
                { L_,    100, 11, 31, 23, 59, 62, 1,    0,  0,  0 },
            };
  ...
  //v-------------------^
    ...
    int status = bdet_DatetimeUtil::convertFromTm(&result, INPUT);
  ...
  //v-----------^
    bdet_DatetimeUtil::convertToTm(&tmp, INITIAL_VALUE);

    bdet_Datetime result(1, 2, 3, 4, 5, 6, 7);

    LOOP_ASSERT(i, 0 == bdet_DatetimeUtil::convertFromTm(&result, tmp));
