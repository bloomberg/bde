...
class EarlyEpochCopier {
  public:
    EarlyEpochCopier() {
        const bdet_Datetime &epoch = bdet_EpochUtil::epoch();
        if (0 != &epoch) {  // THIS IS NOT A TYPO!
            epochAddressIsNotZero = 1;
            new((void *)epochBuffer) bdet_Datetime(bdet_EpochUtil::epoch());
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      case 6: {
...

        {
            ...
            bdet_EpochUtil::convertFromTimeT(&datetime, myStdTime);
...

        {
            ...
            if (verbose)
                bsl::cout << bdet_EpochUtil::convertFromTimeT(myStdTime)
                          << bsl::endl;
        }
...

        {
            ...
            bdet_EpochUtil::convertFromDatetimeInterval(&datetime,
                                                     datetimeInterval);
...

        {
            ...
            // Works when 'datetime > epoch'.
            int status = bdet_EpochUtil::convertToDatetimeInterval(
                                                             &datetimeInterval,
                                                             datetime);
  ...
  //v-----------^
    ...
    int status = Util::convertToDatetimeInterval(&result, INPUT);// TEST
    ...
    if (!STATUS) {
        resultDup = Util::convertToDatetimeInterval(INPUT);      // TEST
    }
    ...
    Util::convertFromDatetimeInterval(&result2, result);         // TEST
    if (veryVerbose) { cout << "After: "; P(result2); }

    if (veryVerbose) { cout << "Before: "; P(result2Dup); }
    result2Dup = Util::convertFromDatetimeInterval(resultDup);   // TEST
  ...
  //^-------v
        ...
        for (unsigned int i = 0; i < numTrials; ++i) {
            ...
            bdet_DatetimeInterval currentValue(0, 0, 0, 0, pseudoRandomValue);
  ...
  //v-------^
    ...
    LOOP_ASSERT(i, 0 == Util::convertToDatetimeInterval(&result,
                                                                  tmp));
  ...
  //^-------v
        ...
        for (unsigned int i = 0; i < numTrials; ++i) {
            ...
            bdet_TimeInterval currentValue(pseudoRandomValue, 0);
            pseudoRandomValue += PRIME;
            pseudoRandomValue %= SIZE;
  ...
  //v-------^
    ...
    if (veryVeryVerbose) { cout<<"Before: "; P(result); }
    LOOP_ASSERT(i, 0 == Util::convertToTimeInterval(&result, tmp));
    if (veryVeryVerbose) { cout<<" After: "; P(result); }
  ...
  //v-------^
    ...
    if (veryVeryVerbose) { cout<<"Before: "; P(result); }
    LOOP_ASSERT(i, 0 == Util::convertToTimeT(&result, tmp));
    if (veryVeryVerbose) { cout<<" After: "; P(result); }
  ...
  //^-----------v
        ...
        static const struct {
            ...
            { L_, -62135769602LL, FAILURE,    0,  0,  0,  0,  0,  0 },
            { L_, -62135769601LL, FAILURE,    0,  0,  0,  0,  0,  0 },
        ...
        for (int ti = 0; ti < NUM_DATA; ++ti) {
            ...
            if (veryVerbose) { cout << "Before: "; P(result) }
            int status = Util::convertFromTimeT64(&result,
                                                                    INPUT);
            if (veryVerbose) { cout << " After: "; P_(result) }
