// bdex_genericbyteoutstream.t.cpp   -*-C++-*-
...
#include <bdesb_fixedmemoutput.h>               // for testing only
...
//                              Overview
//                              --------
// For all output methods in 'bdex_GenericByteOutStream', the formatting of
...
// 'bdex_GenericByteOutStream' to be 'putInt8'.
//
// The class under test is a class template, so we choose to test it with a
// 'bdesb_FixedMemOutput' parameter, but test case [25] verifies that it can be
// instantiated by a class that has the minimal requirements.
//-----------------------------------------------------------------------------
// [ 2] bdex_GenericByteOutStream(bsl::streambuf *);
// [ 2] ~bdex_GenericByteOutStream();
// [24] bdex_GenericByteOutStream& flush();
// [23] void invalidate();
...
typedef bdex_GenericByteOutStream<bdesb_FixedMemOutput> Obj;
...
class my_FixedMemOutStreamBuf {
    // This class implements a very basic 'bdesb_FixedMemOutput' suitable
    // for use in 'bdex_GenericByteOutStream'.

  private:
    char            *d_buffer_p;    // output buffer
    bsl::streamsize  d_length;      // length of output buffer
    bsl::streamsize  d_pos;         // output cursor
    int              d_flushCount;  // number of calls to 'pubsync'

  public:
    // CREATORS
    my_FixedMemOutStreamBuf(char *buffer, bsl::streamsize length);
    ...
    my_FixedMemOutStreamBuf* pubsetbuf(char *buffer, bsl::streamsize length);
    ...
    int sputc(char c);
        // Write the specified character 'c' to this buffer.
...
// MANIPULATORS
char *my_FixedMemOutStreamBuf::data()
{
    return d_buffer_p;
}
...
int my_FixedMemOutStreamBuf::pubsync()
{
...
int my_FixedMemOutStreamBuf::sputc(char c)
{
    BSLS_ASSERT(d_pos < d_length);
    d_buffer_p[d_pos] = c;
    ++d_pos;
    return c;
}

bsl::streamsize my_FixedMemOutStreamBuf::sputn(const char *c,
                                               bsl::streamsize length)
{
    BSLS_ASSERT(d_pos + length <= d_length);
    bsl::memcpy(d_buffer_p + d_pos, c, length);
    d_pos += length;
    return length;
}

// ACCESSORS
const char *my_FixedMemOutStreamBuf::data() const
{
    return d_buffer_p;
}

bsl::streamsize my_FixedMemOutStreamBuf::capacity() const
{
    return d_length;
}
...
bsl::streamsize my_FixedMemOutStreamBuf::length() const
{
    return d_pos;
}

typedef bdex_GenericByteOutStream<my_FixedMemOutStreamBuf> GObj;
...
//  }
      ...
      case 25: {
        // --------------------------------------------------------------------
        // CLASS 'bdex_GenericBdexByteOutStream'
        // Concerns:
        //   That 'bdex_GenericBdexByteOutStream' can be parameterized
        //   by a bare-bones 'bsl::streambuf' implementation that only provides
        //   'putc', 'putn' and 'pubsync'.
        //
        // Plan:
        ...
        if (verbose)
            cout << "\nGENERIC TEST FOR 'bdex_GenericBdexByteOutStream'"
                 << "\n================================================"
                 << endl;
        ...
        {
            if (verbose) cout << "\nCreate object x1 using default ctor.\n";
            char workSpace[100];
      ...
      case 24: {
        ...
        using namespace genericTest;
        typedef bdex_GenericByteOutStream<my_FixedMemOutStreamBuf> Obj;
        ...
        // this operation does not work on MSVC 2005 - the compiler does
        // not allow function template arguments to other function templates.
        // see http://msdn2.microsoft.com/en-us/library/wykfwk36(vs.80).aspx
        x << bdexFlush<Obj>;
        ASSERT(1 == sb.flushCount());
#else
        if (verbose) cout << endl
                          << "FLUSH (SKIPPED on this compiler)" << endl
                          << "================================" << endl;
#endif
      ...
      case 22: {
                          ...
                          << "===========================" << endl;
        {
            ...
            {
                char workSpace[100];
                bdesb_FixedMemOutput sb(workSpace, 100);
                Obj x(&sb);
                                    x.putInt8(0xff);
      ...
      case 2: {
        ...
        //
        // Testing:
        //   bdex_GenericByteOutStream(bsl::streambuf *);
        //   ~bdex_GenericByteOutStream();  // via purify
        //   putInt8(int value);
        //   putUint8(int value);
        //   removeAll();
      ...
      case 1: {
        // --------------------------------------------------------------------
        // BREATHING TEST:
        //   Create 'bdex_GenericByteOutStream' objects using default and
      ...
      case -1: {
        ...
        if (verbose)
            cout << "\tTest performance on ctor+putInt8+dtor." << endl;
        {
            ...
            for (int i = 0; i <= NUM_OUT_ITERS; ++i) {
                bdesb_FixedMemOutput sb(workSpace, SIZE);
                bdex_GenericByteOutStream<bdesb_FixedMemOutput> x1(&sb);
        ...
        if (verbose)
            cout << "\tTest performance on ctor+putInt32+dtor." << endl;
        {
            for (int i = 0; i < SIZE; ++i) {
                workSpace[i] = (char)(i * (i + 1) / 2);
            }
        ...
        if (verbose) cout << "\nWith bdesb_FixedMemOutStreamBuf:" << endl;
