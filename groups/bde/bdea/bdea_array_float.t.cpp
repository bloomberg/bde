...
// [11] bdea_Array(int iLen, float iVal, *ba = 0);
// [17] bdea_Array(const InitialCapacity& ne, *ba = 0);
// [11] bdea_Array(const float *sa, int ne, *ba = 0);
// [ 7] bdea_Array(const bdea_Array<float>& original, *ba = 0);
// [ 2] ~bdea_Array();
// [ 9] bdea_Array<float>& operator=(const bdea_Array<float>& rhs);
// [14] float& operator[](int index);
// [13] void append(float item);
// [13] void append(const bdea_Array<float>& sa);
// [13] void append(const bdea_Array<float>& sa, int si, int ne);
// [13] void append(const float *sa, int si, int ne);
// [18] float *data();
// [13] void insert(int di, float item);
// [13] void insert(int di, const bdea_Array<float>& sa);
// [13] void insert(int di, const bdea_Array<float>& sa, int si, int ne);
// [13] void insert(int di, const float *sa, int si, int ne);
// [13] void remove(int index);
// [13] void remove(int index, int ne);
// [ 2] void removeAll();
// [14] void replace(int di, float item);
// [14] void replace(int di, const bdea_Array<float>& sa, int si, int ne);
...
// [12] void setLength(int newLength, float iVal);
...
// [ 4] const float& operator[](int index) const;
// [18] const float *data() const;
// [ 4] int length() const;
...
//
// [ 6] operator==(const bdea_Array<float>&, const bdea_Array<float>&);
// [ 6] operator!=(const bdea_Array<float>&, const bdea_Array<float>&);
// [ 5] operator<<(ostream&, const bdea_Array<float>&);
// [10] operator>>(bdex_InStream&, bdea_Array<float>&);
// [10] operator<<(bdex_OutStream&, const bdea_Array<float>&);
//-----------------------------------------------------------------------------
// [ 1] BREATHING TEST
// [19] USAGE EXAMPLE
// [ 2] BOOTSTRAP: void append(float item); // no aliasing
...
// [ 3] int ggg(bdea_Array<float> *object, const char *spec, int vF = 1);
// [ 3] bdea_Array<float>& gg(bdea_Array<float>* object, const char *spec);
// [ 8] bdea_Array<float>   g(const char *spec);
...
typedef bdea_Array<float> Obj;
typedef float Element;

const Element VALUES[] = { 1.5F, 2.5F, 3.5F,
                           4.75F, -5.25F }; // avoid DEFAULT_VALUE
...
// letters [A .. E] correspond to arbitrary (but unique) float values to be
// appended to the 'bdea_Array<float>' object.  A tilde ('~') indicates that
...
int ggg(bdea_Array<float> *object, const char *spec, int verboseFlag = 1)
...
bdea_Array<float>& gg(bdea_Array<float> *object, const char *spec)
...
bdea_Array<float> g(const char *spec)
    // Return, by value, a new object corresponding to the specified 'spec'.
{
    bdea_Array<float> object((bdema_Allocator *)0);
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      case 19: {
        ...
        const float E1 = 100.01F;
        const float E2 = 200.02F;

        bdea_Array<float> a(&testAllocator);
                                        ASSERT( 0 == a.length());
        ...
        const float E3 = 300.03F;

        a[0] = E3;                      ASSERT( 2 == a.length());
                                        ASSERT(E3 == a[0]);
                                        ASSERT(E2 == a[1]);

        const float E4 = 400.04F;
...
//      ostrstream out(buf, sizeof buf);     *** MISMATCH caused by EPC bug ***
        ...
        const char *const EXPECTED = "[ 300.03 400.04 200.02 ]\n";
      ...
      case 18: {
        ...
        //
        // Plan:
        //   For each object X in a set of 'bdea_Array<float>' objects ordered
...
#undef NL
      ...
      case 14: {
        ...
        // Testing:
        //   void replace(int di, float item);
        //   void replace(int di, const bdea_Array<float>& sa, int si, int ne);
      ...
      case 13: {
        ...
        //   void append(const bdea_Array<float>& sa);
        //   void append(const bdea_Array<float>& sa, int si, int ne);
        //   void append(const float *sa, int si, int ne);
        //
        //   void insert(int di, float item);
        //   void insert(int di, const bdea_Array<float>& sa);
        //   void insert(int di, const bdea_Array<float>& sa, int si, int ne);
        //   void insert(int di, const float *sa, int si, int ne);
        //
      ...
      case 12: {
        ...
        //   void setLength(int newLength, float iVal);
        // --------------------------------------------------------------------
        ...
        if (verbose) cout << "\nTesting 'setLengthRaw'" << endl;
        {
            ...
            const Element VALUE  = 1.2F; // ADJUST
        ...
        if (verbose) cout << "\nTesting 'setLength(int)'" << endl;
        {
            const int lengths[] = { 0, 1, 2, 3, 4, 5, 7, 8, 9, 15, 16, 17 };
            const int NUM_TESTS = sizeof lengths / sizeof lengths[0];
            const Element I_VALUE       = 1.2F;
            const Element DEFAULT_VALUE = 0.0F;
            for (int i = 0; i < NUM_TESTS; ++i) {
                const int a1 = lengths[i];
                if (verbose) { cout << "\t"; P(a1); }
                for (int j = 0; j < NUM_TESTS; ++j) {
        ...
        if (verbose) cout << "\nTesting 'setLength(int, float)'" << endl;
        {
            const int lengths[] = { 0, 1, 2, 3, 4, 5, 7, 8, 9, 15, 16, 17 };
            const int NUM_TESTS = sizeof lengths / sizeof lengths[0];
            const Element I_VALUE = 1.2F;
            const Element F_VALUE = 2.4F;
            for (int i = 0; i < NUM_TESTS; ++i) {
                const int a1 = lengths[i];
                if (verbose) { cout << "\t"; P(a1); }
                for (int j = 0; j < NUM_TESTS; ++j) {
      ...
      case 11: {
        ...
        //   bdea_Array(int iLen, float iVal, *ba = 0);
        //   bdea_Array(const float *sa, int ne, *ba = 0);
        // --------------------------------------------------------------------
        ...
        {
            const Element DEFAULT_VALUE = 0.0F;  // ADJUST
        ...
        if (verbose) cout << "\tWithout passing in an allocator." << endl;
        {
            const Element VALUE = 1.1F;  // ADJUST
            const int MAX_LENGTH = 10;
            for (int length = 0; length <= MAX_LENGTH; ++length) {
                if (verbose) P(length);
        ...
        if (verbose) cout << "\t\tWith no exceptions." << endl;
        {
            const Element VALUE = 2.2F;  // ADJUST
            const int MAX_LENGTH = 10;
            for (int length = 0; length <= MAX_LENGTH; ++length) {
                if (verbose) P(length);
        ...
        if (verbose) cout << "\t\tWith exceptions." << endl;
        {
            const Element VALUE = 3.3F;  // ADJUST
            const int MAX_LENGTH = 10;
            for (int length = 0; length <= MAX_LENGTH; ++length) {
              BEGIN_BDEMA_EXCEPTION_TEST {
                if (verbose) P(length);
        ...
        if (verbose) cout << "\tIn place using a buffer allocator." << endl;
        {
            char memory[1024];
            bdema_BufferAllocator a(memory, sizeof memory);
            const Element VALUE = 4.4F;  // ADJUST
            const int MAX_LENGTH = 10;
            for (int length = 0; length <= MAX_LENGTH; ++length) {
                if (verbose) P(length);
                Obj *doNotDelete =
...

        {
            const Element DA[] =
                { 1.0F, 1.1F, 1.2F, 1.3F, 1.4F,
                  1.5F, 1.6F, 1.7F, 1.8F, 1.9F }; // ADJUST
      ...
      case 10: {
        ...
        //   operator>>(bdex_InStream&, bdea_Array<float>&);
        //   operator<<(bdex_OutStream&, const bdea_Array<float>&);
        // --------------------------------------------------------------------
        ...
        if (verbose) cout << "\nWire format direct tests." << endl;
        {
            static const struct {
                ...
                { L_,   "",              1,   1, "\x00"                     },
                { L_,   "A",             1,   5, "\x01\x3f\xc0\x00\x00"     },
                { L_,   "AB",            1,   9,
                                     "\x02\x3f\xc0\x00\x00\x40\x20\x00\x00" },
                { L_,   "ABC",           1,  13,
                     "\x03\x3f\xc0\x00\x00\x40\x20\x00\x00\x40\x60\x00\x00" },
                { L_,   "ABCD",          1,  17,
     "\x04\x3f\xc0\x00\x00\x40\x20\x00\x00\x40\x60\x00\x00\x40\x98\x00\x00" },
                { L_,   "ABCDE",         1,  21,
                        "\x05\x3f\xc0\x00\x00\x40\x20\x00\x00\x40\x60\x00"
                                     "\x00\x40\x98\x00\x00\xc0\xa8\x00\x00" },
            };
            const int NUM_DATA = sizeof DATA / sizeof *DATA;
      ...
      case 7: {
        ...
        //
        // Testing:
        //   bdea_Array(const bdea_Array& original, *ba = 0);
        // --------------------------------------------------------------------
      ...
      case 6: {
        ...
        //
        // Testing:
        //   operator==(const bdea_Array<float>&, const bdea_Array<float>&);
        //   operator!=(const bdea_Array<float>&, const bdea_Array<float>&);
        // --------------------------------------------------------------------
      ...
      case 5: {
        ...
        //
        // Testing:
        //   operator<<(ostream&, const bdea_Array<float>&);
        // --------------------------------------------------------------------
      ...
      case 3: {
        ...
        // Testing:
        //   bdea_Array<float>& gg(bdea_Array<float>* object,const char *spec);
        //   int ggg(bdea_Array<float> *object, const char *spec, int vF = 1);
        //   void stretch(Obj *object, int size);
        //   void stretchRemoveAll(Obj *object, int size);
      ...
      case 2: {
        ...
        //   ~bdea_Array();
        //   BOOTSTRAP: void append(float item); // no aliasing
        //   void removeAll();
        // --------------------------------------------------------------------
