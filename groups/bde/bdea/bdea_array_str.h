...
#define INCLUDED_BDEA_ARRAY_STR

//@PURPOSE: Provide an in-place array of 'bdet_String' values.
//
//@DEPRECATED: This component is being replaced with 'std::vector'.
//
//@CLASSES:
//   bdea_Array<bdet_String>: memory manager for in-place array of
//                            'bdet_String' values
//
...
//
//@DESCRIPTION: This component implements an in-place array of 'bdet_String'
...
// memory when strings are deleted from the array.  The bdet_String destructor
...
// First create an empty 'bdea_Array<bdet_String>' 'a' and populate it with two
// elements 'E1' and 'E2':
//..
//      const bdet_String E1 = "a";
//      const bdet_String E2 = "bb";
//
//      bdea_Array<bdet_String> a;      assert( 0 == a.length());
//
...
// removed from the array. (Note that if sizeof(bdet_String) is greater than
...
template <class T> class bdea_Array;
                      ...
                      // =============================
                      // class bdea_Array<bdet_String>
                      // =============================

template<>
class bdea_Array<bdet_String> {
    // This class implements an efficient, in-place array of 'bdet_String'
    ...
    bdet_String *d_array_p;    // dynamically allocated array (d_size elements)
  ...
  private:
    void appendImp(const bdet_String& item);
  ...
  public:
    ...
    struct InitialCapacity { int d_i;  ~InitialCapacity() { }
        ...
        //..
        //   bdea_Array<bdet_String>
        //                      x(bdea_Array<bdet_String>::InitialCapacity(8));
        //..
    ...
    void append(const bdet_String& item);
        // Append to the end of this array the value of the specified 'item'.
        // Note that this function is logically equivalent to the following:
        // insert(length(), item).

    void append(const bdea_Array<bdet_String>& srcArray);
    ...
    void insert(int dstIndex, const bdet_String& item);
    ...
    void insert(int dstIndex, const bdea_Array<bdet_String>& srcArray);
    ...
    void replace(int dstIndex, const bdet_String& item);
    ...
    void setLength(int newLength, const bdet_String& initialValue);
    ...
    // ACCESSORS
    const bdet_String& operator[](int index) const;
...
std::ostream& operator<<(std::ostream&                  stream,
                         const bdea_Array<bdet_String>& array);
...
// ===========================================================================
                                ...
                                // ------------
inline
bdet_String& bdea_Array<bdet_String>::operator[](int index)
{
    return d_array_p[index];
}

inline
bdet_String *bdea_Array<bdet_String>::data()
{
    return d_array_p;
}

inline
void bdea_Array<bdet_String>::replace(int dstIndex, const bdet_String& item)
{
    d_array_p[dstIndex] = item;
}

inline
void bdea_Array<bdet_String>::swap(int index1, int index2)
{
    // Assumption: bdet_String is bitwise-moveable.
    bdeimp_BitwiseCopy<bdet_String>::swap(&d_array_p[index1],
                                          &d_array_p[index2]);
}

inline
void bdea_Array<bdet_String>::append(const bdet_String& item)
{
    if (d_length < d_size) {
...
inline void bdea_Array<bdet_String>::reserveCapacity(int numElements)
{
...
template <class STREAM>
inline
STREAM& bdea_Array<bdet_String>::bdexStreamIn(STREAM& stream, int version)
{
    if (stream) {
        switch (version) {  // Switch on the schema version (starting with 1).
          case 1: {
            ...
            for (int i = 0; i < d_length; ++i) {
                d_array_p[i].streamInRaw(stream, 1);
                if (!stream) break;
            }
...
template <class STREAM>
inline
STREAM& bdea_Array<bdet_String>::streamInRaw(STREAM& stream, int version)
{
...
inline
const bdet_String& bdea_Array<bdet_String>::operator[](int index) const
{
    return d_array_p[index];
}

inline
const bdet_String *bdea_Array<bdet_String>::data() const
{
    return d_array_p;
}

inline
int bdea_Array<bdet_String>::length() const
{
    return d_length;
}
...
template <class STREAM>
inline
STREAM& bdea_Array<bdet_String>::bdexStreamOut(STREAM& stream,
                                               int     version) const
{
    switch (version) {
      case 1: {
        ...
        for (int i = 0; i < d_length; ++i) {
            d_array_p[i].streamOutRaw(stream, 1);
        }
      } break;
    }
    return stream;
...
template <class STREAM>
inline
STREAM& bdea_Array<bdet_String>::streamOutRaw(STREAM& stream,
                                              int     version) const
{
