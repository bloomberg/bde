...
//-----------------------------------------------------------------------------
// [ 2] bdea_Array<int>(bdema_Allocator *ba = 0);
// [11] bdea_Array<int>(const Explicit& iLen, *ba = 0);
// [11] bdea_Array<int>(int iLen, int iVal, *ba = 0);
// [17] bdea_Array<int>(const InitialCapacity& ne, *ba = 0);
// [11] bdea_Array<int>(const int *sa, int ne, *ba = 0);
// [ 7] bdea_Array<int>(const bdea_Array<int>& original, *ba = 0);
// [ 2] ~bdea_Array<int>();
// [ 9] bdea_Array<int>& operator=(const bdea_Array<int>& rhs);
// [14] int& operator[](int index);
// [13] void append(int item);
// [13] void append(const bdea_Array<int>& sa);
// [13] void append(const bdea_Array<int>& sa, int si, int ne);
// [13] void append(const int *sa, int si, int ne);
// [18] int *data();
// [13] void insert(int di, int item);
// [13] void insert(int di, const bdea_Array<int>& sa);
// [13] void insert(int di, const bdea_Array<int>& sa, int si, int ne);
// [13] void insert(int di, const int *sa, int si, int ne);
// [13] void remove(int index);
// [13] void remove(int index, int ne);
// [ 2] void removeAll();
// [14] void replace(int di, int item);
// [14] void replace(int di, const bdea_Array<int>& sa, int si, int ne);
...
//
// [ 6] operator==(const bdea_Array<int>&, const bdea_Array<int>&);
// [ 6] operator!=(const bdea_Array<int>&, const bdea_Array<int>&);
// [ 5] operator<<(ostream&, const bdea_Array<int>&);
// [10] operator>>(bdex_InStream&, bdea_Array<int>&);
// [10] operator<<(bdex_OutStream&, const bdea_Array<int>&);
//-----------------------------------------------------------------------------
// [ 1] BREATHING TEST
// [19] USAGE EXAMPLE
// [ 2] BOOTSTRAP: void append(int item); // no aliasing
...
// [ 3] int ggg(bdea_Array<int> *object, const char *spec, int vF = 1);
// [ 3] bdea_Array<int>& gg(bdea_Array<int>* object, const char *spec);
// [ 8] bdea_Array<int>   g(const char *spec);
...
// appended to the 'bdea_Array<int>' object.  A tilde ('~') indicates that
...
int ggg(bdea_Array<int> *object, const char *spec, int verboseFlag = 1)
...
bdea_Array<int>& gg(bdea_Array<int> *object, const char *spec)
...
bdea_Array<int> g(const char *spec)
    // Return, by value, a new object corresponding to the specified 'spec'.
{
    bdea_Array<int> object((bdema_Allocator *)0);
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      case 19: {
        ...
        bdea_Array<int> a(&testAllocator);
                                        ASSERT( 0 == a.length());
      ...
      case 18: {
        ...
        //
        // Plan:
        //   For each object X in a set of 'bdea_Array<int>' objects ordered
      ...
      case 17: {
        ...
        if (verbose) cout <<
            "\nTesting 'bdea_Array<int>(capacity, ba)' Constructor" << endl;
...
#undef NL
      ...
      case 14: {
        ...
        // Testing:
        //   void replace(int di, int item);
        //   void replace(int di, const bdea_Array<int>& sa, int si, int ne);
      ...
      case 13: {
        ...
        //   void append(const bdea_Array<int>& sa);
        //   void append(const bdea_Array<int>& sa, int si, int ne);
        //   void append(const int *sa, int si, int ne);
        //
        //   void insert(int di, int item);
        //   void insert(int di, const bdea_Array<int>& sa);
        //   void insert(int di, const bdea_Array<int>& sa, int si, int ne);
        //   void insert(int di, const int *sa, int si, int ne);
        //
      ...
      case 12: {
        ...
        if (verbose) cout << "\nTesting 'setLength(int)'" << endl;
        {
            const int lengths[] = { 0, 1, 2, 3, 4, 5, 7, 8, 9, 15, 16, 17 };
            const int NUM_TESTS = sizeof lengths / sizeof lengths[0];
            const Element I_VALUE       = 1;    // ADJUST **
        ...
        if (verbose) cout << "\nTesting 'setLength(int, int)'" << endl;
        {
            ...
            const int NUM_TESTS = sizeof lengths / sizeof lengths[0];
            const Element I_VALUE = 1;  // ADJUST **
            const Element F_VALUE = 2;  // ADJUST **
            for (int i = 0; i < NUM_TESTS; ++i) {
                const int a1 = lengths[i];
                if (verbose) { cout << "\t"; P(a1); }
                for (int j = 0; j < NUM_TESTS; ++j) {
      ...
      case 11: {
        ...
        //   bdea_Array(int iLen, int iVal, *ba = 0);
        //   bdea_Array(const int *sa, int ne, *ba = 0);
        // --------------------------------------------------------------------
      ...
      case 10: {
        ...
        //   operator>>(bdex_InStream&, bdea_Array<int>&);
        //   operator<<(bdex_OutStream&, const bdea_Array<int>&);
        // --------------------------------------------------------------------
        ...
        if (verbose) cout << "\t\tGood stream (for control)." << endl;
        {
            ...
            out.putArrayInt32(a, A_LEN);        // ADJUST ***
            const char *const OD  = out.data();
            const int         LOD = out.length();
        ...
        if (verbose) cout << "\nWire format direct tests." << endl;
        {
            static const struct {
                ...
                { L_,   "",              1,   1, "\x00"                     },
                { L_,   "A",             1,   5, "\x01\x00\x00\x00\x01"     },
                { L_,   "AB",            1,   9,
                                     "\x02\x00\x00\x00\x01\x00\x00\x00\x02" },
                { L_,   "ABC",           1,  13,
                     "\x03\x00\x00\x00\x01\x00\x00\x00\x02\x00\x00\x00\x03" },
                { L_,   "ABCD",          1,  17,
     "\x04\x00\x00\x00\x01\x00\x00\x00\x02\x00\x00\x00\x03\x00\x00\x00\x04" },
                { L_,   "ABCDE",         1,  21,
                     "\x05\x00\x00\x00\x01\x00\x00\x00\x02\x00\x00\x00\x03"
                                         "\x00\x00\x00\x04\xff\xff\xff\xfb" },
            };
            const int NUM_DATA = sizeof DATA / sizeof *DATA;
      ...
      case 7: {
        ...
        //
        // Testing:
        //   bdea_Array(const bdea_Array<int>& original, *ba = 0);
        // --------------------------------------------------------------------
      ...
      case 6: {
        ...
        //
        // Testing:
        //   operator==(const bdea_Array<int>&, const bdea_Array<int>&);
        //   operator!=(const bdea_Array<int>&, const bdea_Array<int>&);
        // --------------------------------------------------------------------
      ...
      case 5: {
        ...
        //
        // Testing:
        //   operator<<(ostream&, const bdea_Array<int>&);
        // --------------------------------------------------------------------
      ...
      case 3: {
        ...
        // Testing:
        //   bdea_Array<int>& gg(bdea_Array<int>* object, const char *spec);
        //   int ggg(bdea_Array<int> *object, const char *spec, int vF = 1);
        //   void stretch(Obj *object, int size);
        //   void stretchRemoveAll(Obj *object, int size);
      ...
      case 2: {
        ...
        //   ~bdea_Array();
        //   BOOTSTRAP: void append(int item); // no aliasing
        //   void removeAll();
        // --------------------------------------------------------------------
