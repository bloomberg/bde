...
#define INCLUDED_BDEA_ARRAY_TIME

//@PURPOSE: Provide an in-place array of 'bdet_Time' values.
//
//@DEPRECATED: This component is being replaced with 'std::vector'.
//
//@CLASSES:
//   bdea_Array<bdet_Time>: memory manager for in-place array of 'bdet_Time'
//
//@AUTHOR: Paul Staniforth (pstaniforth)
//
//@SEE_ALSO: bdea_array2_time
//
//@DESCRIPTION: This component implements an in-place array of 'bdet_Time'
// values.  The contained elements are stored contiguously in memory and can be
// accessed directly via the 'const' and non-'const' 'data()' methods.  Note
// that appending, inserting, and removing elements can potentially alter the
// memory addresses of other elements in the array.  Consider instead using
// 'bdea_array2_time' if the memory address of each contained element must
...
// First create an empty 'bdea_Array<bdet_Time>' 'a' and populate it with two
// elements 'E1' and 'E2':
//..
//      const bdet_Time E1(1, 30, 0);
//      const bdet_Time E2(2, 10, 3, 1);
//
//      bdea_Array<bdet_Time> a;        assert( 0 == a.length());
//
...
template <class T> class bdea_Array;
                      ...
                      // ===========================
                      // class bdea_Array<bdet_Time>
                      // ===========================

template<>
class bdea_Array<bdet_Time> {
    // This class implements an efficient, in-place array of 'bdet_Time' values
    ...
    friend int operator==(const bdea_Array<bdet_Time>& lhs,
                          const bdea_Array<bdet_Time>& rhs);
  ...
  public:
    ...
    struct InitialCapacity { int d_i; ~InitialCapacity() { }
        ...
        //..
        //   bdea_Array<bdet_Time>
        //                        x(bdea_Array<bdet_Time>::InitialCapacity(8));
        //..
    ...
    bdea_Array(int               initialLength,
        ...
        // value using the default constructor of 'bdet_Time' if 'initialValue'
        // is not specified.  Optionally specify a 'basicAllocator' used to
    ...
    bdea_Array(const bdet_Time *srcArray,
               int              numElements,
               bdema_Allocator *basicAllocator = 0);
    ...
    bdea_Array(const bdea_Array<bdet_Time>&  original,
               bdema_Allocator              *basicAllocator = 0);
    ...
    // MANIPULATORS
    bdea_Array<bdet_Time>& operator=(const bdea_Array<bdet_Time>& rhs);
    ...
    void append(const bdet_Time& item);
    ...
    void append(const bdea_Array<bdet_Time>& srcArray);
    ...
    void append(const bdet_Time *srcArray,
                int              srcIndex,
                int              numElements);
        // Append to the end of this array the specified 'numElements' values
        // from the specified 'srcArray', beginning at the specified
    ...
    void insert(int dstIndex, const bdet_Time& item);
    ...
    void insert(int dstIndex, const bdea_Array<bdet_Time>& srcArray);
    ...
    void insert(int                          dstIndex,
                const bdea_Array<bdet_Time>& srcArray,
                int                          srcIndex,
                int                          numElements);
    ...
    void replace(int dstIndex, const bdet_Time& item);
    ...
    void setLength(int newLength, const bdet_Time& initialValue);
        ...
        // constructor of 'bdet_Time' if 'initialValue' is not specified.  The
        // behavior is undefined unless 0 <= newLength.
    ...
    // ACCESSORS
    const bdet_Time& operator[](int index) const;
...
// FREE OPERATORS
inline
int operator==(const bdea_Array<bdet_Time>& lhs,
               const bdea_Array<bdet_Time>& rhs);
...
std::ostream& operator<<(std::ostream&                stream,
                         const bdea_Array<bdet_Time>& array);
...
inline
bdet_Time *bdea_Array<bdet_Time>::data()
{
    return (bdet_Time *)d_array.data();
}

inline
const bdet_Time *bdea_Array<bdet_Time>::data() const
{
    return (const bdet_Time *)d_array.data();
}

inline
int bdea_Array<bdet_Time>::length() const
{
    return d_array.length() / sizeof(bdet_Time);
}
...
inline
bdea_Array<bdet_Time>::bdea_Array(bdema_Allocator *basicAllocator)
: d_array(bdema_CharArray::InitialCapacity(sizeof(bdet_Time)), basicAllocator)
{
}

inline
bdea_Array<bdet_Time>::bdea_Array(const Explicit&  initialLength,
                                  bdema_Allocator *basicAllocator)
: d_array(bdema_CharArray::InitialCapacity(
                                        initialLength.d_i > 0
                                        ? initialLength.d_i * sizeof(bdet_Time)
                                        : sizeof(bdet_Time)),
          bdema_CharArray::POWER_OF_2_HINT,
          basicAllocator)
{
    d_array.setLengthRaw(initialLength.d_i * sizeof(bdet_Time));
    bdeimp_DuffsDevice<bdet_Time>::initialize(data(),
                                              bdet_Time(),
                                              initialLength.d_i);
}

inline
bdea_Array<bdet_Time>::bdea_Array(int               initialLength,
                                  const bdet_Time&  initialValue,
                                  bdema_Allocator  *basicAllocator)
: d_array(bdema_CharArray::InitialCapacity(initialLength > 0
                                           ? initialLength * sizeof(bdet_Time)
                                           : sizeof(bdet_Time)),
          bdema_CharArray::POWER_OF_2_HINT,
          basicAllocator)
{
    d_array.setLengthRaw(initialLength * sizeof(bdet_Time));
    bdeimp_DuffsDevice<bdet_Time>::initialize(data(),
                                              initialValue,
                                              initialLength);
}
...
inline
bdea_Array<bdet_Time>::bdea_Array(const InitialCapacity&  numElements,
                                  bdema_Allocator        *basicAllocator)
: d_array(bdema_CharArray::InitialCapacity(numElements.d_i > 0
                                          ? numElements.d_i * sizeof(bdet_Time)
                                          : sizeof(bdet_Time)),
          basicAllocator)
{
...
inline
bdea_Array<bdet_Time>::bdea_Array(const bdet_Time *srcArray,
                                  int              numElements,
                                  bdema_Allocator *basicAllocator)
: d_array(bdema_CharArray::InitialCapacity(numElements > 0
                                           ? numElements * sizeof(bdet_Time)
                                           : sizeof(bdet_Time)),
          bdema_CharArray::POWER_OF_2_HINT,
          basicAllocator)
{
    d_array.setLengthRaw(numElements * sizeof(bdet_Time));
    std::memcpy(data(), srcArray, numElements * sizeof(bdet_Time));
}

inline
bdea_Array<bdet_Time>::bdea_Array(const bdea_Array<bdet_Time>&  original,
                                  bdema_Allocator              *basicAllocator)
: d_array(bdema_CharArray::InitialCapacity(
                                        original.length() > 0
                                        ? original.length() * sizeof(bdet_Time)
                                        : sizeof(bdet_Time)),
          bdema_CharArray::POWER_OF_2_HINT,
          basicAllocator)
{
    d_array.setLengthRaw(original.length() * sizeof(bdet_Time));
    std::memcpy(data(), original.data(),
                original.length() * sizeof(bdet_Time));
}

inline
bdea_Array<bdet_Time>::~bdea_Array()
{
}
...
inline
bdea_Array<bdet_Time>& bdea_Array<bdet_Time>::
                                    operator=(const bdea_Array<bdet_Time>& rhs)
{
    d_array = rhs.d_array;
...
inline
bdet_Time& bdea_Array<bdet_Time>::operator[](int index)
{
    return data()[index];
}

inline
void bdea_Array<bdet_Time>::append(const bdet_Time& item)
{
    const bdet_Time itemTmp = item;  // guard against alias
    int len = length();
    d_array.setLengthRaw(d_array.length() + sizeof(bdet_Time));
    data()[len] = itemTmp;
}

inline
void bdea_Array<bdet_Time>::append(const bdea_Array<bdet_Time>& srcArray)
{
...
{
    d_array.append(srcArray.d_array,
                   srcIndex * sizeof(bdet_Time),
                   numElements * sizeof(bdet_Time));
}

inline
void bdea_Array<bdet_Time>::
               append(const bdet_Time *srcArray, int srcIndex, int numElements)
{
...
inline
void bdea_Array<bdet_Time>::insert(int dstIndex, const bdet_Time& item)
{
    d_array.insert(dstIndex * sizeof(bdet_Time),
                   (const char *)&item,
                   0,
                   sizeof(bdet_Time));
}
...
inline
void bdea_Array<bdet_Time>::
                    insert(int dstIndex, const bdea_Array<bdet_Time>& srcArray)
{
    d_array.insert(dstIndex * sizeof(bdet_Time), srcArray.d_array);
}

inline
void bdea_Array<bdet_Time>::insert(int                          dstIndex,
                                   const bdea_Array<bdet_Time>& srcArray,
                                   int                          srcIndex,
                                   int                          numElements)
{
...
inline
void bdea_Array<bdet_Time>::remove(int index)
{
    d_array.remove(index * sizeof(bdet_Time), sizeof(bdet_Time));
}

inline
void bdea_Array<bdet_Time>::remove(int index, int numElements)
{
    d_array.remove(index * sizeof(bdet_Time), numElements * sizeof(bdet_Time));
}

inline
void bdea_Array<bdet_Time>::removeAll()
{
    d_array.removeAll();
}

inline
void bdea_Array<bdet_Time>::replace(int dstIndex, const bdet_Time& item)
{
    data()[dstIndex] = item;
}

inline
void bdea_Array<bdet_Time>::replace(int                          dstIndex,
                                    const bdea_Array<bdet_Time>& srcArray,
                                    int                          srcIndex,
                                    int                          numElements)
{
    d_array.replace(dstIndex * sizeof(bdet_Time),
                    srcArray.d_array,
                    srcIndex * sizeof(bdet_Time),
                    numElements * sizeof(bdet_Time));
}

inline void bdea_Array<bdet_Time>::reserveCapacity(int numElements)
{
    d_array.reserveCapacity(numElements * sizeof(bdet_Time));
}

inline
void bdea_Array<bdet_Time>::reserveCapacityRaw(int numElements)
{
    d_array.reserveCapacityRaw(numElements * sizeof(bdet_Time));
}

inline
void bdea_Array<bdet_Time>::setLengthRaw(int newLength)
{
    d_array.setLengthRaw(newLength * sizeof(bdet_Time));
}

inline
void bdea_Array<bdet_Time>::setLength(int newLength)
{
    int len = length();
    d_array.setLengthRaw(newLength * sizeof(bdet_Time));
    if (newLength > len) {
        bdeimp_DuffsDevice<bdet_Time>::initialize(data() + len,
                                                  bdet_Time(),
                                                  newLength - len);
    }
}
                                      ...
                                      const bdet_Time& initialValue)
{
    ...
    d_array.setLengthRaw(newLength * sizeof(bdet_Time));
    if (newLength > len) {
        const bdet_Time initialValueTmp = initialValue;  // may be alias
        bdeimp_DuffsDevice<bdet_Time>::initialize(data() + len,
                                                  initialValueTmp,
                                                  newLength - len);
    }
...
inline
void bdea_Array<bdet_Time>::swap(int index1, int index2)
{
    bdet_Time *array_p = data();
    bdet_Time tmp = array_p[index1];
...
template <class STREAM>
inline
STREAM& bdea_Array<bdet_Time>::bdexStreamIn(STREAM& stream, int version)
{
    if (stream) {
...
template <class STREAM>
inline
STREAM& bdea_Array<bdet_Time>::streamInRaw(STREAM& stream, int version)
{
...
inline
const bdet_Time& bdea_Array<bdet_Time>::operator[](int index) const
{
    return data()[index];
}
...
template <class STREAM>
inline
STREAM& bdea_Array<bdet_Time>::bdexStreamOut(STREAM& stream, int version) const
{
    switch (version) {
      case 1: {
        int len = length();
...
template <class STREAM>
inline
STREAM& bdea_Array<bdet_Time>::streamOutRaw(STREAM& stream, int version) const
{
