...
class my_Class5 : public my_Class4 {
  ...
  public:
    // CREATORS
    explicit my_Class5(bslma::Allocator *a = 0) : my_Class4(a) {}
...
#define TEST_OP2(typeNum, op, expVal, expAlloc) {                             \
    typedef my_Class ## typeNum Type;                                         \
    static const int EXP_VAL = (expVal);                                      \
    bslma::Allocator *const EXP_ALLOC = (expAlloc);                           \
    my_ClassDef rawBuf;                                                       \
...
#define TEST_PAIR2(op, expVal0, expA0, expVal1, expA1) {                      \
    static const int EXP_VAL0 = (expVal0);                                    \
    bslma::Allocator *const EXP_ALLOC0 = (expA0);                             \
    static const int EXP_VAL1 = (expVal1);                                    \
    bslma::Allocator *const EXP_ALLOC1 = (expA1);                             \
...
#define TEST_PAIROP2(typeNum0, typeNum1, op, expVal0, expA0, expVal1, expA1) {\
    typedef my_Pair_ ## typeNum0 ## _ ## typeNum1 Type;                       \
    TEST_PAIR2(op, expVal0, expA0, expVal1, expA1);                           \
  }
    // This 'macro' evaluates the specified 'op' expression in the namespace
    // under test, involving the address 'objPtr' of an object of type
    // 'my_Pair_N0_N1' (where 'N0' and 'N1' stand for the specified 'typeNum0
...
#define TEST_PAIRAOP2(typeNum0, typeNum1, op, expVal0, expA0, expVal1, expA1) \
  {                                                                           \
    typedef my_PairA_ ## typeNum0 ## _ ## typeNum1 Type;                      \
    TEST_PAIR2(op, expVal0, expA0, expVal1, expA1);                           \
  }
...
#define TEST_PAIRAAOP2(typeNum0, typeNum1, op, expVal0, expA0, expVal1, expA1)\
  {                                                                           \
    typedef my_PairAA_ ## typeNum0 ## _ ## typeNum1 Type;                     \
    TEST_PAIR2(op, expVal0, expA0, expVal1, expA1);                           \
  }
...
// 'bsl' when building against opensource 'Bloomberg/bsl', 'destruct' is not
// available.  Defining the following macro (thus not testing the method) is
// preferred over reviving the entire implementation of
// 'bdealg_ScalarPrimitives'.

#ifdef destruct
#undef destruct
#endif
#define destruct bslalg::ScalarDestructionPrimitives::destroy
        ...
        //      #      Operation     Val Alloc
        //      == ================= === =====
        TEST_OP2(1, destruct(objPtr), 91, 0);
        TEST_OP2(2, destruct(objPtr), 92, 0);

        // my_Pair                           first     second
        //           #_#   Operation        Val Alloc Val Alloc
        //          == == ================= === ===== === =====
        TEST_PAIROP2(1, 1, destruct(objPtr), 91, 0,    91, 0);
        TEST_PAIROP2(1, 2, destruct(objPtr), 91, 0,    92, 0);
        TEST_PAIROP2(2, 1, destruct(objPtr), 92, 0,    91, 0);
        TEST_PAIROP2(2, 2, destruct(objPtr), 92, 0,    92, 0);

        // my_PairA                           first     second
        //            #_#  Operation         Val Alloc Val Alloc
        //            = =  ================= === ===== === =====
        TEST_PAIRAOP2(1, 2, destruct(objPtr), 91, 0,    92, 0);
        TEST_PAIRAOP2(2, 2, destruct(objPtr), 92, 0,    92, 0);

        // my_PairAA                           first     second
        //             #_#  Operation         Val Alloc Val Alloc
        //             = =  ================= === ===== === =====
        TEST_PAIRAAOP2(2, 2, destruct(objPtr), 92, 0,    92, 0);
...
#undef destruct
      ...
      case 4: {
        ...
        //   'bslma::Allocator *' and 'void *', and verify that the values
