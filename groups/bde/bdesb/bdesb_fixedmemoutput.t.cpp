...
// 'bdesb_FixedMemOutput', as well as each public method in the
// 'bdesb_FixedMemOutput' class that is not part of the 'basic_streambuf'
// protocol.
//
...
//   A 'bdesb_FixedMemOutput' is created with a client-supplied C char
...
//
//    o bdesb_FixedMemOutput(char *buffer, bsl::streamsize length);
//
// Primary Manipulators:
//   We can bring a 'bdesb_FixedMemOutput' to any achievable white-box
...
//   'bdesb_FixedMemOutput' component has only 'length', 'data' and 'capacity'
//   as accessors, and so they form our accessor set:
//
...
//-----------------------------------------------------------------------------
// CREATORS
// [ 3] bdesb_FixedMemOutput(char *buffer, bsl::streamsize length);
// [ 6] ~bdesb_FixedMemOutput();
//
// MANIPULATORS
...
// [ 7] streamsize xsputn(const char_type, streamsize);
//
// ACCESSORS
...
// [ 1] BREATHING TEST
// [ 2] TEST APPARATUS
// [ 5] ostream& operator<<(ostream&, const bdesb_FixedMemOutput&);
//
// USAGE EXAMPLE
...
//-----------------------------------------------------------------------------
               ...
               // ====================================
               // operator<< for bdesb_MemOutput
               // ====================================
// FREE OPERATORS
bsl::ostream& operator<<(bsl::ostream&                     stream,
                         const bdesb_FixedMemOutput& streamBuffer);
...
//..
 class my_CapitalizingStream {
     ...
     char *d_buffer;
     bdesb_FixedMemOutput *d_streamBuf;
   ...
   public:
     ...
     // ACCESSORS
     const bdesb_FixedMemOutput* streamBuf() { return d_streamBuf; }
...
//..
 ...
 my_CapitalizingStream::my_CapitalizingStream()
 {
     ...
     d_streamBuf = new bdesb_FixedMemOutput(d_buffer,
                                                           STREAMBUF_CAPACITY);
 }
...
int main(int argc, char **argv)
{
      ...
      case 8: {
...

        {
            ...
            for (int i = 0; i < DATA_LEN; ++i ) {
                ...
                bdesb_FixedMemOutput mSB(buffer, INITIAL_BUFSIZE);

                bdesb_FixedMemOutput::int_type ret;
                          ...
                          << endl;
        {
            ...
            for (int i = 0; i < DATA_LEN; ++i ) {
                ...
                memset(buffer, 'Z', INITIAL_BUFSIZE);
                bdesb_FixedMemOutput mSB(buffer, INITIAL_BUFSIZE);
                const bdesb_FixedMemOutput& SB = mSB;
      ...
      case 7: {
...

        {
            ...
            for(int i = 0; i < DATA_LEN; ++i ) {
                ...
                bdesb_FixedMemOutput sb(bytes, DATA[i].d_strCap);
                ...
                LOOP2_ASSERT(LINE,
                             retResult,
                             DATA[i].d_returnVal == retResult );
                LOOP2_ASSERT(LINE,
                             sb.length(),
                             DATA[i].d_numChars == sb.length());
                delete [] bytes;
            }
        }
      } break;
    ...
    "There are no non-primary constructors, and the destructor does nothing."
      ...
      case 5: {
        ...
        //
        // Testing:
        //   ostream& operator<<(ostream&, const bdesb_MemOutput&);
        // --------------------------------------------------------------------
...

        {
            ...
            char buffer[INITIAL_BUFSIZE];
            memset(buffer, 'Z', INITIAL_BUFSIZE);
            bdesb_FixedMemOutput mSB(buffer,
                                            INITIAL_BUFSIZE);
            const bdesb_FixedMemOutput& SB = mSB;
      ...
      case 4: {
        ...
        //     stream buffer.
        //   - That capacity return the number of characters left to be written
        ...
        //
        //   (data and length)
        //   - Create an empty 'bdesb_MemOutput' and verify its length.
        ...
        //   - Table-Based and Brute-Force Implementation Technique for 'sputc'
        ...
        if (verbose) cout << "\nTesting sputc." << endl;
        {
            typedef bdesb_FixedMemOutput::char_type T;
        ...
        }
        {
            ...
            for(int i = 0; i < DATA_LEN; ++i ) {
                const int LINE      = DATA[i].d_line;
                char *bytes = new char[DATA[i].d_strCap];
                bdesb_FixedMemOutput mSB(bytes, DATA[i].d_strCap);
                const bdesb_FixedMemOutput& SB = mSB;
        ...
        }
        {
            ...
            char buffer[5];
            memset(buffer, 'X', 5);
            bdesb_FixedMemOutput x(buffer, 5);
        ...
        }
        {
            ...
            static const struct {
                ...
                bdesb_FixedMemOutput::pos_type  d_amount;
                                              // amount to seek
                int         d_retVal;         // expected return value
            } DATA[] = {
            ...
            for (int i = 0; i < DATA_LEN; ++i ) {
                ...
                bdesb_FixedMemOutput::pos_type ret;
      ...
      case 1: {
        ...
        if (verbose) cout <<
          "\nMake sure we can create and use a 'bdesb_FixedMemOutput'."
                          << endl;
        {
            char buffer[INITIAL_BUFSIZE];
            bdesb_FixedMemOutput mSB(buffer, INITIAL_BUFSIZE);
