...
//
//@SEE_ALSO: bdesb_overflowmemoutput
//
...
// dynamically allocated overflow buffer.  As with 'bdesb_fixedmemoutput',
// users supply the character buffer at construction.  Unlike
// 'bdesb_fixedmemoutput', they can no longer reinitialize the stream buffer
// with a different character buffer by calling the 'pubsetbuf' method;
// instead, if that buffer runs out, the 'bdesb_OverflowMemOutput' will
// allocate another buffer (see "Overflow Buffer" below).   The only difference
// between this component and 'bdesb_overflowmemoutput' is that the class
// 'bdesb_OverflowMemOutput' *does* derive from a 'bsl::streambuf'.  Method
...
//
//      // ACCESSORS
//      const bdesb_OverflowMemOutput *streamBuf();
...
//
//      d_streamBuf = new(*d_allocator_p) bdesb_OverflowMemOutput(
//                                                          d_buffer,
//                                                          STREAMBUF_CAPACITY,
//                                                          d_allocator_p);
//  }
...
class bdesb_OverflowMemOutStreamBuf : public bsl::streambuf {
    ...
    // parameter to 'bdex_GenericByteOutStream' as well as 'bdex_ByteOutStream'
    // or 'bdex_ByteOutStreamFormatter'.
