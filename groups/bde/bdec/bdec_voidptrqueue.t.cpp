...
// [11] bdec_VoidPtrQueue(const void **sa, int ne, *ba = 0);
// [ 7] bdec_VoidPtrQueue(const bdec_VoidPtrQueue& original, *ba = 0);
// [ 2] ~bdec_VoidPtrQueue();
// [ 9] bdec_VoidPtrQueue& operator=(const bdec_VoidPtrQueue& rhs);
// [14] float& operator[](int index);
// [13] void append(void *item);
// [13] void append(const bdec_VoidPtrQueue& sa);
// [13] void append(const bdec_VoidPtrQueue& sa, int si, int ne);
// [14] float& back();
// [14] float& front();
// [13] void insert(int di, void *item);
// [13] void insert(int di, const bdec_VoidPtrQueue& sa);
// [13] void insert(int di, const bdec_VoidPtrQueue& sa, int si, int ne);
// [13] void popBack();
// [13] void popFront();
// [13] void pushBack(void *item);
// [13] void pushFront(void *item);
// [13] void remove(int index);
// [13] void remove(int index, int ne);
// [ 2] void removeAll();
// [14] void replace(int di, void *item);
// [14] void replace(int di, const bdec_VoidPtrQueue& sa, int si, int ne);
...
// [12] void setLength(int newLength, void *iVal);
...
//
// [ 6] operator==(const bdec_VoidPtrQueue&, const bdec_VoidPtrQueue&);
// [ 6] operator!=(const bdec_VoidPtrQueue&, const bdec_VoidPtrQueue&);
// [ 5] operator<<(ostream&, const bdec_VoidPtrQueue&);
// [10] operator>>(bdex_InStream&, bdec_VoidPtrQueue&);
// [10] operator<<(bdex_OutStream&, const bdec_VoidPtrQueue&);
//-----------------------------------------------------------------------------
// [ 1] BREATHING TEST
// [18] USAGE EXAMPLE
// [ 2] BOOTSTRAP: void popFront();
// [ 2] BOOTSTRAP: void pushBack(void *item);
...
// [ 3] int ggg(bdec_VoidPtrQueue *object, const char *spec, int vF = 1);
// [ 3] bdec_VoidPtrQueue& gg(bdec_VoidPtrQueue* object, const char *spec);
// [ 8] bdec_VoidPtrQueue   g(const char *spec);
...
typedef bdec_VoidPtrQueue Obj;
typedef void *Element;

const Element VALUES[] = { (void *) 0xa1,
                           (void *) 0xb2,
                           (void *) 0xc3,
                           (void *) 0xd4,
                           (void *) 0xfff5 }; // avoid DEFAULT_VALUE
...
// letters [A .. E] correspond to arbitrary (but unique) 'void *' values to be
// appended to the 'bdec_VoidPtrQueue' object.  A tilde ('~') indicates that
...
int ggg(bdec_VoidPtrQueue *object, const char *spec, int verboseFlag = 1)
...
bdec_VoidPtrQueue& gg(bdec_VoidPtrQueue *object, const char *spec)
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      ...
      case 17: {
        ...
        //
        // Testing:
        //   bdec_VoidPtrQueue(const InitialCapacity& ne, *ba = 0);
    ...
    case 16: {
                          ...
                          << endl;
        {
            ...
            for (int iLen = 0; iLen < NUM_TRIALS; ++iLen) { // initial length
                ...
                for (int pos1 = 0; pos1 < iLen; ++pos1) { // for each position
                    for (int pos2 = 0; pos2 < iLen; ++pos2) { // for each pos.
                        Obj mT(bdec_VoidPtrQueue::InitialCapacity(8));
                        mT.pushBack(VA);
                        mT.pushBack(VA);
                        mT.pushBack(VA);
                        mT.popFront();
...
#undef NL
      ...
      case 14: {
        ...
        // Testing:
        //   void replace(int di, void *item);
        //   void replace(int di, const bdec_VoidPtrQueue& sa, int si, int ne);
      ...
      case 13: {
        ...
        //   void append(const bdec_VoidPtrQueue& sa);
        //   void append(const bdec_VoidPtrQueue& sa, int si, int ne);
        //
        ...
        //   void popFront();
        //
        //   void insert(int di, void *item);
        //   void insert(int di, const bdec_VoidPtrQueue& sa);
        //   void insert(int di, const bdec_VoidPtrQueue& sa, int si, int ne);
        //
      ...
      case 12: {
        ...
        //   void setLength(int newLength, void *iVal);
        // --------------------------------------------------------------------
        ...
        if (verbose) cout << "\nTesting 'setLengthRaw'" << endl;
        {
            ...
            const Element VALUE  = (void *)0x1; // ADJUST
        ...
        if (verbose) cout << "\nTesting 'setLength(int)'" << endl;
        {
            ...
            const Element I_VALUE       = (void *)0x1;
            const Element DEFAULT_VALUE = (void *)0x0;
            for (int i = 0; i < NUM_TESTS; ++i) {
                const int a1 = lengths[i];
                if (verbose) { cout << "\t"; P(a1); }
                for (int j = 0; j < NUM_TESTS; ++j) {
        ...
        if (verbose) cout << "\nTesting 'setLength(int, float)'" << endl;
        {
            ...
            const int shifts[] =  { 0, 1, 2, 3, 4, 5, 7, 8, 9, 15, 16, 17 };
            const int NUM_SHIFTS = sizeof shifts / sizeof shifts[0];
            const Element I_VALUE = (void *)0x2;
            const Element F_VALUE = (void *)0x4;
            for (int i = 0; i < NUM_TESTS; ++i) {
                const int a1 = lengths[i];
                if (verbose) { cout << "\t"; P(a1); }
                for (int j = 0; j < NUM_TESTS; ++j) {
      ...
      case 11: {
        ...
        //
        // Testing:
        //   bdec_VoidPtrQueue(unsigned int iLen, *ba = 0);
        //   bdec_VoidPtrQueue(int iLen, void *iVal, *ba = 0);
        //   bdec_VoidPtrQueue(const void **sa, int ne, *ba = 0);
        // --------------------------------------------------------------------
        ...
        if (verbose) cout << "\t\tWith no exceptions." << endl;
        {
            const Element VALUE = (void *)0x2;  // ADJUST
            const int MAX_LENGTH = 10;
            for (int length = 0; length <= MAX_LENGTH; ++length) {
                if (verbose) P(length);
        ...
        if (verbose) cout << "\t\tWith exceptions." << endl;
        {
            const Element VALUE = (void *)0x3;  // ADJUST
            const int MAX_LENGTH = 10;
            for (int length = 0; length <= MAX_LENGTH; ++length) {
              BEGIN_BSLMA_EXCEPTION_TEST {
                if (verbose) P(length);
      ...
      case 10: {
        ...
        // The 'bdec_VoidPtrQueue' does not need a streaming function.
        // --------------------------------------------------------------------

        if (verbose) cout << endl
                          << "Testing Streaming Functionality" << endl
                          << "===============================" << endl
                          << "Streaming is not required in this component"
                          << endl;
      } break;
      case 9: {
        // --------------------------------------------------------------------
      ...
      case 7: {
        ...
        //
        // Testing:
        //   bdec_VoidPtrQueue(const bdec_VoidPtrQueue& original, *ba = 0);
        // --------------------------------------------------------------------
      ...
      case 6: {
        ...
        //
        // Testing:
        //   operator==(const bdec_VoidPtrQueue&, const bdec_VoidPtrQueue&);
        //   operator!=(const bdec_VoidPtrQueue&, const bdec_VoidPtrQueue&);
        // --------------------------------------------------------------------
      ...
      case 5: {
        ...
        //
        // Testing:
        //   operator<<(ostream&, const bdec_VoidPtrQueue&);
        // --------------------------------------------------------------------
        ...
        if (verbose) cout << "\nTesting 'operator<<' (ostream)." << endl;
        {
            static const struct {
                ...
                { L_,   "A",            "[ a1 ]"                        },
                { L_,   "BC",           "[ b2 c3 ]"                     },
                { L_,   "ABCDE",        "[ a1 b2 c3 d4 fff5 ]"          },
                { L_,   "=6+0ABCDE",    "[ a1 b2 c3 d4 fff5 ]"          },
                { L_,   "=6+1ABCDE",    "[ a1 b2 c3 d4 fff5 ]"          },
                { L_,   "=6+2ABCDE",    "[ a1 b2 c3 d4 fff5 ]"          },
                { L_,   "=6+3ABCDE",    "[ a1 b2 c3 d4 fff5 ]"          },
                { L_,   "=6+4ABCDE",    "[ a1 b2 c3 d4 fff5 ]"          },
                { L_,   "=6+5ABCDE",    "[ a1 b2 c3 d4 fff5 ]"          },
                { L_,   "=6+6ABCDE",    "[ a1 b2 c3 d4 fff5 ]"          },
                { L_,   "=6+7ABCDE",    "[ a1 b2 c3 d4 fff5 ]"          },
            };
            const int NUM_DATA = sizeof DATA / sizeof *DATA;
      ...
      case 3: {
        ...
        // Testing:
        //   bdec_VoidPtrQueue& gg(bdec_VoidPtrQueue* object, const char *spec)
        //   int ggg(bdec_VoidPtrQueue *object, const char *spec, int vF = 1);
        //   void rotate(Obj *object, int numPositions);
        //   void stretch(Obj *object, int size);
        //   void stretchRemoveAll(Obj *object, int size);
      ...
      case 2: {
        ...
        //   BOOTSTRAP: void pushBack(void *item);
