...
#define INCLUDED_BDEMA_MULTIPOOL
...
//
//@CLASSES:
//  bdema_Multipool: memory manager that manages pools of varying block sizes
//
//@SEE_ALSO: bdema_pool, bdema_multipoolallocator
//
//@AUTHOR: Shao-wei Hung (shung1)
//
//@DESCRIPTION: This component implements a memory manager, 'bdema_Multipool',
// that maintains a configurable number of 'bdema_Pool' objects, each
// dispensing memory blocks of a unique size.  The 'bdema_Pool' objects are
...
// the 'release' method and the destructor of a 'bdema_Multipool' release all
// memory currently allocated via the object.
//
// A 'bdema_Multipool' can be depicted visually:
//..
...
// 'bdema_Pool' maintained by the multipool, from which memory blocks of
// uniform size are dispensed to users.
//
///Configuration at Construction
///-----------------------------
// When creating a 'bdema_Multipool', clients can optionally configure:
//
...
// currently installed default allocator at the time the 'bdema_Multipool' was
// created.
//
...
class bdema_Multipool {
    ...
    // unique size.  The 'bdema_Pool' objects are placed in an array, with each
    ...
    // DATA
    bdema_Pool      *d_pools_p;       // array of memory pools, each dispensing
  ...
  private:
    // NOT IMPLEMENTED
    bdema_Multipool(const bdema_Multipool&);
    bdema_Multipool& operator=(const bdema_Multipool&);

  public:
    // CREATORS
    bdema_Multipool(bslma_Allocator                  *basicAllocator = 0);
    bdema_Multipool(int                               numPools,
                    bslma_Allocator                  *basicAllocator = 0);
    bdema_Multipool(bsls_BlockGrowth::Strategy        growthStrategy,
                    bslma_Allocator                  *basicAllocator = 0);
    bdema_Multipool(int                               numPools,
...
template <class TYPE>
inline
void bdema_Multipool::deleteObject(const TYPE *object)
{
...
template <class TYPE>
inline
void bdema_Multipool::deleteObjectRaw(const TYPE *object)
{
...
// ACCESSORS
inline
int bdema_Multipool::numPools() const
{
    return d_numPools;
}

inline
int bdema_Multipool::maxPooledBlockSize() const
{
