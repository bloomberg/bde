...
#include <bsl_cstring.h>              // memset(), memcpy(), strlen()
...
// [3] bdema_BufferAllocator(buf, bsz, callback);
// [3] bdema_BufferAllocator(buf, bsz, strategy, callback);
// [3] ~bdema_BufferAllocator();
// [3] void *allocate(sz);
...
typedef bdema_BufferAllocator Obj;
...
// Idiom for calling an allocator and converting 'bsl::bad_alloc' to
...
int doAlloc(void *p)
{
    ...
    args.d_retValue = args.d_alloc->allocate(args.d_size);
    return 0;
}
    ...
    switch (test) { case 0:
      case 6: {
...

        {
            ...
            bdema_BufferAllocator allocator(memory, sizeof memory,
                                            callbackFunc);
...
// The following code uses pipe() and fork(), so only works on Unix.
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            bdema_BufferAllocator a(memory, MEM_SZ, strategy);
            const int allocSz = 8;
            a.allocate(allocSz);
            const int offset =
                          calcOffset(memory,
      ...
      case 3: {
        ...
        //
        // Testing:
        //   bdema_BufferAllocator(buf, bsz, callback);
        //   bdema_BufferAllocator(buf, bsz, strategy, callback);
        ...
        for (int i = 0; i < (int)(sizeof BUF_SZ / sizeof *BUF_SZ); ++i) {
            ...
            for (int j = 0; j < (int)(sizeof ALLOC_SZ / sizeof *ALLOC_SZ);
                ...
                BSLS_TRY {
                    aX = callAllocate(&mX, szM);
                    aY = callAllocate(&mY, szM);
                }
                //BSLS_CATCH (bsls_StdExceptionTranslator::bad_alloc) {
                BSLS_CATCH (const bsl::bad_alloc&) {
                    // Allocation was supposed to fail
                    LOOP2_ASSERT(i, j, 0 != sz && 0 == aM);
                }

                BSLS_TRY {
                    aZ = callAllocate(&mZ, szN);
                }
                BSLS_CATCH (const bsl::bad_alloc&) {
                    // Allocation was supposed to fail
   ...
   {  L_,     60,       N8,       60,       1,       {NA, NA},  {60, 60}   }
      ...
      case 1: {
        ...
        {
            bdema_BufferAllocator a(buffer, BUF_SZ);
            char *freePtr = buffer;
            ...
            for (int i = 0; i < SIZE_LEN; ++i) {
                ...
                char *ret = 0;
                BSLS_TRY {
                    ret = (char*) callAllocate(&a, SIZE[i]);
