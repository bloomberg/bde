...
//..
// 2. The length stored in the 'bdede_Md5' object is the byte length of the
...
static void populateResultBuffer(bdede_Md5::Md5Digest *result,
                                 unsigned int         *state)
{
    ...
    for (int i = 0; i < 4; ++i) {
        resultBytes[    i * 4] =  state[i]        & 0xff;
        resultBytes[1 + i * 4] = (state[i] >>  8) & 0xff;
        resultBytes[2 + i * 4] = (state[i] >> 16) & 0xff;
        resultBytes[3 + i * 4] = (state[i] >> 24) & 0xff;
    }
}
...
bdede_Md5::bdede_Md5(const void *data, int length)
: d_length(0)
{
...
bdede_Md5::bdede_Md5(const bdede_Md5& original)
...
void bdede_Md5::loadDigestAndReset(bdede_Md5::Md5Digest *result)
{
    BSLS_ASSERT(result);
...
// ACCESSORS
void bdede_Md5::loadDigest(bdede_Md5::Md5Digest *result) const
{
    BSLS_ASSERT(result);
...
bsl::ostream& bdede_Md5::print(bsl::ostream& stream) const
{
    bdede_Md5::Md5Digest result;
    loadDigest(&result);
