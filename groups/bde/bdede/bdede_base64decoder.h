// bdede_base64decoder.h                                              -*-C++-*-
#ifndef INCLUDED_BDEDE_BASE64DECODER
#define INCLUDED_BDEDE_BASE64DECODER
...
//
//@CLASSES:
//   bdede_Base64Decoder: automata performing Base64 decoding operations
//
//@SEE_ALSO: 'bdede_base64encoder'
//
...
///Usage Example
///-------------
// The following example shows how to use a 'bdede_Base64Decoder' object to
...
// monitoring functionality of 'bdede_Base64Decoder', the fixed buffer sizes
...
//     };
//..
// We declare a 'bdede_Base64Decoder' object 'converter', which will decode the
...
//..
//     bdede_Base64Decoder converter(false);   // Do not report errors.
//
...
class bdede_Base64Decoder {
    // This class implements a mechanism capable of converting data
    // of arbitrary length from its corresponding Base64 representation.
  ...
  private:
    // NOT IMPLEMENTED
    bdede_Base64Decoder(const bdede_Base64Decoder&);
    bdede_Base64Decoder& operator=(const bdede_Base64Decoder&);
  ...
  public:
    ...
    // CREATORS
    bdede_Base64Decoder(bool unrecognizedIsErrorFlag);
    ...
    int endConvert(OUTPUT_ITERATOR  out,
        ...
        // argument) to the specified 'out' buffer; encode any unprocessed
        // input characters that do not complete a 3-byte sequence.  Optionally
        // specify the 'maxNumOut' limit on the number of bytes to output; if
        // 'maxNumOut' is negative, no limit is imposed.  Load into the
        // (optionally) specified 'numOut' the number of output bytes produced.
        // Return 0 on success, the positive number of bytes *still* retained
        // by this decoder if the 'maxNumOut' limit was reached, and a negative
        // value otherwise.  Any retained bytes are available on a subsequent
        // call to 'endConvert'.  Once this method is called, no additional
        // input may be supplied without an intervening call to 'resetState';
        // once this method returns a zero status, a subsequent call will place
        // this decoder in the error state, and return an error status.
...
// CLASS METHODS
inline
int bdede_Base64Decoder::maxDecodedLength(int inputLength)
{
...
// CREATORS
inline
bdede_Base64Decoder::bdede_Base64Decoder(bool unrecognizedIsErrorFlag)
...
// MANIPULATORS
template <typename OUTPUT_ITERATOR, typename INPUT_ITERATOR>
int bdede_Base64Decoder::convert(OUTPUT_ITERATOR out,
                                 INPUT_ITERATOR  begin,
                                 INPUT_ITERATOR  end)
{
    int dummyNumOut;
...
template <typename OUTPUT_ITERATOR>
int bdede_Base64Decoder::endConvert(OUTPUT_ITERATOR out)
{
    int dummyNumOut;
...
inline
void bdede_Base64Decoder::resetState()
{
    d_state        = BDEDE_INPUT_STATE;
    d_outputLength = 0;
    d_bitsInStack  = 0;
...
// ACCESSORS
inline
bool bdede_Base64Decoder::isAcceptable() const
{
    const int residualBits = ((d_outputLength % 3) * 8 + d_bitsInStack) % 24;
...
inline
bool bdede_Base64Decoder::isDone() const
{
    return !d_bitsInStack && BDEDE_DONE_STATE == d_state;
}

inline
bool bdede_Base64Decoder::isError() const
{
    return BDEDE_ERROR_STATE == d_state;
}

inline
bool bdede_Base64Decoder::isMaximal() const
{
...
inline
bool bdede_Base64Decoder::isInitialState() const
{
...
inline
bool bdede_Base64Decoder::isUnrecognizedAnError() const
{
    return d_ignorable_p == s_ignorableStrict_p;
}

inline
int bdede_Base64Decoder::outputLength() const
{
