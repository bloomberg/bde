    ...
    // 'BDEDE_INVALID_INPUT_CHARACTER' bit in '*retCode'.
{
    ...
    if ((firstByte & RANGE1_MASK) == RANGE1_BITS) {
        ...
        insertUcs2Character(dstBufferPtr,
                            dstCapacityPtr,
                            charsWrittenPtr,
                   (short(firstByte & RANGE1_BYTE1_MASK) << RANGE1_BYTE1_SHIFT)
    ...
    if ((firstByte & RANGE2_MASK) == RANGE2_BITS) {
        ...
        insertUcs2Character(dstBufferPtr,
                            dstCapacityPtr,
                            charsWrittenPtr,
                   (short(firstByte & RANGE2_BYTE1_MASK) << RANGE2_BYTE1_SHIFT)
                 | (short(src[1]    & EXTRA_BYTE_MASK)   << RANGE2_BYTE2_SHIFT)
                 |  short(src[2]    & EXTRA_BYTE_MASK));
...
int bdede_CharConvertUcs2::utf8ToUcs2(unsigned short *dstBuffer,
...
int bdede_CharConvertUcs2::utf8ToUcs2(
...
int bdede_CharConvertUcs2::ucs2ToUtf8(char                 *dstBuffer,
                                      bsl::size_t           dstCapacity,
                                      const unsigned short *srcString,
                                      bsl::size_t          *numCharsWritten,
...
int bdede_CharConvertUcs2::ucs2ToUtf8(bsl::string          *result,
                                      const unsigned short *srcString,
                                      bsl::size_t          *numCharsWritten)
{
    // Theory: We're mainly dealing with 7-bit ASCII, so we're going to try for
