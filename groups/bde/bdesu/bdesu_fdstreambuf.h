// bdesu_fdstreambuf.h                                                -*-C++-*-
#ifndef INCLUDED_BDESU_FDSTREAMBUF
#define INCLUDED_BDESU_FDSTREAMBUF
...
//
//@DESCRIPTION: This component implements a class, 'bdesu_FdStreamBuf', derived
...
// 'bdesu_FdStreamBuf'.  An 'bsl::streambuf' provides public methods for
// reading from and writing to a stream of data, which are implemented in terms
// of protected virtual functions.  A 'bdesu_FdStreamBuf' provides an
...
// or other device, and it can be associated with the 'bdesu_FdStreamBuf' at
// construction, or by calling the 'reset' method.  Note that a 'bsl::stream'
// can be initialized with a 'bdesu_FdStreamBuf', making it possible to
...
// has no effect.
//
// The file descriptor type 'bdesu_FileUtil::FileDescriptor' used
// in this component is, on Unix, an 'int' type returned by 'open', and on
// Windows, a 'HANDLE' type returned by 'CreateFile'.  Ideally, a user would
// open the file and obtain the platform-independent
// 'bdesu_FileUtil::FileDescriptor' by calling 'bdesu_FileUtil::open', which
// will call the appropriate routine for the platform and return a
// 'bdesu_FileUtil::FileDescriptor'.  A value of 'bdesu_FileUtil::INVALID_FD'
...
//..
// Note that the 'bdesu_FdStreamBuf' and 'bdesu_FdStreamBuf_FileHandler'
...
//..
// Next we create a 'bdesu_FdStreamBuf' associated with file descriptor
...
//  assert(bdesu_FileUtil::INVALID_FD != fd);
//..
// Now, we create a 'bdesu_FdStreamBuf' object named 'streamBuffer'
...
namespace BloombergLP {
                ...
                //===========================================
                // helper class bdesu_FdStreamBuf_FileHandler
                //===========================================

class bdesu_FdStreamBuf_FileHandler {
    // This private helper class isolates direct operations on files from the
    // 'bdesu_FdStreamBuf' class; it is a thin wrapper around 'bdesu_FileUtil'.
    // One service this class provides is converting between an in-process '\n'
    ...
    // that file descriptor in the service of a 'bdesu_FdStreamBuf'.

  private:
    // PRIVATE TYPES
    typedef bdesu_FileUtil           FileUtil;  // shorthand for
                                                // class 'bdesu_FileUtil'
  ...
  private:
    ...
    // DATA
    FdType d_fileId;               // file descriptor, which is owned if
  ...
  private:
    // NOT IMPLEMENTED
    bdesu_FdStreamBuf_FileHandler(       const bdesu_FdStreamBuf_FileHandler&);
    bdesu_FdStreamBuf_FileHandler& operator=(
                                         const bdesu_FdStreamBuf_FileHandler&);
  ...
  public:
    ...
    // MANIPULATORS
    int reset(bdesu_FileUtil::FileDescriptor fileDescriptor,
              bool                           writableFlag,
        ...
        // 'bdesu_FileUtil::INVALID_FD' is passed to the specified
    ...
    void release();
        ...
        // 'fileDescriptor' is 'bdesu_FileUtil::INVALID_FD' after this call.
    ...
    int clear();
        ...
        // method succeeds with no effect if 'isOpened' was 'false'.  Note that
        // 'fileDescriptor' is always 'bdesu_FileUtil::INVALID_FD' after this
        // call.
    ...
    bsl::streampos seek(bsl::streamoff offset, bdesu_FileUtil::Whence dir);
        // Set the file position associated with this object according to the
        // specified 'dir' behavior.
        //..
        //   * If 'dir' is 'bdesu_FileUtil::BDESU_SEEK_FROM_BEGINNING', set the
        //     position to the specified 'offset' bytes from the beginning of
        //     the file.
        //   * If 'dir' is 'bdesu_FileUtil::BDESU_SEEK_FROM_CURRENT', advance
        //     the position by the specified 'offset' bytes
        //   * If 'dir' is 'bdesu_FileUtil::BDESU_SEEK_FROM_END', set the
        ...
        // beginning of the file on success; and 'bdesu_FileUtil::INVALID_FD'
    ...
    // ACCESSORS
    bdesu_FileUtil::FileDescriptor fileDescriptor() const;
...
class bdesu_FdStreamBuf : public bsl::streambuf {
    ...
    // 'bdesu_FdStreamBuf::FdStreamBufMode'.

  private:
    // PRIVATE TYPES
    typedef bdesu_FileUtil           FileUtil;  // shorthand for bdesu_FileUtil
    typedef FileUtil::FileDescriptor FdType;    // shorthand for file
                                                // descriptor
  ...
  private:
    ...
    int seekInit();
        // Prepare this 'bdesu_FdStreamBuf' for a subsequent seek operation by
  ...
  protected:
    ...
    virtual bdesu_FdStreamBuf *setbuf(char_type       *buffer,
  ...
  private:
    // NOT IMPLEMENTED
    bdesu_FdStreamBuf(           const bdesu_FdStreamBuf&);
    bdesu_FdStreamBuf& operator=(const bdesu_FdStreamBuf&);

  public:
    // CREATORS
    explicit bdesu_FdStreamBuf(
        ...
        // used.  Note that if 'bdesu_FileUtil::INVALID_FD' is passed to
        // 'fileDescriptor', no file descriptor is to be associated with this
        // object.  Also note that the state of the specified 'fileDescriptor'
    ...
    bdesu_FileUtil::FileDescriptor fileDescriptor() const;
        ...
        // 'bdesu_FileUtil::INVALID_FD' if this object is not currently
        // associated with a file descriptor.
...
// MANIPULATORS
inline
void bdesu_FdStreamBuf_FileHandler::release()
{
    d_willCloseOnResetFlag = false;
    reset(bdesu_FileUtil::INVALID_FD, false);
}

inline
int bdesu_FdStreamBuf_FileHandler::clear()
{
    return reset(bdesu_FileUtil::INVALID_FD, false);
}

inline
void bdesu_FdStreamBuf_FileHandler::setWillCloseOnReset(bool booleanValue)
{
...
inline
bool bdesu_FdStreamBuf_FileHandler::isOpened() const
{
    return d_openedFlag;
}

inline
bool bdesu_FdStreamBuf_FileHandler::isRegularFile() const
{
    return d_regularFileFlag;
}

inline
int bdesu_FdStreamBuf_FileHandler::openMode() const
{
    return (int) d_openModeFlags;
}

inline
bool bdesu_FdStreamBuf_FileHandler::willCloseOnReset() const
{
...
bdesu_FdStreamBuf_FileHandler::fileDescriptor() const
{
    return d_fileId;
}
...
// PRIVATE MANIPULATORS
inline
void bdesu_FdStreamBuf::exitPutbackMode()
{
...
inline
bdesu_FdStreamBuf::pos_type
bdesu_FdStreamBuf::seekReturn(pos_type position)
...
inline
void bdesu_FdStreamBuf::release()
{
...
// ACCESSORS
inline
bdesu_FileUtil::FileDescriptor bdesu_FdStreamBuf::fileDescriptor() const
{
    return d_fileHandler.fileDescriptor();
}

inline
bool bdesu_FdStreamBuf::isOpened() const
{
    return d_fileHandler.isOpened();
}

inline
bool bdesu_FdStreamBuf::willCloseOnReset() const
{
