...
typedef bdesu_FdStreamBuf_FileHandler  ObjFileHandler;
typedef bdesu_FdStreamBuf              Obj;
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:
      case 16: {
        ...
        //
        // Concerns:
        //   Demonstrate reading, writing, and seeking with a bdesu_FdStreamBuf
        ...
        //   a 'bdesu_FdStreamBuf' associated with that file descriptor, and
      ...
      case 15: {
        ...
        // In this case, the 'bdesu_FdStreamBuf' will be used for either input
        ...
        // Now, we create an 'bdesu_FdStreamBuf' type of stream buffer
      ...
      case 14: {
...
#endif
        ASSERT(lengthR == fh.seek(0, FileUtil::BDESU_SEEK_FROM_CURRENT));
        ...
        ASSERT(lengthR + 3 == fh.seek(3, FileUtil::BDESU_SEEK_FROM_CURRENT));
        ...
        {
            int cur = fh.seek(0,FileUtil::BDESU_SEEK_FROM_CURRENT);
      ...
      case 12: {
        ...
        char buf[1000];
        {
            ...
            int sts = sb.pubseekoff(0,  bsl::ios_base::cur);
      ...
      case 7: {
...

        {
            ...
            fh.seek(0, FileUtil::BDESU_SEEK_FROM_END);
      ...
      case 6: {
        ...
        {
            FdType fd2 = FileUtil::open(fnBuf2, true, false);
            ASSERT(-1 != (int) fd2);
      ...
      case 5: {
        // --------------------------------------------------------------------
        // bdesu_FdStreamBuf INTERMIXED READ AND WRITE, JUGGLING OPENS
        //
        // Concerns:
        ...
        if (verbose) cout << "bdesu_FdStreamBuf C'TOR TEST\n"
                             "============================\n";
        ...
        {
            ASSERT(fd == FileUtil::open(fnBuf, false, true));
            ASSERT(-1 != (int) fd);
      ...
      case 4: {
        ...
        if (verbose) cout << "bdesu_FdStreamBuf PUBSETBUF TEST\n"
                             "================================\n";
      ...
      case 3: {
        // --------------------------------------------------------------------
        // bdesu_FdStreamBuf INTERMIXED READ AND WRITE
        //
        // Concerns:
        ...
        if (verbose) cout << "bdesu_FdStreamBuf READ-WRITE TEST\n"
                             "=================================\n";
        ...
        {
            FdType fd = FileUtil::open(fnBuf, true, true);
            ASSERT(-1 != (int) fd);
      ...
      case 2: {
        // --------------------------------------------------------------------
        // bdesu_FdStreamBuf BREATHING TEST
        //
        // Concerns:
        //   Basic functionality.
        //
        ...
        if (verbose) cout << "bdesu_FdStreamBuf BREATHING TEST\n"
                             "================================\n";
      ...
      case 1: {
        // --------------------------------------------------------------------
        // bdesu_FdStreamBuf_FileHandler BREATHING TEST
        //
        // Concerns:
        ...
        if (verbose) cout << "bdesu_FdStreamBuf_FileHandler BREATHING TEST\n"
                             "============================================\n";
...

        {
            ...
            int sts = sbFileHandler.read(inBuf1, len1);
            ASSERT(len1 == sts);
            inBuf1[len1] = 0;
            ...
            sts = sbFileHandler.seek(-20, FileUtil::BDESU_SEEK_FROM_CURRENT);
            ...
            sts = sbFileHandler.seek(len1,FileUtil::BDESU_SEEK_FROM_BEGINNING);
            ASSERT(len1 == sts);
            ...
            sts = sbFileHandler.seek(-20,   FileUtil::BDESU_SEEK_FROM_CURRENT);
            ASSERT(len1 + len2 - 20 == sts);
            sts = sbFileHandler.seek(-len2, FileUtil::BDESU_SEEK_FROM_END);
            ASSERT(len1 == sts);
...

        {
            ...
            ASSERT(0 == sbFileHandler.write(line1, len1));
            ASSERT(dLen1 == sbFileHandler.seek(0,
                                               FileUtil::BDESU_SEEK_FROM_END));
            ASSERT(0 == sbFileHandler.write(line2, len2));
            ASSERT(dLen1 + dLen2 ==
                        sbFileHandler.seek(0,
                                           FileUtil::BDESU_SEEK_FROM_CURRENT));
      ...
      case -1: {
        // --------------------------------------------------------------------
        // bdesu_FdStreamBuf HANDLING 5 GIGABYTE FILE
        //
        // Concerns:
        //
        // Plan:
        ...
        FileUtil::seek(fd, 0, FileUtil::BDESU_SEEK_FROM_BEGINNING);
