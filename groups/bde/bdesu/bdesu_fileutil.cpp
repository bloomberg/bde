                        ...
                        int                      mode)
{
    ...
    if (mode == bdesu_MemoryUtil::BDESU_ACCESS_NONE) return -1;
    mode &= 7;
...
int bdesu_FileUtil::unmap(void *addr,
                          int   len)
{
...
int bdesu_FileUtil::move(const char *oldName,
                         const char *newName)
{
    if (exists(newName)) {
        DeleteFile(newName);
...
int bdesu_FileUtil::sync(char *addr,
                         int   numBytes,
                         bool  sync)
{
