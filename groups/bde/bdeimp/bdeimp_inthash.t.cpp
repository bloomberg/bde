...
// (pure procedure) that computes a hash value for 'int's.  The general plan is
// that the method is tested against a set of tabulated test vectors.
//-----------------------------------------------------------------------------
// [ 1] static int hash(int value, int size);  CONCERN: value
// [ 2] static int hash(int value, int size);  CONCERN: distribution
// [-1] static int hash(int value, int size);  CONCERN: timing
//-----------------------------------------------------------------------------
      ...
      case 2: {
                          ...
                          << "distribution." << endl;
        {
            { // test distribution for shifted bits
                ...
                for (int k = 0; k <= 23; ++k) { // 32 - 9 bits for size of 257
                    for (i = 0; i < size; ++i) {
                        ++hit[bdeimp_IntHash::hash(i << k, size)];
                    }
                    for (i = 0; i < size; ++i) {
            ...
            { // test distribution for varying sizes
                ...
                for (int m = 1; m <= 4; m++) {
                    for (int si = 0; si < NUM_DATA; ++si) {
                        ...
                        for (i = 0; i < m*size; ++i) {
                            ++hit[bdeimp_IntHash::hash(i, size)];
                        }
                        for (i = 0; i < size; ++i) {
                          ...
                          << "return value." << endl;
        {
            ...
            for (int ti = 0; ti < NUM_DATA ; ++ti) {
                ...
                if (veryVerbose) {
                    ...
                    P_(SIZE);
                    P(EXP);
                    P(bdeimp_IntHash::hash(VALUE, SIZE));
                    cout << endl;
                }
                LOOP_ASSERT(LINE, EXP == bdeimp_IntHash::hash(VALUE, SIZE));
            }
        }
      } break;
      case -1: {
        ...
        //
        // Testing:
        //   static int hash(int value, int size);  CONCERN: timing
        // --------------------------------------------------------------------
        ...
        unsigned int value = 0;
            ...
            for (int i = 0; i < ITERATIONS; ++i) {
                value += bdeimp_IntHash::hash(i, LENGTH);
            }
            timer.stop();
