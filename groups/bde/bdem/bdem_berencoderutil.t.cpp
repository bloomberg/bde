...
typedef bdem_BerEncoderUtil Obj;
...
template <class STREAM>
inline
STREAM& MySequenceWithNullable::bdexStreamOut(STREAM& stream,
                                              int version) const
{
    switch (version) {
      case 1: {
...
template <class STREAM>
inline
STREAM& MySequenceWithNillable::bdexStreamOut(STREAM& stream,
                                              int version) const
{
    switch (version) {
      case 1: {
...
template <class STREAM>
inline
STREAM& MySequenceWithAnonymousChoiceChoice::bdexStreamIn(STREAM& stream,
                                                          int version)
{
    if (stream) {
        switch (version) {  // Switch on the schema version (starting with 1).
...
template <class STREAM>
inline
STREAM& MySequenceWithAnonymousChoiceChoice::bdexStreamOut(STREAM& stream,
                                                           int version) const
{
    switch (version) {
      case 1: {
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      ...
      case 4: {
        ...
        if (verbose) bsl::cout << "\nEncoding customized string." << bsl::endl;
        {
            ...
            ASSERT(0 == Obj::encode(&osb1, value));
                               ...
                               << bsl::endl;
        {
            ...
            ASSERT(0 == Obj::encode(&osb2, value));
      ...
      case 2: {
...

        {
            ...
            for (int di = 0; di < NUM_DATA; ++di) {
                ...
                LOOP_ASSERT(LINE, 0 == Obj::encode(&osb, VALUE));
            ...
            if (verbose) { cout << "\nTesting for special values" << endl; }
            {
                ...
                for (int di = 0; di < NUM_DATA; ++di) {
                    ...
                    ASSERT(RESULT == Obj::encode(&osb, value));
      ...
      case -1: {
        ...
        for (int i = 0; i < reps; ++i) {
            sb.pubseekpos(0);
            Obj::encode(&sb, request);
        }
        stopwatch.stop();

        ASSERT(minOutputSize <= sb.length());
        ASSERT(sb.length() <= MAX_BUF_SIZE);
        double elapsed = stopwatch.elapsedTime();
        ASSERT(elapsed > 0);

        double berEncodeSpeed = reps / elapsed;
        bsl::cout << "bdem_BerEncoderUtil: " << elapsed << " seconds, "
                  << (reps / elapsed) << " reps/sec, "
                  << sb.length() << " bytes" << bsl::endl;

        bdex_ByteOutStreamFormatter bdexStream(&sb);
        stopwatch.reset();
        stopwatch.start();
        for (int i = 0; i < reps; ++i) {
            sb.pubseekpos(0);
            bdex_OutStreamFunctions::streamOut(bdexStream, request, 1);
        }
        stopwatch.stop();
        ...
        bsl::cout << "BerEncoderUtil is " << (slowDown * 100.0)
