...
//
// Note further that the 'bdem_IdxOffsetMap' is a parameterized type therefore
...
// we provide a helper function to set the 'bdem_IdxOffsetMap' total offset
// (i.e., the cursor position).
//
//-----------------------------------------------------------------------------
// [ 2] bdem_IdxOffsetMap(bdema_Allocator *ba = 0);
// [16] bdem_IdxOffsetMap(const InitialCapacity& ne, *ba = 0);
// [11] bdem_IdxOffsetMap(const my_ElemTypeArray& sa, *ba = 0);
// [ 7] bdem_IdxOffsetMap(const bdem_IdxOffsetMap& original, *ba = 0);
// [ 2] ~bdem_IdxOffsetMap();
...
//-----------------------------------------------------------------------------
// [ 6] int operator==(const bdem_IdxOffsetMap& lhs,
//                     const bdem_IdxOffsetMap& rhs);
// [ 6] int operator!=(const bdem_IdxOffsetMap& lhs,
//                     const bdem_IdxOffsetMap& rhs);
// [ 5] ostream& operator<<(ostream& os, const bdem_IdxOffsetMap& rhs);
//-----------------------------------------------------------------------------
// [ 1] BREATHING TEST
// [19] USAGE EXAMPLE
// [ 2] BOOTSTRAP: void insert(int di, bdem_ElemType::Type item);  // !aliasing
// [ 8] std::vector<double> g(const char *spec);
...
// [ 9] bdem_IdxOffsetMap& operator=(const bdem_IdxOffsetMap& rhs);
//=============================================================================
...
const Element VALUES[] = {
    typesLookupTable[bdem_ElemType::CHAR],
    typesLookupTable[bdem_ElemType::SHORT],
    typesLookupTable[bdem_ElemType::INT],
    typesLookupTable[bdem_ElemType::INT64],
    typesLookupTable[bdem_ElemType::FLOAT],
}; // avoid DEFAULT_VALUE
...
static const char bdemType[] = "ABCDEFGHIJKLMNOPQRSTUV";
...
const TYPE *vectorData(const std::vector<TYPE>& v)
    // Return a pointer to the first element of 'v' or null if 'v' is empty.
{
    return v.empty() ? 0 : &v.front();
}

typedef std::vector<bdem_ElemType::Type> my_ElemTypeArray;

void getElemTypeArray(my_ElemTypeArray         *retArray,
                      const bdem_IdxOffsetMap&  map)
{
    int myLength = map.length();

    retArray->resize(myLength);
    for (int i = 0; i < myLength; ++i) {
        (*retArray)[i] = (bdem_ElemType::Type)map[i].d_elemAttr->d_elemEnum;
    }
}

void getElemAttrArray(std::vector<const bdem_Descriptor*> *retArray,
                      const bdem_IdxOffsetMap&             map)
{
    int  myLength = map.length();
    retArray->resize(myLength);
    for (int i = 0; i < myLength; ++i) {
        (*retArray)[i] = map[i].d_elemAttr;
    }
}


void getOffsetArray(std::vector<int> *retArray, const bdem_IdxOffsetMap& map)
{
    int  myLength = map.length();
    retArray->resize(myLength);
    for (int i = 0; i < myLength; ++i) {
        (*retArray)[i] = map[i].d_offset;
    }
}
    ...
    // The behavior is undefined unless 0 <= size.
{
    ...
    for (int i = 0; i < size; ++i) {
        object->append(typesLookupTable[bdem_ElemType::CHAR]);
    }
...
int operator==(const bdem_IdxOffsetMap& lhs, const bdem_IdxOffsetMap& rhs)
{
    std::vector<int> lhsOffsets;
    my_ElemTypeArray lhsTypes;
    getElemTypeArray(&lhsTypes,lhs);
    getOffsetArray(&lhsOffsets,lhs);

    std::vector<int> rhsOffsets;
...
// inserted into the 'bdem_IdxOFfsetMap' object.  A tilde ('~') indicates that
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      case 19: {
        ...
        m.append(typesLookupTable[bdem_ElemType::DOUBLE]);
        ASSERT( 1                    == m.length());
        ASSERT(bdem_ElemType::DOUBLE == m[0].d_elemAttr->d_elemEnum);
        ASSERT( 0                    == m[0].d_offset);
        ASSERT( 8                    == m.totalOffset());

        m.append(typesLookupTable[bdem_ElemType::INT64]);
        ASSERT( 2                   == m.length());
        ASSERT(bdem_ElemType::INT64 == m[1].d_elemAttr->d_elemEnum);
        ASSERT( 8                   == m[1].d_offset);
        ASSERT(16                   == m.totalOffset());

        m.append(typesLookupTable[bdem_ElemType::DOUBLE_ARRAY]);
        ASSERT( 3                          == m.length());
        ASSERT(bdem_ElemType::DOUBLE_ARRAY == m[2].d_elemAttr->d_elemEnum);
        ASSERT(16                          == m[2].d_offset);
        ASSERT(16 + PTR_SIZE * 6           == m.totalOffset());

        m.append(typesLookupTable[bdem_ElemType::INT64_ARRAY]);
        ASSERT( 4                         == m.length());
        ASSERT(bdem_ElemType::INT64_ARRAY == m[3].d_elemAttr->d_elemEnum);
        ASSERT(16 + PTR_SIZE * 6          == m[3].d_offset);
        ASSERT(16 + PTR_SIZE * 12         == m.totalOffset());
        ...
        const char *const EXPECTED1 =
            PTR_SIZE == 4 ? "Types: [ DOUBLE INT64 DOUBLE_ARRAY INT64_ARRAY ] "
                            "Offsets: [ 0 8 16 40 ] TotalOffset: 64"
                          : "Types: [ DOUBLE INT64 DOUBLE_ARRAY INT64_ARRAY ] "
                            "Offsets: [ 0 8 16 64 ] TotalOffset: 112";

        if (veryVerbose) {
            cout << "EXPECTED: " << EXPECTED1 << endl;
            cout << "  ACTUAL: " << buf       << endl;
        }
        ASSERT(0 == strcmp(EXPECTED1, buf));

        m.remove(1);
        ASSERT( 3                          == m.length());
        ASSERT(bdem_ElemType::DOUBLE       == m[0].d_elemAttr->d_elemEnum);
        ASSERT(bdem_ElemType::DOUBLE_ARRAY == m[1].d_elemAttr->d_elemEnum);
        ASSERT(bdem_ElemType::INT64_ARRAY  == m[2].d_elemAttr->d_elemEnum);
        ASSERT(16 + PTR_SIZE * 12          == m.totalOffset());
        ...
        const char *const EXPECTED2 =
                  PTR_SIZE == 4 ? "Types: [ DOUBLE DOUBLE_ARRAY INT64_ARRAY ] "
                                  "Offsets: [ 0 16 40 ] TotalOffset: 64"
                                : "Types: [ DOUBLE DOUBLE_ARRAY INT64_ARRAY ] "
                                  "Offsets: [ 0 16 64 ] TotalOffset: 112";

        if (veryVerbose) {
            cout << "EXPECTED: " << EXPECTED2 << endl;
            cout << "  ACTUAL: " << buf       << endl;
        }
        ASSERT(0 == strcmp(EXPECTED2, buf));
        ...
        ASSERT(bdem_ElemType::DOUBLE_ARRAY == m[1].d_elemAttr->d_elemEnum);
        ASSERT(bdem_ElemType::INT64_ARRAY  == m[2].d_elemAttr->d_elemEnum);
        ASSERT( 8 + PTR_SIZE * 12          == m.totalOffset());
        ...
        const char *const EXPECTED3 =
                  PTR_SIZE == 4 ? "Types: [ DOUBLE DOUBLE_ARRAY INT64_ARRAY ] "
                                  "Offsets: [ 0 32 8 ] TotalOffset: 56"
                                : "Types: [ DOUBLE DOUBLE_ARRAY INT64_ARRAY ] "
                                  "Offsets: [ 96 48 0 ] TotalOffset: 104";

        if (veryVerbose) {
            cout << "EXPECTED: " << EXPECTED3 << endl;
            cout << "  ACTUAL: " << buf       << endl;
        }
// TBD why failing on IBM and Linux?
#if !defined(BDES_PLATFORM__CMP_AIX) && !defined(BDES_PLATFORM__OS_LINUX)
        LOOP2_ASSERT(EXPECTED3, buf, 0 == strcmp(EXPECTED3, buf));
#endif
      } break;
      case 18: {
        // --------------------------------------------------------------------
...
//BUG this component not yet platform-independent
#ifdef BDES_PLATFORMUTIL__IS_BIG_ENDIAN
        ...
        for (int ti = 0; ti < NUM_DATA; ti++) {
            ...
            Obj x; const Obj &X = x; Obj y; Obj &Y = y;
            gg(&x, SPEC);
            ...
            for (int idx = 0; idx < (int)strlen(SPEC); idx++) {
                ...
                if (veryVerbose) {
                    cout << "\tActual offset: "   << Y[idx].d_offset << endl
                         << "\tExpected offset: " << OFFSET[idx]      << endl
                         << "\tPosition: "        << idx              << endl;
                }
                LOOP_ASSERT(LINE, Y[idx].d_offset == OFFSET[idx]);
            }
            ...
            z.append(typesLookupTable[bdem_ElemType::CHAR]);
            getElemTypeArray(&eta,x);
            ...
            for (int idx = 0; idx < (int)strlen(SPEC); idx++) {
                ...
                // should have a matching expected offset.
                if (veryVerbose) {
                    cout << "\tActual offset: "   << Z[idx].d_offset << endl
                         << "\tExpected offset: " << OFFSET[idx]      << endl
                         << "\tPosition: "        << idx              << endl;
                }
                LOOP_ASSERT(LINE, Z[idx].d_offset == OFFSET[idx]);
            }
            // Assert we have the correct expected offset.
...
//BUG this component not yet platform-independent
        ...
        for (int ti = 0; ti < NUM_DATA; ti++) {
            ...
            for (int idx = 0; idx < (int)strlen(SPEC); idx++) {
                ...
                // should have a matching expected offset.
                if (veryVerbose) {
                    cout << "\tActual offset:   " << X[idx].d_offset << endl
                         << "\tExpected offset: " << OFFSET[idx]      << endl
                         << "\tPosition:        " << idx              << endl;
                }
                LOOP_ASSERT(LINE, X[idx].d_offset == OFFSET[idx]);
            }
            // Assert we have the correct expected offset.
      ...
      case 16: {
        ...
        //
        // Testing:
        //   bdem_IdxOffsetMap(const InitialCapacity& ne, *ba = 0);
        //
        ...
        if (verbose) cout <<
            "\nTesting 'bdem_IdxOffsetMap(capacity, ba)' Constructor" << endl;
...
#undef NL
      ...
      case 13: {
        ...
        //
        //   Note that the 'replace' method in the idxoffsetmap allocates
            ...
            "\nTesting replace(di, sa, si, ne) et. al. (no aliasing)" << endl;
        {
            ...
            for (int ti = 0; ti < NUM_DATA ; ++ti) {
                ...
                if (1 == NE) {
                  BEGIN_BDEMA_EXCEPTION_TEST {
                    Obj x(DD, &testAllocator);  const Obj &X = x;
                    {
                        ...
                        x.replace(DI,
                               typesLookupTable[s[SI].d_elemAttr->d_elemEnum]);
                                                             // src non-'const'
                        if (veryVerbose) {
                            cout << "\t\t\t AFTER: "; P(X);
                        }
                        if (EE != X) {
                            ...
                            LOOP_ASSERT(LINE, X[DI].d_elemAttr->d_elemEnum ==
                                                 S[SI].d_elemAttr->d_elemEnum);
                            // Check to see we have the expected offset.
                            const int align =
                                       typesLookupTable[S[SI].d_elemAttr->
                            ...
                            LOOP_ASSERT(LINE, X[DI].d_offset == new_offset);
                ...
                if (1 == NE) {
                    ...
                    int old_offset = x.totalOffset();
                    x.replace(DI,
                              typesLookupTable[SS[SI].d_elemAttr->d_elemEnum]);
                    if (veryVerbose) { cout << "\t\t\t AFTER: "; P(X); }
                    if (EE != x) {
                        // Check to see we have the expected type.
                        LOOP_ASSERT(LINE, x[DI].d_elemAttr->d_elemEnum ==
                                                SS[SI].d_elemAttr->d_elemEnum);
                        // Check to see we have the expected offset.
                        const int align =
                                      typesLookupTable[SS[SI].d_elemAttr->
                                                      d_elemEnum]->d_alignment;
                        const int size =
                                      typesLookupTable[SS[SI].d_elemAttr->
                                                           d_elemEnum]->d_size;
            ...
            for (int ti = 0; ti < NUM_DATA ; ++ti) {
                ...
                if (1 == NE) {
                  BEGIN_BDEMA_EXCEPTION_TEST {
                    ...
                    if (veryVerbose) { cout << "\t\t\tBEFORE: "; P(X); }
                    x.replace(DI,
                               typesLookupTable[X[SI].d_elemAttr->d_elemEnum]);
                    if (veryVerbose) { cout << "\t\t\t AFTER: "; P(X); }
                    if (EE != x) {
                        // Check to see we have the expected type.
                        LOOP_ASSERT(LINE, x[DI].d_elemAttr->d_elemEnum ==
                                                 X[SI].d_elemAttr->d_elemEnum);
                        // Check to see we have the expected offset.
                        const int align =
                                       typesLookupTable[X[SI].d_elemAttr->
                                                      d_elemEnum]->d_alignment;
                        const int size =
                                       typesLookupTable[X[SI].d_elemAttr->
                                                           d_elemEnum]->d_size;
      ...
      case 12: {
        // --------------------------------------------------------------------
        // TESTING APPEND, INSERT, REMOVE, UPDATETOTALOffSET METHODS
        // Concerns:
        ...
        //    void append(const bdem_IdxOffsetmap<T>& srcMap, int si, int ne);
        //    void append(const bdem_IdxOffsetmap<T>& srcMap);
        //
        ...
        if (verbose) {
            cout << endl
                 << "Testing 'append', 'insert', 'remove' and "
                    "'updateTotalOffset'" << endl
                 << "========================================="
                    "===================" << endl;
        }
        ...
        for (int ti = 0; ti < NUM_DATA ; ++ti) {
              ...
              BEGIN_BDEMA_EXCEPTION_TEST {
                Obj x(DD, &testAllocator);  const Obj &X = x;
                {
                    ...
                    for (int idx = 0; idx < NE; idx++) {
                        const int ALIGN =
                                   typesLookupTable[s[SI+idx].d_elemAttr->
                                                      d_elemEnum]->d_alignment;
                        const int OFFSET = (off + ALIGN - 1) & ~(ALIGN - 1);
                        LOOP_ASSERT(LINE, OFFSET == x[DI+idx].d_offset);
                        off = OFFSET +
                                   typesLookupTable[s[SI+idx].d_elemAttr->
                                                           d_elemEnum]->d_size;
                    }
            ...
            if ((int) strlen(S_SPEC) == NE) {
              BEGIN_BDEMA_EXCEPTION_TEST {
                Obj x(DD, &testAllocator);  const Obj &X = x;
                {
                    ...
                    int off = x.totalOffset();
                    getElemTypeArray(&eta,s);
                    x.insert(DI, vectorData(eta), eta.size(), typesLookupTable);
                                                          // source non-'const'
                    if (veryVerbose) { cout << "\t\t\t AFTER: "; P(X); }
                    for (int idx = 0; idx < s.length(); idx++) {
                        const int ALIGN =
                                      typesLookupTable[s[idx].d_elemAttr->
                                                      d_elemEnum]->d_alignment;
                        const int OFFSET = (off + ALIGN - 1) & ~(ALIGN - 1);
                        LOOP_ASSERT(LINE, OFFSET == x[DI + idx].d_offset);
                        off = OFFSET +
                ...
                Obj x(DD, &testAllocator);  const Obj &X = x;
                {
                    ...
                    x.insert(DI,
                             typesLookupTable[s[SI].d_elemAttr->d_elemEnum]);
                                                          // source non-'const'
                    if (veryVerbose) { cout << "\t\t\t AFTER: "; P(X); }
                    LOOP_ASSERT(LINE, x[DI].d_offset == OFFSET);
                    LOOP_ASSERT(LINE, SS == S);     // source unchanged ?
                }
                                                    // source is out of scope
                getElemTypeArray(&eta,EE);
            ...
            if ((int) strlen(D_SPEC) == DI && 1 == NE) {
              BEGIN_BDEMA_EXCEPTION_TEST {
                Obj x(DD, &testAllocator);  const Obj &X = x;
                {
                    Obj s(SS, &testAllocator);  const Obj &S = s;
                    if (veryVerbose) { cout << "\t\t\tBEFORE: "; P(X); }
                    x.append(typesLookupTable[s[SI].d_elemAttr->d_elemEnum]);
                                                          // source non-'const'
                    if (veryVerbose) { cout << "\t\t\t AFTER: "; P(X); }
                    LOOP_ASSERT(LINE, EE == X);
                    LOOP_ASSERT(LINE, SS == S);     // source unchanged?
              ...
              BEGIN_BDEMA_EXCEPTION_TEST {        // Note specs are switched.
                ...
                if (veryVerbose) { cout << "\t\t\t AFTER: "; P(X); }
                {
                    ...
                    for (idx = 0; idx < DI; idx++) {
                        LOOP_ASSERT(LINE,
                                        EE[idx].d_offset == X[idx].d_offset);
                    }
                    for (; idx < X.length(); idx++) {
                        LOOP_ASSERT(LINE,
                                   EE[idx + NE].d_offset == X[idx].d_offset);
                    }
                }

                // NOTE - this is not a thorough test BUG!
                x.updateTotalOffset();
#if 0
                LOOP_ASSERT(LINE, DD.totalOffset() == X.totalOffset() ||
                           (DD.totalOffset() + NE) == X.totalOffset());
#endif
              } END_BDEMA_EXCEPTION_TEST
            }
            ...
            if (1 == NE) {
              BEGIN_BDEMA_EXCEPTION_TEST {  // Note specs are switched
                ...
                int idx;
                for (idx = 0; idx < DI; idx++) {
                    LOOP_ASSERT(LINE, EE[idx].d_offset == X[idx].d_offset);
                }
                for (; idx < X.length(); idx++) {
                    LOOP_ASSERT(LINE,
                                   EE[idx + NE].d_offset == X[idx].d_offset);
...
#if 0
                LOOP_ASSERT(LINE, DD.totalOffset() == X.totalOffset() ||
                            (DD.totalOffset() + 1) == X.totalOffset());
#endif
              } END_BDEMA_EXCEPTION_TEST
            }
        }
      ...
      case 11: {
        ...
        //      (using a 'bdema_BufferAllocator') and never destroyed.  and use
        //      direct accessors to verify
        //    - length
        //    - element value at each index position { 0 .. length - 1 } using
        //      getType().
        //
        // Testing:
        //   bdem_IdxOffsetMap(const my_ElemTypeArray& sa, *ba = 0);
        // --------------------------------------------------------------------
                          ...
                          << endl;
        {
            ...
            if (verbose) cout << "\tWithout passing in an allocator." << endl;
            {
                for (int ne = 0; ne <= NUM_ELEMENTS; ++ne) {
                    ...
                    for (int i = 0; i < ne; ++i) {
                        LOOP2_ASSERT(ne, i,
                                     DB[i] == X[i].d_elemAttr->d_elemEnum);
                    }
                }
            }
      ...
      case 10: {
        ...
        //
        // Testing:
        //   static int typeAlignment(bdem_ElemType::Type item);
        //   static int typeSize(bdem_ElemType::Type item);
        // --------------------------------------------------------------------
...

        {
            for (int ti = 0; ti < NUM_TYPES; ++ti) {
              switch (bdemType[ti]) {  // AB...UV
                case 'A': {
                  ASSERT(bdes_AlignmentOf<char>::VALUE ==
                         typesLookupTable[bdem_ElemType::CHAR]->d_alignment);
                  ASSERT(sizeof(char) ==
                         typesLookupTable[bdem_ElemType::CHAR]->d_size);
                  if (veryVerbose) {
                      P(typesLookupTable[bdem_ElemType::CHAR]->d_alignment);
                                       T_(); P(bdes_AlignmentOf<char>::VALUE);
                      P(typesLookupTable[bdem_ElemType::CHAR]->d_size);
                                       T_(); P(sizeof(char));
                  }
                } break;
                case 'B': {
                  ASSERT(bdes_AlignmentOf<short>::VALUE ==
                         typesLookupTable[bdem_ElemType::SHORT]->d_alignment);
                  ASSERT(sizeof(short) ==
                         typesLookupTable[bdem_ElemType::SHORT]->d_size);
                  if (veryVerbose) {
                      P(typesLookupTable[bdem_ElemType::SHORT]->d_alignment);
                                      T_(); P(bdes_AlignmentOf<short>::VALUE);
                      P(typesLookupTable[bdem_ElemType::SHORT]->d_size);
                                      T_(); P(sizeof(short));
                  }
                } break;
                case 'C': {
                  ASSERT(bdes_AlignmentOf<int>::VALUE ==
                         typesLookupTable[bdem_ElemType::INT]->d_alignment);
                  ASSERT(sizeof(int) ==
                         typesLookupTable[bdem_ElemType::INT]->d_size);
                  if (veryVerbose) {
                      P(typesLookupTable[bdem_ElemType::INT]->d_alignment);
                                      T_(); P(bdes_AlignmentOf<int>::VALUE);
                      P(typesLookupTable[bdem_ElemType::INT]->d_size);
                                      T_(); P(sizeof(int));
                  }
                } break;
                case 'D': {
                  ASSERT(bdes_AlignmentOf<Int64>::VALUE ==
                         typesLookupTable[bdem_ElemType::INT64]->d_alignment);
                  ASSERT(sizeof(Int64) ==
                         typesLookupTable[bdem_ElemType::INT64]->d_size);
                  if (veryVerbose) {
                      P(typesLookupTable[bdem_ElemType::INT64]->d_alignment);
                                      T_(); P(bdes_AlignmentOf<Int64>::VALUE);
                      P(typesLookupTable[bdem_ElemType::INT64]->d_size);
                                      T_(); P(sizeof(Int64));
                  }
                } break;
                case 'E': {
                  ASSERT(bdes_AlignmentOf<float>::VALUE ==
                         typesLookupTable[bdem_ElemType::FLOAT]->d_alignment);
                  ASSERT(sizeof(float) ==
                         typesLookupTable[bdem_ElemType::FLOAT]->d_size);
                  if (veryVerbose) {
                      P(typesLookupTable[bdem_ElemType::FLOAT]->d_alignment);
                                      T_(); P(bdes_AlignmentOf<float>::VALUE);
                      P(typesLookupTable[bdem_ElemType::FLOAT]->d_size);
                                      T_(); P(sizeof(float));
                  }
                } break;
                case 'F': {
                  ASSERT(bdes_AlignmentOf<double>::VALUE ==
                         typesLookupTable[bdem_ElemType::DOUBLE]->d_alignment);
                  ASSERT(sizeof(double) ==
                         typesLookupTable[bdem_ElemType::DOUBLE]->d_size);
                  if (veryVerbose) {
                      P(typesLookupTable[bdem_ElemType::DOUBLE]->d_alignment);
                                      T_(); P(bdes_AlignmentOf<double>::VALUE);
                      P(typesLookupTable[bdem_ElemType::DOUBLE]->d_size);
                                      T_(); P(sizeof(double));
                  }
                } break;
                case 'G': {
                  ASSERT(bdes_AlignmentOf<std::string>::VALUE ==
                         typesLookupTable[bdem_ElemType::STRING]->d_alignment);
                  ASSERT(sizeof(std::string) ==
                         typesLookupTable[bdem_ElemType::STRING]->d_size);
                  if (veryVerbose) {
                      P(typesLookupTable[bdem_ElemType::STRING]->d_alignment);
                                 T_(); P(bdes_AlignmentOf<std::string>::VALUE);
                      P(typesLookupTable[bdem_ElemType::STRING]->d_size);
                                 T_(); P(sizeof(std::string));
                  }
                } break;
                case 'H': {
                  ASSERT(bdes_AlignmentOf<bdet_Datetime>::VALUE ==
                       typesLookupTable[bdem_ElemType::DATETIME]->d_alignment);
                  ASSERT(sizeof(bdet_Datetime) ==
                         typesLookupTable[bdem_ElemType::DATETIME]->d_size);
                  if (veryVerbose) {
                     P(typesLookupTable[bdem_ElemType::DATETIME]->d_alignment);
                               T_(); P(bdes_AlignmentOf<bdet_Datetime>::VALUE);
                      P(typesLookupTable[bdem_ElemType::DATETIME]->d_size);
                               T_(); P(sizeof(bdet_Datetime));
                  }
                } break;
                case 'I': {
                  ASSERT(bdes_AlignmentOf<bdet_Date>::VALUE ==
                         typesLookupTable[bdem_ElemType::DATE]->d_alignment);
                  ASSERT(sizeof(bdet_Date) ==
                         typesLookupTable[bdem_ElemType::DATE]->d_size);
                  if (veryVerbose) {
                      P(typesLookupTable[bdem_ElemType::DATE]->d_alignment);
                                 T_(); P(bdes_AlignmentOf<bdet_Date>::VALUE);
                      P(typesLookupTable[bdem_ElemType::DATE]->d_size);
                                 T_(); P(sizeof(bdet_Date));
                  }
                } break;
                case 'J': {
                  ASSERT(bdes_AlignmentOf<bdet_Time>::VALUE ==
                         typesLookupTable[bdem_ElemType::TIME]->d_alignment);
                  ASSERT(sizeof(bdet_Time) ==
                         typesLookupTable[bdem_ElemType::TIME]->d_size);
                  if (veryVerbose) {
                      P(typesLookupTable[bdem_ElemType::TIME]->d_alignment);
                                 T_(); P(bdes_AlignmentOf<bdet_Time>::VALUE);
                      P(typesLookupTable[bdem_ElemType::TIME]->d_size);
                                 T_(); P(sizeof(bdet_Time));
                  }
                } break;
                case 'K': {
                  ASSERT(bdes_AlignmentOf<std::vector<char> >::VALUE ==
                     typesLookupTable[bdem_ElemType::CHAR_ARRAY]->d_alignment);
                  ASSERT(sizeof(std::vector<char>) ==
                         typesLookupTable[bdem_ElemType::CHAR_ARRAY]->d_size);
                  if (veryVerbose) {
                   P(typesLookupTable[bdem_ElemType::CHAR_ARRAY]->d_alignment);
                          T_(); P(bdes_AlignmentOf<std::vector<char> >::VALUE);
                      P(typesLookupTable[bdem_ElemType::CHAR_ARRAY]->d_size);
                            T_(); P(sizeof(std::vector<char>));
                  }
                } break;
                case 'L': {
                  ASSERT(bdes_AlignmentOf<std::vector<short> >::VALUE ==
                    typesLookupTable[bdem_ElemType::SHORT_ARRAY]->d_alignment);
                  ASSERT(sizeof(std::vector<short>) ==
                         typesLookupTable[bdem_ElemType::SHORT_ARRAY]->d_size);
                  if (veryVerbose) {
                  P(typesLookupTable[bdem_ElemType::SHORT_ARRAY]->d_alignment);
                         T_(); P(bdes_AlignmentOf<std::vector<short> >::VALUE);
                      P(typesLookupTable[bdem_ElemType::SHORT_ARRAY]->d_size);
                         T_(); P(sizeof(std::vector<short>));
                  }
                } break;
                case 'M': {
                  ASSERT(bdes_AlignmentOf<std::vector<int> >::VALUE ==
                      typesLookupTable[bdem_ElemType::INT_ARRAY]->d_alignment);
                  ASSERT(sizeof(std::vector<int>) ==
                         typesLookupTable[bdem_ElemType::INT_ARRAY]->d_size);
                  if (veryVerbose) {
                    P(typesLookupTable[bdem_ElemType::INT_ARRAY]->d_alignment);
                           T_(); P(bdes_AlignmentOf<std::vector<int> >::VALUE);
                      P(typesLookupTable[bdem_ElemType::INT_ARRAY]->d_size);
                           T_(); P(sizeof(std::vector<int>));
                  }
                } break;
                case 'N': {
                  ASSERT(bdes_AlignmentOf<std::vector<Int64> >::VALUE ==
                    typesLookupTable[bdem_ElemType::INT64_ARRAY]->d_alignment);
                  ASSERT(sizeof(std::vector<Int64>) ==
                         typesLookupTable[bdem_ElemType::INT64_ARRAY]->d_size);
                  if (veryVerbose) {
                  P(typesLookupTable[bdem_ElemType::INT64_ARRAY]->d_alignment);
                         T_(); P(bdes_AlignmentOf<std::vector<Int64> >::VALUE);
                      P(typesLookupTable[bdem_ElemType::INT64_ARRAY]->d_size);
                          T_(); P(sizeof(std::vector<Int64>));
                  }
                } break;
                case 'O': {
                  ASSERT(bdes_AlignmentOf<std::vector<float> >::VALUE ==
                    typesLookupTable[bdem_ElemType::FLOAT_ARRAY]->d_alignment);
                  ASSERT(sizeof(std::vector<float>) ==
                         typesLookupTable[bdem_ElemType::FLOAT_ARRAY]->d_size);
                  if (veryVerbose) {
                  P(typesLookupTable[bdem_ElemType::FLOAT_ARRAY]->d_alignment);
                         T_(); P(bdes_AlignmentOf<std::vector<float> >::VALUE);
                      P(typesLookupTable[bdem_ElemType::FLOAT_ARRAY]->d_size);
                          T_(); P(sizeof(std::vector<float>));
                  }
                } break;
                case 'P': {
                  ASSERT(bdes_AlignmentOf<std::vector<double> >::VALUE ==
                   typesLookupTable[bdem_ElemType::DOUBLE_ARRAY]->d_alignment);
                  ASSERT(sizeof(std::vector<double>) ==
                        typesLookupTable[bdem_ElemType::DOUBLE_ARRAY]->d_size);
                  if (veryVerbose) {
                 P(typesLookupTable[bdem_ElemType::DOUBLE_ARRAY]->d_alignment);
                        T_(); P(bdes_AlignmentOf<std::vector<double> >::VALUE);
                      P(typesLookupTable[bdem_ElemType::DOUBLE_ARRAY]->d_size);
                         T_(); P(sizeof(std::vector<double>));
                  }
                } break;
                case 'Q': {
                  ASSERT(bdes_AlignmentOf<std::vector<std::string> >::VALUE ==
                   typesLookupTable[bdem_ElemType::STRING_ARRAY]->d_alignment);
                  ASSERT(sizeof(std::vector<std::string>) ==
                        typesLookupTable[bdem_ElemType::STRING_ARRAY]->d_size);
                  if (veryVerbose) {
                 P(typesLookupTable[bdem_ElemType::STRING_ARRAY]->d_alignment);
                   T_(); P(bdes_AlignmentOf<std::vector<std::string> >::VALUE);
                    P(typesLookupTable[bdem_ElemType::STRING_ARRAY]->d_size);
                    T_(); P(sizeof(std::vector<std::string>));
                  }
                } break;
                case 'R': {
                  ASSERT(bdes_AlignmentOf<std::vector<bdet_Datetime> >::VALUE
                     == typesLookupTable[bdem_ElemType::DATETIME_ARRAY]->
                                                                  d_alignment);
                  ASSERT(sizeof(std::vector<bdet_Datetime>) ==
                      typesLookupTable[bdem_ElemType::DATETIME_ARRAY]->d_size);
                  if (veryVerbose) {
                      P(typesLookupTable[bdem_ElemType::DATETIME_ARRAY]->
                                                                  d_alignment);
                      T_();
                      P(bdes_AlignmentOf<std::vector<bdet_Datetime> >::VALUE);
                      P(typesLookupTable[bdem_ElemType::DATETIME_ARRAY]->
                                                                       d_size);
                      T_(); P(sizeof(std::vector<bdet_Datetime>));
                  }
                } break;
                case 'S': {
                  ASSERT(bdes_AlignmentOf<std::vector<bdet_Date> >::VALUE ==
                         typesLookupTable[bdem_ElemType::DATE_ARRAY]->
                                                                  d_alignment);
                  ASSERT(sizeof(std::vector<bdet_Date>) ==
                         typesLookupTable[bdem_ElemType::DATE_ARRAY]->d_size);
                  if (veryVerbose) {
                      P(typesLookupTable[bdem_ElemType::DATE_ARRAY]->
                                                                  d_alignment);
                     T_(); P(bdes_AlignmentOf<std::vector<bdet_Date> >::VALUE);
                      P(typesLookupTable[bdem_ElemType::DATE_ARRAY]->d_size);
                      T_(); P(sizeof(std::vector<bdet_Date>));
                  }
                } break;
                case 'T': {
                  ASSERT(bdes_AlignmentOf<std::vector<bdet_Time> >::VALUE ==
                         typesLookupTable[bdem_ElemType::TIME_ARRAY]->
                                                                  d_alignment);
                  ASSERT(sizeof(std::vector<bdet_Time>) ==
                         typesLookupTable[bdem_ElemType::TIME_ARRAY]->d_size);
                  if (veryVerbose) {
                      P(typesLookupTable[bdem_ElemType::TIME_ARRAY]->
                                                                  d_alignment);
                     T_(); P(bdes_AlignmentOf<std::vector<bdet_Time> >::VALUE);
                      P(typesLookupTable[bdem_ElemType::TIME_ARRAY]->d_size);
                      T_(); P(sizeof(std::vector<bdet_Time>));
                  }
                } break;
                case 'U': {
                  ASSERT(bdes_AlignmentOf<ListRep>::VALUE ==
                         typesLookupTable[bdem_ElemType::LIST]->d_alignment);
                  ASSERT(sizeof(ListRep) ==
                         typesLookupTable[bdem_ElemType::LIST]->d_size);
                  if (veryVerbose) {
                      P(typesLookupTable[bdem_ElemType::LIST]->d_alignment);
                                 T_(); P(bdes_AlignmentOf<ListRep>::VALUE);
                      P(typesLookupTable[bdem_ElemType::LIST]->d_size);
                                 T_(); P(sizeof(ListRep));
                  }
                } break;
                case 'V': {
                  ASSERT(bdes_AlignmentOf<TableRep>::VALUE ==
                         typesLookupTable[bdem_ElemType::TABLE]->d_alignment);
                  ASSERT(sizeof(TableRep) ==
                         typesLookupTable[bdem_ElemType::TABLE]->d_size);
                  if (veryVerbose) {
                      P(typesLookupTable[bdem_ElemType::TABLE]->d_alignment);
                                 T_(); P(bdes_AlignmentOf<TableRep>::VALUE);
                      P(typesLookupTable[bdem_ElemType::TABLE]->d_size);
      ...
      case 9: {
        ...
        //
        // Testing:
        //   bdem_IdxOffsetMap& operator=(const bdem_IdxOffsetMap& rhs);
        // -------------------------------------------------------------------
      ...
      case 8: {
        ...
        //
        // Testing:
        //   std::vector<double> g(const char *spec);
        // --------------------------------------------------------------------
      ...
      case 7: {
        ...
        //
        // Testing:
        //   bdem_IdxOffsetMap(const bdem_IdxOffsetMap& original, *ba = 0);
        // --------------------------------------------------------------------
      ...
      case 6: {
        ...
        //
        // Testing:
        //   int operator==(const bdem_IdxOffsetMap& lhs,
        //                  const bdem_IdxOffsetMap& rhs);
        //   int operator!=(const bdem_IdxOffsetMap& lhs,
        //                  const bdem_IdxOffsetMap& rhs);
        // --------------------------------------------------------------------
      ...
      case 5: {
        ...
        //
        // Testing:
        //   ostream& operator<<(ostream& os, const bdem_IdxOffsetMap& rhs);
        // --------------------------------------------------------------------
      ...
      case 4: {
        ...
        //   'bdem_IdxOffsetMap'.  Based on input from table DATA, the size and
        ...
        if (verbose) cout << "\nTesting direct accessors." << endl;
        {
            for (int i = 0; i < NUM_DATA; i++) {
                ...
                for (int j = 0; j < NUM_TYPES; j++) {
                    ...
                    // place.
                    LOOP_ASSERT(LINE, j == DD[CURSOR].d_elemAttr->d_elemEnum);
                    ...
                    LOOP_ASSERT(LINE, JTH_OFFSET == DD[CURSOR].d_offset);
                    ...
                    for (int k = 0; k < (int)e_array.size(); k++) {
                        LOOP_ASSERT(LINE,
                                   DD[k].d_elemAttr->d_elemEnum == e_array[k]);
                        LOOP_ASSERT(LINE, DD[k].d_offset == o_array[k]);
                    }
                }
            }
        }
      ...
      case 3: {
                          ...
                          << "=====================================" << endl;
        {
            ...
            // Apply remove all, then append one of the 22 types and then
            // test length.
                ...
                for (int ti = 0; ti < NUM_DATA ; ++ti) {
                    ...
                    for (int i = 0; i < LENGTH; ++i) {
                        LOOP2_ASSERT(LINE, i, e[i]->d_elemEnum ==
                                                  X[i].d_elemAttr->d_elemEnum);
                        LOOP2_ASSERT(LINE, i, e[i]->d_elemEnum ==
                                                  Y[i].d_elemAttr->d_elemEnum);
                    }
                }
            }
      ...
      case 2: {
        ...
        // Concerns:
        //   1. The default 'bdem_IdxOffsetMap' constructor
        ...
        //          using a 'bdema_TestAllocator' and varying its *allocation*
        //          *limit*.
        //        - Where the object is constructed entirely in static memory
        //          (using a 'bdema_BufferAllocator') and never destroyed.
        //
        ...
        //        - call 'removeAll'
        //          * Verify that the 'bdem_IdxOffsetMap' has no elements
        //            after the call.
        //          * Verify that the total offset is 0.
        //
        // Testing:
        //   bdem_IdxOffsetMap(bdema_Allocator *ba = 0);
        //   ~bdem_IdxOffsetMap();
        //   void removeAll();
        ...
        // a.
        if (verbose) cout
            << "\nTesting 'bdem_IdxOffsetMap()' Constructor." << endl;
        if (verbose) cout << "\tWithout passing in an allocator." << endl;
        {
        ...
        if (verbose) cout << "\nTesting insert of INT." << endl;
        {
            ...
            for (int i = 0; i < NUM_DATA; i++) {
                ...
                for (int j = 0; j < CURSOR; j++) {
                    DD.append(typesLookupTable[bdem_ElemType::CHAR]);
                }
                ...
                LOOP_ASSERT(LINE, bdem_ElemType::INT ==
                                            DD[CURSOR].d_elemAttr->d_elemEnum);

                // ASSERT we have the correct offset.
                LOOP_ASSERT(LINE, OFFSET == DD[CURSOR].d_offset);
            ...
            for (int i = 0; i < NUM_DATA; i++) {
                ...
                for (int j = 0; j < NUM_TYPES; j++) {
                    ...
                    for (int k = 0; k < CURSOR; k++) {
                        DD1.append(typesLookupTable[bdem_ElemType::CHAR]);
                    }
                    ...
                    LOOP_ASSERT(LINE, j == DD1[CURSOR].d_elemAttr->d_elemEnum);
                    ...
                    const int JTH_OFFSET =
                                   (CURSOR + JTH_ALIGN - 1) & ~(JTH_ALIGN - 1);
                    LOOP_ASSERT(LINE, JTH_OFFSET == DD1[CURSOR].d_offset);
        ...
        if (verbose) cout << "\nTesting 'removeAll'." << endl;
        {
            Obj DD2;
            for (int i = 0; i < 2; i++) {
                DD2.insert(0, typesLookupTable[bdem_ElemType::CHAR]);
                DD2.insert(1, typesLookupTable[bdem_ElemType::INT]);
                DD2.insert(2, typesLookupTable[bdem_ElemType::DOUBLE]);
                DD2.insert(3, typesLookupTable[bdem_ElemType::LIST]);
                DD2.insert(4, typesLookupTable[bdem_ElemType::TABLE]);
                DD2.removeAll();
                ASSERT(0 == DD2.length());
      ...
      case 1: {
        ...
        //   'bdem_IdxOffsetMap' works properly.  We are concerned that the
        //   basic (value-semantic) functionality of 'bdem_IdxOffsetMap' works
        //   properly.
        //
        // Plan:
        //   We exercise basic functionality in 12 steps, inspired by the
        //   'std::vector<double>' test driver 11-step breathing test, to which
        ...
        //          a) 0 == x3.length()
        //          b) x3 == x1 x3 != x2
        //      8) Create bdem_idxoffsetmap x4(x2)
        //          a) 1 == x4.length()
        //          b) x4 == x2 x4 != x1 x4 != x3
        ...
        BEGIN_BDEMA_EXCEPTION_TEST {
          ...
          if (verbose) cout << "\n 3. Append a char to x1."
                               "\t\t\t{ x1:V x2: }" << endl;
          x1.append(typesLookupTable[bdem_ElemType::CHAR]);
          ASSERT(X1 != X2);
          ASSERT( 1 == X1.length());
          ...
          if (verbose) cout << "\n 4. Append a char to x2."
                               "\t\t{ x1:V x2:V }" << endl;
          x2.append(typesLookupTable[bdem_ElemType::CHAR]);
          ASSERT(X1 == X2);
          ASSERT( 1 == X2.length());
