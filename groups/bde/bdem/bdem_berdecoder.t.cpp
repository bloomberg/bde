...
#define P(X) { bsl::cout << #X " = "; \
    bdeu_PrintMethods::print(bsl::cout, X, -1, -1); bsl::cout << bsl::endl; }
#define P_(X) { bsl::cout << #X " = "; \
    bdeu_PrintMethods::print(bsl::cout, X, -1, -1); bsl::cout << ", " \
    << bsl::flush; }
...
class MySequenceWithAnonymousChoiceChoice {
  ...
  public:
    ...
    // CREATORS
    explicit
    MySequenceWithAnonymousChoiceChoice(bslma_Allocator *basicAllocator = 0);
...
class MySequenceWithAnonymousChoice {
  ...
  public:
    ...
    // CREATORS
    explicit
    MySequenceWithAnonymousChoice(bslma_Allocator *basicAllocator = 0);
...
inline
STREAM&
MySequenceWithNullable::bdexStreamOut(STREAM& stream, int version) const
{
    switch (version) {
      case 1: {
...
inline
STREAM&
MySequenceWithNillable::bdexStreamOut(STREAM& stream, int version) const
{
    switch (version) {
      case 1: {
        bdex_OutStreamFunctions::streamOut(stream, d_attribute1, 1);
...
inline
STREAM&
MySequenceWithAnonymousChoiceChoice::bdexStreamOut(STREAM& stream,
                                                   int     version) const
{
    switch (version) {
      case 1: {
...
inline
STREAM&
MySequenceWithAnonymousChoice::bdexStreamIn(STREAM& stream, int version)
{
    if (stream) {
        switch (version) {  // Switch on the schema version (starting with 1).
          case 1: {
...
void usageExample()
{
    ...
    ASSERT(0 == retCode);
//..
// Now we will verify the contents of 'osb' using the 'bdem_berdecoder'
// component:
//..
...
void printDiagnostic(bdem_BerDecoder & decoder)
{
    if (veryVerbose) {
...
//       case 18: {
//         // --------------------------------------------------------------------
//         // TESTING encoding & decoding for date/time components using variant
//         //
//         // Concerns:
//         //
//         // Plan:
...
//         if (verbose) bsl::cout
//             << "\nTESTING encoding & decoding for date/time using variant"
//             << "\n======================================================="
//             << bsl::endl;
...
//                         test::BasicRecordWithVariant VALUE4;
//                         VALUE4.i1() = 100;
//                         VALUE4.i2() = 200;
//                         VALUE4.dt() = VALUE1;
//                         VALUE4.s()  = "Hello World";

//                         test::BasicRecordWithVariant VALUE5(VALUE4),
//                                                      VALUE6(VALUE4);
//                         VALUE5.dt() = VALUE2;
//                         VALUE6.dt() = VALUE3;

//                         test::BasicRecordWithVariant value4, value5, value6;
    ...
    {      L_,   9999,    12,   31,    23,    59,   59,    0,   1439     },
      ...
      case 11: {
        ...
        if (verbose) bsl::cout << "\nTesting with null value." << bsl::endl;
        {
                                       ...
                                       << bsl::endl;
            {
                ...
                bdesb_FixedMemInStreamBuf isb(&nokalvaData[0],
                                              nokalvaData.size());
      ...
      case 6: {
        ...
        for (int i = 0; i < DATA_LEN; ++i)
        {
            ...
            bdem_BerDecoderOptions opts;
            opts.setSkipUnknownElements(false);
            if (veryVeryVerbose) {
               opts.setTraceLevel(1);
            }

            bdem_BerDecoder decoder2(&opts);
      ...
      case 3: {
...

        {
            ...
            bdet_Datetime valueOut(YEAR, MONTH, DAY,
      ...
      case -1: {
        ...
        for (int i = 0; i < reps; ++i) {
            isb.pubseekpos(0);
            bdem_BerDecoder decoder;  // Typical usage: single-use object
        ...
        bsl::cout << "    bdem_BerDecoder: " << elapsed << " seconds, "
                  << (reps / elapsed) << " reps/sec" << bsl::endl;

        // Measure bdex encoding and decoding time:
        osb.reserveCapacity(MAX_BUF_SIZE);
        ...
        for (int i = 0; i < reps; ++i) {
            ...
            bdex_OutStreamFunctions::streamOut(
                                            bdexOutStream,
                                            request,
                                            request.maxSupportedBdexVersion());
        }
        stopwatch.stop();
        ...
        isb.pubsetbuf(osb.data(), osb.length());
        bdex_ByteInStreamFormatter bdexInStream(&isb);
        inRequests = new test::TimingRequest[reps];
        stopwatch.reset();
        stopwatch.start();
        for (int i = 0; i < reps; ++i) {
            ...
            bdex_InStreamFunctions::streamIn(
                                      bdexInStream,
                                      inRequests[i],
                                      inRequests[i].maxSupportedBdexVersion());
        }
        stopwatch.stop();
        ...
        double bdexDecodeSpeed = reps / elapsed;
        bsl::cout << "      bdex decoding: " << elapsed << " seconds, "
                  << (reps / elapsed) << " reps/sec" << bsl::endl;
        ...
        slowDown = 1.0 - berDecodeSpeed / bdexDecodeSpeed;
        bsl::cout << "BerDecoder is " << (slowDown * 100.0)
                  << "% slower than bdex" << bsl::endl;

        slowDown = 1.0 - ((berEncodeSpeed + berDecodeSpeed) /
                          (bdexEncodeSpeed + bdexDecodeSpeed));
        bsl::cout << "BerEn/Decoder is " << (slowDown * 100.0)
                  << "% slower than bdex" << bsl::endl;
      } break;
      default: {
