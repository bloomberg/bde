// bdetst_stldeque.t.cpp    -*-C++-*-

/*****************************************************************************
 *                       W A R N I N G !
 *
 * This test driver is a slightly-modified version of the vector test
 * driver.  It tests some things, but should not be considered comprehensive.
 * In particular, push_front is not tested and memory allocation is
 * not checked.  Only operations that work for both vector and deque
 * are tested.
 *****************************************************************************/

#include <bdetst_stldeque.h>
...
// perturbation of internal state.  Note that each 
...
// [ 5] deque<T,A>(size_type n, const T& val = T(), const A& a = A());
// [--] template<class InputIter>
//      deque<T,A>(InputIter first, InputIter last, const A& a = A());
// [ 4] deque<T,A>(const deque<T,A>& orig);
// [ 2] ~deque<T,A>();
...
typedef std::deque<SampleClass> Obj;
...
enum {
        ...
        LARGE_SIZE_VALUE = 10  // Declare a large value for insertions into the
                               // deque. Note this value will cause multiple
                               // resizes during insertion into the deque.
};
...
int main(int argc, char *argv[])
{
	...
	//
      ...
      case 5: {
        ...
        //
        // Testing:
        //   deque<T,A>(size_type n, const T& value = T(), 
        //               const Allocator& a = A());
        // --------------------------------------------------------------------
...
//                     }
      ...
      case 4: {
        ...
        if (verbose) cout <<
            "\n\tTesting Deque Copy constructor." << endl;
...
//                     if (curLen == 0) {
//                         LOOP_ASSERT(SPEC, BB + 0 == AA);
//                         LOOP_ASSERT(SPEC,  B + 0 ==  A);
//                     }
//                     else {
//                         LOOP_ASSERT(SPEC, BB + 1 == AA);
//                         LOOP_ASSERT(SPEC,  B + 1 ==  A);
//                     }
...
//                 LOOP_ASSERT(li, B  - 0 == C);
      ...
      case 1: {
        ...
        std::deque<int, std::allocator<void*> > zz1, zz2(zza);
