// btlsc_timedchannelallocator.t.cpp                                  -*-C++-*-
...
//-----------------------------------------------------------------------------
// CREATORS
// [ 1] virtual ~TimedChannelAllocator();
//
// MANIPULATORS
// [ 1] virtual Channel *allocate(int *, int = 0) = 0;
// [ 1] virtual Channel *timedAllocate(int *, const TI&, int = 0) = 0;
// [ 1] virtual TimedChannel *allocateTimed(int *, int = 0) = 0;
// [ 1] virtual TC *timedAllocateTimed(int *, const TI&, int = 0) = 0;
// [ 1] virtual void deallocate(Channel *channel) = 0;
...
typedef btlsc::TimedChannelAllocator ProtocolClass;

struct ProtocolClassTestImp : bsls::ProtocolTestImp<ProtocolClass> {
    btlsc::Channel *allocate(int *, int = 0)             { return markDone(); }

    btlsc::Channel *timedAllocate(int *, const bsls::TimeInterval&, int = 0)
                                                         { return markDone(); }
    ...
    btlsc::TimedChannel *
    timedAllocateTimed(int *, const bsls::TimeInterval&, int = 0)
                                                         { return markDone(); }
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:
      ...
      case 1: {
        ...
        //   virtual Channel *timedAllocate(int *, const TI&, int = 0) = 0;
        //   virtual TimedChannel *allocateTimed(int *, int = 0) = 0;
        //   virtual TC *timedAllocateTimed(int *, const TI&, int = 0) = 0;
        ...
        BSLS_PROTOCOLTEST_ASSERT(testObj, timedAllocate(0, TI, 0));

        BSLS_PROTOCOLTEST_ASSERT(testObj, allocateTimed(0, 0));

        BSLS_PROTOCOLTEST_ASSERT(testObj, timedAllocateTimed(0, TI, 0));
