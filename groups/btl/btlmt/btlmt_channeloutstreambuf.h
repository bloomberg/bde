// btlmt_channeloutstreambuf.h        -*-C++-*-
#ifndef INCLUDED_BTLMT_CHANNELOUTSTREAMBUF
#define INCLUDED_BTLMT_CHANNELOUTSTREAMBUF
...
//
//@CLASSES:
//    btlmt::ChannelOutStreamBuf: 'bsl::streambuf' for channel communication
//
//@AUTHOR: David Rubin (drubin6)
//
//@SEE_ALSO: btlmt_channelpool, bdlmca_xxxpooledbufferchain, btlmt::DataMsg
//
//@DESCRIPTION: This class provides support for streaming data over TCP/IP
// communication channels.  'btlmt::ChannelOutStreamBuf' implements the
// 'bsl::streambuf' protocol, and encapsulates a 'bdlmca::PooledBufferChain',
// which is the underlying data delivery mechanism employed by the
// 'btlmt::ChannelPool' class.  Hence, clients can more easily send data between
// networked peers managed by a 'btlmt::ChannelPool' using a common and
// ubiquitous streaming mechanism, ignoring the more complex details of the
// 'bdlmca::PooledBufferChain' and 'btlmt::DataMsg' classes.
//
// Additionally, 'btlmt::ChannelOutStreamBuf' enables clients to configure a
...
// 'bdlmca::PooledBufferChainFactory' from which the 'btlmt::ChannelOutStreamBuf'
...
// event.
//
// The cache configuration aspect of the 'btlmt::ChannelOutStreamBuf' provides
...
// 'btlmt::ChannelOutStreamBuf' to echo a 'btlmt::DataMsg'.  The second example
// focuses on the caching capabilities of the 'btlmt::ChannelOutStreamBuf'.
//
///Example 1
///- - - - -
// This example illustrates how to use a 'btlmt::ChannelOutStreamBuf' to echo a
// 'btlmt::DataMsg'.  We present a function, 'echoMessage', which is
// parameterized by a 'btlmt::DataMsg', a 'btlmt::ChannelPool' which received the
// message, and a 'bdlmca::PooledBufferChainFactory'.  In this example, the data
...
//          CACHE_LIFESPAN  = 0
//      };
//      btlmt::ChannelOutStreamBuf outbuf(msg.channelId(), pool, factory,
//                                       LOW_WATER_MARK,
//                                       HIGH_WATER_MARK,
//                                       CACHE_LIFESPAN);
...
///Example 2
///- - - - -
// This example illustrates how to use a 'btlmt::ChannelOutStreamBuf' object to
// cache and send objects represented as 'bdlmxxx::List's.  We present a server
// which accepts client connections and streams data to each client as the it
// becomes available.  Each datum is a 'bdlmxxx::List' containing a random number,
...
// throughput requirements.  This message is also a 'bdlmxxx::List', conforming to
...
//    private:
//      // TYPES
//      typedef bsl::map<int, btlmt::ChannelOutStreamBuf*> MapType;
...
//          d_pool_p->shutdown(channelId, btlmt::ChannelPool::BTEMT_IMMEDIATE);
//        }  break;
//
//        case btlmt::ChannelPool::BTEMT_CHANNEL_UP: {
//          d_lock.lockWrite();
//          d_clients.insert(bsl::make_pair(channelId,
//                                            (btlmt::ChannelOutStreamBuf *)0));
//          d_lock.unlock();
//        }  break;
//      }
//  }
...
//
//      bdlmca::PooledBufferChainStreamBuf inbuf(msg.data());
//      bdlxxxx::ByteInStreamFormatter       bis(&inbuf);
//      bdlmxxx::List                        message;
//      bis >> message;
//      assert(bis);
//
//      *needed   = 1;
...
//
//      if (!bdlmxxx::SchemaAggregateUtil::isListConformant(message,
//                                                      d_schema.record(0)))
//      {
...
//      }
//
//      btlmt::ChannelOutStreamBuf *outbuf = new (*d_allocator_p)
//                                   btlmt::ChannelOutStreamBuf(msg.channelId(),
//                                                             d_pool_p,
//                                                             d_factory_p,
//                                                             lowWaterMark,
//                                                             highWaterMark,
...
//               ++it)
//          {
//              bdlxxxx::ByteOutStreamFormatter bos(it->second);
//              bos << datum << bdexFlush;
//          }
//          d_lock.unlock();
//      }
...
//          it->second->pubsync();
//      }
//      d_lastPush = bdlt::CurrentTime::now();
//      d_lock.unlock();
//  }
//
...
//              bsls::TimeInterval period(kt->second->cacheLifespan() / 1000.0);
...
//
//      btlmt::ChannelPool::ChannelStateChangeCallback ccb(
//              bdlf::MemFnUtil::memFn(&my_DataServer::chanCb, this)
//            , d_allocator_p);
//
//      btlmt::ChannelPool::DataReadCallback dcb(
//              bdlf::MemFnUtil::memFn(&my_DataServer::dataCb, this)
//            , d_allocator_p);
//
//      btlmt::ChannelPool::PoolStateChangeCallback pcb;
//
//      d_dataCb = bdlf::Function<void (*)()>(
//              bdlf::MemFnUtil::memFn(&my_DataServer::generateData, this)
//            , d_allocator_p);
//
//      d_pushCb = bdlf::Function<void (*)()>(
//              bdlf::MemFnUtil::memFn(&my_DataServer::pushData, this)
//            , d_allocator_p);
//
//      bdlmxxx::RecordDef *record = d_schema.createRecord("Connect");
//      assert(record->appendField(bdlmxxx::ElemType::BDEM_INT, "cacheSize"));
//      assert(record->appendField(bdlmxxx::ElemType::BDEM_INT, "maxDelay"));
//
//      record = d_schema.createRecord("Data");
//      assert(record->appendField(bdlmxxx::ElemType::BDEM_INT, "value"));
//
//      // Determine pooled buffer size using a mock connect request.
//      bdlmxxx::List connect;
//      connect.appendInt(0);
//      connect.appendInt(0);
//
//      bdlxxxx::ByteOutStream bos(d_allocator_p);
//      bos << connect;
//      d_bufferSize = bos.length();
//
//      d_factory_p = new (*d_allocator_p)
//                 bdlmca::PooledBufferChainFactory(d_bufferSize, d_allocator_p);
//
//      bslma::RawDeleterProctor<bdlmca::PooledBufferChainFactory,
//                               bslma::Allocator>
...
//                        btlmt::ChannelPool(ccb, dcb, pcb, cpc, d_allocator_p);
//
//      deleter.release();
//  }
//
...
namespace BloombergLP {
                      ...
                      // ===============================
                      // class btlmt::ChannelOutStreamBuf
                      // ===============================


namespace btlmt {class ChannelPool;

class ChannelOutStreamBuf : public bsl::streambuf {
    // This class implements the output functionality of the 'bsl::streambuf'
    // protocol using a client-supplied 'bdlmca::PooledBufferChainFactory'.  TBD
    // doc

  public:
    // TYPES
    typedef bdlf::Function<void (*)(const DataMsg&)> Callback;
  ...
  private:
    ...
    bdlmca::PooledBufferChain  *d_bufferChain_p;        // owned
    ChannelPool        *d_channelPool_p;        // held
    DataMsg             d_dataMsg;         // message containing chain
    ...
    bsls::Types::Int64        d_timeOfLastSend;

  private:
    // not implemented
    ChannelOutStreamBuf(const ChannelOutStreamBuf&);
    ChannelOutStreamBuf& operator=(const ChannelOutStreamBuf&);
  ...
  public:
    ...
    ChannelOutStreamBuf(
                           ...
                           int                             cacheLifespan,
                           bslma::Allocator               *basicAllocator = 0);
        // Create a 'ChannelOutStreamBuf' object which can transmit on
...
// PRIVATE ACCESSORS
inline
int ChannelOutStreamBuf::calculatePutPosition() const
{
...
// MANIPULATORS
inline
void ChannelOutStreamBuf::setSendNotificationCallback(
        const ChannelOutStreamBuf::Callback& callback)
{
    d_callback = callback;
}
...
// ACCESSORS
inline
int ChannelOutStreamBuf::cacheLifespan() const
{
    return d_cacheLifespan;
}

inline
int ChannelOutStreamBuf::highWaterMark() const
{
    return d_highWaterMark;
}

inline
bsl::streamsize ChannelOutStreamBuf::length() const
{
    return calculatePutPosition();
}

inline
int ChannelOutStreamBuf::lowWaterMark() const
{
