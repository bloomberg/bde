// btlmt_channeloutstreambuf.cpp                                      -*-C++-*-
#include <btlmt_channeloutstreambuf.h>

#include <bsls_ident.h>
BSLS_IDENT_RCSID(btlmt_channeloutstreambuf_cpp,"$Id$ $CSID$")
...
#include <btlmt_channelpoolconfiguration.h> // for testing only
#endif
...
namespace btlmt {
                      // -------------------------------
                      // class ChannelOutStreamBuf
                      // -------------------------------

// PRIVATE MANIPULATORS

int ChannelOutStreamBuf::sendDataOverChannel()
{
...
void ChannelOutStreamBuf::setPutPosition(off_type offset)
{
    BSLS_ASSERT(d_bufferChain_p);
    ...
    d_putBufferIndex = static_cast<int>(offset / bufferSize);
    ...
    if (d_putBufferIndex >= numBuffers) {
        ...
        pbump(static_cast<int>(offset - basePosition));
    }
}

// PRIVATE ACCESSORS

int ChannelOutStreamBuf::checkInvariant() const
{
    BSLS_ASSERT(d_bufferChain_p);
...
bsl::streambuf::int_type ChannelOutStreamBuf::overflow(
        int_type insertionChar)
{
    ...
    if (pptr() != epptr()) {
        *pptr() = static_cast<char_type>(insertionChar);
        pbump(1);
...
bsl::streambuf::pos_type ChannelOutStreamBuf::seekoff(
        off_type                offset,
...
bsl::streambuf::pos_type ChannelOutStreamBuf::seekpos(
        pos_type                position,
        bsl::ios_base::openmode which)
{
...
bsl::streamsize ChannelOutStreamBuf::xsputn(
...
ChannelOutStreamBuf::ChannelOutStreamBuf(
        int                             channelId,
...
int ChannelOutStreamBuf::forceSync()
{
