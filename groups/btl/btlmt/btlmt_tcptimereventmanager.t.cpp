...
void
socketCb(
    btlmt::TcpTimerEventManager *pEventManager,
    ...
    // Perform registration requests for the event manager specified by 'arg'.
{
    ...
    testPairs = (btlso::EventManagerTestPair*) (new char[NUM_REGISTER_PAIRS *
...
{
    vector<bdlmtt::ThreadUtil::Handle> threads(numThreads,
                                             bdlmtt::ThreadUtil::Handle());

    for (int i = 0; i < numThreads; ++i) {
        ASSERT(0 == bdlmtt::ThreadUtil::create(&threads[i], bcemt_Attribute(),
                                             func, arg));
    }
    for (int i = 0; i < numThreads; ++i) {
...
static void recordCb(void *context,
    ...
    my_Event event;
    event.d_thread = bdlmtt::ThreadUtil::selfIdAsInt();
...
extern "C" void *executeTest(void *arg) {
    ...
    if (veryVerbose) {
        bdlmtt::LockGuard<bdlmtt::Mutex> lock(&coutMutex);
        P_(bdlmtt::ThreadUtil::selfIdAsInt());
...
//    ASSERT(0 == bsl::memcmp(readBuffer, expBuffer, rc));
    ...
    bdlmtt::ThreadUtil::microSleep(800, 0);
}
    ...
    switch (test) { case 0:
      ...
      case 15: {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            // btlso::SocketHandle::Handle clientFd = writeDataArgs[j].d_handle;
      ...
      case 12: {
          ...
          //
          // Testing:
          //  btlmt::TcpTimerEventManager(Hint , bool, bslma::Allocator*);
          //  btlmt::TcpTimerEventManager(btlso::EventManager  *,
          //                             bslma::Allocator    *);
      ...
      case 8: {
                 ...
                 << endl;
        {
            ...
            for (int i = 0; i < NUM_THREADS; ++i) {
                int rc = bdlmtt::ThreadUtil::create(&workers[i],
                                                  bcemt_Attribute(),
                                                  &deregisterThread, &mX);
                LOOP_ASSERT(i, 0 == rc);
            }
            for (int i = 0; i < NUM_THREADS; ++i) {
                int rc = bdlmtt::ThreadUtil::join(workers[i]);
                LOOP_ASSERT(i, 0 == rc);
            }
...
#ifdef BSLS_PLATFORM_OS_UNIX
        ASSERT((bdlmtt::ThreadUtil::Handle) -1 == X.dispatcherThreadHandle());
#endif
      ...
      case -1: {
        ...
        while (threadPool.numActiveThreads()) {
         printf("=========================================================\n"
            ...
            bdlmtt::ThreadUtil::microSleep(0, 5);  // 5 seconds
        }
