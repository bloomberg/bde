...
//-----------------------------------------------------------------------------
// CREATORS
// [ 2] btlmt::SessionPool(config, poolStateCallback, bslma::Allocator *ta = 0);
// [ 3] btlmt::SessionPool(config, poolStateCallback, useBlobs, *ta = 0);
// [ 2] ~btlmt::SessionPool();
//
// MANIPULATORS
// [13] int start();
...
int TestSession::start()
{
    ...
    if (d_useBlobReadCb) {
        ...
        btlmt::AsyncChannel::ReadCallback callback =
                      bdlf::MemFnUtil::memFn(&TestSession::dataMsgReadCb, this);
...
void runTestFunction(bdlmtt::ThreadUtil::Handle                *connectThreads,
                     bdlmtt::ThreadUtil::Handle                *listenThreads,
                     ...
                     const bdlmca::Blob&                        dataBlob)
{
    ...
    while (numUpConnections < NUM_THREADS) {
        bdlmtt::ThreadUtil::microSleep(50, 0);
    }
...
class TestSessionFactory : public btlmt::SessionFactory {
  ...
  public:
    // CREATORS
    TestSessionFactory(bdlmtt::Barrier *barrier);
...
// CREATORS
TestSessionFactory::TestSessionFactory(bdlmtt::Barrier *barrier)
: d_barrier_p(barrier)
{
}
...
void TestSessionFactory::deallocate(btlmt::Session *session)
{
    if (veryVerbose) {
        MTCOUT << "Deallocate factory called: " << MTENDL;
    }
...
class TesterFactory : public btlmt::SessionFactory {
    ...
    bdlmtt::Barrier                   *d_barrier_p;   // held not owned
  ...
  public:
    ...
    // MANIPULATORS
    virtual void allocate(const bsl::shared_ptr<btlmt::AsyncChannel>& channel,
                          const btlmt::SessionFactory::Callback&      callback);
        // Asynchronously allocate a 'btlmt::Session' object for the
        // specified 'channel', and invoke the specified 'callback' with
        // this session.
                           ...
                           const btlmt::DataMsg& msg)
{
    ...
    *numNeeded   = 1;
    ...
    bdlmtt::ThreadUtil::Handle handle(bdlmtt::ThreadUtil::invalidHandle());
    ASSERT(0 == bdlmtt::ThreadUtil::create(
                &handle,
                bdlf::BindUtil::bind(&TesterSession::delayedChannelAccessor,
                                    this,
                                    d_channel_sp,
                                    d_channel_sp->peerAddress())));
    bdlmtt::ThreadUtil::detach(handle);
}
...
int TesterSession::start()
{
    ...
    bdlf::Function<void (*)(int,
                           int*,
                           int*,
                           const BloombergLP::btlmt::DataMsg&)> f =
...
void TesterFactory::allocate(btlmt::AsyncChannel                    *channel,
                             const btlmt::SessionFactory::Callback&  callback)
{
...
//..
    ...
    {
        btlmt::AsyncChannel::ReadCallback
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      ...
      case 14: {
        ...
        //      channel. Coordinate access to the underlying btlmt::AsyncChannel
      ...
      case 13: {
        ...
        bdlmtt::ThreadUtil::Handle connectThreads[NUM_THREADS];
        bdlmtt::ThreadUtil::Handle listenThreads[NUM_THREADS];
      ...
      case 9: {
        ...
        for (int i = 0; i < NUM_THREADS; ++i) {
            bdlmtt::ThreadUtil::join(connectThreads[i]);
            bdlmtt::ThreadUtil::join(listenThreads[i]);
        }
      } break;
      case 8: {
        // --------------------------------------------------------------------
      ...
      case 6: {
        ...
        PoolCb          poolStateCb(&poolStateCallback);
        bdlmtt::Barrier   barrier(NUM_SESSIONS + 1);
        bsls::AtomicInt numUpConnections(0);
        ...
        btlso::InetStreamSocketFactory<btlso::IPv4Address> socketFactory;
        bsl::vector<bslma::ManagedPtr<btlso::StreamSocket<btlso::IPv4Address> > >
