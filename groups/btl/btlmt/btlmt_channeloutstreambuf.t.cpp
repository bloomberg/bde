// btlmt_channeloutstreambuf.t.cpp            -*-C++-*-
...
//                                  --------
//
// 'btlmt::ChannelOutStreamBuf' is exercised in this test driver from a
...
// callback function.
//-----------------------------------------------------------------------------
// 'btlmt::ChannelOutStreamBuf' private interface
// MANIPULATORS
...
//
// 'btlmt::ChannelOutStreamBuf' protected interface
// MANIPULATORS
...
//
// 'btlmt::ChannelOutStreamBuf' public interface
// CREATORS
// [ 2] btlmt::ChannelOutStreamBuf(
//              int                             channelId,
...
//-----------------------------------------------------------------------------
typedef btlmt::ChannelOutStreamBuf                        Obj;
typedef bdlmca::PooledBufferChainFactory                   BufferFactory;
typedef bdlmca::PooledBufferChain                          Chain;
typedef btlmt::ChannelPoolConfiguration                   Config;
typedef btlmt::ChannelPool                                Pool;
typedef btlso::InetStreamSocketFactory<btlso::IPv4Address> AddressFactory;
...
class my_ServerPool {
    ...
    // runs on top of a single 'btlmt::ChannelPool' instance.

  public:
    // TYPES
    typedef btlmt::ChannelPool::DataReadCallback DataReadCallback;
    ...
    typedef btlmt::ChannelPool::ChannelStateChangeCallback ChannelCallback;
        // This type defines a functor which can be instantiated by a user,
  ...
  private:
    // TYPES
    typedef bsl::map<int, bdlmca::PooledBufferChainFactory*> MapType;
...
, d_allocator_p(bslma::Default::allocator(basicAllocator))
{
    ...
    btlmt::ChannelPool::ChannelStateChangeCallback ccb(
            bdlf::MemFnUtil::memFn(&my_ServerPool::chanCb, this)
          , d_allocator_p);

    btlmt::ChannelPool::DataReadCallback dcb(
            bdlf::MemFnUtil::memFn(&my_ServerPool::dataCb, this)
          , d_allocator_p);
        ...
        int                      backlog)
{
    ...
    bdlmca::PooledBufferChainFactory *factory = new (*d_allocator_p)
                     bdlmca::PooledBufferChainFactory(bufferSize, d_allocator_p);

    bslma::RawDeleterProctor<bdlmca::PooledBufferChainFactory, bslma::Allocator>
...
// ACCESSORS
btlmt::ChannelPool *my_ServerPool::pool() const
{
    return d_pool_p;
}
...
void channelStateMonitor(int channelId, int serverId, int state, void *arg)
{
    ...
    btlmt::ChannelPool *pool = static_cast<btlmt::ChannelPool*>(arg);
    ...
    switch(state){
      ...
      case btlmt::ChannelPool::BTEMT_CHANNEL_UP: {
                                         ...
                                         BufferFactory(BUFFER_SIZE, allocator);
        {
            ...
            int           buffersSent = 0;
            Obj::Callback cb(bdlf::BindUtil::bindA( allocator
    ...
    bslma::Allocator  *allocator)
{
    ...
    // Find buffer size.
    int bufferSize = 0;
    bdlxxxx::ByteStreamImpUtil::getInt32(&bufferSize, msg.data()->buffer(0));
    ASSERT(0 < bufferSize);
    *needed = 1;
    ...
    bslma::Allocator  *allocator)
{
    ...
    bdlxxxx::ByteStreamImpUtil::getInt32(&bufferSize, msg.data()->buffer(0));
    bdlxxxx::ByteStreamImpUtil::getInt32(&cacheLifespan,
    ...
    bslma::Allocator  *allocator)
{
    ...
    *consumed = msg.data()->length();
    ...
    pool->shutdown(msg.channelId(), btlmt::ChannelPool::BTEMT_IMMEDIATE);
}
    ...
    bslma::Allocator  *allocator)
{
    ...
    // Decode query
    int request;
    bdlxxxx::ByteStreamImpUtil::getInt32(&request, msg.data()->buffer(0));
    ...
    *consumed = sizeof request;
    ...
    if (GET_DATA_SIZE == request) {
                                ...
                                BufferFactory(1 + sizeof DATA_SIZE, allocator);
        {
            ...
            bdlxxxx::ByteStreamImpUtil::putInt32(buffer, DATA_SIZE);
...
void my_DataServer::generateData()
{
    ...
    if (0 == generate) {
             ...
             ++it)
        {
            ...
            const int VERSION = bdlmxxx::List::maxSupportedBdexVersion();
        ...
        bdlmtt::Barrier      *barrier)
{
    ...
    btlmt::ChannelPool *pool = *static_cast<btlmt::ChannelPool**>(poolAddr);
    ...
    switch (test) { case 0:
      case 14: {
        ...
        bdlma::TestAllocator ta(veryVeryVerbose);
        {
            ...
            const btlso::IPv4Address SERVER_ADDRESS("127.0.0.1", PORT);
            ...
            ASSERT(0 == channel.isInvalid());
            {
                ...
                bdlxxxx::ByteStreamImpUtil::putInt32(length, MESSAGE.length());
      ...
      case -13: {
        ...
        //
        // Concerns:
        //   That the 'btlmt::ChannelOutStreamBuf' is able to handle the case
        //   when the output stream becomes invalid due to a write error.
        //
        // Plan:
        //   Create a server which allocates a modifiable
        //   'btlmt::ChannelOutStreamBuf' object,  'mX' with a fixed high water
      ...
      case 12: {
        ...
        bdlma::TestAllocator ta(veryVeryVerbose);
        {
            ...
            for (int i = 0; i < NUM_SERVERS; ++i) {
                ...
                const btlso::IPv4Address ADDRESS("127.0.0.1", PORT);
                if (veryVerbose) {
                    MTCOUT << "\n\t"
                ...
                // Initiate test.
                char length[sizeof(int)];
                bdlxxxx::ByteStreamImpUtil::putInt32(length, BUFFER_SIZE);
                channel.write(length, sizeof length);
      ...
      case 10: {
        ...
        //  'btlmt::ChannelOutStreamBuf' object, 'mX', with fixed high and low
        ...
        bdlma::TestAllocator ta(veryVeryVerbose);
        {
            ...
            for (int i = 0; i < DATA_SIZE; (0 < retries) || ++i) {
                ...
                bdlxxxx::ByteStreamImpUtil::putInt32(buffer, BUFFER_SIZE);
                bdlxxxx::ByteStreamImpUtil::putInt32(buffer + sizeof(int),
                                                 CACHE_LIFESPAN);
      ...
      case 9: {
        ...
        //   modifiable 'btlmt::ChannelOutStreamBuf' object,  'mX', with fixed
      ...
      case 6: {
        ...
        //   server, allocate a modifiable 'btlmt::ChannelOutStreamBuf' object,
      ...
      case 5: {
        ...
        //   'btlmt::ChannelOutStreamBuf', so this test case remains empty.
        // --------------------------------------------------------------------
      ...
      case 2: {
        ...
        //
        // Concerns:
        //   * That it is possible to instantiate a 'btlmt::ChannelOutStreamBuf'
        ...
        bdlma::TestAllocator ta(veryVeryVerbose);
        {
            ...
            for (int i = 0; i < NUM_SERVERS; ++i) {
                ...
                bdlxxxx::ByteStreamImpUtil::putInt32(buffer, message);
                channel.write(buffer, sizeof buffer);

                // Read and verify response.
                int DATA_SIZE;
                ASSERT(sizeof(int) == channel.read(buffer, sizeof buffer));
                bdlxxxx::ByteStreamImpUtil::getInt32(&DATA_SIZE, buffer);
                ASSERT(0 < DATA_SIZE);
                ...
                for (int j = 0; j < DATA_SIZE; ++j) {
                    // Initiate next test.
                    bdlxxxx::ByteStreamImpUtil::putInt32(buffer, j);
                    channel.write(buffer, sizeof buffer);
      ...
      case 1: {
        ...
        //   'btlmt::ChannelOutStreamBuf' class.  We want to ensure that channel
        ...
        //
        // Plan:
        //   Create a modifiable 'btlmt::ChannelOutStreamBuf' object 'mX', and a
      ...
      case -1: {
        ...
        bdlma::TestAllocator ta(veryVeryVerbose);
        {
            ...
            for (int i = 0; i < DATA_SIZE; ++i) {
                ...
                bsls::TimeInterval interval(ATTEMPT_INTERVAL);
                ...
                bdlsb::FixedMemOutStreamBuf  outbuf(chain->buffer(0),
                                                   chain->bufferSize());
                bdlxxxx::ByteOutStreamFormatter bos(&outbuf);
                bdlmxxx::List                   connect;
                ...
                btlmt::DataMsg msg(chain, &factory, channels[i]);
