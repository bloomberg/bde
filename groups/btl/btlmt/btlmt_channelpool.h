...
class ChannelPool {
  ...
  public:
    ...
    enum ConnectResolutionMode {
        ...
        // attempt in 'connect'.
        //
        // DEPRECATED:  This 'enum' is deprecated and superseded by
        // 'btlmt::ResolutionMode'.  Additionally, the 'connect' methods taking
        // this 'enum' as argument have been replaced by a 'connect' overload
        // taking 'btlmt::ConnectOptions'.  That method should be used instead.

        e_RESOLVE_ONCE              = ResolutionMode::e_RESOLVE_ONCE,
                                            // perform resolution once prior
                                            // to the first connect attempt

        e_RESOLVE_AT_EACH_ATTEMPT   = ResolutionMode::e_RESOLVE_EACH_TIME
                                            // perform resolution prior to each
                                            // connect attempt
    ...
    enum KeepHalfOpenMode {
        ...
        //
        // DEPRECATED:  This 'enum' is deprecated.  Additionally, the methods
        // taking this 'enum' as argument have been replaced by overloads
        // taking 'btlmt::ConnectOptions' or 'btlmt::ListenOptions'.  Those
  ...
  public:
    ...
    int listen(const btlso::IPv4Address&   endpoint,
        ...
        // if there is already a listening socket associated with 'serverId'
        // (i.e., 'serverId' is not unique) and a negative value if an error
        // occurred.  If 'isRunning' is 'false', this method will return
        // success, but the channel pool will not actively begin listening for
        // a connection until 'isRunning' becomes 'true'.  Every time a
    ...
    int connect(const char                 *hostname,
        ...
        // a positive value if there is already an active connection attempt
        // with the same 'sourceId' (in which case this connection attempt may
        // be retried after that other connection either succeeds, fails, or
        // times out), or a negative value if an error occurred.  If
        // 'isRunning' is 'false', this method will return success, but the
        // channel pool will not actively try connecting to the peer until
        // 'isRunning' becomes 'true'.  The behavior is undefined unless
        ...
        // generated, one for each 'interval'.  Also note that no callbacks
        ...
        // not overlap.  Finally, note that the lifetime of the 'hostname' need
        // not extend past the return of this function call, that is,
        // 'hostname' need not remain valid until the last connection attempt
        // but can be deleted upon return.
        //
    ...
    int connect(const btlso::IPv4Address&   serverAddress,
        ...
        // channel pool will not actively try connecting to the peer until
        // 'isRunning' becomes 'true'.  Return 0 on successful initiation, a
        // positive value if there is an active connection attempt with the
        // same 'sourceId' (in which case this connection attempt may be
        // retried after that other connection either succeeds, fails, or
        // times out), or a negative value if an error occurred.  The behavior
        // is undefined unless '0 < numAttempts', and either '0 < interval' or
        ...
        // note that no callbacks will be invoked if the return value is
        // non-zero.  Also note that the same 'sourceId' can be used in several
        // calls to 'connect' or 'import' as long as two calls to connect with
        // the same 'sourceId' do not overlap.
        //
        // DEPRECATED: Use the 'connect' overload taking a
        // 'btlmt::ConnectOptions' argument instead.
    ...
    int import(bslma::ManagedPtr<btlso::StreamSocket<btlso::IPv4Address> >
        ...
        //
        // DEPRECATED: Use the 'connect' overload taking a boolean flag to
        // specify if half open connections are allowed instead.
