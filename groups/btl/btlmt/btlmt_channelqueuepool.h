// btlmt_channelqueuepool.h           -*-C++-*-
#ifndef INCLUDED_BTLMT_CHANNELQUEUEPOOL
#define INCLUDED_BTLMT_CHANNELQUEUEPOOL
...
//
//@SEE_ALSO: btlmt_message btlmt_channelpool btlmt_channelpoolconfiguration
//
//@CLASSES:
//   btlmt::ChannelQueuePool: queue-based message transport.
//
...
// supplied at construction (see the 'btlmt_channelpool' component for
...
///Data Messages
///- - - - - - -
// The 'btlmt::Message' class provides two similar means of encoding data
// messages, the 'btlmt::DataMsg' type (built around 'bdlmca::PooledBufferChain')
// and the 'btlmt::BlobMsg' type (built around 'bdlmca::Blob').  Both
// 'btlmt::DataMsg' and 'btlmt::Blob' provide a means of combining individual
// memory buffers into a complete data message.
//
// A 'btlmt::ChannelPoolQueue' will enqueue data messages *only* as
// 'btlmt::DataMsg' objects on the _incoming_ queue, but the client can enqueue
// both 'btlmt::DataMsg' and 'btlmt::BlobMsg' objects on the _outgoing_ queue.
// Both message types consist of a shared-pointer to the data (either a
// 'bdlmca::PooledBufferChain' or 'bdlmca::Blob'), along with the data length, and
// the source channel ID.
//
// A client implicitly adopts ownership of the underlying
// 'bdlmca::PooledBufferChain' when it removes a 'btlmt::DataMsg' from the
// incoming queue.  When the client destroys the last copy of the incoming
// 'btlmt::DataMsg' object (i.e., the reference count of the contained shared
...
// by the 'btlmt::ChannelQueuePool' and will be deallocated automatically (using
...
//
///btlmt::DataMsg' Objects on the Incoming Queue
///- - - - - - - - - - - - - - - - - - - - - -
// One potentially confusing aspect of this component is that data messages (of
// type 'btlmt::DataMsg') read from the channel queue pool's incoming message
// queue *may* *not* start at the first byte of the contained
// 'bdlmca::PooledBufferChain' and *may* *not* be of length
// 'bdlmca::PooledBufferChain::length()'.  Instead, the channel queue pool
// encodes the offset of the beginning of the message, and the actual length of
// the message, in the user data fields of the 'btlmt::DataMsg' object.  Each
// message actually begins at the 'btlmt::DataMsg::userDataField1()' byte of the
...
// 'minIncomingMessageSize()' value in the 'btlmt::ChannelPoolConfiguration'
...
// 'btlmt::ChannelPoolConfiguration' suplied to the channel queue pool on
...
// 'btlmt::DataMsg') read from the channel queue pool's incoming message queue
...
// the 'bdlmca::PooledBufferChain' and are the length of the buffer chain.  It
// also means that the 'btlmt::DataMsg' objects enqueued on the incoming queue
...
// is enqueued, those messages are enqueued in a single 'btlmt::DataMsg' object
...
//
//      bdlmca::PooledBlobBufferFactory factory(1024);
//      bdlcc::Queue<btlmt::Message> incoming, outgoing;
//      btlmt::ChannelQueuePool qp(&incoming, &outgoing, cb, config);
//
...
//
//      while(1) {
//          btlmt::Message msg = incoming.popFront();
//          switch(msg.type()) {
...
// all, and uses a new capability of 'btlmt_channelqueuepool' to handle data
// messages of the 'btlmt::BlobMsg' type.
//..
//          }
//      }
//  }
...
namespace btlmt {class Message;
                        ...
                        // ============================
                        // class ChannelQueuePool
                        // ============================

class ChannelQueuePool {
    // This class provides a queue-based message transport similar to a channel
    // pool (see the component 'btlmt_channelpool').  This channel queue pool
    ...
    // configured at construction by passing a 'ChannelPoolConfiguration'
    // object.

  public:
    typedef bdlf::Function<void (*)(int *, int *, const char *, int)>
        ...
        // queue pool will enqueue a 'DataMsg' containing the first
  ...
  private:
    // DATA
    ChannelPool                     *d_channelPool_p;
    ChannelPool::PoolStateChangeCallback
                                           d_poolCbFunctor;
    ChannelPool::ChannelStateChangeCallback
                                           d_channelCbFunctor;
    ChannelPool::DataReadCallback    d_dataCbFunctor;

    bdlcc::Queue<Message>             *d_incomingQueue_p;
    bdlcc::Queue<Message>             *d_outgoingQueue_p;
  ...
  private:
    // NOT IMPLEMENTED
    ChannelQueuePool(const ChannelQueuePool& original);
    ChannelQueuePool& operator=(const ChannelQueuePool& rhs);
  ...
  public:
    ...
    int connect(const btlso::IPv4Address& server,
                int                      numAttempts,
    ...
    int shutdown(int channelId, ChannelPool::ShutdownMode mode);
    ...
    int registerClock(const bsls::TimeInterval& startTime,
    ...
    void getLocalAddress(btlso::IPv4Address *result, int channelId);
    ...
    void getPeerAddress(btlso::IPv4Address *result, int channelId);
        // Load, into the specified 'result', the complete IP address
    ...
    const btlso::IPv4Address *serverAddress(int id) const;
...
// MANIPULATORS
inline
int ChannelQueuePool::close(int serverId)
{
    return d_channelPool_p->close(serverId);
}

inline
int ChannelQueuePool::listen(int port, int backlog,
...
inline
int ChannelQueuePool::listen(int port, int backlog, int serverId,
                                   const bsls::TimeInterval& timeout,
                                   int reuseAddress)
{
...
inline
int ChannelQueuePool::listen(const btlso::IPv4Address& endpoint,
...
inline
int ChannelQueuePool::connect(const btlso::IPv4Address& server,
                                    int numAttempts,
                                    const bsls::TimeInterval& interval, int id)
{
    return d_channelPool_p->connect(server, numAttempts, interval, id);
}

inline
int ChannelQueuePool::shutdown(int channelId,
                                     ChannelPool::ShutdownMode mode) {
    return d_channelPool_p->shutdown(channelId, mode);
}

inline void ChannelQueuePool::deregisterClock(int clockId)
{
    d_channelPool_p->deregisterClock(clockId);
}

inline
int ChannelQueuePool::getServerSocketOption(int *result, int option,
                                                  int level,
                                                  int serverId)
{
...
inline
int ChannelQueuePool::getSocketOption(int *result, int option,
...
inline
int ChannelQueuePool::getLingerOption(
...
inline
int ChannelQueuePool::setSocketOption(int option, int level, int value,
                                            int channelId)
{
...
inline
int ChannelQueuePool::setServerSocketOption(int option, int level,
...
inline
int ChannelQueuePool::setLingerOption(
        const btlso::SocketOptUtil::LingerData& value, int channelId)
{
...
// ACCESSORS
inline
int ChannelQueuePool::busyMetrics() const
{
    return d_channelPool_p->busyMetrics();
}

inline
int ChannelQueuePool::numChannels() const
{
    return d_channelPool_p->numChannels();
}

inline
const btlso::IPv4Address *ChannelQueuePool::serverAddress(int id) const
{
    return d_channelPool_p->serverAddress(id);
}

inline
void ChannelQueuePool::getLocalAddress(btlso::IPv4Address *result,
                                             int                channelId)
{
...
inline
void ChannelQueuePool::getPeerAddress(btlso::IPv4Address *result,
                                            int                channelId)
{
