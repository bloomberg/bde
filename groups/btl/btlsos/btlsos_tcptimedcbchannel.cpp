...
enum {
    RCALLBACK_SIZE  = sizeof(btlsc::TimedCbChannel::ReadCallback),
    BRCALLBACK_SIZE = sizeof(btlsc::TimedCbChannel::BufferedReadCallback),
    WCALLBACK_SIZE  = sizeof(btlsc::TimedCbChannel::WriteCallback),
...
public:
    ...
    // CREATORS
    TcpTimedCbChannel_RReg(char *buffer, int length,
                                 int numSysCalls,
...
public:
    ...
    // CREATORS
    TcpTimedCbChannel_WReg(const char *buffer, int length,
                                  int numSysCalls,
...
void TcpTimedCbChannel::bufferedReadCb()
{
    ...
    if (0 < s) {
        ...
        BSLS_ASSERT(s < 0);
        BSLS_ASSERT(s != btlso::SocketHandle::BTESO_ERROR_INTERRUPTED);
        BSLS_ASSERT(s != btlso::SocketHandle::BTESO_ERROR_WOULDBLOCK);
    ...
    while (1) {
        switch(d_currentReadRequest_p->d_category) {
          case TcpTimedCbChannel_RReg::NON_BUFFERED:
          {
              ...
              else if (s == btlso::SocketHandle::BTESO_ERROR_WOULDBLOCK) {
                                     ...
                                     int                   flags)
{
    ...
    if (0 == d_writeRequests.size()) {
        ...
        if (s == numBytes) {
        ...
        else if (0 < s || btlso::SocketHandle::BTESO_ERROR_WOULDBLOCK == s) {
            ...
            if (flags & btesc_Flag::BTESC_ASYNC_INTERRUPT) {
                ...
                TcpTimedCbChannel_WReg *request =
                    new (d_wrequestPool)
                                TcpTimedCbChannel_WReg(numBytes,
                                                              numBytes,
                                                              writeCallback,
                                                              flags);
...
int TcpTimedCbChannel::bufferedWritev(
        const btls::Ovec      *buffers,
        int                   numBuffers,
        const WriteCallback&  writeCallback,
        int                   flags)
...
void TcpTimedCbChannel::setReadEventManager(
        btlso::TimerEventManager *manager)
{
    BSLS_ASSERT(manager);
...
void TcpTimedCbChannel::setWriteEventManager(
        btlso::TimerEventManager *manager)
{
    BSLS_ASSERT(manager);
