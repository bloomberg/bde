...
// [ 6] btlsc::Channel *timedAllocate()
// [ 7] btlsc::Channel *timedAllocateTimed()
// [ 8] int open();
// [ 8] virtual void invalidate()
// [ 8] int setOption();
// [ 8] int getOption();
// [ 9] int close();
// [ 9] void deallocate();
    ...
    case 'A': {  // an "ACCEPTOR" request
        if (e_CHANNEL == command->d_channelType) {
            if (INFINITED == command->d_timeout) {
                ...
                *newChannel = acceptor->timedAllocate(status,
            ...
            if (INFINITED == command->d_timeout) {
                ...
                *newChannel = acceptor->timedAllocateTimed(status,
                        *command->d_timeout + bdlt::CurrentTime::now(),
                        command->d_interruptFlags);
            }
        }
    ...
    case 'D': {
        if (channels->size()) {
            acceptor->deallocate((*channels)[0]); // This is hard-coded, just
                                                  // to remove a channel.
            channels[0].erase(channels->begin());
        }
        else {
            rCode = -1;
        }
...
// ===================>
      ...
      case 7: {
          ...
          //     2. return the expected 'status' value and the 'null' channel
          //        when 'timeout' is reached for the allocate" function .
          //     3. keep trying to accept a new connection until one is
          //        established within the range of timeout requirement.
          //     4. keep trying to accept a new connection until one is
          ...
          //     5. establish a channel of the expected type, i.e., the new
          //        established channel should be of type
          //        'btlsos::TcpTimedChannel';
          //     6. establish new channels after the previous request is
          //        interrupted by an "asynchronous event";
          //     7. establish multiple channels;
          //     8. establish new channels after a channel is deallocated,
          //        and the other channel is not affected;
          //     9. work fine after calls to other "allocate" functions;
          //    10. not establish any new channels after the 'acceptor' is
          //        closed;
          //    11. establish new channels after the 'acceptor' is opened
          //        again successfully;
          //    12. return the expected 'status' value and the 'null' channel
          //        when an error occurs;
          //    13. not establish any new channels after the 'acceptor' is
          //        invalidated;
          //
          // Plan:
          //   After a 'btlsos::TcpTimedAcceptor' object 'acceptor' is
          //   created, four steps are involved to test this function:
          //   Note that concerns about the occurrences of "asynchronous event"
          //   could be hard to simulate because in each timed "allocate"
          //   function the server socket is set to "non-blocking" mode,
          //   and so concern 1, 4, 6 are not tested in this case.
          //   Also note that the simulation of "hard" errors (concern 12) is
          //   impossible for the test because of the existence of assert()
          //   call in the implementation of timed "allocate" functions.
          //
          //   Step 1: (for concern 1 - 10)
          //     Build a set 'DATA' of 'TestCommand', where each entry
          //     specifies: (1) which function to be invoked; (2) what
          //     arguments to be passed for this call and (3) the results
          //     expected to be returned.  A thread is created to act as a
          //     client for the test.  The client will submit the expected
          //     number of connection requests and/or generate signals
          //     to the main thread if specified.
          //
          //   Step 2: (for concern 11)
...

          {
              ...
              for (int i = 0; i < NUM_VALUES; i++) { // different acceptors
                  ...
                  int existing = 0;
                  if (verbose) {
                      QT("Step 1 for testing 'timedAllocateTimed' method:");
                      QT("===============================================");
                  }
...
// ===================>
                  ...
                  if (verbose) {
                      QT("Step 2 for testing 'timedAllocateTimed' method:");
                      QT("===============================================");
                  }
...
// ===================>
                  ...
                  if (verbose) {
                      QT("Step 3 for testing 'timedAllocateTimed' method:");
                      QT("===============================================");
                  }
...
// ===================>
                      ...
                      int ret = bslmt::ThreadUtil::create(&threadHandle,
                                                         ...
                                                         server);
                      ASSERT(0 == ret);
                      bsls::TimeInterval time = *DATA.d_timeout +
                  ...
                  if (verbose) {
                      QT("Step 4 for testing 'timedAllocateTimed' method:");
                      QT("===============================================");
                  }
...
// ===================>
      ...
      case 6: {
          ...
          //     client for the test.  The client will submit the expected
          //     number of connection requests and/or generate signals
          //     to the main thread if specified.  Note that concerns about
          //     the occurrences of "asynchronous event" could be hard to
          //     simulate because in each timed "allocate" function the
          //     server socket is set to "non-blocking" mode, and so
          //     concern 1, 4, 6 are not tested in this case.
          //
          //   Step 2: (for concern 11)
          //     Reopen the 'acceptor' with a valid server address.  Build a
          //     set 'DATA' of 'TestCommand', where each entry will issue a
...

          {
              ...
              for (int i = 0; i < NUM_VALUES; i++) { // different acceptors
                  ...
                  if (verbose) {
                      QT("Step 1 for testing 'timedAllocate' method:");
                      QT("==========================================");
                  }
...
// ===================>
                  ...
                  if (verbose) {
                      QT("Step 2 for testing 'timedAllocate' method:");
                      QT("==========================================");
                  }
...
// ===================>
                  ...
                  if (verbose) {
                      QT("Step 4 for testing 'timedAllocate' method:");
                      QT("==========================================");
                  }
                  {
                      // Close the old server socket to deallocate it.
...
// ===============>
      ...
      case 2: {
          ...
          //   can create different 'btlsos::TcpTimedAcceptor' objects
          //   successfully, based on different parameters; (2) if can
          //   destructor any 'btlsos::TcpTimedAcceptor' object successfully.
          // Plan:
          //   Build a set of data 'VALUES' to construct different
          //   'btlsos::TcpTimedAcceptor' objects.  With each objects,
          //   establish a specified number of channels, verify the result
          //   from each channel operation.
          // Testing:
          //   virtual btlsos::TcpTimedAcceptor();
          //   virtual btlsos::TcpTimedAcceptor(..., int initialCapacity, ...);
          //   virtual ~btlsos::TcpTimedAcceptor();
          // ----------------------------------------------------------------
          if (verbose) {
