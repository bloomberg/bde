    ...
    if (channel) {
        ...
        bdlf::Function<void (*)(int, int)> callback(
                bdlf::BindUtil::bind(
                  ...
                  , channel
                  , 0));
        if (channel->timedBufferedWrite(d_controlBuffer, k_BUFFER_SIZE,
...
class my_DataStream {
  ...
  private:
    ...
    public:
      // CREATORS
      my_DataStream(btlso::StreamSocketFactory<btlso::IPv4Address> *factory,
                      btlso::TimerEventManager                *manager,
...
void my_DataStream::allocateCb(btlsc::TimedCbChannel *channel, int status,
                               btlsc::TimedCbChannel **cachedChannel) {
...
int main(int argc, char *argv[])
{
    ...
    switch (test) {case 0:
      case -4: {
        ...
        bsl::vector<bdlqq::ThreadUtil::Handle> threadHandles(numThreads);

        for (int i = 0; i < numThreads; ++i) {
            LOOP_ASSERT(i, 0 == bdlqq::ThreadUtil::create(&threadHandles[i],
                                                      echoClientThread,
                                                      &state));
        ...
        for (int i = 0; i < numThreads; ++i) {
            LOOP_ASSERT(i, 0 == bdlqq::ThreadUtil::join(threadHandles[i]))
