...
typedef btlsos::TcpTimedConnector Obj;
    ...
    // operations.
{
    ...
    if (verbose) {
        P_(info.d_serverSocket_p->handle());
    ...
    case 'A': {  // an "ACCEPTOR" request
        if (e_CHANNEL == command->d_channelType) {
            if (INFINITED == command->d_timeout) {
                ...
                *newChannel = connector->timedAllocate( status,
            ...
            if (INFINITED == command->d_timeout) {
                ...
                *newChannel = connector->timedAllocateTimed(status,
                               *command->d_timeout + bdlt::CurrentTime::now(),
                               command->d_interruptFlags);
            }
        }
...
//..
      ...
      case 8: {
          ...
          // ----------------------------------------------------------------
          if (verbose) {
              QT("Testing MANIPULATOR methods");
              QT("===========================");
          }
...
// ===================>
                     ...
                     if (veryVerbose) {
                         P(channels.size()); P(connector.numChannels());
                     }
...
// TBD FIX ME
          ...
          //     6. return the expected 'status' value and the 'null' channel
          //        when 'timeout' is reached for the allocate" function .
          //     7. keep trying to establish a new connection until one is
          ...
          //     8. establish new channels after the previous request is
          //        interrupted by an "asynchronous event";
          //     9. return the expected 'status' value and the 'null' channel
          //        when an error occurs;
          //    10. not establish any new channels after the 'connector' is
          //        invalidated;
          //
          // Plan:
          //   After a 'btlsos::TcpTimedConnector' object 'connector' is
          ...
          //
          //   Step 3: (for concern 8)
          //     Reset the 'connector's server address to a valid one, try
          //     establishing the expected number of connections by
          //     building a set 'DATA' of 'TestCommand', where each
          //     entry will issue a request to establish a connection.
          //     A thread will be created as the server to establish a
          //     connection while no connection is expected to be established.
          //
          //   Step 4: (for concern 9)
...
// ===================>
                  ...
                  if (verbose) {
                      QT("Step 2 testing 'timedAllocateTimed' method:");
                      QT("===========================================");
                  }
                  {
...
// ===================>
                  ...
                  if (verbose) {
                      QT("Step 4 testing 'timedAllocateTimed' method:");
                      QT("===========================================");
                  }
                  {
                      factory.deallocate(serverSocket);
...
// ===================>
                  ...
                  if (verbose)  {
                      QT("Step 5 testing 'timedAllocateTimed' method:");
                      QT("===========================================");
                  }
                  {
                      factory.deallocate(serverSocket);
...
// ===================>
                  ...
                  if (verbose) {
                      QT("Step 5 for testing 'timedAllocate' method:");
                      QT("==========================================");
                  }
                  {
                      factory.deallocate(serverSocket);
...
// ===================>
                  ...
                  if (verbose) {
                      QT("Step 5 for testing 'allocateTimed' method:");
                      QT("==========================================");
                  }
...
// ===============>
      ...
      case 2: {
          ...
          //   can create different 'btlsos::TcpTimedConnector' objects
          //   successfully, based on different parameters; (2) if can
          //   destructor any 'btlsos::TcpTimedConnector' object successfully.
          // Plan:
          //   Build a set of data 'VALUES' to construct different
          //   'btlsos::TcpTimedConnector' objects.  With each objects,
