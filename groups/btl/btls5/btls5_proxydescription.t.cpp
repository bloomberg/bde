...
// [11] btls5::ProxyDescription(address, *a = 0);
// [ 3] btls5::ProxyDescription(address, credentials, *a = 0);
// [ 7] btls5::ProxyDescription(original, *a = 0);
// [ 2] ~btls5::ProxyDescription();
//
// MANIPULATORS
// [ 9] operator=(rhs) = default;
// [ 2] void setAddress(const btlso::Endpoint& value);
// [ 2] void setCredentials(const btls5::Credentials& value);
// [ 8] void swap(ProxyDescription& other);
//
// ACCESSORS
// [ 4] const btlso::Endpoint& address() const;
// [ 4] const btls5::Credentials& credentials() const;
...
//
// FREE FUNCTIONS
// [ 8] void swap(ProxyDescription& a, ProxyDescription& b);
//-----------------------------------------------------------------------------
// [ 1] BREATHING TEST
// [12] USAGE EXAMPLE
...
typedef btls5::ProxyDescription Obj;
...
{
    //LINE  MEM             HNAME      PORT       UNAME        PASSWD
    //----  ---             -----      ----       -----        ------
    ...
    // 'endpoint'
    { L_,   '?',              "a",       1,    "uname1",     "passwd1"    },
    { L_,   '?',             "AB",      16,    "uname2",     "passwd2"    },
    { L_,   '?',     "1234567890",     255,    "uname3",     "passwd3"    },
    { L_,   'Y',      LONG_STRING,    9999,    "uname4",     "passwd4"    },
    { L_,   'Y',    LONGER_STRING,   32767,    "uname5",     "passwd5"    },
    { L_,   'Y',   LONGEST_STRING,   65535,    "uname6",     "passwd6"    },

    // 'credential'

    { L_,   '?',  "hname1",       1,              "a",        "passwd1"   },
    { L_,   '?',  "hname2",      16,             "AB",        "passwd2"   },
    { L_,   '?',  "hname3",     255,     "1234567890",        "passwd3"   },
    { L_,   'Y',  "hname4",    9999,      LONG_STRING,        "passwd4"   },
    { L_,   'Y',  "hname5",   32767,    LONGER_STRING,        "passwd5"   },
    { L_,   'Y',  "hname6",   65535,   LONGEST_STRING,        "passwd6"   },

    { L_,   '?',  "hname1",       1,        "uname1",                 "a"   },
    { L_,   '?',  "hname2",      16,        "uname2",                "AB"   },
    { L_,   '?',  "hname3",     255,        "uname3",        "1234567890"   },
    { L_,   'Y',  "hname4",    9999,        "uname4",         LONG_STRING   },
    { L_,   'Y',  "hname5",   32767,        "uname5",       LONGER_STRING   },
    { L_,   'Y',  "hname6",   65535,        "uname6",      LONGEST_STRING   },
...
//..
      ...
      case 11: {
        ...
        //
        // Testing:
        //   ProxyDescription(const Endpoint& p, *a = 0);
        // --------------------------------------------------------------------
        ...
        static const DefaultDataRow DATA[] =
            {
                ...
                { L_,   '?',             "AB",     16  },
                { L_,   '?',     "1234567890",    255  },
                { L_,   'Y',      LONG_STRING,   9999  },
                { L_,   'Y',    LONGER_STRING,  32767  },
                { L_,   'Y',   LONGEST_STRING,  65535  },
            };
             ...
             "\nCreate objects with various allocator configurations." << endl;
        {
            ...
            for (int ti = 0; ti < NUM_DATA; ++ti) {
                ...
                const char *const HNAME  = DATA[ti].d_hostname;
                const int         PORT   = DATA[ti].d_portNum;
                const Endpoint    ADDR(HNAME, PORT);

                if (veryVerbose) { T_ P_(MEM) P_(ADDR) }
                ...
                for (char cfg = 'a'; cfg <= 'c'; ++cfg) {
                    ...
                    switch (CONFIG) {
                      case 'a': {
                        objPtr = new (fa) Obj(ADDR);
                        objAllocatorPtr = &da;
                      } break;
                      case 'b': {
                        objPtr = new (fa) Obj(ADDR, 0);
                        objAllocatorPtr = &da;
                      } break;
                      case 'c': {
                        objPtr = new (fa) Obj(ADDR, &sa);
                        objAllocatorPtr = &sa;
                      } break;
                      default: {
                    ...
                    LOOP4_ASSERT(LINE, CONFIG, CREDS, X.credentials(),
                                 CREDS == X.credentials());
                    ...
                    LOOP2_ASSERT(LINE, CONFIG,
                                &oa == X.credentials().username().allocator());
        ...
        if (verbose) cout << "\nTesting with injected exceptions." << endl;
        {
            ...
            for (int ti = 0; ti < NUM_DATA; ++ti) {
                ...
                if (veryVerbose) { T_ P_(MEM) P(ADDR) }
                ...
                BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(sa) {
                    ...
                    Obj obj(ADDR, &sa);
                    LOOP3_ASSERT(LINE, CREDS, obj.credentials(),
      ...
      case 9: {
        ...
        //
        // Testing:
        //   operator=(const ProxyDescription& rhs);
        // --------------------------------------------------------------------
        ...
        for (int ti = 0; ti < NUM_DATA; ++ti) {
            ...
            const char *const HNAME1  = DATA[ti].d_hostname;
            const int         PORT1   = DATA[ti].d_portNum;
            const char *const UNAME1  = DATA[ti].d_username;
            const char *const PASSWD1 = DATA[ti].d_password;

            bslma::TestAllocator scratch("scratch", veryVeryVeryVerbose);

            const Endpoint    ADDR1(HNAME1, PORT1, &scratch);
            const Credentials CREDS1(UNAME1, PASSWD1, &scratch);

            const Obj  Z(ADDR1, CREDS1, &scratch);
            const Obj ZZ(ADDR1, CREDS1, &scratch);
            ...
            for (int tj = 0; tj < NUM_DATA; ++tj) {
                ...
                const char *const HNAME2  = DATA[tj].d_hostname;
                const int         PORT2   = DATA[tj].d_portNum;
                const char *const UNAME2  = DATA[tj].d_username;
                const char *const PASSWD2 = DATA[tj].d_password;

            const Endpoint    ADDR2(HNAME2, PORT2, &scratch);
            const Credentials CREDS2(UNAME2, PASSWD2, &scratch);

                bslma::TestAllocator oa("object", veryVeryVeryVerbose);

                {
                    Obj mX(ADDR2, CREDS2, &oa);  const Obj& X = mX;
      ...
      case 8: {
        ...
        //   void swap(ProxyDescription& a, b);
        // --------------------------------------------------------------------
        ...
        for (int ti = 0; ti < NUM_DATA; ++ti) {
            ...
            Obj mW(ADDR1, CREDS1, &oa);  const Obj& W = mW;
            ...
            for (int tj = 0; tj < NUM_DATA; ++tj) {
                ...
                const Credentials CREDS2(UNAME2, PASSWD2, &scratch);
                      ...
                      Obj mY(ADDR2, CREDS2, &oa);  const Obj& Y = mY;
                                                                       ...
                                                                       << endl;
        {
            ...
            const char     UNAME[]  = "a_" SUFFICIENTLY_LONG_STRING;
            const char     PASSWD[] = "b_" SUFFICIENTLY_LONG_STRING;
            const char     NAME[]   = "c_" SUFFICIENTLY_LONG_STRING;
            const int      PORT     = 255;
            ...
            const Credentials CREDS(UNAME, PASSWD, &scratch);

                  Obj mX(&oa);  const Obj& X = mX;
            const Obj XX(X, &scratch);

                  Obj mY(ADDR, CREDS, &oa);  const Obj& Y = mY;
            const Obj YY(Y, &scratch);
      ...
      case 7: {
        ...
        //
        // Testing:
        //   ProxyDescription(const ProxyDescription& o, *bA = 0);
        // --------------------------------------------------------------------
             ...
             "\nCreate objects with various allocator configurations." << endl;
        {
            ...
            for (int ti = 0; ti < NUM_DATA; ++ti) {
                ...
                const Endpoint    ADDR(HNAME, PORT);
                const Credentials CREDS(UNAME, PASSWD);

                bslma::TestAllocator scratch("scratch", veryVeryVeryVerbose);
                const Obj  Z(ADDR, CREDS, &scratch);
                const Obj ZZ(ADDR, CREDS, &scratch);
        ...
        if (verbose) cout << "\nTesting with injected exceptions." << endl;
        {
            for (int ti = 0; ti < NUM_DATA; ++ti) {
                ...
                if (veryVerbose) { T_ P_(MEM) P_(CREDS) P(ADDR) }
      ...
      case 6: {
        ...
        //
        // Testing:
        //   bool operator==(const ProxyDescription& lhs, rhs);
        //   bool operator!=(const ProxyDescription& lhs, rhs);
        // --------------------------------------------------------------------
        ...
        typedef const Endpoint    T1;                // 'address'
        typedef const Credentials T2;                // 'credentials'

        // Attribute 1 Values: 'address'

        const T1 A1(SUFFICIENTLY_LONG_STRING, 1);
        const T1 B1(LONG_STRING, 255);

        // Attribute 2 Values: 'address'

        const T2 A2(LONGER_STRING, LONGEST_STRING);
        const T2 B2("username", "password");
        ...
        static const struct {
            ...
            const Credentials  d_credentials;
        } DATA[] = {
        ...
        for (int ti = 0; ti < NUM_DATA; ++ti) {
            ...
            const Credentials&  CREDS1 = DATA[ti].d_credentials;

            if (veryVerbose) { T_ P_(LINE1) P_(CREDS1) P(ADDR1) }

            // Ensure an object compares correctly with itself (alias test).
            {
                bslma::TestAllocator scratch("scratch", veryVeryVeryVerbose);

                const Obj X(ADDR1, CREDS1, &scratch);
            ...
            for (int tj = 0; tj < NUM_DATA; ++tj) {
                ...
                const Credentials&  CREDS2  = DATA[tj].d_credentials;

                if (veryVerbose) { T_ T_ P_(LINE2) P_(CREDS2) P(ADDR2) }
                ...
                for (char cfg = 'a'; cfg <= 'b'; ++cfg) {
                    ...
                    const Obj X(ADDR1, CREDS1, &xa);
                    const Obj Y(ADDR2, CREDS2, &ya);
      ...
      case 5: {
        ...
        //   operator<<(ostream& output, const ProxyDescription& object);
        // --------------------------------------------------------------------
...
#define SP " "
        ...
        //LINE L SPL  UNAME PASSWD  HNAME PORT  EXP
        //---- - ---  ----- ------  ----- ----  ---
...
#undef SP
                          ...
                          << endl;
        {
            for (int ti = 0; ti < NUM_DATA; ++ti) {
                ...
                if (veryVerbose) { T_ P_(CREDS) P(ADDR) }

                if (veryVeryVerbose) { T_ T_ Q(EXPECTED) cout << EXP; }

                const Obj X(ADDR, CREDS);
      ...
      case 4: {
        ...
        typedef Endpoint    T1;     // 'address'
        typedef Credentials T2;     // 'credentials'
                ...
                "\nVerify all basic accessors report expected values." << endl;
        {
            ...
            const T2& credentials = X.credentials();
            LOOP2_ASSERT(D2, credentials, D2 == credentials);
        ...
        if (veryVerbose) { T_ Q(credentials) }
        {
            mX.setCredentials(A2);

            bslma::TestAllocatorMonitor oam(&oa), dam(&da);

            const T2& credentials = X.credentials();
            LOOP2_ASSERT(A2, credentials, A2 == credentials);
             ...
             "\nCreate objects with various allocator configurations." << endl;
        {
            ...
            for (int ti = 0; ti < NUM_DATA; ++ti) {
                ...
                for (char cfg = 'a'; cfg <= 'c'; ++cfg) {
                    ...
                    switch (CONFIG) {
                      case 'a': {
                          objPtr = new (fa) Obj(ADDR, CREDS);
                        objAllocatorPtr = &da;
                      } break;
                      case 'b': {
                        objPtr = new (fa) Obj(ADDR, CREDS, 0);
                        objAllocatorPtr = &da;
                      } break;
                      case 'c': {
                        objPtr = new (fa) Obj(ADDR, CREDS, &sa);
                        objAllocatorPtr = &sa;
                      } break;
                      default: {
                        LOOP2_ASSERT(LINE, CONFIG, !"Bad allocator config.");
        ...
        if (verbose) cout << "\nTesting with injected exceptions." << endl;
        {
            ...
            for (int ti = 0; ti < NUM_DATA; ++ti) {
                ...
                const Endpoint    ADDR(DATA[ti].d_hostname,
                                       DATA[ti].d_portNum,
                                       &scratch);
                const Credentials CREDS(DATA[ti].d_username,
                                        DATA[ti].d_password,
                                        &scratch);
      ...
      case 2: {
        ...
        //
        // Testing:
        //   explicit ProxyDescription(bslma::Allocator *basicAllocator = 0);
        //   ~ProxyDescription() = default;
        //   void setAddress(const Endpoint& address);
        ...
        for (char cfg = 'a'; cfg <= 'c'; ++cfg) {
            ...
            LOOP_ASSERT(CONFIG, &oa == X.credentials().username().allocator());
            ...
            LOOP3_ASSERT(CONFIG, D1, X.address(), D1 == X.address());
            LOOP3_ASSERT(CONFIG, D2, X.credentials(), D2 == X.credentials());
            ...
            // 'endpoint'
            {
                ...
                LOOP_ASSERT(CONFIG, D2 == X.credentials());
            ...
            // 'credentials'
            {
                ...
                LOOP_ASSERT(CONFIG, D1 == X.address());
                LOOP_ASSERT(CONFIG, A2 == X.credentials());

                bslma::TestAllocatorMonitor tam(&oa);

                mX.setCredentials(B2);
                LOOP_ASSERT(CONFIG, D1 == X.address());
                LOOP_ASSERT(CONFIG, B2 == X.credentials());

                mX.setCredentials(D2);
                LOOP_ASSERT(CONFIG, D1 == X.address());
                LOOP_ASSERT(CONFIG, D2 == X.credentials());
      ...
      case 1: {
        ...
        Endpoint address1("localhost", 8194);
        Obj proxy1(address1);
        ASSERT(proxy1.address() == address1);
        ASSERT(proxy1.credentials().username().empty());
        verbose && cout << "proxy1=" << proxy1 << endl;

        Endpoint address2("example.com", 80);
        Credentials credentials2("john.smith", "pass2");
        Obj proxy2(address2, credentials2);
        ...
        string password("A fairly long password"
                        " to avoid short-string optimization"
                        " and cause a likely memory allocation");
        Credentials credentials3(username, password);
