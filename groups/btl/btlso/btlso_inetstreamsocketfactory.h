...
// type 'btlso::InetStreamSocket<ADDRESS>' conforming to the
...
// Therefore, the template parameter will always be 'btlso::IPv4Address'.  The
// factory, 'btlso::InetStreamSocketFactory<ADDRESS>', creates and destroys
// instances of the 'btlso::InetStreamSocket<ADDRESS>'.  Two interfaces are
...
// instance of 'btlso::InetStreamSocket<ADDRESS>' must be destroyed using the
// deallocate operation of 'btlso::InetStreamSocketFactory<ADDRESS>'.
//
...
// 'btlso::InetStreamSocket' is not *thread-enabled* (it is not safe to invoke
// methods from different threads on the same instance).
// 'btlso::InetStreamSocket' is *thread-safe*, meaning that distinct threads can
...
class InetStreamSocketFactory : public StreamSocketFactory<ADDRESS>
{
  ...
  public:
    // CREATORS
    explicit InetStreamSocketFactory(
    ...
    virtual StreamSocket<ADDRESS> *allocate();
        ...
        // 'InetStreamSocket<ADDRESS>'.  Return the address of the
        // newly-created socket on success, and 0 otherwise.

    virtual StreamSocket<ADDRESS> *allocate(
                                            SocketHandle::Handle handle);
        // Create a stream-based socket of type
        // 'InetStreamSocket<ADDRESS>' attached to the specified socket
...
class InetStreamSocket : public StreamSocket<ADDRESS> {
    // This class implements the 'StreamSocket<ADDRESS>' protocol to
  ...
  private:
    ...
    InetStreamSocket<ADDRESS>& operator=(
                                       const InetStreamSocket<ADDRESS>&);

    // FRIENDS
    friend class InetStreamSocketFactory<ADDRESS>;
  ...
  public:
    ...
    virtual int accept(StreamSocket<ADDRESS> **socket);
        ...
        // stream socket of type 'InetStreamSocket<ADDRESS>' into the
        ...
        // 'SocketHandle::BTESO_ERROR_WOULDBLOCK'.  Note that this socket
    ...
    virtual int read(char *buffer, int length);
        ...
        // 'SocketHandle::BTESO_ERROR_WOULDBLOCK' if zero bytes were
        // immediately available.  If the connection has been closed and there
        // is no data available, 'SocketHandle::BTESO_ERROR_EOF' is
    ...
    virtual int waitForIO(bteso_Flag::IOWaitType   type,
        ...
        // value of 'SocketHandle::BTESO_ERROR_TIMEDOUT' is returned.  If
        // this call is interrupted,
        // 'SocketHandle::BTESO_ERROR_INTERRUPTED' is returned.
    ...
    virtual int connectionStatus() const;
        ...
        // 'SocketHandle::BTESO_ERROR_CONNDEAD' is returned.  A value of
        // 'SocketHandle::BTESO_ERROR_CONNDEAD' is also returned if a
    ...
    virtual int socketOption(int *result, int level, int option) const;
        ...
        // Return 0 on success and a non-zero value otherwise.
                ...
                // ============================================
                // class InetStreamSocket_AutoCloseSocket
                // ============================================
...
class InetStreamSocket_AutoCloseSocket {
    ...
    // we can't use a 'StreamSocketFactoryAutoDeallocateGuard' since
  ...
  private:
    // NOT IMPLEMENTED
    InetStreamSocket_AutoCloseSocket(
                                const InetStreamSocket_AutoCloseSocket&);
    InetStreamSocket_AutoCloseSocket& operator=(
                                const InetStreamSocket_AutoCloseSocket&);
  public:
    // CREATORS
    explicit InetStreamSocket_AutoCloseSocket(
...
// MANIPULATORS
template <class ADDRESS>
int InetStreamSocket<ADDRESS>::accept(
                                          StreamSocket<ADDRESS> **socket)
{
    ...
    InetStreamSocket_AutoCloseSocket autoDeallocate(newHandle);

    *socket = new (*d_allocator_p) InetStreamSocket<ADDRESS>(
                                    ...
                                    ADDRESS                       *peerAddress)
{
    ...
    int ret = SocketImpUtil::accept<ADDRESS>(&newHandle, peerAddress,
                                                   d_handle);
    if (ret != 0) {
        return ret;                                                   // RETURN
    }
...
int InetStreamSocket<ADDRESS>::read(char *buffer, int length)
{
    ...
    if (length > 0 && ret == 0) {
        ret = SocketHandle::BTESO_ERROR_EOF;
    }
                                           ...
                                           int                 numBuffers)
{
    ...
    if (ret == 0) {
        ...
        for (i = 0; i < numBuffers; ++i) {
            if (buffers[i].length()) {
                return SocketHandle::BTESO_ERROR_EOF;           // RETURN
            }
        }
    }
...
int InetStreamSocket<ADDRESS>::writev(const btls::Iovec   *buffers,
                                            int                 numBuffers)
{
    int ret = SocketImpUtil::writev(d_handle,
...
int InetStreamSocket<ADDRESS>::writev(const btls::Ovec   *buffers,
                                            int                numBuffers)
{
    int ret = SocketImpUtil::writev(d_handle, buffers, numBuffers);
                                                 ...
                                                 bteso_Flag::BlockingMode mode)
{
    return IoUtil::setBlockingMode(
                                       ...
                                       ? IoUtil::BTESO_BLOCKING
                                       : IoUtil::BTESO_NONBLOCKING);
}
...
int InetStreamSocket<ADDRESS>::setLingerOption(
                                const SocketOptUtil::LingerData& options)
{
    return SocketOptUtil::setOption(handle(),
          SocketOptUtil::BTESO_SOCKETLEVEL,
          SocketOptUtil::BTESO_LINGER,
          options) ? SocketHandle::BTESO_ERROR_UNCLASSIFIED : 0;
}

template <class ADDRESS>
int InetStreamSocket<ADDRESS>::setOption(int level,
                                               int option,
                                               int value)
{
    return SocketOptUtil::setOption(handle(),
          level, option,
          value) ? SocketHandle::BTESO_ERROR_UNCLASSIFIED : 0;
}
...
int InetStreamSocket<ADDRESS>::shutdown(bteso_Flag::ShutdownType value)
{
    static enum SocketImpUtil::ShutDownType shutDownTypeMapping[] = {
         SocketImpUtil::BTESO_SHUTDOWN_RECEIVE,
         SocketImpUtil::BTESO_SHUTDOWN_SEND,
         SocketImpUtil::BTESO_SHUTDOWN_BOTH
    };
                                              ...
                                              const bsls::TimeInterval& timeout)
{
    ...
    int rc = SocketHandle::BTESO_ERROR_UNCLASSIFIED;
    ...
    if (ret > 0) {
        ...
        rc = SocketHandle::BTESO_ERROR_TIMEDOUT;
    }
    else if (EINTR == errno) {
        rc = SocketHandle::BTESO_ERROR_INTERRUPTED;
    }
    else {
        rc = SocketHandle::BTESO_ERROR_UNCLASSIFIED;
    }
#else
    fd_set writeset;
    fd_set excset;
...
template <class ADDRESS>
int InetStreamSocket<ADDRESS>::waitForAccept(
                                              const bsls::TimeInterval& timeout)
{
                                        ...
                                        bteso_Flag::BlockingMode *result) const
{
    ...
    if (res == 0) {
        *result = (mode == IoUtil::BTESO_BLOCKING)
...
int InetStreamSocket<ADDRESS>::connectionStatus() const
{
    ...
    return SocketHandle::BTESO_ERROR_CONNDEAD;
}
...
{
    return SocketOptUtil::getOption(
                                        result,
                                        handle(),
                                        SocketOptUtil::BTESO_SOCKETLEVEL,
                                        SocketOptUtil::BTESO_LINGER)
           ? SocketHandle::BTESO_ERROR_UNCLASSIFIED
           : 0;
}
...
int InetStreamSocket<ADDRESS>::socketOption(int *result,
                                                  ...
                                                  int  option) const
{
    return SocketOptUtil::getOption(result, handle(), level, option)
           ? SocketHandle::BTESO_ERROR_UNCLASSIFIED
           : 0;
}
...
StreamSocket<ADDRESS> *InetStreamSocketFactory<ADDRESS>::allocate()
{
    ...
    int ret = SocketImpUtil::open<ADDRESS>(
                   &newSocketHandle, SocketImpUtil::BTESO_SOCKET_STREAM);

    if (ret < 0) {
        return 0;                                                     // RETURN
    }

    InetStreamSocket_AutoCloseSocket autoDeallocate(newSocketHandle);

    // On successfully opening a new socket, create a new
    // 'InetStreamSocket' for this socket.

    StreamSocket<ADDRESS> *newStreamSocket =  new (*d_allocator_p)
                               InetStreamSocket<ADDRESS>(newSocketHandle,
                                                               d_allocator_p);
        ...
        SocketHandle::Handle socketHandle)
{
    ...
    return new (*d_allocator_p) InetStreamSocket<ADDRESS>(
        ...
        StreamSocket<ADDRESS> * socket)
{
    ...
    ((InetStreamSocket<ADDRESS> *) socket)->
                                            ~InetStreamSocket<ADDRESS>();
...
inline
InetStreamSocket_AutoCloseSocket::
InetStreamSocket_AutoCloseSocket(SocketHandle::Handle socketHandle)
...
// MANIPULATORS
inline
void InetStreamSocket_AutoCloseSocket::release()
{
    d_valid = 0;
}
